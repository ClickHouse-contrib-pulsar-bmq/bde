...
template <class VECTOR, class ELEMENT_TYPE> struct IsVector {
  static const bool value =
      bsl::is_same<VECTOR, bsl::vector<ELEMENT_TYPE>>::value
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      ||
      bsl::is_same<VECTOR, std::experimental::pmr::vector<ELEMENT_TYPE>>::value
#endif
...
inline void computeSerialMonthAndDay(int *serialMonth, int *day,
...
// valid 'bdlt::Date'.
{
  ...
  int resultDay =
      bsl::min(last, (dayOfFeb == 0 || month != 2) ? day : dayOfFeb);
...
int computeMonthRange(int *startSerialMonth, int *endSerialMonth,
                      int earliestSerialMonth, int latestSerialMonth,
                      int exampleSerialMonth, int intervalInMonths)
...
int adjustMonthRange(int *startSerialMonth, int *endSerialMonth, int startDay,
                     int endDay, int earliestDay, int latestDay,
                     int earliestSerialMonth, int latestSerialMonth,
...
inline void
generateFromDayIntervalImp(VECTOR *schedule, const bdlt::Date &earliest,
                           const bdlt::Date &latest, const bdlt::Date &example,
                           int intervalInDays)
...
// '0 <= targetDayOfFeb <= 29'.
{
  ...
  if (u::computeMonthRange(&startSerialMonth, &endSerialMonth,
                           earliestSerialMonth, latestSerialMonth,
  ...
  int endDay =
      u::getDayOfMonth(U_SERIAL2Y(endSerialMonth), U_SERIAL2M(endSerialMonth),
                       targetDayOfMonth, targetDayOfFeb)
          .day();

  if (u::adjustMonthRange(&startSerialMonth, &endSerialMonth, startDay, endDay,
                          earliestDay, latestDay, earliestSerialMonth,
                          latestSerialMonth, intervalInMonths)) {
    // empty schedule

    return; // RETURN
  }

  schedule->reserve((endSerialMonth - startSerialMonth) / intervalInMonths + 1);

  for (int sm = startSerialMonth; sm <= endSerialMonth;
       sm += intervalInMonths) {
    schedule->push_back(u::getDayOfMonth(U_SERIAL2Y(sm), U_SERIAL2M(sm),
                                         targetDayOfMonth, targetDayOfFeb));
  }
}

template <class VECTOR>
inline void generateFromBusinessDayOfMonthImp(
    VECTOR *schedule, const bdlt::Date &earliest, const bdlt::Date &latest,
    int exampleYear, int exampleMonth, int intervalInMonths,
    const bdlt::Calendar &calendar, int targetBusinessDayOfMonth)
// Load, into the specified 'schedule', the chronologically increasing
...
// '1 <= abs(targetBusinessDayOfMonth) <= 31'.
{
  ...
  BSLS_ASSERT(-31 <= targetBusinessDayOfMonth &&
              31 >= targetBusinessDayOfMonth && 0 != targetBusinessDayOfMonth);
  ...
  int startDay;
  {
    ...
    if (bdlt::CalendarUtil::nthBusinessDayOfMonthOrMaxIfValid(
            &rv, calendar, U_SERIAL2Y(startSerialMonth),
            U_SERIAL2M(startSerialMonth), targetBusinessDayOfMonth)) {
      // empty schedule
  ...
  int endDay;
  {
    ...
    if (bdlt::CalendarUtil::nthBusinessDayOfMonthOrMaxIfValid(
            &rv, calendar, U_SERIAL2Y(endSerialMonth),
            U_SERIAL2M(endSerialMonth), targetBusinessDayOfMonth)) {
      // empty schedule
  ...
  for (int sm = startSerialMonth; sm <= endSerialMonth;
    ...
    bdlt::Date rv;
    if (bdlt::CalendarUtil::nthBusinessDayOfMonthOrMaxIfValid(
            &rv, calendar, U_SERIAL2Y(sm), U_SERIAL2M(sm),
...
template <class VECTOR>
inline void generateFromDayOfWeekAfterDayOfMonthImp(
    VECTOR *schedule, const bdlt::Date &earliest, const bdlt::Date &latest,
    int exampleYear, int exampleMonth, int intervalInMonths,
    bdlt::DayOfWeek::Enum dayOfWeek, int dayOfMonth)
// Load, into the specified 'schedule', the chronologically increasing
...
// '1 <= intervalInMonths', and '1 <= dayOfMonth <= 31'.
{
  ...
  int endDay;
  {
    ...
    if (!bdlt::Date::isValidYearMonthDay(startYear, startMonth, dayOfMonth) ||
        !bdlt::Date::isValidYearMonthDay(endYear, endMonth, dayOfMonth)) {
      // empty schedule
    ...
    startDay = bdlt::DateUtil::nextDayOfWeekInclusive(
                   dayOfWeek, bdlt::Date(startYear, startMonth, dayOfMonth))
                   .day();

    endDay = bdlt::DateUtil::nextDayOfWeekInclusive(
                 dayOfWeek, bdlt::Date(endYear, endMonth, dayOfMonth))
                 .day();
  }
  ...
  for (int sm = startSerialMonth; sm <= endSerialMonth;
    ...
    schedule->push_back(bdlt::DateUtil::nextDayOfWeekInclusive(
        dayOfWeek, bdlt::Date(year, month, dayOfMonth)));
  }
}

template <class VECTOR>
inline void generateFromDayOfWeekInMonthImp(
    VECTOR *schedule, const bdlt::Date &earliest, const bdlt::Date &latest,
    int exampleYear, int exampleMonth, int intervalInMonths,
    bdlt::DayOfWeek::Enum dayOfWeek, int occurrenceWeek)
// Load, into the specified 'schedule', the chronologically increasing
...
// '1 <= occurrenceWeek <= 4'.
{
  ...
  int startDay = bdlt::DateUtil::nthDayOfWeekInMonth(
                     U_SERIAL2Y(startSerialMonth), U_SERIAL2M(startSerialMonth),
                     dayOfWeek, occurrenceWeek)
                     .day();

  int endDay = bdlt::DateUtil::nthDayOfWeekInMonth(U_SERIAL2Y(endSerialMonth),
  ...
  for (int sm = startSerialMonth; sm <= endSerialMonth;
    ...
    schedule->push_back(bdlt::DateUtil::nthDayOfWeekInMonth(
        U_SERIAL2Y(sm), U_SERIAL2M(sm), dayOfWeek, occurrenceWeek));
  }
}
...
void ScheduleGenerationUtil::generateFromDayInterval(
    ...
    const bdlt::Date &latest, const bdlt::Date &example, int intervalInDays) {
  u::generateFromDayIntervalImp(schedule, earliest, latest, example,
                                intervalInDays);
}
...
void ScheduleGenerationUtil::generateFromDayOfMonth(
    ...
    int intervalInMonths, int targetDayOfMonth, int targetDayOfFeb) {
  u::generateFromDayOfMonthImp(schedule, earliest, latest, exampleYear,
                               exampleMonth, intervalInMonths, targetDayOfMonth,
                               targetDayOfFeb);
}
...
void ScheduleGenerationUtil::generateFromDayOfMonth(
    ...
    int exampleMonth, int intervalInMonths, int targetDayOfMonth,
    int targetDayOfFeb) {
  u::generateFromDayOfMonthImp(schedule, earliest, latest, exampleYear,
                               exampleMonth, intervalInMonths, targetDayOfMonth,
                               targetDayOfFeb);
...
void ScheduleGenerationUtil::generateFromBusinessDayOfMonth(
    bsl::vector<bdlt::Date> *schedule, const bdlt::Date &earliest,
    const bdlt::Date &latest, int exampleYear, int exampleMonth,
    int intervalInMonths, const bdlt::Calendar &calendar,
    int targetBusinessDayOfMonth) {
  u::generateFromBusinessDayOfMonthImp(schedule, earliest, latest, exampleYear,
                                       exampleMonth, intervalInMonths, calendar,
                                       targetBusinessDayOfMonth);
}
...
void ScheduleGenerationUtil::generateFromDayOfWeekAfterDayOfMonth(
  ...
  u::generateFromDayOfWeekAfterDayOfMonthImp(
      schedule, earliest, latest, exampleYear, exampleMonth, intervalInMonths,
...
void ScheduleGenerationUtil::generateFromDayOfWeekInMonth(
    bsl::vector<bdlt::Date> *schedule, const bdlt::Date &earliest,
    const bdlt::Date &latest, int exampleYear, int exampleMonth,
    int intervalInMonths, bdlt::DayOfWeek::Enum dayOfWeek, int occurrenceWeek) {
  u::generateFromDayOfWeekInMonthImp(schedule, earliest, latest, exampleYear,
                                     exampleMonth, intervalInMonths, dayOfWeek,
                                     occurrenceWeek);
}
