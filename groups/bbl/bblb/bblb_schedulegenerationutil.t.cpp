...
static void toString(bsl::ostringstream *output, const VECTOR &date)
...
public:
  ...
  // pointed to by 'result' is valid, but its value is undefined.
  {
    ...
    static const int HOLIDAYS[][2] = {
        // Month Day
        // -----  ---
        {1, 23}, {2, 4},   {2, 25},  {2, 27},  {2, 28},  {5, 1},   {6, 24},
        {8, 26}, {10, 28}, {11, 22}, {11, 25}, {12, 25}, {12, 31},
    };
    ...
    for (int year = startYear; year <= endYear; year++) {
      for (size_t i = 0; i < NUM_HOLIDAYS; i++) {
        result->addHoliday(bdlt::Date(year, HOLIDAYS[i][0], HOLIDAYS[i][1]));
      }
    }
...
int main(int argc, char *argv[]) {
  ...
  case 7: {
    ...
    //
    // First, we define the inputs and output to the schedule generation
    // function:
    //..
    ...
    //..
    // Now, we invoke the 'generateFromDayOfMonth' routine to obtain the subset
    // of dates:
    //..
    bblb::ScheduleGenerationUtil::generateFromDayOfMonth(
        &schedule, earliest, latest, example.year(), example.month(),
        9,   // 'intervalInMonths'
        23); // 'targetDayOfMonth'
             //..
  ...
  case 6: {
    ...
    static const struct {
      ...
      const char *d_expectedOutputString_p;
        ...
        {L_, 20150123, 20150123, DAY(FRI), 2015, 1, 1, 4, "20150123,"},
    ...
    for (int ivt = e_BEGIN; ivt < e_END; ++ivt) {
      ...
      for (size_t di = 0; di < NUM_DATA; ++di) {
        ...
        bdlt::Date latest =
            bdlt::DateUtil::convertFromYYYYMMDDRaw(INPUT[di].d_latestYYYYMMDD);
        ...
        switch (vt) {
        case e_BSL: {
          Obj::generateFromDayOfWeekInMonth(&scheduleBsl, earliest, latest,
                                            exampleYear, exampleMonth, interval,
                                            dayOfWeek, ocurrenceWeek);
          toString(&output, scheduleBsl);
        } break;
        case e_STD: {
          Obj::generateFromDayOfWeekInMonth(&scheduleStd, earliest, latest,
                                            exampleYear, exampleMonth, interval,
                                            dayOfWeek, ocurrenceWeek);
...
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
        case e_PMR: {
          Obj::generateFromDayOfWeekInMonth(&schedulePmr, earliest, latest,
                                            exampleYear, exampleMonth, interval,
                                            dayOfWeek, ocurrenceWeek);
        ...
        LOOP3_ASSERT(di, output.str(), expectedOutput,
      ...
      cout << "\nNegative Testing." << endl;
    {
      ...
      ASSERT_FAIL(Obj::generateFromDayOfWeekInMonth(&scheduleBsl, d, d - 1,
                                                    2010, 2, 1, DAY(FRI), 1));
      ASSERT_FAIL(Obj::generateFromDayOfWeekInMonth(&scheduleStd, d, d - 1,
                                                    2010, 2, 1, DAY(FRI), 1));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      ASSERT_FAIL(Obj::generateFromDayOfWeekInMonth(&schedulePmr, d, d - 1,
                                                    2010, 2, 1, DAY(FRI), 1));
#endif
      ASSERT_PASS(Obj::generateFromDayOfWeekInMonth(&scheduleBsl, d, d, 2010, 2,
                                                    1, DAY(FRI), 1));
      ASSERT_PASS(Obj::generateFromDayOfWeekInMonth(&scheduleStd, d, d, 2010, 2,
                                                    1, DAY(FRI), 1));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      ASSERT_PASS(Obj::generateFromDayOfWeekInMonth(&schedulePmr, d, d, 2010, 2,
                                                    1, DAY(FRI), 1));
#endif

      // 'exampleYear'

      ASSERT_FAIL(Obj::generateFromDayOfWeekInMonth(&scheduleBsl, d, d, 0, 2, 1,
                                                    DAY(FRI), 1));
      ASSERT_FAIL(Obj::generateFromDayOfWeekInMonth(&scheduleStd, d, d, 0, 2, 1,
                                                    DAY(FRI), 1));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      ASSERT_FAIL(Obj::generateFromDayOfWeekInMonth(&schedulePmr, d, d, 0, 2, 1,
                                                    DAY(FRI), 1));
#endif
      ASSERT_PASS(Obj::generateFromDayOfWeekInMonth(&scheduleBsl, d, d, 1, 2, 1,
                                                    DAY(FRI), 1));
      ASSERT_PASS(Obj::generateFromDayOfWeekInMonth(&scheduleStd, d, d, 1, 2, 1,
                                                    DAY(FRI), 1));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      ASSERT_PASS(Obj::generateFromDayOfWeekInMonth(&schedulePmr, d, d, 1, 2, 1,
                                                    DAY(FRI), 1));
#endif
      ASSERT_PASS(Obj::generateFromDayOfWeekInMonth(&scheduleBsl, d, d, 9999, 2,
                                                    1, DAY(FRI), 1));
      ASSERT_PASS(Obj::generateFromDayOfWeekInMonth(&scheduleStd, d, d, 9999, 2,
                                                    1, DAY(FRI), 1));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      ASSERT_PASS(Obj::generateFromDayOfWeekInMonth(&schedulePmr, d, d, 9999, 2,
                                                    1, DAY(FRI), 1));
#endif
      ASSERT_FAIL(Obj::generateFromDayOfWeekInMonth(&scheduleBsl, d, d, 10000,
                                                    2, 1, DAY(FRI), 1));
      ASSERT_FAIL(Obj::generateFromDayOfWeekInMonth(&scheduleStd, d, d, 10000,
                                                    2, 1, DAY(FRI), 1));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      ASSERT_FAIL(Obj::generateFromDayOfWeekInMonth(&schedulePmr, d, d, 10000,
                                                    2, 1, DAY(FRI), 1));
#endif

      // 'exampleMonth'

      ASSERT_FAIL(Obj::generateFromDayOfWeekInMonth(&scheduleBsl, d, d, 2010, 0,
                                                    1, DAY(FRI), 1));
      ASSERT_FAIL(Obj::generateFromDayOfWeekInMonth(&scheduleStd, d, d, 2010, 0,
                                                    1, DAY(FRI), 1));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      ASSERT_FAIL(Obj::generateFromDayOfWeekInMonth(&schedulePmr, d, d, 2010, 0,
                                                    1, DAY(FRI), 1));
#endif
      ASSERT_PASS(Obj::generateFromDayOfWeekInMonth(&scheduleBsl, d, d, 2010, 1,
                                                    1, DAY(FRI), 1));
      ASSERT_PASS(Obj::generateFromDayOfWeekInMonth(&scheduleStd, d, d, 2010, 1,
                                                    1, DAY(FRI), 1));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      ASSERT_PASS(Obj::generateFromDayOfWeekInMonth(&schedulePmr, d, d, 2010, 1,
                                                    1, DAY(FRI), 1));
#endif
      ASSERT_PASS(Obj::generateFromDayOfWeekInMonth(&scheduleBsl, d, d, 2010,
                                                    12, 1, DAY(FRI), 1));
      ASSERT_PASS(Obj::generateFromDayOfWeekInMonth(&scheduleStd, d, d, 2010,
                                                    12, 1, DAY(FRI), 1));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      ASSERT_PASS(Obj::generateFromDayOfWeekInMonth(&schedulePmr, d, d, 2010,
                                                    12, 1, DAY(FRI), 1));
#endif
      ASSERT_FAIL(Obj::generateFromDayOfWeekInMonth(&scheduleBsl, d, d, 2010,
                                                    13, 1, DAY(FRI), 1));
      ASSERT_FAIL(Obj::generateFromDayOfWeekInMonth(&scheduleStd, d, d, 2010,
                                                    13, 1, DAY(FRI), 1));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      ASSERT_FAIL(Obj::generateFromDayOfWeekInMonth(&schedulePmr, d, d, 2010,
                                                    13, 1, DAY(FRI), 1));
#endif

      // 'interval'

      ASSERT_FAIL(Obj::generateFromDayOfWeekInMonth(&scheduleBsl, d, d, 2010, 2,
                                                    0, DAY(FRI), 1));
      ASSERT_FAIL(Obj::generateFromDayOfWeekInMonth(&scheduleStd, d, d, 2010, 2,
                                                    0, DAY(FRI), 1));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      ASSERT_FAIL(Obj::generateFromDayOfWeekInMonth(&schedulePmr, d, d, 2010, 2,
                                                    0, DAY(FRI), 1));
#endif
      ASSERT_PASS(Obj::generateFromDayOfWeekInMonth(&scheduleBsl, d, d, 2010, 2,
                                                    1, DAY(FRI), 1));
  ...
  case 5: {
    ...
    static const struct {
      ...
      const char *d_expectedOutputString_p;
        ...
        {L_, 20150123, 20150123, DAY(FRI), 2015, 1, 1, 20, "20150123,"},
    ...
    for (int ivt = e_BEGIN; ivt < e_END; ++ivt) {
      ...
      for (size_t di = 0; di < NUM_DATA; ++di) {
        ...
        switch (vt) {
        case e_BSL: {
          Obj::generateFromDayOfWeekAfterDayOfMonth(
              &scheduleBsl, earliest, latest, exampleYear, exampleMonth,
              interval, dayOfWeek, dayOfMonth);
          toString(&output, scheduleBsl);
        } break;
        case e_STD: {
          Obj::generateFromDayOfWeekAfterDayOfMonth(
              &scheduleStd, earliest, latest, exampleYear, exampleMonth,
              interval, dayOfWeek, dayOfMonth);
          toString(&output, scheduleStd);
        } break;
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
        case e_PMR: {
          Obj::generateFromDayOfWeekAfterDayOfMonth(
              &schedulePmr, earliest, latest, exampleYear, exampleMonth,
              interval, dayOfWeek, dayOfMonth);
          toString(&output, schedulePmr);
        } break;
#endif
  ...
  case 4: {
...

    {
      ...
      noLateBusinessDays.addWeekendDaysTransition(bdlt::Date(2016, 6, 1), dows);
      ...
      noLateBusinessDays.addWeekendDaysTransition(bdlt::Date(2016, 1, 1), dows);
      noLateBusinessDays.addWeekendDaysTransition(bdlt::Date(2017, 1, 1), dows);
    }
    ...
    static const struct {
      ...
      const char *d_expectedOutputString_p;
        ...
        {L_, 20150123, 20190203, &cal1, 2019, 2, 17, 4, "20160404,20170904,"},
        ...
        {L_, 20150123, 20150223, &cal2, 2015, 1, 1, 4, "20150206,"},
    ...
    for (int ivt = e_BEGIN; ivt < e_END; ++ivt) {
      ...
      for (size_t di = 0; di < NUM_DATA; ++di) {
        ...
        switch (vt) {
        case e_BSL: {
          Obj::generateFromBusinessDayOfMonth(
              &scheduleBsl, earliest, latest, exampleYear, exampleMonth,
              interval, *calendar, targetBusDay);
          toString(&output, scheduleBsl);
        } break;
        case e_STD: {
          Obj::generateFromBusinessDayOfMonth(
        ...
        LOOP3_ASSERT(LINE, output.str(), expectedOutput,
                     output.str() == expectedOutput);
      }
    }
      ...
      cout << "\nNegative Testing." << endl;
    {
      ...
      ASSERT_FAIL(Obj::generateFromBusinessDayOfMonth(&scheduleBsl, d, d - 1,
                                                      2010, 2, 1, cal1, 1));
      ASSERT_FAIL(Obj::generateFromBusinessDayOfMonth(&scheduleStd, d, d - 1,
                                                      2010, 2, 1, cal1, 1));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      ASSERT_FAIL(Obj::generateFromBusinessDayOfMonth(&schedulePmr, d, d - 1,
                                                      2010, 2, 1, cal1, 1));
#endif
      ASSERT_PASS(Obj::generateFromBusinessDayOfMonth(&scheduleBsl, d, d, 2010,
                                                      2, 1, cal1, 1));
      ASSERT_PASS(Obj::generateFromBusinessDayOfMonth(&scheduleStd, d, d, 2010,
                                                      2, 1, cal1, 1));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      ASSERT_PASS(Obj::generateFromBusinessDayOfMonth(&schedulePmr, d, d, 2010,
                                                      2, 1, cal1, 1));
#endif

      // 'exampleYear'

      ASSERT_FAIL(Obj::generateFromBusinessDayOfMonth(&scheduleBsl, d, d, 0, 2,
                                                      1, cal1, 1));
      ASSERT_FAIL(Obj::generateFromBusinessDayOfMonth(&scheduleStd, d, d, 0, 2,
                                                      1, cal1, 1));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      ASSERT_FAIL(Obj::generateFromBusinessDayOfMonth(&schedulePmr, d, d, 0, 2,
                                                      1, cal1, 1));
#endif
      ASSERT_PASS(Obj::generateFromBusinessDayOfMonth(&scheduleBsl, d, d, 1, 2,
                                                      1, cal1, 1));
      ASSERT_PASS(Obj::generateFromBusinessDayOfMonth(&scheduleStd, d, d, 1, 2,
                                                      1, cal1, 1));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      ASSERT_PASS(Obj::generateFromBusinessDayOfMonth(&schedulePmr, d, d, 1, 2,
                                                      1, cal1, 1));
#endif
      ASSERT_PASS(Obj::generateFromBusinessDayOfMonth(&scheduleBsl, d, d, 9999,
                                                      2, 1, cal1, 1));
      ASSERT_PASS(Obj::generateFromBusinessDayOfMonth(&scheduleStd, d, d, 9999,
                                                      2, 1, cal1, 1));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      ASSERT_PASS(Obj::generateFromBusinessDayOfMonth(&schedulePmr, d, d, 9999,
                                                      2, 1, cal1, 1));
#endif
      ASSERT_FAIL(Obj::generateFromBusinessDayOfMonth(&scheduleBsl, d, d, 10000,
                                                      2, 1, cal1, 1));
      ASSERT_FAIL(Obj::generateFromBusinessDayOfMonth(&scheduleStd, d, d, 10000,
                                                      2, 1, cal1, 1));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      ASSERT_FAIL(Obj::generateFromBusinessDayOfMonth(&schedulePmr, d, d, 10000,
                                                      2, 1, cal1, 1));
#endif

      // 'exampleMonth'

      ASSERT_FAIL(Obj::generateFromBusinessDayOfMonth(&scheduleBsl, d, d, 2010,
                                                      0, 1, cal1, 1));
      ASSERT_FAIL(Obj::generateFromBusinessDayOfMonth(&scheduleStd, d, d, 2010,
                                                      0, 1, cal1, 1));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      ASSERT_FAIL(Obj::generateFromBusinessDayOfMonth(&schedulePmr, d, d, 2010,
                                                      0, 1, cal1, 1));
#endif
      ASSERT_PASS(Obj::generateFromBusinessDayOfMonth(&scheduleBsl, d, d, 2010,
                                                      1, 1, cal1, 1));
  ...
  case 3: {
    ...
    static const struct {
      ...
      const char *d_expectedOutputString_p;
        ...
        {L_, 20150123, 20150123, 2015, 1, 1, 23, 28, "20150123,"},
    ...
    for (int ivt = e_BEGIN; ivt < e_END; ++ivt) {
      ...
      for (size_t di = 0; di < NUM_DATA; ++di) {
        ...
        switch (vt) {
        case e_BSL: {
          Obj::generateFromDayOfMonth(&scheduleBsl, earliest, latest,
                                      exampleYear, exampleMonth, interval,
                                      targetDayOfMonth, targetDayOfFeb);
          toString(&output, scheduleBsl);
        } break;
        case e_STD: {
          Obj::generateFromDayOfMonth(&scheduleStd, earliest, latest,
                                      exampleYear, exampleMonth, interval,
                                      targetDayOfMonth, targetDayOfFeb);
          toString(&output, scheduleStd);
        } break;
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
        case e_PMR: {
          Obj::generateFromDayOfMonth(&schedulePmr, earliest, latest,
                                      exampleYear, exampleMonth, interval,
                                      targetDayOfMonth, targetDayOfFeb);
          toString(&output, schedulePmr);
        } break;
      ...
      cout << "\nNegative Testing." << endl;
    {
      ...
      ASSERT_FAIL(Obj::generateFromDayOfMonth(&scheduleBsl, d, d - 1, 2010, 2,
                                              1, 1, 1));
      ASSERT_FAIL(Obj::generateFromDayOfMonth(&scheduleStd, d, d - 1, 2010, 2,
                                              1, 1, 1));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      ASSERT_FAIL(Obj::generateFromDayOfMonth(&schedulePmr, d, d - 1, 2010, 2,
                                              1, 1, 1));
#endif
      ASSERT_PASS(
          Obj::generateFromDayOfMonth(&scheduleBsl, d, d, 2010, 2, 1, 1, 1));
      ASSERT_PASS(
          Obj::generateFromDayOfMonth(&scheduleStd, d, d, 2010, 2, 1, 1, 1));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      ASSERT_PASS(
          Obj::generateFromDayOfMonth(&schedulePmr, d, d, 2010, 2, 1, 1, 1));
#endif

      // 'exampleYear'

      ASSERT_FAIL(
          Obj::generateFromDayOfMonth(&scheduleBsl, d, d, 0, 2, 1, 1, 1));
      ASSERT_FAIL(
          Obj::generateFromDayOfMonth(&scheduleStd, d, d, 0, 2, 1, 1, 1));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      ASSERT_FAIL(
          Obj::generateFromDayOfMonth(&schedulePmr, d, d, 0, 2, 1, 1, 1));
#endif
      ASSERT_PASS(
          Obj::generateFromDayOfMonth(&scheduleBsl, d, d, 1, 2, 1, 1, 1));
      ASSERT_PASS(
          Obj::generateFromDayOfMonth(&scheduleStd, d, d, 1, 2, 1, 1, 1));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      ASSERT_PASS(
          Obj::generateFromDayOfMonth(&schedulePmr, d, d, 1, 2, 1, 1, 1));
#endif
      ASSERT_PASS(
          Obj::generateFromDayOfMonth(&scheduleBsl, d, d, 9999, 2, 1, 1, 1));
      ASSERT_PASS(
          Obj::generateFromDayOfMonth(&scheduleStd, d, d, 9999, 2, 1, 1, 1));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      ASSERT_PASS(
          Obj::generateFromDayOfMonth(&schedulePmr, d, d, 9999, 2, 1, 1, 1));
#endif
      ASSERT_FAIL(
          Obj::generateFromDayOfMonth(&scheduleBsl, d, d, 10000, 2, 1, 1, 1));
      ASSERT_FAIL(
          Obj::generateFromDayOfMonth(&scheduleStd, d, d, 10000, 2, 1, 1, 1));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      ASSERT_FAIL(
          Obj::generateFromDayOfMonth(&schedulePmr, d, d, 10000, 2, 1, 1, 1));
#endif

      // 'exampleMonth'

      ASSERT_FAIL(
          Obj::generateFromDayOfMonth(&scheduleBsl, d, d, 2010, 0, 1, 1, 1));
      ASSERT_FAIL(
          Obj::generateFromDayOfMonth(&scheduleStd, d, d, 2010, 0, 1, 1, 1));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      ASSERT_FAIL(
          Obj::generateFromDayOfMonth(&schedulePmr, d, d, 2010, 0, 1, 1, 1));
#endif
      ASSERT_PASS(
          Obj::generateFromDayOfMonth(&scheduleBsl, d, d, 2010, 1, 1, 1, 1));
      ASSERT_PASS(
          Obj::generateFromDayOfMonth(&scheduleStd, d, d, 2010, 1, 1, 1, 1));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      ASSERT_PASS(
          Obj::generateFromDayOfMonth(&schedulePmr, d, d, 2010, 1, 1, 1, 1));
#endif
      ASSERT_PASS(
          Obj::generateFromDayOfMonth(&scheduleBsl, d, d, 2010, 12, 1, 1, 1));
      ASSERT_PASS(
          Obj::generateFromDayOfMonth(&scheduleStd, d, d, 2010, 12, 1, 1, 1));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      ASSERT_PASS(
          Obj::generateFromDayOfMonth(&schedulePmr, d, d, 2010, 12, 1, 1, 1));
#endif
      ASSERT_FAIL(
          Obj::generateFromDayOfMonth(&scheduleBsl, d, d, 2010, 13, 1, 1, 1));
      ASSERT_FAIL(
          Obj::generateFromDayOfMonth(&scheduleStd, d, d, 2010, 13, 1, 1, 1));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      ASSERT_FAIL(
          Obj::generateFromDayOfMonth(&schedulePmr, d, d, 2010, 13, 1, 1, 1));
#endif

      // 'interval'

      ASSERT_FAIL(
          Obj::generateFromDayOfMonth(&scheduleBsl, d, d, 2010, 2, 0, 1, 1));
      ASSERT_FAIL(
          Obj::generateFromDayOfMonth(&scheduleStd, d, d, 2010, 2, 0, 1, 1));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      ASSERT_FAIL(
          Obj::generateFromDayOfMonth(&schedulePmr, d, d, 2010, 2, 0, 1, 1));
#endif
      ASSERT_PASS(
          Obj::generateFromDayOfMonth(&scheduleBsl, d, d, 2010, 2, 1, 1, 1));
      ASSERT_PASS(
      ...
      ASSERT_FAIL(
          Obj::generateFromDayOfMonth(&scheduleBsl, d, d, 2010, 2, 1, 0, 1));
      ASSERT_FAIL(
          Obj::generateFromDayOfMonth(&scheduleStd, d, d, 2010, 2, 1, 0, 1));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      ASSERT_FAIL(
          Obj::generateFromDayOfMonth(&schedulePmr, d, d, 2010, 2, 1, 0, 1));
#endif
      ASSERT_PASS(
          Obj::generateFromDayOfMonth(&scheduleBsl, d, d, 2010, 2, 1, 1, 1));
      ASSERT_PASS(
...
#endif
      ASSERT_PASS(
          Obj::generateFromDayOfMonth(&scheduleBsl, d, d, 2010, 2, 1, 31, 1));
      ASSERT_PASS(
          Obj::generateFromDayOfMonth(&scheduleStd, d, d, 2010, 2, 1, 31, 1));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      ASSERT_PASS(
          Obj::generateFromDayOfMonth(&schedulePmr, d, d, 2010, 2, 1, 31, 1));
#endif
      ASSERT_FAIL(
          Obj::generateFromDayOfMonth(&scheduleBsl, d, d, 2010, 2, 1, 32, 1));
      ASSERT_FAIL(
          Obj::generateFromDayOfMonth(&scheduleStd, d, d, 2010, 2, 1, 32, 1));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      ASSERT_FAIL(
          Obj::generateFromDayOfMonth(&schedulePmr, d, d, 2010, 2, 1, 32, 1));
#endif

      // 'targetDayOfFeb'

      ASSERT_FAIL(
          Obj::generateFromDayOfMonth(&scheduleBsl, d, d, 2010, 2, 1, 3, -1));
      ASSERT_FAIL(
          Obj::generateFromDayOfMonth(&scheduleStd, d, d, 2010, 2, 1, 3, -1));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      ASSERT_FAIL(
          Obj::generateFromDayOfMonth(&schedulePmr, d, d, 2010, 2, 1, 3, -1));
#endif
      ASSERT_PASS(
          Obj::generateFromDayOfMonth(&scheduleBsl, d, d, 2010, 2, 1, 3, 0));
      ASSERT_PASS(
          Obj::generateFromDayOfMonth(&scheduleStd, d, d, 2010, 2, 1, 3, 0));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      ASSERT_PASS(
          Obj::generateFromDayOfMonth(&schedulePmr, d, d, 2010, 2, 1, 3, 0));
#endif
      ASSERT_PASS(
          Obj::generateFromDayOfMonth(&scheduleBsl, d, d, 2010, 2, 1, 3, 1));
      ASSERT_PASS(
...
#endif
      ASSERT_PASS(
          Obj::generateFromDayOfMonth(&scheduleBsl, d, d, 2010, 2, 1, 3, 29));
      ASSERT_PASS(
          Obj::generateFromDayOfMonth(&scheduleStd, d, d, 2010, 2, 1, 3, 29));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      ASSERT_PASS(
          Obj::generateFromDayOfMonth(&schedulePmr, d, d, 2010, 2, 1, 3, 29));
#endif
      ASSERT_FAIL(
          Obj::generateFromDayOfMonth(&scheduleBsl, d, d, 2010, 2, 1, 3, 30));
      ASSERT_FAIL(
          Obj::generateFromDayOfMonth(&scheduleStd, d, d, 2010, 2, 1, 3, 30));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      ASSERT_FAIL(
          Obj::generateFromDayOfMonth(&schedulePmr, d, d, 2010, 2, 1, 3, 30));
#endif
    }
  } break;
  case 2: {
    ...
    static const struct {
      ...
      const char *d_expectedOutputString_p;
        ...
        {L_, 20140505, 20140512, 20010201, 1,
         "20140505,20140506,20140507,20140508,20140509,20140510,20140511,"
         "20140512,"},
        {L_, 20150123, 20150203, 20190205, 7, "20150127,20150203,"},
        {L_, 20150123, 20150304, 20190223, 14, "20150131,20150214,20150228,"},
    ...
    for (int ivt = e_BEGIN; ivt < e_END; ++ivt) {
      ...
      for (size_t di = 0; di < NUM_DATA; ++di) {
        ...
        bdlt::Date latest =
            bdlt::DateUtil::convertFromYYYYMMDDRaw(INPUT[di].d_latestYYYYMMDD);
        bdlt::Date example =
            bdlt::DateUtil::convertFromYYYYMMDDRaw(INPUT[di].d_exampleYYYYMMDD);
        ...
        switch (vt) {
        case e_BSL: {
          Obj::generateFromDayInterval(&scheduleBsl, earliest, latest, example,
                                       interval);
          toString(&output, scheduleBsl);
        } break;
        case e_STD: {
          Obj::generateFromDayInterval(&scheduleStd, earliest, latest, example,
                                       interval);
          toString(&output, scheduleStd);
        } break;
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
        case e_PMR: {
          Obj::generateFromDayInterval(&schedulePmr, earliest, latest, example,
                                       interval);
          toString(&output, schedulePmr);
        } break;
#endif
      ...
      cout << "\nNegative Testing." << endl;
    {
      ...
      ASSERT_FAIL(Obj::generateFromDayInterval(&scheduleBsl, d, d - 1, d, 2));
      ASSERT_FAIL(Obj::generateFromDayInterval(&scheduleStd, d, d - 1, d, 2));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      ASSERT_FAIL(Obj::generateFromDayInterval(&schedulePmr, d, d - 1, d, 2));
#endif
      ASSERT_PASS(Obj::generateFromDayInterval(&scheduleBsl, d, d, d, 2));
      ASSERT_PASS(Obj::generateFromDayInterval(&scheduleStd, d, d, d, 2));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      ASSERT_PASS(Obj::generateFromDayInterval(&schedulePmr, d, d, d, 2));
#endif

      // 'interval'

      ASSERT_FAIL(Obj::generateFromDayInterval(&scheduleBsl, d, d, d, 0));
      ASSERT_FAIL(Obj::generateFromDayInterval(&scheduleStd, d, d, d, 0));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      ASSERT_FAIL(Obj::generateFromDayInterval(&schedulePmr, d, d, d, 0));
#endif
      ASSERT_PASS(Obj::generateFromDayInterval(&scheduleBsl, d, d, d, 1));
      ASSERT_PASS(Obj::generateFromDayInterval(&scheduleStd, d, d, d, 1));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      ASSERT_PASS(Obj::generateFromDayInterval(&schedulePmr, d, d, d, 1));
#endif
    }
  } break;
  case 1: {
