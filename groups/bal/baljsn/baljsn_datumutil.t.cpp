...
void testCase07(const TestCase07Data *data, int numData,
                const FailureListData *failureList, int numFailureList) {
  ...
  if (verbose)
    cout << "\nTest decoding JSON test suite test cases." << endl;
  ...
  for (int ti = 0; ti < numData; ++ti) {
    ...
    const bool IS_ALLOWED_FAILURE_LISTED =
        (allowedFailureList.find(TEST_NAME) != allowedFailureList.end());
    ...
    if (IS_VALID != (0 == rc)) {
      if (!IS_ALLOWED_FAILURE_LISTED) {
        ...
        ASSERTV(LINE, rc, IS_VALID, IS_VALID == (rc == 0));
      }
    } else if (IS_ALLOWED_FAILURE_LISTED) {
      ASSERTV("UNEXPECTED ALLOWED_FAILURE_LIST ENTRY RESULT", LINE, TEST_NAME,
              IS_VALID, rc, false);
    }
  }
...
void testCase06(const TestCase6Data *data, int numData) {
  if (verbose)
    cout << "\nTest encoding various datums." << endl;
  ...
  for (int ti = 0; ti < numData; ++ti) {
    ...
    const int RC_BY_STRICT[] = {data[ti].d_rcNoStrictTypes,
                                data[ti].d_rcWithStrictTypes};

    if (veryVerbose) {
      T_ P_(LINE) P_(DATUM) P_(ES) P_(IIL) P_(SPL) P_(JSON) P_(RC_BY_STRICT[0])
          P(RC_BY_STRICT[1]);
    }
...

    {
      ...
      if (bdld::Datum::e_DECIMAL64 == DATUM.type()) {
        ...
        while (bsl::string::npos != (offset = exp.find_first_of('"'))) {
          exp.erase(offset, 1);
        }
      ...
      for (int strictTypes = 0; strictTypes < 2; ++strictTypes) {
          ...
          cout << "int encode(string*,const Datum&,Opts&);" << endl;
        {
          ...
          if (0 == rc || 2 <= rc) {
            ASSERTV(LINE, RC, rc, DATUM, JSON, result, JSON == result);
          }
        }

        // Test that default Options are handled correctly.
        if (baljsn::EncodingStyle::e_COMPACT == ES && 0 == IIL && 0 == SPL &&
            0 == strictTypes) {
          if (veryVeryVerbose)
            cout << "int encode(string*,const Datum&);" << endl;
          {
            STRING result;
        ...
        if (veryVeryVerbose)
          cout << "int encode(ostream&, const Datum&,Opts&);" << endl;
        {
          ...
          if (0 == rc || 2 <= rc) {
            const bslstl::StringRef result(sb.data(), sb.length());
        ...
        if (baljsn::EncodingStyle::e_COMPACT == ES && 0 == IIL && 0 == SPL &&
            0 == strictTypes) {
          if (veryVeryVerbose)
            cout << "int encode(ostream&,const Datum&);" << endl;
          {
            bdlsb::MemOutStreamBuf sb(&ta);
            bsl::ostream os(&sb);
...
template <class STRING> void testCase04() {
  ...
  PVVV("\ta. round-trip simple string.");
  {
    ...
    ASSERTV(EXP_STRING, other->theString(), EXP_STRING == other->theString());
  }
  ...
  PVV("2. Verify we can round-trip 'int's");
  {
    ...
    ASSERTV(double(obj), other->theDouble(), double(obj) == other->theDouble());
  }
  ...
  PVV("10. Verify we can round-trip 'int64's");
  {
    ...
    PVVV("\tobj: " << obj << ", other->theDouble(): " << other->theDouble());
...
template <class STRING> void testCase03() {
  bslma::TestAllocator ta("test");
...
template <class STRING> void testCase01() {
  bslma::TestAllocator ta("test");
...
int main(int argc, char *argv[]) {
  ...
  case 9: {
    ...
    //
    // First, we create our 'Datum' object, using the 'bdld::DatumMaker'
    // utility:
    //..
    ...
    bdld::Datum books =
        m.a(m.m("Author", "Ann Leckie", "Title", "Ancillary Justice"),
            m.m("Author", "John Scalzi", "Title", "Redshirts"));
    //..
    ...
    //..
    // Finally, we can decode the 'booksJSON' and make sure we got the same
    // value back:
    //..
    ...
    ///- - - - - - - - - - - - - - - - - - -
    // The following example illustrates the construction of a 'Datum' object
    // from a JSON input.
    //
    ...
    // number, and 'double' is the supported representation of a JSON number
    // (see
    // {'Supported Types'}).
  } break;
  case 8: {
    ...
    if (verbose)
      cout << "REPRODUCE BUG FROM DRQS 165776192\n"
              "=================================\n";
    bslma::TestAllocator ta("test", veryVeryVeryVerbose);
  ...
  case 7: {
    ...
    static const TC::TestCase07Data DATA[] = {
        {L_, "n_array_1_true_without_comma.json", "[1 true]", false},
        {L_, "n_array_a_invalid_utf8.json", "[a\345]", false},
        {L_, "n_array_colon_instead_of_comma.json", "[\"\": 1]", false},
        {L_, "n_array_comma_after_close.json", "[\"\"],", false},
        {L_, "n_array_comma_and_number.json", "[,1]", false},
        {L_, "n_array_double_comma.json", "[1,,2]", false},
        {L_, "n_array_double_extra_comma.json", "[\"x\",,]", false},
        {L_, "n_array_extra_close.json", "[\"x\"]]", false},
        {L_, "n_array_extra_comma.json", "[\"\",]", false},
        {L_, "n_array_incomplete.json", "[\"x\"", false},
        {L_, "n_array_incomplete_invalid_value.json", "[x", false},
        {L_, "n_array_inner_array_no_comma.json", "[3[4]]", false},
        {L_, "n_array_invalid_utf8.json", "[\377]", false},
        {L_, "n_array_items_separated_by_semicolon.json", "[1:2]", false},
        {L_, "n_array_just_comma.json", "[,]", false},
        {L_, "n_array_just_minus.json", "[-]", false},
        {L_, "n_array_missing_value.json", "[   , \"\"]", false},
        {L_, "n_array_newlines_unclosed.json",
         "[\"a\","
         "4"
         ",1,",
         false},
        {L_, "n_array_number_and_comma.json", "[1,]", false},
        {L_, "n_array_number_and_several_commas.json", "[1,,]", false},
        {L_, "n_array_spaces_vertical_tab_formfeed.json", "[\"\013a\"\\f]",
         false},
        {L_, "n_array_star_inside.json", "[*]", false},
        {L_, "n_array_unclosed.json", "[\"\"", false},
        {L_, "n_array_unclosed_trailing_comma.json", "[1,", false},
        {L_, "n_array_unclosed_with_new_lines.json",
         "[1,"
         "1"
         ",1",
         false},
        {L_, "n_array_unclosed_with_object_inside.json", "[{}", false},
        {L_, "n_incomplete_false.json", "[fals]", false},
        {L_, "n_incomplete_null.json", "[nul]", false},
        {L_, "n_incomplete_true.json", "[tru]", false},
        {L_, "n_multidigit_number_then_00.json", "123\000", false},
        {L_, "n_number_++.json", "[++1234]", false},
        {L_, "n_number_+1.json", "[+1]", false},
        {L_, "n_number_+Inf.json", "[+Inf]", false},
        {L_, "n_number_-01.json", "[-01]", false},
        {L_, "n_number_-1.0..json", "[-1.0.]", false},
        {L_, "n_number_-2..json", "[-2.]", false},
        {L_, "n_number_-NaN.json", "[-NaN]", false},
        {L_, "n_number_.-1.json", "[.-1]", false},
        {L_, "n_number_.2e-3.json", "[.2e-3]", false},
        {L_, "n_number_0.1.2.json", "[0.1.2]", false},
        {L_, "n_number_0.3e+.json", "[0.3e+]", false},
        {L_, "n_number_0.3e.json", "[0.3e]", false},
        {L_, "n_number_0.e1.json", "[0.e1]", false},
        {L_, "n_number_0_capital_E+.json", "[0E+]", false},
        {L_, "n_number_0_capital_E.json", "[0E]", false},
        {L_, "n_number_0e+.json", "[0e+]", false},
        {L_, "n_number_0e.json", "[0e]", false},
        {L_, "n_number_1.0e+.json", "[1.0e+]", false},
        {L_, "n_number_1.0e-.json", "[1.0e-]", false},
        {L_, "n_number_1.0e.json", "[1.0e]", false},
        {L_, "n_number_1_000.json", "[1 000.0]", false},
        {L_, "n_number_1eE2.json", "[1eE2]", false},
        {L_, "n_number_2.e+3.json", "[2.e+3]", false},
        {L_, "n_number_2.e-3.json", "[2.e-3]", false},
        {L_, "n_number_2.e3.json", "[2.e3]", false},
        {L_, "n_number_9.e+.json", "[9.e+]", false},
        {L_, "n_number_expression.json", "[1+2]", false},
        {L_, "n_number_hex_1_digit.json", "[0x1]", false},
        {L_, "n_number_hex_2_digits.json", "[0x42]", false},
        {L_, "n_number_Inf.json", "[Inf]", false},
        {L_, "n_number_infinity.json", "[Infinity]", false},
        {L_, "n_number_invalid+-.json", "[0e+-1]", false},
        {L_, "n_number_invalid-negative-real.json", "[-123.123foo]", false},
        {L_, "n_number_invalid-utf-8-in-bigger-int.json", "[123\345]", false},
        {L_, "n_number_invalid-utf-8-in-exponent.json", "[1e1\345]", false},
        {L_, "n_number_invalid-utf-8-in-int.json", "[0\345]", false},
        {L_, "n_number_minus_infinity.json", "[-Infinity]", false},
        {L_, "n_number_minus_sign_with_trailing_garbage.json", "[-foo]", false},
        {L_, "n_number_minus_space_1.json", "[- 1]", false},
        {L_, "n_number_NaN.json", "[NaN]", false},
        {L_, "n_number_neg_int_starting_with_zero.json", "[-012]", false},
        {L_, "n_number_neg_real_without_int_part.json", "[-.123]", false},
        {L_, "n_number_neg_with_garbage_at_end.json", "[-1x]", false},
        {L_, "n_number_real_garbage_after_e.json", "[1ea]", false},
        {L_, "n_number_real_with_invalid_utf8_after_e.json", "[1e\345]", false},
        {L_, "n_number_real_without_fractional_part.json", "[1.]", false},
        {L_, "n_number_starting_with_dot.json", "[.123]", false},
        {L_, "n_number_U+FF11_fullwidth_digit_one.json", "[\357\274\221]",
         false},
        {L_, "n_number_with_alpha.json", "[1.2a-3]", false},
        {L_, "n_number_with_alpha_char.json", "[1.8011670033376514H-308]",
         false},
        {L_, "n_number_with_leading_zero.json", "[012]", false},
        {L_, "n_object_bad_value.json", "[\"x\", truth]", false},
        {L_, "n_object_bracket_key.json", "{[: \"x\"}", false},
        {L_, "n_object_comma_instead_of_colon.json", "{\"x\", null}", false},
        {L_, "n_object_double_colon.json", "{\"x\"::\"b\"}", false},
        {L_, "n_object_emoji.json", "{\360\237\207\250\360\237\207\255}",
         false},
        {L_, "n_object_garbage_at_end.json", "{\"a\":\"a\" 123}", false},
        {L_, "n_object_key_with_single_quotes.json", "{key: \047value\047}",
         false},
        {L_, "n_object_lone_continuation_byte_in_key_and_trailing_comma.json",
         "{\"\271\":\"0\",}", false},
        {L_, "n_object_missing_colon.json", "{\"a\" b}", false},
        {L_, "n_object_missing_key.json", "{:\"b\"}", false},
        {L_, "n_object_missing_semicolon.json", "{\"a\" \"b\"}", false},
        {L_, "n_object_missing_value.json", "{\"a\":", false},
        {L_, "n_object_no-colon.json", "{\"a\"", false},
        {L_, "n_object_non_string_key.json", "{1:1}", false},
        {L_, "n_object_non_string_key_but_huge_number_instead.json",
         "{9999E9999:1}", false},
        {L_, "n_object_repeated_null_null.json", "{null:null,null:null}",
         false},
        {L_, "n_object_several_trailing_commas.json", "{\"id\":0,,,,,}", false},
        {L_, "n_object_single_quote.json", "{\047a\047:0}", false},
        {L_, "n_object_trailing_comma.json", "{\"id\":0,}", false},
        {L_, "n_object_trailing_comment.json", "{\"a\":\"b\"}/**/", false},
        {L_, "n_object_trailing_comment_open.json", "{\"a\":\"b\"}/**//",
         false},
        {L_, "n_object_trailing_comment_slash_open.json", "{\"a\":\"b\"}//",
         false},
        {L_, "n_object_trailing_comment_slash_open_incomplete.json",
         "{\"a\":\"b\"}/", false},
        {L_, "n_object_two_commas_in_a_row.json", "{\"a\":\"b\",,\"c\":\"d\"}",
         false},
        {L_, "n_object_unquoted_key.json", "{a: \"b\"}", false},
        {L_, "n_object_unterminated-value.json", "{\"a\":\"a", false},
        {L_, "n_object_with_single_string.json", "{ \"foo\" : \"bar\", \"a\" }",
         false},
        {L_, "n_object_with_trailing_garbage.json", "{\"a\":\"b\"}#", false},
        {L_, "n_single_space.json", " ", false},
        {L_, "n_string_1_surrogate_then_escape.json", "[\"\\uD800\\\"]", false},
        {L_, "n_string_1_surrogate_then_escape_u.json", "[\"\\uD800\\u\"]",
         false},
        {L_, "n_string_1_surrogate_then_escape_u1.json", "[\"\\uD800\\u1\"]",
         false},
        {L_, "n_string_1_surrogate_then_escape_u1x.json", "[\"\\uD800\\u1x\"]",
         false},
        {L_, "n_string_accentuated_char_no_quotes.json", "[\303\251]", false},
        {L_, "n_string_backslash_00.json", "[\"\\\000\"]", false},
        {L_, "n_string_escape_x.json", "[\"\\x00\"]", false},
        {L_, "n_string_escaped_backslash_bad.json", "[\"\\\\\\\"]", false},
        {L_, "n_string_escaped_ctrl_char_tab.json", "[\"\\\t\"]", false},
        {L_, "n_string_escaped_emoji.json", "[\"\\\360\237\214\200\"]", false},
        {L_, "n_string_incomplete_escape.json", "[\"\\\"]", false},
        {L_, "n_string_incomplete_escaped_character.json", "[\"\\u00A\"]",
         false},
        {L_, "n_string_incomplete_surrogate.json", "[\"\\uD834\\uDd\"]", false},
        {L_, "n_string_incomplete_surrogate_escape_invalid.json",
         "[\"\\uD800\\uD800\\x\"]", false},
        {L_, "n_string_invalid-utf-8-in-escape.json", "[\"\\u\345\"]", false},
        {L_, "n_string_invalid_backslash_esc.json", "[\"\\a\"]", false},
        {L_, "n_string_invalid_unicode_escape.json", "[\"\\uqqqq\"]", false},
        {L_, "n_string_invalid_utf8_after_escape.json", "[\"\\\345\"]", false},
        {L_, "n_string_leading_uescaped_thinspace.json", "[\\u0020\"asd\"]",
         false},
        {L_, "n_string_no_quotes_with_bad_escape.json", "[\\n]", false},
        {L_, "n_string_single_doublequote.json", "\"", false},
        {L_, "n_string_single_quote.json", "[\047single quote\047]", false},
        {L_, "n_string_single_string_no_double_quotes.json", "abc", false},
        {L_, "n_string_start_escape_unclosed.json", "[\"\\", false},
        {L_, "n_string_unescaped_crtl_char.json", "[\"a\000a\"]", false},
        {L_, "n_string_unescaped_newline.json",
         "[\"new\n"
         "line\"]",
         false},
        {L_, "n_string_unescaped_tab.json", "[\"\t\"]", false},
        {L_, "n_string_unicode_CapitalU.json", "\"\\UA66D\"", false},
        {L_, "n_string_with_trailing_garbage.json", "\"\"x", false},
        {L_, "n_structure_angle_bracket_..json", "<.\076", false},
        {L_, "n_structure_angle_bracket_null.json", "[<null\076]", false},
        {L_, "n_structure_array_trailing_garbage.json", "[1]x", false},
        {L_, "n_structure_array_with_extra_array_close.json", "[1]]", false},
        {L_, "n_structure_array_with_unclosed_string.json", "[\"asd]", false},
        {L_, "n_structure_ascii-unicode-identifier.json", "a\303\245", false},
        {L_, "n_structure_capitalized_True.json", "[True]", false},
        {L_, "n_structure_close_unopened_array.json", "1]", false},
        {L_, "n_structure_comma_instead_of_closing_brace.json", "{\"x\": true,",
         false},
        {L_, "n_structure_double_array.json", "[][]", false},
        {L_, "n_structure_end_array.json", "]", false},
        {L_, "n_structure_incomplete_UTF8_BOM.json", "\357\273{}", false},
        {L_, "n_structure_lone-invalid-utf-8.json", "\345", false},
        {L_, "n_structure_lone-open-bracket.json", "[", false},
        {L_, "n_structure_no_data.json", "", false},
        {L_, "n_structure_null-byte-outside-string.json", "[\000]", false},
        {L_, "n_structure_number_with_trailing_garbage.json", "2@", false},
        {L_, "n_structure_object_followed_by_closing_object.json", "{}}",
         false},
        {L_, "n_structure_object_unclosed_no_value.json", "{\"\":", false},
        {L_, "n_structure_object_with_comment.json", "{\"a\":/*comment*/\"b\"}",
         false},
        {L_, "n_structure_object_with_trailing_garbage.json",
         "{\"a\": true} \"x\"", false},
        {L_, "n_structure_open_array_apostrophe.json", "[\047", false},
        {L_, "n_structure_open_array_comma.json", "[,", false},
        {L_, "n_structure_open_array_open_object.json", "[{", false},
        {L_, "n_structure_open_array_open_string.json", "[\"a", false},
        {L_, "n_structure_open_array_string.json", "[\"a\"", false},
        {L_, "n_structure_open_object.json", "{", false},
        {L_, "n_structure_open_object_close_array.json", "{]", false},
        {L_, "n_structure_open_object_comma.json", "{,", false},
        {L_, "n_structure_open_object_open_array.json", "{[", false},
        {L_, "n_structure_open_object_open_string.json", "{\"a", false},
        {L_, "n_structure_open_object_string_with_apostrophes.json",
         "{\047a\047", false},
        {L_, "n_structure_open_open.json", "[\"\\{[\"\\{[\"\\{[\"\\{", false},
        {L_, "n_structure_single_eacute.json", "\351", false},
        {L_, "n_structure_single_star.json", "*", false},
        {L_, "n_structure_trailing_#.json", "{\"a\":\"b\"}#{}", false},
        {L_, "n_structure_U+2060_word_joined.json", "[\342\201\240]", false},
        {L_, "n_structure_uescaped_LF_before_string.json", "[\\u000A\"\"]",
         false},
        {L_, "n_structure_unclosed_array.json", "[1", false},
        {L_, "n_structure_unclosed_array_partial_null.json", "[ false, nul",
         false},
        {L_, "n_structure_unclosed_array_unfinished_false.json", "[ true, fals",
         false},
        {L_, "n_structure_unclosed_array_unfinished_true.json", "[ false, tru",
         false},
        {L_, "n_structure_unclosed_object.json", "{\"asd\":\"asd\"", false},
        {L_, "n_structure_unicode-identifier.json", "\303\245", false},
        {L_, "n_structure_UTF8_BOM_no_data.json", "\357\273\277", false},
        {L_, "n_structure_whitespace_formfeed.json", "[\014]", false},
        {L_, "n_structure_whitespace_U+2060_word_joiner.json", "[\342\201\240]",
         false},
        {L_, "y_array_arraysWithSpaces.json", "[[]   ]", true},
        {L_, "y_array_empty-string.json", "[\"\"]", true},
        {L_, "y_array_empty.json", "[]", true},
        {L_, "y_array_ending_with_newline.json", "[\"a\"]\n", true},
        {L_, "y_array_false.json", "[false]", true},
        {L_, "y_array_heterogeneous.json", "[null, 1, \"1\", {}]", true},
        {L_, "y_array_null.json", "[null]", true},
        {L_, "y_array_with_1_and_newline.json",
         "[1\n"
         "]",
         true},
        {L_, "y_array_with_leading_space.json", " [1]", true},
        {L_, "y_array_with_several_null.json", "[1,null,null,null,2]", true},
        {L_, "y_array_with_trailing_space.json", "[2] ", true},
        {L_, "y_number.json", "[123e65]", true},
        {L_, "y_number_0e+1.json", "[0e+1]", true},
        {L_, "y_number_0e1.json", "[0e1]", true},
        {L_, "y_number_after_space.json", "[ 4]", true},
        {L_, "y_number_double_close_to_zero.json",
         "[-0.00000000000000000000000000"
         "000000000000000000000000000000"
         "0000000000000000000001]",
         true},
        {L_, "y_number_int_with_exp.json", "[20e1]", true},
        {L_, "y_number_minus_zero.json", "[-0]", true},
        {L_, "y_number_negative_int.json", "[-123]", true},
        {L_, "y_number_negative_one.json", "[-1]", true},
        {L_, "y_number_negative_zero.json", "[-0]", true},
        {L_, "y_number_real_capital_e.json", "[1E22]", true},
        {L_, "y_number_real_capital_e_neg_exp.json", "[1E-2]", true},
        {L_, "y_number_real_capital_e_pos_exp.json", "[1E+2]", true},
        {L_, "y_number_real_exponent.json", "[123e45]", true},
        {L_, "y_number_real_fraction_exponent.json", "[123.456e78]", true},
        {L_, "y_number_real_neg_exp.json", "[1e-2]", true},
        {L_, "y_number_real_pos_exponent.json", "[1e+2]", true},
        {L_, "y_number_simple_int.json", "[123]", true},
        {L_, "y_number_simple_real.json", "[123.456789]", true},
        {L_, "y_object.json", "{\"asd\":\"sdf\", \"dfg\":\"fgh\"}", true},
        {L_, "y_object_basic.json", "{\"asd\":\"sdf\"}", true},
        {L_, "y_object_duplicated_key.json", "{\"a\":\"b\",\"a\":\"c\"}", true},
        {L_, "y_object_duplicated_key_and_value.json",
         "{\"a\":\"b\",\"a\":\"b\"}", true},
        {L_, "y_object_empty.json", "{}", true},
        {L_, "y_object_empty_key.json", "{\"\":0}", true},
        {L_, "y_object_escaped_null_in_key.json", "{\"foo\\u0000bar\": 42}",
         true},
        ...
        {L_, "y_object_long_strings.json",
         ...
         "xxxxxxxxxxxxxxxx\"}",
         true},
        {L_, "y_object_simple.json", "{\"a\":[]}", true},
        ...
        {L_, "y_string_backslash_and_u_escaped_zero.json", "[\"\\\\u0000\"]",
         true},
        {L_, "y_string_backslash_doublequotes.json", "[\"\\\"\"]", true},
        {L_, "y_string_comments.json", "[\"a/*b*/c/*d//e\"]", true},
        {L_, "y_string_double_escape_a.json", "[\"\\\\a\"]", true},
        {L_, "y_string_double_escape_n.json", "[\"\\\\n\"]", true},
        {L_, "y_string_escaped_control_character.json", "[\"\\u0012\"]", true},
        {L_, "y_string_escaped_noncharacter.json", "[\"\\uFFFF\"]", true},
        {L_, "y_string_in_array.json", "[\"asd\"]", true},
        {L_, "y_string_in_array_with_leading_space.json", "[ \"asd\"]", true},
        {L_, "y_string_last_surrogates_1_and_2.json", "[\"\\uDBFF\\uDFFF\"]",
         true},
        {L_, "y_string_nbsp_uescaped.json", "[\"new\\u00A0line\"]", true},
        {L_, "y_string_nonCharacterInUTF-8_U+10FFFF.json",
         "[\"\364\217\277\277\"]", true},
        {L_, "y_string_nonCharacterInUTF-8_U+FFFF.json", "[\"\357\277\277\"]",
         true},
        {L_, "y_string_null_escape.json", "[\"\\u0000\"]", true},
        {L_, "y_string_one-byte-utf-8.json", "[\"\\u002c\"]", true},
        {L_, "y_string_pi.json", "[\"\317\200\"]", true},
        {L_, "y_string_reservedCharacterInUTF-8_U+1BFFF.json",
         "[\"\360\233\277\277\"]", true},
        {L_, "y_string_simple_ascii.json", "[\"asd \"]", true},
        {L_, "y_string_space.json", "\" \"", true},
        {L_, "y_string_surrogates_U+1D11E_MUSICAL_SYMBOL_G_CLEF.json",
         "[\"\\uD834\\uDd1e\"]", true},
        {L_, "y_string_three-byte-utf-8.json", "[\"\\u0821\"]", true},
        {L_, "y_string_two-byte-utf-8.json", "[\"\\u0123\"]", true},
        {L_, "y_string_u+2028_line_sep.json", "[\"\342\200\250\"]", true},
        {L_, "y_string_u+2029_par_sep.json", "[\"\342\200\251\"]", true},
        {L_, "y_string_uEscape.json", "[\"\\u0061\\u30af\\u30EA\\u30b9\"]",
         true},
        {L_, "y_string_uescaped_newline.json", "[\"new\\u000Aline\"]", true},
        {L_, "y_string_unescaped_char_delete.json", "[\"\177\"]", true},
        {L_, "y_string_unicode.json", "[\"\\uA66D\"]", true},
        {L_, "y_string_unicode_2.json",
         "[\"\342\215\202\343\210\264\342\215\202\"]", true},
        ...
        {L_, "y_string_unicode_U+2064_invisible_plus.json", "[\"\\u2064\"]",
         true},
        {L_, "y_string_unicode_U+FDD0_nonchar.json", "[\"\\uFDD0\"]", true},
        {L_, "y_string_unicode_U+FFFE_nonchar.json", "[\"\\uFFFE\"]", true},
        {L_, "y_string_unicodeEscapedBackslash.json", "[\"\\u005C\"]", true},
        {L_, "y_string_utf8.json", "[\"\342\202\254\360\235\204\236\"]", true},
        {L_, "y_string_with_del_character.json", "[\"a\177a\"]", true},
        {L_, "y_structure_lonely_false.json", "false", true},
        {L_, "y_structure_lonely_int.json", "42", true},
        {L_, "y_structure_lonely_negative_real.json", "-0.1", true},
        {L_, "y_structure_lonely_null.json", "null", true},
        {L_, "y_structure_lonely_string.json", "\"asd\"", true},
        {L_, "y_structure_lonely_true.json", "true", true},
        {L_, "y_structure_string_empty.json", "\"\"", true},
        {L_, "y_structure_trailing_newline.json", "[\"a\"]\n", true},
        {L_, "y_structure_true_in_array.json", "[true]", true},
        {L_, "y_structure_whitespace_array.json", " [] ", true}
    ...
    // overly permissive in what we accept is not a problem.
      ...
      (defined(BSLS_PLATFORM_CMP_MSVC) && BSLS_PLATFORM_CMP_VERSION < 1900))
        ...
        {L_, "n_object_trailing_comment_slash_open_incomplete.json", "TBD"},
    ...
    enum { k_NUM_FAILURE_LIST = sizeof(failureList) / sizeof(*failureList) };

    TC::testCase07<bsl::string>(DATA, k_NUM_DATA, failureList,
                                k_NUM_FAILURE_LIST);
    TC::testCase07<std::string>(DATA, k_NUM_DATA, failureList,
                                k_NUM_FAILURE_LIST);
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR_STRING
    TC::testCase07<std::experimental::pmr::string>(
        DATA, k_NUM_DATA, failureList, k_NUM_FAILURE_LIST);
#endif
  } break;
  case 6: {
    //---------------------------------------------------------------------
...
#define DU_SNaN bsl::numeric_limits<double>::signaling_NaN()
        ...
        {L, m(DU_QNaN), CO, 0, 0, "\"nan\"", 0, 2},
        {L, m(DU_QNaN), PR, 0, 0, "\"nan\"", 0, 2},
        {L, m(DU_QNaN), PR, 1, 0, "\"nan\"", 0, 2},
        {L, m(DU_QNaN), PR, 0, 1, "\"nan\"", 0, 2},
        {L, m(DU_QNaN), PR, 1, 1, " \"nan\"", 0, 2},
        ...
        {L, m(DU_SNaN), CO, 0, 0, "\"nan\"", 0, 2},
        {L, m(DU_SNaN), PR, 0, 0, "\"nan\"", 0, 2},
        {L, m(DU_SNaN), PR, 1, 0, "\"nan\"", 0, 2},
        {L, m(DU_SNaN), PR, 0, 1, "\"nan\"", 0, 2},
        {L, m(DU_SNaN), PR, 1, 1, " \"nan\"", 0, 2},
...
#undef DU_SNan
        ...
        // e_STRING
        {L, m("Hello"), CO, 0, 0, "\"Hello\"", 0, 0},
        {L, m("Hello"), PR, 0, 0, "\"Hello\"", 0, 0},
        {L, m("Hello"), PR, 1, 0, "\"Hello\"", 0, 0},
        {L, m("Hello"), PR, 0, 1, "\"Hello\"", 0, 0},
        {L, m("Hello"), PR, 1, 1, " \"Hello\"", 0, 0},

        {L, m(STR256), CO, 0, 0, "\"" STR256 "\"", 0, 0},
        {L, m(STR256), PR, 0, 0, "\"" STR256 "\"", 0, 0},
        {L, m(STR256), PR, 1, 0, "\"" STR256 "\"", 0, 0},
        {L, m(STR256), PR, 0, 1, "\"" STR256 "\"", 0, 0},
        {L, m(STR256), PR, 1, 1, " \"" STR256 "\"", 0, 0},
        ...
        // e_ERROR
        {L, m(bdld::DatumError(-1)), CO, 0, 0, "N/A", -1, -1},
        {L, m(bdld::DatumError(-1)), PR, 0, 0, "N/A", -1, -1},
        {L, m(bdld::DatumError(-1)), PR, 1, 0, "N/A", -1, -1},
        {L, m(bdld::DatumError(-1)), PR, 0, 1, "N/A", -1, -1},
        {L, m(bdld::DatumError(-1)), PR, 1, 1, "N/A", -1, -1},

        // e_DATE
        {L, m(bdlt::Date(2019, 8, 30)), CO, 0, 0, "\"2019-08-30\"", 0, 1},
        {L, m(bdlt::Date(2019, 8, 30)), PR, 0, 0, "\"2019-08-30\"", 0, 1},
        {L, m(bdlt::Date(2019, 8, 30)), PR, 1, 0, "\"2019-08-30\"", 0, 1},
        {L, m(bdlt::Date(2019, 8, 30)), PR, 0, 1, "\"2019-08-30\"", 0, 1},
        {L, m(bdlt::Date(2019, 8, 30)), PR, 1, 1, " \"2019-08-30\"", 0, 1},

        // e_TIME
        {L, m(bdlt::Time(23, 59, 58, 765)), CO, 0, 0, "\"23:59:58.765\"", 0, 1},
        {L, m(bdlt::Time(23, 59, 58, 765)), PR, 0, 0, "\"23:59:58.765\"", 0, 1},
        {L, m(bdlt::Time(23, 59, 58, 765)), PR, 1, 0, "\"23:59:58.765\"", 0, 1},
        {L, m(bdlt::Time(23, 59, 58, 765)), PR, 0, 1, "\"23:59:58.765\"", 0, 1},
        {L, m(bdlt::Time(23, 59, 58, 765)), PR, 1, 1, " \"23:59:58.765\"", 0,
         1},

        // e_DATETIME
        {L, m(bdlt::Datetime(2019, 8, 30, 23, 59, 58, 765)), CO, 0, 0,
         "\"2019-08-30T23:59:58.765\"", 0, 1},
        {L, m(bdlt::Datetime(2019, 8, 30, 23, 59, 58, 765)), PR, 0, 0,
         "\"2019-08-30T23:59:58.765\"", 0, 1},
        {L, m(bdlt::Datetime(2019, 8, 30, 23, 59, 58, 765)), PR, 1, 0,
         "\"2019-08-30T23:59:58.765\"", 0, 1},
        {L, m(bdlt::Datetime(2019, 8, 30, 23, 59, 58, 765)), PR, 0, 1,
         "\"2019-08-30T23:59:58.765\"", 0, 1},
        {L, m(bdlt::Datetime(2019, 8, 30, 23, 59, 58, 765)), PR, 1, 1,
         " \"2019-08-30T23:59:58.765\"", 0, 1},

        // e_DATETIME_INTERVAL
        {L, m(bdlt::DatetimeInterval(234, 11, 23, 45, 678, 900)), CO, 0, 0,
         "\"+234_11:23:45.678900\"", 0, 1},
        {L, m(bdlt::DatetimeInterval(234, 11, 23, 45, 678, 900)), PR, 0, 0,
         "\"+234_11:23:45.678900\"", 0, 1},
        {L, m(bdlt::DatetimeInterval(234, 11, 23, 45, 678, 900)), PR, 1, 0,
         "\"+234_11:23:45.678900\"", 0, 1},
        {L, m(bdlt::DatetimeInterval(234, 11, 23, 45, 678, 900)), PR, 0, 1,
         "\"+234_11:23:45.678900\"", 0, 1},
        {L, m(bdlt::DatetimeInterval(234, 11, 23, 45, 678, 900)), PR, 1, 1,
         " \"+234_11:23:45.678900\"", 0, 1},
        // e_INTEGER64
        {L, m(bsls::Types::Int64(8000000000LL)), CO, 0, 0, "8e+09", 0, 1},
        {L, m(bsls::Types::Int64(8000000000LL)), PR, 0, 0, "8e+09", 0, 1},
        {L, m(bsls::Types::Int64(8000000000LL)), PR, 1, 0, "8e+09", 0, 1},
        {L, m(bsls::Types::Int64(8000000000LL)), PR, 0, 1, "8e+09", 0, 1},
        {L, m(bsls::Types::Int64(8000000000LL)), PR, 1, 1, " 8e+09", 0, 1},

        // e_USERDEFINED
        {L, bdld::Datum::createUdt(0, 1), CO, 0, 0, "N/A", -1, -1},
        {L, bdld::Datum::createUdt(0, 1), PR, 0, 0, "N/A", -1, -1},
        {L, bdld::Datum::createUdt(0, 1), PR, 1, 0, "N/A", -1, -1},
        {L, bdld::Datum::createUdt(0, 1), PR, 0, 1, "N/A", -1, -1},
        {L, bdld::Datum::createUdt(0, 1), PR, 1, 1, "N/A", -1, -1},

        // e_BINARY
        {L, bdld::Datum::copyBinary(0, 0, &bsa), CO, 0, 0, "N/A", -1, -1},
        {L, bdld::Datum::copyBinary(0, 0, &bsa), PR, 0, 0, "N/A", -1, -1},
        {L, bdld::Datum::copyBinary(0, 0, &bsa), PR, 1, 0, "N/A", -1, -1},
        {L, bdld::Datum::copyBinary(0, 0, &bsa), PR, 0, 1, "N/A", -1, -1},
        {L, bdld::Datum::copyBinary(0, 0, &bsa), PR, 1, 1, "N/A", -1, -1},

        // e_DECIMAL64
        {L, bdld::Datum::createDecimal64(BDLDFP_DECIMAL_DD(0.0), &bsa), CO, 0,
         0, "\"0.0\"", 0, 1},
        {L, bdld::Datum::createDecimal64(BDLDFP_DECIMAL_DD(0.0), &bsa), PR, 0,
         0, "\"0.0\"", 0, 1},
        {L, bdld::Datum::createDecimal64(BDLDFP_DECIMAL_DD(0.0), &bsa), PR, 1,
         0, "\"0.0\"", 0, 1},
        {L, bdld::Datum::createDecimal64(BDLDFP_DECIMAL_DD(0.0), &bsa), PR, 0,
         1, "\"0.0\"", 0, 1},
        {L, bdld::Datum::createDecimal64(BDLDFP_DECIMAL_DD(0.0), &bsa), PR, 1,
        ...
        // e_ARRAY of e_DOUBLE
        {L, m.a(1.0, 1.0), CO, 0, 0, "[1,1]", 0, 0},
        {L, m.a(1.0, 1.0), PR, 0, 0, "[\n1,\n1\n]", 0, 0},
        {L, m.a(1.0, 1.0), PR, 1, 0, "[\n1,\n1\n]", 0, 0},
        {L, m.a(1.0, 1.0), PR, 0, 1, "[\n 1,\n 1\n]", 0, 0},
        {L, m.a(1.0, 1.0), PR, 1, 1, " [\n  1,\n  1\n ]", 0, 0},
        ...
        // e_MAP of e_INTEGER
        {L, m.m("a", 1, "b", 2), CO, 0, 0, "{\"a\":1,\"b\":2}", 0, 1},
        {L, m.m("a", 1, "b", 2), PR, 0, 0, "{\n\"a\" : 1,\n\"b\" : 2\n}", 0, 1},
        {L, m.m("a", 1, "b", 2), PR, 1, 0, "{\n\"a\" : 1,\n\"b\" : 2\n}", 0, 1},
        {L, m.m("a", 1, "b", 2), PR, 0, 1, "{\n \"a\" : 1,\n \"b\" : 2\n}", 0,
         1},
        {L, m.m("a", 1, "b", 2), PR, 1, 1, " {\n  \"a\" : 1,\n  \"b\" : 2\n }",
         0, 1},

        // e_MAP of e_DOUBLE
        {L, m.m("a", 1.0, "b", 2.0), CO, 0, 0, "{\"a\":1,\"b\":2}", 0, 0},
        {L, m.m("a", 1.0, "b", 2.0), PR, 0, 0, "{\n\"a\" : 1,\n\"b\" : 2\n}", 0,
         0},
        {L, m.m("a", 1.0, "b", 2.0), PR, 1, 0, "{\n\"a\" : 1,\n\"b\" : 2\n}", 0,
         0},
        {L, m.m("a", 1.0, "b", 2.0), PR, 0, 1, "{\n \"a\" : 1,\n \"b\" : 2\n}",
         0, 0},
        {L, m.m("a", 1.0, "b", 2.0), PR, 1, 1,
         " {\n  \"a\" : 1,\n  \"b\" : 2\n }", 0, 0},

        // e_MAP of e_ARRAY of e_DOUBLE
        {L, m.m("a", m.a(1.0), "b", m.a(2.0)), CO, 0, 0,
         "{\"a\":[1],\"b\":[2]}", 0, 0},
        {L, m.m("a", m.a(1.0), "b", m.a(2.0)), PR, 0, 0,
         "{\n\"a\" : [\n1\n],\n\"b\" : [\n2\n]\n}", 0, 0},
        {L, m.m("a", m.a(1.0), "b", m.a(2.0)), PR, 1, 0,
         "{\n\"a\" : [\n1\n],\n\"b\" : [\n2\n]\n}", 0, 0},
        {L, m.m("a", m.a(1.0), "b", m.a(2.0)), PR, 0, 1,
         "{\n \"a\" : [\n  1\n ],\n \"b\" : [\n  2\n ]\n}", 0, 0},
        {L, m.m("a", m.a(1.0), "b", m.a(2.0)), PR, 1, 1,
         " {\n  \"a\" : [\n   1\n  ],\n  \"b\" : [\n   2\n  ]\n }", 0, 0},

        // e_MAP of e_MAP of e_DOUBLE
        {L, m.m("a", m.m("a", 1.0, "b", 2.0)), CO, 0, 0,
         "{\"a\":{\"a\":1,\"b\":2}}", 0, 0},
        {L, m.m("a", m.m("a", 1.0, "b", 2.0)), PR, 0, 0,
         "{\n\"a\" : {\n\"a\" : 1,\n\"b\" : 2\n}\n}", 0, 0},
        {L, m.m("a", m.m("a", 1.0, "b", 2.0)), PR, 1, 0,
         "{\n\"a\" : {\n\"a\" : 1,\n\"b\" : 2\n}\n}", 0, 0},
        {L, m.m("a", m.m("a", 1.0, "b", 2.0)), PR, 0, 1,
         "{\n \"a\" : {\n  \"a\" : 1,\n  \"b\" : 2\n }\n}", 0, 0},
        {L, m.m("a", m.m("a", 1.0, "b", 2.0)), PR, 1, 1,
         " {\n  \"a\" : {\n   \"a\" : 1,\n   \"b\" : 2\n  }\n }", 0, 0},
        // e_MAP of e_INTEGER with duplicate keys
        {L, m.m("a", 1, "a", 2), CO, 0, 0, "{\"a\":1,\"a\":2}", 0, 1},
        {L, m.m("a", 1, "a", 2), PR, 0, 0, "{\n\"a\" : 1,\n\"a\" : 2\n}", 0, 1},
        {L, m.m("a", 1, "a", 2), PR, 1, 0, "{\n\"a\" : 1,\n\"a\" : 2\n}", 0, 1},
        {L, m.m("a", 1, "a", 2), PR, 0, 1, "{\n \"a\" : 1,\n \"a\" : 2\n}", 0,
         1},
        {L, m.m("a", 1, "a", 2), PR, 1, 1, " {\n  \"a\" : 1,\n  \"a\" : 2\n }",
         0, 1},
...
#undef L
  ...
  case 5: {
    ...
    const bdld::Datum LONG_DATUM_ARRAY =
        m.a(m(STR256), m(STR256), m(STR256), m(STR256), m(STR256), m(STR256),
            m(STR256), m(STR256));
    const bdld::Datum LONG_DATUM_OBJECT =
        m.m(STR1, STR256, STR2, STR256, STR4, STR256, STR8, STR256, STR16,
            STR256, STR64, STR256, STR256, STR256);
...
#define WS "   \t       \n      \v       \f       \r       "
        ...
        struct {
          ...
          bdld::Datum d_datum;
            ...
            {L_, "{\"object\":{}" WS "}", 0, 2, m.m("object", m.m())},
            {L_, "{\"object\":{}}}" WS, -3, 2, m()},
            {L_, "{\"firstName\":\"Bart\"}", 0, 1, m.m("firstName", "Bart")},
            {L_, "{\"firstName\":" WS "\"Bart\"}", 0, 1,
             m.m("firstName", "Bart")},
            {L_, "{\"firstName\":\"Bart\"" WS "}", 0, 1,
             m.m("firstName", "Bart")},
            {L_,
             "{\"Name\":{"
             "\"first\":\"Bart\","
             ...
             "}"
             "}",
             0, 2, m.m("Name", m.m("first", "Bart", "last", "Simpson"))},
            {L_,
             "{\"Name\":{"
             "\"first\":\"Bart\"," WS "\"last\":\"Simpson\""
             "}"
             "}",
             0, 2, m.m("Name", m.m("first", "Bart", "last", "Simpson"))},
            // Duplicate key test
            {L_,
             "{\"Name\":{"
             "\"first\":\"Bart\"," WS "\"first\":\"Lisa\""
             "}"
             "}",
             0, 2, m.m("Name", m.m("first", "Bart"))},
            {L_,
             "{\"Family\":["
             "\"Homer\","
             "\"Marge\","
             ...
             "}",
             0, 2,
             m.m("Family", m.a(m("Homer"), m("Marge"), m("Bart"), m("Lisa"),
                               m("Maggie"), m("Santa's LH")))},
            {L_,
             "{\"Family\":" WS "["
             "\"Homer\","
             "\"Marge\","
            ...
            {L_, "{\"a\\/b\":\"c\\/d\"}", 0, 1, m.m("a/b", "c/d")},
            {L_, "{\"a\\bb\":\"c\\bd\"}", 0, 1, m.m("a\bb", "c\bd")},
            {L_, "{\"a\\fb\":\"c\\fd\"}", 0, 1, m.m("a\fb", "c\fd")},
            {L_, "{\"a\\nb\":\"c\\nd\"}", 0, 1, m.m("a\nb", "c\nd")},
            {L_, "{\"a\\rb\":\"c\\rd\"}", 0, 1, m.m("a\rb", "c\rd")},
            {L_, "{\"a\\tb\":\"c\\td\"}", 0, 1, m.m("a\tb", "c\td")},
            {L_, "{\"a\\\\b\":\"c\\\\d\"}", 0, 1, m.m("a\\b", "c\\d")},
            {L_, "{\"a\\u0020b\":\"c\\u0020d\"}", 0, 1, m.m("a b", "c d")},
...
#undef WS
        ...
        for (int ti = 0; ti < NUM_DATA; ++ti) {
          ...
          for (bsl::vector<baljsn::DatumDecoderOptions>::const_iterator i =
            ...
            if (DEPTH == 0 || OPTIONS_DEPTH >= DEPTH) {
              ...
              ASSERTV(LINE, DEPTH, OPTIONS_DEPTH, RC == rc);
              ...
              ASSERTV(LINE, DEPTH, OPTIONS_DEPTH, rc, 0 > rc);

              if (0 == rc) {
                ASSERTV(LINE, DEPTH, OPTIONS_DEPTH, DATUM, result,
                        DATUM == *result);
                ASSERTV(os.str(), os.str().length(), 0 == os.str().length());
              } else {
                ASSERTV(os.str(), os.str().length(), 0 != os.str().length());
  ...
  case 2: {
    ...
    PVVV("\te. Parse a large number, with a large exponent.");
    {
      ...
      ASSERT(0 == result);
      ASSERT(datum->isDouble());
      LOOP_ASSERT(datum->theDouble(), 3000000000000000LL == datum->theDouble());
    }
    ...
    PVVV("\ta. Parse a normal string.");
    {
      ...
      ASSERTV(datum->theString(), "hello", "hello" == datum->theString());
    }
    ...
    PVVV("\tb. Parse a string with an escaped quote.");
    {
      ...
      ASSERT(0 == result);
      ASSERT(datum->isString());
      ASSERTV(datum->theString(), "bob says \"hi.\"",
    ...
    PVV("5. Verify we can parse an array.");
    {
      ...
      LOOP_ASSERT(array[0].theDouble(), -31.4159 == array[0].theDouble());

      ASSERT(array[1].isDouble());
      LOOP_ASSERT(array[1].theDouble(), 31.4159 == array[1].theDouble());

      ASSERT(array[2].isDouble());
      LOOP_ASSERT(array[2].theDouble(), -3.14159 == array[2].theDouble());
    ...
    PVV("6. Verify we can parse an object.");
    {
      ...
      ASSERT(0 != val);
      ASSERT(val->isDouble());
      LOOP_ASSERT(val->theDouble(), 3000000000000000LL == val->theDouble());
