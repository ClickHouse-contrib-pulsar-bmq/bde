...
// LOCAL METHODS
int decodeValue(bdld::ManagedDatum *result, bsl::ostream *errorStream,
                baljsn::Tokenizer *tokenizer, int maxNestedDepth);
...
int encodeValue(baljsn::SimpleFormatter *formatter, const bdld::Datum &datum,
                int *strictTypesCheckStatus, bsl::string_view *name = 0);
...
int decodeObject(bdld::ManagedDatum *result, bsl::ostream *errorStream,
                 baljsn::Tokenizer *tokenizer, int maxNestedDepth)
// Decode into the specified '*result' the JSON object in the specified
// '*tokenizer', updating the specified '*errorStream' if any errors are
// detected, including if the specified 'maxNestedDepth' is exceeded.
{
  ...
  while (baljsn::Tokenizer::e_END_OBJECT != tokenizer->tokenType()) {
    ...
    if (0 != baljsn::ParserUtil::getUnquotedString(&key, tokenContents)) {
      if (errorStream) {
        *errorStream << "decodeValue: getUnquotedString for key failed\n";
      }
      return -3; // RETURN
    }
    ...
    int rc = decodeValue(&elementValue, errorStream, tokenizer, maxNestedDepth);
...
int decodeArray(bdld::ManagedDatum *result, bsl::ostream *errorStream,
                baljsn::Tokenizer *tokenizer, int maxNestedDepth)
...
inline int encodeImp(STRING *result, const bdld::Datum &datum,
...
int extractValue(bdld::ManagedDatum *result, baljsn::Tokenizer *tokenizer)
...
int decodeValue(bdld::ManagedDatum *result, bsl::ostream *errorStream,
  ...
  case baljsn::Tokenizer::e_START_OBJECT: {
    int rc =
        u::decodeObject(result, errorStream, tokenizer, maxNestedDepth - 1);
    if (0 != rc) {
      if (errorStream) {
  ...
  case baljsn::Tokenizer::e_START_ARRAY: {
    int rc = u::decodeArray(result, errorStream, tokenizer, maxNestedDepth - 1);
    if (0 != rc) {
      if (errorStream) {
  ...
  default: {
    if (errorStream) {
      *errorStream << "Unexpected token: " << tokenizer->tokenType() << '\n';
    }
    return -3; // RETURN
  } break;
  }
...
int encodeArray(baljsn::SimpleFormatter *formatter,
                const bdld::DatumArrayRef &datum, int *strictTypesCheckStatus,
...
// value if 'datum' cannot be encoded, which should stop further encoding.
{
  ...
  baljsn::SimpleFormatter::ArrayFormattingStyle style =
      datum.length() ? baljsn::SimpleFormatter::e_REGULAR_ARRAY_FORMAT
  ...
  for (bsl::size_t i = 0; 0 == result && i < datum.length(); ++i) {
    result = u::encodeValue(formatter, datum[i], strictTypesCheckStatus);
  }
...
int encodeObject(baljsn::SimpleFormatter *formatter,
                 const bdld::DatumMapRef &datum, int *strictTypesCheckStatus,
                 bsl::string_view *name = 0)
...
// value if 'datum' cannot be encoded, which should stop further encoding.
{
  ...
  for (bsl::size_t i = 0; 0 == result && i < datum.size(); ++i) {
    ...
    result = u::encodeValue(formatter, datum[i].value(), strictTypesCheckStatus,
                            &name);
  }
...
int encodeValue(baljsn::SimpleFormatter *formatter, const bdld::Datum &datum,
                int *strictTypesCheckStatus, bsl::string_view *name) {
  int result = -1;
  ...
  case bdld::Datum::e_MAP: {
    result = u::encodeObject(formatter, datum.theMap(), strictTypesCheckStatus);
  } break;
  case bdld::Datum::e_ARRAY: {
    result =
        u::encodeArray(formatter, datum.theArray(), strictTypesCheckStatus);
  } break;
...
// CLASS METHODS
int DatumUtil::decode(bdld::ManagedDatum *result, bsl::ostream *errorStream,
  ...
  int rc =
      u::decodeValue(&value, errorStream, &tokenizer, options.maxNestedDepth());
  if (0 != rc) {
    if (errorStream) {
      *errorStream << "decodeValue failed, rc = " << rc << '\n';
    }
