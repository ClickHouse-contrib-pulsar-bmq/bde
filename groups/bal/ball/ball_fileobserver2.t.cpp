...
void aSsErT2(bool condition, const char *message, int line) {
  if (condition) {
    cerr << "Error " __FILE__ "(" << line << "): " << message << "    (failed)"
         << endl;
...
void enableFileLogging(bsl::shared_ptr<ball::FileObserver2> &observer,
...
// the filename.
{
  ...
  if (!addTimestamp) {
    ...
    ASSERT(0 == observer->enableFileLogging(fileName.c_str(), addTimestamp));
    ASSERT(true == observer->isFileLoggingEnabled());
    ASSERT(1 == observer->enableFileLogging(fileName.c_str(), addTimestamp));
  }
}
...
// Output the specified 'record' into the specified 'stream'.
{
  ...
  const int numBytesWritten = record.fixedFields().timestamp().printToBuffer(
      buffer, k_SIZE, fractionalSecondPrecision);
...
class LogRotationCallbackTester {
  ...
  public:
    ...
    explicit Rep(bslma::Allocator *basicAllocator)
        ...
        : d_invocations(0), d_status(0), d_rotatedFileName(basicAllocator) {}
  };

  enum { k_UNINITIALIZED = INT_MIN };
...
{
  ball::RecordAttributes attr(bdlt::CurrentTime::utc(), 1, 2, "FILENAME", 3,
                              "CATEGORY", 32, message);
...
bsls::TimeInterval TestCurrentTimeCallback::load() { return s_utcTime; }
...
public:
  ...
  static void setLocalTimeOffset(bsls::Types::Int64 localTimeOffsetInSeconds);
...
bsls::TimeInterval
TestLocalTimeOffsetCallback::loadLocalTimeOffset(const bdlt::Datetime &) {
  ++s_loadCount;
...
int TestLocalTimeOffsetCallback::loadCount() { return s_loadCount; }

void splitStringIntoLines(bsl::vector<bsl::string> *result, const char *ascii) {
  ASSERT(result);
  ASSERT(ascii);

  for (bdlb::Tokenizer itr(bslstl::StringRef(ascii), bslstl::StringRef(""),
                           bslstl::StringRef("\n"));
       itr.isValid(); ++itr) {
...
void getDatetimeField(bsl::string *result, const bsl::string &fileName,
                      int recordNumber) {
...
int main(int argc, char *argv[]) {
  ...
  case 14: {
    ...
    // and set the logging "pass-through" level -- the level at which log
    // records are published to registered observers -- to 'DEBUG':
    //..
    ...
    // takes the configuration object just created.  The guard will initialize
    // the logger manager singleton on creation and destroy the singleton upon
    // destruction.  This guarantees that any resources used by the logger
    // manager will be properly released when they are not needed:
    //..
    ...
    //..
    // Note that in this configuration the user may end up with multiple log
  ...
  case 13: {
    ...
    if (verbose)
      cout << "REPRODUCE BUG FROM DRQS 123123158\n"
              "=================================\n";
    ...
    configuration.setDefaultThresholdLevelsIfValid(
        ball::Severity::e_TRACE, ball::Severity::e_TRACE,
        ball::Severity::e_WARN, ball::Severity::e_FATAL);
  ...
  case 12: {
    ...
    const bdlt::Datetime UTC_ARRAY[] = {
        bdlt::EpochUtil::epoch(), bdlt::Datetime(2017, 1, 1, 12, 46, 30, 0),
    ...
    if (verbose)
      cout << "\nTest TestCurrentTimeCallback: Installed" << endl;
    {
      ...
      cout << "\nTest TestLocalTimeOffsetCallback: Direct" << endl;
    {
      for (int i = 0; i < NUM_LTO_ARRAY; ++i) {
        ...
        if (veryVerbose) {
          T_ P_(i) P(localTimeOffset)
        }

        TestLocalTimeOffsetCallback::setLocalTimeOffset(localTimeOffset);
        for (int j = 0; j < NUM_UTC_ARRAY; ++j) {
          bdlt::Datetime utcDatetime = UTC_ARRAY[j];

          if (veryVerbose) {
            T_ T_ P_(j) P(utcDatetime)
          }

          Int64 result =
              TestLocalTimeOffsetCallback::loadLocalTimeOffset(utcDatetime)
                  .totalSeconds();
          ++loadCount;

          ASSERTV(i, j, localTimeOffset == result);
          ASSERTV(i, j, loadCount == TestLocalTimeOffsetCallback::loadCount());
        }
      }
    }

    if (verbose)
      cout << "\nTest TestLocalTimeOffsetCallback: Installed" << endl;
    {
      ...
      for (int i = 0; i < NUM_LTO_ARRAY; ++i) {
        ...
        for (int j = 0; j < NUM_UTC_ARRAY; ++j) {
          ...
          Int64 result = bdlt::LocalTimeOffset::localTimeOffset(utcDatetime)
                             .totalSeconds();
          ++loadCount;
  ...
  case 11: {
    ...
    ASSERT(0 == configuration.setDefaultThresholdLevelsIfValid(
                    ball::Severity::e_OFF, ball::Severity::e_TRACE,
                    ball::Severity::e_OFF, ball::Severity::e_OFF));
...

    {
      ...
      if (veryVerbose) {
        T_ P(testLocalTimeOffsetInSeconds);
      }
      ...
      bdlt::Datetime DT =
          testUtcDatetime +
          bdlt::DatetimeInterval(0, 0, 0, testLocalTimeOffsetInSeconds, 0);
  ...
  case 8: {
    ...
    if (veryVerbose)
      cout << "Test normal rotation" << endl;
    {
    ...
    if (veryVerbose)
      cout << "Test delayed logging" << endl;
    {
      bslma::TestAllocator ta("test", veryVeryVeryVerbose);
    ...
    if (veryVerbose)
      cout << "Test 'disableTimeIntervalRotation" << endl;
    {
      bslma::TestAllocator ta("test", veryVeryVeryVerbose);
    ...
    if (veryVerbose)
      cout << "\tTest a delayed rotateOnTimeInterval" << "(DRQS 87930585)"
           << bsl::endl;
    {
  ...
  case 7: {
    ...
    if (verbose)
      cout << "Test-case infrastructure setup." << endl;
    {
      bdls::TempDirectoryGuard tempDirGuard("ball_");
      bsl::string fileName(tempDirGuard.getTempDirName());
      bdls::PathUtil::appendRaw(&fileName, "testLog");
      ...
      if (verbose)
        cout << "Testing file observer with existing file." << endl;
      {
        char buffer[512];
      ...
      if (verbose)
        cout << "Testing file observer with suppressed filename "
                "uniqueness"
             << endl;
      {
        char buffer[512];
  ...
  case 5: {
    ...
    if (veryVerbose)
      cout << "\tTest a rotation that will fail without a callback." << endl;
    {
    ...
    if (veryVerbose)
      cout << "\tTest a with publication rotation" << endl;
    {
      bslma::TestAllocator ta("test", veryVeryVeryVerbose);
    ...
    if (veryVerbose)
      cout << "\tTest a re-entrant rotation" << endl;
    {
      bslma::TestAllocator ta("test", veryVeryVeryVerbose);
  ...
  case 4: {
    ...
    if (veryVerbose)
      cout << "Test infrastructure setup." << endl;
    ...
    if (verbose)
      cout << "Test table of file names" << endl;
    {
      static const struct {
        int d_line;
        const char *d_suffix;
      ...
      for (int ti = 0; ti < NUM_DATA; ++ti) {
        ...
        for (const char *c = PAT; *c != '\0'; ++c) {
          switch (*c) {
          case 'Y': {
            oss << bsl::setw(4) << bsl::setfill('0') << startDatetime.year();
          } break;
          case 'M': {
            oss << bsl::setw(2) << bsl::setfill('0') << startDatetime.month();
          } break;
          case 'D': {
            oss << bsl::setw(2) << bsl::setfill('0') << startDatetime.day();
          } break;
          case 'h': {
            oss << bsl::setw(2) << bsl::setfill('0') << startDatetime.hour();
          } break;
          case 'm': {
            oss << bsl::setw(2) << bsl::setfill('0') << startDatetime.minute();
          } break;
          case 's': {
            oss << bsl::setw(2) << bsl::setfill('0') << startDatetime.second();
          } break;
          case 'p': {
            oss << processId;
          } break;
        ...
        ASSERTV(LINE, actualFileName, 1 == FsUtil::exists(actualFileName));
        ...
        if (!UNI) {
          ...
          ASSERTV(LINE, rotatedFileName, 1 == FsUtil::exists(rotatedFileName));
    ...
    if (verbose)
      cout << "Testing filename time-stamp uses "
           << "'isPublishInLocalTimeEnabled'" << endl;
    {
        ...
        cout << "\tTesting utc pattern in filename." << endl;
      {
        ...
        bdlt::Datetime timestamp(
            bsl::atoi(year.c_str()), bsl::atoi(month.c_str()),
            bsl::atoi(day.c_str()), bsl::atoi(hour.c_str()),
            bsl::atoi(minute.c_str()), bsl::atoi(second.c_str()));
        ...
        ASSERTV(timestamp, start, end, start <= timestamp && timestamp <= end);
      }
  ...
  case 3: {
    ...
    if (verbose)
      cout << "Testing output when the stream fails" << " (UNIX only)." << endl;
    {
      bslma::TestAllocator ta("ta", veryVeryVeryVerbose);
      ...
      const bool fail =
          (bsl::string::npos == line.find("Error on file stream"));
      ASSERT(!fail);
      if (fail)
        cout << "line: " << line << endl;
  ...
  case 2: {
      ...
      cout << "Test-case infrastructure setup." << endl;
    {
      ...
      if (verbose)
        cout << "Testing lifetime-constrained rotation." << endl;
      ...
      if (verbose)
        cout << "Testing forced rotation." << endl;
      {
        bslmt::ThreadUtil::microSleep(0, 2);
        mX->forceRotation();
        BALL_LOG_TRACE << "log 1";
      ...
      if (verbose)
        cout << "Testing size-constrained rotation." << endl;
      {
        bslmt::ThreadUtil::microSleep(0, 2);
        ASSERT(0 == X->rotationSize());
        mX->rotateOnSize(1);
        ...
        for (int i = 0; i < (int)globbuf.gl_pathc - 3; ++i) {
          ASSERT(1024 < FsUtil::getFileSize(globbuf.gl_pathv[i + 2]));
        }
  ...
  case 1: {
    ...
    //   bool isFileLoggingEnabled(std::experimental::pmr::string *result)
    //   const; bool isPublishInLocalTimeEnabled() const;
    // --------------------------------------------------------------------
...
// TBD fix this for Windows !!!
    ...
    bslma::TestAllocator ta("ta", veryVeryVeryVerbose);
    {
      ...
      if (verbose)
        cout << "Testing default output format." << endl;
      {
        bdls::TempDirectoryGuard tempDirGuard("ball_");
        bsl::string fileName(tempDirGuard.getTempDirName());
        bdls::PathUtil::appendRaw(&fileName, "testLog");
        ...
        ASSERT(currentLocalTime - currentUTCTime == X->localTimeOffset());

        bsl::shared_ptr<ball::StreamObserver> refX(
            new (ta) ball::StreamObserver(&dos), &ta);
      ...
      if (verbose)
        cout << "Testing format with local time offset." << endl;
      {
        bdls::TempDirectoryGuard tempDirGuard("ball_");
        bsl::string fileName(tempDirGuard.getTempDirName());
        bdls::PathUtil::appendRaw(&fileName, "testLog");
        ...
        testOs << " " << bdls::ProcessUtil::getProcessId() << ":"
               << bslmt::ThreadUtil::selfIdAsUint64() << " FATAL " << __FILE__
               << ":" << __LINE__ - 5 << " TestCategory log FATAL " << "\n";
        ...
        ASSERTV((fileObsHour + difference + 24) % 24, defaultObsHour,
      ...
      if (verbose)
        cout << "Testing file logging." << endl;
      {
        bdls::TempDirectoryGuard tempDirGuard("ball_");
        bsl::string fileName(tempDirGuard.getTempDirName());
        bdls::PathUtil::appendRaw(&fileName, "testLog");
      ...
      if (verbose)
        cout << "Testing '%%' in file name pattern." << endl;
      {
        static const struct {
          ...
          const char *d_fileNameSuffix_p; // filename suffix
            ...
            //----  --------------  ---------------
            {L_, "foo", "foo"},           {L_, "foo%", "foo%"},
            {L_, "foo%bar", "foo%bar"},   {L_, "foo%%", "foo"},
            {L_, "foo%%bar", "foobar"},   {L_, "foo%%%", "foo%"},
            {L_, "foo%%%bar", "foo%bar"},
        };
        ...
        for (int ti = 0; ti < NUM_DATA; ++ti) {
          ...
          }
          {
            ...
            ASSERTV(LINE,
                    std::string(expected.cbegin(), expected.cend()) == actual);
          }
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR_STRING
          {
            ...
            ASSERTV(LINE, std::experimental::pmr::string(
                              expected.cbegin(), expected.cend()) == actual);
          }
      ...
      if (verbose)
        cout << "Testing functors with the same format." << endl;
      {
        bdls::TempDirectoryGuard tempDirGuard("ball_");
        bsl::string baseName(tempDirGuard.getTempDirName());
        bdls::PathUtil::appendRaw(&baseName, "testLog");
      ...
      if (verbose)
        cout << "Testing functors with different formats." << endl;
      {
        bdls::TempDirectoryGuard tempDirGuard("ball_");
        bsl::string baseName(tempDirGuard.getTempDirName());
        bdls::PathUtil::appendRaw(&baseName, "testLog");
        ...
        mX1->setLogFileFunctor(
            ball::RecordStringFormatter("\n%p %t %s %f %c %m %u\n"));

        BALL_LOG_WARN << "log";

        ASSERT(2 == readFileIntoString(__LINE__, fileName1a, fileContent));
        ...
        ASSERT(2 == readFileIntoString(__LINE__, fileName1b, fileContent));
        ...
        ASSERT(2 == readFileIntoString(__LINE__, fileName1c, fileContent));
        ASSERT(file2 == fileContent);
