...
enum {
  ...
  k_BUFFER_SIZE = k_DELIMETER_LENGTH                           // '\n'
                  + k_TIMESTAMP_ID_LENGTH + k_DELIMETER_LENGTH // ' '
                  + k_PROCESS_ID_LENGTH + k_DELIMETER_LENGTH   // ':'
                  + k_THREAD_ID_LENGTH + k_DELIMETER_LENGTH    // ' '
                  + k_SEVERITY_LENGTH + k_DELIMETER_LENGTH     // ' '
                  + k_MAX_PATH_LENGTH + k_DELIMETER_LENGTH     // ':'
                  + k_LINE_NUM_LENGTH + k_DELIMETER_LENGTH,    // ' '
...
// Return the specified 'timestamp' in the 'YYYYMMDD_hhmmss' format.
{
  ...
  snprintf(buffer, sizeof buffer, "%04d%02d%02d_%02d%02d%02d", timestamp.year(),
...
static void getLogFileName(bsl::string *logFileName,
                           ...
                           const char *logFilePattern, bool publishInLocalTime)
...
// 'timestampUtc'.
{
  ...
  *timestampUtc = bdlt::CurrentTime::utc();
  ...
  if (publishInLocalTime) {
    logFileTimestamp += bdlt::LocalTimeOffset::localTimeOffset(*timestampUtc);
  }
  ...
  for (; *logFilePattern; ++logFilePattern) {
    if ('%' == *logFilePattern) {
      if (*++logFilePattern) {
        ...
        case 'Y': {
          os << bsl::setw(4) << bsl::setfill('0') << logFileTimestamp.year();
        } break;
        case 'M': {
          os << bsl::setw(2) << bsl::setfill('0') << logFileTimestamp.month();
        } break;
        case 'D': {
          os << bsl::setw(2) << bsl::setfill('0') << logFileTimestamp.day();
        } break;
        case 'h': {
          os << bsl::setw(2) << bsl::setfill('0') << logFileTimestamp.hour();
        } break;
        case 'm': {
          os << bsl::setw(2) << bsl::setfill('0') << logFileTimestamp.minute();
        } break;
        case 's': {
          os << bsl::setw(2) << bsl::setfill('0') << logFileTimestamp.second();
        } break;
        case 'p': {
...
// a non-zero value otherwise.
{
  ...
  FileUtil::FileDescriptor fd =
      FileUtil::open(filename, FileUtil::e_OPEN_OR_CREATE,
                     FileUtil::e_READ_APPEND, FileUtil::e_KEEP);
  ...
  bdls::FdStreamBuf *streamBuf =
      dynamic_cast<bdls::FdStreamBuf *>(stream->rdbuf());
...
bool fuzzyEqual(const bdlt::Datetime &a, const bdlt::Datetime &b,
...
static bdlt::Datetime
computeNextRotationTime(const bdlt::Datetime &referenceStartTime,
...
void FileObserver2::logRecordDefault(bsl::ostream &stream, const Record &record)

{
  ...
  *ptr = '\n';
  ...
  snprintf(ptr, sizeof(buffer) - length - 1, " %d:%llu %s %s:%d ",
           fixedFields.processID(), fixedFields.threadID(),
           Severity::toAscii((Severity::Level)fixedFields.severity()),
           fixedFields.fileName(), fixedFields.lineNumber());
...
int FileObserver2::rotateFile(bsl::string *rotatedLogFileName) {
  ...
  if (0 != d_logStreamBuf.clear()) {
    ...
    snprintf(errorBuffer, sizeof errorBuffer,
             "Unable to close old log file: %s.", d_logFileName.c_str());
    bsls::Log::platformDefaultMessageHandler(bsls::LogSeverity::e_WARN,
                                             __FILE__, __LINE__, errorBuffer);
  ...
  *rotatedLogFileName = d_logFileName;
  ...
  getLogFileName(&d_logFileName, &d_logFileTimestampUtc,
                 d_logFilePattern.c_str(), d_publishInLocalTime);
  ...
  if (bdls::FilesystemUtil::exists(d_logFileName.c_str()) &&
    ...
    if (0 == bsl::rename(d_logFileName.c_str(), newFileName.c_str())) {
      *rotatedLogFileName = newFileName;
      ...
      snprintf(errorBuffer, sizeof errorBuffer, "Cannot rename %s to %s: %s.",
  ...
  if (0 < d_rotationInterval.totalSeconds()) {
    d_nextRotationTimeUtc =
        computeNextRotationTime(d_rotationReferenceTime, d_publishInLocalTime,
                                d_rotationInterval, d_logFileTimestampUtc);
  }
  ...
  if (0 != openLogFile(&d_logOutStream, d_logFileName.c_str())) {
    ...
    bsls::Log::platformDefaultMessageHandler(bsls::LogSeverity::e_ERROR,
                                             __FILE__, __LINE__, errorBuffer);
    return k_ROTATE_SUCCESS != returnStatus ? k_ROTATE_RENAME_AND_NEW_LOG_ERROR
...
FileObserver2::FileObserver2(bslma::Allocator *basicAllocator)
    : d_logStreamBuf(bdls::FilesystemUtil::k_INVALID_FD, false, true, false,
                     basicAllocator),
      d_logOutStream(&d_logStreamBuf), d_logFilePattern(basicAllocator),
      ...
      d_publishInLocalTime(false), d_suppressUniqueFileName(false),
      d_rotationSize(0), d_rotationInterval(0),
...
void FileObserver2::disableLifetimeRotation() { disableTimeIntervalRotation(); }
...
void FileObserver2::publish(const Record &record, const Context &) {
...

  {
    bslmt::LockGuard<bslmt::Mutex> guard(&d_mutex);
    rotationStatus =
        rotateIfNecessary(&rotatedFileName, record.fixedFields().timestamp());
    ...
    if (d_logStreamBuf.isOpened()) {
      ...
      if (!d_logOutStream) {
        ...
        snprintf(errorBuffer, sizeof errorBuffer,
                 "Error on file stream for %s: %s.", d_logFileName.c_str(),
                 bsl::strerror(getErrorCode()));
        bsls::Log::platformDefaultMessageHandler(
            bsls::LogSeverity::e_ERROR, __FILE__, __LINE__, errorBuffer);
...
void FileObserver2::rotateOnTimeInterval(
    const bdlt::DatetimeInterval &interval) {
  rotateOnTimeInterval(interval, d_publishInLocalTime
                                     ? bdlt::CurrentTime::local()
                                     : bdlt::CurrentTime::utc());
}

void FileObserver2::rotateOnTimeInterval(const bdlt::DatetimeInterval &interval,
...
bool FileObserver2::isFileLoggingEnabled(
    std::experimental::pmr::string *result) const {
  return isFileLoggingEnabledImpl(result);
}
