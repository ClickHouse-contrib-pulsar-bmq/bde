...
OccurrenceInfo::OccurrenceInfo()
    : d_isRequired(false), d_isHidden(false), d_defaultValue() {}

OccurrenceInfo::OccurrenceInfo(bslma::Allocator *basicAllocator)
    : d_isRequired(false), d_isHidden(false), d_defaultValue(basicAllocator) {}

OccurrenceInfo::OccurrenceInfo(OccurrenceType type,
                               bslma::Allocator *basicAllocator)
    : d_isRequired(e_REQUIRED == type), d_isHidden(e_HIDDEN == type),
      d_defaultValue(basicAllocator) {}

OccurrenceInfo::OccurrenceInfo(char defaultValue,
                               bslma::Allocator *basicAllocator)
    : d_isRequired(false), d_isHidden(false),
...
OccurrenceInfo::OccurrenceInfo(const bsl::string &defaultValue,
  ...
  // 'std::experimental::pmr::string's can be passed, and will coerce to a
  // 'bsl::string', which would've happened anyway if 'defaultValue' were a
  // string view.
  //
  // So changing 'defaultValue' to a string view would accomplish no gain in
  // the cases of 'const char *', 'std::string', and
  // 'std::experimental::pmr::string', and results in a loss in the case of
  // 'bsl::string'.
}
...
OccurrenceInfo::OccurrenceInfo(const bsl::vector<bdlt::Datetime> &defaultValue,
                               bslma::Allocator *basicAllocator)
    : d_isRequired(false), d_isHidden(false),
      d_defaultValue(defaultValue, basicAllocator) {}
...
OccurrenceInfo::OccurrenceInfo(const OccurrenceInfo &original,
                               bslma::Allocator *basicAllocator)
    : d_isRequired(original.d_isRequired), d_isHidden(original.d_isHidden),
...
bool OccurrenceInfo::isHidden() const { return d_isHidden; }

bool OccurrenceInfo::isRequired() const { return d_isRequired; }

OccurrenceInfo::OccurrenceType OccurrenceInfo::occurrenceType() const {
  return d_isRequired ? e_REQUIRED : (d_isHidden ? e_HIDDEN : e_OPTIONAL);
}
...
bsl::ostream &OccurrenceInfo::print(bsl::ostream &stream, int level,
