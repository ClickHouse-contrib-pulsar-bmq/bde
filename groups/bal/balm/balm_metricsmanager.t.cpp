...
BSLA_MAYBE_UNUSED bsl::ostream &operator<<(bsl::ostream &stream,
                                           u::LibType lt) {
...
inline bool recordLess(const balm::MetricRecord &lhs,
...
bool withinWindow(const bdlt::DatetimeTz &localTime,
                  const bdlt::Datetime &utcExpectedTime, int windowMs)
...
inline const balm::Category *firstCategory(const balm::MetricSampleGroup &group)
...
bool withinWindow(const bsls::TimeInterval &value,
                  const bsls::TimeInterval &expectedValue, int windowMs)
...
// 'windowMs' (milliseconds) of the specified 'expectedValue'.
{
  ...
  bool withinWindow =
      (expectedValue - window) < value && (expectedValue + window) > value;
...
public:
  ...
  // MANIPULATORS
  void recordMetrics(bsl::vector<balm::MetricRecord> *records, bool resetFlag);
...
// CREATORS
inline TestCallback::TestCallback(Id metricId, bslma::Allocator *allocator)
    : d_numInvocations(0), d_record(metricId), d_reset(false),
      d_allocator_p(allocator) {}

inline TestCallback::TestCallback(const balm::MetricRecord &record,
                                  bslma::Allocator *allocator)
    : d_numInvocations(0), d_record(record), d_reset(false),
      d_allocator_p(allocator) {}
...
inline balm::MetricRecord &TestCallback::record() { return d_record; }

inline void TestCallback::reset() { d_numInvocations = 0; }

// ACCESSORS
inline const balm::MetricRecord &TestCallback::record() const {
  return d_record;
}

inline balm::MetricId TestCallback::metricId() const {
  return d_record.metricId();
}

inline balm::MetricsManager::RecordsCollectionCallback
TestCallback::function() {
  balm::MetricsManager::RecordsCollectionCallback cb =
      bdlf::BindUtil::bind(&TestCallback::recordMetrics, this,
                           bdlf::PlaceHolders::_1, bdlf::PlaceHolders::_2);
  return cb;
}

inline int TestCallback::invocations() const { return d_numInvocations; }

inline bool TestCallback::resetFlag() const { return d_reset; }
...
class LockAndModifyWorker {
  ...
  void worker();
  static void dummyCallback(bsl::vector<balm::MetricRecord> *, bool) {}

public:
  // CREATORS
  LockAndModifyWorker(bslmt::Mutex *mutex, balm::MetricsManager *obj)
      : d_mutex_p(mutex), d_obj_p(obj),
        d_myCategory_p(
            obj->metricRegistry().addCategory("LOCKANDMODIFYWORKER")) {}

  int start() {
    d_done = 0;
    return bslmt::ThreadUtil::create(
        &d_thread, bdlf::MemFnUtil::memFn(&LockAndModifyWorker::worker, this));
  }
  ...
  while (!d_done) {
    ...
    balm::MetricsManager::CallbackHandle handle =
        d_obj_p->registerCollectionCallback(
            d_myCategory_p, &LockAndModifyWorker::dummyCallback);
...
public:
  // CREATORS
  explicit LockingPublisher(bslmt::Mutex *mutex) : d_mutex_p(mutex) {}
...
public:
  // CREATORS
  explicit TestPublisher(bslma::Allocator *allocator);
...
// CREATORS
inline TestPublisher::TestPublisher(bslma::Allocator *allocator)
    : d_numInvocations(0), d_recordBuffer(allocator),
      d_sortedRecords(allocator), d_sample(allocator),
      d_elapsedTimes(allocator) {}

inline TestPublisher::~TestPublisher() {}
...
// ACCESSORS
inline int TestPublisher::invocations() const { return d_numInvocations; }

inline const balm::MetricSample &TestPublisher::lastSample() const {
  return d_sample;
}

inline const bsl::set<bsls::TimeInterval> &
TestPublisher::lastElapsedTimes() const {
  return d_elapsedTimes;
}

inline const bdlt::DatetimeTz &TestPublisher::lastTimeStamp() const {
  return d_sample.timeStamp();
}

inline const bsl::vector<balm::MetricRecord> &
TestPublisher::lastRecords() const {
  return d_sortedRecords;
}

inline int TestPublisher::indexOf(const balm::MetricId &id) const {
  balm::MetricRecord searchRecord(id);
  bsl::vector<balm::MetricRecord>::const_iterator it = bsl::lower_bound(
      d_sortedRecords.begin(), d_sortedRecords.end(), searchRecord, recordLess);
  if (it == d_sortedRecords.end()) {
    return -1; // RETURN
  }
  return (it->metricId() == id) ? static_cast<int>(it - d_sortedRecords.begin())
                                : -1;
}

inline bool TestPublisher::contains(const balm::MetricId &id) const {
  return indexOf(id) != -1;
}
...
template <class T> class CombinationIterator {
...
// PRIVATE MANIPULATORS
template <class T> void CombinationIterator<T>::createCurrentCombination() {
...
CombinationIterator<T>::CombinationIterator(const bsl::vector<T> &values,
                                            bslma::Allocator *allocator)
    : d_values(values, allocator), d_currentCombination(allocator), d_bits(0),
...
// MANIPULATORS
template <class T> inline bool CombinationIterator<T>::next() {
  if (d_bits >= d_maxBits) {
    return false; // RETURN
  }
  ++d_bits;
...
// ACCESSORS
template <class T>
inline const typename bsl::vector<T> &CombinationIterator<T>::current() const {
  return d_currentCombination;
}

template <class T>
inline bool CombinationIterator<T>::includesElement(int index) const {
...
void stringId(bsl::string *resultId, const char *heading, int value1,
              int value2)
// Populate the specified 'resultId' with a null-terminated string
// identifier containing the specified 'heading' concatenated with the
...
public:
  // CREATORS
  ConcurrencyTest(int numThreads, balm::MetricsManager *manager,
                  bslma::Allocator *basicAllocator)
      : d_pool(numThreads, 1000, basicAllocator), d_barrier(numThreads),
        d_manager_p(manager), d_allocator_p(basicAllocator) {
    d_pool.start();
  }
...
void ConcurrencyTest::execute() {
  ...
  for (int i = 0; i < 10; ++i) {
    ...
    stringId(&uniqueString1, "U1",
             static_cast<int>(bslmt::ThreadUtil::selfIdAsInt()), i);
    stringId(&uniqueString2, "U2",
             static_cast<int>(bslmt::ThreadUtil::selfIdAsInt()), i);
    ...
    mX->collectSample(&sample, &records, allCategories.data(),
    ...
    // Test 'publish'.
    mX->publish(allCategories.data(), static_cast<int>(allCategories.size()));

    // Verify the callbacks were invoked.
    ASSERT(2 <= aCb.invocations());
    ASSERT(2 <= bCb.invocations());
    LOOP2_ASSERT(bslmt::ThreadUtil::selfIdAsInt(), s1Cb.invocations(),
                 2 == s1Cb.invocations());
    LOOP2_ASSERT(bslmt::ThreadUtil::selfIdAsInt(), s2Cb.invocations(),
                 2 == s2Cb.invocations());
...
public:
  // CREATORS
  explicit SimpleStreamPublisher(bsl::ostream &stream);
...
void SimpleStreamPublisher::publish(const balm::MetricSample &metricValues) {
  if (metricValues.numRecords() > 0) {
    d_stream << metricValues.timeStamp() << " " << metricValues.numRecords()
             << " Records" << bsl::endl;

    balm::MetricSample::const_iterator sIt = metricValues.begin();
    for (; sIt != metricValues.end(); ++sIt) {
      d_stream << "\tElapsed Time: "
               << sIt->elapsedTime().totalSecondsAsDouble() << "s" << bsl::endl;
      balm::MetricSampleGroup::const_iterator gIt = sIt->begin();
      for (; gIt != sIt->end(); ++gIt) {
        d_stream << "\t" << gIt->metricId() << " [count = " << gIt->count()
                 << ", total = " << gIt->total() << ", min = " << gIt->min()
...
public:
  ...
  //..
  explicit EventHandler(balm::MetricsManager *manager)
      : d_eventMessageSizes_p(
            manager->collectorRepository().getDefaultCollector("MyCategory",
                                                               "messageSizes")),
        d_eventFailures_p(manager->collectorRepository().getDefaultCollector(
  ...
  {
    int returnCode = 0;
    d_eventMessageSizes_p->update(static_cast<double>(eventMessage.size()));
...
public:
  // CREATORS
  explicit EventHandlerWithCallback(balm::MetricsManager *manager,
...
void EventHandlerWithCallback::collectMetricsCb(
    bsl::vector<balm::MetricRecord> *records, bool resetFlag) {
  int numEvents = resetFlag ? (int)d_numEvents.swap(0) : (int)d_numEvents;
...
EventHandlerWithCallback::EventHandlerWithCallback(
    balm::MetricsManager *manager, bslma::Allocator *basicAllocator)
    : d_numEvents(0), d_periodStart(bdlt::CurrentTime::now()),
      d_eventsPerSecId(),
  ...
  // 'balm::MetricsManager::CallbackHandle' returned for the registered
  // callback; we will use this handle later to remove the callback from the
  // metrics manager.
  //..
  d_callbackHandle = d_metricsManager_p->registerCollectionCallback(
      METRIC_CATEGORY,
      bdlf::BindUtil::bind(&EventHandlerWithCallback::collectMetricsCb, this,
                           bdlf::PlaceHolders::_1, bdlf::PlaceHolders::_2));
}
...
//..
EventHandlerWithCallback::~EventHandlerWithCallback() {
  int rc = d_metricsManager_p->removeCollectionCallback(d_callbackHandle);
  ASSERT(0 == rc);
}
...
int EventHandlerWithCallback::handleEvent(int eventId,
  ...
  // expensive (e.g., metrics requiring a system call to collect), clients
  // should test whether the metric is enabled before performing the collection
  // operation.
  //..
...
int main(int argc, char *argv[]) {
  ...
  case 26: {
    ...
    // 'SimpleStreamPublisher' object.  Note that,  'SimpleStreamPublisher' is
    // an example implementation of the 'balm::Publisher' protocol defined in
    // the 'balm_publisher' component, in practice clients can use a standard
    ...
    bsl::shared_ptr<balm::Publisher> publisher(
        new (*allocator) SimpleStreamPublisher(bsl::cout), allocator);
    ...
    ///- - - - - - - - - - - - - - - -
    // The metrics manager provides a 'publish' operation to publish metrics for
    // a category or set of categories.  In this example we will use the metrics
    // manager's 'publish' operation to publish metrics for "MyCategory".  We
    // will record metrics for "MyCategory" using instances of the
    // 'EventHandler' and 'EventHandlerWithCallback' classes (defined above).
    ...
    // mechanism (e.g., see the 'balm::PublicationScheduler').
    //..
    ...
    //         MyCategory.messageSize [count = 1, total = 28, min = 28, max =
    //         28] MyCategory.eventFailure [count = 0, total = 0, ... ]
    // 09FEB2009_18:52:51.096+0000 3 Records
    //         Elapsed Time: 0.002217s
    //         MyCategory.EventsPerSecond [count = 1, total = 453.721, ... ]
    //         MyCategory.messageSize [count = 2, total = 56, min = 28, max =
    //         28] MyCategory.eventFailure [count = 0, total = 0, ... ]
    //..
  ...
  case 25: {
    ...
    if (verbose)
      cout << endl << "TEST CONCURRENCY" << endl << "================" << endl;
...

    {
      bslmt::Mutex lock;
      bsl::shared_ptr<balm::Publisher> publisher(
          new (testAllocator) LockingPublisher(&lock), &testAllocator);
      balm::MetricsManager manager(&testAllocator);
  ...
  case 23: {
    ...
    if (veryVerbose)
      cout << "\tTest 'publish' with 'resetFlag' 'false'" << endl;
  ...
  case 22: {
    ...
    if (veryVerbose)
      cout << "\tTest collectSample() for all categories" << endl;

    {
      ...
      for (int i = 0; i < sample.numGroups(); ++i) {
        ...
        for (; record < group.records() + group.numRecords(); ++record) {
          ASSERT(0 == bsl::strcmp(record->metricId().categoryName(), name));
      ...
      for (int i = 0; i < NUM_CATEGORIES; ++i) {
        for (int j = 0; j < NUM_METRICS; ++j) {
          balm::MetricRecord record;
          Collector *col = rep.getDefaultCollector(CATEGORIES[i], METRICS[j]);
          col->load(&record);
          ASSERT(1 == record.count());
          ASSERT(1 == record.total());
          ASSERT(1 == record.min());
...

    {
      ...
      if (veryVerbose)
        cout << "\tTest collectSample() for some categories\n";
      ...
      do {
        ...
        mX.collectSample(&sample, &recordsBsl, cats.data(),
                         static_cast<int>(cats.size()), false);
        ...
        // Test with a reset.
        mX.collectSample(&sample, &recordsBsl2, cats.data(),
                         static_cast<int>(cats.size()), true);
  ...
  case 19: {
    ...
    bsl::set<balm::Publisher *> generalPublishers;
    {
      ...
      BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(testAllocator) {
        ...
        for (int i = 0; i < NUM_CATEGORIES; ++i) {
          bsl::vector<balm::Publisher *> publishers;
          ASSERT(0 == MX.findSpecificPublishers(&publishers, CATEGORIES[i]));
        }
      }
      BSLMA_TESTALLOCATOR_EXCEPTION_TEST_END
    }
  ...
  case 18: {
    ...
    for (int i = 0; i < NUM_CATEGORIES; ++i) {
      ...
      bsl::vector<balm::Publisher *>::const_iterator expIt = results.begin();
  ...
  case 17: {
    ...
    for (int i = 0; i < NUM_CATEGORIES; ++i) {
      ...
      ASSERT(NUM_PUBLISHERS == MX.findSpecificPublishers(&exp_results, CAT));
      ASSERT(NUM_PUBLISHERS == MX.findSpecificPublishers(&results, CATEGORY));
    ...
    for (int i = 0; i < NUM_CATEGORIES; ++i) {
      ...
      for (int j = 0; j < NUM_METRICS; ++j) {
        ...
        for (; it != callbacks.end(); ++it) {
          ...
          const int EXP = (info.first == Obj::e_INVALID_HANDLE) ? 0 : 1;
  ...
  case 14: {
    ...
    do {
      ...
      for (int i = 0; i < NUM_CATEGORIES; ++i) {
        for (int j = 0; j < NUM_METRICS; ++j) {
          Collector *col =
              repository.getDefaultCollector(CATEGORIES[i], METRICS[j]);
          col->reset();
          col->update(1);
        }
  ...
  case 13: {
    ...
    do {
      ...
      ASSERT(static_cast<int>(combIt.current().size()) == sample.numGroups());
      ...
      for (bsl::size_t i = 0; i < combIt.current().size(); ++i) {
        ...
        for (int j = 0; j < sample.numGroups(); ++j) {
          ...
          if (CATEGORY == group.records()->metricId().category()) {
            ASSERT(-1 == groupIndex);
            ASSERT(withinWindow(group.elapsedTime(), elapsedTime, 10));
            groupIndex = j;
          }
          for (int k = 0; k < group.numRecords(); ++k) {
            ASSERT((groupIndex == j) ==
                   (CATEGORY == group.records()[k].metricId().category()));
          }
        }
        ASSERT(-1 != groupIndex);
      }
    ...
    for (int li = u::e_BEGIN; li < u::e_END; ++li) {
      ...
      do {
        ...
        if (0 == sz) {
          ...
          ASSERT(1 == gPub.invocations());
          ASSERT(withinWindow(gPub.lastTimeStamp(), tmStamp, 10));
          ASSERT(static_cast<int>(sz) == gPub.lastSample().numGroups());
        }
      ...
      ASSERT(u::e_BSL != libType || 0 == defaultAllocator.numBytesInUse());
    }
  } break;
  case 8: {
    // --------------------------------------------------------------------
    ...
    do {
      ...
      for (int i = 0; i < NUM_CATEGORIES; ++i) {
        for (int j = 0; j < NUM_METRICS; ++j) {
          Collector *col =
              repository.getDefaultCollector(CATEGORIES[i], COL_METRICS[j]);
          col->reset();
          col->update(1);
        }
      }
      ...
      if (categories.size() > 0) {
        mX.publish(categories.data(), static_cast<int>(categories.size()));
      }
      ...
      for (int i = 0; i < NUM_CATEGORIES; ++i) {
        for (int j = 0; j < NUM_METRICS; ++j) {
          ...
          Collector *collector = repository.getDefaultCollector(colId);
        ...
        for (bsl::size_t j = 0; j < specificPublishers.size(); ++j) {
          ...
          if (combIt.includesElement(i)) {
            ...
            for (int k = 0; k < NUM_METRICS; ++k) {
              Id colId = registry.getId(CATEGORIES[i], COL_METRICS[k]);
              Id cbId = registry.getId(CATEGORIES[i], CB_METRICS[k]);
      ...
      ASSERT(NUM_PUBLISHERS == MX.findGeneralPublishers(&generalPublishers));
      for (int i = 0; i < NUM_PUBLISHERS; ++i) {
        ...
        for (int j = 0; j < NUM_CATEGORIES; ++j) {
          for (int k = 0; k < NUM_METRICS; ++k) {
            Id colId = registry.getId(CATEGORIES[j], COL_METRICS[k]);
            Id cbId = registry.getId(CATEGORIES[j], CB_METRICS[k]);
            ASSERT(combIt.includesElement(j) == pub->contains(colId));
            ASSERT(combIt.includesElement(j) == pub->contains(cbId));
          }
        }
        pub->reset();
      }
  ...
  case 7: {
    ...
    for (int li = u::e_BEGIN; li < u::e_END; ++li) {
      ...
      if (veryVerbose)
        cout << "Test general publishers" << bsl::endl;
      ...
      for (int i = 0; i < NUM_PUBS; ++i) {
        ...
        for (int j = 0; j < NUM_CATEGORIES; ++j) {
          ...
          switch (libType) {
          case u::e_BSL: {
            ASSERT(0 == mX.findSpecificPublishers(&pubsFoundBsl, CAT));
            ASSERT(0 == pubsFoundBsl.size());
          } break;
          case u::e_STD: {
            ASSERT(0 == mX.findSpecificPublishers(&pubsFoundStd, CAT));
            ASSERT(0 == pubsFoundStd.size());
          } break;
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
          case u::e_PMR: {
            ASSERT(0 == mX.findSpecificPublishers(&pubsFoundPmr, CAT));
      ...
      for (int i = 0; i < NUM_CATEGORIES; ++i) {
        const Category *CAT = registry.getCategory(CATEGORIES[i]);
        mX.collectorRepository().getDefaultCollector(CATEGORIES[i], "A");
        mX.publish(CAT);
    ...
    for (int li = u::e_BEGIN; li < u::e_END; ++li) {
      ...
      if (veryVerbose)
        cout << "Test specific publishers" << bsl::endl;

      for (int i = 0; i < NUM_CATEGORIES; ++i) {
        const Category *CAT = registry.getCategory(CATEGORIES[i]);
        mX.collectorRepository().getDefaultCollector(CATEGORIES[i], "A");
        for (int j = 0; j < NUM_PUBS; ++j) {
          PubPtr pub_p(TEST_PUBS[j], bslstl::SharedPtrNilDeleter(), Z);
          ASSERT(0 == mX.addSpecificPublisher(CAT, pub_p));
          ...
          case u::e_BSL: {
            ...
            ASSERT(0 == pubsFoundBsl.size());
            ASSERT(j + 1 == MX.findSpecificPublishers(&pubsFoundBsl, CAT));
            pb = u::vDataBegin(pubsFoundBsl);
            pe = u::vDataEnd(pubsFoundBsl);
          } break;
          case u::e_STD: {
            ...
            ASSERT(0 == pubsFoundStd.size());
            ASSERT(j + 1 == MX.findSpecificPublishers(&pubsFoundStd, CAT));
            pb = u::vDataBegin(pubsFoundStd);
            pe = u::vDataEnd(pubsFoundStd);
          } break;
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
          case u::e_PMR: {
            ...
            ASSERT(0 == pubsFoundPmr.size());
            ASSERT(j + 1 == MX.findSpecificPublishers(&pubsFoundPmr, CAT));
  ...
  case 4: {
    ...
    }
    {
      ...
      for (int i = 0; i < NUM_VALUES; ++i) {
        ...
        bdlt::Date dt =
            bdlt::DateUtil::convertFromYYYYMMDDRaw(VALUES[i].d_timeStamp);
        bdlt::DatetimeTz TIME_STAMP(bdlt::Datetime(dt), 0);

        sample.setTimeStamp(TIME_STAMP);
        sample.appendGroup(records.data(), static_cast<int>(records.size()),
                           ELAPSED_TIME);
        ...
        ASSERT(i + 1 == static_cast<int>(tp1.lastElapsedTimes().size()));
        ASSERT(i + 1 == static_cast<int>(tp2.lastElapsedTimes().size()));
        ...
        for (int j = 0; j < sample.numGroups(); ++j) {
          ...
          ASSERT(
              tp1.lastElapsedTimes().end() !=
              tp1.lastElapsedTimes().find(sample.sampleGroup(j).elapsedTime()));
          ASSERT(
              tp2.lastElapsedTimes().end() !=
              tp2.lastElapsedTimes().find(sample.sampleGroup(j).elapsedTime()));
        ...
        bsl::vector<balm::MetricRecord>::const_iterator it = allRecords.begin();
    ...
    }
    {
      ...
      const char *COMBINATIONS[] = {
          COM_0, COM_1, COM_2,  COM_3,  COM_4,  COM_5,  COM_6,  COM_7,
          COM_8, COM_9, COM_10, COM_11, COM_12, COM_13, COM_14, COM_15};
      ...
      do {
        ...
        for (int j = 0; j < static_cast<int>(bsl::strlen(VALUES)); ++j) {
  ...
  case 1: {
    ...
    TestPublisher tpa_1(Z), tpa_2(Z), tpb_1(Z), tpb_2(Z), tpc_1(Z), tpc_2(Z);
    ...
    if (verbose) {
      ...
      cout << "\tCreate a metrics manager register data" << endl;
    }
    ...
    TestCallback tcba_1(METRIC_A, Z), tcba_2(METRIC_A, Z), tcbb_1(METRIC_B, Z),
        tcbb_2(METRIC_B, Z), tcbc_1(METRIC_C, Z), tcbc_2(METRIC_C, Z);
