  ...
  {                                                                            \
    {                                                                          \
      ...
      if (veryVerbose && firstTime)                                            \
        cout << "### BALM RETRY TEST" << endl;                                 \
      firstTime = 0;                                                           \
    }                                                                          \
    if (veryVeryVerbose)                                                       \
      cout << "### Begin balm retry test." << endl;                            \
    ...
    do {                                                                       \
      if (0 != __attempt && !__quietMode) {                                    \
        cout << "Test failure; retry.  Attempt: " << __attempt + 1 << " of "   \
             << __numAttempts << endl;                                         \
      }                                                                        \
...
#define BALM_END_RETRY_TEST                                                    \
  ...
  if (__savedStatus != testStatus) {                                           \
    if (!__quietMode) {                                                        \
      cout << "RETRY_TEST failure after " << __numAttempts << " attempts"      \
           << endl;                                                            \
    }                                                                          \
  }                                                                            \
  }
...
typedef bsl::pair<const balm::Category *, bsls::TimeInterval> ScheduleElement;
...
typedef bsl::map<const Category *, bsls::TimeInterval> CategoryScheduleOracle;
typedef bsl::map<bsls::TimeInterval, bsl::set<const balm::Category *>>
    ScheduleOracle;
...
#define BEGIN_BALM_EXCEPTION_TEST                                              \
  {                                                                            \
    {                                                                          \
      static int firstTime = 1;                                                \
      if (veryVerbose && firstTime)                                            \
        cout << "### BALM EXCEPTION TEST -- (ENABLED) --" << endl;             \
      firstTime = 0;                                                           \
    }                                                                          \
    if (veryVeryVerbose)                                                       \
      cout << "### Begin balm exception test." << endl;                        \
...
#define END_BALM_EXCEPTION_TEST                                                \
    ...
    if ((veryVerbose && balmExceptionLimit) || veryVeryVerbose) {              \
      ...
      if (veryVeryVerbose) {                                                   \
        cout << " BEDMA_EXCEPTION: " << "alloc limit = "                       \
             << balmExceptionCounter << ", "                                   \
  ...
  while (1)                                                                    \
    ;                                                                          \
  if (veryVeryVerbose)                                                         \
    cout << "### End balm exception test." << endl;                            \
  }
#else
#define BEGIN_BALM_EXCEPTION_TEST                                              \
  {                                                                            \
    ...
    if (verbose && firstTime) {                                                \
      cout << "### BALM EXCEPTION TEST -- (NOT ENABLED) --" << endl;           \
      firstTime = 0;                                                           \
    }                                                                          \
  }
#define END_BALM_EXCEPTION_TEST
...
template <class PAIR> struct IsPair {
...
private:
  ...
  typedef bsl::map<InvocationId, InvocationSummary, InvocationIdLess>
      InvocationMap;
...
public:
  ...
  findInvocation(const bsl::set<const balm::Category *> &categories) const;
...
// CREATORS
inline TestPublisher::InvocationSummary::InvocationSummary()
    : d_numInvocations(0), d_timeStamp(), d_elapsedTime() {}
// -------------------------------------
...
inline TestPublisher::TestPublisher(bslma::Allocator *allocator)
    : d_numInvocations(0), d_invocations(allocator), d_timeStamp(),
...
inline void TestPublisher::reset() {
  d_numInvocations = 0;
  d_invocations.clear();
  d_elapsedTimes.clear();
}
...
inline int TestPublisher::uniqueInvocations() const {
  return static_cast<int>(d_invocations.size());
}

inline const TestPublisher::InvocationSummary *TestPublisher::findInvocation(
...
inline const bsls::TimeInterval &TestPublisher::lastElapsedTime() const {
...
bsl::ostream &TestPublisher::print(bsl::ostream &stream, int level,
  ...
  for (; it != d_invocations.end(); ++it) {
    ...
    stream << "[ " << it->second.d_numInvocations << " "
           << it->second.d_timeStamp << " " << it->second.d_elapsedTime
           << " ( ";
    bsl::set<const balm::Category *>::const_iterator cIt = it->first.begin();
...
const char *Action::typeString(Type type) {
  switch (type) {
  case Action::SCHEDULE_CATEGORY:
    return "SCHEDULE_CATEGORY";
  case Action::SCHEDULE_DEFAULT:
...
// CREATORS
inline Action::Action(Type type, const balm::Category *category, int interval)
    : d_type(type), d_category_p(category), d_interval(interval) {}

// ACCESSORS
inline Action::Type Action::type() const { return d_type; }

inline const balm::Category *Action::category() const { return d_category_p; }

inline int Action::interval() const { return d_interval; }
...
public:
  // CREATORS
  ConcurrencyTest(int numThreads, balm::PublicationScheduler *scheduler,
                  bdlmt::TimerEventScheduler *eventScheduler,
                  bslma::Allocator *basicAllocator)
      : d_pool(numThreads, 1000, basicAllocator), d_barrier(numThreads),
        d_scheduler_p(scheduler), d_eventScheduler_p(eventScheduler),
...
void ConcurrencyTest::execute() {
  ...
  for (int testRun = 0; testRun < 2; ++testRun) {
    ...
    for (int i = 0; i < 50; ++i) {
      ...
      ASSERT(!defaultSet || bsls::TimeInterval(5) == foundInterval ||
    ...
    for (int i = 0; i < 50; ++i) {
      ...
      ASSERT(!found || foundInterval == bsls::TimeInterval(3) ||
             foundInterval == bsls::TimeInterval(1));
      found = MX.findCategorySchedule(&foundInterval, "D");
      ASSERT(!found || foundInterval == bsls::TimeInterval(4) ||
             foundInterval == bsls::TimeInterval(1));
      found = MX.findCategorySchedule(&foundInterval, S1);
      ASSERT(!found || foundInterval == bsls::TimeInterval(INTERVAL_1));
      found = MX.getDefaultSchedule(&foundInterval);
      ASSERT(!found || foundInterval == bsls::TimeInterval(5) ||
             foundInterval == bsls::TimeInterval(1));
...
// information about the lowest index where the two values differ.
{
  ...
  bsl::cout << "expcetedValue[" << index << "] = " << (int)*exp << " (" << *exp
            << ")" << bsl::endl
            << "actualValue[" << index << "] = " << (int)*act << " (" << *act
            << ")" << bsl::endl;
...
void gg(bsl::vector<Action> *actions, balm::MetricRegistry &registry,
    ...
    for (; sIt != metricValues.end(); ++sIt) {
      if (sIt == metricValues.begin() ||
        ...
        d_stream << "\tElapsed Time: "
                 << sIt->elapsedTime().totalSecondsAsDouble() << "s"
                 << bsl::endl;
      }
      balm::MetricSampleGroup::const_iterator gIt = sIt->begin();
      for (; gIt != sIt->end(); ++gIt) {
...
int main(int argc, char *argv[]) {
  ...
  case 13: {
    ...
    // 'balm::PublicationScheduler'. Before instantiating the publication
    // scheduler, we create a 'bdlmt::TimerEventScheduler' as well as a
    // 'balm::MetricsManager'.  We obtain collectors for three different metric
    // categories, "A", "B", and "C", that we will use to generate metric values
    // for publication.
    //..
    ...
    balm::Collector *B =
        manager.collectorRepository().getDefaultCollector("B", "1");
    balm::Collector *C =
        manager.collectorRepository().getDefaultCollector("C", "1");
    //..
    // We now create an instance of 'SimpleStreamPublisher', which implements
    // the 'balm::Publisher' protocol)  Note that, 'SimpleStreamPublisher' is an
    ...
    //..
    // We now register the 'publisher' we have created with the metrics
    // 'manager' to publish our categories.  Then, we 'start' the timer-event
    // scheduler we will supply to the 'balm::PublicationScheduler'.
    //..
    manager.addGeneralPublisher(publisher);
    timer.start();
    //..
    // Now we construct a 'balm::PublicationScheduler' and pass it the
    // respective addresses of both the metrics manager and the timer-event
    // scheduler.  We schedule the publication of category "A" and "B" every .05
    // seconds, then we set the default publication to every .10 seconds.  Note
    // that those time intervals were chosen to ensure fast and consistent
    // output for this example.  In normal usage the interval between
    // publications should be large enough to ensure that metric publication
    // does not negatively affect the performance of the application (a 30
    // second interval is typical).
    //..
    ...
    // 'balm::MetricsManager::publish' is invoked.  Also note that category 'C'
    // is published as part of the scheduled default publication.
  ...
  case 11: {
    ...
    for (int i = 0; i < NUM_SPECS; ++i) {
      BEGIN_BALM_EXCEPTION_TEST {
        ...
        ASSERT((defaultInterval == INVALID) || (defaultInterval == interval));
        ...
        int uniqueIntervals =
            countUniqueIntervals(intervalMap, defaultInterval);
  ...
  case 8: {
    ...
    for (int i = 0; i < NUM_SPECS; ++i) {
      ...
      typedef std::pair<const balm::Category *, bsls::TimeInterval> PairStd;
  ...
  case 7: {
    ...
    for (int i = 0; i < NUM_SPECS; ++i) {
      ...
      ASSERT(!MX.getDefaultSchedule(&interval));
      for (int j = 0; j < NUM_CATEGORIES; ++j) {
        ASSERT(!MX.findCategorySchedule(&interval, allCategories[j]));
      }
      ASSERT(0 == timer.numClocks());
      ASSERT(0 == timer.numEvents());
    }
    ...
    for (int i = 0; i < NUM_SPECS; ++i) {
      ...
      for (bsl::size_t j = 0; j < actions.size(); ++j) {
        ...
        for (bsl::size_t k = 0; k < elementsToRemove.size(); ++k) {
          ...
          bsls::TimeInterval interval;
          ASSERT((expDefault != INVALID) == MX.getDefaultSchedule(&interval));
          ...
          for (int l = 0; l < NUM_CATEGORIES; ++l) {
            ...
            ASSERT(found == MX.findCategorySchedule(&interval, CAT));
            ASSERT((!found) || interval == expSchedule[CAT]);
          }

          // Verify the correct number of clocks in the underlying
          // event scheduler.
          int uniqueIntervals = countUniqueIntervals(expSchedule, expDefault);
          ASSERT(uniqueIntervals == timer.numClocks());
          ASSERT(0 == timer.numEvents());
        }
        ASSERT(0 == timer.numClocks());
  ...
  case 5: {
    ...
    const char *TEST_SPECS[] = {"A1",       "A1B2",     "A1X1",    "A1X2A2X1",
                                "A1A2X2X1", "A1B2A2B2", "A1B2B1A2"};
    const int NUM_SPECS = sizeof TEST_SPECS / sizeof *TEST_SPECS;
    ...
    for (int i = 0; i < NUM_SPECS; ++i) {
      BALM_BEGIN_RETRY_TEST(3) {
        ...
        ScheduleOracle schedule(Z);
        {
          ...
          CategoryScheduleOracle::const_iterator csIt = catSchedule.begin();
          ...
          if (defaultInterval != INVALID) {
            bsl::insert_iterator<bsl::set<const Category *>> iIt(
                schedule[defaultInterval], schedule[defaultInterval].begin());
            bsl::set_difference(allCategories.begin(), allCategories.end(),
                                nonDefaultCategories.begin(),
                                nonDefaultCategories.end(), iIt);
          }
        }
        ...
        const int WAIT_MICRO =
            static_cast<int>((TIME_UNIT_MICRO * 8) + (TIME_UNIT_MICRO / 3));
        ...
        ASSERT(tp.uniqueInvocations() == static_cast<int>(schedule.size()));
        ...
        for (; it != schedule.end(); ++it) {
          ...
          LOOP2_ASSERT(expInvocations, invocation->d_numInvocations,
  ...
  case 3: {
    ...
    for (int i = 0; i < NUM_SPECS; ++i) {
      ...
      // 'intervalMap').
      bsls::TimeInterval interval;
      ASSERT((defaultInterval != INVALID) == MX.getDefaultSchedule(&interval));
      ASSERT((defaultInterval == INVALID) || (defaultInterval == interval));
      for (int i = 0; i < NUM_CATEGORIES; ++i) {
      ...
      // scheduler.
      int uniqueIntervals = countUniqueIntervals(intervalMap, defaultInterval);
      ASSERT(uniqueIntervals == timer.numClocks());
      ASSERT(0 == timer.numEvents());
      ASSERT(daNumBytes == defaultAllocator.numBytesInUse());
    }
  ...
  case 2: {
    ...
    }
    {
      ...
      {
        static int count = 0;
        BALM_BEGIN_RETRY_TEST_IMP(5, !verbose) { ++count; }
        BALM_END_RETRY_TEST
        ASSERT(1 == count);
      }
      {
