...
inline bsls::TimeInterval makeInvalidInterval()
...
struct ScheduleElementSort {
  ...
  // ACCESSOR
  bool operator()(const ScheduleElement &lhs,
                  const ScheduleElement &rhs) const {
...
bsl::ostream &
printCategorySet(bsl::ostream &stream,
...
int getCategoryScheduleImp(VECTOR *result, bslmt::Mutex *mutex,
...
class PublicationScheduler_ClockData {
  ...
  bdlmt::TimerEventScheduler::Handle d_handle; // handle to the associated clock

  bsl::set<const Category *> d_categories; // set of categories published
...
private:
  ...
  operator=(const PublicationScheduler_ClockData &);
...
PublicationScheduler_ClockData::PublicationScheduler_ClockData(
    ...
    : d_handle(bdlmt::TimerEventScheduler::e_INVALID_HANDLE),
      d_categories(basicAllocator), d_default(false),
      d_nonDefaultCategories(basicAllocator) {}

PublicationScheduler_ClockData::~PublicationScheduler_ClockData() {}

// MANIPULATORS
inline bslmt::Mutex *PublicationScheduler_ClockData::mutex() {
  return &d_mutex;
}

inline bdlmt::TimerEventScheduler::Handle &
PublicationScheduler_ClockData::handle() {
  return d_handle;
}

inline bsl::set<const Category *> &
PublicationScheduler_ClockData::categories() {
  return d_categories;
}

inline bool &PublicationScheduler_ClockData::defaultClock() {
  return d_default;
}
...
private:
  ...
  PublicationScheduler_Proctor &operator=(const PublicationScheduler_Proctor &);
...
// CREATORS
inline PublicationScheduler_Proctor::PublicationScheduler_Proctor(
    PublicationScheduler *scheduler)
    : d_scheduler_p(scheduler) {}

PublicationScheduler_Proctor::~PublicationScheduler_Proctor() {
  if (d_scheduler_p) {
    PublicationScheduler::Clocks::iterator it = d_scheduler_p->d_clocks.begin();
    for (; it != d_scheduler_p->d_clocks.end(); ++it) {
      ...
      if (bdlmt::TimerEventScheduler::e_INVALID_HANDLE !=
          it->second->handle()) {
        d_scheduler_p->d_scheduler_p->cancelClock(it->second->handle(), true);
      }
    }
...
// MANIPULATORS
inline void PublicationScheduler_Proctor::release() { d_scheduler_p = 0; }
...
PublicationScheduler::PublicationScheduler(
    MetricsManager *metricsManager, bdlmt::TimerEventScheduler *eventScheduler,
    bslma::Allocator *basicAllocator)
    : d_scheduler_p(eventScheduler), d_manager_p(metricsManager),
      d_categories(basicAllocator), d_clocks(basicAllocator),
      d_defaultInterval(u::makeInvalidInterval()), d_mutex(),
      d_allocator_p(bslma::Default::allocator(basicAllocator)) {}

PublicationScheduler::~PublicationScheduler() { cancelAll(); }

// MANIPULATORS
void PublicationScheduler::scheduleCategory(
    const Category *category, const bsls::TimeInterval &interval) {
  ...
  if (clockIt == d_clocks.end()) {
    ...
    clock->handle() = d_scheduler_p->startClock(
        interval,
        bdlf::BindUtil::bind(&PublicationScheduler::publish, this, clock));
  } else {
  ...
  if (!clock->defaultClock() && d_defaultInterval != u::makeInvalidInterval()) {
    Clocks::iterator dfltIt = d_clocks.find(d_defaultInterval);
    BSLS_ASSERT(d_clocks.end() != dfltIt);
...
bool PublicationScheduler::findCategorySchedule(
    bsls::TimeInterval *result, const Category *category) const {
  bslmt::LockGuard<bslmt::Mutex> guard(&d_mutex);
...
bool PublicationScheduler::getDefaultSchedule(
...
int PublicationScheduler::getCategorySchedule(
    bsl::vector<bsl::pair<const Category *, bsls::TimeInterval>> *result)
    const {
  return u::getCategoryScheduleImp(result, &d_mutex, d_categories);
}

int PublicationScheduler::getCategorySchedule(
    std::vector<std::pair<const Category *, bsls::TimeInterval>> *result)
    const {
  return u::getCategoryScheduleImp(result, &d_mutex, d_categories);
}
...
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
int PublicationScheduler::getCategorySchedule(
    std::experimental::pmr::vector<
        std::pair<const Category *, bsls::TimeInterval>> *result) const {
  return u::getCategoryScheduleImp(result, &d_mutex, d_categories);
}
#endif

bsl::ostream &PublicationScheduler::print(bsl::ostream &stream, int level,
  ...
  for (; sIt != schedule.end(); ++sIt) {
    ...
    stream << "[ " << sIt->first->name() << " -> " << sIt->second << " ]"
           << SEP;
  }
  bdlb::Print::indent(stream, level + 1, spacesPerLevel);
  stream << "]" << SEP;
