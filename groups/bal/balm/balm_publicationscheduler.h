...
class PublicationScheduler {
  ...
  typedef bsl::map<bsls::TimeInterval, bsl::shared_ptr<ClockData>> Clocks;
...
// MANIPULATORS
inline MetricsManager *PublicationScheduler::manager() { return d_manager_p; }

inline void
PublicationScheduler::scheduleCategory(const char *category,
                                       const bsls::TimeInterval &interval) {
  scheduleCategory(d_manager_p->metricRegistry().getCategory(category),
                   interval);
}

inline int PublicationScheduler::cancelCategorySchedule(const char *category) {
  return cancelCategorySchedule(
      d_manager_p->metricRegistry().getCategory(category));
}

// ACCESSORS
inline const MetricsManager *PublicationScheduler::manager() const {
  return d_manager_p;
}
...
PublicationScheduler::findCategorySchedule(bsls::TimeInterval *result,
  ...
  return findCategorySchedule(
      result, d_manager_p->metricRegistry().getCategory(category));
}
