...
template <class VECTOR, class MEMBER> struct IsVector {
...
inline void combine(balm::MetricRecord *record,
                    const balm::MetricRecord &value) {
...
template <class COLLECTOR> class CollectorRepository_Collectors {
  ...
  operator=(const CollectorRepository_Collectors &);
...
public:
  ...
  // ACCESSORS
  template <class VECTOR> int getAddedCollectors(VECTOR *collectors) const;
...
CollectorRepository_Collectors<COLLECTOR>::CollectorRepository_Collectors(
    const MetricId &metricId, bslma::Allocator *basicAllocator)
    : d_defaultCollector(metricId), d_addedCollectors(basicAllocator),
      d_allocator_p(bslma::Default::allocator(basicAllocator)) {}

template <class COLLECTOR>
inline CollectorRepository_Collectors<
    COLLECTOR>::~CollectorRepository_Collectors() {}
...
CollectorRepository_Collectors<COLLECTOR>::addCollector() {
  Collector collectorPtr(new (*d_allocator_p)
                             COLLECTOR(d_defaultCollector.metricId()),
                         d_allocator_p);
...
template <class COLLECTOR>
void CollectorRepository_Collectors<COLLECTOR>::collectAndReset(
    MetricRecord *record) {
...
template <class COLLECTOR>
void CollectorRepository_Collectors<COLLECTOR>::collect(MetricRecord *record) {
...
int CollectorRepository_Collectors<COLLECTOR>::getAddedCollectors(
...
class CollectorRepository_MetricCollectors {
  ...
  // PRIVATE TYPES
  typedef CollectorRepository_Collectors<Collector> Collectors;
  typedef CollectorRepository_Collectors<IntegerCollector> IntCollectors;
  ...
  CollectorRepository_MetricCollectors &
  operator=(const CollectorRepository_MetricCollectors &);
...
public:
  ...
  const CollectorRepository_Collectors<IntegerCollector> &intCollectors() const;
...
// CREATORS
inline CollectorRepository_MetricCollectors::
    CollectorRepository_MetricCollectors(const MetricId &id,
                                         bslma::Allocator *basicAllocator)
    : d_collectors(id, basicAllocator), d_intCollectors(id, basicAllocator) {}

inline CollectorRepository_MetricCollectors::
    ~CollectorRepository_MetricCollectors() {}

// MANIPULATORS
inline CollectorRepository_Collectors<Collector> &
CollectorRepository_MetricCollectors::collectors() {
  return d_collectors;
}

inline CollectorRepository_Collectors<IntegerCollector> &
...
// ACCESSORS
inline const CollectorRepository_Collectors<Collector> &
CollectorRepository_MetricCollectors::collectors() const {
  return d_collectors;
}

inline const CollectorRepository_Collectors<IntegerCollector> &
CollectorRepository_MetricCollectors::intCollectors() const {
  return d_intCollectors;
}

inline const MetricId &CollectorRepository_MetricCollectors::metricId() const {
...
template <class VECTOR>
inline void CollectorRepository::collectAndResetImp(VECTOR *records,
...
template <class VECTOR>
inline void CollectorRepository::collectImp(VECTOR *records,
                                            const Category *category) {
  BSLMF_ASSERT((u::IsVector<VECTOR, balm::MetricRecord>::value));
...
CollectorRepository::getMetricCollectors(const MetricId &metricId) {
  ...
  if (cIt == d_collectors.end()) {
    ...
    cIt = d_collectors.insert(bsl::make_pair(metricId, collectorsPtr)).first;
...
IntegerCollector *
CollectorRepository::getDefaultIntegerCollector(const MetricId &metricId) {
  // First, obtain a read-lock, and test if the 'MetricCollectors' object
  // for 'metricId' already exists.
  {
    bslmt::ReadLockGuard<bslmt::RWMutex> guard(&d_rwMutex);
...
bsl::shared_ptr<Collector>
CollectorRepository::addCollector(const MetricId &metricId) {
  bslmt::WriteLockGuard<bslmt::RWMutex> guard(&d_rwMutex);
...
int CollectorRepository::getAddedCollectors(
  ...
  if (cIt != d_collectors.end()) {
    ...
    numFound += cIt->second->intCollectors().getAddedCollectors(intCollectors);
  }
  return numFound;
}
...
int CollectorRepository::getAddedCollectors(
    ...
    std::experimental::pmr::vector<bsl::shared_ptr<IntegerCollector>>
        *intCollectors,
    const MetricId &metricId) {
  int numFound = 0;
  bslmt::ReadLockGuard<bslmt::RWMutex> guard(&d_rwMutex);
