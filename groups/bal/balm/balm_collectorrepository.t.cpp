...
bool recordLess(const Rec &lhs, const Rec &rhs) {
  int cmp =
      bsl::strcmp(lhs.metricId().categoryName(), rhs.metricId().categoryName());
  if (0 == cmp) {
    cmp = bsl::strcmp(lhs.metricId().metricName(), rhs.metricId().metricName());
  }
  return cmp < 0;
}
...
public:
  // CREATORS
  ThreadTester(int numThreads, balm::CollectorRepository *repository,
               bslma::Allocator *basicAllocator)
      : d_pool(numThreads, 1000, basicAllocator), d_barrier(numThreads),
        d_repository_p(repository), d_allocator_p(basicAllocator) {
    d_pool.start();
  }
...
void ThreadTester::execute() {
  ...
  Obj &mX = *d_repository_p;
  const Obj &MX = mX;
  Registry &reg = mX.registry();
  const Registry &REG = MX.registry();
  ...
  for (int i = 0; i < 10; ++i) {
    ...
    const bsl::size_t NUM_CATEGORIES = sizeof(CATEGORIES) / sizeof(*CATEGORIES);
...
void ThreadTester::runTest() {
  bsl::function<void()> job =
      bdlf::BindUtil::bind(&ThreadTester::execute, this);
...
int main(int argc, char *argv[]) {
  ...
  case 9: {
    ...
    // then looking up collectors in that repository, and finally collecting
    // values from the repository.  We start by creating creating a repository
    // and looking up 2 collectors and 2 integer collectors.
    //..
    ...
    //..
    // We can use the repository to collect recorded values from the collectors
    // it manages.  Since there are collectors for four metrics, there should be
    // four recorded values.  Note the order in which the records are returned
    // is undefined.
    //..
  ...
  case 6: {
    ...
    {
      if (veryVerbose) {
        cout << "\tTest 'getDefaultCollector' followed by" << " 'addCollector'"
             << endl;
      }
  ...
  case 5: {
    ...
    {
      if (veryVerbose) {
        cout << "\tTest 'addCollector' creates valid new collectors." << endl;
      }
      ...
      for (; it != ids.end(); ++it) {
...

        {
          ...
          bsl::set<Col *> colSet(exp_ColV.begin(), exp_ColV.end());
          bsl::set<ICol *> iColSet(exp_iColV.begin(), exp_iColV.end());
  ...
  case 3: {
    ...
    {
      if (veryVerbose) {
        cout << "\tTest 'addCollector' followed by " << "'getDefaultCollector'"
             << endl;
      }
  ...
  case 2: {
    ...
    if (verbose)
      cout << "\nTesting basic mainpulator and accessors." << endl;
    ...
    }
    {
      ...
      IColSPtr dummyICol((ICol *)0xDEAD, bslstl::SharedPtrNilDeleter(), 0);
      ...
      for (int i = 0; i < NUM_METRICS; ++i) {
        ...
        const int NUM = mX.getAddedCollectors(&colV, &iColV, METRIC_AA);
  ...
  case 1: {
    ...
    Id METRIC_BB = reg.getId("B", "B");
    {
      ...
      if (veryVerbose) {
        cout << "\tTest addCollector(const balm::MetricId& )" << bsl::endl;
      }
    ...
    }
    {
      ...
      if (veryVerbose) {
        cout << "\tTest getDefaultIntegerCollector(const "
             << "bslstl::StringRef &, const bslstl::StringRef& )" << bsl::endl;
      }
    ...
    }
    {
      ...
      int num = mX.getAddedCollectors(&collectors, &intCollectors, METRIC_AA);
