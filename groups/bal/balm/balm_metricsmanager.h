...
private:
  ...
  void collectSampleImp(MetricSample *sample,
                        ...
                        const Category *const categories[], int numCategories,
...
public:
  // CREATORS
  explicit MetricsManager(bslma::Allocator *basicAllocator = 0);
  ...
  // MANIPULATORS
  CallbackHandle
  registerCollectionCallback(const char *categoryName,
  ...
  void collectSample(MetricSample *sample, bsl::vector<MetricRecord> *records,
                     bool resetFlag = false);
  void collectSample(MetricSample *sample, bsl::vector<MetricRecord> *records,
                     const Category *const categories[], int numCategories,
                     bool resetFlag = false);
  ...
  void publish(const Category *const categories[], int numCategories,
               bool resetFlag = true);
...
// MANIPULATORS
inline MetricsManager::CallbackHandle
MetricsManager::registerCollectionCallback(
    const char *categoryName, const RecordsCollectionCallback &callback) {
  return registerCollectionCallback(d_metricRegistry.getCategory(categoryName),
                                    callback);
}

inline int MetricsManager::addSpecificPublisher(
...
inline void MetricsManager::setCategoryEnabled(const Category *category,
                                               bool value) {
  d_metricRegistry.setCategoryEnabled(category, value);
}

inline void MetricsManager::setCategoryEnabled(const char *category,
                                               bool isEnabled) {
  d_metricRegistry.setCategoryEnabled(d_metricRegistry.getCategory(category),
                                      isEnabled);
}

inline void MetricsManager::setAllCategoriesEnabled(bool value) {
  d_metricRegistry.setAllCategoriesEnabled(value);
}

inline CollectorRepository &MetricsManager::collectorRepository() {
  return d_collectors;
}

inline MetricRegistry &MetricsManager::metricRegistry() {
...
inline int
MetricsManager::findSpecificPublishers(bsl::vector<Publisher *> *publishers,
                                       const char *categoryName) const {
...
inline int
MetricsManager::findSpecificPublishers(std::vector<Publisher *> *publishers,
                                       const char *categoryName) const {
  const Category *categoryPtr = d_metricRegistry.findCategory(categoryName);
  return categoryPtr ? findSpecificPublishers(publishers, categoryPtr) : 0;
}
...
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
inline int MetricsManager::findSpecificPublishers(
    std::experimental::pmr::vector<Publisher *> *publishers,
    const char *categoryName) const {
  const Category *categoryPtr = d_metricRegistry.findCategory(categoryName);
  return categoryPtr ? findSpecificPublishers(publishers, categoryPtr) : 0;
...
inline const CollectorRepository &MetricsManager::collectorRepository() const {
  return d_collectors;
}

inline const MetricRegistry &MetricsManager::metricRegistry() const {
  return d_metricRegistry;
}
