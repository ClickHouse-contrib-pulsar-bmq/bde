...
struct MetricsManager_PublicationHelper {
  ...
  static void collect(bsl::vector<MetricRecord> *records,
                      bsls::TimeInterval *elapsedTime, MetricsManager *manager,
                      const Category *category, const bsls::TimeInterval &now,
                      bool resetFlag);
  ...
  static void publishAll(balm::MetricsManager *manager,
                         const SET &excludedCategories, bool resetFlag);
...
template <class CONTAINER> class MapProctor {
  ...
  bsl::pair<typename CONTAINER::iterator,
            bool>
      d_iterator; // map element to remove
...
private:
  // NOT IMPLEMENTED
  MetricsManager_PublisherRegistry(const MetricsManager_PublisherRegistry &);
  MetricsManager_PublisherRegistry &
  operator=(const MetricsManager_PublisherRegistry &);
...
public:
  ...
  // CREATORS
  explicit MetricsManager_PublisherRegistry(bslma::Allocator *basicAllocator);
  ...
  // ACCESSORS
  template <class VECTOR> int findGeneralPublishers(VECTOR *publishers) const;
...
class MetricsManager_CallbackRegistry {
  ...
  typedef MetricsManager::RecordsCollectionCallback RecordsCollectionCallback;
  typedef bsl::multimap<const Category *, RecordsCollectionCallback>
      CallbackMap;
  // A 'CallbackMap' type is a map from a category to the sequence of
  // 'MetricsCollectionCallback' functions for that category.

  typedef bsl::map<CallbackHandle, CallbackMap::iterator> CallbackHandleMap;
...
private:
  // NOT IMPLEMENTED
  MetricsManager_CallbackRegistry(const MetricsManager_CallbackRegistry &);
  MetricsManager_CallbackRegistry &
  operator=(const MetricsManager_CallbackRegistry &);
...
public:
  ...
  // CREATORS
  explicit MetricsManager_CallbackRegistry(
  ...
  // MANIPULATORS
  CallbackHandle
  registerCollectionCallback(const Category *category,
  ...
  // ACCESSORS
  int findCallbacks(bsl::vector<const RecordsCollectionCallback *> *callbacks,
                    const Category *category) const;
...
template <class SET, class KEY = typename SET::key_type> struct IsSet {
  static const bool value =
      bsl::is_same<SET, bsl::set<KEY>>::value
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
...
struct SampleDescription {
  ...
  // CREATORS
  SampleDescription(int beginIndex, int size,
                    const bsls::TimeInterval &elapsedTime)
      : d_beginIndex(beginIndex), d_size(size), d_elapsedTime(elapsedTime) {}
};
...
void MetricsManager_PublicationHelper::updateSampleCache(
    SampleCache *sampleCache, const bsl::shared_ptr<Publisher> &publisher,
    const MetricSampleGroup &sampleGroup, const bdlt::DatetimeTz &timeStamp) {
...
void MetricsManager_PublicationHelper::collect(
    bsl::vector<MetricRecord> *records, bsls::TimeInterval *elapsedTime,
    MetricsManager *manager, const Category *category,
    const bsls::TimeInterval &now, bool resetFlag) {
...
void MetricsManager_PublicationHelper::publish(
    ...
    const ConstForwardCategoryIterator &categoriesBegin,
    const ConstForwardCategoryIterator &categoriesEnd, bool resetFlag)

{
  if (categoriesBegin == categoriesEnd) {
    return; // RETURN
  }
  typedef bsl::vector<bsl::shared_ptr<bsl::vector<MetricRecord>>> RecordBuffer;
  ...
  for (catIt = categoriesBegin; catIt != categoriesEnd; ++catIt) {
    ...
    MetricSampleGroup sampleGroup(
        records->data(), static_cast<int>(records->size()), elapsedTime);
    ...
    for (; sIt != sEnd; ++sIt) {
      updateSampleCache(&sampleCache, sIt->second, sampleGroup, timeStamp);
    }
  }
...
template <class SET>
void MetricsManager_PublicationHelper::publishAll(MetricsManager *manager,
  ...
  if (!includedCats.empty()) {
    manager->publish(includedCats.data(), static_cast<int>(includedCats.size()),
                     resetFlag);
  }
}
...
inline MapProctor<CONTAINER>::MapProctor(
    CONTAINER *map, const typename CONTAINER::iterator &iterator)
    : d_map(map), d_iterator(bsl::make_pair(iterator, true)) {}

template <class CONTAINER>
inline MapProctor<CONTAINER>::MapProctor(
    CONTAINER *map, bsl::pair<typename CONTAINER::iterator, bool> &insertResult)
    : d_map(map), d_iterator(insertResult) {}

template <class CONTAINER> inline MapProctor<CONTAINER>::~MapProctor() {
...
// MANIPULATORS
template <class CONTAINER> inline void MapProctor<CONTAINER>::release() {
  d_map = 0;
}
...
MetricsManager_PublisherRegistry::MetricsManager_PublisherRegistry(
    ...
    : d_specificPublishers(basicAllocator), d_registry(basicAllocator),
...
int MetricsManager_PublisherRegistry::addGeneralPublisher(
    const bsl::shared_ptr<Publisher> &publisher) {
  if (d_generalPublishers.end() != d_generalPublishers.find(publisher)) {
...
int MetricsManager_PublisherRegistry::addSpecificPublisher(
  ...
  SpecificPublishers::iterator it = d_specificPublishers.insert(
      SpecificPublishers::value_type(category, publisher));
...
int MetricsManager_PublisherRegistry::removePublisher(
  ...
  bsl::shared_ptr<Publisher> publisherSPtr(const_cast<Publisher *>(publisher),
...
inline MetricsManager_PublisherRegistry::general_iterator
...
inline MetricsManager_PublisherRegistry::specific_iterator
...
inline MetricsManager_PublisherRegistry::specific_iterator
MetricsManager_PublisherRegistry::lowerBound(const Category *category) {
  return d_specificPublishers.lower_bound(category);
}

inline MetricsManager_PublisherRegistry::specific_iterator
MetricsManager_PublisherRegistry::upperBound(const Category *category) {
...
int MetricsManager_PublisherRegistry::findSpecificPublishers(
    VECTOR *publishers, const Category *category) const {
  typedef typename VECTOR::value_type ValueType;
...
MetricsManager_CallbackRegistry::MetricsManager_CallbackRegistry(
    bslma::Allocator *basicAllocator)
    : d_nextHandle(0), d_callbacks(basicAllocator), d_handles(basicAllocator) {}
...
MetricsManager_CallbackRegistry::registerCollectionCallback(
    const Category *category, const RecordsCollectionCallback &callback) {
...
inline MetricsManager_CallbackRegistry::iterator
...
inline void MetricsManager::collectSampleImp(MetricSample *sample,
  ...
  for (; category != categories + numCategories; ++category) {
    ...
    MetricsManager_PublicationHelper::collect(records, &elapsedTime, this,
                                              *category, now, resetFlag);
    ...
    if (0 < size) {
      samples.push_back(u::SampleDescription(beginIndex, size, elapsedTime));
    }
  }
  ...
  for (; it != samples.end(); ++it) {
    sample->appendGroup(&(*records)[it->d_beginIndex], it->d_size,
                        it->d_elapsedTime);
  }
}
...
MetricsManager::MetricsManager(bslma::Allocator *basicAllocator)
    : d_metricRegistry(basicAllocator),
      d_collectors(&d_metricRegistry, basicAllocator), d_callbacks(0),
      d_publishers(0), d_creationTime(bdlt::CurrentTime::now()),
      d_prevResetTimes(basicAllocator), d_publishLock(), d_rwLock(),
      d_allocator_p(bslma::Default::allocator(basicAllocator)) {
  d_callbacks.load(new (*d_allocator_p)
                       MetricsManager_CallbackRegistry(d_allocator_p),
                   d_allocator_p);

  d_publishers.load(new (*d_allocator_p)
                        MetricsManager_PublisherRegistry(d_allocator_p),
                    d_allocator_p);
}
...
void MetricsManager::collectSample(MetricSample *sample,
  ...
  collectSampleImp(sample, records, allCategories.data(),
                   static_cast<int>(allCategories.size()), resetFlag);
}

void MetricsManager::collectSample(MetricSample *sample,
                                   bsl::vector<MetricRecord> *records,
                                   const Category *const categories[],
                                   int numCategories, bool resetFlag) {
  collectSampleImp(sample, records, categories, numCategories, resetFlag);
}

void MetricsManager::publish(const Category *category, bool resetFlag) {
  publish(&category, 1, resetFlag);
}

void MetricsManager::publish(const Category *const categories[],
                             int numCategories, bool resetFlag) {
  MetricsManager_PublicationHelper::publish(
      this, categories, categories + numCategories, resetFlag);
}
...
void MetricsManager::publish(
    const std::experimental::pmr::set<const Category *> &categories,
    bool resetFlag) {
  MetricsManager_PublicationHelper::publish(this, categories.begin(),
                                            categories.end(), resetFlag);
}
...
void MetricsManager::publishAll(bool resetFlag) {
  bsl::vector<const Category *> allCategories;
  d_metricRegistry.getAllCategories(&allCategories);
  publish(allCategories.data(), static_cast<int>(allCategories.size()),
          resetFlag);
}

void MetricsManager::publishAll(
    const bsl::set<const Category *> &excludedCategories, bool resetFlag) {
  MetricsManager_PublicationHelper::publishAll(this, excludedCategories,
                                               resetFlag);
}

void MetricsManager::publishAll(
    const std::set<const Category *> &excludedCategories, bool resetFlag) {
  MetricsManager_PublicationHelper::publishAll(this, excludedCategories,
                                               resetFlag);
}
...
MetricsManager::CallbackHandle MetricsManager::registerCollectionCallback(
    const Category *category, const RecordsCollectionCallback &callback) {
...
int MetricsManager::findSpecificPublishers(bsl::vector<Publisher *> *publishers,
                                           const Category *category) const {
  bslmt::ReadLockGuard<bslmt::RWMutex> guard(&d_rwLock);
  return d_publishers->findSpecificPublishers(publishers, category);
}

int MetricsManager::findSpecificPublishers(std::vector<Publisher *> *publishers,
                                           const Category *category) const {
  bslmt::ReadLockGuard<bslmt::RWMutex> guard(&d_rwLock);
  return d_publishers->findSpecificPublishers(publishers, category);
}
