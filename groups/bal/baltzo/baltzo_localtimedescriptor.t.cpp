...
int main(int argc, char *argv[]) {
  ...
  case 14: {
    ...
    // local time information contained in the "Zoneinfo" binary data files.
    // Once we have obtained this information, we can use it to convert times
    // from one time zone to another.  The following code illustrates how to
    // perform such conversions using 'baltzo::LocalTimeDescriptor'.
    //
    // First, we define a 'baltzo::LocalTimeDescriptor' object that
    // characterizes the local time in effect for New York Daylight-Saving Time
    // in 2010:
    //..
    ...
    //..
    // Next, we convert 'newYorkDatetime' to its corresponding UTC value using
    // the 'newYorkDst' descriptor (created above); note that, when converting
    // from a local time to a UTC time, the *signed* offset from UTC is
    // *subtracted* from the local time:
    //..
    ...
    //..
    // Now, we convert 'utcDatetime' to its corresponding local-time value in
    // Rome using the 'romeDst' descriptor (created above):
    //..
    ...
    //
    // Finally, we verify that the result corresponds to the expected local
    // time, "Jul 20, 2010 17:00":
    //..
  ...
  case 13: {
    ...
    if (verbose)
      cout << "\nAssign the address of 'isValidUtcOffsetInSeconds' to a "
              "variable."
           << endl;
    {
    ...
    if (verbose)
      cout << "\nCreate a table of distinct candidate attribute values."
           << endl;
    ...
    for (int ti = 0; ti < NUM_DATA; ++ti) {
      ...
      if (veryVerbose) {
        T_ P_(LINE) P_(OFFSET) P(EXPECTED)
      }
  ...
  case 12: {
    ...
    if (verbose)
      cout << endl << "BSLX STREAMING" << endl << "==============" << endl;
    ...
    for (int ti = 0; ti < NUM_DATA; ++ti) {
      ...
      for (int tj = 0; tj < NUM_DATA; ++tj) {
...

        {
          ...
          Obj mF(OFFSET1, FLAG1, DESC1, &s1);
          const Obj &F = mF;
          Obj mX(OFFSET2, FLAG2, DESC2, &s1);
          const Obj &X = mX;
          ...
          ASSERTV(LINE1, LINE2, &s1, ALLOC_OF(X), &s1 == X.get_allocator());
          ASSERTV(LINE1, LINE2, &s1, ALLOC_OF(F), &s1 == F.get_allocator());
      ...
      for (int tj = 0; tj < NUM_DATA; ++tj) {
...

        {
          ...
          BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(s2) {
            ...
            ASSERTV(LINE1, LINE2, &s2, ALLOC_OF(X), &s2 == X.get_allocator());
            ASSERTV(LINE1, LINE2, &s1, ALLOC_OF(F), &s1 == F.get_allocator());
    ...
    for (int ti = 0; ti < NUM_DATA; ++ti) {
      ...
      for (int tj = 0; tj < NUM_DATA; ++tj) {
...

        {
          ...
          ASSERTV(LINE1, LINE2, &oa, ALLOC_OF(X), &oa == X.get_allocator());
  ...
  case 9: {
    ...
    for (int ti = 0; ti < NUM_DATA; ++ti) {
      ...
      for (int tj = 0; tj < NUM_DATA; ++tj) {
        ...
        Obj mY(OFFSET2, FLAG2, DESC2, &oa2);
        const Obj &Y = mY;
        const Obj YY(Y, &scratch);
  ...
  case 8: {
      ...
      for (int ti = 0; ti < NUM_DATA; ++ti) {
        ...
        for (char cfg = 'a'; cfg <= 'd'; ++cfg) {
          ...
          case 'b': {
            ...
            objAlloc = objAllocatorPtr->numBytesInUse();
            objPtr = new (fa)
                Obj(bslmf::MovableRefUtil::move(mF), Obj::allocator_type());
          } break;
          case 'c': {
          ...
          case 'd': {
            ...
            objAlloc = objAllocatorPtr->numBytesInUse();
            objPtr = new (fa) Obj(bslmf::MovableRefUtil::move(mF), alloc);
          } break;
          default: {
          ...
          if (objAllocatorPtr != F.get_allocator()) {
            ...
            ASSERTV(LINE, CONFIG, s1.numBytesInUse(), moveBytesUsed,
          ...
          ASSERTV(LINE, CONFIG, &oa == X.description().get_allocator());

          // Also invoke the object's 'get_allocator' accessor, as
          // well as that of 'Z'.

          ASSERTV(LINE, CONFIG, &oa, ALLOC_OF(X), &oa == X.get_allocator());
      ...
      cout << "\nTesting with injected exceptions." << endl;
    {
      for (int ti = 0; ti < NUM_DATA; ++ti) {
        ...
        if (veryVerbose) {
          T_ P_(MEM) P_(OFFSET) P_(FLAG) P(DESC)
        }
        ...
        ASSERTV(LINE, &scratch, ALLOC_OF(Z), &scratch == Z.get_allocator());
        ASSERTV(LINE, da.numBlocksInUse(), 0 == da.numBlocksInUse());
        ASSERTV(LINE, s1.numBlocksInUse(), 0 == s1.numBlocksInUse());
        ASSERTV(LINE, s2.numBlocksInUse(), 0 == s2.numBlocksInUse());
      }
    }
  } break;
  case 7: {
  ...
  case 6: {
    ...
    for (int ti = 0; ti < NUM_DATA; ++ti) {
      ...
      if (veryVerbose) {
        T_ P_(LINE1) P_(OFFSET1) P_(FLAG1) P(DESC1)
      }
...
#define SP " "
        ...
        {L_, 0, 0, 89, true, "a",
         "[" NL "utcOffsetInSeconds = 89" NL "dstInEffectFlag = true" NL
         "description = \"a\"" NL "]" NL},
        ...
        {L_, 0, -1, 89, true, "a",
         "[" SP "utcOffsetInSeconds = 89" SP "dstInEffectFlag = true" SP
         "description = \"a\"" SP "]"},
        ...
        {L_, 3, 2, 89, true, "a",
         "      [" NL "        utcOffsetInSeconds = 89" NL
         "        dstInEffectFlag = true" NL "        description = \"a\"" NL
         "      ]" NL},
        ...
        {L_, 2, 3, 7, false, "bc",
         "      [" NL "         utcOffsetInSeconds = 7" NL
         "         dstInEffectFlag = false" NL
         "         description = \"bc\"" NL "      ]" NL},
        ...
        {L_, -8, -8, 7, false, "bc",
         "[" NL "    utcOffsetInSeconds = 7" NL "    dstInEffectFlag = false" NL
         "    description = \"bc\"" NL "]" NL},
        // -----------------------------------------------------------------
...
#undef SP
      ...
      cout << "\nTesting with various print specifications." << endl;
    {
      for (int ti = 0; ti < NUM_DATA; ++ti) {
        ...
        if (veryVerbose) {
          T_ P_(L) P_(SPL) P_(OFF) P_(FLAG) P(DESC)
        }
  ...
  case 4: {
    ...
    if (verbose)
      cout << "\nCreate two test allocators; install one as the default."
           << endl;
  ...
  case 3: {
    ...
    if (verbose)
      cout << endl << "VALUE CTOR" << endl << "==========" << endl;
      ...
      cout << "\nCreate objects with various allocator configurations." << endl;
    {
      ...
      for (int ti = 0; ti < NUM_DATA; ++ti) {
        ...
        const std::experimental::pmr::string DESCP = DATA[ti].d_description;
#endif
        ...
        for (char cfg = 'a'; cfg <= 'm'; ++cfg) {
          ...
          case 'b': {
            objAllocatorPtr = &da;
            objPtr = new (fa) Obj(OFFSET, FLAG, DESC, Obj::allocator_type());
          } break;
          case 'c': {
            objAllocatorPtr = &sa;
            objPtr = new (fa) Obj(OFFSET, FLAG, DESC, objAllocatorPtr);
          } break;
          case 'd': {
            objAllocatorPtr = &da;
          ...
          case 'e': {
            objAllocatorPtr = &da;
            objPtr = new (fa) Obj(OFFSET, FLAG, DESCB, Obj::allocator_type());
          } break;
          case 'f': {
            objAllocatorPtr = &sa;
            objPtr = new (fa) Obj(OFFSET, FLAG, DESCB, objAllocatorPtr);
          } break;
          case 'g': {
            objAllocatorPtr = &da;
          ...
          case 'h': {
            objAllocatorPtr = &da;
            objPtr = new (fa) Obj(OFFSET, FLAG, DESCS, Obj::allocator_type());
          } break;
          case 'i': {
            objAllocatorPtr = &sa;
            objPtr = new (fa) Obj(OFFSET, FLAG, DESCS, objAllocatorPtr);
          } break;
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR_STRING
          case 'j': {
            objAllocatorPtr = &da;
          ...
          case 'k': {
            objAllocatorPtr = &da;
            objPtr = new (fa) Obj(OFFSET, FLAG, DESCP, Obj::allocator_type());
          } break;
          case 'l': {
            objAllocatorPtr = &sa;
            objPtr = new (fa) Obj(OFFSET, FLAG, DESCP, objAllocatorPtr);
          } break;
#else
          case 'j':
            BSLA_FALLTHROUGH;
          ...
          ASSERTV(LINE, CONFIG, DESC, X.description(), DESC == X.description());
      ...
      cout << "\nTesting with injected exceptions." << endl;
    {
      ...
      for (int ti = 0; ti < NUM_DATA; ++ti) {
        ...
        BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(sa) {
          ...
          ASSERTV(LINE, DESC, obj.description(), DESC == obj.description());
  ...
  case 2: {
    ...
    for (char cfg = 'a'; cfg <= 'd'; ++cfg) {
      ...
      ASSERTV(CONFIG, D1, X.utcOffsetInSeconds(), D1 == X.utcOffsetInSeconds());
      ASSERTV(CONFIG, D2, X.dstInEffectFlag(), D2 == X.dstInEffectFlag());
      ASSERTV(CONFIG, D3, X.description(), D3 == X.description());
  ...
  case 1: {
    ...
    if (verbose)
      cout << "\ta. Check initial value of 'w'." << endl;
    if (veryVeryVerbose) {
      T_ T_ P(W)
    }
    ...
    if (verbose)
      cout << "\tb. Try equality operators: 'w' <op> 'w'." << endl;
    ...
    if (verbose)
      cout << "\ta. Check initial value of 'x'." << endl;
    if (veryVeryVerbose) {
      T_ T_ P(X)
    }
    ...
    if (verbose)
      cout << "\tb. Try equality operators: 'x' <op> 'w', 'x'." << endl;
    ...
    if (verbose)
      cout << "\ta. Check new value of 'x'." << endl;
    if (veryVeryVerbose) {
      T_ T_ P(X)
    }
    ...
    if (verbose)
      cout << "\ta. Check initial value of 'y'." << endl;
    if (veryVeryVerbose) {
      T_ T_ P(Y)
    }
    ...
    if (verbose)
      cout << "\tb. Try equality operators: 'y' <op> 'w', 'x', 'y'" << endl;
    ...
    if (verbose)
      cout << "\ta. Check initial value of 'z'." << endl;
    if (veryVeryVerbose) {
      T_ T_ P(Z)
    }
    ...
    if (verbose)
      cout << "\tb. Try equality operators: 'z' <op> 'w', 'x', 'y', 'z'."
           << endl;
    ...
    if (verbose)
      cout << "\ta. Check new value of 'z'." << endl;
    if (veryVeryVerbose) {
      T_ T_ P(Z)
    }
    ...
    if (verbose)
      cout << "\ta. Check new value of 'w'." << endl;
    if (veryVeryVerbose) {
      T_ T_ P(W)
    }
    ...
    if (verbose)
      cout << "\tb. Try equality operators: 'w' <op> 'w', 'x', 'y', 'z'."
           << endl;
    ...
    if (verbose)
      cout << "\tb. Try equality operators: 'x' <op> 'w', 'x', 'y', 'z'."
           << endl;
    ...
    if (verbose)
      cout << "\ta. Check (same) value of 'x'." << endl;
    if (veryVeryVerbose) {
      T_ T_ P(X)
    }
