...
template <class CCHAR_VECTOR> struct IsCCharVector {
  static const bool value =
      bsl::is_same<CCHAR_VECTOR, bsl::vector<const char *>>::value ||
      bsl::is_same<CCHAR_VECTOR, std::vector<const char *>>::value
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      || bsl::is_same<CCHAR_VECTOR,
                      std::experimental::pmr::vector<const char *>>::value
#endif
      ;
};
...
enum {
  k_NUM_BALTZO_DATA_LOCATIONS =
      sizeof BALTZO_DATA_LOCATIONS / sizeof *BALTZO_DATA_LOCATIONS - 1
};
...
static baltzo::ZoneinfoCache *initSystemDefaultCache()
...
inline void loadDefaultZoneinfoDataLocationsImp(CCHAR_VECTOR *locations) {
  BSLS_ASSERT(locations);

  BSLMF_ASSERT(u::IsCCharVector<CCHAR_VECTOR>::value);

  locations->insert(locations->end(), BALTZO_DATA_LOCATIONS + 0,
  ...
  for (int ii = 0; ii < u::k_NUM_BALTZO_DATA_LOCATIONS; ++ii) {
    ...
    if (DataFileLoader::isPlausibleZoneinfoRootPath(path)) {
      BSLS_LOG_INFO("The environment variable "
                    ...
                    "falling back on default location: %s",
                    path);
      return path; // RETURN
    }
  }
