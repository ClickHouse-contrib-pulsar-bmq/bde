...
template <class VECTOR> void test2() {
  if (verbose)
  ...
  BSLMF_ASSERT(
      (bsl::is_same<typename VECTOR::value_type, const char *>::value));
  ...
  LOOP2_ASSERT(L_, locations.size(), k_NUM_VALUES == locations.size());
  ...
  for (int ii = 0; ii < k_NUM_VALUES; ++ii) {
    ...
    LOOP2_ASSERT(LINE, EXP_PATH, 0 == bsl::strcmp(RESULT_PATH, EXP_PATH));
  }
    ...
    cout << "\nNegative Testing." << endl;
  {
    ...
    if (veryVerbose)
      cout << "\t'CLASS METHOD 'loadDefaultZoneinfoDataLocations' " << endl;
    {
...
int main(int argc, char *argv[]) {
  ...
  if (!bdls::FilesystemUtil::exists(ETC_UTC_FILE)) {
    writeData(ETC_UTC_FILE, reinterpret_cast<const char *>(ETC_UTC_DATA),
              sizeof(ETC_UTC_DATA));
  }

  if (!bdls::FilesystemUtil::exists(GMT_FILE)) {
    writeData(GMT_FILE, reinterpret_cast<const char *>(GMT_DATA),
              sizeof(GMT_DATA));
  }
  ...
  case 6: {
    ...
    int rc = getLocalTimeDescriptor(
        &result, bdlt::Datetime(bdlt::Date(2011, 03, 21), bdlt::Time(20, 57)),
        "America/New_York");
    ASSERT(0 == rc);
    ...
    ///- - - - - - - - - - - - - - - - - - - - - - - - - - - -
    // In this example we demonstrate how to configure the default time-zone
    // cache for a process.  Note that many application may not need to
    // explicitly configure the default cache object, but can instead use the
    // automatically configured default object.  Also note that the default
    // time-zone cache is intended to be configured by the *owner* of 'main'.
    // The default object should be set during the initialization of an
    // application (while the task has a single thread) and unset just prior to
  ...
  case 5: {
    ...
    if (verbose)
      cout << "\nPassing in a cache object and testing GA" << endl;
    {
      const bsls::Types::Int64 GA_NUM_BYTES = globalAllocator.numBytesInUse();
    ...
    if (verbose)
      cout << "\nNot passing in a cache object and testing GA" << endl;
    {
      ...
      LOOP3_ASSERT(L_, GA_NUM_BYTES, globalAllocator.numBytesInUse(),
                   GA_NUM_BYTES <= globalAllocator.numBytesInUse());

      const bsls::Types::Int64 GA_NUM_BYTES2 = globalAllocator.numBytesInUse();
    ...
    if (verbose)
      cout << "\nTesting after 'setDefaultCache'" << endl;
    {
      const bsls::Types::Int64 GA_NUM_BYTES = globalAllocator.numBytesInUse();
      ...
      const bsls::Types::Int64 GA_NUM_BYTES3 = globalAllocator.numBytesInUse();
      RESULT = Obj::defaultCache(0);
      LOOP2_ASSERT(L_, RESULT, &cache == RESULT);
      LOOP_ASSERT(L_, GA_NUM_BYTES3 == globalAllocator.numBytesInUse());

      baltzo::ZoneinfoCache tempCache(&loader);
      const bsls::Types::Int64 GA_NUM_BYTES4 = globalAllocator.numBytesInUse();
    ...
    if (verbose)
      cout << "\nTesting after 'setDefaultCache(0)'" << endl;
    {
      const bsls::Types::Int64 GA_NUM_BYTES = globalAllocator.numBytesInUse();
  ...
  case 4: {
    ...
    if (verbose)
      cout << "\nTesting invoking 'setDefaultCache' and GA." << endl;
    {
      const bsls::Types::Int64 GA_NUM_BYTES = globalAllocator.numBytesInUse();
    ...
    if (verbose)
      cout << "\nInvoking 'setDefaultCache' a second time." << endl;
      ...
      cout << "\nNegative Testing." << endl;
    {
      ...
      if (veryVerbose)
        cout << "\t'CLASS METHOD 'setDefaultCache' " << endl;
      {
        baltzo::ZoneinfoCache cache(&loader);
  ...
  case 3: {
    ...
    if (verbose)
      cout << endl
           << "'defaultZoneinfoDataLocation' CLASS METHOD" << endl
           << "===============================================" << endl;

#ifndef BSLS_PLATFORM_OS_WINDOWS
    if (verbose)
      cout << "Testing with 'BDE_ZONEINFO_ROOT_PATH' set" << endl;
    {
      const bsls::Types::Int64 DA_NUM_BYTES = defaultAllocator.numBytesInUse();
    ...
    if (verbose)
      cout << "Testing with 'BDE_ZONEINFO_ROOT_PATH' set to an "
           << "existing path not containing Zoneinfo data "
           << "(expected to log a complaint)" << endl;
    {
    ...
    if (verbose)
      cout << "Testing with 'BDE_ZONEINFO_ROOT_PATH' unset" << endl;
    {
      ...
      cout << "Testing with 'BDE_ZONEINFO_ROOT_PATH' set" << endl;
    {
      ...
      LOOP2_ASSERT(L_, RESULT, 0 == bsl::strcmp(RESULT, ".\\defaultzictest"));
      LOOP2_ASSERT(L_, defaultAllocator.numBytesInUse(),
                   DA_NUM_BYTES == defaultAllocator.numBytesInUse());
    }
    ...
    if (verbose)
      cout << "Testing with 'BDE_ZONEINFO_ROOT_PATH' set to an "
           << "existing path not containing Zoneinfo data" << endl;
    {
      ...
      LOOP2_ASSERT(L_, RESULT, 0 == bsl::strcmp(RESULT, "\\C:\\Windows"));
      LOOP2_ASSERT(L_, defaultAllocator.numBytesInUse(),
                   DA_NUM_BYTES == defaultAllocator.numBytesInUse());
    }
  ...
  case 2: {
    ...
    if (verbose)
      cout << endl
           << "'loadDefaultZoneinfoDataLocations' CLASS METHOD" << endl
           << "===============================================" << endl;
  ...
  case 1: {
    ...
    if (verbose)
      cout << endl << "Testing the system default cache" << endl;
    ...
    if (verbose)
      cout << "\nTesting user defind default caches" << endl;
