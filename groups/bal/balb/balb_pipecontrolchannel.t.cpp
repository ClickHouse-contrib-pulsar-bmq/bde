...
bsl::ostream &operator<<(bsl::ostream &stream, PipeNameForm pnf) {
  const char *str = e_CONST_CHAR_STAR == pnf ? "'const char *'"
                    : e_BSL_STRING == pnf    ? "bsl::string"
                    : e_STD_STRING == pnf    ? "std::string"
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR_STRING
                    : e_PMR_STRING == pnf ? "pmr::string"
#endif
                    : e_STRING_REF == pnf ? "bslstl::StringRef"
                    : e_END == pnf        ? "end"
                                          : "unknown";
...
public:
  ...
  explicit ControlServer(bslma::Allocator *basicAllocator = 0)
      : d_channel(bdlf::BindUtil::bind(&ControlServer::onMessage, this,
  ...
  void shutdown() { d_channel.shutdown(); }

  void stop() { d_channel.stop(); }

  // ACCESSORS
  bsl::size_t numMessages() const { return d_messages.size(); }

  const bsl::string &message(int index) const { return d_messages[index]; }
};
//..

// ----------------------------------------------------------------------------

static void threadSend(const bsl::string &pipeName, const bsl::string &message,
                       int numIterations) {
...
void verifyPayload(const bsl::string &expected, const bslstl::StringRef &found,
...
void verifyPayloadCountDown(const bsl::string &expected,
                            const bslstl::StringRef &found, bslmt::Latch *latch)
// Verify that the specified 'found' payload has the same value as the
  ...
  // read 'd_messageLength + 1' bytes from it.
  {
    ...
    if (fds[0].revents & POLLIN) {
      bsls::Types::Int64 bytesRead =
          read(d_readFd, d_buffer, d_messageLength + 1);
      savedErrno = errno;
...
public:
  ...
  explicit IllegalReader(bslmt::Barrier *barrier, int messageLength)
      : d_thread(0), d_messageLength(messageLength), d_barrier_p(barrier),
        d_readFd(-1), d_controlReadFd(-1), d_controlWriteFd(-1) {
    ASSERT(d_barrier_p);
  ...
  // Stops reading after receiving a message or when 'stop' is called.
  {
    ...
    d_readFd = open(pipeName.c_str(), O_RDONLY | O_NONBLOCK | O_CLOEXEC);
#else
    ...
    if (-1 == d_readFd) {
      ...
      printf("Unable to open pipe '%s' for reading. errno = %d (%s)",
             pipeName.c_str(), savedErrno, bsl::strerror(savedErrno));

      return false; // RETURN
    }

    if (!createControlPipe())
      return false; // RETURN

    const int rc = bslmt::ThreadUtil::create(
        &d_thread, bslmt::ThreadAttributes(),
  ...
  // Stop the reading thread and close any open pipes.
  {
    ...
    if (-1 != d_readFd)
      close(d_readFd);
    if (-1 != d_controlReadFd)
      close(d_controlReadFd);
    if (-1 != d_controlWriteFd)
      close(d_controlWriteFd);
  }
};
#endif
...
void testCase14(const string &pipeName) {
  ...
  do {
    ...
    pipe = CreateFileW(wPipeName.data(), GENERIC_WRITE, 0, NULL, OPEN_EXISTING,
                       0, NULL);
...
extern "C" void onSigPipe(int) { BSLS_LOG_WARN("SIGPIPE received"); }

void myCallback(bslmt::Condition *condition, bsls::AtomicBool *conditionFlag,
...
int main(int argc, char *argv[]) {
  ...
  if (test >= 0) {
    ...
    if (veryVerbose)
      bsl::cerr << "pipeName: " << pipeName << bsl::endl;
  }
  ...
  case 13: {
    ...
    bslma::TestAllocator ta(veryVeryVeryVerbose);
    {
      ...
      for (int iteration = 0; iteration < NUM_DATA; ++iteration) {
        ...
        if (veryVerbose) {
          T_ P_(iteration) P(messageLength)
        }
        ...
        balb::PipeControlChannel channel(
            bdlf::BindUtil::bind(&verifyPayloadCountDown, message,
                                 bdlf::PlaceHolders::_1, &latch),
            &ta);
        // start the channel
        ...
        threadGroup.addThreads(
            bdlf::BindUtil::bind(&threadSend, bsl::string(pipeName),
                                 message + "\n", numMessages),
            1);
  ...
  case 12: {
    ...
    dfltThreadName.resize(bsl::min<bsl::size_t>(15, dfltThreadName.length()));
    ...
    for (int attrI = 0; attrI < 2; ++attrI) {
      ...
      const bsl::string &EXP_THREAD_NAME =
          PASS_ATTRIBUTES ? RAW_EXP_THREAD_NAME : DFLT_THREAD_NAME;

      for (int pnfi = e_BEGIN; pnfi < e_END; ++pnfi) {
        const PipeNameForm PIPE_NAME_FORM = static_cast<PipeNameForm>(pnfi);
        ...
        balb::PipeControlChannel pipeChannel(
            bdlf::BindUtil::bind(&attributesTestCallback, &threadName, &message,
                                 bdlf::PlaceHolders::_1, &barrier));

        int rc = -1;
        if (PASS_ATTRIBUTES) {
          switch (PIPE_NAME_FORM) {
          case e_CONST_CHAR_STAR: {
            rc = pipeChannel.start(PIPE_NAME_BSL.c_str(), SET_ATTR);
          } break;
          case e_BSL_STRING: {
    ...
    defined(BSLS_PLATFORM_OS_SOLARIS)
        ASSERTV(PIPE_NAME_FORM, threadName, EXP_THREAD_NAME == threadName);
  ...
  case 11: {
    ...
    balb::PipeControlChannel pipeChannel(bdlf::BindUtil::bind(
        &myCallback, &condition, &conditionFlag, bdlf::PlaceHolders::_1));

    pipeChannel.start(pipeName);
    bslmt::ThreadUtil::Handle thread;
    int rc = bslmt::ThreadUtil::create(
        &thread, bdlf::BindUtil::bind(&sendHello, pipeName.c_str()));
    ASSERT(rc == 0);
    if (rc != 0) {
      break; // BREAK
    }
  ...
  case -10: {
...
#else
    if (argc != 5 ||
        (argv[2][0] != 'Y' && argv[2][0] != 'Z' && argv[2][0] != 'A')) {
    ...
    bslmt::Barrier barrier(2);
    balb::PipeControlChannel channel(
        bdlf::BindUtil::bind(&verifyPayload, bsl::string(argv[4]),
                             bdlf::PlaceHolders::_1, &barrier));
  ...
  case 10: {
    ...
    char buffer[512];
    char verb = veryVeryVerbose ? 'A' : verbose ? 'Z' : 'Y';

    snprintf(buffer, 512, "%s -10 %c %s QUICKBROWNFOX_CASE10", argv[0], verb,
             pipeName.c_str());
    ...
    if (verbose) {
      cout << "Sending QUICKBROWNFOX_CASE10 to " << pipeName << endl;
    }
  ...
  case -9: {
    ...
    if (argc != 4 || (argv[2][0] != 'Y' && argv[2][0] != 'Z')) {
    ...
    balb::PipeControlChannel channel(bdlf::BindUtil::bind(
        &verifyPayload, message, bdlf::PlaceHolders::_1, &barrier));

    ASSERT(0 == channel.start(argv[3]));

    if (verbose) {
      cerr << "\tCase 9 client: channel started, sending A..." << endl;
    }
    cout << "A\n" << flush;

    bslmt::ThreadGroup threadGroup;
    threadGroup.addThreads(
        bdlf::BindUtil::bind(&threadSend, bsl::string(argv[3]), message + "\n",
                             (int)NUM_ITERATIONS),
        NUM_CLIENTS);
  ...
  case 9: {
    ...
    char buffer[512];
    snprintf(buffer, 512, "%s -9 %c %s", argv[0], verbose ? 'Z' : 'Y',
             pipeName.c_str());
    FILE *childFD = popen(buffer, "r");
    BSLS_ASSERT(0 != childFD); // test invariant
    do {
  ...
  case -8: {
    ...
    if (verbose) {
      cerr << "\tCase 8 client: channel started, seg faulting..." << endl;
    }
    cout << "A\n" << flush;
  ...
  case 8: {
    ...
    char buffer[512];
    snprintf(buffer, 512, "%s -8 %c %s", argv[0], verbose ? 'Z' : 'Y',
             pipeName.c_str());
    FILE *childFD = popen(buffer, "r");
    BSLS_ASSERT(0 != childFD); // test invariant
    do {
...
//..
  ...
  case 6: {
    ...
    bslma::TestAllocator ta(veryVeryVeryVerbose);
    {
      ...
      for (int iteration = 0; iteration < NUM_DATA; ++iteration) {
        ...
        balb::PipeControlChannel channel(
            bdlf::BindUtil::bind(&verifyPayload, message,
                                 bdlf::PlaceHolders::_1, &barrier),
            &ta);
  ...
  case 5: {
    ...
    bslma::TestAllocator ta(veryVeryVeryVerbose);
    {
      ...
      balb::PipeControlChannel channel(
          bdlf::BindUtil::bind(&verifyPayload, bsl::string(MESSAGE),
                               bdlf::PlaceHolders::_1, &barrier),
          &ta);
