...
TestData DATA[] = {
    {"<schema elementFormDefault='qualified'></schema>", false, ""},
    {"<schema targetNamespace='zzz'></schema>", true, "zzz"},
    {"<nonschema targetNamespace='zzz'></nonschema>", true, "zzz"},
    {"<!-- targetNamespace='yyy' -->"
     "<schema targetNamespace='zzz'></schema>",
     true, "zzz"},
    {"<schema targetNamespace='zzz'", true, "zzz"}};
  ...
  case 3: {
    ...
    for (bsl::size_t i = 0; i < sizeof(DATA) / sizeof(DATA[0]); ++i) {
...

      {
        ...
        bool rc = balxml::Util::extractNamespaceFromXsd(is.rdbuf(), &tnsReal);

        LOOP3_ASSERT(i, rc, DATA[i].d_result, rc == DATA[i].d_result);

        if (rc) {
          LOOP3_ASSERT(i, tnsExpected, tnsReal, tnsExpected == tnsReal);
        }
      }
