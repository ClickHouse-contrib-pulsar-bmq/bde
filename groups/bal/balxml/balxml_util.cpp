...
inline bool extractNamespaceFromXsd_Impl(const bsl::string_view &xsdSource,
  ...
  for (;;) {
    ...
    if (npos != startTargetNamespace) {
      ...
      size_type startOpenComment =
          xsdSource.rfind("<!--", startTargetNamespace);

      if (npos != startOpenComment) {
        // There is a '<!--'; find whether there is '-->' before this
        // "targetNamespace".

        size_type startCloseComment =
            xsdSource.find("-->", startOpenComment + 4);
        // 4 is strlen("<!--")
  ...
  if (npos != startTargetNamespace) {
    ...
    size_type startNamespace =
        xsdSource.find_first_of("'\"", startTargetNamespace + 16);
    ...
    if (npos != startNamespace) {
      ...
      if (npos != endNamespace) {
        bsl::string_view sub = xsdSource.substr(
            startNamespace + 1, endNamespace - startNamespace - 1);
...
template <class STRING_TYPE>
inline bool extractNamespaceFromXsd_Impl(bsl::streambuf *xsdSource,
                                         STRING_TYPE *targetNamespace) {

  bsl::streambuf::pos_type pos =
      xsdSource->pubseekoff(0, bsl::ios_base::cur, bsl::ios::in);
...
bool Util::extractNamespaceFromXsd(
