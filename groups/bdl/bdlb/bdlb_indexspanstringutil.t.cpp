...
int main(int argc, char *argv[]) {
  ...
  case 4: {
    ...
    // parsing to be stored in 'IndexSpan' objects.  The parser will have either
    // a pointer (or "begin" iterator) into the original string input and then
    // another pointer (or iterator) or a length to identify the end of the
    // input. Turning the beginning and ending identifiers into an 'IndexSpan'
    // is a simple calculation, but one that is verbose and potentially error
    // prone.  Instead of implementing the calculation ourselves we use the
    // convenience function 'create' from 'IndexSpanStringUtil'.
    //
    ...
    bdlb::IndexSpan first =
        bdlb::IndexSpanStringUtil::create(full, full.begin(), it);
    //..
    // Next, we implement the parsing of the middle name, this time using
    // length, rather than an end iterator (demonstrating an alternative
    // 'create' overload provided by 'IndexSpanStringUtil'):
    //..
    ++it; // Skip the space
    ...
    bdlb::IndexSpan last = bdlb::IndexSpanStringUtil::createFromPositions(
        full, it - full.begin(), full.length());
    //..
    // Finally, we verify that the resulting 'IndexSpan' objects correctly
    // describe the parsed names of the 'full' name:
    //..
    ...
    ///- - - - - - - - - - - - - - - - - - -
    // Suppose that we have 'IndexSpan' objects that define the 'first',
    // 'middle', and 'last' part of a string that has a full name in it and we
    // want to get actual string references that correspond to those parts of
    // the string.  The 'bind' functions of 'IndexSpanStringUtil' provide that
    // functionality.  The 'bind' functions return a 'StringRef' into the
    // original string (so the characters of the string are not copied).  Note
    // that this example builds on Example 1.
    //
    // First, we define a string reference of the parsed string to show that
    // 'bind' works both on strings and string references:
    //..
  ...
  case 3: {
    ...
    //:   'std::string', and 'std::experimental::pmr::string' objects as first
    //:   argument, as well as their wide equivalents.
    //
    // Plan:
    ...
    //:   input as we cannot create a valid iterator', or even a 'StringRef',
    //:   that would point into the same allocated memory area (as required by
    ...
    if (veryVerbose)
      cout << "Positive narrow string overload tests.\n";
...
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR_STRING
      const std::experimental::pmr::string k_PMRSTR(k_STRREF.data(),
                                                    k_STRREF.size());
#endif
#endif
...
#undef VERIFY_CREATE
    ...
    if (veryVerbose)
      cout << "Negative narrow string overload tests.\n";
...
#undef TEST_CREATE_FROM_SUBSTR
    ...
    if (veryVerbose)
      cout << "Positive wide string overload tests.\n";
...
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR_STRING
      const std::experimental::pmr::wstring k_PMRSTR(k_STRREF.data(),
                                                     k_STRREF.size());
#endif
#endif
      const bdlb::IndexSpan k_EXPECTED(k_POS, k_LEN);
...
#ifdef BDE_BUILD_TARGET_EXC
    if (veryVerbose)
      cout << "Negative wide string overload tests.\n";
...
#undef TEST_CREATE_FROM_SUBSTR
  ...
  case 2: {
    ...
    //:    'bslstl::StringRef', 'std::string', and
    //:    'std::experimental::pmr::string' first argument.
    //:
    ...
    if (veryVeryVerbose)
      cout << "Positive testing of narrow strings\n";
...
#undef TEST_BIND_VERIFY
...
#ifdef BDE_BUILD_TARGET_EXC
    if (veryVeryVerbose)
      cout << "Negative testing of narrow strings\n";
    ...
    if (veryVeryVerbose)
      cout << "Positive testing of wide strings\n";
...
#ifdef BDE_BUILD_TARGET_EXC
    if (veryVeryVerbose)
      cout << "Negative testing of wide strings\n";
