...
struct GuidUtil {
  ...
  static void guidToString(std::experimental::pmr::string *result,
                           const Guid &guid);
#endif
...
// MANIPULATORS
inline void GuidState_Imp::generateRandomBits(
    bsl::uint32_t (*out)[GuidState_Imp::k_GENERATOR_COUNT]) {
...
// ---------------
// CLASS METHODS
inline void GuidUtil::generate(Guid *result, bsl::size_t numGuids) {
  generate(reinterpret_cast<unsigned char *>(result), numGuids);
}

inline int GuidUtil::getVersion(const Guid &guid) {
