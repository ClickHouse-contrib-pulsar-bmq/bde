...
// [ 6] pad(std::experimental::pmr::string *str, size_type numChars, char
// padChar = ' '); [ 5] rtrim(char *str); [ 5] rtrim(bsl::string *str); [ 5]
// rtrim(std::string *str); [ 5] rtrim(std::experimental::pmr::string *str); [
// 5] rtrim(cchar *str, int *L); [ 9] strstr(cchar *str, int strL, cchar
// *subStr, int subStrL); [ 9] strstrCaseless(cchar *str, int strL, cchar
// *subStr, int subStrL); [ 9] strrstr(cchar *str, int strL, cchar *subStr, int
// subStrL); [ 9] strrstrCaseless(cchar *str, int strL, cchar *subStr, int
// subStrL); [ 7] strnlen(cchar *str, int maximumLength); [ 8]
// toFixedLength(char *d, int dL, cchar *s, int sL, char pad= ' '); [ 2]
// toLower(char *string); [ 2] toLower(char *string, int length); [ 2]
// toLower(bsl::string *string); [ 2] toLower(std::string *string); [ 2]
// toLower(std::experimental::pmr::string *string); [ 2] toUpper(char *string);
...
static inline const char *ns(const char *string, int len)
// Return the specified 'string' if 'len >= 0', otherwise return null.
{
  return len < 0 ? 0 : string;
}

static inline int nneg(int len)
...
int main(int argc, char *argv[]) {
  ...
  case 11: {
...

    {
      ...
      for (int i = 0; i < DATA_LEN; ++i) {
        ...
        result = bdlb::String::copy(ns(STRING, CLIP_LEN), nneg(CLIP_LEN), &ta);
  ...
  case 9: {
    ...
    for (int i = 0; i < NUM_DATA; ++i) {
      ...
      const int RESULTREVERSECASELESS = DATA[i].d_resultReverseCaseless;

      if (veryVeryVerbose) {
        if (STRING) {
          T_ T_ P(STRING)
        }
        T_ T_ P(STRINGLEN) if (SUBSTRING){T_ T_ P(SUBSTRING)} T_ T_ P(
            SUBSTRINGLEN) T_ T_ P(RESULT) T_ T_ P(RESULTCASELESS)
            T_ T_ P(RESULTREVERSE) T_ T_ P(RESULTREVERSECASELESS)
      }

      // Run 'strstr', 'strstrCaseless', 'strrstr', and
      // 'strrstrCaseless'.
      const char *strstrResult =
          Util::strstr(ns(STRING, STRINGLEN), nneg(STRINGLEN),
                       ns(SUBSTRING, SUBSTRINGLEN), nneg(SUBSTRINGLEN));

      const char *strstrCaselessResult =
          Util::strstrCaseless(ns(STRING, STRINGLEN), nneg(STRINGLEN),
                               ns(SUBSTRING, SUBSTRINGLEN), nneg(SUBSTRINGLEN));

      const char *strrstrResult =
          Util::strrstr(ns(STRING, STRINGLEN), nneg(STRINGLEN),
                        ns(SUBSTRING, SUBSTRINGLEN), nneg(SUBSTRINGLEN));

      const char *strrstrCaselessResult = Util::strrstrCaseless(
          ns(STRING, STRINGLEN), nneg(STRINGLEN), ns(SUBSTRING, SUBSTRINGLEN),
          nneg(SUBSTRINGLEN));
      ...
      if (-1 == RESULT) {
        ...
        if (veryVeryVerbose) {
          T_ T_ P(RESULT) T_ T_ bsl::cout
              << "STRING + RESULT: " << bsl::hex
              << static_cast<const void *>(STRING + RESULT) << bsl::dec
              << bsl::endl;
          T_ T_ bsl::cout << "STRSTRRESULT: " << bsl::hex
                          << static_cast<const void *>(strstrResult) << bsl::dec
                          << bsl::endl;
        }
        ASSERT(strstrResult == (-1 == STRINGLEN ? 0 : STRING + RESULT));
      }
      ...
      if (-1 == RESULTCASELESS) {
        ...
        if (veryVeryVerbose) {
          T_ T_ P(RESULTCASELESS) T_ T_ bsl::cout
              << "STRING + RESULTCASELESS: " << bsl::hex
              << static_cast<const void *>(STRING + RESULTCASELESS) << bsl::dec
              << bsl::endl;
          T_ T_ bsl::cout << "STRSTRCASELESSRESULT: " << bsl::hex
                          << static_cast<const void *>(strstrCaselessResult)
                          << bsl::dec << bsl::endl;
        }
        ASSERT(strstrCaselessResult ==
               (-1 != STRINGLEN ? STRING + RESULTCASELESS : 0));
      }
      ...
      if (-1 == RESULTREVERSE) {
        ...
        if (veryVeryVerbose) {
          T_ T_ P(RESULT) T_ T_ bsl::cout
              << "STRING + RESULTREVERSE: " << bsl::hex
              << static_cast<const void *>(STRING + RESULTREVERSE) << bsl::dec
              << bsl::endl;
          T_ T_ bsl::cout << "STRRSTRRESULT: " << bsl::hex
                          << static_cast<const void *>(strrstrResult)
                          << bsl::dec << bsl::endl;
        }
        ASSERT(strrstrResult == (-1 != STRINGLEN ? STRING + RESULTREVERSE : 0));
      }
      ...
      if (-1 == RESULTREVERSECASELESS) {
        ...
        if (veryVeryVerbose) {
          T_ T_ P(RESULTREVERSECASELESS) T_ T_ bsl::cout
              << "STRING + RESULTREVERSECASELESS: " << bsl::hex
              << static_cast<const void *>(STRING + RESULTREVERSECASELESS)
              << bsl::dec << bsl::endl;
          T_ T_ bsl::cout << "STRRSTRCASELESSRESULT: " << bsl::hex
                          << static_cast<const void *>(strrstrCaselessResult)
                          << bsl::dec << bsl::endl;
        }
        ASSERT(strrstrCaselessResult ==
               (-1 != STRINGLEN ? STRING + RESULTREVERSECASELESS : 0));
      }
    }
  } break;
  case 8: {
    ...
    static const char *STRINGS[] = {
        ...
        // strings of length 3
        "   ", "\t\n\t", "***", "a**", "*a*", "**a", "aa*", "a*a", "*aa", "000",
        "a00", "0a0", "00a", "aa0", "a0a", "0aa", "aaa", "a  ", " a ", "  a",
        "aa ", "a a", " aa",
      ...
      for (int charIdx = 0; charIdx < NUM_CHARS; ++charIdx) {
        ...
        for (int dstLen = 0; dstLen < BUFSIZE; ++dstLen) {
          ...
          for (int srcLen = -1; srcLen < LEN; ++srcLen) {
            ...
            if ('x' != PADCHAR) {
              Util::toFixedLength(dstBuf, dstLen, ns(SRC, srcLen), nneg(srcLen),
                                  PADCHAR);
            } else {
              Util::toFixedLength(dstBuf, dstLen, ns(SRC, srcLen),
                                  nneg(srcLen));
              // default 'padChar'
            }
            ...
            bsl::string result(SRC, 0, bsl::min(nneg(srcLen), dstLen));
            ...
            if (dstLen < BUFSIZE) {
              ASSERTV(strIdx, charIdx, dstLen, srcLen,
                      0 == bsl::memcmp(&dstBuf[dstLen], &atBuf[dstLen],
                                       BUFSIZE - dstLen));
            }
          }
        }
  ...
  case 7: {
    ...
    for (int i = 0; STRINGS[i]; ++i) {
      ...
      for (int l = 0; l < 2 * LEN; ++l) {
        ...
        ASSERTV(i, l, bsl::min(l, LEN) == Util::strnlen(STRINGS[i], l));
      }
    }
  ...
  case 6: {
    ...
    //  pad(std::experimental::pmr::string *str, size_type numChars, char
    //  padChar = ' ');
    // --------------------------------------------------------------------
    ...
    static const char *STRINGS[] = {// strings of length 0 or 1
                                    "", " ", "\t", "p", "?", "1",
      ...
      for (int charIdx = 0; charIdx < NUM_CHARS; ++charIdx) {
        ...
        for (int length = 0; length < 2 * LEN; ++length) {
          ...
          if (LEN >= length) {
            ...
            ASSERTV(strIdx, charIdx,
                    bslString == STRING + bsl::string(length - LEN, CHKCHAR));
            ASSERTV(strIdx, charIdx,
                    stdString == STRING + std::string(length - LEN, CHKCHAR));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR_STRING
            ASSERTV(strIdx, charIdx,
                    pmrString == STRING + std::experimental::pmr::string(
                                              length - LEN, CHKCHAR));
#endif
          }
        }
  ...
  case 5: {
    ...
    static const char *STRINGS[] = {"", "t", "test", "test  test", 0};
    ...
    if (verbose)
      cout << "\tTesting 'char *', 'bsl::string', 'std::string'" << endl;
    ...
    for (int i = 0; STRINGS[i]; ++i) {
      ...
      for (int padBefore = 0; padBefore < 5; ++padBefore) {
        ...
        for (int padAfter = 0; padAfter < 5; ++padAfter) {
          ...
          ASSERTV(i,
                  0 == bsl::strncmp(bslLTrim.c_str(), nonNullString, length));
          ...
          ASSERTV(i,
                  0 == bsl::strncmp(bslRTrim.c_str(), nonNullString, length));
          ...
          ASSERTV(bsl::strlen(STRINGS[i]), bsl::strlen(STRINGS[i]) <= INT_MAX);
          ASSERTV(i, static_cast<int>(bsl::strlen(STRINGS[i])) == length);
          ASSERTV(i, 0 == bsl::strncmp(STRINGS[i], nonNullString, length));
        }
      }
    }

    if (verbose)
      cout << "\tTesting 'std::experimental::pmr::string'" << endl;
  ...
  case 4: {
    ...
    for (int nb = 0; nb < NUM_DATA; ++nb) {
      ...
      if (veryVeryVerbose) {
        T_ T_ P(STRING1) T_ T_ P(STRING2)
      }
      ...
      for (int i = 0; i < 2; ++i) {
        ...
        ASSERTV(nb, i, length1 < static_cast<int>(sizeof nonNullString1));
        int length2 = static_cast<int>(string2.size());
        ASSERTV(nb, i, length2 < static_cast<int>(sizeof nonNullString2));
        ...
        for (int len = -1; len <= length1; ++len) {
          ...
          result = result > 0 ? 1 : (result < 0 ? -1 : 0);
          ASSERTV(nb, result == Util::lowerCaseCmp(ns(nonNullString1, len),
                                                   nneg(len), cstring2));
          ASSERTV(nb, result == Util::lowerCaseCmp(ns(nonNullString1, len),
                                                   nneg(len), string2));
        }
        ...
        for (int len = -1; len <= length2; ++len) {
          ...
          result = result > 0 ? -1 : (result < 0 ? 1 : 0);
          ASSERTV(nb, result == Util::lowerCaseCmp(cstring1,
                                                   ns(nonNullString2, len),
                                                   nneg(len)));
          ASSERTV(nb,
                  result == Util::lowerCaseCmp(string1, ns(nonNullString2, len),
                                               nneg(len)));
        }
        ...
        for (int len1 = -1; len1 <= length1; ++len1) {
          ...
          for (int len2 = -1; len2 <= length2; ++len2) {
            ...
            result = lowerStr1.compare(0, nneg(len1), lowerStr2, 0, nneg(len2));
            result = result > 0 ? 1 : (result < 0 ? -1 : 0);
            ASSERTV(nb, result == Util::lowerCaseCmp(
                                      ns(nonNullString1, len1), nneg(len1),
                                      ns(nonNullString2, len2), nneg(len2)));
          }
        }
        ...
        for (int len = -1; len <= length1; ++len) {
          ...
          result = result > 0 ? 1 : (result < 0 ? -1 : 0);
          ASSERTV(nb, result == Util::upperCaseCmp(ns(nonNullString1, len),
                                                   nneg(len), cstring2));
          ASSERTV(nb, result == Util::upperCaseCmp(ns(nonNullString1, len),
                                                   nneg(len), string2));
        }
        ...
        for (int len = -1; len <= length2; ++len) {
          ...
          result = result > 0 ? -1 : (result < 0 ? 1 : 0);
          ASSERTV(nb, result == Util::upperCaseCmp(cstring1,
                                                   ns(nonNullString2, len),
                                                   nneg(len)));
          ASSERTV(nb,
                  result == Util::upperCaseCmp(string1, ns(nonNullString2, len),
                                               nneg(len)));
        }
        ...
        for (int len1 = -1; len1 <= length1; ++len1) {
          ...
          for (int len2 = -1; len2 <= length2; ++len2) {
            ...
            result = upperStr1.compare(0, nneg(len1), upperStr2, 0, nneg(len2));
            result = result > 0 ? 1 : (result < 0 ? -1 : 0);
            ASSERTV(nb, result == Util::upperCaseCmp(
                                      ns(nonNullString1, len1), nneg(len1),
                                      ns(nonNullString2, len2), nneg(len2)));
          }
        }
  ...
  case 3: {
    ...
    static const char *STRINGS[] = {
        ...
        "wXyZ", "WxYz", "abcdefghijklmnopqrstuvwxyz",
    ...
    for (int i = 0; STRINGS[i]; ++i) {
      ...
      for (int j = 0; STRINGS[j]; ++j) {
        ...
        ASSERTV(i, j, result == Util::areEqualCaseless(cstring1, cstring2));
        ASSERTV(i, j, result == Util::areEqualCaseless(cstring1, string2));
        ASSERTV(i, j, result == Util::areEqualCaseless(string1, string2));
        ASSERTV(i, j, result == Util::areEqualCaseless(string1, cstring2));

        // Concern 2

        char nonNullString1[64], nonNullString2[64];
        int length1 = static_cast<int>(string1.size());
        ASSERTV(i, j, length1 < static_cast<int>(sizeof nonNullString1));
        int length2 = static_cast<int>(string2.size());
        ASSERTV(i, j, length2 < static_cast<int>(sizeof nonNullString2));
        ...
        for (int len = -1; len <= length1; ++len) {
          ...
          ASSERTV(i, j,
                  result == Util::areEqualCaseless(ns(nonNullString1, len),
                                                   nneg(len), cstring2));
        ...
        for (int len = -1; len <= length2; ++len) {
          ...
          ASSERTV(i, j,
                  result == Util::areEqualCaseless(
                                string1, ns(nonNullString2, len), nneg(len)));

          ASSERTV(i, j,
                  result == Util::areEqualCaseless(
                                cstring1, ns(nonNullString2, len), nneg(len)));
        }
        ...
        for (int len1 = -1; len1 <= length1; ++len1) {
          ...
          for (int len2 = -1; len2 <= length2; ++len2) {
            ...
            result = (nneg(len1) == nneg(len2)) &&
                     !strncasecmp(nonNullString1, nonNullString2, nneg(len1));
            ASSERTV(i, j,
                    result == Util::areEqualCaseless(
                                  ns(nonNullString1, len1), nneg(len1),
                                  ns(nonNullString2, len2), nneg(len2)));
  ...
  case 2: {
    ...
    for (int i = 0; STRINGS[i]; ++i) {
...

      {
        ...
        bsl::transform(lowerStr.begin(), lowerStr.end(), lowerStr.begin(),
                       ::tolower);
        ...
        ASSERTV(i, 0 == bsl::strncmp(lowerStr.c_str(), nonNullString, LENGTH));
      ...
      }
      {
        ...
        bsl::transform(upperStr.begin(), upperStr.end(), upperStr.begin(),
                       ::toupper);
        ...
        ASSERTV(i, 0 == bsl::strncmp(upperStr.c_str(), nonNullString, LENGTH));
        ASSERTV(i, 'z' == nonNullString[LENGTH]);
      }
    }
  } break;
  case 1: {
    ...
    if (verbose)
      cout << "\tTesting 'areEqualCaseless'" << endl;
    ...
    if (verbose)
      cout << "\tTesting 'lowerCaseCmp'" << endl;
    ...
    if (verbose)
      cout << "\tTesting 'upperCaseCmp'" << endl;
    ...
    if (verbose)
      cout << "\tTesting 'toFixedLength'" << endl;
    ...
    if (verbose)
      cout << "\tTesting 'toLower' and 'toUpper'" << endl;
