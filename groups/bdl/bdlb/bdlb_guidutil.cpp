...
inline void makeRfc4122Compliant(unsigned char *bytes, const unsigned char *end)
...
// which formats a 'bdlb::Guid' into a string buffer.
{
  ...
  {
    bsl::span<char, Guid::k_GUID_NUM_CHARS> span(data, Guid::k_GUID_NUM_CHARS);
...
inline void reseed(GuidState_Imp *guidStatePtr)
...
// specified 'guidStatePtr' with seeds from 'RandomDevice'.
{
  ...
  typedef bsl::array<bsl::uint64_t, GuidState_Imp::k_GENERATOR_COUNT> state_t;
  ...
  if (0 != RandomDevice::getRandomBytesNonBlocking(
    ...
    state[2] =
        (seed << 32) ^ reinterpret_cast<uintptr_t>(&registerForkCallback);
...
void GuidUtil::generate(unsigned char *result, bsl::size_t numGuids) {
  ...
  if (BSLS_PERFORMANCEHINT_PREDICT_LIKELY(
          0 == RandomDevice::getRandomBytesNonBlocking(bytes, end - bytes))) {
    makeRfc4122Compliant(bytes, end);
  } else {
    for (bsl::size_t i = 0; i < numGuids; i++) {
      Guid temp = generateNonSecure();
      memcpy(result + i * Guid::k_GUID_NUM_BYTES, temp.data(),
             Guid::k_GUID_NUM_BYTES);
    }
  }
}
...
Guid GuidUtil::dnsNamespace() {
  static const unsigned char k_DNS_UUID[] = {0x6b, 0xa7, 0xb8, 0x10, 0x9d, 0xad,
...
Guid GuidUtil::urlNamespace() {
  static const unsigned char k_URL_UUID[] = {0x6b, 0xa7, 0xb8, 0x11, 0x9d, 0xad,
                                             0x11, 0xd1, 0x80, 0xb4, 0x00, 0xc0,
                                             0x4f, 0xd4, 0x30, 0xc8};
...
Guid GuidUtil::oidNamespace() {
  static const unsigned char k_OID_UUID[] = {0x6b, 0xa7, 0xb8, 0x12, 0x9d, 0xad,
                                             0x11, 0xd1, 0x80, 0xb4, 0x00, 0xc0,
                                             0x4f, 0xd4, 0x30, 0xc8};
...
int GuidUtil::guidFromString(Guid *result, const bsl::string_view &guidString) {
  ...
  while (iter != end && i < Guid::k_GUID_NUM_BYTES) {
    ...
    if (0 == charToHex(&upper, *iter) && 0 == charToHex(&lower, *(iter + 1))) {
      t_guid[i] =
          static_cast<unsigned char>(((upper << 4) & 0Xf0) | (lower & 0x0f));
      i++;
      iter += 2;
    } else {
      ++iter;
...
void GuidUtil::guidToString(std::experimental::pmr::string *result,
                            const Guid &guid) {
  guidToStringImpl(result, guid);
}
#endif
