...
// [ 4] void guidToString(std::experimental::pmr::string *result, const Guid&
// guid) [ 4] bsl::string guidToString(const Guid& guid) [ 5] Uint64
// getMostSignificantBits(const Guid& guid) [ 6] Uint64
// getLeastSignificantBits(const Guid& guid) [ 7] void generateNonSecure(Guid
// *result, size_t numGuids) [ 7] void generateNonSecure(unsigned char *result,
// size_t numGuids) [ 7] Guid generateNonSecure() [ 8] Guid
// generateFromName(const Guid& nsId, const string_view& name) [ 9] Guid
// dnsNamespace() [ 9] Guid urlNamespace() [ 9] Guid oidNamespace() [ 9] Guid
// x500Namespace()
// ----------------------------------------------------------------------------
...
struct UuidComponents {
  struct {
    unsigned char low[4], mid[2], high_vers[2];
  } time;
  unsigned char clock_seq[2];
  unsigned char node_id[6];
};

const Element VALUES[] = {{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                           0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},

                          {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08,
                           0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f},

                          {0x5c, 0x9d, 0x4e, 0x50, 0x0d, 0xf1, 0x11, 0xe4, 0x91,
                           0x91, 0x08, 0x00, 0x20, 0x0c, 0x9a, 0x66},

                          {0x5c, 0x9d, 0x4e, 0x51, 0x0d, 0xf1, 0x11, 0xe4, 0x91,
                           0x91, 0x08, 0x00, 0x20, 0x0c, 0x9a, 0x66},

                          {0x5c, 0x9d, 0x4e, 0x52, 0x0d, 0xf1, 0x11, 0xe4, 0x91,
                           0x91, 0x08, 0x00, 0x20, 0x0c, 0x9a, 0x66},

                          {0x5c, 0x9d, 0x4e, 0x53, 0x0d, 0xf1, 0x11, 0xe4, 0x91,
                           0x91, 0x08, 0x00, 0x20, 0x0c, 0x9a, 0x66},

                          {0x5c, 0x9d, 0x4e, 0x54, 0x0d, 0xf1, 0x11, 0xe4, 0x91,
                           0x91, 0x08, 0x00, 0x20, 0x0c, 0x9a, 0x66},

                          {0x5c, 0x9d, 0x4e, 0x56, 0x0d, 0xf1, 0x11, 0xe4, 0x91,
                           0x91, 0x08, 0x00, 0x20, 0x0c, 0x9a, 0x66},
...
const Element &V0 = VALUES[0], // V0, V1, ... are used in
    &V1 = VALUES[1], &V2 = VALUES[2], &V3 = VALUES[3], &V4 = VALUES[4],
...
// Additional output is generated based on the specified 'verboseFlag'.
{
  ...
  if (verboseFlag)
    printf("\tUse %s as a base filename.\n", result);
...
class MyEmployee {
  ...
  //..
  // For the sake of brevity, we provide a limited amount of data in each
  // record. We additionally show a very limited scope of functionality.
  //..
  // DATA
...
MyEmployee::MyEmployee(const string &name, double salary)
    : d_name(name), d_salary(salary) {
  bdlb::GuidUtil::generate(&d_guid);
}

// ACCESSORS
const bdlb::Guid &MyEmployee::Guid() const { return d_guid; }

const bsl::string &MyEmployee::name() const { return d_name; }

double MyEmployee::salary() const { return d_salary; }
...
int main(int argc, char *argv[]) {
  ...
  case 11: {
    ...
    if (verbose)
      cout << endl
           << "TESTING 'generateNonSecure' FROM FORK PER DRQS 168925481" << endl
           << "========================================================"
           << endl;

#ifndef BSLS_PLATFORM_OS_WINDOWS
    enum { k_GUID_STR_SIZE = 36, k_GUID_COUNT = 16 };
    ...
    bdlb::GuidUtil::generateNonSecure(preForkGuids.data(), preForkGuids.size());
    ...
    if (pid_t childPid = fork()) {
      ...
      if (veryVerbose) {
        bsl::cout << "will write guids to file: " << fileName << bsl::endl;
      }
      ...
      for (bsl::vector<Obj>::const_iterator it = tempChildBuffer.cbegin();
           it != tempChildBuffer.cend(); ++it) {
        bsl::string guidStr;
        bdlb::GuidUtil::guidToString(&guidStr, *it);
        if (veryVerbose) {
          bsl::cout << "generated child guid: " << guidStr << bsl::endl;
        }
        if (output) {
    ...
    for (bsl::vector<Obj>::const_iterator it = postForkParentGuids.cbegin();
         it != postForkParentGuids.cend(); ++it) {

      finalList.push_back(*it);
      if (veryVerbose) {
        bsl::cout << "checking post fork parent guid: " << *it << bsl::endl;
      }
    }
  ...
  case 10: {
    ...
    if (verbose)
      cout << endl
           << "TESTING 'generateNonSecure' FROM MULTIPLE THREADS" << endl
           << "=================================================" << endl;
    ...
    for (int i = 0; i < NUM_THREADS; ++i) {
      ASSERT(0 == bslmt::ThreadUtil::create(&handles[i], &threadFunction,
                                            &threadData));
    }
  ...
  case 8: {
    ...
    if (verbose)
      cout << endl
           ...
           << "==========================" << endl;
    const char *nsIdStrings[] = {"88515314-9c0d-44ac-bf6d-9a4cb5b2ec89",
                                 "80d90399-1dbe-498c-ae89-4855ebfa548f",
                                 "adde14a7-e049-4dfd-90da-d680b724a49c"};

    const int k_NUM_NAMESPACE_IDS = sizeof nsIdStrings / sizeof *nsIdStrings;
    ...
    for (int i = 0; i < NUM_DATA; ++i) {
      ...
      ASSERTV(LINE, Util::guidToString(result), Util::guidToString(expected),
              result == expected);
  ...
  case 6: {
    ...
    static const struct {
      ...
      const Element *d_array; // byte array
        ...
        // Line             // Array
        {L_, &V0}, {L_, &V1}, {L_, &V2}, {L_, &V3}, {L_, &V4},
        {L_, &V5}, {L_, &V6}, {L_, &V7}, {L_, &V8},
    };
    const int NUM_DATA = sizeof DATA / sizeof *DATA;
    for (int ti = 0; ti < NUM_DATA; ++ti) {
      ...
      if (veryVerbose) {
        P(LINE);
        P(X);
        cout << "lsb = " << hex << BSLS_BYTEORDER_BE_U64_TO_HOST(lsb) << dec
             << endl;
      }
  ...
  case 5: {
    ...
    for (int ti = 0; ti < NUM_DATA; ++ti) {
      ...
      if (veryVerbose) {
        P(LINE);
        P(X);
        cout << "msb = " << hex << BSLS_BYTEORDER_BE_U64_TO_HOST(msb) << dec
             << endl;
      }
  ...
  case 4: {
    ...
    //  void guidToString(std::experimental::pmr::string *result, const Guid&
    //  guid) bsl::string guidToString(const Guid& guid)
    // --------------------------------------------------------------------
    if (verbose)
      cout << endl
      ...
      const Element d_expGuidArray; // an array with the same value as
        ...
        {L_,
         ...
         {0x00, 0x10, 0x20, 0x30, 0x40, 0x50, 0x60, 0x70, 0x80, 0x90, 0xa0,
          0xb0, 0xc0, 0xd0, 0xe0, 0xf0}},
        {L_,
         "AA1A2A3A4A5A6A7A8A9AAABACADAEAFA",
         0,
         {0xaa, 0x1a, 0x2a, 0x3a, 0x4a, 0x5a, 0x6a, 0x7a, 0x8a, 0x9a, 0xaa,
        ...
        {L_,
         ...
         {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
          0x00, 0x00, 0x00, 0x00, 0x00}},
        {L_,
         "{ 00010203-0405-060708090a0b0c0d0e0f }",
         1,
         {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
          0x00, 0x00, 0x00, 0x00, 0x00}},
    };
  ...
  case -1: {
    ...
    cout << "\n\n";
    cout << "batchSize" << ", threadCount" << ", executable" << ", userTime"
         << ", systemTime" << ", wallTime" << ", cpuTime" << endl;
      ...
      for (int bsp = 0; bsp <= MAX_BATCH_POWER_OF_2; bsp++) {
        ...
        for (int i = 0; i < numThreads; ++i) {
          ASSERT(0 == bslmt::ThreadUtil::create(
                          &handles[i], &threadMultiPerfFunction, &threadData));
        }
        ...
        cout << threadData.d_batchSize << ", " << numThreads << ", " << argv[0]
  ...
  case -2: {
    ...
    if (verbose)
      cout << endl
           << "COLLISION TESTING 'GuidState_Imp'" << endl
           << "=================================" << endl;
    ...
    bsl::array<uint64_t, bdlb::GuidState_Imp::k_GENERATOR_COUNT> seeds = {0, 0,
                                                                          0, 0};
...

    {
      ...
      for (uint64_t i = 0; i <= ITERATIONS; ++i) {
        ...
        if (exists) {
          // Create test guid:
          bdlb::Guid guid;
          bsl::memcpy(reinterpret_cast<unsigned char *>(&guid), randomInts,
        ...
        if (it != check.end()) {
          if (++it->second > 1) {
            ...
            printf("Collision encountered at iteration #%llu with "
                   "GUID %s\n",
                   static_cast<long long unsigned int>(i), guidStr.c_str());
          }
        }
      }
