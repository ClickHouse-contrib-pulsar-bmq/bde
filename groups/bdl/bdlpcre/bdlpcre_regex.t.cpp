...
// [ 4] int match(std::experimental::pmr::vector<bsl::string_view> *result, ...)
// const; [ 4] int matchRaw(const bsl::string_view& subject, ...) const; [ 4]
// int matchRaw(const char *subject, ...) const; [ 4] int
// matchRaw(bsl::pair<size_t, size_t> *result, ...) const; [ 4] int
// matchRaw(bsl::vector<bsl::pair<size_t, size_t> > *result,...)const; [ 4] int
// matchRaw(bsl::vector<bsl::string_view> *result, ...) const; [ 4] int
// matchRaw(std::vector<bsl::string_view> *result, ...) const; [ 4] int
// matchRaw(std::experimental::pmr::vector<bsl::string_view> *result, ...)
// const; [ 5] int match(bslstl::StringRef *result, ...) const; [ 5] int
// match(bsl::vector<bslstl::StringRef> *result, ...) const; [ 5] int
// matchRaw(bslstl::StringRef *result, ...) const; [ 5] int
// matchRaw(bsl::vector<bslstl::StringRef> *result, ...) const; [11] int
// numSubpatterns() const; [ 2] const bsl::string& pattern() const; [11] int
// subpatternIndex(const char *name) const; [15] int depthLimit(int); [15] int
// defaultDepthLimit(int); [13] bool isJitAvailable(); [16] int
// replace(bsl::string *, int *, const string_view&, ...) const; [16] int
// replace(std::string *, int *, const string_view&, ...) const; [16] int
// replace(std::experimental::pmr::string *, int *, const string_view&, ...)
// const; [16] int replaceRaw(bsl::string *, int *, const string_view&, ...)
// const; [16] int replaceRaw(std::string *, int *, const string_view&, ...)
// const; [16] int replaceRaw(std::experimental::pmr::string *, int *, ...)
// const;
// ----------------------------------------------------------------------------
// [ 1] BREATHING TEST
// [ 7] k_FLAG_CASELESS
...
template <class T> void printValue(ostream &out, const T &value) {
  out << value;
}
...
public:
  ...
  MatchJob(const RegEx *regEx, const char *subject, const char *match)
      ...
      // and 'match'.
      : d_regEx_p(regEx), d_subject_p(subject), d_match_p(match){};
...
void MatchJob::doMatch() const {
  ...
  ASSERTV(result1.first, result1.second,
          subjectLength >= result1.first + result1.second);

  ASSERTV(d_match_p ==
          bslstl::StringRef(d_subject_p + result1.first, result1.second));
  ...
  for (size_t i = 0; i < result2.size(); ++i) {
    ASSERTV(i, d_subject_p, result2[i].first, result2[i].second,
            subjectLength >= result2[i].first + result2[i].second);
  }

  ASSERTV(d_match_p ==
          bslstl::StringRef(d_subject_p + result2[0].first, result2[0].second));
...
// This thread function performs match test for precompiled 'RegEx' object.
{
  ...
  if (veryVeryVerbose) {
    cout << "Match thread started with id: "
         << bslmt::ThreadUtil::selfIdAsUint64() << endl;
  }
...
public:
  // CREATORS
  PrepareJob(RegEx *regEx, const char *pattern, int flags, size_t jitStackSize)
      // Create match prepare object with the specified 'regEx', 'pattern',
      // 'flags', and 'jitStackSize'
      : d_regEx_p(regEx), d_pattern_p(pattern), d_flags(flags),
...
void PrepareJob::doPrepare() {
  int retCode = d_regEx_p->prepare(0, 0, d_pattern_p, d_flags, d_jitStackSize);
...
int countNames(const bsl::vector<bsl::pair<bsl::string_view, int>> &names,
               const char *name) {
  int count = 0;
...
//..
int parseSubject(bsl::string *result, const char *message,
...
//      // a non-zero value otherwise with no effect on 'result'.
{
  ...
  // the text between the ':' and the '\r' in the "Subject:" field of the
  // header) by the name "subjectText":
  //..
  ...
  int returnValue =
      regEx.prepare(&errorMessage, &errorOffset, PATTERN,
                    RegEx::k_FLAG_CASELESS | RegEx::k_FLAG_MULTILINE);
  ...
  // 'message' that match the prepared 'PATTERN'.  All variants of the
  // overloaded 'match' method return 0 if a match is found, and return a
  ...
  // within 'message'.  The text is then extracted from 'message' and assigned
  // to the 'result' "out" parameter:
  //..
  const bsl::pair<size_t, size_t> capturedSubject =
      matchVector[regEx.subpatternIndex("subjectText")];

  *result =
      bsl::string(&message[capturedSubject.first], capturedSubject.second);
...
int main(int argc, char *argv[]) {
  ...
  case 22: {
    ...
    //..
    // The following array contains the sample Internet e-mail message from
    // which we will extract the subject:
    //..
    ...
    // 'RFC822_MESSAGE'.  The assertions verify that the subject of the message
    // is correctly extracted and assigned to the local 'subject' variable:
    //..
    //  int main()
    //  {
    bsl::string subject;
    const int returnValue =
        parseSubject(&subject, RFC822_MESSAGE, sizeof(RFC822_MESSAGE) - 1);
    ASSERT(0 == returnValue);
  ...
  case 21: {
    ...
    if (verbose)
      cout << "\tVerify not supplying duplicate option"
              " results in an error"
           << endl;
    {
    ...
    if (verbose)
      cout << "\tSupply (?J) in the regular expression" << endl;
    {
    ...
    if (verbose)
      cout << "\tSupply 'k_FLAG_DUPNAMES' flag to prepare" << endl;
    {
      int retCode =
          mX.prepare(&errorMsg, &errorOffset, PATTERN, Obj::k_FLAG_DUPNAMES);
      ASSERTV(errorMsg, errorOffset, retCode == 0);
    }

    if (verbose)
      cout << "\tVerify 'subpatternIndex()' fails with"
              " duplicate name"
           << endl;
    { ASSERT(X.subpatternIndex(SUBPATTERN_NAME) == -1); }

    if (verbose)
      cout << "\tVerify 'namedSubpatterns()'" << endl;
    {
    ...
    if (verbose)
      cout << "\tVerify that the feature works for"
              " non-duplicate names as well"
           << endl;
    {
  ...
  case 20: {
    ...
    // --------------------------------------------------------------------
    if (verbose)
      cout << endl << "JIT REGRESSION" << endl << "==============" << endl;
    ...
    retCode = mX.prepare(&errorMsg, &errorOffset, PATTERN, Obj::k_FLAG_JIT, 0);
    ...
    retCode = mY.prepare(&errorMsg, &errorOffset, PATTERN, 0, 0);
    ASSERTV(retCode, errorMsg, errorOffset, 0 == retCode);
  ...
  case 19: {
    ...
    for (int ti = 0; ti < NUM_DATA; ++ti) {
      for (char cfg = 'a'; cfg <= 'c'; ++cfg) {
        ...
        switch (cfg) {
        case 'a': {
          retCode = mX.prepare(&errorMsg, &errorOffset, PATTERN, 0, 0);
        } break;
        case 'b': {
          retCode =
              mX.prepare(&errorMsg, &errorOffset, PATTERN, Obj::k_FLAG_JIT, 0);
        } break;
        case 'c': {
          retCode = mX.prepare(&errorMsg, &errorOffset, PATTERN,
                               Obj::k_FLAG_JIT, 8192);
        } break;
        default: {
        ...
        for (int i = 0; i < 8; ++i) {
          int rc =
              bslmt::ThreadUtil::create(&threads[i], testMatchFunction, &job);
          ASSERTV(rc, 0 == rc);
        }
  ...
  case 18: {
    ...
    bdlma::BufferedSequentialAllocator arena(buffer.buffer(), sizeof(buffer));
  ...
  case 17: {
    ...
    for (size_t i = 0; i < NUM_DATA; ++i) {
      ...
      int retCode =
          mX.prepare(&errorMsg, &errorOffset, PATTERN, Obj::k_FLAG_UTF8);
  ...
  case 16: {
...

    {
      ...
      static const struct {
        ...
        int d_expectedOffset;
                  ...
                  //                                  expected  error
                  // line   subject      replacement    flags        string rc
                  // offset
                  //----   -------      -----------   --------     --------
                  //--------  ------
                  {L_, "=**=", "A", 0, "=**=", 0, 0},
      ...
      for (size_t i = 0; i < NUM_DATA; ++i) {
        ...
        if (veryVerbose) {
          T_ P_(LINE) P_(SUBJECT) P_(REPLACEMENT) P_(FLAGS) P_(RC) P(EXPECTED)
        }
        ...
        for (size_t j = 0; j < EXPECTED_LENGTH + 1; ++j) {
          ...
          { // replace
            ...
            retCode = X.replace(&result1, &offset, SUBJECT, REPLACEMENT, FLAGS);
            ...
            retCode = X.replace(&result2, &offset, SUBJECT, REPLACEMENT, FLAGS);
            ...
            retCode = X.replace(&result3, &offset, SUBJECT, REPLACEMENT, FLAGS);
          ...
          { // replaceRaw
            ...
            retCode =
                X.replaceRaw(&result1, &offset, SUBJECT, REPLACEMENT, FLAGS);
            ...
            retCode =
                X.replaceRaw(&result2, &offset, SUBJECT, REPLACEMENT, FLAGS);
            ...
            retCode =
                X.replaceRaw(&result3, &offset, SUBJECT, REPLACEMENT, FLAGS);
    ...
    { // Test NO UTF8 CHECK
      ...
      for (int i = 0; i < NUM_DATA; ++i) {
        ...
        size_t errorOffset;
        int retCode =
            mX.prepare(&errorMsg, &errorOffset, PATTERN, PREPARE_OPTIONS);
        ...
        retCode =
            UTF8_VALIDATION
                ? X.replace(&result, &offset,
                            string_view((const char *)SUBJECT_UTF8, 2),
                            string_view((const char *)REPLACE_UTF8, 2), 0)
                : X.replaceRaw(&result, &offset,
                               string_view((const char *)SUBJECT_UTF8, 2),
                               string_view((const char *)REPLACE_UTF8, 2), 0);
        ...
        retCode =
            UTF8_VALIDATION
                ? X.replace(&result, &offset,
                            ...
                            string_view((const char *)&REPLACE_REGULAR, 1), 0)
                : X.replaceRaw(&result, &offset,
                               string_view((const char *)&SUBJECT_REGULAR, 1),
  ...
  case 14: {
...

    {
      ...
      ASSERT(0 == mX1.prepare(&errorMsg, &errorOffset, PATTERN, 0));
      ASSERT(0 ==
             mX2.prepare(&errorMsg, &errorOffset, PATTERN, Obj::k_FLAG_JIT));
...

    {
      ...
      ASSERT(0 ==
             mX1.prepare(&errorMsg, &errorOffset, PATTERN, Obj::k_FLAG_JIT, 0));
      ASSERT(0 ==
             mX2.prepare(&errorMsg, &errorOffset, PATTERN, Obj::k_FLAG_JIT, 1));
...

    {
      ...
      ASSERT(0 ==
             mX.prepare(&errorMsg, &errorOffset, PATTERN, Obj::k_FLAG_JIT, 1));
      if (Obj::isJitAvailable()) {
      ...
      ASSERT(0 == mX.prepare(&errorMsg, &errorOffset, PATTERN, Obj::k_FLAG_JIT,
                             32768));
      if (Obj::isJitAvailable()) {
  ...
  case 12: {
...

    {
      ...
      ASSERT(0 == mX.prepare(&errorMsg, &errorOffset, PATTERN,
...

      {
        ...
        bsl::string realMatch(&TEST_STRING[match.first], match.second,
                              Z2); // do not use default allocator!
      ...
      const size_t LEN = 3;
      const char EXPECTED_MATCHES[LEN][30] = {"bbasm_SecurityCache", "bbasm",
                                              "SecurityCache"};

      ASSERTV(vMatch.size(), LEN == (int)vMatch.size());
      for (size_t i = 0; i < LEN; ++i) {
        bsl::string realMatch(&TEST_STRING[vMatch[i].first], vMatch[i].second,
                              Z2); // do not use default allocator!
  ...
  case 11: {
    ...
    const char CS_WORD[4][10] = {"Security", "Data", "Chunk", "Resolver"};
    ...
    for (int i = 0; i < NUM_PATTERNS; ++i) {
      ...
      if (veryVeryVerbose) {
        T_ T_ P_(X.numSubpatterns()) P_(X.subpatternIndex(SPN_PKG))
            P(X.subpatternIndex(SPN_NAME))
      }

      ASSERTV(LINE, X.numSubpatterns(), NUM_SUBPATTERNS == X.numSubpatterns());
      ...
      bsl::string BAD_SPN_NAME(SPN_NAME);
      BAD_SPN_NAME += "xYz";
      bsl::string BAD_SPN_PKG(SPN_PKG);
      BAD_SPN_PKG += "AbC";
      ...
      for (int j = 0; j < NUM_WORDS; ++j) {
        ...
        bsl::string BAD_SPN_WORDJ(SPN_WORDJ);
        BAD_SPN_WORDJ += "#%$";
      ...
      if (veryVeryVerbose) {
        cout << "\n\tMatching with offsets based 'match'/'matchRaw'" << endl;
      }
      {
        ...
        for (int i = 0; i < 2; ++i) {
          ...
          if (4 == NUM_WORDS) {
            ...
            const pair<size_t, size_t> NOT_FOUND(Obj::k_INVALID_OFFSET, 0);
      ...
      }
      {
        ...
        for (int i = 0; i < 2; ++i) {
          ...
          if (4 == NUM_WORDS) {
            ...
            const bslstl::StringRef &lastElement = vMatch[vMatch.size() - 1];
            ...
            const bslstl::StringRef csLastWord = vMatch[vMatch.size() - 1];
  ...
  case 10: {
    ...
    Obj mX(&ta);
    const Obj &X = mX; // has   'k_FLAG_DOTMATCHESALL'
    Obj mY(&ta);
    const Obj &Y = mY; // !have 'k_FLAG_DOTMATCHESALL'
    ...
    int retCode =
        mX.prepare(&errorMsg, &errorOffset, PATTERN, Obj::k_FLAG_DOTMATCHESALL);
    ASSERTV(errorMsg, errorOffset, 0 == retCode);
    ...
    for (size_t i = 0; i < NUM_DATA; ++i) {
      ...
      const size_t MATCH_LENGTH = SUBJECT_LEN - PREAMBLE_LEN - POSTAMBLE_LEN;

      if (veryVerbose) {
        T_ P_(LINE) P_(SUBJECT) P_(SUBJECT_LEN) P_(PREAMBLE_LEN)
            P(POSTAMBLE_LEN)
      }
  ...
  case 9: {
    ...
    for (size_t i = 0; i < NUM_DATA; ++i) {
      ...
      Obj mX(&ta);
      const Obj &X = mX; // has   'k_FLAG_UTF8'
      Obj mY(&ta);
      const Obj &Y = mY; // !have 'k_FLAG_UTF8'
...

      {
        ...
        retCode = X.match(&sv, string_view((const char *)UTF8_SUBJECT, 2));
        ASSERTV(LINE, retCode, 0 == retCode);

        retCode = Y.match(&sv, string_view((const char *)UTF8_SUBJECT, 2));
        ASSERTV(LINE, retCode, 0 != retCode);
        ...
        retCode = X.match(&vsv1, string_view((const char *)UTF8_SUBJECT, 2));
        ASSERTV(LINE, retCode, 0 == retCode);

        retCode = Y.match(&vsv1, string_view((const char *)UTF8_SUBJECT, 2));
        ASSERTV(LINE, retCode, 0 != retCode);

        std::vector<bsl::string_view> vsv2;

        retCode = X.match(&vsv2, string_view((const char *)UTF8_SUBJECT, 2));
        ASSERTV(LINE, retCode, 0 == retCode);

        retCode = Y.match(&vsv2, string_view((const char *)UTF8_SUBJECT, 2));
        ASSERTV(LINE, retCode, 0 != retCode);

#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
        std::experimental::pmr::vector<bsl::string_view> vsv3;

        retCode = X.match(&vsv3, string_view((const char *)UTF8_SUBJECT, 2));
        ASSERTV(LINE, retCode, 0 == retCode);

        retCode = Y.match(&vsv3, string_view((const char *)UTF8_SUBJECT, 2));
        ASSERTV(LINE, retCode, 0 != retCode);
#endif
      }

      {
        ...
        retCode = X.match(string_view((const char *)&REGULAR_SUBJECT, 1));
        ASSERTV(LINE, retCode, 0 != retCode);
        retCode = Y.match(string_view((const char *)&REGULAR_SUBJECT, 1));
        ASSERTV(LINE, retCode, 0 == retCode);
        ...
        retCode = X.match(&sv, string_view((const char *)&REGULAR_SUBJECT, 1));
        ASSERTV(LINE, retCode, 0 != retCode);

        retCode = Y.match(&sv, string_view((const char *)&REGULAR_SUBJECT, 1));
        ASSERTV(LINE, retCode, 0 == retCode);
        ...
        retCode =
            X.match(&vsv1, string_view((const char *)&REGULAR_SUBJECT, 1));
        ASSERTV(LINE, retCode, 0 != retCode);

        retCode =
            Y.match(&vsv1, string_view((const char *)&REGULAR_SUBJECT, 1));
        ASSERTV(LINE, retCode, 0 == retCode);

        std::vector<bsl::string_view> vsv2;

        retCode =
            X.match(&vsv2, string_view((const char *)&REGULAR_SUBJECT, 1));
        ASSERTV(LINE, retCode, 0 != retCode);

        retCode =
            Y.match(&vsv2, string_view((const char *)&REGULAR_SUBJECT, 1));
        ASSERTV(LINE, retCode, 0 == retCode);
        ...
        retCode =
            X.match(&vsv3, string_view((const char *)&REGULAR_SUBJECT, 1));
        ASSERTV(LINE, retCode, 0 != retCode);

        retCode =
            Y.match(&vsv3, string_view((const char *)&REGULAR_SUBJECT, 1));
        ASSERTV(LINE, retCode, 0 == retCode);
#endif
      }
    }
  ...
  case 8: {
    ...
    Obj mX(&ta);
    const Obj &X = mX; // has 'k_FLAG_MULTILINE'
    Obj mY(&ta);
    const Obj &Y = mY; // !have 'k_FLAG_MULTILINE'
    ...
    int retCode =
        mX.prepare(&errorMsg, &errorOffset, PATTERN, Obj::k_FLAG_MULTILINE);
    ASSERTV(errorMsg, errorOffset, 0 == retCode);
    ...
    for (size_t i = 0; i < NUM_DATA; ++i) {
      ...
      const size_t MATCH_LENGTH =
          SUBJECT_LEN - PREAMBLE_LEN - POSTAMBLE_LEN - 2;
  ...
  case 7: {
    ...
    Obj mX(&ta);
    const Obj &X = mX; // has   'k_FLAG_CASELESS'
    Obj mY(&ta);
    const Obj &Y = mY; // !have 'k_FLAG_CASELESS'
    ...
    int retCode =
        mX.prepare(&errorMsg, &errorOffset, PATTERN, Obj::k_FLAG_CASELESS);
  ...
  case 6: {
      ...
      cout << "\nTesting a valid pattern." << endl;
    {
      ...
      const int flags = Obj::k_FLAG_MULTILINE | Obj::k_FLAG_CASELESS |
                        Obj::k_FLAG_UTF8 | Obj::k_FLAG_DOTMATCHESALL |
                        Obj::k_FLAG_JIT;
  ...
  case 5: {
    ...
    for (size_t i = 0; i < NUM_DATA; ++i) {
      ...
      for (size_t subjectStart = 0; subjectStart <= SUBJECT_LEN;
        ...
        retCodeView = X.match(&matchView, string_view(SUBJECT, SUBJECT_LEN),
                              subjectStart);
        retCode = X.match(&match, SUBJECT, SUBJECT_LEN, subjectStart);
        retCodeRawView = X.matchRaw(
            &matchRawView, string_view(SUBJECT, SUBJECT_LEN), subjectStart);
        retCodeRaw = X.matchRaw(&matchRaw, SUBJECT, SUBJECT_LEN, subjectStart);
        ...
        retCodeView = X.match(&vMatchView, string_view(SUBJECT, SUBJECT_LEN),
                              subjectStart);
        retCode = X.match(&vMatch, SUBJECT, SUBJECT_LEN, subjectStart);
        retCodeRawView = X.matchRaw(
            &vMatchRawView, string_view(SUBJECT, SUBJECT_LEN), subjectStart);
        retCodeRaw = X.matchRaw(&vMatchRaw, SUBJECT, SUBJECT_LEN, subjectStart);
        ...
        if (subjectStart <= MATCH_OFFSET) {
          ...
          ASSERTV(LINE, subjectStart, MATCH_STRING == vMatchRawView[0]);
      ...
      cout << "\nTesting empty subjects." << endl;
    {
      ...
      Obj mGood(&ta);
      const Obj &GOOD = mGood;
      Obj mNotGood(&ta);
      const Obj &NOT_GOOD = mNotGood;

      if (veryVerbose) {
        cout << "\tPreparing regular expression objects." << endl;
      }

      int retCode = mGood.prepare(&errorMsg, &errorOffset, GOOD_PATTERN, 0);
      ASSERTV(errorMsg, errorOffset, 0 == retCode);

      retCode = mNotGood.prepare(&errorMsg, &errorOffset, NOT_GOOD_PATTERN, 0);
      ASSERTV(errorMsg, errorOffset, 0 == retCode);
  ...
  case 4: {
    ...
    //   ...) const; int matchRaw(const char *subject, ...) const; int
    //   matchRaw(bsl::pair<size_t, size_t> *result, ...) const; int
    //   matchRaw(vector<bsl::pair<size_t, size_t> > *result,...)const; int
    //   matchRaw(bsl::vector<bsl::string_view> *result, ...) const; int
    //   matchRaw(bsl::vector<bsl::string_view> *result, ...) const; int
    //   matchRaw(std::experimental::pmr::vector<bsl::string_view>
    //   *result,...)const;
    // --------------------------------------------------------------------
    if (verbose)
      cout << endl
    ...
    for (size_t i = 0; i < NUM_DATA; ++i) {
      ...
      const string_view EXPECTED =
          string_view(SUBJECT + MATCH_OFFSET, MATCH_LENGTH);
      ...
      for (size_t subjectStart = 0; subjectStart <= SUBJECT_LEN;
        ...
        retCode = X.match(string_view(SUBJECT, SUBJECT_LEN), subjectStart);
        retCode1 = X.match(SUBJECT, SUBJECT_LEN, subjectStart);
        retCodePr = X.match(&pr, SUBJECT, SUBJECT_LEN, subjectStart);
        retCodeSr = X.match(&sr, SUBJECT, SUBJECT_LEN, subjectStart);
        retCodeSv =
            X.match(&sv, string_view(SUBJECT, SUBJECT_LEN), subjectStart);
        retCodeVPr = X.match(&vpr, SUBJECT, SUBJECT_LEN, subjectStart);
        retCodeVSr = X.match(&vsr, SUBJECT, SUBJECT_LEN, subjectStart);
        retCodeVSv =
            X.match(&vsv, string_view(SUBJECT, SUBJECT_LEN), subjectStart);
        retCodeVSv1 =
            X.match(&vsv1, string_view(SUBJECT, SUBJECT_LEN), subjectStart);
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
        retCodeVSv2 =
            X.match(&vsv2, string_view(SUBJECT, SUBJECT_LEN), subjectStart);
#endif
        if (subjectStart <= MATCH_OFFSET) {
          ASSERTV(LINE, subjectStart, 0 == retCode);
          ASSERTV(LINE, subjectStart, 0 == retCode1);
          ASSERTV(LINE, subjectStart, 0 == retCodePr);
          ASSERTV(LINE, subjectStart, pr.first, MATCH_OFFSET == pr.first);
          ASSERTV(LINE, subjectStart, pr.second, MATCH_LENGTH == pr.second);
          ...
          ASSERTV(LINE, subjectStart, EXPECTED, vsr[0], EXPECTED == vsr[0]);
          ASSERTV(LINE, subjectStart, 0 == retCodeVSv);
          ASSERTV(LINE, subjectStart, EXPECTED, vsv[0], EXPECTED == vsv[0]);
          ASSERTV(LINE, subjectStart, 0 == retCodeVSv1);
          ASSERTV(LINE, subjectStart, EXPECTED, vsv1[0], EXPECTED == vsv1[0]);
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
          ASSERTV(LINE, subjectStart, 0 == retCodeVSv2);
          ASSERTV(LINE, subjectStart, EXPECTED, vsv2[0], EXPECTED == vsv2[0]);
#endif
        } else {
          ASSERTV(LINE, subjectStart, 0 != retCode);
          ASSERTV(LINE, subjectStart, 0 != retCode1);
        ...
        retCode = X.matchRaw(string_view(SUBJECT, SUBJECT_LEN), subjectStart);
        retCode1 = X.matchRaw(SUBJECT, SUBJECT_LEN, subjectStart);
        retCodePr = X.matchRaw(&pr, SUBJECT, SUBJECT_LEN, subjectStart);
        retCodeSr = X.matchRaw(&sr, SUBJECT, SUBJECT_LEN, subjectStart);
        retCodeSv =
            X.matchRaw(&sv, string_view(SUBJECT, SUBJECT_LEN), subjectStart);
        retCodeVPr = X.matchRaw(&vpr, SUBJECT, SUBJECT_LEN, subjectStart);
        retCodeVSr = X.matchRaw(&vsr, SUBJECT, SUBJECT_LEN, subjectStart);
        retCodeVSv =
            X.matchRaw(&vsv, string_view(SUBJECT, SUBJECT_LEN), subjectStart);
        retCodeVSv1 =
            X.matchRaw(&vsv1, string_view(SUBJECT, SUBJECT_LEN), subjectStart);
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
        retCodeVSv2 =
            X.matchRaw(&vsv2, string_view(SUBJECT, SUBJECT_LEN), subjectStart);
#endif
        if (subjectStart <= MATCH_OFFSET) {
          ASSERTV(LINE, subjectStart, 0 == retCode);
          ASSERTV(LINE, subjectStart, 0 == retCode1);
...

    {
      ...
      int retCode = mGood.prepare(&errorMsg, &errorOffset, GOOD_PATTERN, 0, 0);
      ASSERTV(errorMsg, errorOffset, 0 == retCode);

      retCode =
          mNotGood.prepare(&errorMsg, &errorOffset, NOT_GOOD_PATTERN, 0, 0);
      ASSERTV(errorMsg, errorOffset, 0 == retCode);
      ...
      cout << "\nNegative Testing." << endl;
    {
      ...
      {
        bsl::vector<bsl::string_view> v, *zv = 0;
        (void)zv;
      ...
      {
        std::vector<bsl::string_view> v, *zv = 0;
        (void)zv;
      ...
      {
        std::experimental::pmr::vector<bsl::string_view> v, *zv = 0;
        (void)zv;
  ...
  case 3: {
      ...
      cout << "\nTesting an invalid pattern." << endl;
    {
      ...
      int retCode = mX.prepare(&errorMsg, &errorOffset, INVALID_PATTERN, 0, 0);
  ...
  case 2: {
      ...
      cout << "\nAllocator installation test." << endl;
    {
      ...
      if (verbose)
        cout << "\tConstructor with explicit default allocator." << endl;
      {
        bslma::TestAllocator da("default", veryVeryVeryVerbose);
  ...
  case 1: {
    ...
    if (verbose) {
      cout << "\nVerifying regular match routine with 0 start " << "position."
           << endl;
    }
...

    {
      ...
      retCode = X.match(&match, TEST_STRING, TEST_STRING_LEN, startPosition);
    ...
    if (verbose) {
      cout << "\nVerifying vector match routine with 0 start " << "position."
           << endl;
    }
...

    {
      ...
      for (int i = 0; i < LEN; ++i) {
        string realMatch(&TEST_STRING[vMatch[i].first], vMatch[i].second);
...

    {
      ...
      retCode = X.match(&vMatch, TEST_STRING, TEST_STRING_LEN, startPosition);
      ASSERT(0 == retCode);

      const int LEN = 3;
      const char EXPECTED_MATCHES[LEN][30] = {"tweut_StringRef", "tweut",
                                              "StringRef"};
  ...
  case -1: {
    ...
    for (size_t i = 0; i < NUM_DATA; ++i) {
...

      {
        ...
        int rc = bslmt::ThreadUtil::create(&handle, testPrepareFunction, &job);
        ASSERTV(rc, 0 == rc);
      ...
      if (veryVerbose) {
        cout << "\tResults:" << endl
             ...
             << "\t\t'prepare'(JIT)   time: " << prepareJitTime << endl;
      }

      ASSERTV(LINE, matchTimeX, matchJitTimeX, matchTimeX > matchJitTimeX);
      ASSERTV(LINE, prepareTime, prepareJitTime, prepareTime < prepareJitTime);
    }
  } break;
  case -2: {
    // --------------------------------------------------------------------
    ...
    if (verbose) {
      cout << "Testing 'match' with UTF string validity check." << endl;
    }

    int retCode =
        mX.prepare(&errorMsg, &errorOffset, PATTERN, Obj::k_FLAG_UTF8, 0);
    ASSERTV(errorMsg, errorOffset, 0 == retCode);
    ...
    if (verbose) {
      cout << "Testing 'match' without UTF string validity check." << endl;
    }
