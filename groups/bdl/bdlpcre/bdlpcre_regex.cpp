...
void *bdlpcre_malloc(size_t size, void *context) {
  ...
  BSLS_TRY { result = basicAllocator->allocate(size); }
  BSLS_CATCH(...) {}
...
struct RegEx_ImpUtil {
  ...
  // CLASS METHODS
  static void assign(bsl::pair<size_t, size_t> *value, const char *subject,
  ...
  static void assign(bsl::string_view *value, const char *subject,
                     size_t offset, size_t length)
...
template <class DATA> class DataExtractor {
...
public:
  // CREATORS
  explicit DataExtractor(DATA *dataPtr) : d_data_p(dataPtr) {
    BSLS_ASSERT(dataPtr);
  }

  // ACCESSORS
  void operator()(const char *subject, PCRE2_SIZE *ovector,
...
template <class VECTOR> class VectorExtractor {
...
public:
  // CREATORS
  explicit VectorExtractor(VECTOR *vectorPtr) : d_vector_p(vectorPtr) {
  ...
  void operator()(const char *subject, PCRE2_SIZE *ovector,
    ...
    for (unsigned int i = 0, j = 0; i < ovectorCount; ++i, j += 2) {
      DataExtractor<typename VECTOR::value_type> extractor(&(*d_vector_p)[i]);
...
private:
  ...
  void deallocateMatchContext(RegEx_MatchContextData *matchContextData) const;
...
public:
  ...
  // MANIPULATORS
  int initialize(pcre2_general_context *pcre2Context, pcre2_code *patternCode,
                 int depthLimit, size_t jitStackSize);
...
RegEx_MatchContext::RegEx_MatchContext()
    : d_pcre2Context_p(0), d_pcre2PatternCode_p(0), d_depthLimit(0),
      d_jitStackSize(0), d_mainThread(bslmt::ThreadUtil::invalidHandle()),
...
// PRIVATE ACCESSORS
int RegEx_MatchContext::allocateMatchContext(
  ...
  pcre2_match_data *matchData_p =
      pcre2_match_data_create_from_pattern(d_pcre2PatternCode_p, 0);
  ...
  if (d_jitStackSize) {
    jitStack_p = pcre2_jit_stack_create(d_jitStackSize, d_jitStackSize,
                                        d_pcre2Context_p);
    if (0 == jitStack_p) {
...
void RegEx_MatchContext::deallocateMatchContext(
    RegEx_MatchContextData *matchContextData) const {
  BSLS_ASSERT(matchContextData);
...
int RegEx_MatchContext::initialize(pcre2_general_context *pcre2Context,
                                   pcre2_code *patternCode, int depthLimit,
                                   size_t jitStackSize) {
...
void RegEx_MatchContext::setDepthLimit(int depthLimit) {
  ...
  if (d_mainThreadMatchData.d_matchContext_p) {
    pcre2_set_match_limit(d_mainThreadMatchData.d_matchContext_p, d_depthLimit);
  }
}

// ACCESSORS
int RegEx_MatchContext::acquireMatchContext(
    RegEx_MatchContextData *matchContextData) const {
  BSLS_ASSERT(matchContextData);
...
int RegEx::prepareImp(char *errorMessage, size_t errorMessageLength,
                      size_t *errorOffset, const char *pattern, int flags,
                      size_t jitStackSize) {
  BSLS_ASSERT(errorMessage);
  BSLS_ASSERT(errorOffset);
  BSLS_ASSERT(pattern);

  const int VALID_FLAGS = k_FLAG_CASELESS | k_FLAG_DOTMATCHESALL |
                          k_FLAG_MULTILINE | k_FLAG_UTF8 | k_FLAG_JIT |
                          k_FLAG_DUPNAMES;
  (void)VALID_FLAGS;
  ...
  d_jitStackSize = (flags & k_FLAG_JIT && isJitAvailable()) ? jitStackSize : 0;
  ...
  pcre2_code *patternCode =
      pcre2_compile(reinterpret_cast<const unsigned char *>(pattern),
                    PCRE2_ZERO_TERMINATED, pcreFlags, &errorCodeFromPcre2,
                    &errorOffsetFromPcre2, d_compileContext_p);

  if (0 == patternCode) {
    int result = pcre2_get_error_message(
        errorCodeFromPcre2, reinterpret_cast<unsigned char *>(errorMessage),
        errorMessageLength);
  ...
  if (flags & k_FLAG_JIT && isJitAvailable()) {
    if (0 != pcre2_jit_compile(patternCode, PCRE2_JIT_COMPLETE)) {
      ...
      bsl::strncpy(errorMessage, "JIT compilation failed.", errorMessageLength);
  ...
  if (RegEx_ImpUtil::k_SUCCESS !=
      d_matchContext->initialize(d_pcre2Context_p, patternCode, d_depthLimit,
                                 d_jitStackSize)) {
    pcre2_code_free(patternCode);
    bsl::strncpy(errorMessage, "Unable to create match contexts.",
                 errorMessageLength);
...
int RegEx::matchImp(const RESULT_EXTRACTOR &extractor, const char *subject,
                    size_t subjectLength, size_t subjectOffset,
  ...
  if (skipUTF8Validation || !(d_flags & k_FLAG_UTF8)) {
    if (d_flags & k_FLAG_JIT && isJitAvailable()) {
      rcPcre2 = pcre2_jit_match(
          d_patternCode_p, actualSubject, subjectLength, subjectOffset, 0,
          matchContextData.d_matchData_p, matchContextData.d_matchContext_p);
    } else {
      rcPcre2 = pcre2_match(d_patternCode_p, actualSubject, subjectLength,
                            subjectOffset, PCRE2_NO_UTF_CHECK,
                            matchContextData.d_matchData_p,
                            matchContextData.d_matchContext_p);
    }
  } else {
    rcPcre2 = pcre2_match(d_patternCode_p, actualSubject, subjectLength,
                          subjectOffset, 0, matchContextData.d_matchData_p,
                          matchContextData.d_matchContext_p);
  }
...
inline void RegEx::namedSubpatternsImp(Vector *result) const {
  BSLS_ASSERT(isPrepared());

  uint32_t nameCount;
  int rc =
      pcre2_pattern_info(d_patternCode_p, PCRE2_INFO_NAMECOUNT, &nameCount);
  ...
  rc = pcre2_pattern_info(d_patternCode_p, PCRE2_INFO_NAMEENTRYSIZE,
                          &nameEntrySize);
  BSLS_ASSERT(rc == 0);

  PCRE2_SPTR nameTable;
  rc = pcre2_pattern_info(d_patternCode_p, PCRE2_INFO_NAMETABLE, &nameTable);
  BSLS_ASSERT(rc == 0);
  (void)rc;
...
#endif
    result->push_back(
        typename Vector::value_type(name, static_cast<int>(groupNum)));
  }
}

template <class STRING>
int RegEx::replaceImp(STRING *result, int *errorOffset,
                      const bsl::string_view &subject,
                      const bsl::string_view &replacement, size_t options,
                      bool skipUTF8Validation) const {
  BSLS_ASSERT(result);
  BSLS_ASSERT(errorOffset);
  BSLS_ASSERT(isPrepared());
  ...
  pcreFlags |=
      options & k_REPLACE_UNKNOWN_UNSET ? PCRE2_SUBSTITUTE_UNKNOWN_UNSET : 0;
  pcreFlags |=
      options & k_REPLACE_UNSET_EMPTY ? PCRE2_SUBSTITUTE_UNSET_EMPTY : 0;
  ...
  do {
    ...
    rcPcre2 = pcre2_substitute(
        ...
        subject.length(), 0, pcreFlags, matchContextData.d_matchData_p,
        matchContextData.d_matchContext_p,
        reinterpret_cast<const unsigned char *>(replacement.data()),
        replacement.length(), reinterpret_cast<unsigned char *>(&(*result)[0]),
        &bufferLength);
...
RegEx::RegEx(bslma::Allocator *basicAllocator)
    : d_flags(0), d_pattern(basicAllocator), d_pcre2Context_p(0),
      d_compileContext_p(0), d_patternCode_p(0),
      d_depthLimit(RegEx::defaultDepthLimit()), d_jitStackSize(0),
      d_allocator_p(bslma::Default::allocator(basicAllocator)) {
  d_matchContext.load(new (*d_allocator_p) RegEx_MatchContext(), d_allocator_p);

  d_pcre2Context_p = pcre2_general_context_create(
      &bdlpcre_malloc, &bdlpcre_free, static_cast<void *>(d_allocator_p));
...
int RegEx::prepare(bsl::nullptr_t errorMessage, size_t *errorOffset,
                   const char *pattern, int flags, size_t jitStackSize) {
  (void)errorMessage;
...
// ACCESSORS
int RegEx::match(const bsl::string_view &subject, size_t subjectOffset) const {
  return match(subject.data(), subject.length(), subjectOffset);
}

int RegEx::match(const char *subject, size_t subjectLength,
                 size_t subjectOffset) const {
  return matchImp(NOP(), subject, subjectLength, subjectOffset, false);
}

int RegEx::match(bsl::pair<size_t, size_t> *result, const char *subject,
                 size_t subjectLength, size_t subjectOffset) const {
  BSLS_ASSERT(result);

  return matchImp(DataExtractor<bsl::pair<size_t, size_t>>(result), subject,
                  subjectLength, subjectOffset, false);
}

int RegEx::match(bsl::string_view *result, const char *subject,
                 size_t subjectLength, size_t subjectOffset) const {
  BSLS_ASSERT(result);

  return matchImp(DataExtractor<bsl::string_view>(result), subject,
                  subjectLength, subjectOffset, false);
}

int RegEx::match(bsl::string_view *result, const bsl::string_view &subject,
                 size_t subjectOffset) const {
  BSLS_ASSERT(result);
...
int RegEx::match(bsl::vector<bsl::pair<size_t, size_t>> *result,
                 const char *subject, size_t subjectLength,
                 size_t subjectOffset) const {
  BSLS_ASSERT(result);

  typedef VectorExtractor<bsl::vector<bsl::pair<size_t, size_t>>> Extractor;

  return matchImp(Extractor(result), subject, subjectLength, subjectOffset,
                  false);
}

int RegEx::match(bsl::vector<bslstl::StringRef> *result, const char *subject,
                 size_t subjectLength, size_t subjectOffset) const {
  BSLS_ASSERT(result);
...
int RegEx::match(bsl::vector<bsl::string_view> *result,
                 const bsl::string_view &subject, size_t subjectOffset) const {
  BSLS_ASSERT(result);

  typedef VectorExtractor<bsl::vector<bsl::string_view>> Extractor;

  return matchImp(Extractor(result), subject.data(), subject.length(),
                  subjectOffset, false);
}
...
int RegEx::match(std::experimental::pmr::vector<bsl::string_view> *result,
  ...
  typedef VectorExtractor<std::experimental::pmr::vector<bsl::string_view>>
      Extractor;
...
int RegEx::matchRaw(const char *subject, size_t subjectLength,
                    size_t subjectOffset) const {
...
int RegEx::matchRaw(bsl::pair<size_t, size_t> *result, const char *subject,
                    size_t subjectLength, size_t subjectOffset) const {
  BSLS_ASSERT(result);

  return matchImp(DataExtractor<bsl::pair<size_t, size_t>>(result), subject,
                  subjectLength, subjectOffset, true);
}

int RegEx::matchRaw(bsl::string_view *result, const char *subject,
                    size_t subjectLength, size_t subjectOffset) const {
  BSLS_ASSERT(result);
...
int RegEx::matchRaw(bsl::string_view *result, const bsl::string_view &subject,
                    size_t subjectOffset) const {
  BSLS_ASSERT(result);
...
int RegEx::matchRaw(bsl::vector<bsl::pair<size_t, size_t>> *result,
                    const char *subject, size_t subjectLength,
                    size_t subjectOffset) const {
  BSLS_ASSERT(result);
...
int RegEx::matchRaw(bsl::vector<bslstl::StringRef> *result, const char *subject,
                    size_t subjectLength, size_t subjectOffset) const {
  BSLS_ASSERT(result);
...
int RegEx::replace(bsl::string *result, int *errorOffset,
                   const bsl::string_view &subject,
                   const bsl::string_view &replacement, size_t options) const {
  return replaceImp(result, errorOffset, subject, replacement, options, false);
}

int RegEx::replace(std::string *result, int *errorOffset,
                   const bsl::string_view &subject,
                   const bsl::string_view &replacement, size_t options) const {
  return replaceImp(result, errorOffset, subject, replacement, options, false);
}
...
int RegEx::replace(std::experimental::pmr::string *result, int *errorOffset,
                   const bsl::string_view &subject,
                   const bsl::string_view &replacement, size_t options) const {
  return replaceImp(result, errorOffset, subject, replacement, options, false);
}
...
int RegEx::replaceRaw(bsl::string *result, int *errorOffset,
                      const bsl::string_view &subject,
                      const bsl::string_view &replacement,
                      size_t options) const {
  return replaceImp(result, errorOffset, subject, replacement, options, true);
}

int RegEx::replaceRaw(std::string *result, int *errorOffset,
                      const bsl::string_view &subject,
                      const bsl::string_view &replacement,
                      size_t options) const {
  return replaceImp(result, errorOffset, subject, replacement, options, true);
...
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR_STRING
int RegEx::replaceRaw(std::experimental::pmr::string *result, int *errorOffset,
                      const bsl::string_view &subject,
                      const bsl::string_view &replacement,
                      size_t options) const {
  return replaceImp(result, errorOffset, subject, replacement, options, true);
...
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
void RegEx::namedSubpatterns(
    std::experimental::pmr::vector<std::pair<bsl::string_view, int>> *result)
    const {
  namedSubpatternsImp(result);
}
#endif
...
int RegEx::numSubpatterns() const {
  ...
  int numSubpatterns;
  int returnValue = pcre2_pattern_info(d_patternCode_p, PCRE2_INFO_CAPTURECOUNT,
                                       &numSubpatterns);
  (void)returnValue;
...
int RegEx::subpatternIndex(const char *name) const {
  ...
  const int index = pcre2_substring_number_from_name(
      d_patternCode_p, reinterpret_cast<const unsigned char *>(name));
