...
public:
  ...
  CountedDelete(const CountedDelete &original)
      : d_isTemp(false)
  // Copy c'tor
  {
    (void)original;
...
TestDriver<KEY_TYPE, DATA_TYPE>::hexGg(Obj *dst, const char *spec, int level) {
...
int TestDriver<KEY_TYPE, DATA_TYPE>::raiseMaximumLevel(Obj *dst) {
  ...
  do {
    ...
    dst->addAtLevelRaw(&pr, k_ADD_BY_MODE_MAX_LEVEL, kto.object(),
                       dto.object());
    ++ret;
...
template <class KEY_TYPE, class DATA_TYPE>
void TestDriver<KEY_TYPE,
                DATA_TYPE>::manyThreadsRandomModesOnOneSkipListMain() {
  // ------------------------------------------------------------------------
  ...
  if (verbose)
    COUT << "manyThreadsRandomModesOnOneSkipListMain<"
  ...
  tg.addThreads(bdlf::BindUtil::bind(&manyThreadsRandomModesOnOneSkipListThread,
                                     &mX, numNodesForKeyVal + 0, &barrier,
                                     &doneFlag),
                numThreads);
...
{
  if (verbose)
    cout << "valueSemanticTest<" << bsls::NameOf<KEY_TYPE>() << ", "
         << bsls::NameOf<DATA_TYPE>() << ">();" << endl;
  ...
  for (int ti = 0; ti < k_NUM_I_ITERATIONS; ++ti) {
    ...
    const int ILEVEL =
        TC::e_LEVELS_RANDOM == LMODE ? -1 : (randGen() % 16) & ~1;

    ASSERT(LMODE < TC::e_LEVELS_END);
    ASSERT((TC::e_LEVELS_RANDOM == LMODE) == (ILEVEL < 0));

    Obj mX(&ta);
    const Obj &X = TD::hexGg(&mX, ISPEC, ILEVEL);
    ASSERT(&mX == &X);
    ...
    for (int tj = 0; tj < u::k_NUM_DATA; ++tj) {
      ...
      const int JLEVEL = TC::e_LEVELS_RANDOM == LMODE ? -1
      ...
      ASSERT(TC::e_LEVELS_MATCH != LMODE || (ILEVEL == JLEVEL && 0 <= ILEVEL));
...
{
  if (verbose)
    cout << "skipTest<" << bsls::NameOf<KEY_TYPE>() << ", "
         << bsls::NameOf<DATA_TYPE>() << ">();" << endl;
...
{
  if (verbose)
    cout << "updateRTest<" << bsls::NameOf<KEY_TYPE>() << ", "
         << bsls::NameOf<DATA_TYPE>() << ">();" << endl;
...
// ---------------------------------------------------------------------------
{
...

  {
    ...
    static TableRecord check2[] = {
        {L_, 3, 4, 0}, {L_, 4, 2, 0}, {L_, 4, 5, 0}, {L_, 5, 6, 0}};
...
{
  if (verbose)
    cout << "updateTest<" << bsls::NameOf<KEY_TYPE>() << ", "
         << bsls::NameOf<DATA_TYPE>() << ">();\n";

  typedef int (Obj::*Updater)(const Pair *, const KEY_TYPE &, bool *, bool);
...

  {
    ...
    for (int i = 0; i < 2; ++i) {
      ...
      for (int j = 0; j < (int)(sizeof(parameters) / sizeof(Parameters)); ++j) {
      ...
      ASSERT(Obj::e_NOT_FOUND == (mX2.*updater)(items[0], KVS[25], 0, false));
      ASSERT(Obj::e_NOT_FOUND == (mX2.*updater)(items[0], KVS[25], 0, true));
      ASSERT(0 == mX2.remove(items[3]));
      ASSERT(Obj::e_NOT_FOUND == (mX2.*updater)(items[3], KVS[25], 0, false));
      ASSERT(Obj::e_NOT_FOUND == (mX2.*updater)(items[3], KVS[25], 0, true));
...

  {
    ...
    for (int ii = k_FORWARD_BEGIN; ii < k_FORWARD_END; ++ii) {
      PairHandle ph;
      ASSERTV(ii, TTF::getIdentifier(KVS[ii]), 0 == X.find(&ph, KVS[ii]));
      ASSERT(ph);
      if (!ph) {
        TD::dump(X);
      }
    ...
    init.erase(init.begin() + k_FORWARD_BEGIN, init.begin() + k_FORWARD_END);

    bsl::reverse(moved.begin(), moved.end());
    init.insert(init.begin() + k_FORWARD_DEST_OFFSET, moved.begin(),
                moved.end());
    ...
    for (int ii = k_BACKWARD_BEGIN; ii < k_BACKWARD_END; ++ii) {
      PairHandle ph;
      ASSERTV(ii, TTF::getIdentifier(KVS[ii]), 0 == X.findR(&ph, KVS[ii]));
      bool newFrontFlag;
    ...
    init.erase(init.begin() + k_BACKWARD_BEGIN, init.begin() + k_BACKWARD_END);

    init.insert(init.begin() + k_BACKWARD_DEST + 1, moved.begin(), moved.end());
    ...
    for (int ii = k_BACKWARD_BEGIN; ii < k_BACKWARD_END; ++ii) {
      ...
      ASSERT(0 == X.skipForward(&ph));
      ASSERTV(TTF::getIdentifier(ph.key()), TTF::getIdentifier(ph.data()), !ph);
...
// ------------------------------------------------------------------------
{
  ...
  if (verbose)
    cout << "newTop/newFront<" << bsls::NameOf<KEY_TYPE>() << ", "
         << bsls::NameOf<DATA_TYPE>() << ">();\n";
...
void removeAllSafetyFunc(Obj *list, int numIterations, u::AddMode mode) {
  int id = ++masterId;
...
public:
  ...
  explicit SimpleScheduler(bslma::Allocator *basicAllocator = 0)
      : d_list(basicAllocator), d_startBarrier(2), d_doneFlag(false)
  // Creator.
  {
...
void run() {
  ...
  IDATA VALUES1[] = {
      {L_, 1, 1, 0}, {L_, 3, 3, 1}, {L_, 0, 0, 2}, {L_, 2, 2, 3}, {L_, 4, 4, 4},
  };

  IDATA VALUES2[] = {
      {L_, 0, 0, 2}, {L_, 1, 1, 0}, {L_, 2, 2, 3}, {L_, 3, 3, 1}, {L_, 4, 4, 4},
  };
  ...
  for (int i = 0; i < k_NUM_THREADS; ++i) {
    bslmt::ThreadUtil::create(
        &threads[i],
        bdlf::BindUtil::bind(&threadFunc, &list, (int)k_NUM_ITERATIONS, i + 1));
  }
...
{
  if (verbose)
    cout << "Data race update vs removeAll\n";
...
{
  if (verbose)
    cout << "Data race removeNode vs removeAll\n";

  Obj mX;

  bslmt::Barrier barrier(k_NUM_REMOVE_NODE_THREADS + k_NUM_REMOVE_ALL_THREADS);
...
inline int ThrashFunctor::cmpKeyAndData(const Pair *h, int key, bool flatten) {
  int readKey = h->key();
  if (-1 != key) {
...
void ThrashFunctor::findChangeMinorHandle(int key) {
  ...
  for (int ii = 0; ii < k_LIMIT; ++ii) {
    ...
    case e_UPDATE: {
      rc = d_list.update(ph, key ^ k_VALUES_UPDATE_TOGGLE, &newFront,
                         allowDuplicates);
    } break;
    case e_UPDATER: {
      rc = d_list.updateR(ph, key ^ k_VALUES_UPDATE_TOGGLE, &newFront,
                          allowDuplicates);
    } break;
    default: {
      BSLS_ASSERT_OPT(0);
...
void ThrashFunctor::findChangeMinorRaw(int key) {
  ...
  for (int ii = 0; ii < k_LIMIT; ++ii) {
    ...
    case e_UPDATE: {
      rc = d_list.update(h, key ^ k_VALUES_UPDATE_TOGGLE, &newFront,
                         allowDuplicates);
    } break;
    case e_UPDATER: {
      rc = d_list.updateR(h, key ^ k_VALUES_UPDATE_TOGGLE, &newFront,
                          allowDuplicates);
    } break;
    default: {
      BSLS_ASSERT_OPT(0);
...
void threadFunc(TimeQ *timeQueue, int numIterations, int sendCount,
                int receiveCount, int delay)
...
// run
{
  ...
  for (int i = 0; i < k_NUM_THREADS; ++i) {
    bslmt::ThreadUtil::create(
        &threads[i], bdlf::BindUtil::bind(
                         &threadFunc, &timeQueue, (int)k_NUM_ITERATIONS,
                         (int)k_SEND_COUNT, (int)k_RCV_COUNT, (int)k_DELAY));
  }
...
int main(int argc, char *argv[]) {
  ...
  case 31: {
    ...
    const unsigned sleepTime =
        static_cast<unsigned>(((double)secondsToRun / periods) * 1000 * 1000);
  ...
  case 30: {
    ...
    if (verbose) {
      const double userPlusSysTime =
          sw.accumulatedUserTime() + sw.accumulatedSystemTime();
      const double nsUSPerIteration =
          1e9 * userPlusSysTime / TC::totalIterations;

      const double nsWallPerIterationPerThread =
          1e9 * sw.accumulatedWallTime() * numThreads / TC::totalIterations;
      P_(TC::totalIterations);
      P(TC::maxLevel);
      P_(userPlusSysTime);
      P(sw.accumulatedWallTime());
      P_(nsUSPerIteration);
      P(nsWallPerIterationPerThread);
    }
  } break;
  case 29: {
    // --------------------------------------------------------------------
  ...
  case 27: {
    ...
    U_RUN_EACH_TYPE(TestDriver, manyThreadsRandomModesOnOneSkipListMain,
    ...
    TestDriver<int, TTF::MethodPtr>::manyThreadsRandomModesOnOneSkipListMain();
    TestDriver<int,
               bsltf::AllocTestType>::manyThreadsRandomModesOnOneSkipListMain();
    TestDriver<int, bsltf::MovableAllocTestType>::
        manyThreadsRandomModesOnOneSkipListMain();
    TestDriver<
        int, bsltf::SimpleTestType>::manyThreadsRandomModesOnOneSkipListMain();
  } break;
  case 26: {
    // --------------------------------------------------------------------
    ...
    if (verbose)
      cout << "THOROUGH SINGLE-THREADED ADD TEST\n"
              "=================================\n";
    ...
    U_RUN_EACH_TYPE(TestDriver, testIndependentAddFunctions,
                    U_TEST_TYPES_REGULAR);
    ...
    TestDriver<int, bsltf::MovableAllocTestType>::testIndependentAddFunctions();
    ...
    U_RUN_EACH_TYPE(TestDriver, testMultiAddFunctions, U_TEST_TYPES_REGULAR);
  ...
  case 25: {
    ...
    if (verbose)
      cout << "REPRODUCE BUG / VERIFY FIX OF DRQS 145745492\n"
              "============================================\n";
  ...
  case 24: {
    ...
    if (verbose)
      cout << "REPRODUCE BUG FROM DRQS 144652915\n";
    ...
    for (int ii = 0; ii < numThreads; ++ii) {
      ...
      int rc = bslmt::ThreadUtil::create(
          &handle, attr, bdlf::BindUtil::bind(&addNodes, &barrier));
      ASSERT(0 == rc);
  ...
  case 23: {
    ...
    if (verbose)
      cout << "TESTING 'allocator' ACCESSOR\n"
              "============================\n";
  ...
  case 19: {
    ...
    for (int i = 0; i < 32; i++, exp /= 4) {
      int miss = static_cast<int>(100 * u::myAbs(n[i] - exp) / exp + 0.5);
      miss *= n[i] > exp ? 1 : -1;
      ...
      if (verbose) {
        ...
        cout.precision(4);
        cout.setf(bsl::ios_base::scientific, bsl::ios_base::floatfield);
      ...
      ASSERTV(exp, n[i], tolPercent, n[i] <= bsl::ceil(exp * (1.0 + tol)));
      ASSERTV(exp, n[i], tolPercent, n[i] >= bsl::floor(exp * (1.0 - tol)));
  ...
  case 18: {
    ...
    U_RUN_EACH_TYPE(TestDriver, valueSemanticTest, U_TEST_TYPES_REGULAR);
  ...
  case 17: {
    ...
    U_RUN_EACH_TYPE(TestDriver, skipTest, U_TEST_TYPES_REGULAR);
  ...
  case 16: {
    ...
    tg.addThreads(
        bdlf::BindUtil::bind(&TC::referenceTestFunc, &mX, maxRefCount),
        numThreads);
    tg.joinAll();
  ...
  case 15: {
    ...
    enum { k_NUM_THREADS = 20, k_NUM_ITERATIONS = 1000 };
    ...
    for (int mi = u::e_BEGIN; mi < u::e_END2; ++mi) {
      ...
      TC::collisions = 0;
      TC::removed = 0;
      tg.addThreads(bdlf::BindUtil::bind(&TC::removeAllSafetyFunc, &mX,
                                         numIterations, mode),
                    numThreads);
      tg.joinAll();
      ASSERT(mX.isEmpty());
      ASSERT(TC::masterId == numThreads);
      ASSERTV(mode, TC::removed, TC::collisions, TC::removed + TC::collisions,
  ...
  case 14: {
    ...
    if (verbose)
      cout << endl << "Remove test" << endl << "===========" << endl;
  ...
  case 12: {
    ...
    U_RUN_EACH_TYPE(TestDriver, updateRTest, U_TEST_TYPES_REGULAR);
  ...
  case 11: {
    ...
    U_RUN_EACH_TYPE(TestDriver, addRTest, U_TEST_TYPES_REGULAR);
  ...
  case 10: {
    ...
    U_RUN_EACH_TYPE(TestDriver, removeAllTest, U_TEST_TYPES_REGULAR);
  ...
  case 9: {
    ...
    U_RUN_EACH_TYPE(TestDriver, findRTest, U_TEST_TYPES_REGULAR);
  ...
  case 8: {
    ...
    U_RUN_EACH_TYPE(TestDriver, findTest, U_TEST_TYPES_REGULAR);
  ...
  case 6: {
    ...
    U_RUN_EACH_TYPE(TestDriver, iterationTest, U_TEST_TYPES_REGULAR);
  ...
  case 5: {
    ...
    if (verbose)
      cout << endl << "update test" << endl << "=============" << endl;

    U_RUN_EACH_TYPE(TestDriver, updateTest, U_TEST_TYPES_REGULAR);
  ...
  case 4: {
    ...
    U_RUN_EACH_TYPE(TestDriver, newFrontTest, U_TEST_TYPES_REGULAR);
  ...
  case 3: {
    ...
    U_RUN_EACH_TYPE(TestDriver, allocationTest, U_TEST_TYPES_REGULAR);
  ...
  case 2: {
    ...
    {
      //..
      // Now, in 'main', verify that the scheduler executes events when
      // expected:
      //..
      ...
      //..
      // Add events out of sequence and ensure they are executed in the proper
      // order.
      //..
      if (veryVerbose)
      ...
      scheduler.scheduleEvent(
          bdlf::BindUtil::bind(&pushBackWrapper, &values, 2), scheduleTime);
      ...
      if (veryVerbose)
        cout << "scheduling: " << scheduleTime << endl;
      scheduler.scheduleEvent(
          bdlf::BindUtil::bind(&pushBackWrapper, &values, 0), scheduleTime);
      ...
      if (veryVerbose)
        cout << "scheduling: " << scheduleTime << endl;
      scheduler.scheduleEvent(
          bdlf::BindUtil::bind(&pushBackWrapper, &values, 1), scheduleTime);
      ...
      const double elapsed =
          bdlt::IntervalConversionUtil::convertToTimeInterval(finish - start)
...
#error "unrecognized platform"
    ...
    for (int runIdx = 0; runIdx < k_NUM_RUNS; ++runIdx) {
      ...
      systemTimes.push_back(1e9 * sw.accumulatedSystemTime() / iterations);
      wallTimes.push_back(1e9 * sw.accumulatedWallTime() * numThreads /
                          iterations);
    }
