...
#define TEST_TYPES_USER_DEFINED                                                \
  bsltf::EnumeratedTestType::Enum, bsltf::UnionTestType,                       \
      ...
      bsltf::BitwiseCopyableTestType, bsltf::BitwiseMoveableTestType,          \
...
#define TEST_TYPES_REGULAR                                                     \
  BSLTF_TEMPLATETESTFACILITY_TEST_TYPES_PRIMITIVE, TEST_TYPES_USER_DEFINED
...
typedef struct ThreadData {
  ...
  ThreadData(Strip_TestUtilType *stripTestUtil_p, int sleep, char lockType,
             int key)
      // Create a 'ThreadData' object with the specified 'stripTestUtil_p',
      // 'sleep', 'lockType', and 'key'.
      : d_stripTestUtil_p(stripTestUtil_p), d_sleep(sleep),
        d_lockType(lockType), d_key(key) {}
} ThreadData;
...
// 'bdlcc::StripedUnorderedContainerImpl_TestUtil'.
{
  ...
  if (verbose)
    cout << endl << "TEST HELPER CLASS" << endl << "-----------------" << endl;
...
struct TestUpdater {
  ...
  // 'value' and 'key'.  Always returns 'true'.
  {
    ...
    if (veryVerbose) {
      bsl::cout << "Updating " << key << ", value: " << *value << bsl::endl;
    }
...
struct TestReader {
  ...
  // the specified 'value' and 'key'.  Always returns 'true'.
  {
    // Print out key and value
    if (veryVerbose) {
      bsl::cout << "Reading " << key << ", value: " << value << bsl::endl;
    }
...
struct TestComputedValueUpdater {
  ...
  // specified 'value' and 'key'.
  {
    // Print out key and value
    if (veryVerbose) {
      bsl::cout << "Set computed value " << key << ", value: " << *value
                << bsl::endl;
    }
    return true;
  }
...
// methods.
{
  ...
  if (veryVerbose)
    cout << "LockRead / 'insertUnique(const KEY& key, const VALUE& value)'"
         << endl;
  {
  ...
  if (veryVerbose)
    cout << "LockRead / 'insertUnique(const KEY& key, VALUE&& value)'" << endl;
  {
    // Time the duration how long it took to run 'insert'.
    TimeType startTime = bsls::TimeUtil::getTimer();
    bslmt::ThreadUtil::create(&handle, workThread, &tdRead);
  ...
  if (veryVerbose)
    cout << "LockRead  / 'setValueFirst(const KEY& key, const VALUE& value)'"
         << endl;
  {
  ...
  if (veryVerbose)
    cout << "LockRead  / 'setComputedValueFirst(const KEY& key, visitor)'"
         << endl;
  {
    // Time the duration how long it took to run 'setValue'
    TimeType startTime = bsls::TimeUtil::getTimer();
  ...
  if (veryVerbose)
    cout << "LockRead  / 'update(const KEY& key, visitor)'" << endl;
  {
  ...
  if (veryVerbose)
    cout << "LockRead  / 'visit(const VisitorFunction&)'" << endl;
  {
  ...
  if (veryVerbose)
    cout << "LockRead  / 'rehash(numBuckets)'" << endl;
  {
    // Time the duration how long it took to run 'setValue'
    TimeType startTime = bsls::TimeUtil::getTimer();
    bslmt::ThreadUtil::create(&handle, workThread, &tdRead);
  ...
  if (veryVerbose)
    cout << "LockWrite / 'getValue' vector" << endl;
  {
    // Time the duration how long it took to run 'getValue'
    TimeType startTime = bsls::TimeUtil::getTimer();
    bslmt::ThreadUtil::create(&handle, workThread, &tdWrite);
  ...
  if (veryVerbose)
    cout << "LockRead  / 'getValue' vector" << endl;
  {
    bslmt::ThreadUtil::create(&handle, workThread, &tdRead);
    smp.wait();
    // Time the duration how long it took to run 'getValue'
  ...
  if (veryVerbose)
    cout << "LockWrite / 'visitReadOnly(visitor)'" << endl;
  {
    // Time the duration how long it took to run 'visit'
    TimeType startTime = bsls::TimeUtil::getTimer();
    bslmt::ThreadUtil::create(&handle, workThread, &tdWrite);
  ...
  if (veryVerbose)
    cout << "LockRead  / 'visitReadOnly(visitor)'" << endl;
  {
    bslmt::ThreadUtil::create(&handle, workThread, &tdRead);
    smp.wait();
    // Time the duration how long it took to run 'visit'
  ...
  if (veryVerbose)
    cout << "LockWrite / 'visitReadOnly(key, visitor)'" << endl;
  {
    // Time the duration how long it took to run 'visit'
    TimeType startTime = bsls::TimeUtil::getTimer();
    bslmt::ThreadUtil::create(&handle, workThread, &tdWrite);
  ...
  if (veryVerbose)
    cout << "LockRead  / 'visitReadOnly(key, visitor)'" << endl;
  {
    bslmt::ThreadUtil::create(&handle, workThread, &tdRead);
    smp.wait();
    // Time the duration how long it took to run 'visit'
  ...
  while (0 == *arg->d_stop_p) {
    ...
    if (veryVeryVerbose) {
      ...
      ss << "AfterGetValue, rc=" << rc << "," << arg->d_workerId << "/"
         << arg->d_numWorkers << ": " << key << ", bucketCount=" << bucketCount
         << "\n";
      bsl::cout << ss.str();
    }
...
// Multi threaded stress test.
{
  ...
  for (int i = 0; i < k_NUM_WORKERS; ++i) {
    ThreadArg arg = {&strip, &stop, writeCounts, k_NUM_ITEMS, i, k_NUM_WORKERS};
    args[i] = arg;

    bslmt::ThreadUtil::create(&queryHandles[i], workerThread, &args[i]);
  }
  ...
  ASSERTV(initialNumBuckets, finalNumBuckets,
...
template <class KEY, class VALUE, class ALLOC> struct IntToPairConverter {
  // Convert an 'int' value to a 'bsl::pair' of the template parameter 'KEY'
  // and 'VALUE' types.

  // CLASS METHODS
  static void createInplace(bsl::pair<KEY, VALUE> *address, int value,
                            ALLOC allocator)
  ...
  // is undefined unless '0 < value < 128'.
  {
    ...
    bsltf::TemplateTestFacility::emplace(tempKey.address(), value,
                                         privateAllocator);

    bsls::ObjectBuffer<VALUE> tempValue;
    bsltf::TemplateTestFacility::emplace(tempValue.address(), value - 'A' + '0',
                                         privateAllocator);

    bsl::allocator_traits<ALLOC>::construct(
        allocator, address, bslmf::MovableRefUtil::move(tempKey.object()),
        bslmf::MovableRefUtil::move(tempValue.object()));
  }
};

template <class TYPE> class TestHashFunctor {
...
template <class TYPE> class TestEqualityComparator {
  // This test class provides a mechanism that defines a function-call
  // operator that compares two objects of the parameterized 'TYPE'.  The
  // function-call operator is implemented with integer comparison using
  // integers converted from objects of 'TYPE' by the class method
...
template <class KEY, class VALUE = KEY, class HASH = TestHashFunctor<KEY>,
...
private:
  ...
  typedef bsltf::TestValuesArray<
      PairType, PairStdAllocator,
      IntToPairConverter<KEY, VALUE, PairStdAllocator>>
      TestValues;
  ...
  struct TestCase19Reader {
    ...
    // CREATORS
    explicit TestCase19Reader(bsl::size_t numSuccess, const TestValues &values);
...
TestDriver<KEY, VALUE, HASH, EQUAL>::TestCase19Reader::TestCase19Reader(
    bsl::size_t numSuccess, const TestValues &values)
    : d_numSuccess(numSuccess), d_values(values) {
...
TestDriver<KEY, VALUE, HASH, EQUAL>::TestCase14Updater::TestCase14Updater(
    int numSuccess, const TestValues &values)
    : d_numSuccess(numSuccess), d_values(values) {
...
void TestDriver<KEY, VALUE, HASH, EQUAL>::testCase19() {
  ...
  if (verbose)
    cout << "/tTesting common read-only visit" << endl;
  ...
  for (bsl::size_t ti = 1; ti < MAX_LENGTH; ++ti) {
    ...
    for (bsl::size_t tj = 0; tj < LENGTH; ++tj) {
      Element element = s_testCase19_visitedElements.find(VALUES[tj].first);
  ...
  if (verbose)
    cout << "/tTesting read-only visit with key" << endl;
    ...
    for (bsl::size_t tj = 0; tj < LENGTH; ++tj) {
      ...
      for (bsl::size_t tk = 0; tk < LENGTH; ++tk) {
        if (INDEX == tk) {
          for (bsl::size_t tl = 0; tl < LENGTH; ++tl) {
            ...
            mX.insertAlways(VALUES[tk].first, VALUES[LENGTH - tl - 1].second);
          }
        } else {
      ...
      for (bsl::size_t tk = 0; tk < LENGTH; ++tk) {
        ...
        if (INDEX == tk) {
          ASSERTV(LENGTH == rc1);
          for (bsl::size_t tl = 0; tl < LENGTH; ++tl) {
            ASSERTV(areEqual(values[tl], VALUES[LENGTH - tl - 1].second));
          }
        } else {
          ASSERTV(1 == rc1);
      ...
      for (bsl::size_t tk = 0; tk < LENGTH; ++tk) {
        bsl::pair<Element, Element> elements =
            s_testCase19_visitedElements.equal_range(VALUES[INDEX].first);
        for (Element element = elements.first; element != elements.second;
             ++element) {
...
void TestDriver<KEY, VALUE, HASH, EQUAL>::testCase18() {
  ...
  // Test empty hash map.
  {
    ...
    // Check missing value
    bsl::size_t rc = mX.insertUnique(VALUES[0].first, MoveUtil::move(val));
  ...
  for (bsl::size_t ti = 1; ti < MAX_LENGTH; ++ti) {
    ...
    bsl::size_t rc =
        mX.insertUnique(VALUES[LENGTH - 1].first, MoveUtil::move(val));
    ASSERTV(rc, 0 == rc);
    ASSERTV(bsltf::MoveState::e_MOVED == val.movedFrom());
    ...
    // Check missing value
    rc = mX.insertUnique(VALUES[LENGTH + 1].first, MoveUtil::move(val1));
    ASSERTV(rc, 1 == rc);
    ...
    BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(supplied) {
      ...
      mX.insertUnique(VALUES[LENGTH + 2].first, MoveUtil::move(val1));
    }
...
void TestDriver<KEY, VALUE, HASH, EQUAL>::testCase17() {
  ...
  // Test empty hash map.
  {
    ...
    // Check missing value.
    bsl::size_t rc = mX.setValueFirst(VALUES[0].first, MoveUtil::move(val));
    ASSERTV(bsltf::MoveState::e_MOVED == val.movedFrom());
    ASSERTV(rc, 0 == rc);
    ASSERTV(1 == X.size());
  ...
  for (bsl::size_t ti = 1; ti < MAX_LENGTH; ++ti) {
    ...
    // Check existing value.
    bsl::size_t rc =
        mX.setValueFirst(VALUES[LENGTH - 1].first, MoveUtil::move(val));
    ASSERTV(rc, 1 == rc);
    ASSERTV(bsltf::MoveState::e_MOVED == val.movedFrom());
...
void TestDriver<KEY, VALUE, HASH, EQUAL>::testCase15() {
  ...
  for (bsl::size_t ti = 0; ti < 6; ++ti) {
    ...
    for (int i = 0; i <= 5; ++i) { // Insert types
      ...
      for (bsl::size_t tj = 0; tj < LENG; ++tj) {
        ...
        switch (i) {
        case 0: { // insert
          bsl::size_t rc = mX.insertUnique(VALUES[tj].first, VALUES[tj].second);
          ASSERTV(1 == rc)
        } break;
        case 1: { // insert move
          bsl::size_t rc = mX.insertUnique(
              VALUES[tj].first, bslmf::MovableRefUtil::move(VALUES[tj].second));
          ASSERTV(1 == rc)
        } break;
        case 2: { // setValue
          bsl::size_t rc =
              mX.setValueFirst(VALUES[tj].first, VALUES[tj].second);
          ASSERTV(0 == rc)
        } break;
        ...
        case 4: { // insertBulk
          ...
          typename Obj::KVType item(VALUES[tj].first, VALUES[tj].second,
                                    &supplied);
          insertVec.push_back(item);
          bsl::size_t rc =
              mX.insertBulkUnique(insertVec.begin(), insertVec.end());
          ASSERTV(1 == rc)
        } break;
        ...
        if (i == 4 || (i == 5 && bslma::UsesBslmaAllocator<VALUE>::value)) {
          dam.reset();
        }
        ...
        if (tj < initialNumBuckets) {
          ...
          if (tj == initialNumBuckets) {
            bsl::size_t lastKeyBucketIdx = X.bucketIndex(VALUES[tj].first);
            for (bsl::size_t j = 0; j < initialNumBuckets; ++j) {

              if (lastKeyBucketIdx != j &&
                  lastKeyBucketIdx != j + initialNumBuckets) {

                ASSERTV(ti, tj, j, bucketSizes[j], X.bucketSize(j),
  ...
  for (bsl::size_t ti = 0; ti < 5; ++ti) {
    ...
    ASSERTV(LENG, X.bucketCount(), initialNumBuckets == X.bucketCount());
...
void TestDriver<KEY, VALUE, HASH, EQUAL>::testCase14() {
  ...
  for (bsl::size_t ti = 1; ti < MAX_LENGTH; ++ti) {
    ...
    TestCase14Updater testCase14UpdaterSuccess(static_cast<int>(LENGTH / 2),
                                               VALUES);
...
void TestDriver<KEY, VALUE, HASH, EQUAL>::testCase13_update() {
  ...
  for (bsl::size_t ti = 1; ti < MAX_LENGTH; ++ti) {
    ...
    rc = mX.update(VALUES[LENGTH - 1].first, testCase13UpdaterFail);
    ASSERTV(rc, -1 == rc);
...
void TestDriver<KEY, VALUE, HASH, EQUAL>::testCase12() {
  ...
  {
    if (veryVerbose)
      cout << endl << "Test 'setComputedValueFirst'" << endl;
    ...
    // Test empty hash map.
    {
      ...
      rc1 = mX.setComputedValueAll(VALUES[1].first, testCase12Updater);
      ASSERTV(rc1, 0 == rc1);
      ASSERTV(2 == X.size());
    ...
    for (bsl::size_t ti = 1; ti < MAX_LENGTH; ++ti) {
      ...
      // Check existing value
      int rc1 =
          mX.setComputedValueFirst(VALUES[LENGTH - 1].first, testCase12Updater);
      ASSERTV(rc1, 1 == rc1);
      ...
      BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(supplied) {
        ...
        mX.setComputedValueFirst(VALUES[LENGTH + 2].first, testCase12Updater);
      }
      BSLMA_TESTALLOCATOR_EXCEPTION_TEST_END;
  ...
  {
    if (veryVerbose)
      cout << endl << "Test 'setComputedValueAll'" << endl;
    ...
    for (bsl::size_t ti = 1; ti < MAX_LENGTH; ++ti) {
      ...
      // Check existing value.
      int rc1 =
          mX.setComputedValueAll(VALUES[LENGTH - 1].first, testCase12Updater);
      ASSERTV(rc1, 1 == rc1);
      ASSERTV(true == s_testCase12_found);
      ASSERTV(VALUES[LENGTH - 1].first, s_testCase12_key,
              areEqual(VALUES[LENGTH - 1].first, s_testCase12_key));
      ...
      // Check missing value.
      rc1 = mX.setComputedValueAll(VALUES[LENGTH + 1].first, testCase12Updater);
      ASSERTV(rc1, 0 == rc1);
      ...
      BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(supplied) {
        ...
        mX.setComputedValueAll(VALUES[LENGTH + 2].first, testCase12Updater);
      }
      BSLMA_TESTALLOCATOR_EXCEPTION_TEST_END;
      s_testCase12_alloc = 0;
...
void TestDriver<KEY, VALUE, HASH, EQUAL>::testCase12_noncopyable() {
  ...
  // 'testCase14' in 'StripedUnorderedMultiMap.
  {
    ...
    // Test empty hash map.
    {
      ...
      // Check missing value for 'setComputedValueFirst'.
      int rc1 = mX.setComputedValueFirst(KEYS[0], testCase12Updater);
      ASSERTV(rc1, 0 == rc1);
      ASSERTV(1 == X.size());
      ...
      // Check missing value for 'setComputedValueAll'.
      rc1 = mX.setComputedValueAll(KEYS[1], testCase12Updater);
      ASSERTV(rc1, 0 == rc1);
      ASSERTV(2 == X.size());
    ...
    for (bsl::size_t ti = 1; ti < MAX_LENGTH; ++ti) {
      ...
      // Check existing value
      int rc1 = mX.setComputedValueFirst(KEYS[LENGTH - 1], testCase12Updater);
      ASSERTV(rc1, 1 == rc1);
      ASSERTV(true == s_testCase12_found);
      ...
      // Check missing value.
      rc1 = mX.setComputedValueFirst(KEYS[LENGTH + 1], testCase12Updater);
      ASSERTV(rc1, 0 == rc1);
      ...
      for (bsl::size_t tj = 0; tj < LENGTH; ++tj) {
        ...
        ASSERTV(1 == rc);
        ASSERTV(
            areEqual(value, tj == LENGTH - 1 ? VALUES[LENGTH] : VALUES[tj]));
      }
      ...
      BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(supplied) {
        ...
        mX.setComputedValueFirst(KEYS[LENGTH + 2], testCase12Updater);
      }
      BSLMA_TESTALLOCATOR_EXCEPTION_TEST_END;
      s_testCase12_alloc = 0;
    } // END Testing 'setComputedValueFirst' with unique values
  ...
  // This is equivalent to 'testCase15' in 'StripedUnorderedMultiMap'.
  {
    ...
    for (bsl::size_t ti = 1; ti < MAX_LENGTH; ++ti) {
      ...
      // Check existing value.
      int rc1 = mX.setComputedValueAll(KEYS[LENGTH - 1], testCase12Updater);
      ASSERTV(rc1, 1 == rc1);
      ASSERTV(true == s_testCase12_found);
      ASSERTV(KEYS[LENGTH - 1], s_testCase12_key,
              areEqual(KEYS[LENGTH - 1], s_testCase12_key));
      ...
      // Check missing value.
      rc1 = mX.setComputedValueAll(KEYS[LENGTH + 1], testCase12Updater);
      ASSERTV(rc1, 0 == rc1);
      ...
      BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(supplied) {
        ...
        mX.setComputedValueAll(KEYS[LENGTH + 2], testCase12Updater);
      }
      BSLMA_TESTALLOCATOR_EXCEPTION_TEST_END;
      s_testCase12_alloc = 0;
    } // END Testing 'e_ALL' with unique values
...
void TestDriver<KEY, VALUE, HASH, EQUAL>::testCase11() {
  ...
  if (verbose)
    cout << endl << "TEST 'setValue'" << endl << "===============" << endl;
  ...
  for (bsl::size_t ti = 1; ti < MAX_LENGTH; ++ti) {
    ...
    // Check existing value.
    bsl::size_t rc =
        mX.setValueAll(VALUES[LENGTH - 1].first, VALUES[LENGTH].second);
    ASSERTV(rc, 1 == rc);
    ...
    rc = mX.setValueAll(VALUES[LENGTH + 1].first, VALUES[LENGTH + 1].second);
    ASSERTV(rc, 0 == rc);
    ...
    // Exception testing - check missing value
    BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(supplied) {
      mX.setValueAll(VALUES[LENGTH + 1].first, VALUES[LENGTH + 1].second);
    }
    BSLMA_TESTALLOCATOR_EXCEPTION_TEST_END;
  }
  ASSERT(dam.isTotalSame());
...
void TestDriver<KEY, VALUE, HASH, EQUAL>::testCase10() {
  ...
  if (verbose)
    cout << endl << "TEST 'insertBulk'" << endl << "=================" << endl;
  ...
  {
    if (veryVerbose)
      cout << endl << "Test 'insertBulkUnique'" << endl;
      ...
      for (bsl::size_t tj = 0; tj < LENGTH; ++tj) {
        ...
        for (bsl::size_t tk = 0; tk <= 3 && tk <= LENGTH; ++tk) {
          ...
          if (tk == 0) {
            bsl::vector<typename Obj::KVType> insertVec(&supplied);
            sam.reset();
            rc = mX.insertBulkUnique(insertVec.cbegin(), insertVec.cbegin());
          ...
          for (bsl::size_t i = 0; i < tk; ++i) {
            typename Obj::KVType item(VALUES[i + tkOffset].first,
          ...
          for (bsl::size_t i = 0; i < tk; ++i) {
            typename Obj::KVType item(VALUES[i + tkOffset + LENGTH].first,
          ...
          rc = mX.insertBulkUnique(insertVec1.cbegin(), insertVec1.cend());
    ...
    for (bsl::size_t ti = 0; ti < MAX_LENGTH; ++ti) {
      ...
      for (bsl::size_t tj = 0; tj < LENGTH; ++tj) {
        ...
        for (bsl::size_t v = 0; v < LENGTH; ++v) {
          mX.insertUnique(VALUES[v].first, VALUES[v].second);
          mX.insertAlways(VALUES[v].first, VALUES[LENGTH - v - 1].second);
        }
        ASSERTV(2 * LENGTH == X.size());
  ...
  {
    if (veryVerbose)
      cout << endl << "Test 'insertBulkAlways'" << endl;
      ...
      for (bsl::size_t tj = 0; tj < LENGTH; ++tj) {
        ...
        for (bsl::size_t tk = 0; tk <= 3 && tk <= LENGTH; ++tk) {
          ...
          if (tk == 0) {
            bsl::vector<typename Obj::KVType> insertVec(&supplied);
            sam.reset();
            mX.insertBulkAlways(insertVec.cbegin(), insertVec.cbegin());
          ...
          mX.insertBulkAlways(insertVec1.cbegin(), insertVec1.cend());
    ...
    for (bsl::size_t ti = 0; ti < MAX_LENGTH; ++ti) {
      ...
      for (bsl::size_t tj = 0; tj < LENGTH; ++tj) {
        ...
        for (bsl::size_t i = tkOffset; i < LENGTH; ++i) {
          ...
          rc = X.getValue(&values, VALUES[i].first);
          ASSERTV(2 == rc);
          ASSERTV(areEqual(values[0], VALUES[i].second) ||
                  areEqual(values[0], VALUES[LENGTH - i - 1].second));
          ASSERTV(areEqual(values[1], VALUES[i].second) ||
                  areEqual(values[1], VALUES[LENGTH - i - 1].second));
        }
      } // END Loop on 'tj'
  ...
  if (verbose)
    cout << "Exception Testing - 'insertBulkUnique'" << endl;
  {
    bslma::TestAllocator supplied("supplied", veryVeryVeryVerbose);
    bslma::TestAllocatorMonitor sam(&supplied);
  ...
  if (verbose)
    cout << "Exception Testing - 'insertBulkAlways'" << endl;
  {
    bslma::TestAllocator supplied("supplied", veryVeryVeryVerbose);
    bslma::TestAllocatorMonitor sam(&supplied);
    ...
    cout << "Negative Testing" << endl;
  {
    ...
    bsl::vector<typename Obj::KVType> insertVec(&scratch);
    typename Obj::KVType item(VALUES[0].first, VALUES[0].second, &scratch);
    insertVec.push_back(item);

    bsls::AssertTestHandlerGuard hG;

    ASSERT_FAIL(mX.insertBulkUnique(insertVec.cend(), insertVec.cbegin()));
    ASSERT_PASS(mX.insertBulkUnique(insertVec.cbegin(), insertVec.cbegin()));
    ASSERT_PASS(mX.insertBulkUnique(insertVec.cbegin(), insertVec.cend()));

    ASSERT_FAIL(mX.insertBulkAlways(insertVec.cend(), insertVec.cbegin()));
    ASSERT_PASS(mX.insertBulkAlways(insertVec.cbegin(), insertVec.cbegin()));
    ASSERT_PASS(mX.insertBulkAlways(insertVec.cbegin(), insertVec.cend()));
  }
}
...
void TestDriver<KEY, VALUE, HASH, EQUAL>::testCase8() {
  ...
  if (verbose)
    cout << endl << "TEST 'eraseBulk'" << endl << "================" << endl;
  ...
  {
    if (veryVerbose)
      cout << endl << "Test 'eraseBulk' First" << endl;
...

    {
      ...
      bsl::size_t count =
          mX.eraseBulkFirst(eraseKeys.cbegin(), eraseKeys.cend());
      ASSERTV(0 == count);
      ASSERTV(0 == X.size());
    }
    ...
    // Verify the tail pointer is maintained correctly.
    {
      ...
      sam.reset();
      {
        ...
        ASSERT(eraseKeys.size() ==
               mX.eraseBulkAll(eraseKeys.cbegin(), eraseKeys.cend()));
      ...
      for (bsl::size_t tj = 0; tj < LENGTH; ++tj) {
        ...
        for (bsl::size_t tk = 0; tk <= 3 && tk <= LENGTH; ++tk) {
          ...
          if (tk == 0) {
            ...
            bsl::size_t count =
                mX.eraseBulkFirst(eraseKeys.cbegin(), eraseKeys.cbegin());
            ASSERTV(0 == count);
            ASSERTV(LENGTH == X.size());
            ASSERTV(sam.isInUseSame());
            continue;
          ...
          for (bsl::size_t v = 0; v < LENGTH; v += tk) {
            ...
            bsl::size_t rc =
                mX.eraseBulkFirst(eraseKeys.cbegin(), eraseKeys.cend());
            bsl::size_t expNumErased = LENGTH - v >= tk ? tk : LENGTH - v;
            bsl::size_t expSize = LENGTH - v >= tk ? LENGTH - v - tk : 0;
            ASSERTV(ti, tj, tk, v, rc, expNumErased, rc == expNumErased);
            ASSERTV(ti, tj, tk, v, rc, expSize, expSize == X.size());

            // Confirm reduction in size for each of the buckets.
            for (bsl::size_t i = 0; i < tk; ++i) {
              bsl::size_t newBucketSize = X.bucketSize(bucketIdxs[i]);
              if (expNumErased == tk) {
            ...
            // Confirm temporary memory usage of default allocator.
            ASSERTV(ti, tj, tk, v, rc, expNumErased, dam.isTotalUp());
          }
          ASSERTV(0 == X.size());
        } // END Loop on 'tk'
      } // END Loop on 'tj'
  ...
  {
    if (veryVerbose)
      cout << endl << "Test 'eraseBulkAll'" << endl;
    ...
    // Try to erase from an empty hash map, 'eraseBulkAll'.
    {
      ...
      bsl::size_t count = mX.eraseBulkAll(eraseKeys.cbegin(), eraseKeys.cend());
      ASSERTV(0 == count);
      ASSERTV(0 == X.size());
    }
      ...
      for (bsl::size_t tj = 0; tj < LENGTH; ++tj) {
        ...
        for (bsl::size_t tk = 0; tk <= 3 && tk <= LENGTH; ++tk) {
          ...
          if (tk == 0) {
            ...
            bsl::size_t count =
                mX.eraseBulkAll(eraseKeys.cbegin(), eraseKeys.cbegin());
            ASSERTV(0 == count);
            ASSERTV(LENGTH == X.size());
            ASSERTV(sam.isInUseSame());
            continue;
      ...
      for (bsl::size_t tj = 0; tj < LENGTH; ++tj) {
        ...
        for (bsl::size_t tk = 0; tk <= 3 && tk <= LENGTH; ++tk) {
          ...
          for (bsl::size_t v = 0; v < LENGTH; v += tk) {
            ...
            bsl::size_t expSize = LENGTH - v >= tk ? 2 * (LENGTH - v - tk) : 0;
    ...
    cout << "Negative Testing" << endl;
  {
    ...
    ASSERT_FAIL(mX.eraseBulkFirst(eraseKeys.cend(), eraseKeys.cbegin()));
    ASSERT_PASS(mX.eraseBulkFirst(eraseKeys.cbegin(), eraseKeys.cbegin()));
    ASSERT_PASS(mX.eraseBulkFirst(eraseKeys.cbegin(), eraseKeys.cend()));
...
void TestDriver<KEY, VALUE, HASH, EQUAL>::testCase23() {
  ...
  if (verbose)
    cout << endl << "TEST 'eraseIf'" << endl << "==============" << endl;
  ...
  typename Obj::EraseIfValuePredicate valueIsOne(bsl::allocator_arg, &ia,
                                                 valueOnePred);
  ...
  {
    if (veryVerbose)
      cout << endl << "Test 'eraseFirstIf'" << endl;
  ...
  {
    if (veryVerbose)
      cout << endl << "Test 'eraseAllIf'" << endl;
...
void TestDriver<KEY, VALUE, HASH, EQUAL>::testCase7() {
  ...
  if (verbose)
    cout << endl << "TEST 'erase'" << endl << "============" << endl;
  ...
  {
    if (veryVerbose)
      cout << endl << "Test 'eraseFirst'" << endl;
  ...
  {
    if (veryVerbose)
      cout << endl << "Test 'eraseFirst' against duplicates" << endl;
...
void TestDriver<KEY, VALUE, HASH, EQUAL>::testCase5() {
  ...
  if (verbose)
    cout << endl << "TEST 'getValue'" << endl << "===============" << endl;
...
void TestDriver<KEY, VALUE, HASH, EQUAL>::testCase3() {
  ...
  for (bsl::size_t ti = 0; ti < NUM_CONFIGS; ++ti) {
      ...
      cout << "Depth 0" << endl;
    {
      ...
      } INSERTS0[] = {// LINE KEY VAL MULT EXP_RC EXP_SIZE EXP_BIDX EXP_BSIZE
                      //---- --- --- ---- ------ -------- -------- ---------
                      {L_, 0, 10, 'U', 1, 1, 0, 1},
                      {L_, 0, 10, 'A', 1, 1, 0, 1},
                      {L_, 1, 11, 'U', 1, 1, 1, 1},
                      {L_, 1, 11, 'A', 1, 1, 1, 1}};
      const bsl::size_t NUM_INSERTS0 = sizeof INSERTS0 / sizeof *INSERTS0;
      ...
      for (bsl::size_t tj = 0; tj < NUM_INSERTS0; ++tj) {
        ...
        const bsl::size_t EXP_BIDX = INSERTS0[tj].d_key % NUM_BUCKETS;
        ...
        BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(sa) {
          ...
          // TEST OBJECT
          Obj mX(NUM_BUCKETS, NUM_STRIPES, &sa);
          const Obj &X = mX;
          ASSERT(true == X.empty());
          ASSERT(0 == X.size());
          ...
          bsls::Types::Int64 numBlocksTotalAfore = sa.numBlocksTotal();
          ...
          bsls::Types::Int64 numBlocksTotalAfter = sa.numBlocksTotal();
          ASSERT(dam1.isTotalSame());
          ASSERT(numBlocksTotalAfter - numBlocksTotalAfore ==
                 EXPECTED_NUM_ALLOCATIONS);
      ...
      cout << "Depth 1" << endl;
    {
      ...
      } INSERTS1[] = {// LINE OBJ  KEY  VAL  MULT EXP_RC EXP_SIZE #V1 #V2
                      //---- ---  ---  ---  ---- ------ -------- --- ---
      ...
      const bsl::size_t NUM_INSERTS1 = sizeof INSERTS1 / sizeof *INSERTS1;
      ...
      for (bsl::size_t tj = 0; tj < NUM_INSERTS1; ++tj) {
        ...
        const bsl::size_t EXP_BIDX = INSERTS1[tj].d_key % NUM_BUCKETS;
        ...
        BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(sa) {
          ...
          for (typename bsl::vector<VALUE>::const_iterator itr = values.begin(),
                                                           end = values.end();
               end != itr; ++itr) {
          ...
          if (veryVeryVerbose) {
            P(LINE)
            P_(VALUE1)
            P_(EXP_V1COUNT) P(V1COUNT) P_(VALUE2) P_(EXP_V2COUNT) P(V2COUNT)
          }
      ...
      cout << "Depth 2" << endl;
    {
      ...
      const struct {
        ...
        KeyValuesArray d_expKeyValuesArray[3];
          ...
          {L_,
           ...
           0,
           2,
           {{K1A, {V1A, V1A, nnn}}, {nnn, {nnn, nnn, nnn}}}},
          {L_,
           K1A,
           V1A,
           K1A,
           ...
           0,
           2,
           {{K1A, {V1A, V2A, nnn}}, {nnn, {nnn, nnn, nnn}}
          ...
          {L_,
           ...
           1,
           3,
           {{K1A, {V1A, V1A, V1A}}, {nnn, {nnn, nnn, nnn}}}},
          {L_,
           K1A,
           V1A,
           K1A,
           ...
           1,
           3,
           {{K1A, {V1A, V1A, V2A}}, {nnn, {nnn, nnn, nnn}}
      ...
      const bsl::size_t NUM_INSERTS2 = sizeof INSERTS2 / sizeof *INSERTS2;
      ...
      for (bsl::size_t tj = 0; tj < NUM_INSERTS2; ++tj) {
        ...
        if (veryVerbose) {
          T_ P(LINE) T_ P_(E1KEY) P(E1VALUE) T_ P_(E2KEY) P(E2VALUE) T_ P_(IKEY)
              P_(IVALUE) P(IMULT)
        }
        ...
        const KeyValuesArray *const EXP_ELEMENTS =
            INSERTS2[tj].d_expKeyValuesArray;
        ...
        BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(sa) {
          ...
          for (bsl::size_t i = 0; i < 3; ++i) {
            ...
            bsl::sort(foundValuesAsInt.begin(), foundValuesAsInt.end());

            // Compare found values to expected values.

            ASSERTV(key, EXP_VALUES.size(), foundValuesAsInt.size(),
...
void TestDriver<KEY, VALUE, HASH, EQUAL>::testCase2() {
  ...
  if (veryVerbose)
    cout << endl << "Configuration Tests" << endl;
  {
    for (char cfg = 'a'; cfg <= 'g'; ++cfg) {
      ...
      BSLMF_ASSERT(Obj::k_DEFAULT_NUM_BUCKETS != NON_DEFAULT_NUM_BUCKETS);

      BSLMF_ASSERT(Obj::k_DEFAULT_NUM_STRIPES != NON_DEFAULT_NUM_STRIPES);
      ...
      case 'a': {
        ...
        objPtr = new (fa) Obj(NON_DEFAULT_NUM_BUCKETS, NON_DEFAULT_NUM_STRIPES);
        objAllocatorPtr = &da;
      ...
      case 'b': {
        if (veryVeryVerbose) {
          cout << "Default allocator explicitly (0) specified" << endl;
        }

        objPtr =
            new (fa) Obj(NON_DEFAULT_NUM_BUCKETS, NON_DEFAULT_NUM_STRIPES, 0);
        objAllocatorPtr = &da;
        exp_num_buckets = NON_DEFAULT_NUM_BUCKETS;
        exp_num_stripes = NON_DEFAULT_NUM_STRIPES;
      } break;
      ...
      case 'c': {
        ...
        objPtr =
            new (fa) Obj(NON_DEFAULT_NUM_BUCKETS, NON_DEFAULT_NUM_STRIPES, &sa);
        objAllocatorPtr = &sa;
        exp_num_buckets = NON_DEFAULT_NUM_BUCKETS;
        exp_num_stripes = NON_DEFAULT_NUM_STRIPES;
      } break;
      ...
      case 'g': {
        if (veryVeryVerbose) {
          cout << "Lower limit: number of buckets and stripes" << endl;
        }
      ...
      bslma::TestAllocator &noa =
          ('c' != CONFIG && 'd' != CONFIG && 'g' != CONFIG) ? sa : da;
      if (veryVeryVerbose) {
        cout << " oa" << ": " << (&oa == &sa ? "supplied" : "default") << endl;
        cout << "noa" << ": " << (&noa == &sa ? "supplied" : "default") << endl;
      }
...
int main(int argc, char *argv[]) {
  ...
  case 12: {
    ...
    TestDriver<bsltf::AllocTestType,
