...
TestString::TestString(bslma::Allocator *allocator)
    : d_allocator_p(bslma::Default::allocator(allocator)), d_string_p(0) {}
...
TestString &TestString::operator=(const TestString &rhs) {
  ...
  if (rhs.d_string_p) {
    d_string_p =
        new (*d_allocator_p) bsl::string(*rhs.d_string_p, d_allocator_p);
  }
  return *this;
}
...
void populate(Container *container_p, const TimeIntervalList &timeValues,
              int numItems, int *randSeed_p)
...
public:
  // CREATORS
  explicit TestLockObject(const bdlcc::TimeQueue<TestLockObject> *queue = 0,
                          int *numDestructions = 0, int verbose = 0);
...
// CREATORS
TestLockObject::TestLockObject(const bdlcc::TimeQueue<TestLockObject> *queue,
                               int *numDestructions, int verbose)
    : d_timeQueue_p(queue), d_numDestructions_p(numDestructions),
      d_verbose(verbose) {}
...
// MANIPULATORS
void TestLockObject::reset() { d_timeQueue_p = 0; }
...
template <class VECTOR> void testCasePopLE() {
  static const u::VecType vecType =
      bsl::is_same<bsl::vector<Item>, VECTOR>::value ? u::e_BSL
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      : bsl::is_same<std::experimental::pmr::vector<Item>, VECTOR>::value
          ? u::e_PMR
#endif
      : bsl::is_same<std::vector<Item>, VECTOR>::value ? u::e_STD
                                                       : u::e_NUM_VEC_TYPES;
  ...
  if (verbose)
    cout << "\t'popLE' without maxTimers.  VecType: " << vecType << endl;
  {
    ...
    for (int i = 0; i < NUM_POPS_NM; ++i) {
      ...
      if (buffer.size() && EXPNUMITEMS == (int)buffer.size()) {
        for (int j = 0; j < EXPNUMITEMS; ++j) {
          ...
          const bsls::TimeInterval EXPTIME(VALUES[I].d_secs, VALUES[I].d_nsecs);
          ...
          ASSERTV(j, true != X.isRegisteredHandle(handles[I]));
        }
      }
    }
  }
  ...
  if (verbose)
    cout << "\t'popLE' with maxTimers.  VecType: " << vecType << endl;
  {
    int handles[NUM_VALUES];
...
void threadFunc(bdlcc::TimeQueue<int> *timeQueue, int numIterations,
                int sendCount, int receiveCount, int delay) {
...
protected:
  ...
  void dataAvailable(my_Connection *connection, void *buffer_p, int length);
...
public:
  // CREATORS
  explicit my_Server(int ioTimeout, bslma::Allocator *basicAllocator = 0);
...
my_Server::my_Server(int ioTimeout, bslma::Allocator *basicAllocator)
    : d_timeQueue(basicAllocator), d_ioTimeout(ioTimeout),
...
void my_Server::newConnection(my_Connection *connection) {
  ...
  connection->d_timerId = d_timeQueue.add(
      bdlt::CurrentTime::now() + d_ioTimeout, connection, &isNewTop);
  if (isNewTop) {
...
void my_Server::removeConnection(my_Connection *connection) {
  ...
  bsl::vector<my_Connection *>::iterator begin = d_connections.begin(),
                                         end = d_connections.end(), it = begin;
  for (; it != end; ++it) {
    if (connection == *it) {
...
//..
void my_Server::dataAvailable(my_Connection *connection, void *buffer_p,
                              int length) {
...
void my_Server::monitorTimers() {
  while (!d_done) {
    bsl::vector<bdlcc::TimeQueueItem<my_Connection *>> expiredTimers;
    {
      ...
      d_timeQueue.popLE(bsls::SystemTime::nowRealtimeClock(), &expiredTimers,
                        &newLength, &minTime);

      if (!expiredTimers.size()) {
        if (newLength) {
          // no expired timers, but unexpired timers remain.
          d_timerChangedCond.timedWait(&d_timerMonitorMutex, minTime);
        } else {
    ...
    if (length) {
      bdlcc::TimeQueueItem<my_Connection *> *data = &expiredTimers.front();
...
int my_Server::start() {
  ...
  if (bslmt::ThreadUtil::create(&d_connectionThreadHandle, attr,
                                &my_connectionMonitorThreadEntry, this)) {
    return -1; // RETURN
  }

  if (bslmt::ThreadUtil::create(&d_timerThreadHandle, attr,
                                &my_timerMonitorThreadEntry, this)) {
    return -1; // RETURN
  }
  return 0;
}
...
public:
  // CREATORS
  explicit my_TestSession(int verbose) : my_Session(), d_verbose(verbose) {}
  ...
  virtual int processData(void *data, int length) {
    ...
    if (d_verbose) {
      bsl::cout << bdlt::CurrentTime::utc() << ": ";
      bsl::cout << "Processing data at address " << data << " and length "
                << length << ".\n";
    }
    return 0;
  }
...
public:
  // CREATORS
  explicit my_TestServer(int ioTimeout, int verbose = 0,
                         bslma::Allocator *basicAllocator = 0)
      : my_Server(ioTimeout, basicAllocator), d_verbose(verbose) {}
...
void my_TestServer::monitorConnections() {
  ...
  if (d_verbose) {
    bsl::cout << bdlt::CurrentTime::utc() << ": ";
    bsl::cout << "Connection " << connection1 << " receives " << length
              << " bytes " << endl;
  }
...
int main(int argc, char *argv[]) {
  ...
  case 14: {
    ...
    if (verbose)
      cout << "CONCERN: ORDER PRESERVATION\n"
              "===========================\n";
...

    {
      ...
      if (verbose)
        cout << "<| Via Multi popLE with maxTimers: " << k_MAX_TIMERS
             << " |>\n";
      ...
      for (unsigned jj = 0; 0 < X.length(); ++jj) {
        ...
        int newLength;
        bsls::TimeInterval newMinTime;
        mX.popLE(maxTime, k_MAX_TIMERS, &outVec, &newLength, &newMinTime);
        outLen = outVec.size();
  ...
  case 13: {
...

    {
      ...
      bsls::TimeInterval futureTime = u::now() + bsls::TimeInterval(600, 0);
  ...
  case 12: {
    ...
    if (verbose)
      cout << endl
           << "Testing \"Remove All\" followed by \"Add\"" << endl
           << "======================================" << endl;

    {
      if (veryVerbose)
        bsl::cout << "a) removeAll" << bsl::endl;
...

    {
      ...
      for (bsl::set<int>::const_iterator it = handles.begin();
           it != handles.end(); ++it) {
    ...
    {
      if (veryVerbose)
        bsl::cout << "e) popFront (once)" << bsl::endl;
  ...
  case 11: {
    ...
    for (int i = 0; i < k_NUM_THREADS; ++i) {
      ...
      bslmt::ThreadUtil::create(&threads[i], testAddUpdatePopRemoveAll,
                                (void *)&info[i]);
    }

    bslmt::ThreadUtil::create(&threads[k_NUM_THREADS], testLengthAndCountLE,
                              NULL);
  ...
  case 9: {
    ...
    if (verbose)
      cout << "\t'popLE' without maxTimers." << endl;
    {
      static const struct {
        int d_line;
        int d_secs;
        ...
        int d_expNumItems;
        const char *d_expItems;
      } POP_DATA[] = {// line  secs    nsecs   expNumItems expItems
                      //---- ------ --------- ----------- ----------------
                      {L_, 1, 0, 2, "\x6\xd"},
                      {L_, 1, 0, 0, ""},
                      //  {  L_ , 1    , 9999998 , 2         , "\x4\xb" },
    ...
    if (verbose)
      cout << "\t'popLE' with maxTimers." << endl;
    {
      static const struct {
        int d_line;
        int d_secs;
        ...
        int d_expNumItems;
        const char *d_expItems;
      } POP_DATA[] = {// line secs   nsecs   maxNum expNum expItems
                      //---- ---- --------- ------ ------ ----------------
                      {L_, 1, 0, 0, 0, ""},
                      {L_, 1, 0, 1, 1, "\x6"},
                      {L_, 1, 0, 1, 1, "\xd"},
  ...
  case 8: {
...

    {
      ...
      for (int i = 0; i < NUM_VALUES; ++i) {
        ...
        Item item(&ta);
        ASSERTV(LINE, true == X.isRegisteredHandle(handles[i]));
        ASSERTV(LINE, 0 == mX.update(handles[i], UPDTIME, &isNewTop));
  ...
  case 6: {
...

    {
      ...
      static const struct {
        ...
        int d_nsecs;
        const char *d_value;
      } VALUES[] = {// line secs  nsecs    value
                    //---- ----- -------- --------
                    {L_, 2, 1000000, VA}, {L_, 2, 1000000, VB},
                    {L_, 2, 1000000, VC}, {L_, 2, 1000001, VB},
                    {L_, 1, 9999998, VC}, {L_, 1, 9999999, VD},
                    {L_, 0, 0000000, VE}};
...

    {
      ...
      static const struct {
        ...
        int d_expMinNsecs;
          ...
          //------- ---------- -----------
          {6, 1, 9999998}, {4, 1, 9999999}, {5, 2, 1000000}, {0, 2, 1000000},
          {1, 2, 1000000}, {2, 2, 1000001}, {3, 0, 0},
      };
      ...
      for (int i = 0; i < NUM_VALUES; ++i) {
        ...
        Item item(&ta);
        ASSERTV(i, true == X.isRegisteredHandle(handles[I]));
        ASSERTV(i, 0 == mX.popFront(&item, &newLength, &newMinTime));
        ASSERTV(i, EXPTIME == item.time());
        ASSERTV(i, EXPVAL == item.data());
        ASSERTV(i, handles[I] == item.handle());
        ASSERTV(i, (NUM_VALUES - 1 - i) == X.length());
...

    {
      ...
      static const struct {
        ...
        const char *d_value;
                    ...
                    //---- ----- -------- --------
                    {L_, 0, 0000000, VE}, {L_, 1, 9999998, VC},
                    {L_, 1, 9999999, VD}, {L_, 2, 1000000, VA},
                    {L_, 2, 1000000, VB}, {L_, 2, 1000000, VC},
                    {L_, 2, 1000001, VB}};
  ...
  case 4: {
...

    {
      ...
      for (int i = 0; i < NUM_VALUES; ++i) {
        ...
        ASSERTV(LINE,
                0 == mX.remove(handles[i], &newLength, &newMinTime, &buffer));
  ...
  case 3: {
...

    {
      ...
      static const struct {
        ...
        int d_isNewTop;
        int d_countLE;
      } VALUES[] = {// line secs  nsecs    value    isNewTop countLE
                    //---- ----- -------- -------- -------- -------
                    {L_, 2, 1000000, VA, 1, 0}, {L_, 2, 1000000, VB, 0, 0},
                    {L_, 2, 1000000, VC, 0, 0}, {L_, 2, 1000001, VB, 0, 0},
                    {L_, 1, 9999998, VC, 1, 1}, {L_, 1, 9999999, VD, 0, 2},
                    {L_, 1, 9999999, VE, 0, 3}, {L_, 1, 9999999, VC, 0, 4},
                    {L_, 0, 0000000, VE, 1, 5}};
...

    {
      ...
      static const struct {
        ...
        const char *d_value;
        int d_isNewTop;
      } VALUES[] = {// line secs  nsecs    value    isNewTop
                    //---- ----- -------- -------- --------
                    {L_, 2, 1000000, VA, 1},
                    {L_, 2, 1000001, VB, 0},
                    {L_, 1, 9999998, VC, 1},
  ...
  case 2: {
      ...
      cout << "\tWithout allocators.\n";
    {
      ...
      if (verbose)
        cout << "\t\tConstructor without key.\n";
      Obj mY(TIME, STRDATA, HANDLE);
      const Obj &Y = mY;
      ASSERT(TIME == Y.time());
      ...
      if (verbose)
        cout << "\t\tConstructor with key.\n";
      Obj mZ(TIME, STRDATA, HANDLE, KEY);
      const Obj &Z = mZ;
      ASSERT(TIME == Z.time());
      ...
      cout << "\tWith allocators.\n";
    {
      ...
      if (verbose)
        cout << "\t\tConstructor with key.\n";
      Obj mZ(TIME, STRDATA, HANDLE, KEY, &ta);
      const Obj &Z = mZ;
      {
        const bsls::Types::Int64 NUM_ALLOC = ta.numAllocations();
        ASSERT(TIME == Z.time());
  ...
  case 1: {
...

    {
      ...
      if (verbose)
        cout << tItem.time() << " " << tItem.data() << endl;
