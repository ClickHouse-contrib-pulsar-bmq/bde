...
// Usage example 1.
{
  ...
  // that we must search the results for the expected values because the order
  // in which values are retrieved is not specified.
  //
}
...
private:
  ...
  struct TestCaseEraseIfValuePredicate {
    ...
    // CREATORS
    explicit TestCaseEraseIfValuePredicate(const VALUE &value, const OP &op)
        // Create a 'TestCaseEraseIfValuePredicate' with the specified
        // 'value' and specified 'op'.
        : d_value(value), d_op(op) {}
...
void TestDriver<KEY, VALUE, HASH, EQUAL>::testCase26() {
  ...
  for (bsl::size_t ti = 1; ti < MAX_LENGTH; ++ti) {
    ...
    for (bsl::size_t tj = 1; tj < LENGTH; ++tj) {
      ...
      for (bsl::size_t v = 0; v < tj; ++v) {
        ...
        bsl::size_t rc = mX.eraseFirstIf(VALUES[v].first, valueGreaterThanZero);
        ASSERTV(rc, rc == 1);
        ASSERTV(tj, v, X.size(), tj - v - 1 == X.size());

        bsl::size_t newBucketSize = X.bucketSize(bucketIdx);
        ASSERTV(newBucketSize, oldBucketSize,
  ...
  for (bsl::size_t ti = 1; ti < MAX_LENGTH; ++ti) {
    ...
    for (bsl::size_t tj = 1; tj < LENGTH; ++tj) {
      ...
      for (bsl::size_t v = 0; v < tj; ++v) {
        ...
        ASSERTV(veryNewBucketSize, oldBucketSize,
  ...
  // Erase fails for the elements when predicate returns false
  {
    ...
    bslma::TestAllocatorMonitor sam(&supplied);
    bsl::size_t rc = mX.eraseFirstIf(VALUES[0].first, valueGreaterThanZero);
    ASSERTV(rc, rc == 0);
    ASSERTV(mX.size(), mX.size() == 1);
    ASSERT(sam.isInUseSame());
  }
...
void TestDriver<KEY, VALUE, HASH, EQUAL>::testCase25() {
  ...
  if (verbose)
    cout << endl << "TEST 'eraseAllIf'" << endl << "=================" << endl;
  ...
  for (bsl::size_t ti = 1; ti < MAX_LENGTH; ++ti) {
    ...
    for (bsl::size_t tj = 0; tj < LENGTH; ++tj) {
      ...
      for (bsl::size_t v = 0; v < tj; ++v) {
        ...
        bsl::size_t rc = mX.eraseAllIf(VALUES[v].first, valueGreaterThanZero);
        ASSERTV(rc == 1);
      ...
      for (bsl::size_t tk = 0; tk < LENGTH; ++tk) {
        if (INDEX == tk) {
          for (bsl::size_t tl = 0; tl < LENGTH; ++tl) {
            ...
            mX.insert(VALUES[tk].first, VALUES[LENGTH - tl - 1].second);
          }
        } else {
  ...
  for (bsl::size_t ti = 0; ti < 6; ++ti) {
    ...
    for (int i = 0; i <= 7; ++i) { // Insert types
      ...
      for (bsl::size_t tj = 0; tj < LENG; ++tj) {
        ...
        case 3: { // setValueAll
          bsl::size_t rc = mX.setValueAll(VALUES[tj].first, VALUES[tj].second);
          ASSERTV(0 == rc)
        } break;
        ...
        if (i == 5 || (i >= 6 && bslma::UsesBslmaAllocator<VALUE>::value)) {
          dam.reset();
        }
...
void TestDriver<KEY, VALUE, HASH, EQUAL>::testCase16() {
  ...
  if (verbose)
    cout << endl << "TEST 'update'" << endl << "=============" << endl;
...
void TestDriver<KEY, VALUE, HASH, EQUAL>::testCase15() {
  ...
  // Test empty hash map.
  {
    ...
    int rc1 = mX.setComputedValueAll(VALUES[0].first, testCase12Updater);
    ASSERTV(rc1, 0 == rc1);
    ASSERTV(1 == X.size());
...
void TestDriver<KEY, VALUE, HASH, EQUAL>::testCase14() {
  ...
  for (bsl::size_t ti = 1; ti < MAX_LENGTH; ++ti) {
    ...
    // Check missing value.
    rc1 = mX.setComputedValueFirst(VALUES[LENGTH + 1].first, testCase12Updater);
    ASSERTV(rc1, 0 == rc1);
...
void TestDriver<KEY, VALUE, HASH, EQUAL>::testCase12() {
  ...
  for (bsl::size_t ti = 1; ti < MAX_LENGTH; ++ti) {
    ...
    // Check existing value.
    bsl::size_t rc =
        mX.setValueFirst(VALUES[LENGTH - 1].first, VALUES[LENGTH].second);
    ASSERTV(rc, 1 == rc);
    ...
    rc = mX.setValueFirst(VALUES[LENGTH + 1].first, VALUES[LENGTH + 1].second);
    ASSERTV(rc, 0 == rc);
    ...
    // Exception testing - check missing value
    BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(supplied) {
      mX.setValueFirst(VALUES[LENGTH + 1].first, VALUES[LENGTH + 1].second);
    }
    BSLMA_TESTALLOCATOR_EXCEPTION_TEST_END;
  }
...
void TestDriver<KEY, VALUE, HASH, EQUAL>::testCase11() {
  ...
  // Try to insert into an empty hash map.
  {
    ...
    for (bsl::size_t tj = 0; tj < MAX_LENGTH; ++tj) {
      typename Obj::KVType item(VALUES[tj].first, VALUES[tj].second, &scratch);
      insertVec.push_back(item);
    }
      ...
      for (bsl::size_t tk = 0; tk <= 3 && tk <= LENGTH; ++tk) {
        ...
        for (bsl::size_t i = 0; i < tk; ++i) {
          typename Obj::KVType item(VALUES[i + tkOffset].first,
                                    VALUES[i + tkOffset + 1].second, &scratch);
          insertVec.push_back(item);
        }
...
void TestDriver<KEY, VALUE, HASH, EQUAL>::testCase8() {
  ...
  if (verbose)
    cout << endl << "TEST 'eraseAll'" << endl << "===============" << endl;
...
void TestDriver<KEY, VALUE, HASH, EQUAL>::testCase7() {
  ...
  if (verbose)
    cout << endl << "TEST 'eraseFirst'" << endl << "=================" << endl;
...
void TestDriver<KEY, VALUE, HASH, EQUAL>::testCase3() {
  ...
  for (bsl::size_t ti = 0; ti < NUM_CONFIGS; ++ti) {
      ...
      cout << "Depth 0" << endl;
    {
      ...
      } INSERTS0[] = {// LINE KEY VAL EXP_SIZE EXP_BIDX EXP_BSIZE
                      //---- --- --- -------- -------- ---------
                      {L_, 0, 10, 1, 0, 1},
                      {L_, 1, 11, 1, 1, 1}};
      ...
      cout << "Depth 1" << endl;
    {
      ...
      } INSERTS1[] = {// LINE OBJ  KEY  VAL  EXP_RC EXP_SIZE #V1 #V2
                      //---- ---  ---  ---  ------ -------- --- ---
                      {L_, 'A', KA, VA1, 1, 2, 2, 0},
                      {L_, 'A', KA, VA2, 1, 2, 1, 1},
                      {L_, 'B', KB, VB1, 1, 2, 2, 0},
...
void TestDriver<KEY, VALUE, HASH, EQUAL>::testCase1() {
...

  {
    ...
    mX.insert(TstFacility::create<KEY>(1), TstFacility::create<VALUE>(11));
    mX.insert(TstFacility::create<KEY>(2), TstFacility::create<VALUE>(22));
    mX.insert(TstFacility::create<KEY>(3), TstFacility::create<VALUE>(33));
    mX.insert(TstFacility::create<KEY>(4), TstFacility::create<VALUE>(44));
    mX.insert(TstFacility::create<KEY>(5), TstFacility::create<VALUE>(55));

    ASSERTV(X.size(), X.size() == 5);

    {
      VALUE value;
      bsl::size_t rc = mX.getValueFirst(&value, TstFacility::create<KEY>(1));
      ASSERTV(rc, 1 == rc);
    ...
    {
      VALUE value;
      bsl::size_t rc = mX.getValueFirst(&value, TstFacility::create<KEY>(3));
      ASSERTV(rc, 1 == rc);
...
namespace hPerf {
template <class KEY, class VAL> class HBenchmark {
...
public:
  HBenchmark(int numStripes, int numBuckets, int maxSize, bool enblRehash,
...
HBenchmark<KEY, VAL>::HBenchmark(int numStripes, int numBuckets, int maxSize,
                                 bool enblRehash,
                                 bslma::Allocator *basicAllocator)
    : d_numStripes(numStripes), d_numBuckets(numBuckets), d_maxSize(maxSize),
      d_enblRehash(enblRehash), d_map_p(0), d_curValue(0), d_countErr(0),
...
void HBenchmark<KEY, VAL>::initializeSampleNoInsert(bool) {
  // For test types 0, 1
  d_map_p = new (*d_allocator_p)
      MapType(static_cast<bsl::size_t>(d_numBuckets),
              static_cast<bsl::size_t>(d_numStripes), d_allocator_p);
  d_curValue = 0;
  d_countErr = 0;
}

template <class KEY, class VAL> void HBenchmark<KEY, VAL>::cleanupSample(bool) {
  d_allocator_p->deleteObject(d_map_p);
}

// ACCESSORS
template <class KEY, class VAL>
inline int HBenchmark<KEY, VAL>::countErr() const {
  return d_countErr;
}

// TEST FUNCTIONS
template <class KEY, class VAL> void HBenchmark<KEY, VAL>::insert(int) {
  // insert a single element into the hash map. Test type 1
  int key = d_curValue++;
  KEY ky = makeKey(key);
  VAL value = makeValue(key);
  ...
  if (BSLS_PERFORMANCEHINT_PREDICT_UNLIKELY(
          d_maxSize == static_cast<int>(d_map_p->size()))) {
    d_map_p->clear();
    d_curValue = 0;
  }
}

template <class KEY, class VAL> void HBenchmark<KEY, VAL>::findExist(int) {
  // find a single element that exists in the hash map. Test type 2
  int key = d_curValue++;
  KEY ky = makeKey(key);
  VAL value;
  bsl::size_t num = d_map_p->getValueFirst(&value, ky);
  if (BSLS_PERFORMANCEHINT_PREDICT_UNLIKELY(0 == num))
    ++d_countErr;
  if (BSLS_PERFORMANCEHINT_PREDICT_UNLIKELY(d_curValue >= d_maxSize))
    d_curValue = 0;
}

template <class KEY, class VAL> void HBenchmark<KEY, VAL>::findMiss(int) {
  // find a single element that is missing in the hash map. Test type 3
  int key = d_curValue++;
...
template <class KEY, class VAL> void HBenchmark<KEY, VAL>::erase(int) {
  // erase a single element from the hash map. Test type 4
  int key = d_curValue++;
  KEY ky = makeKey(key);
...
int main(int argc, char *argv[]) {
  ...
  case 24: {
    ...
    {
      bsl::vector<bdlcc::StripedUnorderedMultiMap<int, bsl::string>> mX(2);
    ...
    ASSERTV(numBytesInUse, supplied.numBytesInUse(),
            numBytesInUse == supplied.numBytesInUse());
    ...
    numBytesInUse = supplied.numBytesInUse();
    {
      ...
      ASSERTV(numBytesInUse, numBytesInUse2, numBytesInUse2 > numBytesInUse);
      ...
      ASSERTV(numBytesInUse2, numBytesInUse3, numBytesInUse3 > numBytesInUse2);
      ...
      ASSERTV(numBytesInUse3, numBytesInUse4, numBytesInUse4 == numBytesInUse3);
    }
    ASSERTV(numBytesInUse, supplied.numBytesInUse(),
            numBytesInUse == supplied.numBytesInUse());
  } break;
  ...
  case -1: {
    ...
    if (argc < 3) {
      bsl::cout
          ...
          << "\n"
          << "Test types: 1-insert, 2-find, 3-not find, 4-erase, "
             "5-insert+find\n"
    ...
    for (;;) {
      ...
      bsl::function<void(bool)> initFunc =
          (testType % 4) == 1
              ? bdlf::BindUtil::bind(&Bench::initializeSampleNoInsert, &hb,
                                     bdlf::PlaceHolders::_1)
              : bdlf::BindUtil::bind(&Bench::initializeSample, &hb,
                                     bdlf::PlaceHolders::_1);
      bsl::function<void(bool)> cleanFunc = bdlf::BindUtil::bind(
          &Bench::cleanupSample, &hb, bdlf::PlaceHolders::_1);

      switch (testType) {
      case 1: { // insert
        runFunc1 =
            bdlf::BindUtil::bind(&Bench::insert, &hb, bdlf::PlaceHolders::_1);
      } break;
      case 2: { // find exist
        runFunc1 = bdlf::BindUtil::bind(&Bench::findExist, &hb,
                                        bdlf::PlaceHolders::_1);
      } break;
      case 3: { // find miss
        runFunc1 =
            bdlf::BindUtil::bind(&Bench::findMiss, &hb, bdlf::PlaceHolders::_1);
      } break;
      case 4: { // erase
        runFunc1 =
            bdlf::BindUtil::bind(&Bench::erase, &hb, bdlf::PlaceHolders::_1);
      } break;
      case 5: { // insert + read
        runFunc1 =
            bdlf::BindUtil::bind(&Bench::insert, &hb, bdlf::PlaceHolders::_1);
      ...
      tb.execute(&res, numMillis, numSamples, initFunc,
      ...
      bsl::cout << bsl::fixed << bsl::setprecision(0) << testType << ","
                ...
                << enblRehash << "," << percentiles[0] << "," << percentiles[1]
                << "," << percentiles[2] << "," << percentiles[3] << ","
                << percentiles[4] << "," << countErr;
      if (tGId2 >= 0) {
        res.getPercentiles(&percentiles, tGId2);
        bsl::cout << bsl::fixed << bsl::setprecision(0) << "," << percentiles[0]
      ...
      ++pctIdx;
      if (pctIdx < static_cast<int>(percentages.size()))
        continue;
      pctIdx = 0;
      ++loadIdx;
      if (loadIdx < static_cast<int>(loads.size()))
        continue;
      loadIdx = 0;
      ++numBucketIdx;
      if (numBucketIdx < static_cast<int>(numBuckets.size()))
        continue;
      numBucketIdx = 0;
      ++numStripeIdx;
      if (numStripeIdx < static_cast<int>(numStripes.size()))
        continue;
      numStripeIdx = 0;
      ++testTypeIdx;
      if (testTypeIdx < static_cast<int>(testTypes.size()))
        continue;
      testTypeIdx = 0;
      ++numThread2Idx;
      if (numThread2Idx < static_cast<int>(numThreads2.size()))
        continue;
      numThread2Idx = 0;
      ++numThread1Idx;
      if (numThread1Idx >= static_cast<int>(numThreads1.size()))
        break;
    }
  } break;
  case -2: {
