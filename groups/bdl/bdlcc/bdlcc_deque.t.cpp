...
public:
  ...
  MoveCopyAllocTestType &
  operator=(bslmf::MovableRef<MoveCopyAllocTestType> rhs);
  // Assign to this object the value of the specified 'rhs' object, and
...
// MANIPULATORS
inline void MoveCopyAllocTestType::setMovedInto(MoveState::Enum value) {
  d_movedInto = value;
}

// ACCESSORS
inline int MoveCopyAllocTestType::data() const {
  return d_data_p ? *d_data_p : 0;
}

inline MoveState::Enum MoveCopyAllocTestType::movedFrom() const {
  return d_movedFrom;
}

inline MoveState::Enum MoveCopyAllocTestType::movedInto() const {
  return d_movedInto;
}

// Aspects

inline bslma::Allocator *MoveCopyAllocTestType::allocator() const {
  return d_allocator_p;
}

// FREE FUNCTION
inline MoveState::Enum getMovedFrom(const MoveCopyAllocTestType &object) {
  return object.movedFrom();
}

inline MoveState::Enum getMovedInto(const MoveCopyAllocTestType &object) {
  return object.movedInto();
}

inline void setMovedInto(MoveCopyAllocTestType *object, MoveState::Enum value) {
...
MoveCopyAllocTestType::MoveCopyAllocTestType(bslma::Allocator *basicAllocator)
    : d_data_p(0), d_allocator_p(bslma::Default::allocator(basicAllocator)),
      d_self_p(this), d_movedFrom(bsltf::MoveState::e_NOT_MOVED),
...
MoveCopyAllocTestType::MoveCopyAllocTestType(
    const MoveCopyAllocTestType &original, bslma::Allocator *basicAllocator)
    : d_data_p(0), d_allocator_p(bslma::Default::allocator(basicAllocator)),
      d_self_p(this), d_movedFrom(bsltf::MoveState::e_NOT_MOVED),
      d_movedInto(bsltf::MoveState::e_NOT_MOVED) {
  d_data_p = static_cast<int *>(d_allocator_p->allocate(sizeof(int)));
...
MoveCopyAllocTestType::~MoveCopyAllocTestType() {
  ...
  BSLS_ASSERT_OPT(!!d_data_p == (bsltf::MoveState::e_NOT_MOVED == d_movedFrom));
...
MoveCopyAllocTestType::operator=(const MoveCopyAllocTestType &rhs) {
  if (&rhs != this) {
    int *newData = static_cast<int *>(d_allocator_p->allocate(sizeof(int)));
    d_allocator_p->deallocate(d_data_p);
    d_data_p = newData;
    *d_data_p = rhs.data();
...
// FREE OPERATORS
inline bool bsltf::operator==(const bsltf::MoveCopyAllocTestType &lhs,
...
namespace bslma {
template <>
struct UsesBslmaAllocator<bsltf::MoveCopyAllocTestType> : bsl::true_type {};
} // namespace bslma
} // namespace BloombergLP
...
template <>
struct HasWellBehavedMove<bsltf::MovableAllocTestType>
    : public bsl::false_type {};

template <>
struct HasWellBehavedMove<bsltf::AllocTestType> : public bsl::true_type {};

template <>
struct HasWellBehavedMove<bsltf::MoveCopyAllocTestType>
    : public bsl::true_type {};

template <class TYPE> struct IsMoveAware : public bsl::false_type {};
...
template <class TYPE> struct IsAllocating : public bsl::false_type {};
...
template <class TYPE> void setData(TYPE *dst, int src) { dst->setData(src); }

template <> void setData<Element>(Element *dst, int src) { *dst = src; }

template <class TYPE> int getData(const TYPE &src) {
  return TTF::getIdentifier(src);
}

template <> int getData<Element>(const Element &src) {
...
template <> int getData<MCElement>(const MCElement &src) { return src.data(); }

const void *p(long unsigned u) { return reinterpret_cast<const void *>(u); }
...
struct BreakTestAllocator : public bslma::TestAllocator {
  ...
  // CREATORS
  explicit BreakTestAllocator(bslma::Allocator *basicAllocator = 0)
      : TestAllocator("break_test", basicAllocator) {}
  explicit BreakTestAllocator(const char *name,
                              bslma::Allocator *basicAllocator = 0)
      : TestAllocator(name, basicAllocator) {}
  explicit BreakTestAllocator(bool verboseFlag,
                              bslma::Allocator *basicAllocator = 0)
      : TestAllocator("break_test", verboseFlag, basicAllocator) {}
  explicit BreakTestAllocator(const char *name, bool verboseFlag = false,
                              bslma::Allocator *basicAllocator = 0)
      : TestAllocator(name, verboseFlag, basicAllocator) {}
...
public:
  explicit RandChar(int startSeed = 12345)
      : d_randGen(startSeed)
...
public:
  // CREATOR
  explicit RandElement(unsigned seed)
      : d_randGen(seed)
...
public:
  // CREATOR
  explicit Rand1To8(unsigned seed)
      : d_randGen(seed), d_rand(0), d_bitsInDRand(0)
...
template <class ELEMENT> class ObjChecker {
...
public:
  ...
  ObjChecker(const bdlcc::Deque<ELEMENT> &toBeModified,
             const bdlcc::Deque<ELEMENT> &toBeCompared, int line)
      ...
      : d_toBeModified_p(&toBeModified), d_toBeCompared_p(&toBeCompared),
        d_line(line) {}
...
template <class ELEMENT> struct VecChecker {
  // DATA
  ...
  VecChecker(const bsl::vector<ELEMENT> &toBeModified,
             const bsl::vector<ELEMENT> &toBeCompared, int line)
      : d_toBeModified_p(&toBeModified), d_toBeCompared_p(&toBeCompared),
        d_line(line)
  ...
  {                                                                            \
    ...
    BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(*EXCEP_TEST_OBJ_ALLOC) {          \
      u::ObjChecker<EXCEP_TEST_ELEM_TYPE> EXCEP_TEST_OBJ_CHECKER(              \
          (EXCEP_TEST_OBJ_OBJ), EXCEP_TEST_OBJ_COMPARE, __LINE__);
...
#define BEGIN_EXCEP_TEST_OBJ_VEC(EXCEP_TEST_ELEM_TYPE, EXCEP_TEST_OBJ_OBJ,     \
                                 EXCEP_TEST_OBJ_VEC)                           \
  {                                                                            \
    ...
    const bdlcc::Deque<EXCEP_TEST_ELEM_TYPE> EXCEP_TEST_OBJ_COMPARE(           \
        (EXCEP_TEST_OBJ_OBJ), &nda);                                           \
    const bsl::vector<EXCEP_TEST_ELEM_TYPE> EXCEP_TEST_OBJ_VEC_COMPARE(        \
        (EXCEP_TEST_OBJ_VEC), &nda);                                           \
    BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(*EXCEP_TEST_VEC_ALLOC) {          \
      u::ObjChecker<EXCEP_TEST_ELEM_TYPE> EXCEP_TEST_OBJ_CHECKER(              \
          (EXCEP_TEST_OBJ_OBJ), EXCEP_TEST_OBJ_COMPARE, __LINE__);             \
      u::VecChecker<EXCEP_TEST_ELEM_TYPE> EXCEP_TEST_VEC_CHECKER(              \
          (EXCEP_TEST_OBJ_VEC), EXCEP_TEST_OBJ_VEC_COMPARE, __LINE__);
...
struct Event {
  enum EventType { e_IN_PROGRESS = 1, e_TASK_COMPLETE = 2 };
...
struct WorkerFunctor {
  ...
  // at once to maximize concurrency.
  {
    ...
    while (evnum < k_NUM_TO_PUSH) {
      ...
      Event ev = {Event::e_IN_PROGRESS, d_workerId, evnum++,
                  "In-Progress Event"};
    ...
    Event ev = {Event::e_TASK_COMPLETE, d_workerId, evnum, "Task Complete"};
...
struct WorkRequest {
  // PUBLIC TYPES
  enum RequestType { e_WORK = 1, e_STOP = 2 };
...
struct ConsumerFunctor {
  ...
  // CREATORS
  explicit ConsumerFunctor(bdlcc::Deque<WorkRequest> *container)
...
struct ProducerFunctor {
  ...
  // CREATORS
  explicit ProducerFunctor(bdlcc::Deque<WorkRequest> *container)
...
template <class ELEMENT> class TimedPopRecordBack {
...
public:
  // CREATORS
  TimedPopRecordBack(bdlcc::Deque<ELEMENT> *deque, bslmt::Barrier *barrier,
                     bsls::TimeInterval timeout, const ELEMENT &value)
      : d_deque_p(deque), d_barrier_p(barrier), d_timeout(timeout),
        d_expected(value)
  ...
  // will succeed without timing out.
  {
    ...
    BEGIN_EXCEP_TEST_OBJ(ELEMENT, *d_deque_p) {
      ...
      timeoutFlag = d_deque_p->timedPopBack(&result, start + d_timeout);
    }
    END_EXCEP_TEST_OBJ
    end = u::now();
    ASSERTV(numThrows, expAssignMove,
            !PLAT_EXC || (!u::IsAllocating<ELEMENT>::value ||
                          (u::IsMoveAware<ELEMENT>::value && expAssignMove)) ==
                             (0 == numThrows));
    const bsltf::MoveState::Enum expMS =
        u::IsMoveAware<ELEMENT>::value ? expAssignMoveState : e_UNKNOWN;
...
template <class ELEMENT> class TimedPopRecordFront {
...
public:
  // CREATORS
  TimedPopRecordFront(bdlcc::Deque<ELEMENT> *deque, bslmt::Barrier *barrier,
                      bsls::TimeInterval timeout, const ELEMENT &value)
      : d_deque_p(deque), d_barrier_p(barrier), d_timeout(timeout),
        d_expected(value)
  // Create a test object that will access the specified '*deque' and
  ...
  // will succeed without timing out.
  {
    ...
    BEGIN_EXCEP_TEST_OBJ(ELEMENT, *d_deque_p) {
      ...
      timeoutFlag = d_deque_p->timedPopFront(&result, start + d_timeout);
    }
    END_EXCEP_TEST_OBJ
    end = u::now();
    ASSERTV(numThrows, expAssignMove,
...
template <class ELEMENT> void testTimedPushPopMove() {
  if (verbose)
    cout << "testTimedPushPopMove<" << NameOf<ELEMENT>() << ">()\n";
  ...
  bsltf::MoveState::Enum ecm =
      u::IsMoveAware<ELEMENT>::value ? e_MOVED : e_UNKNOWN;

  if (verbose)
    cout << "\tWith 'timedPopBack'" << endl;
  {
    bsls::TimeInterval T3(3 * DECI_SEC); // 0.3s

    bslmt::Barrier barrier(2);
    bdlcc::Deque<ELEMENT> x(1, &ta);
    ...
    bsls::TimeInterval start = u::now();
    int rc = x.timedPushBack(bslmf::MovableRefUtil::move(vA), start + T3);
    bsls::TimeInterval end = u::now();
    ASSERT(0 == rc);
    ASSERT(end < start + T3);
    ASSERTV(ta.numAllocations() - tana, (u::IsAllocating<ELEMENT>::value &&
    ...
    ASSERT(1 == X.length());
    start = u::now();
    rc = x.timedPushBack(bslmf::MovableRefUtil::move(vB), start + T3);
    end = u::now();
    ASSERT(0 != rc);
    ASSERT(end >= start + T3);
    ...
    cout << "\tWith 'timedPopFront'" << endl;
  {
    ...
    Int64 tana = ta.numAllocations();
    bsls::TimeInterval start = u::now();
    int rc = x.timedPushFront(bslmf::MovableRefUtil::move(vA), start + T3);
    bsls::TimeInterval end = u::now();
    ASSERT(0 == rc);
    ASSERT(end < start + T3);
    ASSERTV(ta.numAllocations() - tana, (u::IsAllocating<ELEMENT>::value &&
    ...
    ASSERT(1 == X.length());
    start = u::now();
    rc = x.timedPushFront(bslmf::MovableRefUtil::move(vB), start + T3);
    end = u::now();
    ASSERT(0 != rc);
    ASSERT(end >= start + T3);
    ASSERTV(ta.numAllocations() - tana, 0 == ta.numAllocations() - tana);
...
template <class ELEMENT> void testSingleMovingPushesPops() {
  bslma::TestAllocator da(veryVeryVeryVerbose);
  bslma::DefaultAllocatorGuard defaultAllocatorGuard(&da);
  ...
  bsltf::MoveState::Enum ecmnma =
      bslmf::IsSame<ELEMENT, bsltf::MoveCopyAllocTestType>::value ? e_NOT_MOVED
                                                                  : ecm;
  ...
  if (verbose)
    cout << "testSingleMovingPushesPops<" << NameOf<ELEMENT>() << ">()\n";

  if (verbose)
    cout << "\t1. Explicit Pushes and Pops\n";
  {
    enum { NUM_V = 4 };
    ...
    for (int tryPush = 0; tryPush < 2; ++tryPush) {
      ...
      ASSERTV(numPopValueAllocs, da.numBlocksTotal() - expDaNumBlocksTotal,
              0 == da.numBlocksTotal() - expDaNumBlocksTotal);

      if (verbose)
        cout << "\t\t'pushFront' && 'length'\n";
      {
        ASSERT(0 == u::myLength(X));
  ...
  if (verbose)
    cout << "\t3. Explicit Pushes and Pops, Alloc & Excep\n";
  {
    enum { NUM_V = 4 };
      ...
      cout << "\t\t'pushBack' && 'length', matching\n";
    {
      ...
      END_EXCEP_TEST_OBJ
      ASSERTV(numThrows,
              !PLAT_EXC || (expCreateAlloc ? 0 < numThrows : 0 == numThrows));
    ...
    if (verbose)
      cout << "\t\t'pushBack' && 'length', non-matching\n";
    {
      ASSERT(2 == u::myLength(X));
      ...
      END_EXCEP_TEST_OBJ
      ASSERTV(NameOf<ELEMENT>(), numThrows,
              !PLAT_EXC || (u::IsAllocating<ELEMENT>::value ? 0 < numThrows
                                                            : 0 == numThrows));
      ...
      cout << "\t\t'popBack' && 'length' -- matching\n";
    {
      ...
      END_EXCEP_TEST_OBJ
      ASSERTV(numThrows, eam, NameOf<ELEMENT>(),
              !PLAT_EXC || (u::IsAllocating<ELEMENT>::value &&
                            e_MOVED != eam) == (0 < numThrows));
      ASSERT(V[1] == eTa);
    ...
    if (verbose)
      cout << "\t\t'popBack' && 'length' -- non-matching\n";
    {
      ASSERT(0 == u::myLength(X));
      x.pushBack(V[0]);
      x.pushBack(V[1]);
    ...
    if (verbose)
      cout << "\t\t'pushFront' && 'length', matching\n";
    {
      ASSERT(0 == u::myLength(X));
    ...
    if (verbose)
      cout << "\t\t'pushFront' && 'length', non-matching\n";
    {
      ASSERT(2 == u::myLength(X));
    ...
    if (verbose)
      cout << "\t\t'popFront' && 'length' -- non-matching\n";
    {
      ASSERT(0 == u::myLength(X));
      x.pushFront(V[0]);
      x.pushFront(V[1]);
...
public:
  // CREATOR
  BufferedPopper(bdlcc::Deque<unsigned> *container, bool popFromFront,
                 bslma::Allocator *alloc)
      : d_container(*container), d_buffer(alloc), d_rand1To8(20),
        d_popFromFront(popFromFront), d_currentIndex(0)
...
public:
  // CREATOR
  MultiThreadedRangeTryPushTest(bdlcc::Deque<unsigned> *container, unsigned id,
                                bdlcc::Deque<unsigned> *endDeque)
      : d_container(*container), d_endDeque(*endDeque), d_hiId(id << ID_SHIFT),
        d_id(id)
  ...
  // 64K to the front.
  {
    ...
    for (int isForward = 0; isForward < 2; ++isForward) {
      ...
      if (veryVerbose)
        bsl::printf("Producer %u %s pass start\n", d_id,
      ...
      for (unsigned u = 0; u < PUSHES_PER_THREAD;) {
        ASSERTV(d_container.length(), d_container.length() <= HIGH_WATER_MARK);
        ...
        if (1 == left) {
          ...
          if (veryVerbose)
            bsl::printf("Producer %u pushed ~0\n", d_id);
          break;
        }
        ...
        const size_t numPushed =
            isForward ? d_container.tryPushBack(array + 0, array + len)
      ...
      ASSERTV(reinterpret_cast<void *>(visitFlags), d_id, 0x1ff == visitFlags);
      if (veryVerbose)
        bsl::printf("Producer %u %s pass finish\n", d_id,
                    isForward ? "forward" : "backward");
...
void testForcePushRemoveAllAux(const bool match, const bool moveFlag,
                               const bool pushFront, const u::VecType vecType) {
  enum {
    k_NUM_ELEMENTS = 10,
    k_WELL_BEHAVED = u::HasWellBehavedMove<ELEMENT>::value,
    k_IS_MOVE_AWARE = u::IsMoveAware<ELEMENT>::value,
    k_IS_MOVE_COPY = bsl::is_same<ELEMENT, bsltf::MoveCopyAllocTestType>::value
  };
  ...
  bslma::TestAllocator &va = match ? (u::e_BSL == vecType ? ta : da) : oa;

  bslma::DefaultAllocatorGuard defaultAllocatorGuard(&da);

  bdlcc::Deque<ELEMENT> mX(2, &ca);
  const bdlcc::Deque<ELEMENT> &X = mX;

  ASSERT(0 == X.length());

  bsltf::MoveState::Enum expMove = !u::IsMoveAware<ELEMENT>::value ? e_UNKNOWN
  ...
  case u::e_BSL: {
    ...
    if (PLAT_EXC) {
      ...
      ASSERTV(numThrows,
              !match || !moveFlag || !k_IS_MOVE_AWARE || numThrows <= 1);
    }
  ...
  if (u::e_BSL == vecType) {
    ...
    expMove = !u::IsMoveAware<ELEMENT>::value ? e_UNKNOWN
              : !match && k_IS_MOVE_COPY      ? e_NOT_MOVED
                                              : e_MOVED;
...
template <class ELEMENT> void testForcePushRemoveAll() {
  const char *name = NameOf<ELEMENT>();

  if (verbose)
    cout << "Force Push Remove All: " << name << endl;
    ...
    for (int moveFlag = 0; moveFlag < 2; ++moveFlag) {
      for (int pushFront = 0; pushFront < 2; ++pushFront) {
        for (int vi = u::e_BEGIN; vi < u::e_END; ++vi) {
          ...
          testForcePushRemoveAllAux<ELEMENT>(match, moveFlag, pushFront,
                                             vecType);
        }
      }
    }
...
public:
  MultiThreadedForcePushTest(Container *container, unsigned id)
      : d_container_p(container), d_id(id << 30), d_rand1To8(id)
  // Create a 'MultiThreadedForcePushTest' object that will access the
  ...
  // 64K to the front.
  {
    ...
    barrier.wait();
    if (veryVerbose)
      bsl::printf("Producer %u forward pass start\n", d_id >> 30);
    ...
    if (veryVerbose)
      bsl::printf("Producer %u forward pass finish\n", d_id >> 30);
    visitFlags = 0;
    barrier.wait();
    ASSERT(0 == d_container_p->length());
    barrier.wait();
    if (veryVerbose)
      bsl::printf("Producer %u backward pass start\n", d_id >> 30);
    ...
    ASSERTV(u::p(visitFlags), (d_id >> 30), 0xfe == visitFlags);
    if (veryVerbose)
      bsl::printf("Producer %u backward pass finish\n", d_id >> 30);
  }
};
...
struct FATHWMStressTest {
  ...
  FATHWMStressTest(Obj *container, bslmt::Barrier *barrier)
      : d_container_p(container), d_barrier_p(barrier)
...
enum { NUM_ITERATIONS = 2000, HIGH_WATER_MARK = 100, NUM_THREADS = 9 };
...
template <class VECTOR> class TestPopFront {
  ...
  static const u::VecType s_vecType =
      bsl::is_same<bsl::vector<ELEMENT>, VECTOR>::value ? u::e_BSL
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
...
public:
  TestPopFront(bdlcc::Deque<ELEMENT> *mX, bslma::TestAllocator *alloc)
      // Create a 'TestPopFront' object accessing the specified 'mX' and
      // using the specified 'alloc'.
      : d_mX_p(mX), d_alloc_p(alloc) {
  ...
  // values are as expected.
  {
    ...
    IntPtr maxVecSize = 0;
    int maxVecSizeAt = 0;
    bsltf::MoveState::Enum vms = !k_IS_MOVE_AWARE ? e_UNKNOWN : e_MOVED;
    bsltf::MoveState::Enum eam =
        !k_IS_MOVE_AWARE ? e_UNKNOWN : expAssignMoveState;
    ...
    if (veryVerbose) {
      cout << "    PopFront: max vecsize = " << maxVecSize << " at "
           << maxVecSizeAt << endl;
    }
  }
};

template <class VECTOR> class TestPopBack {
  typedef typename VECTOR::value_type ELEMENT;
  typedef bslalg::ConstructorProxy<VECTOR> ProxyVector;
...
public:
  TestPopBack(bdlcc::Deque<ELEMENT> *mX, bslma::TestAllocator *alloc)
  ...
  // are as expected.
  {
    ...
    while (expectedVal < 50) {
      ...
      for (int ii = 0; ii < s; ++ii) {
        const int d = u::getData(v[ii]);
        ASSERTV(s_vecType, expectedVal, d, expectedVal++ == d && "popBack");
        ASSERTV(vms, bsltf::getMovedInto(v[ii]),
                u::e_BSL != s_vecType || vms == bsltf::getMovedInto(v[ii]));
      }
      if (s > maxVecSize) {
    ...
    if (veryVerbose) {
      cout << "    PopBack: max vecsize = " << maxVecSize << " at "
           << maxVecSizeAt << endl;
    }
  }
};

template <class ELEMENT> void testMultiThreadedTryPop() {
  const char *name = NameOf<ELEMENT>();
  if (verbose)
    cout << "testMultiThreadedTryPop<" << name << ">()\n";
...

  {
    ...
    for (int run = u::e_BEGIN; run < u::e_END * 3; ++run) {
      ...
      if (veryVerbose)
        cout << "  " << vecType << "::vector<" << name << ">, run: " << run % 3
             << endl;
...
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
      case u::e_PMR: {
        TestPopFront<std::experimental::pmr::vector<ELEMENT>> frontFunc(&mX,
                                                                        &ta);
        bslmt::ThreadUtil::create(&handle, frontFunc);
      } break;
#endif
...
template <class ELEMENT> class HighWaterMarkFunctor {
  bdlcc::Deque<ELEMENT> *d_deque_p;
  bsls::TimeInterval d_timeout;

public:
  explicit HighWaterMarkFunctor(bdlcc::Deque<ELEMENT> *deque)
...
template <class ELEMENT> void highWaterMarkTest() {
  bslma::TestAllocator ta(veryVeryVeryVerbose);
  bslma::TestAllocator da(veryVeryVeryVerbose);
  bslma::DefaultAllocatorGuard defaultAllocatorGuard(&da);
...

  {
    ...
    if (veryVerbose)
      cout << "After join: " << ta.numBlocksInUse() << endl;
...
enum { VALID_VAL = 45, TERMINATE = 46 };
...
public:
  EmptyDequeFunctor(Obj *deque, bslmt::Barrier *barrier, int *status)
      ...
      : d_deque_p(deque), d_barrier_p(barrier), d_status_p(status) {
    ASSERT(deque);
    ASSERT(barrier);
    ASSERT(status);
...
template <class VECTOR> void testSingleThreadedTryPop() {
  typedef typename VECTOR::value_type ELEMENT;
  ...
  for (int ti = 0; ti < 2; ++ti) {
    ...
    bslma::TestAllocator &xa = u::e_BSL == vecType ? ta : (match ? da : ta);
    bslma::TestAllocator &va = u::e_BSL == vecType ? (match ? ta : oa) : da;

    bdlcc::Deque<ELEMENT> mX(&xa);
    const bdlcc::Deque<ELEMENT> &X = mX;
    bsl::deque<ELEMENT> mY(&xa);
    const bsl::deque<ELEMENT> &Y = mY;
    ProxyVector pv(&va);
    VECTOR &v = pv.object();
    ...
    if (veryVerbose)
      cout << "\tPopFront: " << (match ? "" : "no ") << "match, " << endl;
    ...
    BSLMA_TESTALLOCATOR_EXCEPTION_TEST_END
    ASSERTV(numThrows, NameOf<ELEMENT>(), match,
            !PLAT_EXC || (k_ALLOC && (!match || !k_IS_MOVE_AWARE ||
                                      !expAssignMove)) == (0 < numThrows));
    ASSERT(0 == sts);
    ...
    if (u::e_BSL == vecType) {
      ...
      ASSERTV(numThrows, NameOf<ELEMENT>(), match,
              !PLAT_EXC ||
                  (k_ALLOC && (!match || !k_IS_MOVE_AWARE)) == (0 < numThrows));
    }
    ...
    if (veryVerbose)
      cout << "\tPopBack:  " << (match ? "" : "no ") << "match, " << endl;
...
enum WorkerType { e_SRC_PUSHER, e_DIRECT_PUSHER, e_POPPER, e_TRANSFERRER };

enum { k_DST_HIGH_WATER_MARK = 100, k_SRC_HIGH_WATER_MARK = 200 };

enum {
  k_NUM_TO_SRC_PUSH = 2 * 1000,
  ...
  k_NUM_THREADS = k_NUM_SRC_PUSHERS + k_NUM_DIRECT_PUSHERS + k_NUM_POPPERS + 1
};
...
public:
  // CREATORS
  ProctorTestFunctor(Obj *dst_p, Obj *src_p, WorkerType workerType,
                     int workerIdx, bool backWard);
...
// CREATOR
ProctorTestFunctor::ProctorTestFunctor(Obj *dst_p, Obj *src_p,
                                       WorkerType workerType, int workerIdx,
                                       bool backWard)
    : d_dst_p(dst_p), d_src_p(src_p),
      d_workerType(static_cast<unsigned char>(workerType)),
      d_backWard(backWard), d_workerIdx(workerIdx * 1000 * 1000) {
...
void ProctorTestFunctor::transferrer() const {
  ...
  for (bsl::size_t toTransfer = k_NUM_TO_TRANSFER; toTransfer > 0;
       toTransfer -= thisTransfer) {
    ...
    if (thisTransfer < 10 && toTransfer > 10) {
      ...
      if (dstSize >= dstFull) {
        ...
        thisTransfer = bsl::min(dstFull - dstSize, thisTransfer);
    ...
    if (0 == thisTransfer) {
      ...
      iterator begin =
          d_backWard ? trimProctor->end() - thisTransfer : trimProctor->begin();
      iterator end = d_backWard ? trimProctor->end() : begin + thisTransfer;
...
public:
  // CREATORS
  TimedHWMRecordBack(bdlcc::Deque<AElement> *deque, bsls::TimeInterval timeout,
                     const AElement &value)
      : d_deque_p(deque), d_timeout(timeout), d_toBeInserted(value)
  ...
  {
    waitingFlag = 1;
    ASSERT(0 == d_deque_p->timedPushBack(d_toBeInserted, u::now() + d_timeout));
  }
};
...
public:
  // CREATORS
  TimedHWMRecordFront(bdlcc::Deque<AElement> *deque, bsls::TimeInterval timeout,
                      const AElement &value)
      : d_deque_p(deque), d_timeout(timeout), d_toBeInserted(value)
  // Create a test object accessing the specified '*deque', that will
  // push the specified 'value', with the specified 'timeout'.
  ...
  // Do the 'timedPushFront' with the values specified at construction.
  {
    waitingFlag = 1;
    ASSERT(0 ==
           d_deque_p->timedPushFront(d_toBeInserted, u::now() + d_timeout));
  }
};
...
public:
  // CREATORS
  StraightHWMFunctorBack(bdlcc::Deque<Element> *deque, const Element &value)
      : d_deque_p(deque), d_toBeInserted(value)
...
public:
  // CREATORS
  StraightHWMFunctorFront(bdlcc::Deque<Element> *deque, const Element &value)
      : d_deque_p(deque), d_toBeInserted(value)
  // Create a test object accessing the specified '*deque' with the
  // specified 'value'.
  {
...
public:
  // CREATORS
  TimedPopRecordBack(bdlcc::Deque<AElement> *deque, bslmt::Barrier *barrier,
                     bsls::TimeInterval timeout, const AElement &val)
      : d_deque_p(deque), d_barrier_p(barrier), d_timeout(timeout),
        d_timeoutFlag(0), d_waitingFlag(1), d_expected(val)
  // Create a test object that will access the specified '*deque' and
  // with the specified 'val' and the specified 'timeout' and block on
...
public:
  // CREATORS
  TimedPopRecordFront(bdlcc::Deque<AElement> *deque, bslmt::Barrier *barrier,
                      bsls::TimeInterval timeout, const AElement &value)
      : d_deque_p(deque), d_barrier_p(barrier), d_timeout(timeout),
        d_timeoutFlag(0), d_waitingFlag(1), d_expected(value)
  // Create a test object that will access the specified '*deque' and
  // with the specified 'val' and the specified 'timeout' and block on
...
extern "C" void *testClass4BackCaller(void *arg)
...
extern "C" void *testClass4FrontCaller(void *arg)
// Cast the specified 'arg' to a test object and call it.
{
...
enum PopMode { BY_VALUE, THROUGH_PTR };
...
public:
  // CREATORS
  PushPopRecordBack(bdlcc::Deque<Element> *deque, const Element &value,
                    PopMode popMode)
      : d_deque_p(deque), d_waitingFlag(0), d_expected(value),
        d_popMode(popMode)
...
public:
  // CREATORS
  PushPopRecordFront(bdlcc::Deque<Element> *deque, const Element &value,
                     PopMode popMode)
      : d_deque_p(deque), d_waitingFlag(0), d_expected(value),
        d_popMode(popMode)
...
extern "C" void *pushPopFunctionBack(void *arg)
...
extern "C" void *pushPopFunctionFront(void *arg)
// The specified 'arg' refers to a test object, call its specified
// 'callback'.
{
...
int main(int argc, char *argv[]) {
  ...
  case 28: {
...

    {
      ...
      while (numCompleted < k_NUM_THREADS) {
        ...
        ++numEvents;
        if (verbose) {
          cout << "[" << ev.d_workerId << "] " << ev.d_eventNumber << ". "
  ...
  case 27: {
    ...
    // Next, in 'main', define the number of consumer and producer threads
    // (these numbers must be equal).
    //..
    enum {
      k_NUM_CONSUMER_THREADS = 10,
    ...
    bslmt::ThreadUtil::Handle
        handles[k_NUM_CONSUMER_THREADS + k_NUM_PRODUCER_THREADS];
    //..
    // Now, spawn all the consumers and producers:
    //..
    int ti = 0, rc;
    while (ti < k_NUM_CONSUMER_THREADS) {
      rc = bslmt::ThreadUtil::create(&handles[ti++], ConsumerFunctor(&deque));
      ASSERT(0 == rc);
    }
    while (ti < k_NUM_CONSUMER_THREADS + k_NUM_PRODUCER_THREADS) {
      rc = bslmt::ThreadUtil::create(&handles[ti++], ProducerFunctor(&deque));
      ASSERT(0 == rc);
    }
    //..
    // Finally, join all the threads after they finish and confirm the container
    // is empty afterward:
    //..
    while (ti > 0) {
  ...
  case 26: {
    ...
    if (verbose)
      cout << "TESTING TIMED POP & TIMED PUSH FUNCTIONS -- MOVE SEMANTICS\n"
              "==========================================================\n";
  ...
  case 25: {
    ...
    if (verbose)
      cout << "SINGLE-THREADED TESTING SINGLE MOVING PUSHES, POPS\n"
              "==================================================\n";
  ...
  case 22: {
    ...
    if (verbose)
      cout << "TESTING TYPE TRAITS\n"
              "===================\n";

    BSLMF_ASSERT(bslma::UsesBslmaAllocator<Obj>::value);

    BSLMF_ASSERT(!bslalg::TypeTraitHasStlIterators::Metafunction<Obj>::value);
    BSLMF_ASSERT(!bslalg::TypeTraitBitwiseCopyable::Metafunction<Obj>::value);
    BSLMF_ASSERT(!bslalg::TypeTraitBitwiseMoveable::Metafunction<Obj>::value);
  ...
  case 20: {
    ...
    if (verbose)
      cout << "MULTITHREADED RANGE TRY PUSH TEST\n"
              "=================================\n";
    ...
    for (int ii = 0; ii < NUM_THREADS; ++ii) {
      ...
      int rc = bslmt::ThreadUtil::create(
          &handles[ii],
          MultiThreadedRangeTryPushTest(&container, ii, endDeques[ii]));
      ASSERT(0 == rc);
    }
    ...
    for (int isForward = 0; isForward < 2; ++isForward) {
      ...
      barrier.wait();
      if (veryVerbose)
        bsl::printf("Main thread, %s pass start\n",
                    isForward ? "forward" : "backward");
      ...
      if (veryVerbose)
        bsl::printf("Main thread, %s pass finish, numNulls: %u\n",
  ...
  case 19: {
    ...
    if (verbose)
      cout << "SINGLE-THREADED SINGLE ITEM FORCE PUSH TEST\n"
              "===========================================\n";
  ...
  case 18: {
    ...
    if (verbose)
      cout << "SINGLE-THREADED RANGE TRY PUSH TEST\n"
              "===================================\n";
    ...
    for (size_t hwm = 1; hwm < 10; ++hwm) {
      ...
      for (size_t numToPrePush = 0; numToPrePush < 4; ++numToPrePush) {
        for (size_t numToPush = numToPrePush; numToPush < 20; ++numToPush) {
          if (0 < numToPrePush) {
          ...
          const size_t expected =
              bsl::max(bsl::min(hwm, numToPush), numToPrePush);
  ...
  case 17: {
    ...
    if (verbose)
      cout << "MULTITHREADED FORCED PUSH TEST\n"
              "==============================\n";
    ...
    for (int ii = 0; ii < 4; ++ii) {
      int rc = bslmt::ThreadUtil::create(
          &handles[ii], MultiThreadedForcePushTest(&container, ii));
      ASSERT(0 == rc);
    }
    ...
    barrier.wait();
    if (veryVerbose)
      bsl::printf("Main thread, forward pass start\n");
    ...
    if (veryVerbose)
      bsl::printf("Main thread, forward pass finish\n");
    bsl::memset(expecteds, 0, sizeof(expecteds));
    barrier.wait();
    barrier.wait();
    if (veryVerbose)
      bsl::printf("Main thread, backward pass start\n");
    ...
    if (veryVerbose)
      bsl::printf("Main thread, backward pass finish\n");
  ...
  case 16: {
    ...
    if (verbose)
      cout << "STRESS-TEST HIGH WATER MARK WITH FORCING\n"
              "========================================\n";

    using namespace FORCE_AND_TRY_HIGH_WATER_MARK_STRESS_TEST;

    const int START_LENGTHS[] = {0, 1, 2, 5, 10, 15};
    enum { NUM_START_LENGTHS = sizeof START_LENGTHS / sizeof *START_LENGTHS };

    const size_t HIGH_WATER_MARK = 3;
    int timesFull = 0;
    int minTimesFull = 0;

    static const Element startArray[] = {0, 1, 2,  3,  4,  5,  6,  7,
    ...
    for (int move = 0; move < 2; ++move) {
      for (int ti = 0; ti < NUM_START_LENGTHS; ++ti) {
        ...
        Obj mX(startArray + 0, startArray + startLength, HIGH_WATER_MARK, &ta);
        const Obj &X = mX;
        bslmt::Barrier barrier(2);
        ...
        bslmt::ThreadUtil::Handle handle;
        bslmt::ThreadUtil::create(&handle, FATHWMStressTest(&mX, &barrier));
        ...
        if (startLength > 0) {
          ...
          ASSERT(1 == X.length());
          if (startLength >= 2) {
            mX.forcePushBack(startArray + 1, startArray + startLength - 1);
        ...
        if (startLength > 0) {
          ...
          ASSERT(1 == X.length());
          if (startLength >= 2) {
            mX.forcePushFront(startArray + 1, startArray + startLength - 1);
  ...
  case 15: {
    ...
    if (verbose)
      cout << "STRESS-TEST HIGH WATER MARK\n"
              "===========================\n";
    ...
    enum {
      NUM_HIGH_WATER_MARKS = sizeof HIGH_WATER_MARKS / sizeof *HIGH_WATER_MARKS
    };
  ...
  case 13: {
    ...
    for (int config = 0; config < 4; ++config) {
      ...
      for (numPushers = 1; numPushers <= NUM_THREADS; numPushers += 2) {
        ...
        for (int numPoppers = 1; numPoppers <= NUM_THREADS; numPoppers += 2) {
          if (veryVerbose) {
            cout << "----------------------------------------------\n";
            P_(backwards);
            P_(popThroughPtr);
  ...
  case 12: {
    ...
    if (verbose)
      cout << "MULTITHREADED TEST OF TRYPOPFRONT, TRYPOPBACK\n"
              "=============================================\n";
  ...
  case 11: {
    ...
    if (verbose)
      cout << "TEST HIGH WATER MARK BLOCKING\n"
              "=============================\n";
  ...
  case 10: {
    ...
    if (verbose)
      cout << "TEST BLOCKING ON EMPTY DEQUE\n"
              "============================\n";
  ...
  case 9: {
    ...
    if (verbose)
      cout << "TEST TRYPOPFRONT, TRYPOPBACK -- SINGLE THREAD\n"
              "=============================================\n";
  ...
  case 8: {
      ...
      cout << "bsl::vector\n";
    {
      ...
      BEGIN_EXCEP_TEST_OBJ_VEC(AElement, mX, buffer) { mX.removeAll(&buffer); }
...
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
    if (verbose)
      cout << "std::experimental::pmr::vector\n";
    {
      AObj mX(&ta);
  ...
  case 7: {
...

    {
      ...
      if (verbose)
        cout << "Single-Threaded ConstProctor Testing\n";
      {
        ...
        ConstProctor prB;
        const Obj::ConstProctor &PRB = prB;
        prB.load(&X);
        ASSERT(&*prB == pdx);
        ASSERT(&*PRB == pdx);
        ASSERT(!prB.isNull());
      ...
      if (verbose)
        cout << "Single-Threaded Proctor Testing\n";
      {
        Obj::Proctor pr(&mX);
        const Obj::Proctor &PR = pr;
    ...
    if (verbose)
      cout << "Multi-Threaded Proctor Testing\n";
    for (int backWard = 0; backWard < 2; ++backWard) {
      Obj dstDeque(TC::k_DST_HIGH_WATER_MARK, &ta);
      Obj srcDeque(TC::k_SRC_HIGH_WATER_MARK, &ta);

      const TC::ProctorTestFunctor popper(&dstDeque, &srcDeque, TC::e_POPPER, 0,
                                          backWard);
      const TC::ProctorTestFunctor transferrer(&dstDeque, &srcDeque,
                                               TC::e_TRANSFERRER, 0, backWard);
      ...
      for (ti = 1; ti <= TC::k_NUM_DIRECT_PUSHERS; ++ti) {
        const TC::ProctorTestFunctor directPusher(
            &dstDeque, &srcDeque, TC::e_DIRECT_PUSHER, ti, backWard);
        rc = bslmt::ThreadUtil::create(&threads[threadIdx++], directPusher);
        ASSERT(0 == rc);
      }

      for (; ti <= TC::k_NUM_DIRECT_PUSHERS + TC::k_NUM_SRC_PUSHERS; ++ti) {
        const TC::ProctorTestFunctor pusher(&dstDeque, &srcDeque,
                                            TC::e_SRC_PUSHER, ti, backWard);
  ...
  case 6: {
    ...
    if (verbose)
      cout << "\tWith 'timed{Pop,Push}Back'" << endl;
    ...
    for (size_t ti = 0; ti < NUM_VALUES; ++ti) {
      ...
      bslmt::ThreadUtil::Handle thread;
      bslmt::ThreadUtil::create(&thread, &TC::doTimedHWMRecordBack, &testObj);
    ...
    if (verbose)
      cout << "\tWith 'timed{Pop,Push}Front'" << endl;
    ...
    for (unsigned ti = 0; ti < NUM_VALUES; ++ti) {
      ...
      bslmt::ThreadUtil::Handle thread;
      bslmt::ThreadUtil::create(&thread, &TC::doTimedHWMRecordFront, &testObj);
    ...
    if (verbose)
      cout << "Single-threaded 'timedPop{Front,Back}' with exceptions\n";
    for (int ti = 0; ti < 2; ++ti) {
      bool doFront = ti;
  ...
  case 5: {
    ...
    if (verbose)
      cout << "TESTING PUSH FUNCTIONS WITH HIGH WATER MARK\n"
              "===========================================\n";
  ...
  case 4: {
    ...
    if (verbose)
      cout << "TESTING TIMED POP FUNCTIONS -- MT\n"
              "=================================\n";
    ...
    bslma::TestAllocator ta(veryVeryVeryVerbose);
    {
      ...
      bslmt::ThreadUtil::Handle thread;
      bslmt::ThreadUtil::create(&thread, &testClass4BackCaller, &testAObj);
      ...
      cout << "\tWith 'timedPopFront'" << endl;
    {
      ...
      bslmt::ThreadUtil::Handle thread;
      bslmt::ThreadUtil::create(&thread, &testClass4FrontCaller, &testAObj);
    ...
    if (verbose)
      cout << "\tEnsure wait time on timeout" << endl;
    {
  ...
  case 3: {
    ...
    if (verbose)
      cout << "TESTING PUSH AND POP FUNCTIONS IN MT\n"
              "====================================\n";

    if (verbose)
      cout << "\tWith 'popBack'" << endl;
    bslma::TestAllocator ta(veryVeryVeryVerbose);
    for (int ti = 0; ti < 2; ++ti) {
      const int T = 1 * MICRO_100TH_SEC; // in microseconds
      Obj x(&ta);
  ...
  case 2: {
    ...
    if (verbose)
      cout << "SINGLE-THREADED TESTING SINGLE PUSHES, POPS, AND LENGTH\n"
              "=======================================================\n";
      ...
      cout << "\t3. Explicit Pushes and Pops, Alloc & Excep\n";
    {
        ...
        cout << "\t\t'pushBack' && 'length'\n";
      {
        ...
        BEGIN_EXCEP_TEST_OBJ(AElement, x) { x.pushBack(V[0]); }
        END_EXCEP_TEST_OBJ

        ASSERT(1 == u::myLength(X));
        ASSERT(V[0] == u::myBack(X));

        BEGIN_EXCEP_TEST_OBJ(AElement, x) { x.pushBack(V[1]); }
        END_EXCEP_TEST_OBJ
        ...
        cout << "\t\t'popBack' && 'length'\n";
      {
        ...
        ASSERT(2 == u::myLength(X));
        AElement e(&ta);
        BEGIN_EXCEP_TEST_OBJ(AElement, x) { x.popBack(&e); }
        END_EXCEP_TEST_OBJ
        ASSERT(V[1] == e);
        ...
        cout << "\t\t'pushFront' && 'length'\n";
      {
        ...
        BEGIN_EXCEP_TEST_OBJ(AElement, x) { x.pushFront(V[2]); }
        END_EXCEP_TEST_OBJ

        ASSERT(1 == u::myLength(X));
        ASSERT(V[2] == u::myBack(X));

        BEGIN_EXCEP_TEST_OBJ(AElement, x) { x.pushFront(V[3]); }
        END_EXCEP_TEST_OBJ
        ...
        cout << "\t\t'popFront' && 'length'\n";
      {
        ...
        ASSERT(2 == u::myLength(X));
        AElement e(&ta);
        BEGIN_EXCEP_TEST_OBJ(AElement, x) { x.popFront(&e); }
        END_EXCEP_TEST_OBJ
        ASSERT(V[3] == e);
  ...
  case 1: {
    ...
    if (verbose)
      cout << "Exercising default c'tor and basic methods\n";
    {
      Obj x1(&ta);
    ...
    if (verbose)
      cout << "Exercising bsl::deque c'tor\n";
    {
    ...
    if (verbose)
      cout << "Exercising bdlcc::Deque with highWaterMark\n";
    {
      bsl::deque<Element> q1(&ta);
      const bsl::deque<Element> &Q1 = q1;
      const unsigned int HIGH_WATER_MARK = 4;

      Obj x1(Q1.begin(), Q1.end(), HIGH_WATER_MARK, &ta);
      const Obj &X1 = x1;
    ...
    if (verbose)
      cout << "Exercising proctor access\n";
    {
      bsl::deque<Element> q1(&ta);
      const bsl::deque<Element> &Q1 = q1;
      q1.push_back(VA);
      ...
      cout << "Exercising 'highWaterMark'\n";
    {
      ...
      ASSERT(bsl::numeric_limits<bsl::size_t>::max() == x.highWaterMark());
