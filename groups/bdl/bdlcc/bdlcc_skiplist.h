...
template <class KEY, class DATA> struct SkipList_Node {
...
struct SkipList_PoolUtil {
  ...
  static PoolManager *createPoolManager(int *objectSizes, int numLevels,
...
template <class KEY, class DATA> class SkipList_NodeCreationHelper {
...
public:
  // CREATORS
  SkipList_NodeCreationHelper(PoolManager *poolManager, Node *node,
...
private:
  ...
  template <class VECTOR, class VALUE_TYPE> class IsVector;
  class PairFactory;
  ...
  template <class VECTOR> int removeAllImp(VECTOR *removed);
  // Remove all items from this list, and append to the specified
  // 'removed' vector objects referring to the removed nodes.  Note that
  // the objects appended to 'removed' will be in ascending order by key
  // value.  Note that 'removed' may be 0, in which case removed nodes
  ...
  int updateNode(bool *newFrontFlag, Node *node, const KEY &newKey,
...
public:
  ...
  void addAtLevelRaw(Pair **result, int level, const KEY &key, const DATA &data,
                     bool *newFrontFlag = 0);
  ...
  int addAtLevelUniqueRaw(Pair **result, int level, const KEY &key,
                          const DATA &data, bool *newFrontFlag = 0);
  // Add the specified 'key' / 'data' pair to this list at the specified
  // 'level', and load into the specified 'result' a reference to the
  // pair in the list.  The 'result' reference must be released (using
  // 'releaseReferenceRaw') when it is no longer needed.  Load into the
  ...
  void addRaw(Pair **result, const KEY &key, const DATA &data,
              bool *newFrontFlag = 0);
  // Add the specified 'key' / 'data' pair to this list, and load into
  ...
  int addUniqueRaw(Pair **result, const KEY &key, const DATA &data,
                   bool *newFrontFlag = 0);
  // Add the specified 'key' / 'data' pair to this list, and load into
  // the specified 'result' a reference to the pair in the list.  The
  // 'result' reference must be released (using 'releaseReferenceRaw')
  ...
  void addAtLevelRawR(Pair **result, int level, const KEY &key,
                      const DATA &data, bool *newFrontFlag = 0);
  // Add the specified 'key' / 'data' pair to this list at the specified
  // 'level', and load into the specified 'result' a reference to the
  ...
  int addAtLevelUniqueRawR(Pair **result, int level, const KEY &key,
                           const DATA &data, bool *newFrontFlag = 0);
  // Add the specified 'key' / 'data' pair to this list at the specified
  // 'level', and load into the specified 'result' a reference to the
  // pair in the list.  Search for the correct position for 'key' from
  ...
  void addRawR(Pair **result, const KEY &key, const DATA &data,
               bool *newFrontFlag = 0);
  // Add the specified 'key' / 'data' pair to this list, and load into
  // the specified 'result' a reference to the pair in the list.  Search
  // for the correct position for 'key' from the back of the list (in
  ...
  int addUniqueRawR(Pair **result, const KEY &key, const DATA &data,
                    bool *newFrontFlag = 0);
  // Add the specified 'key' / 'data' pair to this list, and load into
  // the specified 'result' a reference to the pair in the list.  Search
  // for the correct position for 'key' from the back of the list (in
  ...
  int update(const Pair *reference, const KEY &newKey, bool *newFrontFlag = 0,
  ...
  int updateR(const Pair *reference, const KEY &newKey, bool *newFrontFlag = 0,
              bool allowDuplicates = true);
  // Assign the specified 'newKey' value to the pair identified by the
  // specified 'reference', moving the pair within the list as necessary.
  ...
  bsl::ostream &print(bsl::ostream &stream, int level = 0,
...
class SkipList<KEY, DATA>::IsVector {
  ...
  // 'VECTOR' is a 'bsl', 'std', or 'std::experimental::pmr'
  // 'vector<VALUE_TYPE>'.
...
template <class KEY, class DATA> class SkipList<KEY, DATA>::PairFactory {
public:
  // CREATORS
...
template <class KEY, class DATA> class SkipList<KEY, DATA>::PairHandleFactory {
  // DATA
  SkipList *d_list_p;
...
inline SkipList_DoubleLockGuard::SkipList_DoubleLockGuard(bslmt::Mutex *lock1,
                                                          bslmt::Mutex *lock2)
...
// CREATORS
template <class KEY, class DATA>
inline SkipListPairHandle<KEY, DATA>::SkipListPairHandle()
    : d_list_p(0), d_node_p(0) {}

template <class KEY, class DATA>
inline SkipListPairHandle<KEY, DATA>::SkipListPairHandle(
    SkipList<KEY, DATA> *list, Pair *reference)
    : d_list_p(list), d_node_p(reference) {}
...
// MANIPULATORS
template <class KEY, class DATA>
inline SkipListPairHandle<KEY, DATA> &
...
template <class KEY, class DATA>
inline void SkipListPairHandle<KEY, DATA>::release() {
  if (d_node_p) {
...
template <class KEY, class DATA>
inline void
SkipListPairHandle<KEY, DATA>::releaseReferenceRaw(SkipList<KEY, DATA> **list,
                                                   Pair **reference) {
  BSLS_ASSERT(list);
...
template <class KEY, class DATA>
inline void
SkipListPairHandle<KEY, DATA>::reset(const SkipList<KEY, DATA> *list,
                                     Pair *reference) {
  release();
...
// ACCESSORS
template <class KEY, class DATA>
inline DATA &SkipListPairHandle<KEY, DATA>::data() const {
...
template <class KEY, class DATA>
inline bool SkipListPairHandle<KEY, DATA>::isValid() const {
  return d_node_p != 0 && d_list_p != 0;
}

template <class KEY, class DATA>
inline const KEY &SkipListPairHandle<KEY, DATA>::key() const {
  BSLS_ASSERT_SAFE(isValid());
...
template <class KEY, class DATA>
inline bdlcc::SkipListPairHandle<KEY, DATA>::operator const bdlcc::SkipListPair<
    KEY, DATA> *() const {
  return d_node_p;
}
...
inline SkipList_NodeCreationHelper<KEY, DATA>::SkipList_NodeCreationHelper(
    PoolManager *poolManager, Node *node, bslma::Allocator *basicAllocator)
    : d_node_p(node), d_poolManager_p(poolManager), d_keyFlag(false),
...
template <class KEY, class DATA>
inline void
SkipList_NodeCreationHelper<KEY, DATA>::construct(const KEY &key,
                                                  const DATA &data) {
  BSLS_ASSERT(d_node_p);

  bslalg::ScalarPrimitives::copyConstruct(BSLS_UTIL_ADDRESSOF(d_node_p->d_key),
                                          key, d_allocator_p);
  d_keyFlag = true;

  bslalg::ScalarPrimitives::copyConstruct(BSLS_UTIL_ADDRESSOF(d_node_p->d_data),
                                          data, d_allocator_p);
...
// CREATORS
template <class KEY, class DATA>
inline SkipList<KEY, DATA>::PairFactory::PairFactory(SkipList *) {}

// ACCESSOR
template <class KEY, class DATA>
inline typename SkipList<KEY, DATA>::Pair *
...
// CREATORS
template <class KEY, class DATA>
inline SkipList<KEY, DATA>::PairHandleFactory::PairHandleFactory(SkipList *list)
    : d_list_p(list) {}

// ACCESSOR
template <class KEY, class DATA>
inline typename SkipList<KEY, DATA>::PairHandle
...
// PRIVATE CLASS METHODS
template <class KEY, class DATA>
inline DATA &SkipList<KEY, DATA>::data(const Pair *reference) {
  BSLS_ASSERT(reference);
...
template <class KEY, class DATA>
inline const KEY &SkipList<KEY, DATA>::key(const Pair *reference) {
  BSLS_ASSERT(reference);
...
template <class KEY, class DATA>
inline BSLS_KEYWORD_CONSTEXPR bsls::Types::IntPtr
SkipList<KEY, DATA>::offsetOfPtrs() {
...
template <class KEY, class DATA>
inline typename SkipList<KEY, DATA>::Node *
...
template <class KEY, class DATA>
void SkipList<KEY, DATA>::addNodeImpR(bool *newFrontFlag, Node *newNode,
                                      bool lock) {
...
template <class KEY, class DATA>
inline void SkipList<KEY, DATA>::addNodeR(bool *newFrontFlag, Node *newNode) {
...
template <class KEY, class DATA> void SkipList<KEY, DATA>::initialize() {
  typedef bsls::Types::IntPtr IntPtr;
  ...
  d_poolManager_p =
      PoolUtil::createPoolManager(nodeSizes, k_MAX_NUM_LEVELS, d_allocator_p);

  d_head_p = reinterpret_cast<Node *>(
      PoolUtil::allocate(d_poolManager_p, k_MAX_LEVEL));
...
template <class KEY, class DATA>
void SkipList<KEY, DATA>::insertImp(bool *newFrontFlag, Node *location[],
                                    Node *node) {
...
template <class KEY, class DATA>
void SkipList<KEY, DATA>::moveImp(bool *newFrontFlag, Node *location[],
                                  Node *node) {
  BSLS_ASSERT(location);
  BSLS_ASSERT(node);
...
template <class KEY, class DATA>
inline void SkipList<KEY, DATA>::releaseNode(Node *node) {
  BSLS_ASSERT(node);
...
int SkipList<KEY, DATA>::removeAllMaybeUnlock(VECTOR *removed, bool unlock) {
  ...
  typedef typename bsl::conditional<bsl::is_same<ValueType, Pair *>::value,
                                    PairFactory, PairHandleFactory>::type
      FactoryType;
...
template <class KEY, class DATA>
int SkipList<KEY, DATA>::updateNode(bool *newFrontFlag, Node *node,
                                    const KEY &newKey, bool allowDuplicates) {
  BSLS_ASSERT(node);
...
template <class KEY, class DATA>
int SkipList<KEY, DATA>::updateNodeR(bool *newFrontFlag, Node *node,
                                     const KEY &newKey, bool allowDuplicates) {
  BSLS_ASSERT(node);
...
// PRIVATE ACCESSORS
template <class KEY, class DATA>
SkipList_Node<KEY, DATA> *SkipList<KEY, DATA>::backNode() const {
  LockGuard guard(&d_lock);
...
template <class KEY, class DATA>
SkipList_Node<KEY, DATA> *
SkipList<KEY, DATA>::findNodeLowerBound(const KEY &key) const {
  Node *locator[k_MAX_NUM_LEVELS];
...
template <class KEY, class DATA>
SkipList_Node<KEY, DATA> *
SkipList<KEY, DATA>::findNodeUpperBound(const KEY &key) const {
  Node *locator[k_MAX_NUM_LEVELS];
...
template <class KEY, class DATA>
SkipList_Node<KEY, DATA> *
SkipList<KEY, DATA>::findNodeLowerBoundR(const KEY &key) const {
  Node *locator[k_MAX_NUM_LEVELS];
...
template <class KEY, class DATA>
SkipList_Node<KEY, DATA> *
SkipList<KEY, DATA>::findNodeUpperBoundR(const KEY &key) const {
  Node *locator[k_MAX_NUM_LEVELS];
...
template <class KEY, class DATA>
SkipList_Node<KEY, DATA> *SkipList<KEY, DATA>::nextNode(Node *node) const {
...
template <class KEY, class DATA>
SkipList_Node<KEY, DATA> *SkipList<KEY, DATA>::prevNode(Node *node) const {
  BSLS_ASSERT(node != d_head_p);
  BSLS_ASSERT(node != d_tail_p);
...
// CLASS METHODS
template <class KEY, class DATA>
inline int SkipList<KEY, DATA>::level(const Pair *reference) {
  BSLS_ASSERT(reference);
...
SkipList<KEY, DATA>::SkipList(bslma::Allocator *basicAllocator)
    : d_listLevel(0), d_length(0), d_poolManager_p(0),
      d_allocator_p(bslma::Default::allocator(basicAllocator)) {
  initialize();
}
...
template <class KEY, class DATA> SkipList<KEY, DATA>::~SkipList() {
...
// MANIPULATORS
template <class KEY, class DATA>
SkipList<KEY, DATA> &SkipList<KEY, DATA>::operator=(const SkipList &rhs) {
  if (&rhs == this) {
    return *this; // RETURN
  }
  ...
  for (Node *node = rhs.d_head_p->d_ptrs[0].d_next_p;
       node && node != rhs.d_tail_p; node = node->d_ptrs[0].d_next_p) {
    ++node->d_refCount;
    rhsElements.insert(rhsElements.end(), PairHandle())
        ->reset(&rhs, reinterpret_cast<Pair *>(node));
  }
  ...
  for (typename bsl::vector<PairHandle>::iterator it = rhsElements.begin();
    ...
    Node *node = allocateNode(d_rand.randomLevel(), it->key(), it->data());
...
template <class KEY, class DATA>
inline void SkipList<KEY, DATA>::releaseReferenceRaw(const Pair *reference) {
  Node *node = pairToNode(reference);
  releaseNode(node);
}
...
template <class KEY, class DATA>
inline void SkipList<KEY, DATA>::add(PairHandle *result, const KEY &key,
                                     const DATA &data, bool *newFrontFlag) {
  Pair *handle;
...
template <class KEY, class DATA>
inline void SkipList<KEY, DATA>::add(const KEY &key, const DATA &data,
                                     bool *newFrontFlag) {
  Pair **zeroPair = 0;
...
template <class KEY, class DATA>
inline void SkipList<KEY, DATA>::addAtLevelRaw(Pair **result, int level,
...
template <class KEY, class DATA>
int SkipList<KEY, DATA>::addAtLevelUniqueRaw(Pair **result, int level,
                                             const KEY &key, const DATA &data,
                                             bool *newFrontFlag) {
  Node *node = allocateNode(level, key, data);
  if (result) {
...
template <class KEY, class DATA>
inline void SkipList<KEY, DATA>::addRaw(Pair **result, const KEY &key,
                                        const DATA &data, bool *newFrontFlag) {
  addAtLevelRaw(result, d_rand.randomLevel(), key, data, newFrontFlag);
}

template <class KEY, class DATA>
inline int SkipList<KEY, DATA>::addUnique(PairHandle *result, const KEY &key,
                                          const DATA &data,
                                          bool *newFrontFlag) {
  Pair *handle;
...
template <class KEY, class DATA>
inline int SkipList<KEY, DATA>::addUnique(const KEY &key, const DATA &data,
                                          bool *newFrontFlag) {
...
template <class KEY, class DATA>
inline int SkipList<KEY, DATA>::addUniqueRaw(Pair **result, const KEY &key,
                                             const DATA &data,
                                             bool *newFrontFlag) {
...
template <class KEY, class DATA>
inline void
SkipList<KEY, DATA>::addAtLevelRawR(Pair **result, int level, const KEY &key,
                                    const DATA &data, bool *newFrontFlag) {
  Node *node = allocateNode(level, key, data);
  if (result) {
    ++node->d_refCount;
...
template <class KEY, class DATA>
int SkipList<KEY, DATA>::addAtLevelUniqueRawR(Pair **result, int level,
                                              const KEY &key, const DATA &data,
                                              bool *newFrontFlag) {
  Node *node = allocateNode(level, key, data);
  if (result) {
...
template <class KEY, class DATA>
inline void SkipList<KEY, DATA>::addR(PairHandle *result, const KEY &key,
                                      const DATA &data, bool *newFrontFlag) {
  Pair *handle;
...
template <class KEY, class DATA>
inline void SkipList<KEY, DATA>::addR(const KEY &key, const DATA &data,
                                      bool *newFrontFlag) {
  Pair **zeroPair = 0;
...
template <class KEY, class DATA>
inline void SkipList<KEY, DATA>::addRawR(Pair **result, const KEY &key,
                                         const DATA &data, bool *newFrontFlag) {
  addAtLevelRawR(result, d_rand.randomLevel(), key, data, newFrontFlag);
}

template <class KEY, class DATA>
int SkipList<KEY, DATA>::addUniqueR(PairHandle *result, const KEY &key,
                                    const DATA &data, bool *newFrontFlag) {
  Pair *handle;
...
template <class KEY, class DATA>
inline int SkipList<KEY, DATA>::addUniqueR(const KEY &key, const DATA &data,
                                           bool *newFrontFlag) {
  Pair **zeroPair = 0;
...
template <class KEY, class DATA>
inline int SkipList<KEY, DATA>::addUniqueRawR(Pair **result, const KEY &key,
                                              const DATA &data,
                                              bool *newFrontFlag) {
...
template <class KEY, class DATA>
inline int SkipList<KEY, DATA>::popFront(PairHandle *item) {
...
template <class KEY, class DATA>
inline int SkipList<KEY, DATA>::popFrontRaw(Pair **item) {
  Node *node = popFrontImp();
  if (!node) {
    return e_NOT_FOUND; // RETURN
  }
...
template <class KEY, class DATA>
inline int SkipList<KEY, DATA>::remove(const Pair *reference) {
  if (0 == reference) {
...
template <class KEY, class DATA> inline int SkipList<KEY, DATA>::removeAll() {
  return removeAllImp(static_cast<bsl::vector<PairHandle> *>(0));
}

template <class KEY, class DATA>
inline int SkipList<KEY, DATA>::removeAll(bsl::vector<PairHandle> *removed) {
  return removeAllImp(removed);
}

template <class KEY, class DATA>
inline int SkipList<KEY, DATA>::removeAll(std::vector<PairHandle> *removed) {
  return removeAllImp(removed);
}

#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
template <class KEY, class DATA>
inline int SkipList<KEY, DATA>::removeAll(
    std::experimental::pmr::vector<PairHandle> *removed) {
  return removeAllImp(removed);
}
#endif

template <class KEY, class DATA>
inline int SkipList<KEY, DATA>::removeAllRaw(bsl::vector<Pair *> *removed) {
  return removeAllImp(removed);
}

template <class KEY, class DATA>
inline int SkipList<KEY, DATA>::removeAllRaw(std::vector<Pair *> *removed) {
  return removeAllImp(removed);
}

#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
template <class KEY, class DATA>
inline int SkipList<KEY, DATA>::removeAllRaw(
    std::experimental::pmr::vector<Pair *> *removed) {
  return removeAllImp(removed);
}
#endif
...
template <class KEY, class DATA>
inline int SkipList<KEY, DATA>::update(const Pair *reference, const KEY &newKey,
                                       bool *newFrontFlag,
                                       bool allowDuplicates) {
  if (0 == reference) {
    return e_INVALID; // RETURN
...
template <class KEY, class DATA>
inline int SkipList<KEY, DATA>::updateR(const Pair *reference,
                                        const KEY &newKey, bool *newFrontFlag,
                                        bool allowDuplicates) {
  if (0 == reference) {
    return e_INVALID; // RETURN
  }
...
template <class KEY, class DATA>
inline int SkipList<KEY, DATA>::back(PairHandle *back) const {
...
template <class KEY, class DATA>
inline int SkipList<KEY, DATA>::backRaw(Pair **back) const {
...
template <class KEY, class DATA>
inline int SkipList<KEY, DATA>::front(PairHandle *front) const {
  BSLS_ASSERT(front);
...
template <class KEY, class DATA>
inline int SkipList<KEY, DATA>::frontRaw(Pair **front) const {
  BSLS_ASSERT(front);
...
template <class KEY, class DATA>
inline bool SkipList<KEY, DATA>::isEmpty() const {
  LockGuard guard(&d_lock);
...
template <class KEY, class DATA>
inline int SkipList<KEY, DATA>::length() const {
  LockGuard guard(&d_lock);
...
template <class KEY, class DATA>
bsl::ostream &SkipList<KEY, DATA>::print(bsl::ostream &stream, int level,
  ...
  if (0 <= spacesPerLevel) {
    ...
    for (Node *node = d_head_p->d_ptrs[0].d_next_p; node && node != d_tail_p;
      ...
      bdlb::PrintMethods::print(stream, node->d_key, levelPlus2,
                                spacesPerLevel);

      bdlb::Print::indent(stream, levelPlus2, spacesPerLevel);
      stream << "=>\n";

      bdlb::PrintMethods::print(stream, node->d_data, levelPlus2,
                                spacesPerLevel);
      bdlb::Print::indent(stream, levelPlus1, spacesPerLevel);
      stream << "]\n";
    }
...
template <class KEY, class DATA>
inline int SkipList<KEY, DATA>::find(PairHandle *item, const KEY &key) const {
  BSLS_ASSERT(item);
...
template <class KEY, class DATA>
inline int SkipList<KEY, DATA>::findRaw(Pair **item, const KEY &key) const {
  BSLS_ASSERT(item);
...
template <class KEY, class DATA>
inline int SkipList<KEY, DATA>::findR(PairHandle *item, const KEY &key) const {
  BSLS_ASSERT(item);
...
template <class KEY, class DATA>
inline int SkipList<KEY, DATA>::findRRaw(Pair **item, const KEY &key) const {
  BSLS_ASSERT(item);
...
template <class KEY, class DATA>
inline int SkipList<KEY, DATA>::findLowerBound(PairHandle *item,
                                               const KEY &key) const {
  BSLS_ASSERT(item);
...
template <class KEY, class DATA>
inline int SkipList<KEY, DATA>::findLowerBoundRaw(Pair **item,
                                                  const KEY &key) const {
  BSLS_ASSERT(item);
...
template <class KEY, class DATA>
inline int SkipList<KEY, DATA>::findLowerBoundR(PairHandle *item,
                                                const KEY &key) const {
  BSLS_ASSERT(item);
...
template <class KEY, class DATA>
inline int SkipList<KEY, DATA>::findLowerBoundRRaw(Pair **item,
                                                   const KEY &key) const {
  BSLS_ASSERT(item);
...
template <class KEY, class DATA>
inline int SkipList<KEY, DATA>::findUpperBound(PairHandle *item,
                                               const KEY &key) const {
  BSLS_ASSERT(item);
...
template <class KEY, class DATA>
inline int SkipList<KEY, DATA>::findUpperBoundRaw(Pair **item,
                                                  const KEY &key) const {
  BSLS_ASSERT(item);
...
template <class KEY, class DATA>
inline int SkipList<KEY, DATA>::findUpperBoundR(PairHandle *item,
                                                const KEY &key) const {
  BSLS_ASSERT(item);
...
template <class KEY, class DATA>
inline int SkipList<KEY, DATA>::findUpperBoundRRaw(Pair **item,
                                                   const KEY &key) const {
  BSLS_ASSERT(item);
...
// next, previous, & skip*
template <class KEY, class DATA>
inline int SkipList<KEY, DATA>::next(PairHandle *next,
...
template <class KEY, class DATA>
inline int SkipList<KEY, DATA>::nextRaw(Pair **next,
                                        const Pair *reference) const {
  BSLS_ASSERT(next);
  BSLS_ASSERT(reference);
...
template <class KEY, class DATA>
inline int SkipList<KEY, DATA>::previous(PairHandle *prevPair,
                                         const Pair *reference) const {
  if (0 == reference) {
    return e_INVALID; // RETURN
  }
...
template <class KEY, class DATA>
inline int SkipList<KEY, DATA>::previousRaw(Pair **prevPair,
                                            const Pair *reference) const {
...
template <class KEY, class DATA>
inline int SkipList<KEY, DATA>::skipBackward(PairHandle *item) const {
...
template <class KEY, class DATA>
inline int SkipList<KEY, DATA>::skipBackwardRaw(Pair **item) const {
  BSLS_ASSERT(item);
...
template <class KEY, class DATA>
inline int SkipList<KEY, DATA>::skipForward(PairHandle *item) const {
  BSLS_ASSERT(item->isValid());
...
template <class KEY, class DATA>
inline int SkipList<KEY, DATA>::skipForwardRaw(Pair **item) const {
  BSLS_ASSERT(item);
...
template <class KEY, class DATA>
inline bslma::Allocator *SkipList<KEY, DATA>::allocator() const {
...
bool bdlcc::operator==(const SkipList<KEY, DATA> &lhs,
  ...
  for (SkipList_Node<KEY, DATA> *lhsNode = lhs.d_head_p->d_ptrs[0].d_next_p,
    ...
    if (!lhsNode || !rhsNode || lhsNode == lhs.d_tail_p ||
        rhsNode == rhs.d_tail_p) {
...
template <class KEY, class DATA>
inline bsl::ostream &bdlcc::operator<<(bsl::ostream &stream,
