...
// item":
//..
enum { k_MAX_CONSUMER_THREADS = 10 };
...
void myConsumer(bdlcc::Queue<my_WorkRequest> *queue) {
  while (1) {
    ...
    if (item.d_type == my_WorkRequest::e_STOP)
      break;
...
void myProducer(int numThreads) {
  ...
  for (int i = 0; i < numThreads; ++i) {
    bslmt::ThreadUtil::create(&consumerHandles[i], myConsumerThread, &queue);
  }
...
//..
enum { k_MAX_CONSUMER_THREADS = 10, k_MAX_EVENT_TEXT = 80 };
...
void myWorker(int workerId, bdlcc::Queue<my_Event> *queue) {
  ...
  for (evnum = 0; evnum < NEVENTS; ++evnum) {
    my_Event ev = {my_Event::e_IN_PROGRESS, workerId, evnum,
                   "In-Progress Event"};
    queue->pushBack(ev);
  }

  my_Event ev = {my_Event::e_TASK_COMPLETE, workerId, evnum, "Task Complete"};
  queue->pushBack(ev);
}
//..
...
void myObserver() {
  ...
  for (int i = 0; i < NTHREADS; ++i) {
    ...
    bslmt::ThreadUtil::create(&workerHandles[i], myWorkerThread, &workerData);
  }
  int nStop = 0;
  while (nStop < NTHREADS) {
    my_Event ev = queue.popFront();
    bsl::cout << "[" << ev.d_workerId << "] " << ev.d_eventNumber << ". "
...
public:
  explicit TestPopFront(Obj *mX, bslma::Allocator *alloc)
      : d_mX(mX), d_maxVecSize(0), d_maxVecSizeAt(0), d_alloc_p(alloc)
  // c'tor
  {}
  ...
  //
  {
    ...
    while (expectedVal < 50) {
      ...
      for (int i = 0; i < s; ++i) {
        LOOP2_ASSERT(expectedVal, v[i], expectedVal++ == v[i] && "popFront");
      }
      if (s > d_maxVecSize) {
    ...
    if (verbose) {
      cout << "  PopFront: max vecsize = " << d_maxVecSize << " at "
           << d_maxVecSizeAt << endl;
    }
  }
};
...
public:
  explicit TestPopBack(Obj *mX, bslma::Allocator *alloc)
      : d_mX(mX), d_maxVecSize(0), d_maxVecSizeAt(0), d_alloc_p(alloc)
  // c'tor
  {}
  ...
  //
  {
    ...
    while (expectedVal < 50) {
      ...
      int s = static_cast<int>(v.size());
      for (int i = 0; i < s; ++i) {
        LOOP2_ASSERT(expectedVal, v[i], expectedVal++ == v[i] && "popBack");
      }
      if (s > d_maxVecSize) {
        d_maxVecSize = static_cast<int>(v.size());
        d_maxVecSizeAt = expectedVal;
    ...
    if (verbose) {
      cout << "  PopBack: max vecsize = " << d_maxVecSize << " at "
           << d_maxVecSizeAt << endl;
    }
  }
};
...
public:
  ...
  enum { k_VALID_VAL = 45, k_INVALID_VAL = 46 };
...
public:
  enum { k_VALID_VAL = 45, k_TERMINATE = 46 };
...
template <class VECTOR> void testTryPop() {
...
public:
  // CREATORS
  TestClass6(bdlcc::Queue<Element> *queue, MyBarrier *barrier,
             const Element &value)
      : d_queue_p(queue), d_barrier_p(barrier), d_stage(0), d_toBePopped(value),
        d_toBePushed(value)
  // c'tor
  {}
...
public:
  // CREATORS
  TestClass5back(bdlcc::Queue<Element> *queue, MyBarrier *barrier,
                 bsls::TimeInterval timeout, Element value)
      : d_queue_p(queue), d_barrier_p(barrier), d_timeout(timeout),
        d_timeoutFlag(0), d_waitingFlag(1), d_toBeInserted(value)
  //
  {}
  ...
  //
  {
    ...
    d_barrier_p->wait();
    d_timeoutFlag =
        d_queue_p->timedPushBack(d_toBeInserted, u::now() + d_timeout);
    d_waitingFlag = 0;
...
public:
  // CREATORS
  TestClass5front(bdlcc::Queue<Element> *queue, MyBarrier *barrier,
                  bsls::TimeInterval timeout, Element value)
      : d_queue_p(queue), d_barrier_p(barrier), d_timeout(timeout),
        d_timeoutFlag(0), d_waitingFlag(1), d_toBeInserted(value)
  //
  ...
  //
  {
    ...
    d_barrier_p->wait();
    d_timeoutFlag =
        d_queue_p->timedPushFront(d_toBeInserted, u::now() + d_timeout);
    d_waitingFlag = 0;
...
extern "C" void *test5back(void *arg) {
...
extern "C" void *test5front(void *arg) {
  using namespace QUEUE_TEST_CASE_5;
...
public:
  ...
  TestClass4back(bdlcc::Queue<Element> *queue, const Element &value)
      : d_queue_p(queue), d_waitingFlag(0), d_toBeInserted(value)
  //
  {}
...
extern "C" void *test4back(void *arg) {
...
extern "C" void *test4front(void *arg) {
  using namespace QUEUE_TEST_CASE_4;
...
public:
  // CREATORS
  TestClass3back(bdlcc::Queue<Element> *queue, MyBarrier *barrier,
                 bsls::TimeInterval timeout, Element val)
      : d_queue_p(queue), d_barrier_p(barrier), d_timeout(timeout),
...
public:
  // CREATORS
  TestClass3front(bdlcc::Queue<Element> *queue, MyBarrier *barrier,
                  bsls::TimeInterval timeout, Element value)
      : d_queue_p(queue), d_barrier_p(barrier), d_timeout(timeout),
  ...
  //
  {
    ...
    d_waitingFlag = 1;
    d_barrier_p->wait();
    d_timeoutFlag = d_queue_p->timedPopFront(&result, u::now() + d_timeout);
    d_waitingFlag = 0;
...
struct TestStruct3 {
  ...
  //
  {
    ...
    int sts = d_queue_p->timedPopFront(&result, start + d_timeout);
    bsls::TimeInterval end = u::now();
    ASSERT(0 != sts);
    ASSERT(end >= start + d_timeout);

    start = u::now();
    sts = d_queue_p->timedPopBack(&result, start + d_timeout);
    end = u::now();
    ASSERT(0 != sts);
    ASSERT(end >= start + d_timeout);
  }
...
public:
  ...
  TestClass2back(bdlcc::Queue<Element> *queue, const Element &value)
      : d_queue_p(queue), d_waitingFlag(0), d_expected(value)
  // c'tor
  {}
...
extern "C" void *test2back(void *arg) {
...
extern "C" void *test2front(void *arg) {
  using namespace QUEUE_TEST_CASE_2;
...
void runtest(int numIterations, int numPushers, int numPoppers) {
  ...
  tg.addThreads(bdlf::BindUtil::bind(&pusherThread, &control), numPushers);
  tg.addThreads(bdlf::BindUtil::bind(&popperThread, &control), numPoppers);
...
int main(int argc, char *argv[]) {
  ...
  case 20: {
    ...
    if (verbose)
      cout << endl
           ...
           << "===================================" << endl;
    enum { k_NUM_THREADS = 4, k_NUM_ITERATIONS = 5000 };
  ...
  case 19: {
    ...
    if (verbose)
      cout << endl
           << "sequence constraint test using popFront(TYPE*)" << endl
           << "========================" << endl;
    enum { k_NUM_THREADS = 4, k_NUM_ITERATIONS = 5000 };
  ...
  case 14: {
           ...
           << "=======================" << endl;
    {
      ...
      for (int i = 0; i < NTHREADS; ++i) {
        ...
        bslmt::ThreadUtil::create(&workerHandles[i], myWorkerThread, &wdata[i]);
      }
      int n_Stop = 0;
  ...
  case 13: {
    ...
    if (verbose)
      cout << "tryPopFront, tryPopBack test\n";

    using namespace QUEUE_TEST_CASE_13;

    Element waitTable[] = {4, 5, 6, 7, 15, 20, 30, 31, 32, 33, 34};
    int waitTableLength =
        static_cast<int>(sizeof waitTable / sizeof *waitTable);
    ...
    for (int ti = 0; ti < 3 * u::e_NUM_VEC_TYPES; ++ti) {
      ...
      case u::e_PMR: {
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
        TestPopFront<std::experimental::pmr::vector<Element>> testFront(&mX,
                                                                        &ta);
        bslmt::ThreadUtil::create(&handle, testFront);
#else
        ASSERT(0 && "PMR");
      ...
      for (Element e = 0; e < 50; ++e) {
        if (waitTableEnd != bsl::find(waitTableBegin, waitTableEnd, e)) {
          if (veryVeryVerbose) {
  ...
  case 12: {
    ...
    if (verbose) {
      cout << endl
           << "TEST BLOCKING ON HIGH WATERMARK" << endl
           << "===============================" << endl;
    }
  ...
  case 11: {
    ...
    if (verbose) {
      cout << endl
           << "TEST BLOCKING ON EMPTY QUEUE" << endl
           << "============================" << endl;
    }
        ...
        if (-1 != HIGH_WATER_MARK) {
          ...
          if (elapsed < T4) {
            ...
            bsl::cout << "*** Warning:  (line " << __LINE__ << ") "
                      ...
                      << "to run properly ('timedPushBack')" << endl;
          }
        } else {
          barrier.wait();
      ...
      bslma::TestAllocator ta(veryVeryVeryVerbose);
      {
        ...
        if (-1 != HIGH_WATER_MARK) {
          ...
          if (elapsed < T4) {
            ...
            bsl::cout << "*** Warning:  (line " << __LINE__ << ") "
                      << "high load delays prevented test case 5 "
                      << "to run properly ('timedPushFront')" << endl;
          }
        } else {
          barrier.wait();
  ...
  case 5: {
    ...
    for (int i = 0; i < NUM_VALUES; ++i) {
      bslma::TestAllocator ta(veryVeryVeryVerbose);
      {
        ...
        const int HIGH_WATER_MARK = VALUES[i].d_highWaterMark;
        const int INSERTIONS = (HIGH_WATER_MARK < 0) ? 10 : HIGH_WATER_MARK;
        if (veryVerbose) {
          P_(LINE);
          P_(HIGH_WATER_MARK);
          P(INSERTIONS);
  ...
  case 2: {
    ...
    if (verbose)
      cout << "TESTING PUSH, POP, and LENGTH\n"
              "=============================\n";
  ...
  case 1: {
    ...
    {
      if (verbose)
        cout << "Exercising InitCapacity c'tor" << endl;
...

    {
      ...
      bdlc::Queue<Element> q1(&ta);
      const bdlc::Queue<Element> &Q1 = q1;
    ...
    {
      if (verbose)
        cout << "Exercising mutex, condition, and queue access" << endl;
  ...
  case -1: {
    ...
    enum { k_WATERMARK_OFF, k_WATERMARK_ON, k_DONE };
  ...
  case -2: {
    ...
    if (verbose)
      cout << endl << "STRESS TEST -2" << endl << "==============" << endl;
    enum { k_NUM_THREADS = 6, k_NUM_ITERATIONS = 1000000 };
  ...
  case -3: {
    ...
    if (verbose)
      cout << endl << "STRESS TEST -3" << endl << "==============" << endl;
    enum { k_NUM_THREADS = 6, k_NUM_ITERATIONS = 1000000 };
