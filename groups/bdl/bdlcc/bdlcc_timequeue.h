...
template <class DATA> class TimeQueueItem;

// ===============
// class TimeQueue
// ===============
template <class DATA> class TimeQueue {
...
private:
  ...
  struct Node {
    ...
    // CREATORS
    Node()
        : d_index(0), d_key(0), d_prev_p(0), d_next_p(0)
    // Create a 'Node' having a time value of 0.
    {}

    explicit Node(const bsls::TimeInterval &time)
        : d_index(0), d_time(time), d_key(0), d_prev_p(0), d_next_p(0)
  ...
  void popLEImp(const bsls::TimeInterval &time, VECTOR *buffer,
                int *newLength = 0, bsls::TimeInterval *newMinTime = 0);
  ...
  template <class VECTOR>
  void popLEImp(const bsls::TimeInterval &time, int maxTimers, VECTOR *buffer,
                int *newLength = 0, bsls::TimeInterval *newMinTime = 0);
...
public:
  ...
  explicit TimeQueue(bool poolTimerMemory,
                     bslma::Allocator *basicAllocator = 0);
  TimeQueue(int numIndexBits, bool poolTimerMemory,
            bslma::Allocator *basicAllocator = 0);
  ...
  // MANIPULATORS
  Handle add(const bsls::TimeInterval &time, const DATA &data,
             int *isNewTop = 0, int *newLength = 0);
  Handle add(const bsls::TimeInterval &time, const DATA &data, const Key &key,
             int *isNewTop = 0, int *newLength = 0);
  ...
  Handle add(const TimeQueueItem<DATA> &item, int *isNewTop = 0,
             int *newLength = 0);
  ...
  int popFront(TimeQueueItem<DATA> *buffer = 0, int *newLength = 0,
  ...
  void popLE(const bsls::TimeInterval &time,
             bsl::vector<TimeQueueItem<DATA>> *buffer, int *newLength = 0,
             bsls::TimeInterval *newMinTime = 0);
  void popLE(const bsls::TimeInterval &time,
             std::vector<TimeQueueItem<DATA>> *buffer, int *newLength = 0,
             bsls::TimeInterval *newMinTime = 0);
  ...
  int remove(Handle handle, int *newLength = 0,
             bsls::TimeInterval *newMinTime = 0, TimeQueueItem<DATA> *item = 0);
  int remove(Handle handle, const Key &key, int *newLength = 0,
             bsls::TimeInterval *newMinTime = 0, TimeQueueItem<DATA> *item = 0);
  ...
  int update(Handle handle, const bsls::TimeInterval &newTime,
             int *isNewTop = 0);
  int update(Handle handle, const Key &key, const bsls::TimeInterval &newTime,
             int *isNewTop = 0);
...
public:
  // CREATORS
  explicit TimeQueueItem(bslma::Allocator *basicAllocator = 0);
  // Create an empty time queue item.  Optionally specify a
  // 'basicAllocator' used to supply memory.  If 'basicAllocator' is
  // zero, then use the currently installed default allocator.

  TimeQueueItem(const bsls::TimeInterval &time, const DATA &data, Handle handle,
                bslma::Allocator *basicAllocator = 0);
  ...
  TimeQueueItem(const bsls::TimeInterval &time, const DATA &data, Handle handle,
                const Key &key, bslma::Allocator *basicAllocator = 0);
  // Create time queue item holding a copy of the specified 'data', with
...
template <class DATA> template <class VECTOR> struct TimeQueue<DATA>::IsVector {
...
// PRIVATE MANIPULATORS
template <class DATA> inline void TimeQueue<DATA>::freeNode(Node *node) {
  node->d_index =
      ((node->d_index + d_indexIterationInc) & d_indexIterationMask) |
      (node->d_index & d_indexMask);
...
template <class DATA>
template <class VECTOR>
void TimeQueue<DATA>::popLEImp(const bsls::TimeInterval &time, VECTOR *buffer,
                               int *newLength, bsls::TimeInterval *newMinTime) {
  ...
  while (d_map.end() != it && it->first <= time) {
    ...
    do {
      if (buffer) {
        buffer->push_back(TimeQueueItem<DATA>(it->first, node->d_data.object(),
                                              node->d_index, node->d_key,
                                              d_allocator_p));
      }
      freeNode(node);
      node = node->d_next_p;
...
template <class DATA>
template <class VECTOR>
void TimeQueue<DATA>::popLEImp(const bsls::TimeInterval &time, int maxTimers,
                               VECTOR *buffer, int *newLength,
...
template <class DATA> void TimeQueue<DATA>::putFreeNode(Node *node) {
...
template <class DATA> void TimeQueue<DATA>::putFreeNodeList(Node *begin) {
  if (begin) {
    ...
    while (nextFreeNode != d_nextFreeNode_p.testAndSwap(nextFreeNode, begin)) {
      nextFreeNode = d_nextFreeNode_p;
      end->d_next_p = nextFreeNode;
    }
  }
...
TimeQueue<DATA>::TimeQueue(bslma::Allocator *basicAllocator)
    : d_indexMask((1 << k_NUM_INDEX_BITS_DEFAULT) - 1),
      ...
      d_indexIterationInc(d_indexMask + 1), d_nodeArray(basicAllocator),
      d_nextFreeNode_p(0), d_map(basicAllocator), d_length(0),
...
TimeQueue<DATA>::TimeQueue(int numIndexBits, bslma::Allocator *basicAllocator)
    : d_indexMask((1 << numIndexBits) - 1), d_indexIterationMask(~d_indexMask),
      d_indexIterationInc(d_indexMask + 1), d_nodeArray(basicAllocator),
      d_nextFreeNode_p(0), d_map(basicAllocator), d_length(0),
      d_allocator_p(bslma::Default::allocator(basicAllocator)) {
...
template <class DATA>
TimeQueue<DATA>::TimeQueue(int numIndexBits, bool poolTimerMemory,
                           bslma::Allocator *basicAllocator)
    : d_indexMask((1 << numIndexBits) - 1), d_indexIterationMask(~d_indexMask),
      d_indexIterationInc(d_indexMask + 1), d_nodeArray(basicAllocator),
      d_nextFreeNode_p(0), d_map(basicAllocator), d_length(0),
...
template <class DATA> TimeQueue<DATA>::~TimeQueue() {
  removeAll();
...
// MANIPULATORS
template <class DATA>
inline typename TimeQueue<DATA>::Handle
TimeQueue<DATA>::add(const bsls::TimeInterval &time, const DATA &data,
                     int *isNewTop, int *newLength) {
  return add(time, data, Key(0), isNewTop, newLength);
}

template <class DATA>
typename TimeQueue<DATA>::Handle
TimeQueue<DATA>::add(const bsls::TimeInterval &time, const DATA &data,
                     const Key &key, int *isNewTop, int *newLength)

{
  ...
  if (d_nextFreeNode_p) {
    ...
    node->d_index = static_cast<int>(d_nodeArray.size()) | d_indexIterationInc;
  }
  node->d_time = time;
  node->d_key = key;
  bslalg::ScalarPrimitives::copyConstruct(&node->d_data.object(), data,
                                          d_allocator_p);
...
template <class DATA>
inline typename TimeQueue<DATA>::Handle
TimeQueue<DATA>::add(const TimeQueueItem<DATA> &item, int *isNewTop,
                     int *newLength) {
  return add(item.time(), item.data(), item.key(), isNewTop, newLength);
}

template <class DATA>
int TimeQueue<DATA>::popFront(TimeQueueItem<DATA> *buffer, int *newLength,
                              bsls::TimeInterval *newMinTime) {
  bslmt::LockGuard<bslmt::Mutex> lock(&d_mutex);
  MapIter it = d_map.begin();
...
template <class DATA>
inline void TimeQueue<DATA>::popLE(const bsls::TimeInterval &time) {
...
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
template <class DATA>
inline void TimeQueue<DATA>::popLE(
    const bsls::TimeInterval &time,
    std::experimental::pmr::vector<TimeQueueItem<DATA>> *buffer, int *newLength,
    bsls::TimeInterval *newMinTime) {
  popLEImp(time, buffer, newLength, newMinTime);
}
#endif
...
inline void TimeQueue<DATA>::popLE(const bsls::TimeInterval &time,
                                   int maxTimers) {
  popLEImp(time, maxTimers, static_cast<bsl::vector<TimeQueueItem<DATA>> *>(0));
}

template <class DATA>
inline void
TimeQueue<DATA>::popLE(const bsls::TimeInterval &time, int maxTimers,
                       bsl::vector<TimeQueueItem<DATA>> *buffer, int *newLength,
                       bsls::TimeInterval *newMinTime) {
...
inline void
TimeQueue<DATA>::popLE(const bsls::TimeInterval &time, int maxTimers,
                       std::vector<TimeQueueItem<DATA>> *buffer, int *newLength,
                       bsls::TimeInterval *newMinTime) {
  popLEImp(time, maxTimers, buffer, newLength, newMinTime);
}
...
template <class DATA>
inline void TimeQueue<DATA>::popLE(
    const bsls::TimeInterval &time, int maxTimers,
    std::experimental::pmr::vector<TimeQueueItem<DATA>> *buffer, int *newLength,
    bsls::TimeInterval *newMinTime) {
  popLEImp(time, maxTimers, buffer, newLength, newMinTime);
}
...
template <class DATA>
inline int TimeQueue<DATA>::remove(typename TimeQueue<DATA>::Handle handle,
                                   int *newLength,
                                   bsls::TimeInterval *newMinTime,
...
template <class DATA>
inline int TimeQueue<DATA>::update(typename TimeQueue<DATA>::Handle handle,
...
int TimeQueue<DATA>::update(typename TimeQueue<DATA>::Handle handle,
                            const Key &key, const bsls::TimeInterval &newTime,
                            int *isNewTop) {
  bslmt::LockGuard<bslmt::Mutex> lock(&d_mutex);
  int index = ((int)handle & d_indexMask) - 1;
...
// ACCESSORS
template <class DATA> inline int TimeQueue<DATA>::length() const {
  return d_length;
}

template <class DATA>
inline bool TimeQueue<DATA>::isRegisteredHandle(
...
template <class DATA>
inline bool
TimeQueue<DATA>::isRegisteredHandle(typename TimeQueue<DATA>::Handle handle,
                                    const Key &key) const {
  bslmt::LockGuard<bslmt::Mutex> lock(&d_mutex);
...
template <class DATA>
inline int TimeQueue<DATA>::minTime(bsls::TimeInterval *buffer) const {
  bslmt::LockGuard<bslmt::Mutex> lock(&d_mutex);
...
template <class DATA>
inline int TimeQueue<DATA>::countLE(const bsls::TimeInterval &time) const {
  int count = 0;

  bslmt::LockGuard<bslmt::Mutex> lock(&d_mutex);

  for (MapCIter it = d_map.cbegin(); it != d_map.cend() && it->first <= time;
       ++it) {
    ...
    do {
      ...
      BSLMF_ASSERT((1 << k_NUM_INDEX_BITS_MAX) - 1 < INT_MAX);
...
template <class DATA>
TimeQueueItem<DATA>::TimeQueueItem(TimeQueueItem<DATA> const &original,
                                   bslma::Allocator *basicAllocator)
    : d_time(original.d_time)
      // require that 'd_data' be default-constructible, hopefully at no cost
      ,
      d_handle(original.d_handle), d_key(original.d_key) {
  bslma::DestructionUtil::destroy(&d_data);
  bslalg::ScalarPrimitives::copyConstruct(&d_data, original.d_data,
                                          basicAllocator);
}

template <class DATA>
TimeQueueItem<DATA>::TimeQueueItem(const bsls::TimeInterval &time,
                                   const DATA &data, Handle handle,
                                   bslma::Allocator *basicAllocator)
    : d_time(time)
      // require that 'd_data' be default-constructible, hopefully at no cost
      ,
      d_handle(handle), d_key(0) {
  bslma::DestructionUtil::destroy(&d_data);
  bslalg::ScalarPrimitives::copyConstruct(&d_data, data, basicAllocator);
}
...
// MANIPULATORS
template <class DATA>
inline TimeQueueItem<DATA> &
TimeQueueItem<DATA>::operator=(const TimeQueueItem<DATA> &rhs) {
  d_time = rhs.d_time;
  d_data = rhs.d_data;
...
template <class DATA> inline bsls::TimeInterval &TimeQueueItem<DATA>::time() {
  return d_time;
}

template <class DATA> inline DATA &TimeQueueItem<DATA>::data() {
  return d_data;
}
} // namespace bdlcc
...
template <class DATA>
inline typename TimeQueueItem<DATA>::Key &TimeQueueItem<DATA>::key() {
  return d_key;
}

// ACCESSORS
template <class DATA>
inline const bsls::TimeInterval &TimeQueueItem<DATA>::time() const {
  return d_time;
}

template <class DATA> inline const DATA &TimeQueueItem<DATA>::data() const {
  return d_data;
}
} // namespace bdlcc
...
template <class DATA>
inline const typename TimeQueueItem<DATA>::Key &
