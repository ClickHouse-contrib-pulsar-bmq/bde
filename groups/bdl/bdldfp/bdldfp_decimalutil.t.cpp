...
void parseDecimal(unsigned long long *mantissa, int *exponent,
  ...
  bsl::string::const_iterator pos = bsl::find(input.begin(), input.end(), '.');
...
unsigned int split(bsl::vector<bsl::string> &strs, const bsl::string &input,
                   char ch) {
  ...
  bsl::string::const_iterator pos = bsl::find(input.begin(), input.end(), ch);
  ...
  // Add the last one
  strs.push_back(
      bsl::string(initialPos, bsl::min(pos, input.end()) - initialPos + 1));
...
const int numMantissas =
    static_cast<int>(sizeof(mantissas) / sizeof(*mantissas));

const int exps[] = {
    -321, -129, -23, -10, -7, -2, 0, 2, 7, 10, 23, 129, 321,
};
...
template <class EXPECT, class RECEIVED> void checkType(const RECEIVED &) {
  ASSERT(typeid(EXPECT) == typeid(RECEIVED));
}

template <class TYPE> struct NumberMaker;

template <> struct NumberMaker<BDEC::Decimal32> {
...
template <int Size> struct BufferBuf : bsl::streambuf {
  BufferBuf() { reset(); }
  const char *str() {
    *this->pptr() = 0;
    return this->pbase();
...
int main(int argc, char *argv[]) {
  ...
  case 15: {
    ...
    if (verbose)
      cout << endl << "Test Decimal32" << endl << "--------------" << endl;
...
#define DEC(X) BDLDFP_DECIMAL_DF(X)

    {
      ...
      int len = Util::format(buffer, k_BUFFER_SIZE, VALUE);
...

      {
        ...
        const Config MAX_DIGIT_CONFIG(bsl::numeric_limits<Type>::digits10 - 1);
        len = Util::format(buffer, k_BUFFER_SIZE, VALUE, MAX_DIGIT_CONFIG);

        const bsl::string RESULT(buffer, len, pa);
        LOOP3_ASSERT(L_, RESULT, EXPECTED, RESULT == EXPECTED);
        LOOP3_ASSERT(L_, RESULT, DEFAULT, RESULT == DEFAULT);
      }
      {
        ...
        len = Util::format(buffer, k_BUFFER_SIZE, VALUE, CONFIG);
...
#undef DEC
    if (verbose)
      cout << endl << "Test Decimal64" << endl << "--------------" << endl;
#define DEC(X) BDLDFP_DECIMAL_DD(X)
    {
      ...
      const bsl::string DEFAULT(buffer, len, pa);
      {
        ...
        const Config MAX_DAGIT_CONFIG(bsl::numeric_limits<Type>::digits10 - 1);
        len = Util::format(buffer, k_BUFFER_SIZE, VALUE, MAX_DAGIT_CONFIG);
...
#undef DEC
    ...
    if (verbose)
      cout << endl << "Test Decimal128" << endl << "---------------" << endl;
...
#define DEC(X) BDLDFP_DECIMAL_DL(X)
...

    {
...

      {
        // Test that 'format' produces the same resultant string
        // when invoked with 'MAX_DAGIT_CONFIG' configuration.
        const bsl::string EXPECTED("1.234567890123456789012345678901234", pa);
      ...
      }
      {
        ...
        // distinct from the one provided by default configuration
        // object.
        const bsl::string EXPECTED("1.234567890123456789012345678901234e+00",
                                   pa);
...
#define DEC(X) BDLDFP_DECIMAL_DF(X)
      ...
      bsl::cout << "\nDecimal32" << "\n---------" << bsl::endl;
    {
      ...
      }
      {
        struct {
          ...
          Obj d_expected;
            ...
            // LINE |     X     | EXPECTED
            //-----------------------------------
            {L_, DEC(1.0), ZERO_P},  {L_, DEC(12.0), DEC(2.484907)},
            {L_, DEC(-11.0), NAN_N}, {L_, NAN_P, NAN_P},
            {L_, NAN_N, NAN_N},      {L_, INF_P, INF_P},
            {L_, INF_N, NAN_N},      {L_, ZERO_P, INF_N},
            {L_, ZERO_N, INF_N},
        };
      ...
      }
      {
        struct {
          ...
          Obj d_expected;
            ...
            // LINE |     X     | EXPECTED
            //------------------------------
            {L_, DEC(1.0), ZERO_P},      {L_, DEC(10.0), DEC(1.0)},
            {L_, DEC(12.0), DEC(1.0)},   {L_, DEC(-11.0), DEC(1.0)},
            {L_, DEC(-100.0), DEC(2.0)}, {L_, NAN_P, NAN_P},
            {L_, NAN_N, NAN_N},          {L_, INF_P, INF_P},
            {L_, INF_N, INF_P},          {L_, ZERO_P, INF_N},
            {L_, ZERO_N, INF_N},
        };
      ...
      }
      {
        struct {
          ...
          Obj d_expected;
            ...
            // LINE |     X     | EXPECTED
            //-----------------------------------
            {L_, DEC(1.0), ZERO_P},  {L_, DEC(12.0), DEC(1.079181)},
            {L_, DEC(-11.0), NAN_N}, {L_, NAN_P, NAN_P},
            {L_, NAN_N, NAN_N},      {L_, INF_P, INF_P},
            {L_, INF_N, NAN_N},      {L_, ZERO_P, INF_N},
            {L_, ZERO_N, INF_N},
      ...
      if (veryVerbose) {
        T_ bsl::cout << "remainder()" << bsl::endl;
      }
      {
        struct {
          int d_line;
      ...
      }
      {
        ...
        struct {
          ...
          long int d_expected;
            ...
            // LINE |     X    | EXPECTED
            //---------------------------
            {L_, DEC(2.3), 2},   {L_, DEC(2.5), 2},   {L_, DEC(3.5), 4},
            {L_, DEC(-2.3), -2}, {L_, DEC(-2.5), -2}, {L_, DEC(-3.5), -4},
            {L_, NAN_P, NaN},    {L_, NAN_N, NaN},    {L_, INF_P, NaN},
            {L_, INF_N, NaN},    {L_, ZERO_P, 0},     {L_, ZERO_N, 0},
        };
        const int NUM_DATA = static_cast<int>(sizeof DATA / sizeof *DATA);
      ...
      if (veryVerbose) {
        T_ bsl::cout << "nextafter()" << bsl::endl;
      }
      {
        struct {
          int d_line;
      ...
      if (veryVerbose) {
        T_ bsl::cout << "nexttoward()" << bsl::endl;
      }
      {
        ...
        const Obj128 SUBN_128 = bsl::numeric_limits<Obj128>::denorm_min();
        const Obj128 NAN_Q_128 = bsl::numeric_limits<Obj128>::quiet_NaN();
        const Obj128 NAN_S_128 = bsl::numeric_limits<Obj128>::signaling_NaN();
        const Obj128 INF_128 = bsl::numeric_limits<Obj128>::infinity();
      ...
      }
      {
        ...
        struct {
          ...
          long int d_expected;
            ...
            // LINE |     X    | EXPECTED
            //---------------------------
            {L_, DEC(2.3), 2},   {L_, DEC(2.5), 3}, {L_, DEC(-2.3), -2},
            {L_, DEC(-2.5), -3}, {L_, NAN_P, NaN},  {L_, NAN_N, NaN},
            {L_, INF_P, NaN},    {L_, INF_N, NaN},  {L_, ZERO_P, 0},
            {L_, ZERO_N, 0},
        };
        const int NUM_DATA = static_cast<int>(sizeof DATA / sizeof *DATA);
      ...
      if (veryVerbose) {
        T_ T_ bsl::cout << "round(x, p)" << bsl::endl;
      }
      {
        struct {
          int d_line;
      ...
      if (veryVerbose) {
        T_ T_ bsl::cout << "trunc(x, p)" << bsl::endl;
      }
      {
        struct {
          int d_line;
      ...
      }
      {
        struct {
          ...
          Obj d_expected;
            ...
            // LINE |     X     |   EXPECTED
            //-----------------------------------
            {L_, DEC(100.0), DEC(10.0)}, {L_, DEC(2.0), DEC(1.414214)},
            {L_, DEC(-1.0), NAN_P},      {L_, INF_P, INF_P},
            {L_, INF_N, NAN_P},          {L_, NAN_P, NAN_P},
            {L_, NAN_N, NAN_N},          {L_, ZERO_P, ZERO_P},
            {L_, ZERO_N, ZERO_N},
        };
...
#define DEC(X) BDLDFP_DECIMAL_DD(X)
      ...
      bsl::cout << "\nDecimal64" << "\n---------" << bsl::endl;
    {
      ...
      }
      {
        ...
        struct {
          ...
          Obj d_expected;
            ...
            // LINE |     X     |      EXPECTED
            //-----------------------------------------------
            {L_, DEC_X1, RES_1},    {L_, DEC_X2, RES_2},    {L_, NAN_P, NAN_P},
            {L_, NAN_N, NAN_N},     {L_, INF_P, INF_P},     {L_, INF_N, ZERO_P},
            {L_, ZERO_P, DEC(1.0)}, {L_, ZERO_N, DEC(1.0)},
        };
      ...
      }
      {
        ...
        struct {
          ...
          Obj d_expected;
            ...
            // LINE |     X     | EXPECTED
            //------------------------------
            {L_, DEC(1.0), ZERO_P},  {L_, DEC_X, RES_1},
            {L_, DEC(-11.0), NAN_N}, {L_, NAN_P, NAN_P},
            {L_, NAN_N, NAN_N},      {L_, INF_P, INF_P},
            {L_, INF_N, NAN_N},      {L_, ZERO_P, INF_N},
            {L_, ZERO_N, INF_N},
      ...
      }
      {
        struct {
          ...
          Obj d_expected;
            ...
            // LINE |       X        | EXPECTED
            //-----------------------------------
            {L_, DEC(1.0), ZERO_P},     {L_, DEC(99.999999), DEC(1.0)},
            {L_, DEC(-11.0), DEC(1.0)}, {L_, NAN_P, NAN_P},
            {L_, NAN_N, NAN_N},         {L_, INF_P, INF_P},
            {L_, INF_N, INF_P},         {L_, ZERO_P, INF_N},
            {L_, ZERO_N, INF_N},
        };
      ...
      }
      {
        ...
        struct {
          ...
          Obj d_expected;
            ...
            // LINE |    X   |    Y   | EXPECTED
            //-----------------------------------
            {L_, DEC_X, DEC_Y, DEC_R},   {L_, DEC_X_N, DEC_Y, DEC_R_N},
            {L_, DEC_X, DEC_Y_N, DEC_R}, {L_, DEC_X_N, DEC_Y_N, DEC_R_N},
            {L_, DEC_X, NAN_P, NAN_P},   {L_, DEC_X, NAN_N, NAN_P},
            {L_, DEC_X, INF_P, DEC_X},   {L_, DEC_X, INF_N, DEC_X},
            {L_, DEC_X, ZERO_P, NAN_P},  {L_, DEC_X, ZERO_N, NAN_P},
            {L_, NAN_P, DEC_Y, NAN_P},   {L_, INF_P, DEC_Y, NAN_P},
      ...
      }
      {
        ...
        struct {
          ...
          Obj d_expected;
            ...
            // LINE |    X   |    Y   | EXPECTED
            //-----------------------------------
            {L_, DEC_X, DEC_Y, -DEC_R},  {L_, -DEC_X, DEC_Y, DEC_R},
            {L_, DEC_X, -DEC_Y, -DEC_R}, {L_, -DEC_X, -DEC_Y, DEC_R},
            {L_, DEC_X, NAN_P, NAN_P},   {L_, DEC_X, NAN_N, NAN_P},
            {L_, DEC_X, INF_P, DEC_X},   {L_, -DEC_X, INF_N, -DEC_X},
            {L_, DEC_X, ZERO_P, NAN_P},  {L_, DEC_X, ZERO_N, NAN_P},
      ...
      }
      {
        ...
        struct {
          ...
          long int d_expected;
            ...
            //---------------------------------
            {L_, DEC(2.3), 2},   {L_, DEC(2.5), 2},   {L_, DEC(3.5), 4},
            {L_, DEC(-2.3), -2}, {L_, DEC(-2.5), -2}, {L_, DEC(-3.4999999), -3},
            {L_, DEC(-3.5), -4}, {L_, NAN_P, NaN},    {L_, NAN_N, NaN},
            {L_, INF_P, NaN},    {L_, INF_N, NaN},    {L_, ZERO_P, 0},
            {L_, ZERO_N, 0},
        };
        const int NUM_DATA = static_cast<int>(sizeof DATA / sizeof *DATA);
      ...
      }
      {
        ...
        const Obj MAX_P =
            bdldfp::DecimalImpUtil::parse64("9.999999999999999e+384");
        const Obj MAX_N =
            bdldfp::DecimalImpUtil::parse64("-9.999999999999999e+384");
      ...
      }
      {
        ...
        struct {
          ...
          long int d_expected;
            ...
            // LINE |     X          | EXPECTED
            //---------------------------------
            {L_, DEC(2.3), 2},   {L_, DEC(2.4999999), 2}, {L_, DEC(2.5), 3},
            {L_, DEC(-2.3), -2}, {L_, DEC(-2.5), -3},     {L_, NAN_P, NaN},
            {L_, NAN_N, NaN},    {L_, INF_P, NaN},        {L_, INF_N, NaN},
            {L_, ZERO_P, 0},     {L_, ZERO_N, 0},
        };
        const int NUM_DATA = static_cast<int>(sizeof DATA / sizeof *DATA);
      ...
      }
      {
        ...
        struct {
          ...
          Obj d_expected;
            ...
            // LINE |     X     |   EXPECTED
            //-----------------------------------
            {L_, DEC(100.0), DEC(10.0)}, {L_, DEC_1, RES_1},
            {L_, DEC(2.0), SQRT_2},      {L_, DEC(-1.0), NAN_P},
            {L_, INF_P, INF_P},          {L_, INF_N, NAN_P},
            {L_, NAN_P, NAN_P},          {L_, NAN_N, NAN_N},
            {L_, ZERO_P, ZERO_P},        {L_, ZERO_N, ZERO_N},
        };
      ...
      }
      {
        ...
        struct {
          ...
          Obj d_expected;
            ...
            // LINE |     X     | EXPECTED
            //------------------------------
            {L_, DEC_X1, RES_1},         {L_, DEC_X2, RES_2},
            {L_, DEC(9999999.0), INF_P}, {L_, NAN_P, NAN_P},
            {L_, NAN_N, NAN_N},          {L_, INF_P, INF_P},
            {L_, INF_N, ZERO_P},         {L_, ZERO_P, DEC(1.0)},
            {L_, ZERO_N, DEC(1.0)},
        };
      ...
      }
      {
        ...
        struct {
          ...
          long int d_expected;
            ...
            // LINE |     X                   | EXPECTED
            //------------------------------------------
            {L_, DEC(2.3), 2},   {L_, DEC(2.5), 2},
            {L_, DEC(3.5), 4},   {L_, DEC(-2.3), -2},
            {L_, DEC(-2.5), -2}, {L_, DEC(-3.4999999999999999), -3},
            {L_, DEC(-3.5), -4}, {L_, NAN_P, NaN},
            {L_, NAN_N, NaN},    {L_, INF_P, NaN},
            {L_, INF_N, NaN},    {L_, ZERO_P, 0},
            {L_, ZERO_N, 0},
        };
        const int NUM_DATA = static_cast<int>(sizeof DATA / sizeof *DATA);
      ...
      }
      {
        ...
        struct {
          ...
          Obj d_expected;
            ...
            // LINE |  FROM     | TO   | EXPECTED
            //-----------------------------------
            {L_, DEC(0.0), SUBN, SUBN}, {L_, DEC(0.0), -SUBN, -SUBN},
            {L_, NAN_Q, NAN_Q, NAN_Q},  {L_, NAN_S, NAN_S, NAN_S},
            {L_, INF_P, INF_P, INF_P},
        };
        const int NUM_DATA = static_cast<int>(sizeof DATA / sizeof *DATA);
      ...
      }
      {
        ...
        struct {
          ...
          long int d_expected;
            ...
            // LINE |    X     | EXPECTED
            //---------------------------
            {L_, DEC(2.3), 2},   {L_, DEC_1, 2},      {L_, DEC(2.5), 3},
            {L_, DEC(-2.3), -2}, {L_, DEC(-2.5), -3}, {L_, NAN_P, NaN},
            {L_, NAN_N, NaN},    {L_, INF_P, NaN},    {L_, INF_N, NaN},
            {L_, ZERO_P, 0},     {L_, ZERO_N, 0},
        };
...
#define DEC(X) BDLDFP_DECIMAL_DF(X)
    {
      ...
      for (int ti = 0; ti < NUM_DATA; ++ti) {
        ...
        // Test with Decimal32.
        {
          ...
          int ACTUAL_EXPONENT;
          int ACTUAL_CLASS = Util::decompose(&ACTUAL_SIGN, &ACTUAL_SIGNIFICAND,
                                             &ACTUAL_EXPONENT, VALUE);

          ASSERTV(LINE, ACTUAL_CLASS, CLASS, ACTUAL_CLASS == CLASS);
          ASSERTV(LINE, ACTUAL_SIGN, SIGN, ACTUAL_SIGN == SIGN);
          ASSERTV(LINE, ACTUAL_SIGNIFICAND, SIGNIFICAND,
                  ACTUAL_SIGNIFICAND == SIGNIFICAND);
          ASSERTV(LINE, ACTUAL_EXPONENT, EXPONENT, ACTUAL_EXPONENT == EXPONENT);

          if (Util::isFinite(VALUE)) {
            Type INPUT = ImpUtil::makeDecimalRaw32(
                ACTUAL_SIGN * ACTUAL_SIGNIFICAND, ACTUAL_EXPONENT);
...
#define DEC(X) BDLDFP_DECIMAL_DD(X)
    {
      ...
      Type NAN_S_P = bsl::numeric_limits<Type>::signaling_NaN();
      Type NAN_S_N = -bsl::numeric_limits<Type>::signaling_NaN();
      Type MAX = bdldfp::DecimalImpUtil::parse64("9.999999999999999e+384");
        ...
        // Test with Decimal64.
        {
          ...
          if (Util::isFinite(VALUE)) {
            Type INPUT = ACTUAL_SIGN *
                         Util::multiplyByPowerOf10(Type(ACTUAL_SIGNIFICAND),
                                                   ACTUAL_EXPONENT);
...
#define DEC(X) BDLDFP_DECIMAL_DL(X)
    {
      ...
      static const struct {
        ...
        int d_exponent;
          ...
          // Test zero values
          {L_, DEC(0.0), FP_ZERO, 1, 0, 0, -1},
          {L_, DEC(-0.0), FP_ZERO, -1, 0, 0, -1},

          // Test special encoding values
          {L_, INF_P, FP_INFINITE, 1, 0x20000000000000ull, 0, 0x3000},
          {L_, INF_N, FP_INFINITE, -1, 0x20000000000000ull, 0, 0x3000},
          {L_, NAN_Q_P, FP_NAN, 1, 0x20000000000000ull, 0, 0x3800},
          {L_, NAN_Q_N, FP_NAN, -1, 0x20000000000000ull, 0, 0x3800},
          {L_, NAN_S_P, FP_NAN, 1, 0x20000000000000ull, 0, 0x3c00},
          {L_, NAN_S_N, FP_NAN, -1, 0x20000000000000ull, 0, 0x3c00},
          {L_, SUBN_P, FP_SUBNORMAL, 1, 0, 1, -6176},
          {L_, SUBN_N, FP_SUBNORMAL, -1, 0, 1, -6176},

          // // Test boundary values
          {L_, MIN, FP_NORMAL, 1, 0, 1, -6143},
          {L_, -MIN, FP_NORMAL, -1, 0, 1, -6143},
          {L_, MAX, FP_NORMAL, 1, 0x1ED09BEAD87C0ull, 0x378D8E63FFFFFFFFull,
           6111},
          {L_, -MAX, FP_NORMAL, -1, 0x1ED09BEAD87C0ull, 0x378D8E63FFFFFFFFull,
           6111},
          // // Test arbitrary values
          {L_, DEC(1.0), FP_NORMAL, 1, 0, 10, -1},
          {L_, DEC(-1.0), FP_NORMAL, -1, 0, 10, -1},
          {L_, DEC(0.1), FP_NORMAL, 1, 0, 1, -1},
          {L_, DEC(-0.1), FP_NORMAL, -1, 0, 1, -1},
          {L_, DEC(4.25), FP_NORMAL, 1, 0, 425, -2},
          {L_, DEC(-4.25), FP_NORMAL, -1, 0, 425, -2},
          {L_, V1, FP_NORMAL, 1, 1, 0, 0},
          {L_, -V1, FP_NORMAL, -1, 1, 0, 0},
          {L_, V1 * 0x10 + 0x50, FP_NORMAL, 1, 0x10, 0x50, 0},
          {L_, -(V1 * 0x10 + 0x50), FP_NORMAL, -1, 0x10, 0x50, 0},
      };
      const int NUM_DATA = static_cast<int>(sizeof DATA / sizeof *DATA);
      ...
      for (int ti = 0; ti < NUM_DATA; ++ti) {
        ...
        // Test with Decimal64.
        {
          ...
          ASSERTV(LINE, ACTUAL_CLASS, CLASS, ACTUAL_CLASS == CLASS);
          ASSERTV(LINE, ACTUAL_SIGN, SIGN, ACTUAL_SIGN == SIGN);
          ASSERTV(LINE, ACTUAL_SIGNIFICAND.high(), SIGNIFICAND_H,
                  ACTUAL_SIGNIFICAND.high() == SIGNIFICAND_H);
          ASSERTV(LINE, ACTUAL_SIGNIFICAND.low(), SIGNIFICAND_L,
          ...
          if (Util::isFinite(VALUE)) {
            ...
            const Type INPUT = ACTUAL_SIGN * Util::multiplyByPowerOf10(
                                                 SIGNIFICAND, ACTUAL_EXPONENT);
...
#undef DEC
  ...
  case 11: {
...

    {
      ...
      for (int i = 0; i < NUM_DATA; ++i) {
        ...
        const int rcStrRef =
            Util::parseDecimal32(&valueStrRef, bslstl::StringRef(INPUT));
        const int rcBslStr =
            Util::parseDecimal32(&valueBslStr, bsl::string(INPUT, pa));
        const int rcStdStr =
            Util::parseDecimal32(&valueStdStr, std::string(INPUT));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR_STRING
        Type valuePmrStr = ERROR_VALUE;
        const int rcPmrStr = Util::parseDecimal32(
            &valuePmrStr, std::experimental::pmr::string(INPUT, pa));
#endif
        if (IS_VALID) {
        ...
        if (Util::isNan(EXP)) {
          LOOP3_ASSERT(LINE, EXP, valueCStr, Util::isNan(valueCStr));
          LOOP3_ASSERT(LINE, EXP, valueStrRef, Util::isNan(valueStrRef));
          LOOP3_ASSERT(LINE, EXP, valueBslStr, Util::isNan(valueBslStr));
          LOOP3_ASSERT(LINE, EXP, valueStdStr, Util::isNan(valueStdStr));
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR_STRING
          LOOP3_ASSERT(LINE, EXP, valuePmrStr, Util::isNan(valuePmrStr));
#endif
        } else {
      ...
      static const struct {
        ...
        bool d_isValid;        // isValid flag
          ...
          {L_, "9.999999999999999e+500",
           bdldfp::DecimalImpUtil::parse128("9.999999999999999e+500"), true},
          {L_, "-1e-500", bdldfp::DecimalImpUtil::parse128("-1e-500"), true},
  ...
  case 10: {
    ...
    // --------------------------------------------------------------------
    if (verbose)
      bsl::cout << "multiplyByPowerOf10 tests..." << bsl::endl;
    {
      if (veryVerbose)
        bsl::cout << "multiplyByPowerOf10 "
                     "tests on 'int' powers..."
                  << bsl::endl;
      ...
      for (int mi = 0; mi < numMantissas; ++mi) {
        for (int ei = 0; ei < numExps; ++ei) {
          for (int ei2 = 0; ei2 < numExps; ++ei2) {
            ...
            {
              const BDEC::Decimal64 VALUE = Util::makeDecimal64(MANTISSA, EXP);
              ...
              LOOP5_ASSERT(MANTISSA, EXP, POW, ACTUAL_RESULT, EXPECTED_RESULT,
  ...
  case 9: {
    ...
    if (verbose)
      bsl::cout << "fabs Decimal64 tests..." << bsl::endl;
    {
      ...
      for (long long tiM = 0; tiM < numMantissas; ++tiM) {
        for (int tiE = 0; tiE < numExps; ++tiE) {
          const TYPE value = makeNumber(mantissas[tiM], exps[tiE]);

          // Test the value of what quantum returns:
          ASSERT(
              Util::fabs(value) ==
              makeNumber(bsl::max(mantissas[tiM], -mantissas[tiM]), exps[tiE]));
        }
      }
    }
    if (verbose)
      bsl::cout << "fabs Decimal128 tests..." << bsl::endl;
    {
  ...
  case 8: {
    ...
    if (verbose)
      bsl::cout << "quantize Decimal32 tests..." << bsl::endl;
    {
#define DEC(X) BDLDFP_DECIMAL_DF(X)
...
#undef DEC
    ...
    if (verbose)
      bsl::cout << "quantize Decimal64 tests..." << bsl::endl;
    {
#define DEC(X) BDLDFP_DECIMAL_DD(X)
      typedef BDEC::Decimal64 TYPE;
      typedef bsl::numeric_limits<TYPE> NumLim;
...
#undef DEC
    ...
    if (verbose)
      bsl::cout << "quantize Decimal128 tests..." << bsl::endl;
    {
#define DEC(X) BDLDFP_DECIMAL_DL(X)
      typedef BDEC::Decimal128 TYPE;
      typedef bsl::numeric_limits<TYPE> NumLim;
...
#undef DEC
  ...
  case 7: {
    ...
    if (verbose)
      bsl::cout << "quantum Decimal64 tests..." << bsl::endl;
    {
      typedef BDEC::Decimal64 TYPE;
      typedef bsl::numeric_limits<TYPE> NumLim;
      NumberMaker<TYPE> makeNumber;
      ...
      for (long long tiM = 0; tiM < numMantissas; ++tiM) {
        for (int tiE = 0; tiE < numExps; ++tiE) {
          ...
          LOOP6_ASSERT(tiM, tiE, value, mantissas[tiM], exps[tiE],
                       Util::quantum(value), Util::quantum(value) == exps[tiE]);
        }
      }
    }
    if (verbose)
      bsl::cout << "quantum Decimal128 tests..." << bsl::endl;
    {
      typedef BDEC::Decimal128 TYPE;
      typedef bsl::numeric_limits<TYPE> NumLim;
      NumberMaker<TYPE> makeNumber;
      ...
      for (long long tiM = 0; tiM < numMantissas; ++tiM) {
        for (int tiE = 0; tiE < numExps; ++tiE) {
          ...
          // Test the value of what quantum returns:
          LOOP6_ASSERT(tiM, tiE, mantissas[tiM], exps[tiE], value, quantum,
                       quantum == exps[tiE]);
        }
      }
    }
  ...
  case 6: {
    ...
    if (verbose)
      bsl::cout << "sameQuantum Decimal64 tests..." << bsl::endl;

    {
      ...
      for (long long tiM = 0; tiM < numMantissas; ++tiM) {
        for (int tiE = 0; tiE < numExps; ++tiE) {
          const TYPE lhs = makeNumber(mantissas[tiM], exps[tiE]);

          for (long long tjM = 0; tjM < numMantissas; ++tjM) {
            for (int tjE = 0; tjE < numExps; ++tjE) {
              const TYPE rhs = makeNumber(mantissas[tjM], exps[tjE]);

              LOOP4_ASSERT(mantissas[tiM], exps[tiE], mantissas[tjM], exps[tjE],
    ...
    if (verbose)
      bsl::cout << "sameQuantum Decimal128 tests..." << bsl::endl;
  ...
  case 5: {
    ...
    if (verbose)
      bsl::cout << "isFinite Decimal64 tests..." << bsl::endl;
    {
      typedef BDEC::Decimal64 TYPE;
      typedef bsl::numeric_limits<TYPE> NumLim;
      NumberMaker<TYPE> makeNumber;
    ...
    if (verbose)
      bsl::cout << "isFinite Decimal128 tests..." << bsl::endl;
    {
      typedef BDEC::Decimal128 TYPE;
      typedef bsl::numeric_limits<TYPE> NumLim;
      NumberMaker<TYPE> makeNumber;
  ...
  case 4: {
    ...
    if (verbose)
      bsl::cout << "isInf Decimal64 tests..." << bsl::endl;
    {
      typedef BDEC::Decimal64 TYPE;
      typedef bsl::numeric_limits<TYPE> NumLim;
      NumberMaker<TYPE> makeNumber;
    ...
    if (verbose)
      bsl::cout << "isInf Decimal128 tests..." << bsl::endl;
    {
      typedef BDEC::Decimal128 TYPE;
      typedef bsl::numeric_limits<TYPE> NumLim;
      NumberMaker<TYPE> makeNumber;
  ...
  case 3: {
    ...
    if (verbose)
      bsl::cout << "isUnordered Decimal64 tests..." << bsl::endl;
    {
      typedef BDEC::Decimal64 TYPE;
      typedef bsl::numeric_limits<TYPE> NumLim;
      NumberMaker<TYPE> makeNumber;
    ...
    if (verbose)
      bsl::cout << "isUnordered Decimal128 tests..." << bsl::endl;
    {
      typedef BDEC::Decimal128 TYPE;
      typedef bsl::numeric_limits<TYPE> NumLim;
      NumberMaker<TYPE> makeNumber;
  ...
  case 2: {
    ...
    if (verbose)
      bsl::cout << "isNan Decimal64 tests..." << bsl::endl;
    {
      typedef BDEC::Decimal64 TYPE;
      typedef bsl::numeric_limits<TYPE> NumLim;
      NumberMaker<TYPE> makeNumber;
    ...
    if (verbose)
      bsl::cout << "isNan Decimal128 tests..." << bsl::endl;
    {
      typedef BDEC::Decimal128 TYPE;
      typedef bsl::numeric_limits<TYPE> NumLim;
      NumberMaker<TYPE> makeNumber;
  ...
  case 1: {
    ...
    if (veryVerbose)
      bsl::cout << "makeDecimalNNRaw functions" << bsl::endl;

    if (veryVeryVerbose)
      bsl::cout << "makeDecimalRaw32" << bsl::endl;
    ...
    if (veryVeryVerbose)
      bsl::cout << "makeDecimalRaw64" << bsl::endl;

    ASSERT(Util::makeDecimalRaw64(314159, -5) == BDLDFP_DECIMAL_DD(3.14159));
    ASSERT(Util::makeDecimalRaw64(314159u, -5) == BDLDFP_DECIMAL_DD(3.14159));
    ASSERT(Util::makeDecimalRaw64(314159ll, -5) == BDLDFP_DECIMAL_DD(3.14159));
    ASSERT(Util::makeDecimalRaw64(314159ull, -5) == BDLDFP_DECIMAL_DD(3.14159));

    if (veryVeryVerbose)
      bsl::cout << "Testing the quanta of 0 value" << bsl::endl;
    // {DRQS 154126434}
    for (int i = -398; i <= 369; ++i) {
      ...
      int classification = Util::decompose(&sign, &significand, &exponent, d64);
    ...
    if (veryVeryVerbose)
      bsl::cout << "makeDecimalRaw128" << bsl::endl;
    ...
    ASSERT(Util::makeDecimalRaw128(314159, -5) == BDLDFP_DECIMAL_DL(3.14159));
    ASSERT(Util::makeDecimalRaw128(314159u, -5) == BDLDFP_DECIMAL_DL(3.14159));
    ASSERT(Util::makeDecimalRaw128(314159ll, -5) == BDLDFP_DECIMAL_DL(3.14159));
    ...
    for (int i = -6176; i <= 6111; ++i) {
      ...
      int classification =
          Util::decompose(&sign, &significand, &exponent, d128);
    ...
    if (veryVerbose)
      bsl::cout << "makeDecimalNN functions" << bsl::endl;

    if (veryVeryVerbose)
      bsl::cout << "makeDecimal64" << bsl::endl;
...
#pragma GCC diagnostic pop
    ...
    ASSERT(Util::makeDecimal64(314159, -5) == BDLDFP_DECIMAL_DD(3.14159));
    ASSERT(Util::makeDecimal64(314159u, -5) == BDLDFP_DECIMAL_DD(3.14159));

    if (veryVerbose)
      bsl::cout << "parseDecimalNN functions" << bsl::endl;
    ...
    }
    {
      ...
      ASSERT(BDLDFP_DECIMAL_DL(1234567890123456789012345678901234.0) == result);
    }
    ...
    if (veryVerbose)
      bsl::cout << "fma functions" << bsl::endl;

    // TODO TBD - How to test if fma really does not round too early?

    ASSERT(Util::fma(BDLDFP_DECIMAL_DD(3.), BDLDFP_DECIMAL_DD(4.),
                     BDLDFP_DECIMAL_DD(5.)) == BDLDFP_DECIMAL_DD(17.));

    ASSERT(Util::fma(BDLDFP_DECIMAL_DL(3.), BDLDFP_DECIMAL_DL(4.),
                     BDLDFP_DECIMAL_DL(5.)) == BDLDFP_DECIMAL_DL(17.));

    if (veryVerbose)
      bsl::cout << "fabs functions" << bsl::endl;
...
#pragma GCC diagnostic pop
    ...
    if (veryVerbose)
      bsl::cout << "Classification functions" << bsl::endl;

    if (veryVeryVerbose)
      bsl::cout << "Decimal32" << bsl::endl;
    {
...
#undef DECLIT
    ...
    if (veryVerbose)
      bsl::cout << "Comparison functions" << bsl::endl;
...
#define DECLIT(x) BDLDFP_DECIMAL_DF(x)
      ...
      for (unsigned i = 0; i < unorderedValuesSize; ++i) {
        for (unsigned j = 0; j < orderedValuesSize; ++j) {
          ASSERT(Util::isUnordered(unorderedValues[i], orderedValues[j]) ==
                 true);
        }
      }
      ...
      for (unsigned i = 0; i < orderedValuesSize; ++i) {
        for (unsigned j = 0; j < unorderedValuesSize; ++j) {
          ASSERT(Util::isUnordered(orderedValues[i], unorderedValues[j]) ==
                 true);
        }
      }
      ...
      for (unsigned i = 0; i < unorderedValuesSize; ++i) {
        for (unsigned j = 0; j < unorderedValuesSize; ++j) {
          ASSERT(Util::isUnordered(unorderedValues[i], unorderedValues[j]) ==
                 true);
        }
      }
...
#undef DECLIT
    ...
    if (veryVerbose)
      bsl::cout << "Rounding functions" << bsl::endl;
...
#undef DECLIT
    ...
    if (verbose)
      bsl::cout << "sameQuantum 64 tests..." << bsl::endl;
    {
      typedef BDEC::Decimal64 Tested;
      typedef bsl::numeric_limits<Tested> NumLim;
#define DECLIT(x) BDLDFP_DECIMAL_DL(x)
      ...
      LOOP5_ASSERT(anInt, Util::multiplyByPowerOf10(anInt, 5),
                   ...
                   Util::quantum(anInt), Util::quantum(anInt) + 5,
                   Util::quantum(Util::multiplyByPowerOf10(anInt, 5)) ==
                       Util::quantum(anInt) + 5);
    }

    if (verbose)
      bsl::cout << "sameQuantum 128 tests..." << bsl::endl;
    {
      typedef BDEC::Decimal128 Tested;
      typedef bsl::numeric_limits<Tested> NumLim;
#define DECLIT(x) BDLDFP_DECIMAL_DL(x)
  ...
  case -1: {
    ...
    for (size_type i = 0; i < data.size(); ++i) {
      ...
      // Parse the price.
      BDEC::Decimal64 price = BloombergLP::bdldfp::DecimalUtil::makeDecimal64(
  ...
  case -2: {
    ...
    for (size_type i = 0; i < data.size(); ++i) {
      ...
      symbolData[index].d_valueTraded +=
          price * static_cast<double>(data[i].d_quantity);
      symbolData[index].d_volume += data[i].d_quantity;
      symbolData[index].d_vwap =
          (symbolData[index].d_valueTraded /
  ...
  case -5: {
    ...
    for (int iter = 0; iter < numIterations; ++iter) {
      for (int mi = 0; mi < numMantissas; ++mi) {
        for (int ei = 0; ei < numExps; ++ei) {
          BDEC::Decimal64 num = Util::makeDecimal64(mantissas[mi], exps[ei]);
          (void)num;
        }
      }
    }
    ...
    for (int iter = 0; iter < numIterations; ++iter) {
      for (int mi = 0; mi < numMantissas; ++mi) {
        ...
        for (int ei = 0; ei < numExps; ++ei) {
          ...
          BDEC::Decimal64 num = Util::makeDecimalRaw64(mantissas[mi], exps[ei]);
          (void)num;
        }
      }
    }
    ...
    for (int iter = 0; iter < numIterations; ++iter) {
      for (int mi = 0; mi < numMantissas; ++mi) {
        ...
        for (int ei = 0; ei < numExps; ++ei) {
          ...
          BDEC::Decimal32 num =
              Util::makeDecimalRaw32(static_cast<int>(mantissas[mi]), exps[ei]);
          (void)num;
        }
      }
    }
  ...
  case -9: {
    ...
    for (int i = 0; i < numMantissas; ++i) {
      for (int j = 0; j < numExps; ++j) {
        decimals[i * numExps + j] = Util::makeDecimal64(mantissas[i], exps[j]);
      }
    }
    ...
    for (int iter = 0; iter < numIterations; ++iter) {
      for (int di = 0; di < numDecimals; ++di) {
        for (int ei = 0; ei < numExps; ++ei) {
          BDEC::Decimal64 result =
              Util::multiplyByPowerOf10(decimals[di], exps[ei]);
          (void)result;
        }
      }
    }
    ...
    bsl::cout << "Performance test: " << operationsPerSecond
              << " multiplyByPowerOf10 operations per second." << bsl::endl;
    bsl::cout << "Total time: " << totalTime << " seconds." << bsl::endl;
