...
// [ 5] static int write(std::experimental::pmr::string*, const Json&, const
// WriteOptions&); [ 5] static int write(std::string*, const Json&); [ 5] static
// int write(std::string*, const Json&, const WriteOptions&); [ 6] static
// ostream& printError(ostream&, streambuf *, const Error&); [ 6] static
// ostream& printError(ostream&, istream&, const Error&); [ 6] static ostream&
// printError(ostream&, string_view&, const Error&);
// ----------------------------------------------------------------------------
// [ 1] BREATHING TEST
// [ 8] USAGE EXAMPLE
...
// diagnostics detailing the difference and return 'false' otherwise.
{
  ...
  if (lhs.size() != rhs.size()) {
    ...
    for (; lhsIt != lhs.end(); ++lhsIt, ++rhsIt) {
      if (*lhsIt != *rhsIt) {
        bsl::cout << "Strings differ at position " << int(&*lhsIt - lhs.data())
                  << " lhs='" << *lhsIt << "' rhs='" << *rhsIt << "\n";
        break;
      }
    }
...
JsonObject *appendBoolean(JsonObject *object, const bsl::string_view &name,
                          bool value)
...
JsonObject *appendNumber(JsonObject *object, const bsl::string_view &name,
...
JsonObject *appendString(JsonObject *object, const bsl::string_view &name,
                         const bsl::string_view &value)
...
void checkResult(int line, const bsl::string_view &string, const Json &value,
                 bool isValid)
...
// otherwise with all correct error-handling behavior.
{
  ...
  if (0 == rc) {
    ...
    ASSERTV(line, json.isArray(), value.isArray(),
            value.isArray() == json.isArray());
    // Verify that 'json.isObject()' matches 'value.isObject()'
    ASSERTV(line, json.isObject(), value.isObject(),
            value.isObject() == json.isObject());
    // Verify that 'err.location()' is unset.
    ASSERTV(line, string, rc, err.location(), Location(0) == err.location());
    // Verify that 'err.message()' is unset.
    ASSERTV(line, string, rc, err.message(), err.message().length(),
            0 == err.message().length());
    // Verify that 'json' returns the expected 'value'.
    ASSERTV(line, string, value, json, value == json);
  } else {
...
// this component and return zero.
{
  ...
  case 3: {
    ...
    int maxNestedDepth = bslim::FuzzUtil::consumeNumberInRange<int>(
        &fdv, 1, FUZZ_MAX_NESTED_DEPTH);
    ...
    bslim::FuzzUtil::consumeRandomLengthString(&randomChars, &fdv,
                                               fdv.length());

    bdlsb::FixedMemInStreamBuf input(randomChars.data(), randomChars.size());

    bdljsn::Json result;
    bdljsn::Error errorDescription;

    int rc =
        bdljsn::JsonUtil::read(&result, &errorDescription, &input, readOptions);
    if (0 == rc) {
...
static bsl::size_t NUM_JSON_TEST_SUITE_FOR_BDLJSN =
    sizeof JSON_TEST_SUITE_FOR_BDLJSN / sizeof *JSON_TEST_SUITE_FOR_BDLJSN;
...
// is unknown.
{
  for (JtsuforBdljsnDatum *
           itr = JSON_TEST_SUITE_FOR_BDLJSN,
          *end = JSON_TEST_SUITE_FOR_BDLJSN + NUM_JSON_TEST_SUITE_FOR_BDLJSN;
       end != itr; ++itr) {
...
int main(int argc, char *argv[]) {
  ...
  case 0:
      ...
      cout << "Testing usage example 1" << endl;
    {
      ...
      ///  - - - - - - - - - - - - - - - - - - - -
      // This component provides methods for reading and writing JSON data
      ...
      //..
      // Finally, we'll 'write' the 'result' back into another string and make
      // sure we got the same value back, by using the correct 'WriteOptions' to
      ...
      cout << "Testing usage example 2" << endl;
    {
      ...
      ///- - - - - - - - - - - - - - - - - - - - - - -
      // By populating a 'WriteOptions' object and passing it to 'write', the
      // format of the resulting JSON can be controlled.
      //
      // First, let's populate a 'Json' object named 'json' from an input string
      // using 'read', and create an empty 'options' (see
      // 'bdljsn::WriteOptions'):
      //..
      ...
      //
      // The first set consists of the 'sortMembers' option, which controls
      // whether members of objects are printed in lexicogaphical order.
      //
      // The second set consists of the 'style', 'initialIndentLevel', and
      // 'spacesPerLevel' options - 'style' controls which format is used to
      // render a 'Json', and, if 'bdljsn::WriteStyle::e_PRETTY ==
      // options.style()', the 'spacesPerLevel' and 'initialIndentLevel' options
      // are used to control the indentation of the output.  For any other value
      // of 'options.style()', the 'spacesPerLevel' and 'initialIndentLevel'
      // options have no effect.
      //
      ///'sortMembers'
      ///-  -  -  -  -
      // If 'sortMembers' is true, then the members of an object output by
      // 'write' will be in sorted order.  Otherwise, the elements are written
      // in an (implementation defined) order (that may change).
      //
      // The 'sortMembers' option defaults to 'false' for performance reasons,
      // but applications that rely on stable output text should set
      // 'sortMembers' to 'true' (e.g., in a test where the resulting JSON text
      // is compared for equality) .
      //
      // Here, we set 'sortMembers' to 'true', and verify the resulting JSON
      ...
      //..
      // Had we not specified 'setSortMembers(true)', the order of the "a" and
      // "b" members in the 'output' string would be unpredictable.
      //
      ...
      //
      // Next, we write 'json' using the style 'e_COMPACT' (the default), a
      // single line presentation with no added spaces after ':' and ','
      // elements.
      //..
      rc = bdljsn::JsonUtil::write(&output, json, options);
      ...
      ASSERT(R"JSON({"a": 1, "b": []})JSON" == output);
      //..
      // Next, we write 'json' using the 'e_PRETTY' style, a multiline format
      // where newlines are introduced after each (non-terminal) '{', '[', ',',
      // ']', and
      // '}' character.  Furthermore, the indentation of JSON rendered in the
      // 'e_PRETTY' style is controlled by the other 2 attributes,
      // 'spacesPerLevel' and 'initialIndentLevel'.
      //
      // 'e_PRETTY' styling does not add a newline to the end of the output.
      //
      // 'spacesPerLevel' controls the number of spaces added for each
      // successive indentation level - e.g., if 'spacesPerLevel' is 2, then
      // each nesting level of the rendered JSON is indented 2 spaces.
      //
      // 'initialIndentLevel' controls how much the entire JSON output is
      // indented. It defaults to 0 - if it's a positive value, then the entire
      // JSON is indented by 'initialIndentLevel * spacesPerLevel' spaces.
      //..
      ...
      ASSERT("{"
             ...
             "}" == output);
      //..
      // Finally, if we set 'initialIndentLevel' to 1, then an extra set of 4
      // spaces is prepended to each line, where 4 is the value of
      // 'spacesPerLevel':
      //..
...
//..
  ...
  case 7: {
    ...
    for (bsl::size_t ti = 0; ti < NUM_DATA; ++ti) {
      ...
      const JTSU::Expected EXPECTED = JTSU::e_EITHER == EXPECTED_JTSU
                                          ...
                                          : EXPECTED_JTSU;
      ASSERTV(EXPECTED,
              JTSU::e_ACCEPT == EXPECTED || JTSU::e_REJECT == EXPECTED);

      if (veryVerbose) {
        P_(ti) P_(LINE) P_(LENGTH) P(EXPECTED) P(TEST_NAME);
        P(JSON)
      }
      ...
      ASSERTV(LINE, EXPECTED, rc, (JTSU::e_ACCEPT == EXPECTED) == (0 == rc));

      ASSERTV(LINE, EXPECTED, rc,
              (JTSU::e_ACCEPT == EXPECTED) == (Error() == error));
  ...
  case 6: {
      ...
      cout << "Test overloads" << endl;
    {
...

      {
        ...
        bsl::ostream &result = Util::printError(output, twoLineMesage, error);
  ...
  case 5: {
    ...
    if (verbose)
      cout << "Testing 'static int write(bsl::ostream&, const Json&);'" << endl;
    {
           ...
           << endl;
    {
      ...
      sb.sputc('\0');
      ASSERTV(rc, 0 == rc);
      ASSERTV(outputStr, JSON_PRETTY_IIL_0_SPL_0,
      ...
      sb.sputc('\0');
      ASSERTV(rc, 0 == rc);
      ASSERTV(outputStr, JSON_PRETTY_IIL_1_SPL_2,
              JSON_PRETTY_IIL_1_SPL_2 == outputStr);
    }

    if (verbose)
      cout << "Testing 'static int write(bsl::string*, const Json&);'" << endl;
    {
      bslma::TestAllocator ta;
    ...
    if (verbose)
      cout
          << "Testing 'static int write(std::experimental::pmr::string*, const "
             "Json&);'"
          << endl;
    {
    ...
    if (verbose)
      cout << "Testing 'static int write(std::experimental::pmr::string*, "
              "const Json&, "
              "const WriteOptions&);'"
           << endl;
    {
    ...
    if (verbose)
      cout << "Testing 'static int write(std::string*, const Json&, "
              "const WriteOptions&);'"
           << endl;
    {
      std::string outputStr;
  ...
  case 4: {
    ...
    if (verbose)
      cout << "\tBasic read and write round-trip covering all types" << endl;
    {
      bslma::TestAllocator ta;
      bdlsb::FixedMemInStreamBuf input(SIMPLE_JSON, bsl::strlen(SIMPLE_JSON));
      Error error(&ta);
    ...
    if (verbose)
      bsl::cout << "\tTest write styles" << bsl::endl;
    {
      struct WriteTests {
        int d_line;
        const char *d_json_p;
      ...
      } DATA[] = {// L  JSON                COMPACT        ONELINE       PRETTY
                  {L_, "{}", "{}", "{}", "{}"},
                  {L_, "[]", "[]", "[]", "[]"},
      ...
      for (int i = 0; i < NUM_DATA; ++i) {
        ...
        ASSERTV(LINE, json, compact.str(), EXPCOMPACT,
        ...
        ASSERTV(LINE, json, rc, 0 == rc);
        ASSERTV(LINE, json, oneline.str(), EXPONELINE,
        ...
        ASSERTV(LINE, json, rc, 0 == rc);
        ASSERTV(LINE, json, pretty.str(), EXPPRETTY, EXPPRETTY == pretty.str());
      }
    }

    if (verbose)
      bsl::cout << "\tTest IIL/SPL" << bsl::endl;
    {
      struct WriteTests {
        int d_line;
        const char *d_json_p;
        ...
        const char *d_expPretty_p;
          ...
          {L_, "1.0e+1", 1, 4, "    1.0e+1", "    1.0e+1"},
          {L_, "\"ab\"", 0, 0, "\"ab\"", "\"ab\""},
          {L_, "\"ab\"", 1, 4, "    \"ab\"", "    \"ab\""}
          // Complex types are affected by IIL and SPL for all levels of
          // indentation
          ,
          {L_, "{\"a\":1, \"b\":2}", 0, 0, "{\"a\": 1, \"b\": 2}",
           "{\n"
           "\"a\": 1,\n"
           "\"b\": 2\n"
           "}"},
          {L_, "{\"a\":1, \"b\":2}", 1, 4, "    {\"a\": 1, \"b\": 2}",
           "    {\n"
           "        \"a\": 1,\n"
           "        \"b\": 2\n"
           "    }"},
          {L_, "{\"a\":1, \"b\":2}", 10, 1, "          {\"a\": 1, \"b\": 2}",
           "          {\n"
           "           \"a\": 1,\n"
           "           \"b\": 2\n"
           "          }"},
          {L_, "{\"a\":1, \"b\":2}", 5, 2, "          {\"a\": 1, \"b\": 2}",
           "          {\n"
           "            \"a\": 1,\n"
           "            \"b\": 2\n"
           "          }"},
      ...
      for (int i = 0; i < NUM_DATA; ++i) {
        ...
        rc = Util::write(pretty, json, options);
        ASSERTV(LINE, json, rc, 0 == rc);
        ASSERTV(LINE, json, IIL, SPL, EXPPRETTY.length(), pretty.str().length(),
                EXPPRETTY, pretty.str(), EXPPRETTY == pretty.str());
        ...
        rc = Util::write(oneline, json, options);
        ASSERTV(LINE, json, rc, 0 == rc);
        ASSERTV(LINE, json, IIL, SPL, EXPONELINE.length(),
                oneline.str().length(), EXPONELINE, oneline.str(),
                EXPONELINE == oneline.str());
      }
    }

    if (verbose)
      bsl::cout << "\tTesting sorting keys" << bsl::endl;
    {
      ...
      ASSERTV(sorted.str(), unsorted.str(), sorted.str() != unsorted.str());

      ASSERTV(sorted.str(), SORTED_JSON, SORTED_JSON == sorted.str());
  ...
  case 3: {
      ...
      cout << "\n\tTesting 'static int read(Json*, streambuf*);" << endl;
    {
      ...
      bdlsb::FixedMemInStreamBuf badStream(badJson.data(), badJson.size());

      int rc = Util::read(&json, &badStream);
      ASSERTV(rc, badJson, json, 0 != rc);
      ASSERTV(json.isNull(), true == json.isNull());

      bdlsb::FixedMemInStreamBuf goodStream(goodJson.data(), goodJson.size());
           ...
           << endl;
    {
      ...
      bdlsb::FixedMemInStreamBuf json65DeepStream(json65DeepArray.data(),
           ...
           << endl;
    {
      ...
      // Verify that 'err.message()' is set.
      ASSERTV(err.message(), err.message().length(),
              0 != err.message().length());
    ...
    if (verbose)
      cout << "\n\tTest next input position after 'read'" << endl;
    {
      const char k_EOF = 0;
      ...
      static const struct {
        ...
        const char *d_text_p;
        char d_nextChar;
      } DATA[] = {// L  TEXT          NEXT CHAR
                  //== =============  =========
                  {L_, "null", k_EOF},     {L_, "null ", ' '},
                  {L_, "null a", ' '},     {L_, "null{", '{'},
                  {L_, "true", k_EOF},     {L_, "true ", ' '},
                  {L_, "true a", ' '},     {L_, "true{", '{'},
                  {L_, "1e1", k_EOF},      {L_, "1e1 ", ' '},
                  {L_, "1e1 e", ' '},      {L_, "1e1{", '{'},
                  {L_, "[]", k_EOF},       {L_, "[] ", ' '},
                  ...
                  {L_, "{} ", ' '},        {L_, "{} a", ' '},
                  {L_, "{}a", 'a'},        {L_, "{}{", '{'},
                  {L_, "\"text\"", k_EOF}, {L_, "\"text\" ", ' '},
                  {L_, "\"text\" a", ' '}, {L_, "\"text\"a", 'a'},
                  {L_, "\"text\"{", '{'},  {L_, "\"text\",", ','}};
      ...
      for (int ti = 0; ti < NUM_DATA; ++ti) {
        ...
        const int NEXT_CHAR_INT =
            NEXT_CHAR == k_EOF ? bdlsb::FixedMemInStreamBuf::traits_type::eof()
        ...
        ASSERTV(LINE, TEXT, nextC, NEXT_CHAR_INT, nextC == NEXT_CHAR_INT);
      }
    }
  } break;
  case 2: {
      ...
      cout << "\n\tTest 'read' of 'null' value." << endl;
    {
      static const struct {
        ...
        bool d_isValid;
          ...
          // L                                             STRING   VALID?
          //== ================================================== ========
          {L_, "null", true},        {L_, "       null", true},
          {L_, "null       ", true}, {L_, "nul", false},
          {L_, "       nul", false}, {L_, "nul       ", false}};
      ...
      for (int ti = 0; ti < NUM_DATA; ++ti) {
        ...
        if (veryVerbose) {
          T_ P_(LINE) P_(STRING) P(IS_VALID)
        }
        ...
        if (0 == rc) {
          ...
          // 'read'.
          ASSERTV(LINE, STRING, rc, IS_VALID, true == json.isNull());
        } else {
          // Verify that failure was expected.
          ASSERTV(LINE, STRING, IS_VALID, !IS_VALID);
          // Verify that 'json' is unchanged on failure.
          ASSERTV(LINE, STRING, rc, true == json.isBoolean());
          // Verify that 'err.location()' is set.
          ASSERTV(LINE, STRING, rc, err.location(),
      ...
      cout << "\n\tTest 'read' of 'boolean' values." << endl;
    {
      static const struct {
        ...
        bool d_isValid;
          ...
          //== ======================================== ======== =========
          {L_, "true", true, true},         {L_, "       true", true, true},
          {L_, "true       ", true, true},  {L_, "false", false, true},
          {L_, "      false", false, true}, {L_, "false      ", false, true},
          {L_, "tru", false, false},        {L_, "       yes", false, false},
          {L_, "no        ", false, false}};
      ...
      for (int ti = 0; ti < NUM_DATA; ++ti) {
        ...
        if (0 == rc) {
          ...
          // 'read'.
          ASSERTV(LINE, json.isBoolean(), true == json.isBoolean());
          // Verify that 'json.theBoolean()' returns the expected
          // value.
          ASSERTV(LINE, STRING, VALUE, json.theBoolean(),
      ...
      cout << "\n\tTest 'read' of 'number' values." << endl;
    {
      ...
      for (int ti = 0; ti < NUM_DATA; ++ti) {
        ...
        if (0 == rc) {
          ...
          // 'read'.
          ASSERTV(LINE, json.isBoolean(), true == json.isNumber());
          // Verify that 'json.theNumber()' returns the expected
          // value.
          ASSERTV(LINE, STRING, VALUE, json.theNumber(),
      ...
      */
      ...
      for (int ti = 0; ti < NUM_DATA; ++ti) {
        ...
        if (0 == rc) {
          ...
          // 'read'.
          ASSERTV(LINE, json.isBoolean(), true == json.isString());
          // Verify that 'json.theString()' returns the expected
          // value.
          ASSERTV(LINE, STRING, VALUE, json.theString(),
      ...
      cout << "\n\tTest 'read' of simple 'array' values." << endl;
    {
      // Positive tests
      {
        ...
        ASSERTV(jsonResult, jsonResult.isNull(), true == jsonResult.isNull());

        jsonResult.makeArray();

        ASSERTV(jsonResult, jsonResult.isArray(), true == jsonResult.isArray());
        ...
        ASSERTV(jsonResult, theArray, theArray.size(), 1 == theArray.size());
        ASSERTV(jsonResult, theArray.back(), theArray.back().isNull(),
        ...
        ASSERTV(jsonResult, theArray, theArray.size(), 2 == theArray.size());
        ASSERTV(jsonResult, theArray.back(), theArray.back().isBoolean(),
        ...
        ASSERTV(jsonResult, theArray, theArray.size(), 3 == theArray.size());
        ASSERTV(jsonResult, theArray.back(), theArray.back().isNumber(),
                true == theArray.back().isNumber());
        ASSERTV(jsonResult, theArray.back(), theArray.back().theNumber(),
        ...
        ASSERTV(jsonResult, theArray, theArray.size(), 4 == theArray.size());
        ASSERTV(jsonResult, theArray.back(), theArray.back().isString(),
                true == theArray.back().isString());
        ASSERTV(jsonResult, theArray.back(), theArray.back().theString(),
                "Hello, world!" == theArray.back().theString());

        checkResult(L_, "[null, true, 123.45e+1, \"Hello, world!\"]",
                    jsonResult, true);
        checkResult(L_,
                    WS "[" WS "null" WS "," WS "true" WS "," WS "123.45e+1" WS
                       "," WS "\"Hello, world!\"" WS "]",
                    jsonResult, true);

        appendMemberArray(&theArray);

        ASSERTV(jsonResult, theArray, theArray.size(), 5 == theArray.size());
        ASSERTV(jsonResult, theArray.back(), theArray.back().isArray(),
                true == theArray.back().isArray());
        ASSERTV(jsonResult, theArray.back(), theArray.back().theArray(),
        ...
        checkResult(L_,
                    WS "[" WS "null" WS "," WS "true" WS "," WS "123.45e+1" WS
                       "," WS "\"Hello, world!\"" WS "," WS "[" WS "]" WS "]",
                    jsonResult, true);

        appendMemberObject(&theArray);

        ASSERTV(jsonResult, theArray, theArray.size(), 6 == theArray.size());
        ASSERTV(jsonResult, theArray.back(), theArray.back().isObject(),
                true == theArray.back().isObject());
        ASSERTV(jsonResult, theArray.back(), theArray.back().theObject(),
      ...
      cout << "\n\tTest 'read' of simple 'object' values." << endl;
    {
      // Positive tests
      {
        ...
        ASSERTV(jsonResult, jsonResult.isObject(),
        ...
        ASSERTV(jsonResult, theObject, theObject.size(), 1 == theObject.size());
        ASSERTV(jsonResult, theObject["a"], theObject["a"].isNull(),
                true == theObject["a"].isNull());

        checkResult(L_, "{\"a\":null}", jsonResult, true);
        checkResult(L_, WS "{" WS "\"a\"" WS ":" WS "null" WS "}" WS,
                    jsonResult, true);

        appendBoolean(&theObject, "b", true);

        ASSERTV(jsonResult, theObject, theObject.size(), 2 == theObject.size());
        ASSERTV(jsonResult, theObject["b"], theObject["b"].isBoolean(),
        ...
        ASSERTV(jsonResult, theObject, theObject.size(), 3 == theObject.size());
        ASSERTV(jsonResult, theObject["c"], theObject["c"].isNumber(),
                true == theObject["c"].isNumber());
        ASSERTV(jsonResult, theObject["c"], theObject["c"].theNumber(),
                JsonNumber("123.45e+1") == theObject["c"].theNumber());

        checkResult(L_, "{\"a\":null, \"b\":true, \"c\":123.45e+1}", jsonResult,
                    true);

        appendString(&theObject, "d", "Hello, world!");

        ASSERTV(jsonResult, theObject, theObject.size(), 4 == theObject.size());
        ASSERTV(jsonResult, theObject["d"], theObject["d"].isString(),
                true == theObject["d"].isString());
        ASSERTV(jsonResult, theObject["d"], theObject["d"].theString(),
        ...
        checkResult(L_,
                    ...
                    jsonResult, true);
        checkResult(L_,
                    WS "{" WS "\"a\"" WS ":" WS "null" WS "," WS "\"b\"" WS
                       ":" WS "true" WS "," WS "\"c\"" WS ":" WS "123.45e+1" WS
        ...
        ASSERTV(jsonResult, theObject, theObject.size(), 5 == theObject.size());
        ASSERTV(jsonResult, theObject["e"], theObject["e"].isArray(),
                true == theObject["e"].isArray());
        ASSERTV(jsonResult, theObject["e"], theObject["e"].theArray(),
        ...
        checkResult(L_,
                    WS "{" WS "\"a\"" WS ":" WS "null" WS "," WS "\"b\"" WS
                       ":" WS "true" WS "," WS "\"c\"" WS ":" WS "123.45e+1" WS
                       "," WS "\"d\"" WS ":" WS "\"Hello, world!\"" WS "," WS
                       "\"e\"" WS ":" WS "[" WS "]" WS "}",
                    jsonResult, true);

        addMemberObject(&theObject, "f");

        ASSERTV(jsonResult, theObject, theObject.size(), 6 == theObject.size());
        ASSERTV(jsonResult, theObject["f"], theObject["f"].isObject(),
                true == theObject["f"].isObject());
        ASSERTV(jsonResult, theObject["f"], theObject["f"].theObject(),
      ...
      // Negative tests
      {
        ...
        checkResult(L_, "{\"a\":null, \"b\":true, \"c\":+123.45e+1}",
                    jsonResult, false);
      }
    }

    if (verbose)
      cout << "\n\tTest 'read' of complex nested 'array' values." << endl;
    {
      {
        bslma::TestAllocator scratch("scratch", veryVeryVeryVerbose);
        ...
        ASSERTV(jsonResult, *theArray, theArray->size(), 0 == theArray->size());
        ...
        checkResult(L_,
                    WS "[" WS "null" WS "," WS "[" WS "true" WS "," WS
                       "123.45e+1" WS "," WS "\"Hello, world!\"" WS "]" WS "]",
                    jsonResult, true);
    ...
    if (verbose)
      cout << "\n\tTest 'read' of complex nested 'object' values." << endl;
    {
      {
        bslma::TestAllocator scratch("scratch", veryVeryVeryVerbose);
        ...
        ASSERTV(jsonResult, *theObject, theObject->size(),
                1 == theObject->size());
        ASSERTV(jsonResult, (*theObject)["a"], (*theObject)["a"].isNull(),
                true == (*theObject)["a"].isNull());
        ...
        ASSERTV(jsonResult, *theObject, theObject->size(),
                2 == theObject->size());
        ASSERTV(jsonResult, (*theObject)["a1"], (*theObject)["a1"].isObject(),
        ...
        checkResult(L_, "{\"a\":null, \"a1\":{\"b\":true}}", jsonResult, true);
        ...
        checkResult(L_, "{\"a\":null, \"a1\":{\"b\":true, \"c\":123.45e+1}}",
                    jsonResult, true);
        ...
        checkResult(L_,
                    ...
                    jsonResult, true);
        checkResult(L_,
                    WS "{" WS "\"a\"" WS ":" WS "null" WS "," WS "\"a1\"" WS
                       ":" WS "{" WS "\"b\"" WS ":" WS "true" WS "," WS
                       "\"c\"" WS ":" WS "123.45e+1" WS "," WS "\"d\"" WS ":" WS
                       "\"Hello, world!\"" WS "," WS "\"e\"" WS ":" WS "[" WS
                       "]" WS "," WS "\"f\"" WS ":" WS "{" WS "}" WS "}" WS "}",
                    jsonResult, true);
      }
    }
    ...
    if (verbose)
      cout << "\n\tTest 'allowTrailingText' option" << endl;
    {
      static const struct {
        int d_line;
        const char *d_text_p;
        ...
        bool d_isValidNoTrailingText;
          ...
          //== =============  ======= ============
          {L_, "null", true, true},        {L_, "null ", true, true},
          {L_, "null a", true, false},     {L_, "nulla", false, false},
          {L_, "null{", true, false},      {L_, "null}", true, false},
          {L_, "null[", true, false},      {L_, "null]", true, false},
          {L_, "null,", true, false},      {L_, "null\"", true, false},
          {L_, "true", true, true},        {L_, "true ", true, true},
          {L_, "true a", true, false},     {L_, "truea", false, false},
          {L_, "true{", true, false},      {L_, "true}", true, false},
          {L_, "true[", true, false},      {L_, "true]", true, false},
          {L_, "true,", true, false},      {L_, "true\"", true, false},
          {L_, "1e1", true, true},         {L_, "1e1 ", true, true},
          {L_, "1e1 e", true, false},      {L_, "1e1e", false, false},
          {L_, "1e1{", true, false},       {L_, "1e1}", true, false},
          {L_, "1e1[", true, false},       {L_, "1e1]", true, false},
          {L_, "1e1,", true, false},       {L_, "1e1\"", true, false},
          {L_, "[]", true, true},          {L_, "[] ", true, true},
          {L_, "[] a", true, false},       {L_, "[]a", true, false},
          {L_, "[]{", true, false},        {L_, "[]}", true, false},
          {L_, "[][", true, false},        {L_, "[]]", true, false},
          {L_, "[],", true, false},        {L_, "[]\"", true, false},
          {L_, "{}", true, true},          {L_, "{} ", true, true},
          {L_, "{} a", true, false},       {L_, "{}a", true, false},
          ...
          {L_, "{}[", true, false},        {L_, "{}]", true, false},
          {L_, "{},", true, false},        {L_, "{}\"", true, false},
          {L_, "\"text\"", true, true},    {L_, "\"text\" ", true, true},
          {L_, "\"text\" a", true, false}, {L_, "\"text\"a", true, false},
          {L_, "\"text\"{", true, false},  {L_, "\"text\"}", true, false},
          {L_, "\"text\"[", true, false},  {L_, "\"text\"]", true, false},
          {L_, "\"text\",", true, false},  {L_, "\"text\"\"", true, false}};
      ...
      for (int ti = 0; ti < NUM_DATA; ++ti) {
...

        {
          ...
          ASSERTV(LINE, INPUT, IS_VALID_NO_TT, rc, IS_VALID_NO_TT == (0 == rc));
        }

        {
          ...
          ASSERTV(LINE, INPUT, IS_VALID_TT, rc, IS_VALID_TT == (0 == rc));
        }
      }
    }
    if (verbose)
  ...
  case 1: {
    ...
    if (verbose) {
      bsl::cout << "\tTest handling of misplaced commas and incorrect endings"
                << bsl::endl;
    }
    {
      struct BadCommas {
      ...
      for (int i = 0; i < NUM_DATA; ++i) {
        ...
        if (EXPECTEDRC == rc) {
          ASSERTV(LINE, JSON, EXPECTEDOFFSET, error.location().offset(),
      ...
      bsl::cout << "\tTest write styles" << bsl::endl;
    {
      ...
      for (int i = 0; i < NUM_DATA; ++i) {
        ...
        rc = Util::write(&compact, json, options);
        ASSERTV(LINE, json, rc, 0 == rc);
        ASSERTV(LINE, json, compact, EXPCOMPACT, EXPCOMPACT == compact);
        ...
        rc = Util::write(&oneline, json, options);
        ASSERTV(LINE, json, rc, 0 == rc);
        ASSERTV(LINE, json, oneline, EXPONELINE, EXPONELINE == oneline);
    ...
    {
      {
        ...
        for (int i = 0; i < NUM_DATA; ++i) {
          ...
          for (int depth = 1; depth < JSON_DEPTH + 2; ++depth) {
            ...
            ASSERTV(J, depth, rc, (JSON_DEPTH <= depth) == (0 == rc));
          }
        }
      }
      ...
      // Verify that default options value of 64 is handled correctly.
      {
        ...
        for (int i = 0; i < NUM_DATA; ++i) {
          ...
          // Works with default 'options.maxNestedDepth()'
          {
            ...
            bdljsn::Json result;
            ReadOptions options;
            ASSERTV(options.maxNestedDepth(), 64 == options.maxNestedDepth());
            int rc = Util::read(&result, &error, &input, options);
            ASSERTV(J, rc, options.maxNestedDepth(), (0 == rc));
          }
    ...
    }
    {
      ...
      if (veryVerbose)
        cout << "... bsl::string input" << endl;
      {
        bslma::TestAllocator ta;
        const bsl::string input(SIMPLE_JSON, bsl::strlen(SIMPLE_JSON), &ta);
        Error error(&ta);
    ...
    for (int i = 0; i < NUM_DATA; ++i) {
      ...
      ASSERTV(LINE, JSON, STYLE, INDENT, SPACES, EXPECTED, resultString,
    ...
    for (int i = 0; i < NUM_ERROR_DATA; ++i) {
      ...
      ASSERTV(LINE, JSON, result, location == result.location().offset());
    }
  } break;
  default: {
