...
struct State {
  ...
  // CREATORS
  State(Context context, Json *json,
        const bsl::string_view &member = bsl::string_view())
      : d_context(context), d_json_p(json), d_member(member) {}
};
  ...
  for (const char *cIt = spec; cIt != spec + specLength; ++cIt) {
    ...
    case e_ARRAY: {
      ...
      case '[': {
        ...
        stack.emplace_back(e_ARRAY, &stack.back().d_json_p->theArray().back());
      } break;
      case ']': {
        stack.pop_back();
      } break;
    ...
    case e_OBJECT_EXPECT_VALUE: {
      switch (c) {
      case 's': {
        stack.back().d_json_p->theObject().insert(stack.back().d_member,
                                                  Json("string"));
      ...
      case '[': {
        ...
        stack.emplace_back(
            e_ARRAY,
            &stack.back().d_json_p->theObject()[stack.back().d_member]);
      } break;
      case '{': {
        stack.back().d_json_p->theObject().insert(stack.back().d_member,
                                                  Json(JsonObject()));
...
struct GenerateJsonArray {
  ...
  Json operator()(const Json &element1, const Json &element2,
...
struct GenerateJsonObject {
  ...
  Json operator()(const bsl::string_view &key1, const Json &value1,
                  const bsl::string_view &key2, const Json &value2) const {
    JsonObject result;
  ...
  Json operator()(const bsl::string_view &key1, const Json &value1,
                  const bsl::string_view &key2, const Json &value2,
                  const bsl::string_view &key3, const Json &value3) const {
    JsonObject result;
    result.insert(key1, value1);
    result.insert(key2, value2);
...
#else // 64_BIT
#define SUFFICIENTLY_LONG_STRING                                               \
...
template <class TYPE> class TemplatedJsonObjectInsertionTest {
public:
...
void TemplatedJsonObjectInsertionTest<TYPE>::operator()(
    const TYPE &value, bslma::TestAllocator *oa) {
  ...
  }
  {
    ...
    ASSERTV(value, moveFrom, ca.numBlocksTotal(), BlocksBefore,
            oa->numBlocksTotal(),
            ca.numBlocksTotal() == (oa->numBlocksTotal() - BlocksBefore));
    ASSERTV(value, moveFrom, ca.numBytesTotal(), BytesBefore,
            oa->numBytesTotal(),
...
class ReorderExpectedJsonObjectString {
  ...
  static bsl::vector<bsl::string> extractOrder(const JsonObject &X) {
    ...
    for (JsonObject::ConstIterator iter = X.cbegin(); iter != X.cend();
         ++iter) {
...
public:
  ...
  bsl::string operator()(const bsl::string &input) const {
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      const bsl::string::size_type secondQuote = input.find('\"', prev + 1);
...
void extendedBreathingTest(BSLA_MAYBE_UNUSED bool verbose,
  ...
  // Move Constructor
  {
    ...
    ASSERT(JsonArray(bslmf::MovableRefUtil::move(lvalue1)) == ga("[1s]"));
    ASSERT(JsonArray(bslmf::MovableRefUtil::move(lvalue2), &ta) == ga("[1s]"));
  }
  ...
  // Value Insertion
  {
    ...
    JsonArray::Iterator insertion1 = mX.insert(static_cast<bsl::size_t>(0), Y);
    ...
    JsonArray::Iterator insertion3 =
        mX.insert(static_cast<bsl::size_t>(0), g("n"));
  ...
  // Range Insertion
  {
    ...
    JsonArray::Iterator insertion =
        mX.insert(static_cast<bsl::size_t>(0), range, range + 2);
  ...
  // Range Insertion at an Iterator
  {
    ...
    JsonArray::Iterator insertion = mX.insert(mX.begin(), range, range + 2);
    ASSERT(&*insertion == &mX[0]);
  ...
  // Range Erasure
  {
    ...
    JsonArray::Iterator erasure = mX.erase(mX.begin() + 1, mX.begin() + 3);
    ASSERT(&*erasure == &mX[1]);
  ...
  // Streaming Operator
  {
    ...
    const bsl::string &result = stream.str();
    const bsl::string_view expectedResult = "[1, [null, \"string\"], true, {}]";
    ASSERTV(result, result == expectedResult);
  }
  ...
  // Range Constructor
  {
    JsonObject::Member members[2] = {JsonObject::Member("a", Json()),
    ...
    ASSERT(X.size() == 2);
    ASSERT(X.cbegin()->first == "a" || X.cbegin()->first == "b");
    ASSERT((++X.cbegin())->first == "a" || (++X.cbegin())->first == "b");
    ASSERT(X.cbegin()->second.isNull() || X.cbegin()->second.isString());
    ASSERT((++X.cbegin())->second.isNull() ||
  ...
  // Copy-Assignment Operator
  {
    ...
    ASSERT(Y.size() == 2);
    ASSERT(Y.cbegin()->first == "a" || Y.cbegin()->first == "b");
    ASSERT((++Y.cbegin())->first == "a" || (++Y.cbegin())->first == "b");
    ASSERT(Y.cbegin()->second.isNull() || Y.cbegin()->second.isString());
    ASSERT((++Y.cbegin())->second.isNull() ||
  ...
  // Range Insertion
  {
    ...
    JsonObject::Member members[] = {JsonObject::Member("a", Json()),
                                    JsonObject::Member("b", Json(""))};
  ...
  // Iterator Erasure
  {
    ...
    ASSERT(bErasure == mX.end() || bErasure->first == "a");
    ASSERT(X.size() == 1);
  }
  ...
  // Make Array
  {
    ...
    JsonArray &mZArray = mZ.makeArray(bslmf::MovableRefUtil::move(array));
  ...
  // Make Number
  {
    ...
    JsonNumber &mZNumber = mZ.makeNumber(bslmf::MovableRefUtil::move(number));
  ...
  // Make Object
  {
    ...
    JsonObject &mZObject = mZ.makeObject(bslmf::MovableRefUtil::move(object));
  ...
  // Streaming Operator
  {
    ...
    const bsl::string &result = stream.str();
    const bsl::string_view expectedResult = "[false, {}, 1.2345]";
    ASSERTV(result, result == expectedResult);
  }
...
template <class SPECIALIZATIONS_ONLY> class JsonValueConstructorHelper;

template <> class JsonValueConstructorHelper<JsonNull> {
public:
...
template <> class JsonValueConstructorHelper<bool> {
public:
...
template <> class JsonValueConstructorHelper<float> {
public:
...
template <> class JsonValueConstructorHelper<double> {
public:
...
template <> class JsonValueConstructorHelper<JsonArray> {
public:
...
template <> class JsonValueConstructorHelper<bdldfp::Decimal64> {
public:
...
template <> class JsonValueConstructorHelper<int> {
public:
...
template <> class JsonValueConstructorHelper<unsigned int> {
public:
  static unsigned int extractFromJson(const Json &json, int ALLOC_CONFIG,
                                      char CONFIG)
  // Returns the value of specified type from the specified 'json',
  // suitable for comparison to the value used in construction.  The
  // specified 'ALLOC_CONFIG' and the specified 'CONFIG' will be used to
...
template <> class JsonValueConstructorHelper<bsls::Types::Int64> {
public:
  static bsls::Types::Int64 extractFromJson(const Json &json, int ALLOC_CONFIG,
                                            char CONFIG)
  // Returns the value of specified type from the specified 'json',
  // suitable for comparison to the value used in construction.  The
  // specified 'ALLOC_CONFIG' and the specified 'CONFIG' will be used to
...
template <> class JsonValueConstructorHelper<bsls::Types::Uint64> {
public:
  static bsls::Types::Uint64 extractFromJson(const Json &json, int ALLOC_CONFIG,
                                             char CONFIG)
  // Returns the value of specified type from the specified 'json',
  // suitable for comparison to the value used in construction.  The
  // specified 'ALLOC_CONFIG' and the specified 'CONFIG' will be used to
...
template <> class JsonValueConstructorHelper<JsonNumber> {
public:
...
template <> class JsonValueConstructorHelper<JsonObject> {
public:
...
template <> class JsonValueConstructorHelper<const char *> {
public:
...
template <> class JsonValueConstructorHelper<bsl::string_view> {
public:
  static bsl::string_view extractFromJson(const Json &json, int, char)
  // Returns the value of specified type from the specified 'json',
  // suitable for comparison to the value used in construction.
...
template <> class JsonValueConstructorHelper<bsl::string> {
public:
...
int main(int argc, char *argv[]) {
  ...
  case 42: {
    ...
    if (verbose)
      cout << endl << "DRQS 171793948" << endl << "==============" << endl;
    ...
    }
    {
      ...
      obj.insert(members, members + sizeof(members) / sizeof(members[0]));
      ASSERTV(obj.size(), 4 == obj.size());
    }
    {
      bdljsn::JsonObject obj;
  ...
  case 41: {
    ...
    bdljsn::Json example1, example2;
    {
      ...
      // JSON text using 'bdljsn_jsonutil'.  In this simple example, we
      // demonstrate manually creating the document below and then verify the
      ...
      // 'bdljsn::Json' object to be a 'bdljsn::Json' object, and use the
      // various manipulators of 'bdljsn::JsonObject' to configure its value:
      //..
      ...
      //..
      // Notice that we used 'operator[]' to implicitly create new members of
      // the top-level object.  Using 'json.theObject().insert' would be more
      // efficient (see example 2).
      //
    ...
    }
    {
      ...
      // Using 'operator[]' is intuitive but not the most efficient method to
      // add new members to a 'bdljsn::JsonObject' (similar to using
      // 'operator[]' to add elements to an 'unordered_map').  The following
      // code demonstrates a more efficient way to create the same
      // 'bdljsn::Json' representation as example 1:
      //..
      using namespace bdldfp::DecimalLiterals;
  ...
  case 40: {
    ...
    if (verbose)
      cout << endl << "JSON ACCESSORS" << endl << "==============" << endl;
...

    {
      ...
      if (veryVerbose)
        cout << "bool Json::isBoolean() const;\n";
      {
        ASSERT(!X.isBoolean());
        mX.makeBoolean();
        ASSERT(X.isBoolean());
    ...
    }
    {
      ...
      const JsonNumber NUMBERS[] = {
          ...
          JsonNumber(bsl::numeric_limits<bsls::Types::Int64>::max(), &xa),
          JsonNumber(bsl::numeric_limits<bsls::Types::Uint64>::max(), &xa),
          JsonNumber(bsl::numeric_limits<int>::min(), &xa),
          JsonNumber(bsl::numeric_limits<bsls::Types::Int64>::min(), &xa),
          JsonNumber(bsl::numeric_limits<int>::max() + 1ll, &xa),
          JsonNumber(bsl::numeric_limits<unsigned int>::max() + 1ll, &xa),
          JsonNumber(bsl::numeric_limits<bsls::Types::Int64>::max() + 1ull,
                     &xa),
      ...
      for (long unsigned int i = 0; i < (sizeof(NUMBERS) / sizeof(*NUMBERS));
           ++i) {
        ...
        if (veryVerbose)
          cout << "int Json::asInt(int *r) const;\n";
        {
          int jsValue;
          int nValue;
        ...
        if (veryVerbose)
          cout << "double Json::asDouble() const;\n";
        {
          double jsValue;
          double nValue;
          jsValue = X.asDouble();
    ...
    if (veryVerbose)
      cout << "Json::operator const bool &() const;\n";
    {
      typedef bool Type;
      const Obj X((Type()));
      const Type &ref = static_cast<const Type &>(X);
...
#else
    if (verbose)
      cout << "Explicit operators unsupported.\n";
#endif
    if (veryVerbose)
  ...
  case 39: {
    ...
    //   Json& Json::operator=(const JsonNumber& rhs);
    {
      ...
      ASSERTV(sa.numBlocksInUse(), sa.numBlocksTotal(),
    ...
    //   Json& Json::operator=(const JsonObject& rhs);
    {
      ...
      const JsonObject v(JsonValueConstructorHelper<JsonObject>::getValue(),
                         &xa);
    ...
    //   Json& Json::operator=(bslmf::MovableRef<JsonObject> rhs);
    {
      ...
      JsonObject v(JsonValueConstructorHelper<JsonObject>::getValue(), &sa);
    ...
    //   Json& Json::operator=(const JsonArray& rhs);
    {
      ...
      const JsonArray v(JsonValueConstructorHelper<JsonArray>::getValue(), &xa);
    ...
    //   Json& Json::operator=(bslmf::MovableRef<JsonArray> rhs);
    {
      ...
      JsonArray v(JsonValueConstructorHelper<JsonArray>::getValue(), &sa);
    ...
    //   bool& Json::makeBoolean(bool boolean);
    {
      ...
      ASSERTV(&result, &(X.theBoolean()), &(X.theBoolean()) == &result);
      ASSERTV(sa.numBytesTotal(), 0 == sa.numBytesTotal());
      ASSERTV(da.numBytesTotal(), 0 == da.numBytesTotal());
    }
    //   JsonNumber& Json::makeNumber(const JsonNumber& number);
  ...
  case 37: {
    ...
    if (verbose)
      cout << "\nUse a table of distinct object values and expected memory "
              "usage."
           << endl;
    ...
    for (char ti = 'a'; ti <= JsonValueEnumeration::maxConfig(); ++ti) {
      ...
      const bool MEMSRC1 = JsonValueEnumeration::setValue(&mZ, VALUE_CONFIG1);
      const Obj &Z = mZ;
      ...
      if (veryVerbose) {
        T_ P_(VALUE_CONFIG1) P_(Z) P(ZZ)
      }

      // move assignment with the same allocator

      for (char tj = 'a'; tj <= JsonValueEnumeration::maxConfig(); ++tj) {
...

        {
          ...
          if (veryVerbose) {
            T_ P_(VALUE_CONFIG2) P(F) P(X)
          }

          ASSERTV(VALUE_CONFIG1, VALUE_CONFIG2, F, X,
      ...
      for (char tj = 'a'; tj <= JsonValueEnumeration::maxConfig(); ++tj) {
...

        {
          ...
          BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(s2) {
            ...
            ASSERTV(VALUE_CONFIG1, VALUE_CONFIG2, Z, F, Z == F);

            ASSERTV(VALUE_CONFIG1, VALUE_CONFIG2, &s2, X.allocator(),
                    &s2 == X.allocator());
            ...
            if (!MEMDST2 && MEMSRC1) {
              ASSERTV(VALUE_CONFIG1, VALUE_CONFIG2, 0 < EXCEPTION_COUNT);
            }
#endif
          }
        ...
        ASSERTV(VALUE_CONFIG1, VALUE_CONFIG2, s1.numBlocksInUse(),
                0 == s1.numBlocksInUse());
        ASSERTV(VALUE_CONFIG1, VALUE_CONFIG2, s2.numBlocksInUse(),
                0 == s2.numBlocksInUse());
      }
...

      {
        ...
        ASSERTV(VALUE_CONFIG1, &oa, Z.allocator(), &oa == Z.allocator());
      ...
      ASSERTV(VALUE_CONFIG1, oa.numBlocksInUse(), 0 == oa.numBlocksInUse());
    }
  ...
  case 36: {
    ...
    for (char ti = 'a'; ti <= JsonValueEnumeration::maxConfig(); ++ti) {
      ...
      const bool MEM1 = JsonValueEnumeration::setValue(&mZ, VALUE_CONFIG1);
      ...
      for (char tj = 'a'; tj <= JsonValueEnumeration::maxConfig(); ++tj) {
...

        {
          ...
          const bool MEM2 = JsonValueEnumeration::setValue(&mX, VALUE_CONFIG2);

          if (veryVerbose) {
            T_ P_(VALUE_CONFIG2) P(X)
          }

          ASSERTV(VALUE_CONFIG1, VALUE_CONFIG2, Z, X,
          ...
          ASSERTV(VALUE_CONFIG1, VALUE_CONFIG2, &oa, X.allocator(),
                  &oa == X.allocator());
          ASSERTV(VALUE_CONFIG1, VALUE_CONFIG2, &scratch, Z.allocator(),
                  &scratch == Z.allocator());
          ...
          ASSERTV(VALUE_CONFIG1, VALUE_CONFIG2, 0 == da.numBlocksTotal());
        }
  ...
  case 35: {
    ...
    if (verbose)
      cout << "Tests will assert absense of allocation on swaps of 'Json' "
              "objects with the same allocator.\n";
#else
    const bool swapNoCopy = false;
    if (verbose)
      cout << "Tests will not assert absense of allocation on swaps of 'Json' "
              "objects with the same allocator.\n";
#endif
    ...
    for (char ti = 'a'; ti <= JsonValueEnumeration::maxConfig(); ++ti) {
      ...
      Obj mW(&oa);
      const Obj &W = mW;
      const bool allocated = JsonValueEnumeration::setValue(&mW, VALUE_CONFIG1);

      const Obj XX(W, &scratch);

      if (veryVerbose) {
        T_ P_(VALUE_CONFIG1) P_(W) P(XX)
      }
      ...
      ASSERTV(VALUE_CONFIG1, allocated, oa.numBlocksInUse(),
      ...
      for (char tj = 'a'; tj <= JsonValueEnumeration::maxConfig(); ++tj) {
        ...
        if (veryVerbose) {
          T_ P_(VALUE_CONFIG2) P_(X) P_(Y) P(YY)
        }
        ...
        // member 'swap'
        {
          ...
          ASSERTV(VALUE_CONFIG1, VALUE_CONFIG2, &oa == X.allocator());
          if (swapNoCopy) {
            ASSERTV(VALUE_CONFIG1, VALUE_CONFIG2, oam.isTotalSame());
          }
        }
    ...
    for (char ti = 'a'; ti <= JsonValueEnumeration::maxConfig(); ++ti) {
      ...
      for (char tj = 'a'; tj <= JsonValueEnumeration::maxConfig(); ++tj) {
        ...
        // free function 'swap', different allocator
        {
          ...
          ASSERTV(VALUE_CONFIG1, VALUE_CONFIG2, XX, Y, XX == Y);
          ASSERTV(VALUE_CONFIG1, VALUE_CONFIG2, &oa == X.allocator());
          ASSERTV(VALUE_CONFIG1, VALUE_CONFIG2, &oa2 == Y.allocator());
        }
      }
    }
      ...
      cout << "\nCreate objects with various allocator configurations." << endl;
    {
      ...
      for (char ti = 'a'; ti <= JsonValueEnumeration::maxConfig(); ++ti) {
        ...
        const bool allocated =
            JsonValueEnumeration::setValue(&mZ, VALUE_CONFIG);
        const Obj &Z = mZ;
        ...
        for (char cfg = 'a'; cfg <= 'd'; ++cfg) {
          ...
          case 'b': {
            ...
            objAllocatorPtr = &da;
            objPtr = new (fa) Obj(bslmf::MovableRefUtil::move(mF), 0);
          } break;
          case 'c': {
            // allocator move constructor, same allocator
            objAllocatorPtr = &s1;
            objPtr =
                new (fa) Obj(bslmf::MovableRefUtil::move(mF), objAllocatorPtr);
          } break;
          case 'd': {
            // allocator move constructor, same allocator
            objAllocatorPtr = &s2;
          ...
          ASSERTV(VALUE_CONFIG, CONFIG, 2 * sizeof(Obj) == fa.numBytesInUse());

          Obj &mX = *objPtr;
          const Obj &X = mX;

          if (veryVerbose) {
            T_ T_ P_(CONFIG) P(mF) P(X)
          }
          ...
          if (objAllocatorPtr != F.allocator()) {
            ...
            ASSERTV(VALUE_CONFIG, CONFIG, s1Alloc, s1.numBytesInUse(),
          ...
          if (X.isArray()) {
            ASSERTV(VALUE_CONFIG, CONFIG, &oa == X.theArray().allocator());
          } else if (X.isNumber()) {
            ASSERTV(VALUE_CONFIG, CONFIG, &oa == X.theNumber().allocator());
          } else if (X.isObject()) {
            ASSERTV(VALUE_CONFIG, CONFIG, &oa == X.theObject().allocator());
          } else if (X.isString()) {
            ASSERTV(VALUE_CONFIG, CONFIG, &oa == X.theString().get_allocator());
          ...
          ASSERTV(VALUE_CONFIG, CONFIG, allocated, oa.numBlocksInUse(),
                  (allocated) == (0 < oa.numBlocksInUse()));
      ...
      cout << "\nCreate objects with various allocator configurations." << endl;
    {
      ...
      for (char ti = 'a'; ti <= JsonValueEnumeration::maxConfig(); ++ti) {
        ...
        for (char cfg = 'a'; cfg <= 'c'; ++cfg) {
          ...
          ASSERTV(VALUE_CONFIG, CONFIG, sizeof(Obj) == fa.numBytesInUse());
      ...
      cout << "\nTesting with injected exceptions." << endl;
    {
      for (char ti = 'a'; ti <= JsonValueEnumeration::maxConfig(); ++ti) {
        ...
        ASSERTV(VALUE_CONFIG, &scratch, Z.allocator(),
                &scratch == Z.allocator());
        ASSERTV(VALUE_CONFIG, da.numBlocksInUse(), 0 == da.numBlocksInUse());
        ASSERTV(VALUE_CONFIG, sa.numBlocksInUse(), 0 == sa.numBlocksInUse());
      }
    }
  } break;
  case 32: {
  ...
  case 30: {
    ...
    mX.makeNull();
    {
      bslma::TestAllocatorMonitor tam(&sa);
      ASSERTV(X.type(), JsonType::e_NULL == X.type());
      const JsonNull &otherNull = X.theNull();
      (void)otherNull;
      ASSERTV(tam.isTotalSame());
    }
  ...
  case 29: {
    ...
    if (verbose)
      cout << endl << "JSON VALUE CTOR" << endl << "===============" << endl;
    ...
    for (int allocCfg = 0; allocCfg < 3; ++allocCfg) {
      for (char cfg = 'a'; cfg <= 's'; ++cfg) {
        ...
        case 'a': { //   Json(const JsonArray &array, *a);
          ...
          ASSERTV(ALLOC_CONFIG, CONFIG, Helper::getType(), objPtr->type(),
                  Helper::getType() == objPtr->type());
          ASSERTV(ALLOC_CONFIG, CONFIG,
                  Helper::extractFromJson(*objPtr, ALLOC_CONFIG, CONFIG),
                  Helper::getValue(),
                  Helper::extractFromJson(*objPtr, ALLOC_CONFIG, CONFIG) ==
                      Helper::getValue());
        } break;
        case 'b': { // Json(bslmf::MovableRef<JsonArray> array, *a);
          ...
          if (0 == ALLOC_CONFIG) {
            ...
            objPtr = new (fa) Obj(bslmf::MovableRefUtil::move(from));
            ...
            bslma::TestAllocatorMonitor tam(objAllocatorPtr);
            objPtr = new (fa) Obj(bslmf::MovableRefUtil::move(from), 0);
            ASSERTV(CONFIG, tam.isTotalSame());
          } else if (2 == ALLOC_CONFIG) {
            objAllocatorPtr = &sa;
            TheType from(Helper::getValue(), objAllocatorPtr);
            ...
            bslma::TestAllocatorMonitor tam(objAllocatorPtr);
            objPtr = new (fa) Obj(bslmf::MovableRefUtil::move(from), &sa);
            ASSERTV(CONFIG, tam.isTotalSame());
          } else {
            BSLS_ASSERT_OPT(!"Bad allocator config.");
          }
        ...
        case 'c': { //   Json(bool boolean, *a);
          ...
          ASSERTV(ALLOC_CONFIG, CONFIG, 0 == objAllocatorPtr->numBlocksTotal());
          ASSERTV(ALLOC_CONFIG, CONFIG, Helper::getType(), objPtr->type(),
                  Helper::getType() == objPtr->type());
          ASSERTV(ALLOC_CONFIG, CONFIG,
                  Helper::extractFromJson(*objPtr, ALLOC_CONFIG, CONFIG),
        ...
        case 'l': { //   Json(const JsonNumber& number, *a);
          ...
          ASSERTV(
              ALLOC_CONFIG, CONFIG,
              Helper::extractFromJson(*objPtr, ALLOC_CONFIG, CONFIG).value(),
              Helper::getString(),
              Helper::extractFromJson(*objPtr, ALLOC_CONFIG, CONFIG).value() ==
                  Helper::getString());
        } break;
        ...
        ASSERTV(CONFIG, sizeof(Obj), fa.numBytesTotal(),
  ...
  case 28: {
    ...
    for (char cfg = 'a'; cfg <= 'c'; ++cfg) {
      ...
      ASSERTV(CONFIG, JsonType::e_NULL, X.type(), JsonType::e_NULL == X.type());
...

      {
        ...
        {
          JsonArray &array = mX.makeArray();
          (void)array;
          ASSERTV(CONFIG, JsonType::e_ARRAY, X.type(),
                  JsonType::e_ARRAY == X.type());
          ASSERTV(CONFIG, &oa, array.allocator(), &oa == array.allocator());
        }

        {
          bool &boolean = mX.makeBoolean();
          (void)boolean;
          ASSERTV(CONFIG, JsonType::e_BOOLEAN, X.type(),
                  JsonType::e_BOOLEAN == X.type());
        }

        {
          mX.makeNull();
          ASSERTV(CONFIG, JsonType::e_NULL, X.type(),
                  JsonType::e_NULL == X.type());
        }

        {
          JsonNumber &number = mX.makeNumber();
          (void)number;
          ASSERTV(CONFIG, JsonType::e_NUMBER, X.type(),
                  JsonType::e_NUMBER == X.type());
          ASSERTV(CONFIG, &oa, number.allocator(), &oa == number.allocator());
        }

        {
          JsonObject &object = mX.makeObject();
          (void)object;
          ASSERTV(CONFIG, JsonType::e_OBJECT, X.type(),
                  JsonType::e_OBJECT == X.type());
          ASSERTV(CONFIG, &oa, object.allocator(), &oa == object.allocator());
        }

        {
          ...
          ASSERTV(CONFIG, JsonType::e_STRING, X.type(),
  ...
  case 27: {
    ...
    const Obj::Member ARRAY[] = {
        Obj::Member("bool", Json(true)), Obj::Member("string", Json("string")),
        Obj::Member("number", Json(1)), Obj::Member("null", Json())};
...

    {
      ...
      typedef JsonObject::ConstIterator (JsonObject::*ConstIterFromStringView)(
...

    {
      ...
      typedef bool (JsonObject::*BoolFromStringView)(const bsl::string_view &)
          const;
  ...
  case 26: {
    ...
    {
      Json &(JsonObject::*funcPtr)(const bsl::string_view &) = &Obj::operator[];
      (void)funcPtr;
    }
    {
    ...
    {
      typedef bsl::pair<JsonObject::Iterator, bool> (JsonObject::*FuncPtr)(
    ...
    {
      typedef JsonObject::Iterator (JsonObject::*FuncPtr)(JsonObject::Iterator);
    ...
    {
      typedef bsl::size_t (JsonObject::*FuncPtr)(const bsl::string_view &);
    ...
    for (char cfg = 'a'; cfg <= 'e'; ++cfg) {
...
#ifdef BSLS_COMPILERFEATURES_SUPPORT_GENERALIZED_INITIALIZERS
        if (veryVerbose)
          cout << "operator=(initializer_list<Json> l)\n";
        Obj &r = (mX = {{KEY, GOOD}});
        ASSERT(&r == &mX);
#else
        if (veryVerbose)
          cout << "init list assignment elided.\n";
        continue;
#endif
      } break;
      case 'b': {
        if (veryVerbose)
          cout << "Json& operator[](string_view k);\n";
        mX[KEY] = GOOD;
      } break;
      case 'c': {
        if (veryVerbose)
      ...
      case 'e': {
        if (veryVerbose)
          cout << "Iter find(const bsl::string_view& k);\n";
    ...
    for (char cfg = 'f'; cfg <= 'h'; ++cfg) {
      ...
      Obj mX(ARRAY, ARRAY + 2, &sa);
      const Obj &X = mX;

      switch (CONFIG) {
      case 'f': {
        if (veryVerbose)
          cout << "Iter erase(Iter p);\n";
      ...
      case 'h': {
        if (veryVerbose)
          cout << "Iter erase(const bsl::string_view& i);\n";
    ...
    for (char cfg = 'i'; cfg <= 'n'; ++cfg) {
      ...
      case 'i': {
        if (veryVerbose)
          cout << "Json& operator[](string_view i);\n";
        mX[KEY] = GOOD;
      } break;
      case 'j': {
        if (veryVerbose)
          cout << "pair<Iter, bool> insert(const Member& m);\n";
      ...
      case 'k': {
        if (veryVerbose)
          cout << "pair<Iter, bool> MovableRef<Member> m);\n";
        Obj::Member moveFrom(GOOD_MEMBER);
        const Obj::IteratorAndStatus r =
            mX.insert(bslmf::MovableRefUtil::move(moveFrom));
        ASSERT(r.first == mX.begin());
        ASSERT(true == r.second);

        Obj::Member moveFrom2(GOOD_MEMBER);
        const Obj::IteratorAndStatus r2 =
            mX.insert(bslmf::MovableRefUtil::move(moveFrom2));
        ASSERT(r2.first == mX.begin());
...
#ifdef BSLS_COMPILERFEATURES_SUPPORT_GENERALIZED_INITIALIZERS
        if (veryVerbose)
          cout << "void insert(initializer_list<Member> m);";
        mX.insert({{KEY, GOOD}});
#else
        if (veryVerbose)
      ...
      case 'n': {
        if (veryVerbose)
          cout << "pair<Iter, bool> insert(string_view k, VALUE v);\n";
    ...
    }
    {
      ...
      Obj mX(&GOOD_MEMBER, &GOOD_MEMBER + 1, &sa);
      const Obj &X = mX;
    ...
    }
    {
      ...
      TemplatedJsonObjectInsertionTest<const char *>()("const char*", &oa);

      TemplatedJsonObjectInsertionTest<bsl::string_view>()("string_view", &oa);
      ...
      TemplatedJsonObjectInsertionTest<Json>()(Json(LONG_STRING, &oa), &oa);
  ...
  case 25: {
    ...
    if (verbose)
      cout << "JSONOBJECT BSLX STREAMING" << endl
           << "=========================" << endl;
  ...
  case 24: {
    ...
    for (int ti = 0; ti < NUM_DATA; ++ti) {
      ...
      const JsonObject::Member ARRAY1[3] = {
          ...
          JsonObject::Member(KEY1, Json(JsonNumber(NUMBER1, &ia), &ia), &ia),
          JsonObject::Member(KEY2, Json(bsl::string(STRING1, &ia), &ia), &ia)};
      ...
      for (int tj = 0; tj < NUM_DATA; ++tj) {
        ...
        const JsonObject::Member ARRAY2[3] = {
            ...
            JsonObject::Member(KEY1, Json(JsonNumber(NUMBER2, &ia), &ia), &ia),
            JsonObject::Member(KEY2, Json(bsl::string(STRING2, &ia), &ia),
                               &ia)};
...

        {
          ...
          Obj mF(ARRAY1, ARRAY1 + 3, &s1);
          const Obj &F = mF;
          Obj mX(ARRAY2, ARRAY2 + 3, &s1);
          const Obj &X = mX;
          ...
          ASSERTV(LINE1, LINE2, &s1, X.allocator(), &s1 == X.allocator());
          ASSERTV(LINE1, LINE2, &s1, F.allocator(), &s1 == F.allocator());
        ...
        ASSERTV(LINE1, LINE2, s1.numBlocksInUse(), 0 == s1.numBlocksInUse());
      }
      ...
      for (int tj = 0; tj < NUM_DATA; ++tj) {
        ...
        const Obj::Member ARRAY2[3] = {
            ...
            Obj::Member(KEY1, Json(JsonNumber(NUMBER2, &ia), &ia), &ia),
            Obj::Member(KEY2, Json(bsl::string(STRING2, &ia), &ia), &ia)};
...

        {
          ...
          BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(s2) {
            ...
            ASSERTV(LINE1, LINE2, &s2, X.allocator(), &s2 == X.allocator());
            ASSERTV(LINE1, LINE2, &s1, F.allocator(), &s1 == F.allocator());
        ...
        ASSERTV(LINE1, LINE2, s1.numBlocksInUse(), 0 == s1.numBlocksInUse());
        ASSERTV(LINE1, LINE2, s2.numBlocksInUse(), 0 == s2.numBlocksInUse());
      }
    ...
    for (int ti = 0; ti < NUM_DATA; ++ti) {
      ...
      for (int tj = 0; tj < NUM_DATA; ++tj) {
...

        {
          ...
          ASSERTV(LINE1, LINE2, &oa, X.allocator(), &oa == X.allocator());
          ...
          ASSERTV(LINE1, LINE2, da.numBlocksTotal(), 0 == da.numBlocksTotal());
        }

        // Verify all memory is released on object destruction.

        ASSERTV(LINE1, LINE2, oa.numBlocksInUse(), 0 == oa.numBlocksInUse());
      }
  ...
  case 22: {
    ...
    for (int ti = 0; ti < NUM_DATA; ++ti) {
      ...
      const bsl::pair<const bsl::string, Json> ARRAY1[3] = {
          bsl::pair<const bsl::string, Json>(KEY0, Json(BOOL1, &ia), &ia),
          bsl::pair<const bsl::string, Json>(
              KEY1, Json(JsonNumber(NUMBER1, &ia), &ia), &ia),
          bsl::pair<const bsl::string, Json>(
              KEY2, Json(bsl::string(STRING1, &ia), &ia), &ia)};

      bslma::TestAllocator oa("object", veryVeryVeryVerbose);
      bslma::TestAllocator scratch("scratch", veryVeryVeryVerbose);

      Obj mW(ARRAY1, ARRAY1 + 3, &oa);
      const Obj &W = mW;
      const Obj XX(W, &scratch);

      if (veryVerbose) {
        T_ P_(LINE1) P_(W) P(XX)
      }
      ...
      for (int tj = 0; tj < NUM_DATA; ++tj) {
        ...
        const bsl::pair<const bsl::string, Json> ARRAY2[3] = {
            bsl::pair<const bsl::string, Json>(KEYS[0], Json(BOOL2, &ia), &ia),
            bsl::pair<const bsl::string, Json>(
                KEYS[1], Json(JsonNumber(NUMBER2, &ia), &ia), &ia),
            bsl::pair<const bsl::string, Json>(
                KEYS[2], Json(bsl::string(STRING2, &ia), &ia), &ia)};

        Obj mX(XX, &oa);
        const Obj &X = mX;
        Obj mY(ARRAY2, ARRAY2 + 3, &oa);
        const Obj &Y = mY;
        const Obj YY(Y, &scratch);

        if (veryVerbose) {
          T_ P_(LINE2) P_(X) P_(Y) P(YY)
        }
           ...
           << endl;
    {
      ...
      const JsonObject::Member ALLOCATING_DATA[] = {
          bsl::pair<const bsl::string, Json>(
              ...
              &scratch),
          bsl::pair<const bsl::string, Json>(
              KEY1, Json(JsonNumber(INT_LONG_STRING, &scratch), &scratch),
              &scratch)};
  ...
  case 21: {
    ...
    if (verbose)
      cout << "\nUse a table of distrinct object values and expected memory "
              "usage."
           << endl;
      ...
      cout << "\nCreate objects with various allocator configurations." << endl;
    {
      ...
      for (int ti = 0; ti < NUM_DATA; ++ti) {
        ...
        const bsl::pair<const bsl::string, Json> ARRAY[3] = {
            bsl::make_pair<const bsl::string, Json>(KEYS[0], Json(BOOL)),
            bsl::make_pair<const bsl::string, Json>(KEYS[1],
                                                    Json(JsonNumber(NUMBER))),
            bsl::make_pair<const bsl::string, Json>(KEYS[2], Json(STRING))};
        ...
        for (char cfg = 'a'; cfg <= 'd'; ++cfg) {
          ...
          ASSERTV(LINE, CONFIG, da.numBlocksInUse(), 0 == da.numBlocksInUse());
          ASSERTV(LINE, CONFIG, fa.numBlocksInUse(), 0 == fa.numBlocksInUse());
          ASSERTV(LINE, CONFIG, s1.numBlocksInUse(), 0 == s1.numBlocksInUse());
          ASSERTV(LINE, CONFIG, s2.numBlocksInUse(), 0 == s2.numBlocksInUse());
        } // end foreach configuration
      } // end foreach row
      ...
      cout << "\nTesting with injected exceptions." << endl;
    {
      for (int ti = 0; ti < NUM_DATA; ++ti) {
        ...
        const bsl::pair<const bsl::string, Json> ARRAY[3] = {
            ...
            bsl::make_pair<const bsl::string, Json>(KEY1,
                                                    Json(JsonNumber(NUMBER))),
  ...
  case 20: {
      ...
      cout << "\nCreate objects with various allocator configurations." << endl;
    {
      ...
      for (int ti = 0; ti < NUM_DATA; ++ti) {
        ...
        for (char cfg = 'a'; cfg <= 'c'; ++cfg) {
          ...
          ASSERTV(LINE, CONFIG, &oa, X.allocator(), &oa == X.allocator());
          ...
          ASSERTV(LINE, CONFIG, da.numBlocksInUse(), 0 == da.numBlocksInUse());
          ASSERTV(LINE, CONFIG, fa.numBlocksInUse(), 0 == fa.numBlocksInUse());
          ASSERTV(LINE, CONFIG, sa.numBlocksInUse(), 0 == sa.numBlocksInUse());
        } // end foreach configuration
      } // end foreach row
      ...
      cout << "\nTesting with injected exceptions." << endl;
    {
      for (int ti = 0; ti < NUM_DATA; ++ti) {
        ...
        ASSERTV(LINE, &scratch, Z.allocator(), &scratch == Z.allocator());
  ...
  case 19: {
    ...
    static const struct {
      ...
      const int d_value4;
        ...
        {L_, 4, 2, 3, EMPTY},     {L_, 1, 4, 3, EMPTY},
        {L_, 1, 2, 4, EMPTY},     {L_, 1, 2, 3, 4},
        {L_, 1, 2, EMPTY, EMPTY}, {L_, 1, EMPTY, EMPTY, EMPTY},
...
#define SP " "
        {L_, 0, 0,
         "{" NL "\"null\": null," NL "\"string\": \"outer\"," NL
         "\"number\": 1," NL "\"array\": [" NL "2," NL "\"inner\"" NL "]" NL
         "}"},
        {L_, 0, 1,
         "{" NL " \"null\": null," NL " \"number\": 1," NL
         " \"string\": \"outer\"," NL " \"array\": [" NL "  2," NL
         "  \"inner\"" NL " ]" NL "}"},

        {L_, 0, -1,
         "{"
         "\"null\": null," SP "\"number\": 1," SP "\"string\": \"outer\"," SP
         "\"array\": ["
         "2," SP "\"inner\""
         "]"
         "}"},
        ...
        {L_, 3, 0,
         "{" NL "\"null\": null," NL "\"number\": 1," NL
         "\"string\": \"outer\"," NL "\"array\": [" NL "2," NL "\"inner\"" NL
         "]" NL "}"},

        {L_, 3, 2,
         "      {" NL "        \"null\": null," NL "        \"number\": 1," NL
         "        \"string\": \"outer\"," NL "        \"array\": [" NL
         "          2," NL "          \"inner\"" NL "        ]" NL "      }"},
...
#undef NL
    ...
    if (verbose)
      cout << "\nTsting with various print specifications." << endl;
    {
      for (int ti = 0; ti < NUM_DATA; ++ti) {
        const int LINE = DATA[ti].d_line;
  ...
  case 17: {
    ...
    {
      if (verbose)
        cout << endl << "JsonObject::operator[]" << endl;
      ...
      if (verbose)
        cout << endl << "Change data and verify expected values." << endl;
  ...
  case 16: {
          ...
          << endl;
    {
      ...
      for (int ti = 0; ti < NUM_DATA; ++ti) {
        ...
        if (veryVerbose) {
          T_ P_(MEM) P_(BOOL) P_(NUMBER) P_(STRING)
        }
        ...
        for (char cfg = 'a'; cfg <= 'c'; ++cfg) {
          ...
          case 'b': {
            objAllocatorPtr = &da;
            objPtr = new (fa) Obj(ARRAY, ARRAY + 3, objAllocatorPtr);
          } break;
          case 'c': {
            objAllocatorPtr = &sa;
            objPtr = new (fa) Obj(ARRAY, ARRAY + 3, objAllocatorPtr);
          ...
          ASSERTV(LINE, CONFIG, BOOL, X[KEY0], BOOL == X[KEY0].theBoolean());
          ...
          ASSERTV(LINE, CONFIG, STRING, X[KEY2], STRING == X[KEY2].theString());
      ...
      cout << "\nTesting with injected exceptions." << endl;
    {
      for (int ti = 0; ti < NUM_DATA; ++ti) {
        ...
        BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(sa) {
          ...
          ASSERTV(LINE, ti, BOOL, obj[KEY0], BOOL == obj[KEY0].theBoolean());
          ...
          ASSERTV(LINE, ti, STRING, obj[KEY2], STRING == obj[KEY2].theString());
  ...
  case 15: {
    ...
    for (char cfg = 'a'; cfg <= 'c'; ++cfg) {
      ...
      ASSERTV(CONFIG, D2, X[KEY1].theString(), X[KEY1].theString() == D2);
...

      {
        ...
        ASSERTV(CONFIG, X[KEY1].type(), X[KEY1].isString());
        ASSERTV(CONFIG, A2, X[KEY1].theString(), X[KEY1].theString() == A2);
        ...
        ASSERTV(CONFIG, X[KEY1].type(), X[KEY1].isString());
        ASSERTV(CONFIG, B2, X[KEY1].theString(), X[KEY1].theString() == B2);
  ...
  case 13: {
...
#ifdef BSLS_COMPILERFEATURES_SUPPORT_GENERALIZED_INITIALIZERS
        if (veryVerbose)
          cout << "assign(initializer_list<Json> l);\n";
        mX.assign({GOOD});
#else
        if (veryVerbose)
          cout << "init list assign member elided.\n";
        continue;
#endif
      } break;
      case 'd': {
        if (veryVerbose)
          cout << "assign(INPUT_ITERATOR first, INPUT_ITERATOR last);\n";
    ...
    for (char cfg = 'i'; cfg <= 'm'; ++cfg) {
      ...
      case 'k': {
        if (veryVerbose)
          cout << "Iter erase(ConstIter f, ConstIter l);\n";
        Obj::Iterator r = mX.erase(X.begin() + 1, X.end());
        ASSERT(r == mX.end());
      } break;
      case 'l': {
        if (veryVerbose)
          cout << "Iter erase(size_t i);\n";
    ...
    for (char cfg = 'n'; cfg <= 't'; ++cfg) {
      ...
      case 'n': {
        if (veryVerbose)
          cout << "resize(size_t c, const Json& j);\n";
        mX.resize(1, GOOD);
      } break;
      case 'o': {
        if (veryVerbose)
          cout << "Iter insert(size_t i, const Json& j);\n";
        Obj::Iterator r = mX.insert(index, GOOD);
        ASSERT(r == mX.begin());
      } break;
      case 'p': {
        if (veryVerbose)
          cout << "Iter insert(size_t i, INPUT_ITER f, INPUT_ITER l);\n";
        Obj::Iterator r = mX.insert(index, &GOOD, &GOOD + 1);
        ASSERT(r == mX.begin());
      } break;
      case 'q': {
        if (veryVerbose)
          cout << "Iter insert(size_t i, MovableRef<Json> j);\n";
        Json from = GOOD;

        Obj::Iterator r = mX.insert(index, bslmf::MovableRefUtil::move(from));
        ASSERT(r == mX.begin());
      } break;
      case 'r': {
        if (veryVerbose)
          cout << "Iter insert(ConstIter p, const Json& j);\n";
        Obj::Iterator r = mX.insert(X.begin(), GOOD);
        ASSERT(r == mX.begin());
      } break;
      case 's': {
        if (veryVerbose)
          cout << "Iter insert(ConstIter p, INPUT_ITER f, INPUT_ITER l);\n";
        Obj::Iterator r = mX.insert(X.begin(), &GOOD, &GOOD + 1);
        ASSERT(r == mX.begin());
      } break;
      case 't': {
        if (veryVerbose)
          cout << "Iter insert(ConstIter p, MovableRef<Json> j);\n";
        Json from = GOOD;

        Obj::Iterator r =
            mX.insert(X.begin(), bslmf::MovableRefUtil::move(from));
        ASSERT(r == mX.begin());
      } break;
      default: {
        BSLS_ASSERT_OPT(!"Bad test case config.");
  ...
  case 12: {
    ...
    if (verbose)
      cout << "JSONARRAY BSLX STREAMING" << endl
           << "========================" << endl;
  ...
  case 9: {
           ...
           << endl;
    {
      ...
      const Json ALLOCATING_DATA[] = {
          Json(bsl::string("a_" SUFFICIENTLY_LONG_STRING, &scratch), &scratch),
    ...
    for (int ti = 0; ti < NUM_DATA; ++ti) {
      ...
      const char *const NUMBER1 = DATA[ti].d_number_p;
      const char *const STRING1 = DATA[ti].d_string_p;
      const Json ARRAY1[3] = {Json(BOOL1), Json(JsonNumber(NUMBER1)),
                              Json(STRING1)};
      ...
      for (int tj = 0; tj < NUM_DATA; ++tj) {
        ...
        const char *const NUMBER2 = DATA[tj].d_number_p;
        const char *const STRING2 = DATA[tj].d_string_p;
        const Json ARRAY2[3] = {Json(BOOL2), Json(JsonNumber(NUMBER2)),
                                Json(STRING2)};
  ...
  case 8: {
      ...
      cout << "\nCreate objects with various allocator configurations." << endl;
    {
      ...
      for (int ti = 0; ti < NUM_DATA; ++ti) {
        ...
        const char *const NUMBER = DATA[ti].d_number_p;
        const char *const STRING = DATA[ti].d_string_p;
        const Json ARRAY[3] = {Json(BOOL), Json(JsonNumber(NUMBER)),
                               Json(STRING)};
...
#define SP " "
        {L_, 0, 0,
         "[" NL "null," NL "1," NL "[" NL "2," NL "\"inner\"" NL "]," NL
         "\"outer\"" NL "]"},
        ...
        {L_, 0, -8,
         "[" NL "    null," NL "    1," NL "    [" NL "        2," NL
         "        \"inner\"" NL "    ]," NL "    \"outer\"" NL "]"},
  ...
  case 4: {
           ...
           << "Verify all basic accessors report expected values." << endl;
    {
      ...
      if (verbose)
        cout << endl << "JsonArray::cend" << endl;
      ASSERTV((X.cend() - 1)->type(), (X.cend() - 1)->isString());
      ASSERTV((X.cend() - 1)->theString(), A3 == (X.cend() - 1)->theString());
          ...
          << endl;
    {
      ...
      for (int ti = 0; ti < NUM_DATA; ++ti) {
        ...
        for (char cfg = 'a'; cfg <= 'c'; ++cfg) {
          ...
          ASSERTV(LINE, CONFIG, BOOL, X[0], BOOL == X[0].theBoolean());
          ...
          ASSERTV(LINE, CONFIG, STRING, X[2], STRING == X[2].theString());
      ...
      cout << "\nTesting with injected exceptions." << endl;
    {
      for (int ti = 0; ti < NUM_DATA; ++ti) {
        ...
        BSLMA_TESTALLOCATOR_EXCEPTION_TEST_BEGIN(sa) {
          ...
          ASSERTV(LINE, ti, BOOL, obj[0], BOOL == obj[0].theBoolean());
          ...
          ASSERTV(LINE, ti, STRING, obj[2], STRING == obj[2].theString());
