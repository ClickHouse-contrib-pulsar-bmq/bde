...
static inline bool isNotContinuation(char value)
...
static inline bool isSurrogateValue(int value)
...
static inline int get2ByteValue(const char *pc)
...
static inline int get3ByteValue(const char *pc)
// Return the integral value of the single code point represented by the
...
static inline int get4ByteValue(const char *pc)
// Return the integral value of the single code point represented by the
...
// sequence describing a single valid code point.
{
  return ((*pc & 0x7) << 18) | ((pc[1] & k_CONT_VALUE_MASK) << 12) |
         ((pc[2] & k_CONT_VALUE_MASK) << 6) | (pc[3] & k_CONT_VALUE_MASK);
}
...
// 'bsl::strlen(string)' Unicode code points.
{
  ...
  while (true) {
    ...
    case 0xe: {
      ...
      if (UNLIKELY((value < k_MIN_3_BYTE_VALUE) || isSurrogateValue(value))) {
        BSLS_PERFORMANCEHINT_UNLIKELY_HINT;
    ...
    case 0xf: {
      ...
      if (UNLIKELY((value < k_MIN_4_BYTE_VALUE) || (value > k_MAX_VALID))) {
        BSLS_PERFORMANCEHINT_UNLIKELY_HINT;

        *invalidString = string;
        return value > k_MAX_VALID ? k_VALUE_LARGER_THAN_0X10FFFF
...
// 'length' Unicode code points.
{
  ...
  while (pc <= pcEnd4) {
    ...
    case 0xd: {
      ...
      if (UNLIKELY(isNotContinuation(pc[1]) || (value < k_MIN_2_BYTE_VALUE))) {
        BSLS_PERFORMANCEHINT_UNLIKELY_HINT;
    ...
    case 0xe: {
      ...
      if (UNLIKELY(isNotContinuation(pc[1]) || isNotContinuation(pc[2]) ||
                   (value < k_MIN_3_BYTE_VALUE) || isSurrogateValue(value))) {
        BSLS_PERFORMANCEHINT_UNLIKELY_HINT;
    ...
    case 0xf: {
      ...
      if (UNLIKELY(bool(0x8 & *pc) || isNotContinuation(pc[1]) ||
                   isNotContinuation(pc[2]) || isNotContinuation(pc[3]) ||
                   (value < k_MIN_4_BYTE_VALUE) || (value > k_MAX_VALID))) {
        BSLS_PERFORMANCEHINT_UNLIKELY_HINT;
      ...
      if (UNLIKELY(isNotContinuation(pc[1]) || isNotContinuation(pc[2]) ||
        ...
        *invalidString = pc;
        return (isNotContinuation(pc[1]) || isNotContinuation(pc[2]))
                   ? k_NON_CONTINUATION_OCTET
               : value < k_MIN_3_BYTE_VALUE ? k_OVERLONG_ENCODING
                                            : k_SURROGATE; // RETURN
      }
...
int Utf8Util_ImpUtil::getLineAndColumnNumber(
    Uint64 *lineNumber, Uint64 *utf8Column, Uint64 *startOfLineByteOffset,
    bsl::streambuf *input, Uint64 byteOffset, char lineDelimeter,
    char *temporaryReadBuffer, int temporaryReadBufferNumBytes) {
  ...
  while (true) {
    ...
    bsl::streamsize numRead = input->sgetn(buffer + remainder, attemptToRead);

    Uint64 onePastLastPosInBuffer = currentPosition + numRead + remainder;
    remainder = 0;
    if ((numRead < attemptToRead) && (targetOffset >= onePastLastPosInBuffer)) {
    ...
    while (currentPosition < onePastLastPosInBuffer) {
      ...
      if (!isValid) {
        ...
        switch (remainder) {
        case 3:
          buffer[2] = position[2]; // FALLTHROUGH
        case 2:
          buffer[1] = position[1]; // FALLTHROUGH
        case 1:
          buffer[0] = position[0];
          break; // BREAK
        default:
...
Utf8Util::IntPtr Utf8Util::advanceIfValid(int *status, const char **result,
                                          const char *string,
  ...
  for (; true; ++ret, string = next) {
    ...
    case 0xd: {
      ...
      if (UNLIKELY(isNotContinuation(*next))) {
        ...
        *status = !*next ? k_END_OF_INPUT_TRUNCATION : k_NON_CONTINUATION_OCTET;
        break;
      }
...
Utf8Util::IntPtr Utf8Util::advanceIfValid(int *status, const char **result,
                                          const char *string, size_type length,
                                          IntPtr numCodePoints) {
  BSLS_ASSERT(status);
  BSLS_ASSERT(result);
...
Utf8Util::IntPtr Utf8Util::advanceRaw(const char **result, const char *string,
                                      IntPtr numCodePoints) {
  BSLS_ASSERT(result);
  BSLS_ASSERT(string);
  BSLS_ASSERT(numCodePoints >= 0);
...
Utf8Util::IntPtr Utf8Util::advanceRaw(const char **result, const char *string,
                                      size_type length, IntPtr numCodePoints) {
  BSLS_ASSERT(result);
  BSLS_ASSERT(string || 0 == length);
  BSLS_ASSERT(numCodePoints >= 0);
...
int Utf8Util::appendUtf8CodePoint(std::experimental::pmr::string *output,
...
bool Utf8Util::isValid(const char **invalidString,
  ...
  return validateAndCountCodePoints(invalidString, string.data(),
                                    string.length()) >= 0;
}

bool Utf8Util::isValidCodePoint(int *status, const char *codePoint,
                                size_type numBytes) {
  BSLS_ASSERT(status);
  BSLS_ASSERT(codePoint);
...
Utf8Util::IntPtr
Utf8Util::numCodePointsIfValid(const char **invalidString,
                               const bsl::string_view &string) {
  BSLS_ASSERT(invalidString);
...
Utf8Util::size_type Utf8Util::readIfValid(int *status, char *outputBuffer,
  ...
  while (true) {
    ...
    *tmpPtr++ = static_cast<char>(c);
    ...
    case 0xe: {
      ...
      if (UNLIKELY((0 != rc) || (value < k_MIN_3_BYTE_VALUE) ||
    ...
    case 0xf: {
      ...
      if (UNLIKELY((0 != rc) || (value < k_MIN_4_BYTE_VALUE) ||
                   (value > k_MAX_VALID))) {
        BSLS_PERFORMANCEHINT_UNLIKELY_HINT;
    ...
    const pos_type pos = input->pubseekoff(tmpBuf - tmpPtr, bsl::ios_base::cur,
                                           bsl::ios_base::in);
