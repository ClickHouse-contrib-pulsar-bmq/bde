...
static const char *const charUtf8MultiLang = (const char *)utf8MultiLang;

enum { NUM_UTF8_MULTI_LANG_CODE_POINTS = 11781 };

static bsl::string dumpStr(const bsl::string &str)
// Returns the specified 'str' in a human-readable hex format.
{
  ...
  for (const char *pc = begin; pc < end; ++pc) {
    ...
    for (int shiftDown = 4; shiftDown >= 0; shiftDown -= 4) {
      ...
      ret.push_back(digit < 10 ? char('0' + digit) : char('a' + digit - 10));
    }
  }
...
static inline int intAbs(int x)
...
static inline unsigned int randUnsigned()
...
static void appendRand1Byte(bsl::string *dst)
...
static void appendRand2Byte(bsl::string *dst)
...
static void appendRand3Byte(bsl::string *dst)
...
static void appendRand4Byte(bsl::string *dst)
...
static void appendRandCorrectCodePoint(bsl::string *dst, bool useZero,
                                       int numBytes = -1)
...
// outside the range '[ 1 .. 4 ]' is specified.
{
  ...
  if (-1 == numBytes) {
    ...
    r = useZero && 1 == numBytes && 0 == u::randUnsigned() % 5 ? 0 : numBytes;
  }
...
static bsl::string code16(int b)
...
static bsl::string code24(int b)
...
static bsl::string code32(int b)
...
static bsl::string utf8Encode(int b)
...
static unsigned int decode8(const char *pc)
...
static unsigned int decode16(const char *pc)
...
static unsigned int decode24(const char *pc)
...
static unsigned int decode32(const char *pc)
...
// the specified 'pc'.
{
  ...
  return ((0x7 & *pc) << 18) | ((0x3f & pc[1]) << 12) | ((0x3f & pc[2]) << 6) |
         (0x3f & pc[3]);
}

static unsigned int decode(const char **pc)
...
static int intendedSequenceLength(char firstChar)
...
{
  u::randAccum = u::randAccum * 6364136223846793005LL + 1442695040888963407LL;
...
static int randVal8(bool never0 = false)
...
static int randVal24(bool neverSurrogates = false)
...
// in the '[0xd800..0xdfff]' surrogates range.
{
  ...
  } while (ret < 0x800 || (neverSurrogates && ret >= 0xd800 && ret <= 0xdfff));

  return ret;
}

static int randVal32()
...
static int randValue(bool strict = false, bool never0 = false)
...
static bool allValid(const bsl::string &str)
...
// valid and 'false' otherwise.
{
  ...
  ASSERT(a ==
         (0 <= Obj::numCodePointsIfValid(&invalid, str.data(), str.length())));
  ASSERT(a == !invalid);

  invalid = 0;
  ASSERT(a == (0 <= Obj::numCodePointsIfValid(&invalid, str.c_str())));
  ASSERT(a == !invalid);

  return a;
}

static IntPtr allNumCodePoints(const bsl::string &str)
...
static IntPtr allNumCodePointsIfValid(const char **invalidPointArg,
...
// 'invalidPointArg'
{
  *invalidPointArg = 0;
  ...
  ASSERT(ret == Obj::numCodePointsIfValid(&invalidPointLocal, str.data(),
                                          str.length()));
  ASSERT(invalidPointLocal == *invalidPointArg);

  invalidPointLocal = 0;
  ASSERT(ret == Obj::numCharactersIfValid(&invalidPointLocal, str.data(),
                                          str.length()));
  ASSERT(invalidPointLocal == *invalidPointArg);

  invalidPointLocal = 0;
  ASSERT(ret == Obj::numCharactersIfValid(&invalidPointLocal, str.c_str()));
  ASSERT(invalidPointLocal == *invalidPointArg);

  return ret;
}

static IntPtr allAdvanceIfValid(int *statusArg, const char **invalidPointArg,
                                const bsl::string &str)
...
// 'invalidPointArg' to the address of the first UTF-8 error.
{
  ...
  const IntPtr ret =
      Obj::advanceIfValid(statusArg, invalidPointArg, str.c_str(), infinity);
  ASSERT(ret == Obj::advanceIfValid(&statusLocal, &invalidPointLocal,
                                    str.data(), str.length(), infinity));
...
static bsl::string clone(const char *pc, int length)
...
//..
int utf8StreambufToString(bsl::string *output, bsl::streambuf *sb)
...
// call, in no case will 'output' contain any invalid UTF-8.
{
  ...
  while (true) {
    ...
    int status;
    IntPtr numBytes = bdlde::Utf8Util::readIfValid(&status, &(*output)[len],
                                                   k_READ_LENGTH, sb);
...
const int surrogates[] = {0xd800, 0xd811, 0xd9a3, 0xd9ff, 0xda00, 0xda35,
                          0xdb80, 0xdbff, 0xdc00, 0xdc48, 0xdd84, 0xddff,
...
static const struct {
  ...
  int d_isValid; // 1 if valid UTF-8; 0 otherwise
    ...
    {L_, "\xc2\x80 ", 3, 2, -1, 1},
    {L_, "\xc3\xbf", 2, 1, -1, 1},
    {L_, "\x01\x20\x7f\xc3\xbf\xdf\xbf\xe0\xa0\x80\xef\xbf\xbf", 13, 7, -1, 1},

    {L_, "\x01\x20\x7f\xc3\xbf\xdf\xbf\xe0\xa0\x80\xef", 11, EIT, 10, 0},

    {L_, "\x01\x20\x7f\xc3\xbf\xdf\xbf\xe0\xa0\x80\xef\xbf", 12, EIT, 10, 0},

    {L_, "\xef\xbf\xbf\xe0\xa0\x80\xdf\xbf\xc3\xbf\x7f\x20\x01", 13, 7, -1, 1},
...
// this component and return zero.
{
  ...
  case 9: {
    ...
    char *outputBuf = static_cast<char *>(bsl::malloc(OUT_BUF_SIZE));
  ...
  case 7: {
    ...
    ASSERT(
        (0 <= numCodePointsReturned && 0 == result) ||
        (0 > numCodePointsReturned && nullBegin <= result && nullEnd > result));
  ...
  case 3: {
    ...
    if (0 == status) {
      IntPtr traversed = Obj::advanceRaw(&result, begin, length, numCodePoints);
    ...
    if (0 == status) {
      IntPtr traversed = Obj::advanceRaw(&result, nullBegin, numCodePoints);
  ...
  case 2: {
    ...
    status = 0;
    result = 0;
    traversed = Obj::advanceIfValid(&status, &result, nullBegin, numCodePoints);
...
int main(int argc, char *argv[]) {
  ...
  case 20: {
    ...
    if (verbose)
      cout << "USAGE EXAMPLE 3: 'readIfValid'\n"
              "==============================\n";
  ...
  case 19: {
    ...
    //..
    // Next, try advancing 2 code points, then 3, then 4, observing that the
    // value returned is the number of Unicode code points advanced.  Note that
    // since we're only advancing over valid UTF-8, we can use either
    // 'advanceRaw' or 'advanceIfValid':
    //..
    ...
    // 'advanceIfValid', and wind up stopping when we encounter invalid input.
    // The behavior of 'advanceRaw' is undefined if it is used on invalid input,
    // so we cannot use it here.  Also note that we will stop at the beginning
    // of the invalid Unicode code point, and not at the first incorrect byte,
    // which is two bytes later:
    //..
    ...
    //..
    // Now, doctor the string to replace the invalid code point with a valid
    // one, so the string is entirely correct UTF-8:
    //..
    ...
    // 'advanceIfValid' does not return an error (non-zero) value to 'status'
  ...
  case 18: {
    ...
    if (verbose)
      cout << "USAGE EXAMPLE 1: 'isValid' AND 'numCodePoints*'\n"
              "===============================================\n";
    ...
    ASSERT(9 ==
           bdlde::Utf8Util::numCodePointsRaw(string.data(), string.length()));
    ASSERT(9 == bdlde::Utf8Util::numCodePointsRaw(string.c_str()));
    //..
    // Next, we encode a lone surrogate value, '0xd8ab', that we encode as the
    // raw 3-byte sequence "\xed\xa2\xab" to avoid validation:
    //..
    ...
    // 'stringWithSurrogate', since the behavior of that method is undefined
    // unless the string is valid.  Instead, the 'numCodePointsIfValid' method
    // can be used on strings whose validity we are uncertain of:
    //..
    ...
    ASSERT(10 == bdlde::Utf8Util::numCodePointsRaw(stringWithNull.data(),
                                                   stringWithNull.length()));
    //..
    // Finally, we encode '0x3a' (':') as an overlong value using 2 bytes, which
    // is not valid UTF-8 (since ':' can be "encoded" in 1 byte):
    //..
  ...
  case 17: {
    ...
    for (int i = 0; i < NUM_DATA; ++i) {
      ...
      const char *errorPosition =
          (ERROR_OFFSET == -1) ? end : INPUT + ERROR_OFFSET;
      ...
      while (position < errorPosition &&
        ...
        bool isValid =
            Obj::isValidCodePoint(&status, position, NUM_BYTES - offset);

        int EXPECTED_NUM_BYTES = Obj::numBytesInCodePoint(position);
        ASSERTV(LINE, INPUT, offset, status, isValid);
        ASSERTV(LINE, INPUT, offset, status, EXPECTED_NUM_BYTES == status);
...
#define U4_4 0xa1
    ...
    const unsigned char documentPtr[] = {
        ...
        U2_2, U2_1, U2_2, U2_1, U2_2, U2_1, U2_2, U1_1, '\n', U3_1, U3_2,
        U3_3, U3_1, U3_2, U3_3, U3_1, U3_2, U3_3, '\n', U4_1, U4_2, U4_3,
        U4_4, U4_1, U4_2, U4_4, U4_4, U1_1, '\n', 0};
    ...
    }
    {
      ...
      for (int bufferSize = 4; bufferSize < 20; ++bufferSize) {
        ...
        for (int i = 0; i < NUM_DATA; ++i) {
          ...
          int rc = ImpUtil::getLineAndColumnNumber(
              &actualLine, &actualColumn, &actualStartLineOffset, &stringbuf,
              OFFSET, '\n', buffer, bufferSize);
    ...
    }
    {
      ...
      rc = Obj::getLineAndColumnNumber(&actualLine, &actualColumn,
                                       &actualStartLineOffset, &stringbuf, 1,
                                       '\n');

      ASSERTV(rc, 0 == rc);

      rc = Obj::getLineAndColumnNumber(&actualLine, &actualColumn,
                                       &actualStartLineOffset, &stringbuf, 3,
                                       '\n');
...
#undef T
  ...
  case 15: {
...

    {
      ...
      if (verbose)
        cout << "ifstr about to be destroyed\n";
    }

    if (verbose)
      cout << "ifstr was destroyed\n";
  ...
  case 14: {
...
#ifdef BDE_BUILD_TARGET_EXC
    {
      ...
      for (int ncp = 0; ncp < 2; ++ncp) {
        ...
        {
          ASSERT_PASS(rc = Obj::advanceIfValid(&status, &result, WOOF, NCP));
          ASSERT(NCP == rc);
          ASSERT(0 == status);
          ...
          ASSERT_FAIL(Obj::advanceIfValid(&status, &result, nullStr, NCP));
          ASSERT(INIT_INT == status);
        ...
        }
        {
          ...
          ASSERT_PASS(rc = Obj::advanceIfValid(&status, &result, WOOF, 4, NCP));
          ASSERT(NCP == rc);
          ASSERT(0 == status);
          ASSERT(WOOF + NCP == result);
          ...
          ASSERT_PASS(
              rc = Obj::advanceIfValid(&status, &result, nullStr, 0, NCP));
          ASSERT(0 == rc);
          ASSERT(0 == status);
          ASSERT(0 == result);
          ...
          ASSERT_FAIL(Obj::advanceIfValid(&status, &result, nullStr, 1, NCP));
          ASSERT(INIT_INT == status);
          ASSERT(INIT_RESULT == result);
        ...
        }
        {
          ...
          ASSERT_PASS(rc = Obj::advanceRaw(&result, nullStr, 0, NCP));
          ASSERT(0 == rc);
          ASSERT(0 == result);
        ...
        cout << "appendUtf8CodePoint\n";
      {
        ...
        ASSERT_FAIL(
            Obj::appendUtf8CodePoint(static_cast<bsl::string *>(0), 'a'));
      }

      if (verbose)
        cout << "appendUtf8Character\n";
      {
        bsl::string s;
        ...
        ASSERT_FAIL(
            Obj::appendUtf8Character(static_cast<bsl::string *>(0), 'a'));
      }
      ...
      if (verbose)
        cout << "isValid -- with result, no length\n";
      {
      ...
      if (verbose)
        cout << "isValid -- with result and length\n";
      {
      ...
      if (verbose)
        cout << "numCharacters -- with length\n";
      {
      ...
      if (verbose)
        cout << "numCharactersIfValid -- no length\n";
      {
      ...
      if (verbose)
        cout << "numCharactersIfValid -- with length\n";
      {
        ...
        ASSERT_PASS(rc = Obj::numCharactersIfValid(&result, nullStr, 0));
        ASSERT(0 == rc);
        ASSERT(INIT_RESULT == result);
      ...
      if (verbose)
        cout << "numCodePointsRaw -- no length\n";
      {
      ...
      if (verbose)
        cout << "numCodePointsRaw -- with length\n";
      {
      ...
      if (verbose)
        cout << "numCodePointsIfValid -- no length\n";
      {
      ...
      if (verbose)
        cout << "numCodePointsIfValid -- with length\n";
      {
        ...
        ASSERT_PASS(rc = Obj::numCodePointsIfValid(&result, nullStr, 0));
        ASSERT(0 == rc);
        ASSERT(INIT_RESULT == result);
  ...
  case 13: {
    ...
    static const struct Data {
      ...
      const char *d_expStr_p;
        ...
        {Obj::k_UNEXPECTED_CONTINUATION_OCTET, "UNEXPECTED_CONTINUATION_OCTET"},
  ...
  case 12: {
    ...
    //   int appendUtf8CodePoint(std::experimental::pmr::string *, unsigned
    //   int); int appendUtf8Character(bsl::string *, unsigned int);
    // --------------------------------------------------------------------
  ...
  case 11: {
    ...
    for (int ti = 0; ti < u::NUM_INTERESTING_CODEPOINTS; ++ti) {
      ...
      const char *UTF8 = u::legalCodepointData[ti].d_utf8_p;
      bsl::size_t UTF8_LEN = strlen(UTF8);
      const int VALUE = static_cast<int>(u::legalCodepointData[ti].d_codePoint);
  ...
  case 10: {
    ...
    for (int ti = 0; ti < u::NUM_INTERESTING_CODEPOINTS; ++ti) {
      ...
      ASSERT(Obj::IntPtr(UTF8.length()) == Obj::numBytesRaw(UTF8, 1));

      ASSERT(Obj::IntPtr(UTF8.length()) == Obj::numBytesIfValid(UTF8, 1));
      ...
      for (int tj = 0; tj < u::NUM_INTERESTING_CODEPOINTS; ++tj) {
        ...
        ASSERT(Obj::IntPtr(UTF8.length()) == Obj::numBytesRaw(UTF8_2, 1));

        // Embedded nulls count as 1 byte.
        ASSERT(1 + Obj::IntPtr(UTF8.length()) == Obj::numBytesRaw(UTF8_2, 2));

        ASSERT(Obj::IntPtr(UTF8_2.length()) == Obj::numBytesRaw(UTF8_2, 3));

        // Adding 2nd code point doesn't change the answer if we only
        // ask for the 1st code point's byte length.
        ASSERT(Obj::IntPtr(UTF8.length()) == Obj::numBytesIfValid(UTF8_2, 1));

        // Embedded nulls count as 1 byte.
        ASSERT(1 + Obj::IntPtr(UTF8.length()) ==
               Obj::numBytesIfValid(UTF8_2, 2));

        ASSERT(Obj::IntPtr(UTF8_2.length()) == Obj::numBytesIfValid(UTF8_2, 3));
      }
    }
  } break;
  case 9: {
    ...
    for (int tj = 0; tj < k_NUM_ITERATIONS; ++tj) {
      ...
      const int numCorrectStrCodePoints = tj ? u::randUnsigned() % 4 + 2 : 0;
      ...
      for (int ti = 0; ti < NUM_DATA; ++ti) {
        ...
        Obj::ErrorStatus ERROR_STATUS = static_cast<Obj::ErrorStatus>(STATUS);
        ...
        if (EIT == ERROR_STATUS &&
          ...
          const int intendedLen = u::intendedSequenceLength(ERROR_STR[OFFSET]);
          ASSERT(len < intendedLen);
          ASSERT(intendedLen - len <= 3);

          errorStr.assign(ERROR_STR, OFFSET);
          u::appendRandCorrectCodePoint(&errorStr, false, intendedLen);
        ...
        const char *const endOfValid = begin + correctStr.length() + OFFSET;
        const char *end;
        IntPtr numCodePoints;
        int sts;

        const IntPtr numOffsetCodePoints =
            Obj::numCodePointsIfValid(&end, errorStr.c_str(), OFFSET);
        ASSERT(0 <= numOffsetCodePoints);

        const IntPtr numCodePointsExp =
            numCorrectStrCodePoints + numOffsetCodePoints;

        if (!zeroUsed) {
          sts = -11;
          end = "woof";
          numCodePoints = Obj::advanceIfValid(&sts, &end, begin, INT_PTR_MAX);
        ...
        numCodePoints =
            Obj::advanceIfValid(&sts, &end, begin, str.length(), INT_PTR_MAX);
        ASSERT(endOfValid == end);
        ASSERT(numCodePointsExp == numCodePoints);
        ...
        numCodePoints = Obj::numCodePointsIfValid(&end, begin, str.length());
        ASSERT(endOfValid == end);
        ASSERTV(LINE, ERROR_STATUS, numCodePoints,
                ERROR_STATUS == numCodePoints);
        ...
        numCodePoints = Obj::advanceIfValid(&sts, &end, str, INT_PTR_MAX);
        ASSERT(endOfValid == end);
        ASSERT(numCodePointsExp == numCodePoints);
        ASSERTV(LINE, ERROR_STATUS, sts, ERROR_STATUS == sts);
        ...
        for (IntPtr endOutLen = intStrLen + 4,
          ...
          for (int tk = 0; tk < 2; ++tk) {
            ...
            IntPtr intBufLen = Obj::readIfValid(&sts, &out[0], outLen, &fsb);
            ASSERT(0 <= intBufLen);

            ASSERTV(u::dumpStr(str), intBufLen, outLen, intBufLen <= outLen);
            ASSERTV(u::dumpStr(str), intBufLen, outLen, intBufLen <= validLen);
            ...
            ASSERTV(u::dumpStr(out), Obj::isValid(&out[0], intBufLen));

            if (CHECK_WITH_SEEK) {
              ASSERT(fsb.pubseekoff(0, bsl::ios_base::cur, bsl::ios_base::in) ==
            ...
            ASSERTV(u::dumpStr(str.substr(intBufLen)), u::dumpStr(remainderStr),
        ...
        zeroUsed =
            zeroUsed || (useZero && bsl::count(str.begin(), str.end(), '\0'));
        ...
        ASSERT(endOfValid == end);
        ASSERT(numCodePointsExp == numCodePoints);
        ASSERTV(LINE, ERROR_STATUS, sts, str[str.length() - 1],
                ERROR_STATUS == sts);
        ...
        numCodePoints = Obj::numCodePointsIfValid(&end, begin, str.length());
        ASSERT(endOfValid == end);
        ASSERTV(LINE, u::dumpStr(errorStr), ERROR_STATUS, numCodePoints,
                ERROR_STATUS == numCodePoints);
  ...
  case 8: {
    ...
    for (int tj = 0; tj < k_NUM_ITERATIONS; ++tj) {
      ...
      if (veryVeryVerbose ||
          (u::intAbs(tj - k_NUM_USE_ZERO_ITERATIONS) < 100 && veryVerbose)) {
        P(u::dumpStr(str));
      }
      ...
      numCodePoints = Obj::advanceRaw(&end, begin, strLength, INT_PTR_MAX);
      ASSERT(endOfString == end);
      ASSERT(numCodePointsExp == numCodePoints);

      end = "woof";
      numCodePoints = Obj::advanceRaw(&end, strView, INT_PTR_MAX);
      ASSERT(endOfString == end);
      ASSERT(numCodePointsExp == numCodePoints);
      ...
      numCodePoints =
          Obj::advanceIfValid(&sts, &end, begin, strLength, INT_PTR_MAX);
      ASSERT(endOfString == end);
      ASSERT(numCodePointsExp == numCodePoints);
      ASSERT(0 == sts);
      ...
      numCodePoints = Obj::advanceIfValid(&sts, &end, strView, INT_PTR_MAX);
      ASSERT(endOfString == end);
      ASSERT(numCodePointsExp == numCodePoints);
      ASSERT(0 == sts);
      ...
      numCodePoints = Obj::numCodePointsIfValid(&end, begin, strLength);
      ASSERT(WOOF == end);
      ASSERT(numCodePointsExp == numCodePoints);
      ...
      for (IntPtr sbLen = sbLen0; sbLen <= sbEnd;
        ...
        for (IntPtr outLen = 4; outLen <= sbLen + 4;
          ...
          ASSERTV(0 < intBufLen || 0 == sbLen || (!inputValid && sbLen < 4));
          ...
          if (inputValid) {
            ASSERT(0 <= sts);
            ASSERT((0 < sts) == (outLen - 4 < static_cast<int>(sbLen)));

            if (4 == outLen && 0 < sbLen) {
              end = WOOF;
              ASSERT(1 == Obj::numCodePointsIfValid(&end, &out[0], intBufLen));
              ASSERT(WOOF == end);
            }
          } else {
            ASSERT(0 != sts);
      ...
      if (!zeroUsed) {
        ...
        numCodePoints = Obj::advanceRaw(&end, begin, numCodePointsExp);
        ASSERT(endOfString == end);
        ASSERT(numCodePointsExp == numCodePoints);
        ...
        numCodePoints =
            Obj::advanceIfValid(&sts, &end, begin, numCodePointsExp);
        ASSERT(endOfString == end);
        ASSERT(numCodePointsExp == numCodePoints);
        ASSERT(0 == sts);
      }
      ...
      numCodePoints =
          Obj::advanceRaw(&end, begin, strLength + 1, numCodePointsExp);
      ASSERT(endOfString == end);
      ASSERT(numCodePointsExp == numCodePoints);

      sts = -11;
      end = "woof";
      numCodePoints = Obj::advanceIfValid(&sts, &end, begin, strLength + 1,
                                          numCodePointsExp);
      ASSERT(endOfString == end);
      ASSERT(numCodePointsExp == numCodePoints);
      ASSERT(0 == sts);
  ...
  case 7: {
    ...
    sts = -2;
    result = "woof";
    numCodePoints = Obj::advanceIfValid(&sts, &result, begin, length, INT_MAX);
    ...
    sts = -2;
    result = "woof";
    numCodePoints = Obj::advanceIfValid(&sts, &result, strView, INT_MAX);
    ASSERT(u::NUM_UTF8_MULTI_LANG_CODE_POINTS - 1 == numCodePoints);
    ASSERT(result == end);
    ASSERT(0 == sts);
    ...
    result = "woof";
    numCodePoints =
        Obj::advanceIfValid(&sts, &result, begin, length + 1, expectedNumCPs);
    ASSERT(expectedNumCPs == numCodePoints);
    ASSERT(result == end);
    ASSERT(0 == sts);
    ...
    result = "woof";
    numCodePoints =
        Obj::advanceIfValid(&sts, &result, begin, 0, expectedNumCPs);
    ...
    result = "woof";
    numCodePoints = Obj::advanceRaw(&result, begin, length + 1, expectedNumCPs);
    ASSERT(expectedNumCPs == numCodePoints);
    ASSERT(result == end);

    result = "woof";
    numCodePoints = Obj::advanceRaw(&result, begin, 0, expectedNumCPs);
    ASSERT(0 == numCodePoints);
    ASSERT(result == begin);
  } break;
  case 6: {
    ...
    for (int ti = 0; ti < NUM_DATA; ++ti) {
      ...
      ERRSTR = 0;
      LOOP_ASSERT(LINE, VALID == Obj::isValid(&ERRSTR, UTF8, NUMBYTES + 1));
      ...
      ERRSTR = 0;
      LOOP_ASSERT(LINE, NUMCPS == Obj::numCodePointsIfValid(&ERRSTR, UTF8));
      ...
      ERRSTR = 0;
      LOOP_ASSERT(LINE,
                  NUMCPS == Obj::numCharactersIfValid(&ERRSTR, UTF8, NUMBYTES));

      ERRSTR = 0;
      ASSERTV(LINE, NUMCPS, rc,
              NUMCPS ==
                  (rc = Obj::numCodePointsIfValid(&ERRSTR, UTF8, NUMBYTES)));
      ...
      ERRSTR = 0;
      if (VALID) {
        LOOP_ASSERT(LINE, NUMCPS + 1 == Obj::numCharactersIfValid(
                                            &ERRSTR, UTF8, NUMBYTES + 1));
        LOOP_ASSERT(LINE, NUMCPS + 1 == Obj::numCodePointsIfValid(
                                            &ERRSTR, UTF8, NUMBYTES + 1));
        LOOP_ASSERT(LINE, 0 == ERRSTR);
      } else {
        LOOP_ASSERT(LINE,
                    Obj::numCharactersIfValid(&ERRSTR, UTF8, NUMBYTES + 1) < 0);
        LOOP_ASSERT(LINE,
                    Obj::numCodePointsIfValid(&ERRSTR, UTF8, NUMBYTES + 1) < 0);
        LOOP_ASSERT(LINE, ERROFF == ERRSTR - UTF8);
      }
    }
  ...
  case 5: {
    ...
    if (verbose)
      cout << "TESTING 'numCharactersRaw'" << endl
           << "==========================" << endl;
    ...
    for (int ti = 0; ti < NUM_DATA; ++ti) {
      ...
      LOOP_ASSERT(LINE, NUMCPS == Obj::numCodePointsRaw(UTF8));

      LOOP_ASSERT(LINE, NUMCPS == Obj::numCodePointsRaw(UTF8, NUMBYTES));
      LOOP_ASSERT(LINE,
                  NUMCPS + 1 == Obj::numCodePointsRaw(UTF8, NUMBYTES + 1));

      // DEPRECATED
      LOOP_ASSERT(LINE, NUMCPS == Obj::numCharactersRaw(UTF8));

      LOOP_ASSERT(LINE, NUMCPS == Obj::numCharactersRaw(UTF8, NUMBYTES));
      LOOP_ASSERT(LINE,
                  NUMCPS + 1 == Obj::numCharactersRaw(UTF8, NUMBYTES + 1));

      LOOP_ASSERT(LINE, NUMCPS == Obj::numCharacters(UTF8));

      LOOP_ASSERT(LINE, NUMCPS == Obj::numCharacters(UTF8, NUMBYTES));
      LOOP_ASSERT(LINE, NUMCPS + 1 == Obj::numCharacters(UTF8, NUMBYTES + 1));
    }
  ...
  case 4: {
    ...
    if (verbose)
      cout << "TESTING SURROGATES" << endl << "==================" << endl;
    ...
    for (int i = 5000; i > 0; --i) {
      ...
      ASSERT(numLeading == u::allAdvanceIfValid(&sts, &invalidPoint, str));
      ASSERT(Obj::k_SURROGATE == sts);
      ASSERT(str.c_str() + preLen == invalidPoint);

      invalidPoint = 0;
      ASSERT(Obj::k_SURROGATE ==
             u::allNumCodePointsIfValid(&invalidPoint, str));
      ASSERT(str.c_str() + preLen == invalidPoint);
    }
  ...
  case 3: {
    ...
    if (verbose)
      cout << "TESTING BYTE-ORDER MARK" << endl
           << "=======================" << endl;
  ...
  case 2: {
    ...
    Char encode2a[] = {b2 | 2, c};
    int val2a = 0x80;
    Char encode2b[] = {b2 | 3, c | 3};
    int val2b = 3 * 64 + 3;
    Char encode2c[] = {b2 | 0xf, c | 0xf};
    int val2c = 15 * 64 + 15;
    Char encode2d[] = {b2 | 0x10, c | 7};
    int val2d = 16 * 64 + 7;
    Char encode2e[] = {b2 | 0x1f, c | 0x3f};
    int val2e = 0x7ff;

    Char encode3a[] = {b3, c | 0x20, c | 0};
    int val3a = 0x800;
    Char encode3b[] = {b3, c | 58, c | 0};
    int val3b = 58 * 64;
    Char encode3c[] = {b3, c | 0x3f, c | 0x3f};
    int val3c = 0xfff;
    ...
    Char encode4a[] = {b4, c | 0x10, c, c};
    int val4a = 0x10000;
    Char encode4b[] = {b4, c | 0x10, c, c | 0xf};
    int val4b = 0x1000f;
    Char encode4c[] = {b4, c | 0x10, c | 0xf, c};
    int val4c = 0x103c0;
    Char encode4d[] = {b4, c | 0x10, c | 0x30, c};
    int val4d = 0x10c00;
    Char encode4e[] = {b4, c | 0x13, c, c};
    int val4e = 0x13000;
    Char encode4f[] = {b4, c | 0x1c, c, c};
    int val4f = 0x1c000;
    Char encode4g[] = {b4 | 1, c, c, c};
    int val4g = 0x40000;
    Char encode4h[] = {b4 | 3, c, c, c};
    int val4h = 0xc0000;
    Char encode4i[] = {b4 | 4, c, c, c};
    int val4i = 0x100000;
...
#undef TEST
    ...
    Char encode2o1[] = {b2 | 1, c | 0x3f};
    int val2o1 = 0x7f;
    Char encode2o2[] = {b2 | 0, c | 3};
    int val2o2 = 0x3;
    Char encode3o1[] = {b3, c | 0x1f, c | 0x3f};
    int val3o1 = 0x7ff;
    Char encode3o2[] = {b3, c | 0, c | 3};
    int val3o2 = 0x3;
    Char encode4o1[] = {b4, c | 0xf, c | 0x3f, c | 0x3f};
    int val4o1 = 0xffff;
    Char encode4o2[] = {b4, c, c, c | 3};
    int val4o2 = 0x3;
...
#undef TEST
  ...
  case 1: {
...

    {
      ...
      for (int i = 0; i <= 33; i += 3) {
        ...
        ASSERT(i / 3 ==
               Obj::numCharacters(u::clone(overStr.data(), i).c_str()));

        ASSERT(i / 3 == Obj::numCodePointsRaw(overStr.data(), i));
        ASSERT(i / 3 ==
               Obj::numCodePointsRaw(u::clone(overStr.data(), i).c_str()));
        ...
        for (int j = i + 1; j < i + 3; ++j) {
          ...
          ASSERT(!Obj::isValid(u::clone(overStr.data(), j).c_str()));
        }
      }
    }
...

    {
      ...
      for (int i = 0; i <= 22; i += 2) {
        ...
        for (int j = i + 1; j < i + 2; ++j) {
          ...
          ASSERT(false == Obj::isValid(u::clone(overStr.data(), j).c_str()));
        }
      }
    }
  ...
  case -1: {
    ...
    if (verbose)
      cout << "RANDOM NUMBER GENERATORS TEST\n"
              "=============================\n";
    ...
    for (int i = 0; i < 60; ++i) {
      for (int j = 0; j < 10; ++j) {
        cout << (j ? ", " : "") << "0x" << bsl::hex << u::randVal24(true);
      }
      cout << endl;
    }
    for (int i = 100 * 1000; i > 0; --i) {
      int j = u::randVal24(true);
      ASSERT((j >= 0x800) & (j <= 0xffff) & !((j >= 0xd800) & (j <= 0xdfff)));
    }
