...
static const bdlde::ByteOrder::Enum e_BACKWARDS =
    bdlde::ByteOrder::e_HOST == bdlde::ByteOrder::e_BIG_ENDIAN
...
inline const char16_t *usToC16(const unsigned short *us) {
...
inline bool bothAnd(bool a, bool b)
...
inline bool allAnd(bool a, bool b)
...
inline bool allAnd(bool a, bool b, bool c)
...
inline bool allAnd(bool a, bool b, bool c, bool d)
...
inline bool allAnd(bool a, bool b, bool c, bool d, bool e)
...
inline bool allAnd(bool a, bool b, bool c, bool d, bool e, bool f)
...
inline bool allAnd(bool a, bool b, bool c, bool d, bool e, bool f, bool g)
...
template <class VALUE_TYPE, bsl::size_t ARRAY_SIZE> class FixedVector {
public:
...
public:
  FixedVector() : d_size(0) {}

  explicit FixedVector(size_type initial_size) : d_size(initial_size) {}

  ~FixedVector() {}

  size_type size() const { return d_size; }

  const_iterator cbegin() const { return d_array; }

  const_iterator cend() const { return d_array + d_size; }

  const_reference operator[](size_type t) const { return d_array[t]; }

  size_type max_size() const { return SIZE; }

  size_type capacity() const { return SIZE; }

  void resize(size_type newSize) { d_size = newSize; }

  void push_back(value_type v) { d_array[d_size++] = v; }

  value_type pop_back() { return d_array[--d_size]; }

  iterator begin() { return d_array; }

  iterator end() { return d_array + d_size; }

  reference operator[](size_type t) { return d_array[t]; }
};
...
//  Non-'const':
template <class VALUE_TYPE> struct ArrayRange {
  ...
  explicit ArrayRange(FixedVector<VALUE_TYPE, LEN> &v)
  ...
  iterator begin() const { return d_array; }

  iterator end() const { return d_array + d_size; }
...
//  Const:
template <class VALUE_TYPE> struct ConstArrayRange {
  typedef VALUE_TYPE value_type;
  typedef VALUE_TYPE const &const_reference;
  typedef const value_type *const_iterator;
  typedef bsl::size_t size_type;
  ...
  template <bsl::size_t LEN>
  explicit ConstArrayRange(const FixedVector<VALUE_TYPE, LEN> &v)
      : d_array(&v[0]), d_size(v.size()) {}

  explicit ConstArrayRange(ArrayRange<VALUE_TYPE> &av)
      : d_array(av.d_array), d_size(av.d_size) {}

  template <bsl::size_t LEN>
  ConstArrayRange(const VALUE_TYPE (&a)[LEN]) : d_array(&a[0]), d_size(LEN) {}

  size_type size() const { return d_size; }

  const_reference operator[](size_type t) const { return d_array[t]; }

  const_iterator begin() const { return d_array; }

  const_iterator cbegin() const { return d_array; }

  const_iterator end() const { return d_array + d_size; }

  const_iterator cend() const { return d_array + d_size; }
};

template <class VALUE_TYPE>
inline bool operator!=(const ConstArrayRange<VALUE_TYPE> &lhs,
...
template <class ARRAY_RANGE> struct GenCheckArrRange {
  ...
  // CREATORS
  explicit GenCheckArrRange(RangeType &range) : d_range(range) {}

  size_type size() const { return d_range.size(); }
  ...
  template <class TO_TYPE> void fill(TO_TYPE *toBuffer) const {
    if (size() > 0) {
      *toBuffer = *d_range.begin(); // (Only compiles if 'TO_TYPE' and
                                    // 'value_type' are compatible.)
      memcpy(toBuffer + 1, d_range.begin() + 1,
  ...
  // 'd_range', up to the length 'd_range.size()', and false otherwise.
  {
    return 0 == size() ||
           (static_cast<value_type>(checkBuffer[0]) == d_range.begin()[0] &&
            (1 == size() || 0 == memcmp(checkBuffer + 1, d_range.begin() + 1,
                                        sizeof(value_type) * (size() - 1))));
  }
};
...
template <class SOURCE> inline const SOURCE &deChar(const SOURCE &i) {
  return i;
}
...
unsigned int deChar(unsigned char c) { return static_cast<unsigned>(c); }
...
template <class T> struct HexPrImpl {
  ConstArrayRange<T> d_av;

  HexPrImpl(const T *ptr, bsl::size_t size) : d_av(ptr, size) {}
};

template <class T> ostream &operator<<(ostream &os, const HexPrImpl<T> &t);

template <class T> struct MixedPrImpl {
  ConstArrayRange<T> d_av;

  MixedPrImpl(const T *ptr, bsl::size_t size) : d_av(ptr, size) {}
};

template <class T> ostream &operator<<(ostream &os, const MixedPrImpl<T> &t);

// Three forms of array range print-in-hex (using a HexPrImpl object)

template <class T> HexPrImpl<T> prHexRange(const T *ptr, size_t size);
template <class T> HexPrImpl<T> prHexRange(const T *first, const T *last);
template <class T, bsl::size_t N>
inline HexPrImpl<T> prHexRange(const FixedVector<T, N> &v) {
  return HexPrImpl<T>(v.begin(), v.size());
}

template <class T> inline HexPrImpl<T> prHexRange(const ArrayRange<T> &v) {
  return HexPrImpl<T>(v.begin(), v.size());
}

// Three forms of array range print-in-mixed (using a MixedPrImpl object)

template <class T> MixedPrImpl<T> prMixedRange(const T *ptr, size_t size);
template <class T> MixedPrImpl<T> prMixedRange(const T *first, const T *last);
template <class T, bsl::size_t N>
inline MixedPrImpl<T> prMixedRange(const FixedVector<T, N> &v) {
  return MixedPrImpl<T>(v.begin(), v.size());
}

template <class T> inline MixedPrImpl<T> prMixedRange(const ArrayRange<T> &v) {
  return MixedPrImpl<T>(v.begin(), v.size());
}
...
template <class ITER, int N_ITER_PARM> struct OdomIter {
  ...
  explicit OdomIter(COLLECTION *const *init)
  ...
  inline const Iter operator[](int i) const
  // Return the true iterator at position 'i'.
  {
    return d_wheels[i].d_pos;
  }

  inline const Iter begin(int i) const
  ...
  inline const Iter end(int i) const
  ...
  // or false if it is at the end of its cycle.
  {
    ...
    for (i = N_ITER - 1;; --i) {
      Wheel &w = d_wheels[i];
      if (w.d_pos != w.d_end && ++w.d_pos != w.d_end) {
        break;
      }
...
template <bsl::size_t N_CHARS_P, int FROM_SIZE_P, int TO_SIZE_P, int MARGIN_P>
struct BufferSizes {
  ...
  enum {
    N_CHARS = N_CHARS_P,     // Number of code points, of whatever size
  ...
  enum {
    FROM_BUF_SIZE = N_CHARS * FROM_SIZE + 1, // + 1 for the null.
    TO_BUF_SIZE = HighBit<N_CHARS * TO_SIZE + 2 * MARGIN + 1>::HIGH_BIT * 2
...
inline bool operator==(const ConvRslt &lhs, const ConvRslt &rhs) {
  return lhs.d_retVal == rhs.d_retVal && lhs.d_symbols == rhs.d_symbols &&
...
template <class CHAR_TYPE> struct SrcSpec {
  const CHAR_TYPE *d_source;
  CHAR_TYPE d_errorChar;
  bsl::size_t d_dstBufSize;

  SrcSpec(const CHAR_TYPE *source, CHAR_TYPE errorChar, bsl::size_t dstBufSize)
      : d_source(source), d_errorChar(errorChar), d_dstBufSize(dstBufSize) {}
};
...
template <class CHAR_TYPE> struct WorkPiece {
  bsl::size_t d_memLength;
  bsl::size_t d_margin;
  bsl::size_t d_winLength; // 2*d_margin + d_winLength <= d_memLength
  CHAR_TYPE d_fillChar;

  WorkPiece(bsl::size_t memLength, bsl::size_t winLength, CHAR_TYPE fillChar,
            bsl::size_t margin = 32)
      : d_memLength(memLength), d_margin(margin), d_winLength(winLength),
        d_fillChar(fillChar) {}

  bsl::size_t size() const { return d_winLength; }

  CHAR_TYPE *begin(CHAR_TYPE *mem) { return &mem[d_margin]; }

  CHAR_TYPE *end(CHAR_TYPE *mem) { return &mem[d_margin + d_winLength]; }
...
template <class CHAR_TYPE> struct BufferedWPiece {
  CHAR_TYPE *const d_buf;
  WorkPiece<CHAR_TYPE> d_wp;

  BufferedWPiece(CHAR_TYPE *bufp, bsl::size_t memLength, bsl::size_t winLength,
                 CHAR_TYPE fillChar, bsl::size_t margin = 32)
      : d_buf(bufp), d_wp(memLength, winLength, fillChar, margin) {}

  CHAR_TYPE *buffer() { return d_buf; }

  WorkPiece<CHAR_TYPE> &workpiece() { return d_wp; }

  bsl::size_t size() const { return d_wp.size(); }

  CHAR_TYPE *begin() { return d_wp.begin(d_buf); }

  CHAR_TYPE *end() { return d_wp.end(d_buf); }

  void fillMargins() { return d_wp.fillMargins(d_buf); }

  int checkMargins() const { return d_wp.checkMargins(d_buf); }

  int checkFinalNull() const { return d_wp.checkFinalNull(d_buf); }
};
...
static int surrogateUtf8ToUtf16(unsigned short *dstBuffer,
                                bsl::size_t dstCapacity, const char *srcBuffer,
                                bsl::size_t *numCodePointsWritten,
                                bsl::size_t *numWordsWritten,
                                unsigned short errorWord,
                                bdlde::ByteOrder::Enum byteOrder) {
  return Util::utf8ToUtf16(dstBuffer, dstCapacity, srcBuffer,
                           numCodePointsWritten, numWordsWritten, errorWord,
                           byteOrder);
}
...
public:
  // CREATOR
  explicit RandGen(unsigned startSeed = 0);
...
inline unsigned RandGen::bits(unsigned numBits) {
  ...
  const unsigned ret =
      static_cast<unsigned>(d_bitAccum & ((1u << numBits) - 1));
...
inline char RandGen::getChar() { return static_cast<char>(bits(8)); }
...
// CREATOR
RandGen::RandGen(unsigned startSeed) : d_seed(startSeed), d_bitAccumNumBits(0) {
...
template <class TO_CHAR, class FROM_CHAR> struct Conversion {
  typedef int (*Function)(TO_CHAR *dstBuf, bsl::size_t toSize,
                          const FROM_CHAR *srcBuf, bsl::size_t *toSymbols,
                          bsl::size_t *toUnits, TO_CHAR errorChar,
                          bdlde::ByteOrder::Enum byteOrder);
  ...
  explicit Conversion(Function converter) : d_converter(converter) {}

  ConvRslt operator()(WorkPiece<TO_CHAR> &to, TO_CHAR *dstBuf,
    ...
    result.d_retVal =
        (d_converter)(to.begin(dstBuf), to.d_winLength, from.d_source,
                      INF == expected.d_symbols ? 0 : &result.d_symbols,
                      INF == expected.d_units ? 0 : &result.d_units,
...
template <class TO_CHAR, class FROM_CHAR> struct ConversionArg {
...
template <class TO_CHAR, class FROM_CHAR> struct FourWayRunner {
  enum { N_WAY = 4 };
  typedef TO_CHAR ToChar;
  typedef FROM_CHAR FromChar;
  ToChar *tcpX;
  ...
  FourWayRunner(TO_CHAR (&outputArray)[N_WAY][OUTPUT_LEN], TO_CHAR fillChar,
                bsl::size_t margin, SrcSpec<FROM_CHAR> &src,
                Conversion<TO_CHAR, FROM_CHAR> &conv, ConvRslt &exp)
      : d_src(src), d_wp(OUTPUT_LEN, src.d_dstBufSize, fillChar, margin),
        d_conv(conv), d_expected(exp) {
    for (int i = 0; i < N_WAY; i++) {
      d_outBuf[i] = outputArray[i];
    }
  }

  void fillMargins(int n) { d_wp.fillMargins(d_outBuf[n]); }

  int checkMargins(int n) { return d_wp.checkMargins(d_outBuf[n]); }

  int checkFinalNull(int n) { return d_wp.checkFinalNull(d_outBuf[n]); }
...
bsl::size_t precomputedDataCount =
    sizeof PRECOMPUTED_DATA / sizeof *PRECOMPUTED_DATA;
...
void buildUpAndTestStringsU2ToU8(
    int idx, int depth, unsigned short *inputBuffer, char *outputBuffer,
    unsigned short *codePointSizes, bsl::size_t totalOutputLength,
    bsl::size_t codePointCount, unsigned short *inputCursor, char *outputCursor,
    int verbose, int veryVerbose);
...
void testSingleOctetPerturbation(const char *input, bsl::size_t perturbationPos,
                                 bsl::size_t perturbationChar,
                                 const unsigned short *origExpectedOutput,
                                 bsl::size_t, unsigned short *,
                                 bsl::size_t codePointCount,
                                 const PerturbationDesc &perturb, int,
                                 int veryVerbose) {
  ...
  int retVal = bdlde::CharConvertUtf16::utf8ToUtf16(
      outputBuffer, codePointCount - 1, inputBuffer, &codePointsWritten);

  LOOP3_ASSERT(L_, OBTS, retVal, OBTS == retVal || BOTH == retVal);

  LOOP3_ASSERT(L_, 0, outputBuffer[codePointCount - 2],
               0 == outputBuffer[codePointCount - 2]);

  retVal = bdlde::CharConvertUtf16::utf8ToUtf16(
      outputBuffer, codePointCount, inputBuffer, &codePointsWritten);

  if (veryVerbose) {
    cout << "PERTURBING:" << "\n\tinputBuffer       =";
    printStr(inputBuffer);
    cout << "\n\toutputBuffer      ="
         << prHexRange(outputBuffer, codePointCount)
         << "\n\tperturbation      = { '" << perturb.d_octetToConvertTo
         << "', '" << (perturb.d_isNewValid ? "Y" : "N") << "', '"
         << perturb.d_newCodePoint << "', " << perturb.d_extraInvalidBefore
         << "', " << perturb.d_extraInvalidAfter << "}"
         << "\n\torigInput         =";
    printStr(input);
    cout << "\n\torigExpectedOutput="
         << prHexRange(origExpectedOutput, codePointCount) << endl;
  }

  if (perturb.d_isNewValid && !before && !after) {
    LOOP3_ASSERT(L_, SUCCESS, retVal, SUCCESS == retVal);
  } else {
  ...
  for (int i = 0; i < (int)codePointCount; ++i) {
    if (i < pos - before) {
      ...
      LOOP3_ASSERT(i, outputBuffer[i], '?', outputBuffer[i] == '?');
    } else if (i == pos) {
      ...
      LOOP4_ASSERT(i, posInOrig, outputBuffer[i], origExpectedOutput[posInOrig],
...
void perturbUtf8AndCheckConversionFailures(const char *input,
  ...
  for (int currentCodePoint = 0; currentCodePoint < (int)codePointCount;
    ...
    case 1: {
      ...
      bsl::size_t testCount =
          sizeof oneOctetCharOctetOne / sizeof *oneOctetCharOctetOne;

      for (int i = 0; i < (int)testCount; ++i) {
        testSingleOctetPerturbation(
            input, currentCodePointStart, currentCodePoint, expected_output,
            totalInputLength, codePointSizes, codePointCount,
            oneOctetCharOctetOne[i], verbose, veryVerbose);
      }
    } break;
    ...
    case 2: {
...

      {
        ...
        static const bsl::size_t testCount =
            sizeof twoOctetCharOctetOne / sizeof *twoOctetCharOctetOne;

        for (int i = 0; i < (int)testCount; ++i) {
          testSingleOctetPerturbation(
              input, currentCodePointStart, currentCodePoint, expected_output,
              totalInputLength, codePointSizes, codePointCount,
              twoOctetCharOctetOne[i], verbose, veryVerbose);
        }
      }
...

      {
        ...
        static const bsl::size_t testCount =
            sizeof twoOctetCharOctetTwo / sizeof *twoOctetCharOctetTwo;

        for (int i = 0; i < (int)testCount; ++i) {
          testSingleOctetPerturbation(
              input, currentCodePointStart + 1, currentCodePoint,
              expected_output, totalInputLength, codePointSizes, codePointCount,
              twoOctetCharOctetTwo[i], verbose, veryVerbose);
        }
      }
    } break;
    ...
    case 3: {
...

      {
        ...
        static const bsl::size_t testCount =
            sizeof threeOctetCharOctetOne / sizeof *threeOctetCharOctetOne;

        for (int i = 0; i < (int)testCount; ++i) {
          testSingleOctetPerturbation(
              input, currentCodePointStart, currentCodePoint, expected_output,
              totalInputLength, codePointSizes, codePointCount,
              threeOctetCharOctetOne[i], verbose, veryVerbose);
        }
        ...
        const unsigned short newChar = static_cast<unsigned short>(
            ((0xc2 & 0x1f) << 6) | (input[currentCodePointStart + 1] & 0x3f));
        const PerturbationDesc perturb = {0xc2, true, newChar, 0, 1};

        testSingleOctetPerturbation(
            input, currentCodePointStart, currentCodePoint, expected_output,
            totalInputLength, codePointSizes, codePointCount, perturb, verbose,
            veryVerbose);
      }
      ...
      // perturbing 3-octet code point, octet 2
      {
        ...
        static const bsl::size_t testCount =
            sizeof threeOctetCharOctetTwo / sizeof *threeOctetCharOctetTwo;

        for (int i = 0; i < (int)testCount; ++i) {
          testSingleOctetPerturbation(
              input, currentCodePointStart + 1, currentCodePoint,
              expected_output, totalInputLength, codePointSizes, codePointCount,
              threeOctetCharOctetTwo[i], verbose, veryVerbose);
        }

        // Changing this byte to a "2-octet char 1" can't be
        // data-driven since we must compute the resulting code point.
        const unsigned short newChar = static_cast<unsigned short>(
            ((0xc2 & 0x1f) << 6) | (input[currentCodePointStart + 2] & 0x3f));
        const PerturbationDesc perturb = {0xc2, true, newChar, 1, 0};

        testSingleOctetPerturbation(
            input, currentCodePointStart + 1, currentCodePoint, expected_output,
            totalInputLength, codePointSizes, codePointCount, perturb, verbose,
            veryVerbose);
      }
...

      {
        ...
        static const bsl::size_t testCount =
            sizeof threeOctetCharOctetThree / sizeof *threeOctetCharOctetThree;

        for (int i = 0; i < (int)testCount; ++i) {
          testSingleOctetPerturbation(
              input, currentCodePointStart + 2, currentCodePoint,
              expected_output, totalInputLength, codePointSizes, codePointCount,
              threeOctetCharOctetThree[i], verbose, veryVerbose);
        }
      }
    } break;
...
void checkForExpectedConversionResultsU8ToU2(const char *input,
  ...
  perturbUtf8AndCheckConversionFailures(input, expected_output,
                                        totalInputLength, codePointSizes,
                                        codePointCount, verbose, veryVerbose);

  for (int bufSize = 0; bufSize < (int)codePointCount; ++bufSize) {
    unsigned short outputBuffer[256] = {0};
    bsl::size_t codePointsWritten = 0;

    retVal = bdlde::CharConvertUtf16::utf8ToUtf16(outputBuffer, bufSize, input,
                                                  &codePointsWritten);

    LOOP5_ASSERT(L_, OUTPUT_BUFFER_TOO_SMALL, retVal, bufSize, codePointCount,
...
void buildUpAndTestStringsU8ToU2(int idx, int depth, char *inputBuffer,
                                 unsigned short *outputBuffer,
                                 unsigned short *codePointSizes,
                                 bsl::size_t totalOutputLength,
                                 bsl::size_t codePointCount, char *inputCursor,
                                 unsigned short *outputCursor, int verbose,
                                 int veryVerbose) {
  ...
  *outputCursor = 0;
  ...
  checkForExpectedConversionResultsU8ToU2(inputBuffer, outputBuffer,
                                          totalOutputLength, codePointSizes,
                                          codePointCount, verbose, veryVerbose);
  ...
  *outputCursor = 0;
  ...
  for (int i = 0; i < (int)precomputedDataCount; ++i) {
    buildUpAndTestStringsU8ToU2(i, depth - 1, inputBuffer, outputBuffer,
                                codePointSizes, totalOutputLength,
                                codePointCount, inputCursor, outputCursor,
                                verbose, veryVerbose);
  }
}
...
template <bsl::size_t N> struct Permuter {
  ...
  unsigned int operator[](int i) const { return d_val[i]; }
...
inline ostream &operator<<(ostream &os, const Permuter<N> &p) {
  return p.print(os);
}
...
const unsigned char u8OneByteCases[] = {
    '\x1', '\x3', '\x7', '\xf', '\x1e', '\x3c', '\x78', '\x7f',
};

const unsigned char u8TwoByteHdrCases[] = {
    '\x2', '\x3', '\x7', '\xe', '\x18', '\x1a', '\x10', '\x15', '\x1b'};
const unsigned char u8ThreeByteHdrCases[] = {
    '\x0', '\x1', '\x3', '\x6', '\x8', '\x9', '\xc', '\xf',
};
...
const unsigned char u8FourByteHdrCases[] = {'\x0', '\x1', '\x2', '\x3', '\x4'};
...
const unsigned char u8InvalidFourByteHdrCases[] = {
    '\x5',
    '\x6',
    '\x7',
};
...
const unsigned char u8ContinByteCases[] = {
    '\x0', '\x1', '\x3', '\x7', '\xf', '\x1e', '\x3a', '\x3c', '\x3f',
};

const unsigned char u8ContinValidFourByteMaxCases[] = {
    '\x0', '\x1', '\x3', '\x7', '\xf',
};

const unsigned char u8ContinInvalidFourByteMaxCases[] = {
    '\x10', '\x1e', '\x3c', '\x3a', '\x3f',
};

const unsigned char u8ReservedRangeLowerContin[] = {
...
const unsigned char u8ReservedRangeUpperContin[] = {
...
const unsigned short u16UpperAndLower[] = {
    0x00, 0x01, 0x02,  0x03,  0x04,  0x06,  0x07,  0x08,  0x0c,  0x0d,
    0x10, 0x18, 0x1c,  0x20,  0x30,  0x38,  0x40,  0x60,  0x70,  0x80,
    0xc0, 0xd0, 0x100, 0x180, 0x1c0, 0x200, 0x300, 0x380, 0x3ff,
};
...
bool testOneErrorCharConversion(int line, ArrayRange<TO_CHAR> const &toBuf,
...
template <class TO_CHAR, class TO_FILL_CHECK, class FR_CHAR,
          class FR_FILL_CHECK>
bool oneStringConversion(int line, BufferedWPiece<TO_CHAR> &toBuf,
...
template <> bool swappedEquals<wchar_t, 2>(wchar_t lhs, wchar_t rhs) {
  return rhs == (((lhs & 0xff) << 8) | ((lhs & 0xff00) >> 8));
}

#ifdef BSLS_LIBRARYFEATURES_HAS_CPP11_BASELINE_LIBRARY
template <> bool swappedEquals<char16_t, 2>(char16_t lhs, char16_t rhs) {
  return rhs == (((lhs & 0xff) << 8) | ((lhs & 0xff00) >> 8));
}
#endif

template <> bool swappedEquals<wchar_t, 4>(wchar_t lhs, wchar_t rhs) {
...
bool swappedRangeEquals(const UTF16_CHAR *lhsBegin, const UTF16_CHAR *lhsEnd,
                        const UTF16_CHAR *rhsBegin, const UTF16_CHAR *rhsEnd) {
  if (lhsEnd - lhsBegin != rhsEnd - rhsBegin) {
    return false; // RETURN
  }

  for (; lhsBegin < lhsEnd; ++lhsBegin, ++rhsBegin) {
    if (!swappedEquals<UTF16_CHAR, sizeof(UTF16_CHAR)>(*lhsBegin, *rhsBegin)) {
      return false; // RETURN
    }
  }
...
char *sBegin(bsl::string &s) { return &s[0]; }

wchar_t *sBegin(bsl::wstring &s) { return &s[0]; }

char *sEnd(bsl::string &s) { return &s[0] + s.length() + 1; }

wchar_t *sEnd(bsl::wstring &s) { return &s[0] + s.length() + 1; }

template <class CHAR_TYPE> CHAR_TYPE *vBegin(bsl::vector<CHAR_TYPE> &v) {
  return &v.front();
}

template <class CHAR_TYPE> CHAR_TYPE *vEnd(bsl::vector<CHAR_TYPE> &v) {
...
template <> struct SwapInPlace_Helper<wchar_t, 2> {
  void operator()(wchar_t *word) {
    *word = static_cast<wchar_t>((static_cast<unsigned short>(*word) >> 8) |
                                 ((*word & 0xff) << 8));
  }
};

#ifdef BSLS_LIBRARYFEATURES_HAS_CPP11_BASELINE_LIBRARY
template <> struct SwapInPlace_Helper<char16_t, 2> {
  void operator()(char16_t *word) {
    *word = static_cast<char16_t>((static_cast<unsigned short>(*word) >> 8) |
                                  ((*word & 0xff) << 8));
  }
};
#endif

template <> struct SwapInPlace_Helper<wchar_t, 4> {
  void operator()(wchar_t *word) {
    const wchar_t w = *word;

    *word = static_cast<wchar_t>((static_cast<unsigned int>(w) >> 24) |
                                 ((w & 0xff0000) >> 8) | ((w & 0xff00) << 8) |
                                 ((w & 0xff) << 24));
  }
};

template <class UTF16_CHAR> void swapInPlace(UTF16_CHAR *word) {
...
static int localUtf16Cmp(const UTF16_WORD *a, const UTF16_WORD *b) {
  int diff;
  while (0 == (diff = *a - *b) && *a) {
    ++a;
    ++b;
...
template <class UTF16_WORD>
static bsl::size_t localUtf16Len(const UTF16_WORD *str) {
...
static bsl::string displayUtf8(const bsl::string &str)
...
template <class UTF16_WORD>
static bsl::string displayUtf16(const UTF16_WORD *utf16Str,
                                bdlde::ByteOrder::Enum byteOrder)
...
class TestDriver {
  ...
  enum {
    k_LO_SURROGATE_BASE = 0xd800u, // low UTF-16 surrogates are in
                                   // the range
                                   // '[ 0xd800 .. 0xdbff ]'
  ...
  static void copyUtf16ToWstring(bsl::wstring *wStr,
  ...
  static void encodeTwoOctets(char *octBuf, UnicodeCodePoint isoBuf);
  // Assume the specified 'isoBuf' will fit in 2 octets of UTF-8 and
  // encode it at the position indicated by the specified 'octBuf'.

  static void encodeThreeOctets(char *octBuf, UnicodeCodePoint isoBuf);
  // Assume the specified 'isoBuf' will fit in 3 octets of UTF-8 and
  // encode it at the position indicated by the specified 'octBuf'.

  static void encodeFourOctets(char *octBuf, UnicodeCodePoint isoBuf);
  ...
  static void appendRandomInvalidSingleOctet(bsl::string *string);
  ...
  static void appendRandomInvalidTwoOctets(bsl::string *string);
  // Append a random invalid two-octet code point to the specified
  // '*string'.

  static void appendRandomInvalidThreeOctets(bsl::string *string);
  // Append a random invalid three-octet code point to the specified
  // '*string'.

  static void appendRandomInvalidFourOctets(bsl::string *string);
  // Append a random invalid four-octet code point to the specified
  // '*string'.

  static void appendRandomValidSingleOctet(bsl::string *string);
  // Append a random non-zero ASCII byte to the specified '*string'.

  static void appendRandomValidTwoOctets(bsl::string *string);
  // Append a random valid two octet sequence to the specified '*string'.

  static void appendRandomValidThreeOctets(bsl::string *string);
  // Append a random valid three octet sequence to the specified
  // '*string'.

  static void appendRandomValidFourOctets(bsl::string *string);
  // Append a random valid four octet sequence to the specified
  // '*string'.

  static void appendRandomValidUtf8CodePoint(bsl::string *string);
  ...
  static void randomUtf8String(bsl::string *result, unsigned numSequences,
                               Validity validity);
  ...
  static void randomInvalidUtf16Vector(bsl::vector<unsigned short> *result,
                                       unsigned numWords,
                                       bdlde::ByteOrder::Enum byteOrder);
...
public:
  ...
  template <class STRING, class WSTRING, class VECTOR_C, class VECTOR_WC,
            class VECTOR_US>
  static void testCase13();
  // Verify backwards byte order translation for 'std' cotainers.

  template <class STRING, class WSTRING> static void testCase12();
  ...
  template <class VECTOR, class WSTRING> static void testCase10();
  // Verify utf8 to utf16 translation.
};
RandGen TestDriver::s_randGen;

inline unsigned TestDriver::numBitsSet(unsigned value) {
  unsigned ret;
...
inline unsigned TestDriver::calculateRandomNumOctets() {
...
inline bool TestDriver::fitsInSingleOctet(UnicodeCodePoint uc) {
  return 0 == (uc & (~UnicodeCodePoint(0) << ONE_OCT_CONT_WID));
}

inline bool TestDriver::fitsInTwoOctets(UnicodeCodePoint uc) {
  return 0 == (uc & (~UnicodeCodePoint(0)
                     << (TWO_OCT_CONT_WID + CONTINUE_CONT_WID)));
}

inline bool TestDriver::fitsInThreeOctets(UnicodeCodePoint uc) {
  return 0 == (uc & (~UnicodeCodePoint(0)
                     << (THREE_OCT_CONT_WID + 2 * CONTINUE_CONT_WID)));
}

inline void TestDriver::encodeTwoOctets(char *octBuf, UnicodeCodePoint isoBuf) {
  BSLS_ASSERT_SAFE(fitsInTwoOctets(isoBuf));

  octBuf[1] = static_cast<char>(CONTINUE_TAG | (isoBuf & ~CONTINUE_MASK));
  octBuf[0] = static_cast<char>(
      TWO_OCTET_TAG | ((isoBuf >> CONTINUE_CONT_WID) & ~TWO_OCTET_MASK));
}

inline void TestDriver::encodeThreeOctets(char *octBuf,
                                          UnicodeCodePoint isoBuf) {
  BSLS_ASSERT_SAFE(fitsInThreeOctets(isoBuf));

  for (unsigned uu = 0; uu < 2; ++uu) {
    octBuf[2 - uu] = static_cast<char>(
        CONTINUE_TAG | ((isoBuf >> uu * CONTINUE_CONT_WID) & ~CONTINUE_MASK));
  }
  octBuf[0] =
      static_cast<char>(THREE_OCTET_TAG | ((isoBuf >> 2 * CONTINUE_CONT_WID) &
                                           ~THREE_OCTET_MASK));
}

inline void TestDriver::encodeFourOctets(char *octBuf,
                                         UnicodeCodePoint isoBuf) {
  for (unsigned uu = 0; uu < 3; ++uu) {
    octBuf[3 - uu] = static_cast<char>(
        CONTINUE_TAG | ((isoBuf >> uu * CONTINUE_CONT_WID) & ~CONTINUE_MASK));
  }
  octBuf[0] = static_cast<char>(
      FOUR_OCTET_TAG | ((isoBuf >> 3 * CONTINUE_CONT_WID) & ~FOUR_OCTET_MASK));
}

inline bool TestDriver::isContinuation(char c) {
...
// PRIVATE CLASS METHODS
void TestDriver::copyUtf16ToWstring(bsl::wstring *wStr,
                                    const bsl::vector<unsigned short> &utf16In,
                                    bdlde::ByteOrder::Enum byteOrder) {
...
// preceding contents of '*string' is a truncated sequence.
{
  ...
  const ErrorType errorType =
      static_cast<ErrorType>(s_randGen.bits(k_RAND_BITS));

  const char c = e_ILLEGAL_CHAR == errorType
                     ? // Bytes with high 5 bits all set '11111xxx' are never
                       // legal characters in UTF-8.
...
{
  enum ErrorType {
    e_TRUNC, // wrong byte appended before sequence
             // completes
  ...
  const ErrorType errorType =
      static_cast<ErrorType>(s_randGen.bits(k_RAND_BITS) % k_NUM_ERROR_TYPES);
...
// '*string'.
{
  ...
  case e_SURROGATE: {
    enum {
      k_MIN_SURROGATE = 0xd800u, // reserved values for
...
// '*string'.
{
  ...
  case e_END: {
    ...
    *string += static_cast<char>(FOUR_OCTET_TAG | s_randGen.bits(3));
    ...
    BSLMF_ASSERT(k_CONT_MOD * k_RAND_FUDGE_MULTIPLE < (1u << k_RAND_CONT_BITS));

    const unsigned contBytes = s_randGen.bits(k_RAND_CONT_BITS) % k_CONT_MOD;
    ASSERT(contBytes <= 2);
  ...
  case e_NON_MINIMAL: {
    enum {
      k_3_OCT_BITS = THREE_OCT_CONT_WID + 2 * CONTINUE_CONT_WID,
...
void TestDriver::appendRandomValidThreeOctets(bsl::string *string) {
  enum {
    k_FLOOR = 1 << (TWO_OCT_CONT_WID + CONTINUE_CONT_WID),
  ...
  } while (k_MIN_UTF_16_DOUBLE_PLANE <= uc && uc <= k_MAX_UTF_16_DOUBLE_PLANE);
...
void TestDriver::randomUtf8String(bsl::string *result, unsigned numSequences,
                                  Validity validity) {
    ...
    if (1 == numSequences) {
      ...
      do {
        ...
        for (unsigned uu = 0; uu < numSequences; ++uu) {
          if (s_randGen.bits(1)) {
            appendRandomInvalidUtf8CodePoint(result);
            adjacentErrors |= 0 <= prevErrorIdx && prevErrorIdx + 1u == uu;
            prevErrorIdx = uu;
          } else {
            appendRandomValidUtf8CodePoint(result);
          }
...
unsigned short TestDriver::randomUtf16SurrogateLo() {
  return static_cast<unsigned short>(k_LO_SURROGATE_BASE +
                                     s_randGen.bits(k_SURROGATE_RAND_BITS));
}

unsigned short TestDriver::randomUtf16SurrogateHi() {
  return static_cast<unsigned short>(k_HI_SURROGATE_BASE +
                                     s_randGen.bits(k_SURROGATE_RAND_BITS));
}

void TestDriver::randomInvalidUtf16Vector(bsl::vector<unsigned short> *result,
                                          unsigned numWords,
                                          bdlde::ByteOrder::Enum byteOrder) {
  ...
  if (1 == numWords) {
    ...
    const unsigned short w =
        s_randGen.bits(1) ? randomUtf16SurrogateLo() : randomUtf16SurrogateHi();
    ...
    do {
      ...
      for (unsigned uu = 0; uu < numWords; ++uu) {
        ...
        const WordType thisWordType =
            static_cast<WordType>(s_randGen.bits(2) % k_NUM_WORD_TYPES);

        const unsigned short w =
            e_SINGLE_WORD == thisWordType    ? randomUtf16NonSurrogate()
            : e_LO_SURROGATE == thisWordType ? randomUtf16SurrogateLo()
...
// ------------------------------------------------------------------------
{
  ...
  for (int ii = 0; ii < k_ITERATIONS; ++ii) {
    ...
    utf16.reserve(4092);
    const unsigned RANDOM_NUM_WORDS =
        ii < 10000 ? (ii < 1000 ? 2 : 3) : 4 + s_randGen.bits(3);

    randomInvalidUtf16Vector(&utf16, RANDOM_NUM_WORDS, byteOrder);
    ASSERT(utf16.size() == RANDOM_NUM_WORDS);

    if (veryVerbose)
      cout << displayUtf16(utf16.data(), byteOrder) << endl;

    // Calculate the size to translate back to UTF8.

    const bsl::size_t PREDICTED_UTF8_LEN = Util::computeRequiredUtf8Bytes(
        utf16.data(), utf16.data() + utf16.size() - 1, byteOrder);
    const bsl::size_t PREDICTED_UTF8_LEN_B = Util::computeRequiredUtf8Bytes(
        utf16.data(), static_cast<unsigned short *>(0), byteOrder);
    ASSERTV(displayUtf16(utf16.data(), byteOrder), PREDICTED_UTF8_LEN,
            PREDICTED_UTF8_LEN_B, PREDICTED_UTF8_LEN == PREDICTED_UTF8_LEN_B);
    ...
    utf8B.reserve(4092);
    int rc = Util::utf16ToUtf8(&utf8B, utf16.data(), &utf8CodePointsB, '?',
                               byteOrder);
    ...
    utf8CodePointsB = -1;
    utf8B.clear();
    rc = Util::utf16ToUtf8(&utf8B, utf16.data(), utf16.size() - 1,
                           &utf8CodePointsB, '?', byteOrder);
    ASSERTV(rc, displayUtf16(utf16.data(), byteOrder), byteOrder,
            sizeof(wchar_t),
            bdlde::CharConvertStatus::k_INVALID_INPUT_BIT == rc);
    ASSERT(utf8CodePointsB <= RANDOM_NUM_WORDS);
    ...
    std::string utf8C;
    utf8C.reserve(4092);
    rc = Util::utf16ToUtf8(&utf8C, utf16.data(), &utf8CodePointsC, '?',
                           byteOrder);
    ASSERTV(rc, displayUtf16(utf16.data(), byteOrder), byteOrder,
            sizeof(wchar_t),
            bdlde::CharConvertStatus::k_INVALID_INPUT_BIT == rc);
    ...
    utf8CodePointsC = -1;
    utf8C.clear();
    rc = Util::utf16ToUtf8(&utf8C, utf16.data(), utf16.size() - 1,
                           &utf8CodePointsC, '?', byteOrder);
    ASSERTV(rc, displayUtf16(utf16.data(), byteOrder), byteOrder,
            sizeof(wchar_t),
            bdlde::CharConvertStatus::k_INVALID_INPUT_BIT == rc);
    ASSERT(utf8CodePointsB <= RANDOM_NUM_WORDS);
    ...
    bsl::size_t len = Util::computeRequiredUtf8Bytes(
        utf16W.data(), utf16W.data() + utf16W.length(), byteOrder);
    ASSERTV(displayUtf16(utf16W.data(), byteOrder), PREDICTED_UTF8_LEN, len,
            PREDICTED_UTF8_LEN == len);
    len =
        Util::computeRequiredUtf8Bytes(utf16W.data(), (wchar_t *)0, byteOrder);
    ASSERTV(displayUtf16(utf16W.data(), byteOrder), PREDICTED_UTF8_LEN, len,
            PREDICTED_UTF8_LEN == len);
    ...
    utf8CodePointsC = -1;
    utf8C.clear();
    rc = Util::utf16ToUtf8(&utf8C, utf16W.data(), &utf8CodePointsC, '?',
                           byteOrder);
    ...
    utf8CodePointsC = -1;
    utf8C.clear();
    rc = Util::utf16ToUtf8(&utf8C, utf16W, &utf8CodePointsC, '?', byteOrder);
    ASSERTV(displayUtf16(utf16W.data(), byteOrder), rc, byteOrder,
            bdlde::CharConvertStatus::k_INVALID_INPUT_BIT == rc);
    ASSERT(PREDICTED_UTF8_LEN == utf8C.length() + 1);
    ASSERT(utf8CodePointsB == utf8CodePointsC);
    ...
    utf8CodePointsC = -1;
    utf8B.clear();
    rc = Util::utf16ToUtf8(&utf8B, utf16W.data(), &utf8CodePointsC, '?',
                           byteOrder);
    ASSERTV(displayUtf16(utf16W.data(), byteOrder), rc, byteOrder,
            bdlde::CharConvertStatus::k_INVALID_INPUT_BIT == rc);
    ASSERT(PREDICTED_UTF8_LEN == utf8B.length() + 1);
    ...
    utf8CodePointsC = -1;
    utf8B.clear();
    rc = Util::utf16ToUtf8(&utf8B, utf16W, &utf8CodePointsC, '?', byteOrder);
    ASSERTV(displayUtf16(utf16W.data(), byteOrder), rc, byteOrder,
            bdlde::CharConvertStatus::k_INVALID_INPUT_BIT == rc);
    ASSERT(PREDICTED_UTF8_LEN == utf8B.length() + 1);
    ASSERT(utf8CodePointsB == utf8CodePointsC);
    ...
    utf8CodePointsC = -1;
    utf8P.clear();
    rc = Util::utf16ToUtf8(&utf8P, utf16.data(), utf16.size() - 1,
                           &utf8CodePointsC, '?', byteOrder);
    ASSERTV(displayUtf16(utf16W.data(), byteOrder), rc, byteOrder,
            bdlde::CharConvertStatus::k_INVALID_INPUT_BIT == rc);
    ASSERT(PREDICTED_UTF8_LEN == utf8P.length() + 1);
    ...
    utf8CodePointsC = -1;
    utf8P.clear();
    rc = Util::utf16ToUtf8(&utf8P, utf16W, &utf8CodePointsC, '?', byteOrder);
    ASSERTV(displayUtf16(utf16.data(), byteOrder), rc, byteOrder,
            bdlde::CharConvertStatus::k_INVALID_INPUT_BIT == rc);
    ASSERT(PREDICTED_UTF8_LEN == utf8P.length() + 1);
    ASSERT(utf8CodePointsB == utf8CodePointsC);
    ...
    utf8CodePointsC = -1;
    utf8P.clear();
    rc = Util::utf16ToUtf8(&utf8P, utf16W.c_str(), &utf8CodePointsC, '?',
                           byteOrder);
    ASSERTV(displayUtf16(utf16.data(), byteOrder), rc, byteOrder,
            bdlde::CharConvertStatus::k_INVALID_INPUT_BIT == rc);
    ASSERT(PREDICTED_UTF8_LEN == utf8P.length() + 1);
    ...
    len = Util::computeRequiredUtf8Bytes(
        utf16U16.data(), utf16U16.data() + utf16U16.length(), byteOrder);

    ASSERTV(displayUtf16(utf16U16.data(), byteOrder), PREDICTED_UTF8_LEN, len,
            PREDICTED_UTF8_LEN == len);

    bsl::size_t utf8CodePointsD = -1;
    std::string utf8D;
    utf8D.reserve(4092);
    rc = Util::utf16ToUtf8(&utf8D, utf16U16.data(), &utf8CodePointsD, '?',
                           byteOrder);
    ...
    bsl::string utf8E(&ta);
    utf8E.reserve(4092);
    rc = Util::utf16ToUtf8(&utf8E, utf16U16.data(), &utf8CodePointsD, '?',
                           byteOrder);
    ASSERTV(displayUtf16(utf16U16.data(), byteOrder), rc, byteOrder,
            bdlde::CharConvertStatus::k_INVALID_INPUT_BIT == rc);
...
// ------------------------------------------------------------------------
{
  ...
  for (int ii = 0; ii < k_ITERATIONS; ++ii) {
    ...
    const unsigned RANDOM_CODE_POINTS = ii < 1000 ? 2 : 3 + s_randGen.bits(3);

    randomUtf8String(&utf8, RANDOM_CODE_POINTS, e_INVALID);

    if (veryVerbose)
      cout << displayUtf8(utf8) << endl;

    // predict length of UTF-16 string needed.

    const bsl::size_t PREDICTED_UTF16_LEN = Util::computeRequiredUtf16Words(
    ...
    int rc = Util::utf8ToUtf16(&utf16, utf8, &utf16CodePoints, '?', byteOrder);
    ASSERT(0 != rc);
    ...
    rc = Util::utf8ToUtf16(&utf16W, utf8, &utf16WCodePoints, '?', byteOrder);
    ASSERT(0 != rc);
    ...
    rc =
        Util::utf8ToUtf16(&utf16U16, utf8, &utf16U16CodePoints, '?', byteOrder);
    ASSERT(0 != rc);
    ...
    ASSERT(0 == bsl::memcmp(utf16.data(), utf16U16.data(),
...
// ------------------------------------------------------------------------
{
  ...
  for (int ii = 0; ii < k_ITERATIONS; ++ii) {
    ...
    const unsigned RANDOM_CODE_POINTS =
        ii < 1000 ? (0 == ii ? 1 : 2) : 3 + s_randGen.bits(3);
    ...
    ASSERT(bdlde::Utf8Util::isValid(utf8));
    {
      ...
      ASSERT(static_cast<int>(RANDOM_CODE_POINTS) - 1 ==
             bdlde::Utf8Util::advanceIfValid(&sts, &endTraverse, begin,
    ...
    utf16.clear();
    utf16CodePoints = -1;
    rc = Util::utf8ToUtf16(&utf16, utf8.c_str(), &utf16CodePoints, '?',
                           byteOrder);
    ASSERT(0 == rc);
    ASSERTV(displayUtf8(utf8), RANDOM_CODE_POINTS, utf16CodePoints,
            RANDOM_CODE_POINTS == utf16CodePoints);
    ...
    utf16s.reserve(4092);
    utf16CodePoints = -1;
    rc = Util::utf8ToUtf16(&utf16s, utf8, &utf16CodePoints, '?', byteOrder);
    ASSERT(0 == rc);
    ASSERTV(displayUtf8(utf8), RANDOM_CODE_POINTS, utf16CodePoints,
            RANDOM_CODE_POINTS == utf16CodePoints);
    ...
    utf16s.clear();
    utf16CodePoints = -1;
    rc = Util::utf8ToUtf16(&utf16s, utf8.c_str(), &utf16CodePoints, '?',
                           byteOrder);
    ASSERT(0 == rc);
    ASSERTV(displayUtf8(utf8), RANDOM_CODE_POINTS, utf16CodePoints,
            RANDOM_CODE_POINTS == utf16CodePoints);
    ...
    utf16p.clear();
    utf16CodePoints = -1;
    rc = Util::utf8ToUtf16(&utf16p, utf8.c_str(), &utf16CodePoints, '?',
                           byteOrder);
    ASSERT(0 == rc);
    ASSERTV(displayUtf8(utf8), RANDOM_CODE_POINTS, utf16CodePoints,
            RANDOM_CODE_POINTS == utf16CodePoints);
    ...
    if (veryVerbose)
      cout << displayUtf16(utf16p.data(), byteOrder) << endl;
#endif
    ...
    utf16CodePoints = -1;
    rc = Util::utf8ToUtf16(&utf16U16b, utf8, &utf16CodePoints, '?', byteOrder);
    ASSERT(0 == rc);
    ASSERTV(displayUtf8(utf8), RANDOM_CODE_POINTS, utf16CodePoints,
            RANDOM_CODE_POINTS == utf16CodePoints);
    ...
    if (veryVerbose)
      cout << displayUtf16(utf16U16b.data(), byteOrder) << endl;
    utf16U16b.clear();
    utf16CodePoints = -1;
    rc = Util::utf8ToUtf16(&utf16U16b, utf8.c_str(), &utf16CodePoints, '?',
                           byteOrder);
    ASSERT(0 == rc);
    ASSERTV(displayUtf8(utf8), RANDOM_CODE_POINTS, utf16CodePoints,
            RANDOM_CODE_POINTS == utf16CodePoints);
    ...
    utf16CodePoints = -1;
    rc = Util::utf8ToUtf16(&utf16U16s, utf8, &utf16CodePoints, '?', byteOrder);
    ASSERT(0 == rc);
    ASSERTV(displayUtf8(utf8), RANDOM_CODE_POINTS, utf16CodePoints,
            RANDOM_CODE_POINTS == utf16CodePoints);
    ...
    if (veryVerbose)
      cout << displayUtf16(utf16U16s.data(), byteOrder) << endl;

    utf16U16s.clear();
    utf16CodePoints = -1;
    rc = Util::utf8ToUtf16(&utf16U16s, utf8.c_str(), &utf16CodePoints, '?',
                           byteOrder);
    ASSERT(0 == rc);
    ASSERTV(displayUtf8(utf8), RANDOM_CODE_POINTS, utf16CodePoints,
            RANDOM_CODE_POINTS == utf16CodePoints);
    ...
    if (veryVerbose)
      cout << displayUtf16(utf16U16sp.data(), byteOrder) << endl;

    utf16U16sp.clear();
    utf16CodePoints = -1;
    rc = Util::utf8ToUtf16(&utf16U16sp, utf8.c_str(), &utf16CodePoints, '?',
                           byteOrder);
    ASSERT(0 == rc);
    ASSERTV(displayUtf8(utf8), RANDOM_CODE_POINTS, utf16CodePoints,
            RANDOM_CODE_POINTS == utf16CodePoints);
    ...
    std::string utf8B;
    utf8B.reserve(4092);
    rc = Util::utf16ToUtf8(&utf8B, utf16.data(), &utf8CodePoints, 0, byteOrder);
    ASSERT(0 == rc);
    ...
    bsl::wstring utf16W(alloc);
    utf16W.reserve(4092);
    rc = Util::utf8ToUtf16(&utf16W, utf8, &utf16CodePoints, '?', byteOrder);
    ASSERT(0 == rc);
    ...
    len = bdlde::ByteOrder::e_HOST == byteOrder
              ...
              : Util::computeRequiredUtf8Bytes(utf16W.data(), (wchar_t *)0,
                                               byteOrder);
    ASSERTV(displayUtf16(utf16W.data(), byteOrder), displayUtf8(utf8),
            PREDICTED_UTF8_LEN, len, PREDICTED_UTF8_LEN == len);
    ...
    utf8C.reserve(4092);
    rc =
        Util::utf16ToUtf8(&utf8C, utf16W.data(), &utf8CodePoints, 0, byteOrder);
    ASSERT(0 == rc);
    ASSERT(RANDOM_CODE_POINTS == utf8CodePoints);
    ...
    bsl::u16string utf16U16(alloc);
    utf16U16.reserve(4092);
    rc = Util::utf8ToUtf16(&utf16U16, utf8, &utf16CodePoints, '?', byteOrder);
    ASSERT(0 == rc);
    ASSERT(RANDOM_CODE_POINTS == utf16CodePoints);
    ASSERT(utf16U16.length() + 1 == utf16.size());

    const bsl::u16string &EXP = bsl::u16string(usToC16(utf16.data()), alloc);
    ASSERT(EXP == utf16U16);
    ...
    len = bdlde::ByteOrder::e_HOST == byteOrder
              ...
              : Util::computeRequiredUtf8Bytes(utf16U16.data(), (char16_t *)0,
                                               byteOrder);
    ASSERTV(displayUtf16(utf16U16.data(), byteOrder), displayUtf8(utf8),
            PREDICTED_UTF8_LEN, len, PREDICTED_UTF8_LEN == len);
    ...
    std::string utf8D;
    utf8D.reserve(4092);
    rc = Util::utf16ToUtf8(&utf8D, utf16U16.data(), &utf8CodePoints, 0,
                           byteOrder);
    ASSERT(0 == rc);
    ASSERT(RANDOM_CODE_POINTS == utf8CodePoints);
...
void TestDriver::testCase13() {
    ...
    cout << "\t\tOutput to wstring\n";
  {
    ...
    WSTRING &dstStrBack = dstStrBackProxy.object();

    int rc = Util::utf8ToUtf16(&dstStr, charUtf8MultiLang, 0, '?');
    ASSERT(0 == rc);

    rc = Util::utf8ToUtf16(&dstStrBack, bsl::string_view(charUtf8MultiLang), 0,
                           '?', e_BACKWARDS);
    ASSERT(0 == rc);

    ASSERT(swappedRangeEquals(&dstStr[0], &dstStr[0] + dstStr.length() + 1,
                              &dstStrBack[0],
                              &dstStrBack[0] + dstStrBack.length() + 1));

    dstStrBack.clear();
    rc = Util::utf8ToUtf16(&dstStrBack, charUtf8MultiLang, 0, '?', e_BACKWARDS);
    ASSERT(0 == rc);
    ...
    rStr.clear();
    rc = Util::utf16ToUtf8(&rStr, WideStringView(dstStrBack), 0, '?',
                           e_BACKWARDS);
    ASSERT(0 == rc);
    ASSERT(rStr == charUtf8MultiLang);
    ...
    rc = Util::utf16ToUtf8(&vectorC, WideStringView(dstStrBack), 0, '?',
                           e_BACKWARDS);
    ASSERT(0 == rc);
    ...
    cout << "\t\tOutput to vector<unsigned short>\n";
  {
    ...
    VECTOR_US &dstVecBack = dstVecBackProxy.object();

    int rc = Util::utf8ToUtf16(&dstVec, charUtf8MultiLang, 0, '?');
    ASSERT(0 == rc);

    rc = Util::utf8ToUtf16(&dstVecBack, bsl::string_view(charUtf8MultiLang), 0,
                           '?', e_BACKWARDS);
    ASSERT(0 == rc);

    typename VECTOR_US::value_type *dstVecData = &dstVec[0];
    typename VECTOR_US::value_type *dstVecBackData = &dstVecBack[0];
    ASSERT(swappedRangeEquals(dstVecData, dstVecData + dstVec.size(),
                              dstVecBackData,
                              dstVecBackData + dstVecBack.size()));

    dstVecBack.clear();
    rc = Util::utf8ToUtf16(&dstVecBack, charUtf8MultiLang, 0, '?', e_BACKWARDS);
    ASSERT(0 == rc);
    ...
    cout << "\t\tOutput to unsigned short memory segment\n";
  {
    ...
    int rc =
        Util::utf8ToUtf16(dstVecUSData, SEG_SIZE, charUtf8MultiLang, &nc, &nw);
    ASSERT(0 == rc);
    ...
    VECTOR_US &dstVecUS2 = dstVecUSProxy2.object();

    dstVecUS2.resize(SEG_SIZE);
    unsigned short *dstVecUSData2 = &dstVecUS2[0];

    bsl::size_t ncb = -1, nwb = -1;
    rc = Util::utf8ToUtf16(dstVecUSData2, SEG_SIZE, charUtf8MultiLang, &ncb,
                           &nwb, '?', e_BACKWARDS);
    ASSERT(0 == rc);
    ASSERT(nwb == nw);
    ASSERT(ncb == nc);
    ...
    ASSERT(swappedRangeEquals(dstVecUSData, dstVecUSData + nw, dstVecUSData2,
                              dstVecUSData2 + nwb));
    ...
    ncb = -1, nwb = -1;
    rc = Util::utf16ToUtf8(dstVecCData, SEG_SIZE, dstVecUSData2, &ncb, &nwb,
                           '?', e_BACKWARDS);
    ASSERT(0 == rc);
    ASSERT(ncb == nc);
    ASSERT(nwb > ncb);
    ...
    cout << "\t\tOutput to wchar_t memory segment\n";
  {
    ...
    bsl::size_t nc = -1, nw = -1;
    int rc =
        Util::utf8ToUtf16(dstVecWCData, SEG_SIZE, charUtf8MultiLang, &nc, &nw);
    ASSERT(0 == rc);
    ...
    VECTOR_WC &dstVecWC2 = dstVecWCProxy2.object();

    dstVecWC2.resize(SEG_SIZE);
    wchar_t *dstVecWCData2 = &dstVecWC2[0];

    bsl::size_t ncb = -1, nwb = -1;
    rc = Util::utf8ToUtf16(dstVecWCData2, SEG_SIZE, charUtf8MultiLang, &ncb,
                           &nwb, '?', e_BACKWARDS);
    ASSERT(0 == rc);
    ASSERT(nwb == nw);
    ASSERT(ncb == nc);
    ...
    ASSERT(swappedRangeEquals(dstVecWCData, dstVecWCData + nw, dstVecWCData2,
                              dstVecWCData2 + nwb));
    ...
    ncb = -1, nwb = -1;
    rc = Util::utf16ToUtf8(dstVecCData, SEG_SIZE, dstVecWCData2, &ncb, &nwb,
                           '?', e_BACKWARDS);
    ASSERT(0 == rc);
    ASSERT(ncb == nc);
    ASSERT(nwb > ncb);
...
template <class STRING, class WSTRING> void TestDriver::testCase12() {
        ...
        for (int tm = 0; tm < k_NUM_SNIPPETS; ++tm) {
          for (int tn = 0; tn < k_NUM_SNIPPETS; ++tn) {
            ...
            const bsl::size_t NUM_CODE_POINTS =
                1 + numCodePoints[ti] + numCodePoints[tj] + numCodePoints[tk] +
                numCodePoints[tm] + numCodePoints[tn];
            ...
            utf8In += utf8s[tm];
            utf8In += utf8s[tn];
            ASSERT(utf8In.length() == utf8s[ti].length() + utf8s[tj].length() +
                                          utf8s[tk].length() +
                                          utf8s[tm].length() +
                                          utf8s[tn].length());
            ASSERT(zeroThere ==
                   (bsl::strlen(utf8In.c_str()) < utf8In.length()));

            bslalg::ConstructorProxy<WSTRING> expectedUtf16Proxy(&ca);
            WSTRING &expectedUtf16 = expectedUtf16Proxy.object();
            ...
            WSTRING &utf16 = utf16Proxy.object();

            bsl::size_t nc = -1;
            ASSERT(0 == Obj::utf8ToUtf16(&utf16, utf8In, &nc));
            ASSERT(expectedUtf16 == utf16);
            ASSERTV(nc, NUM_CODE_POINTS, ti, tj, tk, tn, NUM_CODE_POINTS == nc);

            bslalg::ConstructorProxy<STRING> utf8OutProxy(&ca);
            STRING &utf8Out = utf8OutProxy.object();
            nc = -1;
          ...
          for (WIDE_CHAR *end = start; end <= END; ++end) {
            for (int e = 0; e < 2; ++e) {
              ...
              int rc = Util::utf16ToUtf8(
                  &dstVec, WStringView(start, end - start), &nc, errorByte);
              ASSERT(0 == (rc & bdlde::CharConvertStatus::k_OUT_OF_SPACE_BIT));
              ASSERT(start > START || end < END ||
                     (ii < 6 && jj < 6 && kk < 6) ||
                     bdlde::CharConvertStatus::k_INVALID_INPUT_BIT == rc);
              ASSERT(start > START || end < END || ii >= 4 || jj >= 4 ||
                     kk >= 4 || 0 == rc);
              ...
              int rc2 = Util::utf16ToUtf8(
                  &dstStr, WStringView(start, end - start), &nc2, errorByte);
              ASSERT(rc2 == rc);
              ASSERT(dstVec.size() == dstStr.length() + 1);

              char *dstVecData = &dstVec[0];
              ASSERT(0 ==
                     bsl::memcmp(dstVecData, dstStr.c_str(), dstVec.size()));
              ...
              *end = 0;
              ...
              nc2 = (bsl::size_t)-1;
              rc2 = Util::utf16ToUtf8(&dstStr, start, &nc2, errorByte);
              ASSERT(rc2 == rc);
              ASSERT(dstVec.size() == dstStr.length() + 1);
              ASSERT(0 ==
                     bsl::memcmp(dstVecData, dstStr.c_str(), dstVec.size()));
              ...
              *end = save;
              ...
              nc2 = (bsl::size_t)-1;
              rc2 = Util::utf16ToUtf8(&dstVec, srcVec.begin(), &nc2, errorByte);
              ASSERT(rc2 == rc);
              ...
              nc2 = (bsl::size_t)-1;
              rc2 = Util::utf16ToUtf8(&dstStr, srcVec.begin(), &nc2, errorByte);
              ASSERT(rc2 == rc);
              ...
              for (bsl::size_t cap = 0; cap <= dstCap + 1; ++cap) {
                nc = nw = (bsl::size_t)-1;
                bsl::memset(dstVecData, (char)-1, dstVec.size());
                rc2 = Util::utf16ToUtf8(dstVecData, cap,
                                        WStringView(start, end - start), &nc,
                                        &nw, errorByte);
                ASSERT((cap < dstCap) ==
                ...
                bsl::memset(dstVecData, (char)-1, dstVec.size());
                rc2 = Util::utf16ToUtf8(dstVecData, cap, srcVec.begin(), &nc2,
                                        &nw2, errorByte);
                ASSERT((cap < dstCap) ==
                       !!(rc2 & bdlde::CharConvertStatus::k_OUT_OF_SPACE_BIT));
                ASSERT(cap < dstCap || rc2 == rc);
          ...
          for (WIDE_CHAR *end = start; end <= END; ++end) {
            for (int e = 0; e < 2; ++e) {
              ...
              if (veryVeryVeryVeryVerbose) {
                P_(ii)
                P_(jj) P_(kk) P_(start - START) P_(END - end) P(end - start);
              }

              nc = (bsl::size_t)-1;
              int rc =
                  Util::utf16ToUtf8(&dstVec, WStringView(start, end - start),
                                    &nc, errorByte, e_BACKWARDS);
              ASSERT(0 == (rc & bdlde::CharConvertStatus::k_OUT_OF_SPACE_BIT));
              ASSERT(start > START || end < END ||
                     (ii < 6 && jj < 6 && kk < 6) ||
                     bdlde::CharConvertStatus::k_INVALID_INPUT_BIT == rc);
              ...
              nc2 = (bsl::size_t)-1;
              int rc2 =
                  Util::utf16ToUtf8(&dstStr, WStringView(start, end - start),
                                    &nc2, errorByte, e_BACKWARDS);
              ASSERT(rc2 == rc);
              ASSERT(dstVec.size() == dstStr.length() + 1);
...
template <class VECTOR, class WSTRING> void TestDriver::testCase10() {
  typedef typename WSTRING::value_type WIDE_CHAR;

  struct {
    int d_line;
    const char *d_string;
  } DATA[] = {{L_, "H"},                // -- Valid ASCII
        ...
        for (int m = 1; m < NUM_DATA; m += 2) {
          ...
          for (char *start = INPUT.begin(); start < END; ++start) {
            ...
            for (char *end = END; end >= start; --end) {
              ...
              int rc = Util::utf8ToUtf16(&dstVec,
                                         bsl::string_view(start, end - start),
                                         &nc, errorWord);
              LOOP4_ASSERT(rc, i, k, m,
              ...
              typename VECTOR::iterator found =
                  bsl::find(dstVec.begin(), dstVec.end(), (unsigned short)'?');
              ...
              *end = 0;
              ...
              int rc2 = Util::utf8ToUtf16(&dstVecB, start, &nc2, errorWord);
              ASSERT(rc2 == rc);
              ...
              *end = save;
              ...
              found = bsl::find(dstVecB.begin(), dstVecB.end(),
                                (unsigned short)'?');
              ASSERT(errorWord && rc ? found < dstVecB.end()
                                     : found == dstVecB.end());

              rc2 = Util::utf8ToUtf16(&wDstStr,
                                      bsl::string_view(start, end - start),
                                      &nc2, (WIDE_CHAR)errorWord);
              ASSERT(rc2 == rc);
              ASSERT(nc2 == nc);

              ASSERT(dstVec.size() == wDstStr.length() + 1);
              ASSERT(0 == dstVec.back());
              for (bsl::size_t ii = 0; ii < wDstStr.length(); ++ii) {
              ...
              *end = 0;
              ...
              rc2 = Util::utf8ToUtf16(&wDstStrB, start, &nc2,
                                      (WIDE_CHAR)errorWord);
              ASSERT(rc2 == rc);
              ASSERT(nc2 == nc);
              ...
              *end = save;
              ...
              for (bsl::size_t cap = 1; cap <= dstCap; ++cap) {
                bsl::fill(dstVec.begin(), dstVec.end(), (unsigned short)-1);

                unsigned short *dstVecData = &dstVec[0];

                nc = nw = (bsl::size_t)-1;
                rc = Util::utf8ToUtf16(dstVecData, cap,
                                       bsl::string_view(start, end - start),
                                       &nc, &nw, errorWord);
                ASSERT(nc <= nw);
                ASSERT(nw <= cap);
                ...
                LOOP4_ASSERT(
                    rc, i, k, m,
                    cap < dstCap || INPUT.c_str() < start || END > end ||
                        (i < 4 && k < 4 && m < 4) ||
                        (bdlde::CharConvertStatus::k_INVALID_INPUT_BIT & rc));
                bsl::fill(dstVec.begin(), dstVec.end(), (unsigned short)-1);
                ...
                *end = 0;
                ...
                nc = nw = (bsl::size_t)-1;
                rc = Util::utf8ToUtf16(dstVecData, cap, start, &nc, &nw,
                                       errorWord);
                ASSERT(nc <= nw);
                ASSERT(nw <= cap);
                ASSERT(nw <= dstCap);
                ...
                *end = save;
                ...
                bsl::fill(wDstStr.begin(), wDstStr.end(), (WIDE_CHAR)-1);

                WIDE_CHAR *wDstStrData = &wDstStr[0];
                nc2 = nw2 = (bsl::size_t)-1;
                rc2 = Util::utf8ToUtf16(wDstStrData, cap,
                                        bsl::string_view(start, end - start),
                                        &nc2, &nw2, errorWord);
                ASSERT(rc2 == rc);
                ASSERT(nc2 == nc);
                ...
                for (bsl::size_t ii = 0; ii < nw; ++ii) {
                  ASSERT((WIDE_CHAR)dstVec[ii] == wDstStr[ii]);
                }
                ...
                *end = 0;
                ...
                nc2 = nw2 = (bsl::size_t)-1;
                rc2 = Util::utf8ToUtf16(wDstStrData, cap, start, &nc2, &nw2,
                                        errorWord);
                ASSERT(rc2 == rc);
                ASSERT(nc2 == nc);
                ASSERT(nw2 == nw);
...
int main(int argc, char **argv) {
  ...
  case 18: {
    ...
    if (verbose)
      cout << "USAGE EXAMPLE 2\n"
              "===============\n";

    bslma::DefaultAllocatorGuard daGuard(&ta);

    // The following snippets of code illustrate a typical use of the
    // 'bdlde::CharConvertUtf16' struct's utility functions, first converting
    // from UTF-8 to UTF-16, and then converting back to make sure the round
    // trip returns the same value, translating to STL containers in both
    // directions.
    //
    // First, we declare a string of UTF-8 containing single-, double-, triple-,
    // and quadruple-octet code points:
    //..
    const char utf8MultiLang[] = {"Hello" // -- ASCII
                                  "\xce\x97"
                                  "\xce\x95"
                                  "\xce\xbb" // -- Greek
                                  "\xe4\xb8\xad"
    ...
    //
    ASSERT(1 * NUM_ASCII_CODE_POINTS + 2 * NUM_GREEK_CODE_POINTS +
               3 * NUM_CHINESE_CODE_POINTS + 3 * NUM_HINDI_CODE_POINTS +
               4 * NUM_QUAD_CODE_POINTS ==
           bsl::strlen(utf8MultiLang));
    //..
    // Next, we declare the vector where our UTF-16 output will go, and a
    // variable into which the number of code points (not bytes or words)
    ...
    // the vector will automatically be grown to the correct size.  Also note
    // that if 'v16' were not empty, that wouldn't be a problem -- any contents
    // will be discarded.
    //
    // Then, we do the translation to UTF-16:
    //..
    int retVal = bdlde::CharConvertUtf16::utf8ToUtf16(&v16, utf8MultiLang,
                                                      &utf16CodePointsWritten);
    //
    ASSERT(0 == retVal);     // verify success
    ASSERT(0 == v16.back()); // verify null terminated
                             //..
    // Next, we verify that the number of code points (not bytes or words) that
    // was returned is correct:
    //..
    enum {
      EXPECTED_CODE_POINTS_WRITTEN =
    ...
    // UTF-16 words output and the number of bytes input.  The ASCII code points
    // will take 1 16-bit word apiece, the Greek code points are double octets
    // that will become single 'short' values, the Chinese code points are
    // encoded as UTF-8 triple octets that will turn into single 16-bit words,
    // the same for the Hindi code points, and the quad code points are
    // quadruple octets that will turn into double 'short' values:
    //..
    enum {
      SHRINKAGE =
          NUM_ASCII_CODE_POINTS * (1 - 1) + NUM_GREEK_CODE_POINTS * (2 - 1) +
          NUM_CHINESE_CODE_POINTS * (3 - 1) + NUM_HINDI_CODE_POINTS * (3 - 1) +
    ...
    // number of code points (not bytes or words) translated:
    //..
    bsl::string s;
    ...
    //..
    retVal = bdlde::CharConvertUtf16::utf16ToUtf8(&s, v16.begin(),
                                                  &uf8CodePointsWritten);
    //..
    // Finally, we verify that a successful status was returned, that the output
    // of the reverse transform was identical to the original input, and that
    // the number of code points translated was as expected:
    //..
    ASSERT(0 == retVal);
  ...
  case 17: {
    ...
    //..
    // Then, we create a byte buffer to store the UTF-8 result of the
    // translation in, and variables to monitor counts of code points and bytes
    // translated:
    //..
    char utf8String[7];
    ...
    //..
    int rc = bdlde::CharConvertUtf16::utf16ToUtf8(
        utf8String, sizeof(utf8String), utf16String, &numCodePoints, &numBytes);
    //..
    // Then, we observe that no errors or warnings occurred, and that the
    // numbers of code points and bytes were as expected.  Note that both
    // 'numCodePoints' and 'numBytes' include the terminating 0:
    //..
    ASSERT(0 == rc);
    ...
    //..
    // Next, in preparation for translation back to UTF-16, we create a buffer
    // of 'short' values and the variable 'numWords' to track the number of
    // UTF-16 words occupied by the result:
    //..
    ...
    numCodePoints = numWords = -1; // garbage
                                   ...
                                   //..
    rc = bdlde::CharConvertUtf16::utf8ToUtf16(secondUtf16String, 6, utf8String,
                                              &numCodePoints, &numWords);
    //..
    // Next, we observe that no errors or warnings were reported, and that the
    // number of code points and words were as expected.  Note that
    // 'numCodePoints' and 'numWords' both include the terminating 0:
    //..
    ASSERT(0 == rc);
    ASSERT(6 == numCodePoints);
    ASSERT(6 == numWords);
    //..
    // Now, we observe that our output is identical to the original UTF-16
    // string:
    //..
    ASSERT(0 ==
           bsl::memcmp(utf16String, secondUtf16String, sizeof(utf16String)));
    //..
  ...
  case 16: {
    ...
    if (verbose)
      cout << "UTF-8 LENGTH CALCULATION TEST -- INCORRECT UNICODE\n"
              "==================================================\n";
  ...
  case 15: {
    ...
    if (verbose)
      cout << "UTF-16 LENGTH CALCULATION TEST -- INCORRECT UNICODE\n"
              "===================================================\n";
  ...
  case 14: {
    ...
    if (verbose)
      cout << "UTF-16 & UTF-8 LENGTH CALCULATION TEST -- CORRECT UNICODE\n"
              "=========================================================\n";
  ...
  case 13: {
    ...
    if (verbose)
      cout << "BACKWARDS BYTE ORDER TEST\n"
              "=========================\n";

    if (verbose)
      cout << "\tTesting bsl containers\n";

    TestDriver::testCase13<bsl::string, bsl::wstring, bsl::vector<char>,
                           bsl::vector<wchar_t>, bsl::vector<unsigned short>>();

#ifdef BSLS_LIBRARYFEATURES_HAS_CPP11_BASELINE_LIBRARY
    if (verbose)
      cout << "\tTesting bsl containers\n";

    TestDriver::testCase13<bsl::string, bsl::u16string, bsl::vector<char>,
                           bsl::vector<wchar_t>, bsl::vector<unsigned short>>();
#endif
    ...
    TestDriver::testCase13<std::string, std::wstring, std::vector<char>,
                           std::vector<wchar_t>, std::vector<unsigned short>>();
  ...
  case 11: {
    ...
    if (verbose)
      cout << "UTF-16 -> UTF-8: PILE OF BROKEN GLASS TEST\n"
              "==========================================\n";
  ...
  case 10: {
    ...
    if (verbose)
      cout << "UTF-8 -> UTF-16: PILE OF BROKEN GLASS TEST\n"
              "==========================================\n";
  ...
  case 9: {
    ...
    const char **const utf8BrokenEnd =
        utf8Broken + (sizeof(utf8Broken) / sizeof(char *));
    ...
    for (const char **start = utf8Broken; start < utf8BrokenEnd; ++start) {
      ...
      for (int i = 0; i < 2; ++i) {
        ...
        bdlde::CharConvertUtf16::utf8ToUtf16(&dstVec, pcStart, 0, errorWord);

        bdlde::CharConvertUtf16::utf8ToUtf16(
            &dstVecB, bsl::string_view(pcStart, pcEnd - pcStart), 0, errorWord);
    ...
    for (const char **start = utf8Broken; start < utf8BrokenEnd; ++start) {
      ...
      for (int i = 0; i < 2; ++i) {
        ...
        bdlde::CharConvertUtf16::utf8ToUtf16(&dstW, pcStart, 0, errorWord);

        bdlde::CharConvertUtf16::utf8ToUtf16(
            &dstWB, bsl::string_view(pcStart, pcEnd - pcStart), 0, errorWord);
    ...
    for (const char **start = utf8Broken; start < utf8BrokenEnd; ++start) {
      ...
      for (int i = 0; i < 2; ++i) {
        ...
        bdlde::CharConvertUtf16::utf8ToUtf16(&dstU16, pcStart, 0, errorWord);

        bdlde::CharConvertUtf16::utf8ToUtf16(
            &dstU16B, bsl::string_view(pcStart, pcEnd - pcStart), 0, errorWord);
    ...
    if (verbose)
      cout << "With aeroes at random locations\n";
    ...
    for (const char **start = utf8Broken; start < utf8BrokenEnd; ++start) {
      ...
      for (unsigned uu = 0; uu < len; ++uu) {
        ...
        for (int i = 0; i < 2; ++i) {
          ...
          bdlde::CharConvertUtf16::utf8ToUtf16(&dstW, s, 0, errorWord);

          bdlde::CharConvertUtf16::utf8ToUtf16(
              &dstWB, bsl::string_view(s.data(), s.length()), 0, errorWord);
  ...
  case 8: {
    ...
    if (verbose)
      cout << "Error Sequences\n"
              "===============\n";

    const unsigned char errorUnsignedIn[] = {
        'a', 237, 160, 138, 'b', 237, 164, 139, 'c', 237, 168, 147, 'd',
        237, 174, 148, 'e', 237, 178, 166, 'f', 237, 182, 183, 237, 187,
        136, 'g', 237, 191, 153, 'h', 240, 138, 170, 170, 'i', 224, 158,
        162, 'j', 192, 176, 'k', 132, 'm', 234, 170, 'n', 193, 'o', 0};

    const unsigned short expectedUtf16[] = {
        'a', 'x', 'b', 'x', 'c', 'x', 'd', 'x', 'e', 'x',
        'f', 'x', 'x', 'g', 'x', 'h', 'x', 'i', 'x', 'j',
        'x', 'k', 'x', 'm', 'x', 'n', 'x', 'o', 0};
    const unsigned short expectedUtf16Zero[] = {'a', 'b', 'c', 'd', 'e',
                                                ...
                                                'k', 'm', 'n', 'o', 0};
    enum {
      NUM_EXPECTED_UTF16 = sizeof expectedUtf16 / sizeof *expectedUtf16,
      NUM_EXPECTED_UTF16_ZERO =
          sizeof expectedUtf16Zero / sizeof *expectedUtf16Zero
    };

    const unsigned short errorUtf16InOrig[] = {'a',    0xd81a, 'b',    0xdcb2,
                                               'c',    0xd8a2, 0xda21, 'd',
                                               0xdc34, 0xdc23, 'e',    0};
    enum {
      NUM_ERROR_UTF16_IN_ORIG =
          sizeof errorUtf16InOrig / sizeof *errorUtf16InOrig
    };
    ...
    for (int e = 0; e < 2; ++e) {
      ...
      expectedW.assign(errorWord ? expectedUtf16 : expectedUtf16Zero,
                       errorWord
...

      {
        ...
        int rc = Util::utf8ToUtf16(&utf16Wstring, (const char *)errorUnsignedIn,
                                   &nCodePoints, errorWord);
...

      {
        ...
        bsl::size_t nCodePoints;
        int rc = Util::utf8ToUtf16(&utf16Vec, (const char *)errorUnsignedIn,
                                   &nCodePoints, errorWord);
        ASSERT(Status::k_INVALID_INPUT_BIT == rc);
        LOOP2_ASSERT(nCodePoints, numExpectedWords,
                     numExpectedWords == nCodePoints);
    ...
    for (int e = 0; e < 2; ++e) {
...

      {
        ...
        bsl::size_t nCodePoints;
        int rc = Util::utf16ToUtf8(&utf8Vec, errorUtf16InOrig, &nCodePoints,
                                   errorByte);
        ASSERT(Status::k_INVALID_INPUT_BIT == rc);
...

      {
        ...
        int rc = Util::utf16ToUtf8(&utf8String, errorUtf16In, &nCodePoints,
                                   errorByte);
  ...
  case 7: {
    ...
    if (verbose)
      cout << "TESTING VECTOR METHODS\n"
              "======================\n";

    enum { CAPACITY = 1 << 20 };

    unsigned short *utf16S =
        (unsigned short *)ta.allocate(CAPACITY * sizeof(unsigned short));
    ...
    int rc = Util::utf8ToUtf16(utf16S, CAPACITY, (const char *)utf8MultiLang,
                               &numCodePoints16, &numWords16S);
    ASSERT(0 == rc);

    bsl::size_t nc;
    rc = Util::utf8ToUtf16(utf16W, CAPACITY, (const char *)utf8MultiLang, &nc,
                           &numWords16W);
    ASSERT(0 == rc);
...
#if defined(BSLS_LIBRARYFEATURES_HAS_CPP11_BASELINE_LIBRARY)
    char16_t *utf16C16 = (char16_t *)ta.allocate(CAPACITY * sizeof(char16_t));
    bsl::memset(utf16C16, a5, CAPACITY * sizeof(char16_t));
    bsl::size_t numWords16C16 = 0;

    nc = -1;
    rc = Util::utf8ToUtf16(utf16C16, CAPACITY, (const char *)utf8MultiLang, &nc,
                           &numWords16C16);
    ASSERT(0 == rc);
    ASSERT(numWords16S == numWords16C16);
    ASSERT(numCodePoints16 == nc);
    ASSERT(sizeof(unsigned short) == sizeof(char16_t));
    ASSERT(0 == bsl::memcmp(utf16S, utf16C16, CAPACITY * sizeof(char16_t)));
#endif
    numCodePoints16 = numWords16S = -1;

    bsl::memset(utf16S, a5, CAPACITY * sizeof(unsigned short));
    rc = Util::utf8ToUtf16(utf16S, CAPACITY,
    ...
    bsl::memset(utf16W, a5, CAPACITY * sizeof(wchar_t));
    rc = Util::utf8ToUtf16(utf16W, CAPACITY,
                           bsl::string_view((const char *)utf8MultiLang),
                           &numCodePoints16, &numWords16W);
    ASSERT(0 == rc);
    ASSERT(numWords16S == numWords16W);
    ASSERT(numCodePoints16 == nc);
    for (int i = 0; i < (int)numWords16S; ++i) {
    ...
    nc = -1;
    rc = Util::utf8ToUtf16(utf16C16, CAPACITY,
                           bsl::string_view((const char *)utf8MultiLang), &nc,
                           &numWords16C16);
    ASSERT(0 == rc);
    ASSERT(numWords16S == numWords16C16);
    ASSERT(numCodePoints16 == nc);
...

    {
      ...
      nc = -1;
      rc = Util::utf16ToUtf8(buf.data(), sizeof(utf8MultiLang), utf16S, &nc,
                             &numBytesWritten);
      ASSERT(0 == rc);
      ASSERT(numCodePoints16 == nc);
      ...
      rc = Util::utf16ToUtf8(buf.data(), sizeof(utf8MultiLang), utf16S,
                             numWords16S - 1, &nc, &numBytesWritten);
      ASSERT(0 == rc);
      ASSERT(numCodePoints16 == nc);
      ASSERT(!bsl::strcmp(buf.c_str(), charUtf8MultiLang));
      ASSERT(sizeof(utf8MultiLang) == numBytesWritten);
...

    {
      ...
      bsl::size_t numBytesWritten = -1;
      nc = -1;
      rc = Util::utf16ToUtf8(buf.data(), sizeof(utf8MultiLang), utf16W, &nc,
                             &numBytesWritten);
      ASSERT(0 == rc);
      ASSERT(numCodePoints16 == nc);
      ASSERT(!bsl::strcmp(buf.c_str(), charUtf8MultiLang));
      ...
      bsl::fill(buf.begin(), buf.end(), a5);
      numBytesWritten = nc = -1;
      rc = Util::utf16ToUtf8(buf.data(), sizeof(utf8MultiLang),
                             bsl::wstring_view(utf16W, numWords16W - 1), &nc,
                             &numBytesWritten);
      ASSERT(0 == rc);
      ASSERT(numCodePoints16 == nc);
      ASSERT(!bsl::strcmp(buf.c_str(), charUtf8MultiLang));
...
#if defined(BSLS_LIBRARYFEATURES_HAS_CPP11_BASELINE_LIBRARY)
    {
      ...
      bsl::size_t numBytesWritten = -1;
      nc = -1;
      rc = Util::utf16ToUtf8(buf.data(), sizeof(utf8MultiLang), utf16C16, &nc,
                             &numBytesWritten);
      ASSERT(0 == rc);
      ASSERT(numCodePoints16 == nc);
      ASSERT(!bsl::strcmp(buf.c_str(), charUtf8MultiLang));
      ...
      numBytesWritten = nc = -1;
      rc = Util::utf16ToUtf8(buf.data(), sizeof(utf8MultiLang),
                             bsl::u16string_view(utf16C16, numWords16C16 - 1),
                             &nc, &numBytesWritten);
      ASSERT(0 == rc);
      ASSERT(numCodePoints16 == nc);
      ASSERT(!bsl::strcmp(buf.c_str(), charUtf8MultiLang));
...

    {
      ...
      rc = Util::utf8ToUtf16(&utf16Vec, (const char *)utf8MultiLang,
                             &numCodePoints16Vec);
      ASSERT(0 == rc);
      ...
      unsigned short *z16Ptr = bsl::find(utf16Vec.begin(), utf16Vec.end(), 0);
...

    {
      ...
      rc = Util::utf8ToUtf16(&utf16Wstring, charUtf8MultiLang,
                             &numCodePoints16Wstring);
      ASSERT(0 == rc);

      ASSERT(numCodePoints16Wstring == numCodePoints16);
      ASSERT(numWords16S == utf16Wstring.length() + 1);
      ASSERT(localUtf16Len(utf16Wstring.c_str()) == utf16Wstring.length());
...

    {
      ...
      bsl::fill(utf16Wstring.begin(), utf16Wstring.end(), a5);
      numCodePoints16Wstring = -1;
      rc = Util::utf8ToUtf16(&utf16Wstring, bsl::string_view(charUtf8MultiLang),
                             &numCodePoints16Wstring);
      ASSERT(0 == rc);
...
#if defined(BSLS_LIBRARYFEATURES_HAS_CPP11_BASELINE_LIBRARY)
    {
      ...
      rc = Util::utf8ToUtf16(&utf16U16Str, (const char *)utf8MultiLang, &nc);
      ASSERT(0 == rc);
      ...
      nc = -1;
      rc = Util::utf8ToUtf16(
          &utf16U16Str, bsl::string_view((const char *)utf8MultiLang), &nc);
      ASSERT(0 == rc);
...

    {
      std::vector<char> utf8Vec;
      {
        ...
        rc = Util::utf16ToUtf8(&utf8Vec, utf16S, &numCodePoints8);
        ASSERT(0 == rc);
        ...
        rc = Util::utf16ToUtf8(&utf8Vec, utf16S, numWords16S - 1,
                               &numCodePoints8);
        ASSERT(0 == rc);
      ...
      }
      {
        ...
        rc = Util::utf16ToUtf8(&utf8Vec, utf16W, &numCodePoints8);
        ASSERT(0 == rc);
        ...
        rc = Util::utf16ToUtf8(&utf8Vec,
                               bsl::wstring_view(utf16W, (int)numWords16W - 1),
                               &numCodePoints8);
        ASSERT(0 == rc);
...
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
    {
          ...
          std::experimental::pmr::new_delete_resource());
      {
        ...
        rc = Util::utf16ToUtf8(&utf8VecP, utf16S, &numCodePoints8);
        ASSERT(0 == rc);
        ...
        rc = Util::utf16ToUtf8(&utf8VecP, utf16S, numWords16S - 1,
                               &numCodePoints8);
        ASSERT(0 == rc);
      ...
      }
      {
        ...
        rc = Util::utf16ToUtf8(&utf8VecP, utf16W, &numCodePoints8);
        ASSERT(0 == rc);
...

    {
...
#if defined(BSLS_LIBRARYFEATURES_HAS_CPP11_BASELINE_LIBRARY)
      {
        ...
        rc = Util::utf16ToUtf8(&utf8Vec, utf16C16, &numCodePoints8);
        ASSERT(0 == rc);
...

      {
        ...
        rc = Util::utf16ToUtf8(&utf8String, utf16S, &numCodePoints8);
        ASSERT(0 == rc);
      ...
      }
      {
        ...
        rc = Util::utf16ToUtf8(&utf8String, utf16W, &numCodePoints8);
        ASSERT(0 == rc);
...
#if defined(BSLS_LIBRARYFEATURES_HAS_CPP11_BASELINE_LIBRARY)
      {
        ...
        rc = Util::utf16ToUtf8(&utf8String, utf16C16, &numCodePoints8);
        ASSERT(0 == rc);
      ...
      }
      {
        ...
        rc = Util::utf16ToUtf8(&utf8String, utf16S, numWords16S - 1,
                               &numCodePoints8);
        ASSERT(0 == rc);
  ...
  case 6: {
    ...
    if (verbose)
      cout << "Long UTF-8 Multilingual test case\n"
              "=================================\n";
    ...
    for (unsigned char *pc = utf8MultiLang; *pc; ++pc) {
      if (0 == (0x80 & *pc))
        ++singleBytes;
      else if (0xc0 == (0xe0 & *pc))
        ++doubleHeaders;
      else if (0xe0 == (0xf0 & *pc))
        ++tripleHeaders;
      else if (0xf0 == (0xf8 & *pc))
        ++quadHeaders;
      else if (0x80 == (0xc0 & *pc))
        ++contBytes;
      else
        ++errorBytes;
    }

    const unsigned CODE_POINTS =
        singleBytes + doubleHeaders + tripleHeaders + quadHeaders + 1;
    const unsigned UTF16_WORDS =
        singleBytes + doubleHeaders + tripleHeaders + 2 * quadHeaders + 1;
    ...
    ASSERT(doubleHeaders + tripleHeaders * 2 + quadHeaders * 3 == contBytes);
    ASSERT(0 == errorBytes);
    ...
    int rc = Util::utf8ToUtf16(utf16S, CAPACITY, charUtf8MultiLang,
                               &numCodePoints16, &numWords16S);
    ...
    rc = Util::utf8ToUtf16(utf16SB, CAPACITY,
                           bsl::string_view(charUtf8MultiLang),
                           &numCodePoints16B, &numWords16SB);
    ...
    ASSERTV(CODE_POINTS, numCodePoints16B, CODE_POINTS == numCodePoints16B);
    ...
    ASSERT(0 ==
           std::memcmp(utf16S, utf16SB, numWords16S * sizeof(unsigned short)));
...
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP11_BASELINE_LIBRARY
    char16_t *utf16C = (char16_t *)ta.allocate(CAPACITY * sizeof(char16_t));
    std::size_t numCodePoints16C = 0, numWords16C = 0;

    rc = Util::utf8ToUtf16(utf16C, CAPACITY, charUtf8MultiLang,
                           &numCodePoints16C, &numWords16C);
    ...
    ASSERTV(CODE_POINTS, numCodePoints16C, CODE_POINTS == numCodePoints16C);
    ...
    char16_t *utf16CB =
        static_cast<char16_t *>(ta.allocate(CAPACITY * sizeof(char16_t)));
    numCodePoints16C = 0, numWords16C = 0;

    rc = Util::utf8ToUtf16(utf16CB, CAPACITY,
                           bsl::string_view(charUtf8MultiLang),
                           &numCodePoints16C, &numWords16C);
    ...
    ASSERT(0 == std::memcmp(utf16C, utf16CB, numWords16C * sizeof(char16_t)));

    ASSERT(0 == std::memcmp(utf16S, utf16C, numWords16C * sizeof(char16_t)));

    numCodePoints16C = 0;
    rc = Util::utf16ToUtf8(utf8, CAPACITY, utf16C, &numCodePoints16C,
                           &numBytes8);
    ASSERT(0 == rc);
    ...
    rc = Util::utf16ToUtf8(utf8, CAPACITY, bsl::u16string_view(utf16C),
                           &numCodePoints16C, &numBytes8);
    ASSERT(0 == rc);
    ...
    wchar_t *utf16W =
        static_cast<wchar_t *>(ta.allocate(CAPACITY * sizeof(wchar_t)));
    bsl::size_t numWords16W = 0;
    numCodePoints16 = 0;

    rc = Util::utf8ToUtf16(utf16W, CAPACITY, charUtf8MultiLang,
                           &numCodePoints16, &numWords16W);
    ...
    rc = Util::utf8ToUtf16(utf16WB, CAPACITY,
                           bsl::string_view(charUtf8MultiLang),
                           &numCodePoints16B, &numWords16WB);
    ...
    ASSERT(0 == bsl::memcmp(utf16W, utf16WB, numWords16W * sizeof(wchar_t)));

    numCodePoints8 = 0, numBytes8 = 0;

    bsl::memset(utf8, 0xa5, CAPACITY);

    rc = Util::utf16ToUtf8(utf8, CAPACITY, utf16S, &numCodePoints8, &numBytes8);
    ...
    rc = Util::utf16ToUtf8(&s, utf16S, &numCodePoints8);
    ...
    rc = Util::utf16ToUtf8(&v, utf16S, &numCodePoints8);
    ...
    rc = Util::utf16ToUtf8(utf8, CAPACITY, utf16S, numWords16W - 1,
                           &numCodePoints8, &numBytes8);
    if (verbose) {
      Q(utf16ToUtf8:);
      P_(rc) P_(numCodePoints8) P(numBytes8);
    }
    ...
    rc = Util::utf16ToUtf8(&s, utf16S, numWords16S - 1, &numCodePoints8);
    if (verbose) {
      Q(utf16ToUtf8:);
      P_(rc) P(numCodePoints8);
    }
    ...
    rc = Util::utf16ToUtf8(&v, utf16S, numWords16S - 1, &numCodePoints8);
    ...
    rc = Util::utf16ToUtf8(utf8, CAPACITY, utf16W, &numCodePoints8, &numBytes8);
    ...
    rc = Util::utf16ToUtf8(utf8, CAPACITY,
                           bsl::wstring_view(utf16W, (int)numWords16W - 1),
                           &numCodePoints8, &numBytes8);
  ...
  case 5: {
    ...
    enum {
      MAX_NCHARS = 5 * 5 * 5,           // String contains no repetitions of
    ...
    Permuter<5> ptx5;
    do {
      for (OdomIter<AvCharList::iterator, 4> wheelset(wheelsFive); wheelset;
           wheelset.next()) {
        ...
        for (int ic = 0;
          ...
          for (const char *ccase = u8CodingCases[ic]; *ccase; ++ccase) {
            ...
            case 'b': {
              // Two-byte code point:
              *genp++ = static_cast<unsigned short>(0xc0 | twoHdr);
              *genp++ = static_cast<unsigned short>(0x80 | twoContin);
              *imgp++ = static_cast<unsigned short>(twoHdr << 6 | twoContin);
            ...
            case 'c': {
              ...
              *genp++ = static_cast<unsigned short>(0xe0 | threeHdr);
              *genp++ = static_cast<unsigned short>(0x80 | threeContin1);
              *genp++ = static_cast<unsigned short>(0x80 | threeContin2);
              *imgp++ = static_cast<unsigned short>(
                  threeHdr << 12 | threeContin1 << 6 | threeContin2);
            ...
            case 'd': {
              ...
              *genp++ = 0x80 | fourContin3;
              {
                unsigned int isoChar = (fourHdr << 18 | fourContin1 << 12 |
                                        fourContin2 << 6 | fourContin3) -
                                       0x10000;
                *imgp++ = static_cast<unsigned short>(0xd800 | isoChar >> 10);
                *imgp++ =
                    static_cast<unsigned short>(0xdc00 | (isoChar & 0x3ff));
              }
          ...
          *imgp++ = '\0';
          ...
          if (veryVerbose) {
            ...
            cout << "String " << R_(nSymbols) << R_(nOctets) << R(nWords)
                 << endl;
            if (veryVeryVerbose) {
          ...
          enum {
            BUFFER_ZONE = 32 // Margin to fill and check for
          ...
          GenCheckArrRange<ArrayRange<unsigned short>> imageGenCh(imageArray);

          char source[MAX_NOCTETS];
          ArrayRange<char> sourceArray(source, nOctets);

          unsigned short dest[MAX_NWORDS + 2 * BUFFER_ZONE];
          BufferedWPiece<unsigned short> bwp(dest, MAX_NWORDS + 2 * BUFFER_ZONE,
                                             nWords, 0xffff, BUFFER_ZONE);

          if (!oneStringConversion(__LINE__, bwp, imageGenCh, sourceArray,
                                   genGenCh, ConvRslt(0, nSymbols, nWords))) {
            cout << "(Error converting UTF-8 to UTF-16.)" << endl;

            if (veryVeryVerbose) {
              cout << "Expected " << R(prHexRange(imageArray)) << endl;
            }
          ...
          // backwards generator.
          ArrayRange<unsigned short> invGenArray(bwp.begin(), bwp.end());
          GenCheckArrRange<ArrayRange<unsigned short>> invGenGen(invGenArray);

          unsigned short invSource[MAX_NWORDS];
          ArrayRange<unsigned short> invSourceArray(invSource, nWords);

          char invDest[MAX_NOCTETS + 2 * BUFFER_ZONE];
          BufferedWPiece<char> invBwp(invDest, MAX_NOCTETS + 2 * BUFFER_ZONE,
                                      nOctets, (char)0xff, BUFFER_ZONE);

          if (!oneStringConversion(__LINE__, invBwp, genGenCh, invSourceArray,
                                   invGenGen, ConvRslt(0, nSymbols, nOctets))) {
            cout << "(Error converting UTF-16 to UTF-8.)" << endl;
            if (veryVeryVerbose) {
              cout << "Expected " << R(prHexRange(genArray)) << endl;
            }
  ...
  case 4: {
    ...
    {
      if (verbose)
        cout << "Disallowed Octets:\n";

      const unsigned char DISALLOWED[] = {0xf8, 0xf9, 0xfa, 0xfb, 0xfc,
                                          0xfd, 0xfe, 0xff, 0};
      ...
      for (const unsigned char *pc = DISALLOWED; *pc; ++pc) {
        ...
        input[1] = *pc;
        int rc = Util::utf8ToUtf16(output, sizeof(output), input, &nc, &nw, 0);
    ...
    {
      if (verbose)
        cout << "Continuation Octets out of place:\n";
    ...
    if (verbose)
      cout << "Missing continuation octets\n";

    {
      ...
      for (unsigned h = 2; h <= 4; ++h) {
        for (unsigned c = 0; c < h - 1; ++c) {
          ...
          int rc =
              Util::utf8ToUtf16(output, sizeof(output), input, &nc, &nw, 0);
          ASSERT(1 == rc);
          ASSERT(6 - 1 - c == nc);
          ASSERT(6 - 1 - c == nw);
          ASSERT((unsigned)input[1 + c] == output[0]);

          rc = Util::utf8ToUtf16(output, sizeof(output),
                                 bsl::string_view(input), &nc, &nw, 0);
          ASSERT(1 == rc);
          ASSERT(6 - 1 - c == nc);
          ASSERT(6 - 1 - c == nw);
          ASSERT((unsigned)input[1 + c] == output[0]);
  ...
  case 3: {
    ...
    enum {
      BUFFER_ZONE = 128 // These tests create a space of BUFFER_ZONE
...

    {
      ...
      Conversion<unsigned short, char> u8ToU16(surrogateUtf8ToUtf16);
      ...
      WorkPiece<unsigned short> wp(Sizes::TO_BUF_SIZE, source.d_dstBufSize,
                                   0xffff, BUFFER_ZONE);
      ConvRslt result;
      ...
      if (RUN_AND_CHECK(wp, u16, result, u8ToU16, source, expected)) {
        for (unsigned u8c = 1; u8c < 0x80; ++u8c) {
          ...
          if (!EXPECTED_GOT(u8c, (unsigned)u8[u8c - 1])) {
            cout << "\tSource string damaged at ";
            P(u8c - 1);
          }
        }
...

    {
      ...
      enum {
        TWO_OCTET_POW_TWO = 1 << 11 // Five bits in the header
                                    // octet, six in the
                                    // continuation.
      };
      ...
      if (RUN_AND_CHECK(wp, u16, result, u8ToU16, source, expected)) {
        ...
        for (unsigned u8c = 0x80; u8c < 0x800; ++u8c) {
          ...
          if (!EXPECTED_GOT(0xc0 | (u8c >> 6), deChar(u8[2 * pos]))) {
            cout << "\tat ";
            P(u8c);
          }
          if (!EXPECTED_GOT(0x80 | (u8c & 0x3f), deChar(u8[2 * pos + 1]))) {
            cout << "\tat ";
            P(u8c);
          }
        }
    ...
    {
      enum {
        THREE_OCTET_POW_TWO = 1 << 16, // Four bits in the header
                                       // octet, six in each
                                       // continuation
      ...
      for (unsigned iFirst = 0x0; iFirst < THREE_OCTET_LIM; ++iFirst) {
        ...
        int pos = 0;
        for (unsigned iSecond = rangeStart; iSecond < rangeLimit; ++iSecond) {
          for (unsigned iThird = 0x0; iThird < CONTIN_LIM; ++iThird) {
        ...
        if (!RUN_AND_CHECK(wp, u16, result, u8ToU16, source, expected)) {
          ...
          for (unsigned iSecond = rangeStart; iSecond < rangeLimit; ++iSecond) {
            for (unsigned iThird = 0x0; iThird < CONTIN_LIM; ++iThird) {
              unsigned val = iFirst << 12 | iSecond << 6 | iThird;
              unsigned short at = static_cast<unsigned short>(pos++);

              if (!EXPECTED_GOT(val, wp.begin(u16)[at])) {
                cout << R_(iFirst) << R_(iSecond) << R_(iThird) << R(at)
                     << endl;
              }

              if (!EXPECTED_GOT(0xe0 | iFirst, deChar(u8[3 * at + 0]))) {
                cout << "\tdamaged source code point at" << 3 * pos + 0 << endl;
              }

              if (!EXPECTED_GOT(0x80 | iSecond, deChar(u8[3 * at + 1]))) {
                cout << "\tdamaged source code point at" << 3 * pos + 1 << endl;
              }

              if (!EXPECTED_GOT(0x80 | iThird, deChar(u8[3 * at + 2]))) {
                cout << "\tdamaged source code point at" << 3 * pos + 2 << endl;
              }
            }
          }
    ...
    {
      enum {
        FOUR_OCTET_POW_TWO = 1 << 21, // Three bits in the header
                                      // octet, six in each
                                      // continuation
        CONTIN_LIM = 1 << 6           // Six content bits in a
                                      // continuation octet
      ...
      for (unsigned iFirst = 0x0; iFirst <= 0x4; ++iFirst) {
        ...
        for (unsigned iSecond = rangeStart; iSecond < rangeLimit; ++iSecond) {
          int pos = 0;
          for (unsigned iThird = 0x0; iThird < CONTIN_LIM; ++iThird) {
            for (unsigned iFourth = 0x0; iFourth < CONTIN_LIM; ++iFourth) {
          ...
          WorkPiece<unsigned short> wp(
              Sizes::FROM_BUF_SIZE, source.d_dstBufSize, 0xffff, BUFFER_ZONE);
          ConvRslt result;
          ...
          if (!RUN_AND_CHECK(wp, u16, result, u8ToU16, source, expected)) {
            ...
            for (unsigned iThird = 0x0; iThird < CONTIN_LIM; ++iThird) {
              for (unsigned iFourth = 0x0; iFourth < CONTIN_LIM; ++iFourth) {
                unsigned val =
                    iFirst << 18 | iSecond << 12 | iThird << 6 | iFourth;
                cout << hex;
                if (!EXPECTED_GOT(((val - 0x10000) >> 10) | 0xd800,
                                  // Needless parentheses here to
                                  // silence g++ warning.
                                  wp.begin(u16)[at])) {
                  cout << R_(iFirst) << R_(iSecond) << R_(iThird) << R_(iFourth)
                       << R(at * 2) << endl;
                  cout << R_(val) << R(val - 0x10000) << endl;
                }
                if (!EXPECTED_GOT(((val - 0x10000) & 0x3ff) | 0xdc00,
                                  wp.begin(u16)[at + 1])) {
                  cout << R_(iFirst) << R_(iSecond) << R_(iThird) << R_(iFourth)
                       << R(at * 2 + 1) << endl;
                  cout << R_(val) << R(val - 0x10000) << endl;
                ...
                if (!EXPECTED_GOT(0xf0 | iFirst, deChar(u8[2 * at + 0]))) {
                  cout << "\tdamaged source code point at" << 2 * at + 0
                       << endl;
                }

                if (!EXPECTED_GOT(0x80 | iSecond, deChar(u8[2 * at + 1]))) {
                  cout << "\tdamaged source code point at" << 2 * at + 1
                       << endl;
                }

                if (!EXPECTED_GOT(0x80 | iThird, deChar(u8[2 * at + 2]))) {
                  cout << "\tdamaged source code point at" << 2 * at + 2
                       << endl;
                }

                if (!EXPECTED_GOT(0x80 | iFourth, deChar(u8[2 * at + 3]))) {
                  cout << "\tdamaged source code point at" << 2 * at + 3
                       << endl;
                }
            ...
            if (!EXPECTED_GOT(0, deChar(u8[2 * at]))) {
              cout << "\tdamaged source code point at" << 2 * at << endl;
            }
          }
        }
      }
...

    {
      ...
      Conversion<char, unsigned short> u16ToU8(
          bdlde::CharConvertUtf16::utf16ToUtf8);
      ...
      WorkPiece<char> wp(Sizes::TO_BUF_SIZE, source.d_dstBufSize, (char)0xff,
                         BUFFER_ZONE);
      ConvRslt result;
      ...
      if (RUN_AND_CHECK(wp, u8, result, u16ToU8, source, expected)) {
        ...
        for (unsigned u16c = 1; u16c < 0x80; ++u16c) {
          if (!EXPECTED_GOT(u16c, (unsigned)wp.begin(u8)[u16c - 1])) {
            cout << "\tat ";
            P(u16c - 1);
          }
        }
...

    {
      ...
      if (RUN_AND_CHECK(wp, u8, result, u16ToU8, source, expected)) {
        for (unsigned u8c = 0x80; u8c < 0x800; ++u8c) {
          ...
          if (!EXPECTED_GOT(deChar(wp.begin(u8)[at]), 0xc0 | u8c >> 6)) {
            cout << "\tat ";
            P(u8c - 0x80);
          }
...

    {
      ...
      for (unsigned iFirst = 0x0; iFirst < THREE_OCTET_LIM; ++iFirst) {
        ...
        // cout << hex << R_(iFirst) << R_(rangeStart) << R(rangeLimit) << dec
        // << endl;
        int pos = 0;
        for (unsigned iSecond = rangeStart; iSecond < rangeLimit; ++iSecond) {
          for (unsigned iThird = 0x0; iThird < CONTIN_LIM; ++iThird) {
            ...
            u16[pos++] = static_cast<unsigned short>(
                deChar(iFirst) << 12 | deChar(iSecond) << 6 | deChar(iThird));
        ...
        if (!RUN_AND_CHECK(wp, u8, result, u16ToU8, source, expected)) {
          ...
          for (unsigned iSecond = rangeStart; iSecond < rangeLimit; ++iSecond) {
            for (unsigned iThird = 0x0; iThird < CONTIN_LIM; ++iThird) {
              // cout << hex << R_(iSecond) << R_(iThird) << R(pos) << dec <<
              // endl ;
              if (!EXPECTED_GOT(0xe0 | iFirst, deChar(wp.begin(u8)[pos]))) {
                cout << "\tat " << R_(iFirst) << R_(iSecond) << R(iThird)
                     << endl;
              }
              ...
              if (!EXPECTED_GOT(0x80 | iThird, deChar(wp.begin(u8)[pos + 2]))) {
                cout << "\tat " << R_(iFirst) << R_(iSecond) << R(iThird)
                     << endl;
              }
...

    {
      ...
      for (unsigned iFirst = 0x0; iFirst <= 0x4; ++iFirst) {
        ...
        for (unsigned iSecond = rangeStart; iSecond < rangeLimit; ++iSecond) {
          ...
          for (unsigned iThird = 0x0; iThird < CONTIN_LIM; ++iThird) {
            for (unsigned iFourth = 0x0; iFourth < CONTIN_LIM; ++iFourth) {
              // cout << hex << R_(iThird) << R_(iFourth) << R(pos) << dec <<
              // endl ;
              unsigned long convBuf =
                  ((iFirst << 18) | (iSecond << 12) | (iThird << 6) | iFourth) -
                  0x10000;
              u16[pos++] =
                  static_cast<unsigned short>(0xd800 | (convBuf >> 10));
              u16[pos++] =
                  static_cast<unsigned short>(0xdc00 | (convBuf & 0x3ff));
          ...
          int nchar = pos / 2 + 1; // All the code points we built,
                                   ...
                                   // code points, except for the
                                   // null, will require two words.
          SrcSpec<unsigned short> source(u16, 0, 4 * Sizes::N_CHARS + 1);
          ConvRslt expected(0, nchar, pos * 2 + 1); // The null is
                                                    // still just
                                                    // one word.

          WorkPiece<char> wp(Sizes::TO_BUF_SIZE, source.d_dstBufSize,
                             (char)0xff, BUFFER_ZONE);
          ConvRslt result;
          ...
          if (!RUN_AND_CHECK(wp, u8, result, u16ToU8, source, expected)) {
            ...
            for (unsigned iThird = 0x0; iThird < CONTIN_LIM; ++iThird) {
              for (unsigned iFourth = 0x0; iFourth < CONTIN_LIM; ++iFourth) {
                cout << hex;
                if (!EXPECTED_GOT(deChar(wp.begin(u8)[at]), 0xf0 | iFirst)) {
                  cout << R_(iFirst) << R_(iSecond) << R_(iThird) << R_(iFourth)
                       << R(at * 2) << endl;
                }
                if (!EXPECTED_GOT(deChar(wp.begin(u8)[at + 1]),
    ...
    if (!EXPECTED_GOT(nEightToSixteen, (0x110000 - 1 - 0x800)) && verbose) {
      cout << "\t(Not enough UTF-8 to UTF-16 conversions to "
              "cover the whole range.)"
           << endl;
    }

    if (!EXPECTED_GOT(nSixteenToEight, (0x110000 - 1 - 0x800)) && verbose) {
  ...
  case 2: {
    ...
    enum {
      BUFFER_ZONE = 32, // These tests create a space of BUFFER_ZONE
                        // memory units (bytes or words) around the
                        // output string space and fill it before
                        // testing the conversion.  They check
                        // afterwards that the values stored are
    ...
    // Subtest 2a1: UTF-8 => UTF-16, single byte
    {
      ...
      for (AvCharList::iterator c1i = casesOne.begin(); casesOne.end() != c1i;
           ++c1i) {
        u8[0] = *c1i;
        u8[1] = 0;
        ...
        FourWayRunner<unsigned short, char> runner(u16, 0xffff, BUFFER_ZONE,
                                                   source, u8ToU16, expected);
    ...
    // Subtest 2a3: UTF-8 => UTF-16, three byte
    {
      ...
      for (; c3C2; c3C2.next()) {
        ...
        for (AvCharList::iterator cC3i = casesContin.begin();
          ...
          EXPECTED_GOT((high << 12 | mid << 6 | low), runner.begin(0)[0]);
        }
      }
    }
        ...
        for (; c3C4; c3C4.next()) {
          ...
          if (!RUN_FOUR_WAYS(runner)) {
            if (veryVerbose) {
              ...
              cout << "==========" << endl
                   << hex << R_(deChar(*c4i)) << R_(deChar(*cC2i))
                   << R_(deChar(*cC3i)) << R(deChar(*cC4i)) << dec << endl
                   << prHexRange(a) << endl;
            }
          }
          ...
          EXPECTED_GOT((utf16Conv >> 10) | 0xd800, runner.begin(0)[0]);
          EXPECTED_GOT((utf16Conv & 0x3ff) | 0xdc00, runner.begin(0)[1]);
        }
      }
    }
    ...
    // Subtest 2b1: UTF-16 => UTF-8, single byte
    {
      ...
      for (AvCharList::iterator c1i = casesOne.begin(); casesOne.end() != c1i;
        ...
        FourWayRunner<char, unsigned short> runner(u8, (char)0xff, BUFFER_ZONE,
                                                   source, u16ToU8, expected);
        RUN_FOUR_WAYS(runner);
        EXPECTED_GOT(*c1i, runner.begin(0)[0]);
      }
    }
    ...
    // Subtest 2b3: UTF-16 => UTF-8, three-byte
    {
      ...
      for (; c3C2; c3C2.next()) {
        ...
        for (AvCharList::iterator cC3i = casesContin.begin();
             casesContin.end() != cC3i; ++cC3i) {
          u16[0] = static_cast<unsigned short>(*c3i << 12 | *cC2i << 6 | *cC3i);
          u16[1] = 0;

          SrcSpec<unsigned short> source(u16, 0, 4);
          ConvRslt expected(0, 2, 4);

          FourWayRunner<char, unsigned short> runner(
              u8, (char)0xff, BUFFER_ZONE, source, u16ToU8, expected);
          RUN_FOUR_WAYS(runner);
    ...
    // Subtest 2b4: UTF-16 => UTF-8, four byte
    {
      ...
      for (; c4C2; c4C2.next()) {
        ...
        for (; c3C4; c3C4.next()) {
          ...
          unsigned long iso10646 =
              (*c4i << 18 | *cC2i << 12 | *cC3i << 6 | *cC4i) - 0x10000;
          u16[0] = static_cast<unsigned short>(0xd800 | (iso10646 >> 10));
          u16[1] = static_cast<unsigned short>(0xdc00 | (iso10646 & 0x3ff));
  ...
  case 1: {
    ...
    enum {
      BUFFER_ZONE = 256 // How much memory to check before and
    ...
    //               RUN_FOUR_WAYS
    {
      ...
      if (veryVerbose) {
        cout << "UTF-8 => utf16, null string, room for the null ..." << endl;
      }
    ...
    }
    {
      ...
      if (veryVerbose) {
        cout << "UTF-16 => UTF-8, null string, room for the null ..." << endl;
      }
      ...
      FourWayRunner<char, unsigned short> runner(
          u8, static_cast<char>(0xff), BUFFER_ZONE, source, u16ToU8, expected);
      RUN_FOUR_WAYS(runner);
    ...
    }
    {
      ...
      if (veryVerbose) {
        cout << "UTF-8 => UTF-16, null string, no room for the null ...\n";
      }
    ...
    }
    {
      ...
      if (veryVerbose) {
        cout << "UTF-16 => UTF-8, null string, no room for the null ...\n";
      }
...
template <class T> ostream &operator<<(ostream &os, const MixedPrImpl<T> &t) {
  ...
  for (int i = 0; i < t.d_av.size();) {
    for (; i < t.d_size && static_cast<unsigned int>(t.d_array[i]) < 0x80 &&
           isgraph(t.d_ptr[i]);
         ++i) {
...
int strEq(const CHAR_TYPE *lhs, const CHAR_TYPE *rhs) {
  for (;; ++lhs, ++rhs) {
    if (*lhs != *rhs) {
      return 0; // RETURN
    }
...
void checkForExpectedConversionResultsU2ToU8(unsigned short *input,
                                             ...
                                             bsl::size_t totalOutputLength,
                                             unsigned short *codePointSizes,
                                             bsl::size_t codePointCount, int,
                                             int veryVerbose) {
  int retVal;

  if (veryVerbose) {
    cout << "checkForExpectedConversionResultsU2ToU8("
         << "\n\tinput             =" << prHexRange(input, codePointCount + 1)
         << ",\n\texpected_output   =";
  ...
  for (int bufSize = 0; bufSize < (int)totalOutputLength; ++bufSize) {
    ...
    retVal = bdlde::CharConvertUtf16::utf16ToUtf8(
        outputBuffer, bufSize, input, &codePointsWritten, &bytesWritten);

    LOOP5_ASSERT(L_, OUTPUT_BUFFER_TOO_SMALL, retVal, bufSize,
                 totalOutputLength, OUTPUT_BUFFER_TOO_SMALL == retVal);
  ...
  retVal = bdlde::CharConvertUtf16::utf16ToUtf8(
      outputBuffer, totalOutputLength + 1, input, &codePointsWritten,
      &bytesWritten);
  ...
  LOOP5_ASSERT(L_, totalOutputLength + 1, bytesWritten,
...
void buildUpAndTestStringsU2ToU8(
  ...
  *outputCursor = 0;
  ...
  checkForExpectedConversionResultsU2ToU8(inputBuffer, outputBuffer,
                                          totalOutputLength, codePointSizes,
                                          codePointCount, verbose, veryVerbose);
  ...
  *inputCursor = 0;
  ...
  for (int i = 0; i < (int)precomputedDataCount; ++i) {
    buildUpAndTestStringsU2ToU8(i, depth - 1, inputBuffer, outputBuffer,
                                codePointSizes, totalOutputLength,
                                codePointCount, inputCursor, outputCursor,
                                verbose, veryVerbose);
  }
...
int runPlainTextPerformanceTest(void) {
  ...
  for (int i = 0; i < iterLimit; ++i) {
    ASSERT(SUCCESS ==
           bdlde::CharConvertUtf16::utf8ToUtf16(
               utf16Buffer_p, prideLen, prideAndPrejudice, &codePointsWritten));
    ASSERT(codePointsWritten == prideLen);

    ASSERT(SUCCESS == bdlde::CharConvertUtf16::utf8ToUtf16(
                          utf16Buffer_p, prideLen,
                          bsl::string_view(prideAndPrejudice),
                          &codePointsWritten));
    ASSERT(codePointsWritten == prideLen);

    ASSERT(0 == bdlde::CharConvertUtf16::utf16ToUtf8(
                    utf8Buffer_p, prideLen, utf16Buffer_p, &codePointsWritten,
                    &bytesWritten));
    ASSERT(codePointsWritten == prideLen);
...
ostream &operator<<(ostream &os, const ConvRslt &cvr) {
  ...
  return os << "ret 0x" << hex << cvr.d_retVal << setbase(flags) << " symbols "
            << cvr.d_symbols << " units " << cvr.d_units;
}
...
void WorkPiece<CHAR_TYPE>::fillMargins(CHAR_TYPE *mem) {
  ...
  fillArray(&mem[d_margin + d_winLength], &mem[2 * d_margin + d_winLength],
            d_fillChar);
...
int WorkPiece<CHAR_TYPE>::checkMargins(const CHAR_TYPE *mem) const {
  return checkFill(&mem[0], &mem[d_margin], d_fillChar)
         ...
         && checkFill(&mem[d_margin + d_winLength],
                      &mem[2 * d_margin + d_winLength], d_fillChar);
...
template <class CHAR_TYPE>
ostream &operator<<(ostream &os, const ArrayRange<CHAR_TYPE> &sv) {
  ios_base::fmtflags flags = os.flags();
  char fill = os.fill(' ');
  os << bsl::right << "[";
  ...
  for (const CHAR_TYPE *cp = sv.d_arrayr; *cp;) {
    for (; *cp && static_cast<unsigned int>(*cp) < 0x80 && isgraph(*cp); ++cp) {
    ...
    os << hex;
    for (; static_cast<unsigned int>(*cp) >= 0x80 || (*cp && !isgraph(*cp));
         ++cp) {
      os << ' ' << setw(4) << deChar(*cp);
    }
    os << dec;
...
// succeed, or 'false' if any have failed.
{
  ...
  WorkPiece<TO_CHAR> wp(toBuf.size(), source.d_dstBufSize,
                        (TO_CHAR) ~(TO_CHAR)0, BUFFER_ZONE);
  ...
  if (!RUN_AND_CHECK(wp, to, result, conversion, source, expected)) {
    cout << "\tFailed on converting " << prHexRange(fromBuf) << " from line "
         << line << "/" << __LINE__ << "." << endl;
  ...
  wp = WorkPiece<TO_CHAR>(toBuf.size(), source.d_dstBufSize,
                          (TO_CHAR) ~(TO_CHAR)0, BUFFER_ZONE);

  if (!RUN_AND_CHECK(wp, to, result, conversion, source, expected)) {
    cout << "\tFailed on converting " << prHexRange(fromBuf) << " from line "
         << line << "/" << __LINE__ << " surrounded by 'A' and 'B'." << endl;
  ...
  if (veryVerbose) {
    cout << " - Surrounded by 'A' and 'B', replacement char '#'" << endl;
  }
...
// if any have failed.
{
  ...
  if (!RUN_AND_CHECK(bwp.workpiece(), bwp.buffer(), result, conversion, source,
                     expected)) {
    cout << "\tFailed on converting at line " << line << "/" << __LINE__ << "."
         << endl;

    if (veryVeryVerbose) {
      cout << "From " << prHexRange(fromBuf.begin(), fromBuf.end()) << endl;
...
template <bsl::size_t N> ostream &Permuter<N>::print(ostream &os) const {
  os << "(";
  for (int i = 0; i < N; ++i)
    os << " " << d_val[i];
  return os << " )";
