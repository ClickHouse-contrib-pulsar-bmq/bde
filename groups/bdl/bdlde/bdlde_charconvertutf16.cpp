...
enum {
  ...
  OUT_OF_SPACE_BIT = BloombergLP::bdlde::CharConvertStatus::k_OUT_OF_SPACE_BIT
};
...
template <class UTF16_WORD, unsigned UTF16_WORD_SIZE>
...
template <>
inline unsigned short swappedToHost<unsigned short, 2>(unsigned short uc)
// Return the value of the specified 'uc' in host byte order, where 'uc' is
// assumed to be in a swapped state.
{
...
template <>
inline unsigned short hostToSwapped<unsigned short, 2>(unsigned short uc)
// Return the value of the specified 'uc' with its bytes swapped, where
// 'uc' is assumed to be in host byte order.
{
  return BloombergLP::bsls::ByteOrderUtil::swapBytes(uc);
...
template <>
inline wchar_t swappedToHost<wchar_t, 2>(wchar_t uc)
// Return the value of the specified 'uc' in host byte order, where 'uc' is
// assumed to be in a swapped state.
{
  return BloombergLP::bsls::ByteOrderUtil::swapBytes(uc);
...
template <>
inline wchar_t hostToSwapped<wchar_t, 2>(wchar_t uc)
// Return the value of the specified 'uc' with its bytes swapped, where
// 'uc' is assumed to be in host byte order.
{
  return BloombergLP::bsls::ByteOrderUtil::swapBytes(uc);
...
template <>
inline char16_t swappedToHost<char16_t, 2>(char16_t uc)
// Return the value of the specified 'uc' in host byte order, where 'uc' is
// assumed to be in a swapped state.
{
  return BloombergLP::bsls::ByteOrderUtil::swapBytes(uc);
...
template <>
inline char16_t hostToSwapped<char16_t, 2>(char16_t uc)
// Return the value of the specified 'uc' with its bytes swapped, where
// 'uc' is assumed to be in host byte order.
{
  return BloombergLP::bsls::ByteOrderUtil::swapBytes(uc);
...
template <>
inline wchar_t swappedToHost<wchar_t, 4>(wchar_t uc)
// Return the value of the specified 'uc' in host byte order, where 'uc' is
...
template <>
inline wchar_t hostToSwapped<wchar_t, 4>(wchar_t uc)
// Return the value of the specified 'uc' with its bytes swapped, where
...
struct Capacity {
  ...
  // CREATORS
  explicit Capacity(bsl::size_t capacity) : d_capacity(capacity) {}
...
struct Utf8 {
  ...
  public:
    // CREATORS
    explicit PtrBasedEnd(const char *end)
    ...
    // 'd_end'.  The behavior is undefined unless 'octets <= d_end'.
    {
      ...
      while (octets < d_end && (*octets & CONTINUE_MASK) == CONTINUE_TAG) {
        ++octets;
      }
  ...
  static bool isSingleOctet(OctetType oct)
  ...
  static bool isTwoOctetHeader(OctetType oct)
  ...
  static bool isThreeOctetHeader(OctetType oct)
  ...
  static bool isFourOctetHeader(OctetType oct)
  ...
  static UnicodeCodePoint decodeTwoOctets(const OctetType *octBuf)
  ...
  static UnicodeCodePoint decodeThreeOctets(const OctetType *octBuf)
  ...
  static UnicodeCodePoint decodeFourOctets(const OctetType *octBuf)
  ...
  // encode it at the position indicated by the specified 'octBuf'.
  {
    octBuf[2] = static_cast<char>(CONTINUE_TAG | (isoBuf & ~CONTINUE_MASK));
    octBuf[1] = static_cast<char>(
        CONTINUE_TAG | ((isoBuf >> CONTINUE_CONT_WID) & ~CONTINUE_MASK));
    octBuf[0] =
  ...
  // encode it at the position indicated by the specified 'octBuf'.
  {
    octBuf[3] = static_cast<char>(CONTINUE_TAG | (isoBuf & ~CONTINUE_MASK));
    octBuf[2] = static_cast<char>(
        CONTINUE_TAG | ((isoBuf >> CONTINUE_CONT_WID) & ~CONTINUE_MASK));
    octBuf[1] = static_cast<char>(
        CONTINUE_TAG | ((isoBuf >> 2 * CONTINUE_CONT_WID) & ~CONTINUE_MASK));
    octBuf[0] =
        static_cast<char>(FOUR_OCTET_TAG | ((isoBuf >> 3 * CONTINUE_CONT_WID) &
                                            ~FOUR_OCTET_MASK));
  }
};
...
struct Utf16 {
  ...
  enum {
    RESERVED_MASK = 0xF800, // Common part of first/second header
  ...
  template <class UTF16_WORD> class PtrBasedEnd {
  ...
  public:
    // CREATORS
    explicit PtrBasedEnd(const UTF16_WORD *end) : d_end(end) {}
    // Create a 'PtrBasedEnd' object with the end at the specified
    // 'end'.
  ...
  template <class UTF16_WORD> struct ZeroBasedEnd {
    // The 'class' determines whether translation is at the end of input by
  ...
  static bool isSingleUtf8(UTF16_WORD uc)
  // Return 'true' if the specified 'uc' will fit in a single UTF-8 code
  // point, and 'false' otherwise.
  {
    enum { HIGH_BITS = UTF16_WORD(~UTF16_WORD(0) << Utf8::ONE_OCT_CONT_WID) };

    return 0 == (uc & HIGH_BITS);
  }

  template <class UTF16_WORD>
  static bool isSingleWord(UTF16_WORD uc)
  ...
  template <class UTF16_WORD>
  static bool isFirstWord(UTF16_WORD uc)
  ...
  template <class UTF16_WORD>
  static bool isSecondWord(UTF16_WORD uc)
  ...
  template <class UTF16_WORD>
  static OctetType getUtf8Value(UTF16_WORD uc)
  ...
  static bool fitsInOneWord(UnicodeCodePoint uc)
  ...
  static bool isValidOneWord(UnicodeCodePoint uc)
  // Return 'true' if the specified 'uc' will fit in a single word of
  ...
  static bool isValidTwoWords(UnicodeCodePoint uc)
  ...
  static UnicodeCodePoint combineTwoWords(UnicodeCodePoint first,
...
template <class UTF16_WORD> struct Swapper {
  ...
  // CLASS METHODS
  static UnicodeCodePoint decodeSingleWord(const UTF16_WORD *u16Buf)
  ...
  static UTF16_WORD encodeSingleWord(UnicodeCodePoint uc)
  ...
  static void encodeTwoWords(UTF16_WORD *u16Buf, UnicodeCodePoint uc)
...
template <class UTF16_WORD> struct NoOpSwapper {
  ...
  // that requires two words of UTF-16 to encode.
  {
    ...
    u16Buf[0] = static_cast<UTF16_WORD>(Utf16::FIRST_TAG |
                                        (v >> Utf16::CONTENT_CONT_WID));
    u16Buf[1] = static_cast<UTF16_WORD>(
        Utf16::SECOND_TAG |
        (v & ~(~UnicodeCodePoint(0) << Utf16::CONTENT_CONT_WID)));
  }
  ...
  while (!endFunctor.isFinished(octets)) {
    if (Utf8::isSingleOctet(*octets)) {
      ...
      if (endFunctor.verifyContinuations(octets + 1, 3)) {
        ...
        wordsNeeded +=
            Utf8::fitsInThreeOctets(value) || 0x10ffff < value ? 1 : 2;
        octets += 4;
      } else {
        octets = endFunctor.skipContinuations(octets + 1);
        ++wordsNeeded;
...
template <class UTF16_WORD, class CAPACITY_FUNCTOR, class END_FUNCTOR,
          class SWAPPER>
int localUtf8ToUtf16(UTF16_WORD *dstBuffer, CAPACITY_FUNCTOR dstCapacity,
                     const char *srcBuffer, END_FUNCTOR endFunctor,
                     SWAPPER swapper, bsl::size_t *numCodePointsWritten,
                     bsl::size_t *numWordsWritten, UTF16_WORD errorWord)
...
// template arguments every time this function is called.
{
  ...
  *dstBuffer++ = 0;
  ...
  if (sizeof(UTF16_WORD) > sizeof(unsigned short)) {
    BSLS_ASSERT(4 == sizeof(UTF16_WORD));
    const UTF16_WORD forbiddenMask =
        SWAPPER::swap32(static_cast<UTF16_WORD>(0xffff0000));
...
bsl::size_t requiredUtf8BytesImp(const UTF16_WORD *srcBuffer,
                                 END_FUNCTOR endFunctor, SWAPPER swapper)
...
template <class UTF16_WORD, class CAPACITY_FUNCTOR, class END_FUNCTOR,
          class SWAPPER>
int localUtf16ToUtf8(char *dstBuffer, CAPACITY_FUNCTOR dstCapacity,
                     const UTF16_WORD *srcBuffer, END_FUNCTOR endFunctor,
                     SWAPPER swapper, bsl::size_t *numCodePointsWritten,
                     bsl::size_t *numBytesWritten, char errorByte)
...
// explicitly specify the template parameters for this function.
{
  ...
  while (!endFunctor.isFinished(srcBuffer)) {
    ...
    if (earlyFinish || !Utf16::isFirstWord(word0) ||
        !Utf16::isSecondWord(word1)) {
...
void resizeToZeroTerminate(
    CONTAINER *container, bsl::size_t numBytesWritten,
...
void resizeToZeroTerminate(
    ...
    typename bsl::enable_if<
        bsl::is_same<CONTAINER, bsl::vector<char>>::value ||
            ...
            || bsl::is_same<CONTAINER,
                            std::experimental::pmr::vector<char>>::value ||
            bsl::is_same<CONTAINER,
                         std::experimental::pmr::vector<unsigned short>>::value
#endif
        ,
        void *>::type = 0)
// Truncate the specified container in accordance with the specified
...
int localUtf16ToUtf8Container(CONTAINER *container, const UTF16_WORD *srcBuffer,
                              END_FUNCTOR endFunctor, SWAPPER swapper,
                              bsl::size_t *numCodePointsWritten, char errorByte)
...
{
  bsl::size_t estimatedSize =
      requiredUtf8BytesImp(srcBuffer, endFunctor, swapper);
  ...
  int rc = localUtf16ToUtf8(&(*container)[0], NoOpCapacity(), srcBuffer,
                            endFunctor, swapper, numCodePointsWritten,
                            &numBytesWritten, errorByte);
...
template <class CONTAINER>
int utf8ToUtf16Impl(CONTAINER *dstContainer, const bsl::string_view &srcString,
  ...
  bsl::size_t estLength =
      computeRequiredUtf16WordsImp(srcString.data(), endFunctor);
  ...
  int rc =
      BloombergLP::bdlde::ByteOrder::e_HOST == byteOrder
          ? localUtf8ToUtf16(dstBuffer, NoOpCapacity(), srcString.data(),
                             endFunctor, NoOpSwapper<ValueType>(),
                             numCodePointsWritten, &numWordsWritten, errorWord)
          : localUtf8ToUtf16(dstBuffer, NoOpCapacity(), srcString.data(),
                             endFunctor, Swapper<ValueType>(),
                             numCodePointsWritten, &numWordsWritten, errorWord);
  BSLS_ASSERT(0 == (OUT_OF_SPACE_BIT & rc));
...
template <class CONTAINER>
int utf8ToUtf16Impl(CONTAINER *dstContainer, const char *srcString,
                    bsl::size_t *numCodePointsWritten,
                    typename CONTAINER::value_type errorWord,
                    BloombergLP::bdlde::ByteOrder::Enum byteOrder) {
  typedef typename CONTAINER::value_type ValueType;
  ...
  bsl::size_t estLength = computeRequiredUtf16WordsImp(srcString, endFunctor);
  BSLS_ASSERT(estLength > 0);
  ...
  int rc =
      BloombergLP::bdlde::ByteOrder::e_HOST == byteOrder
          ? localUtf8ToUtf16(dstBuffer, NoOpCapacity(), srcString, endFunctor,
                             NoOpSwapper<ValueType>(), numCodePointsWritten,
                             &numWordsWritten, errorWord)
          : localUtf8ToUtf16(dstBuffer, NoOpCapacity(), srcString, endFunctor,
                             Swapper<ValueType>(), numCodePointsWritten,
                             &numWordsWritten, errorWord);
  BSLS_ASSERT(0 == (OUT_OF_SPACE_BIT & rc));
  if (numWordsWritten != estLength) {
    BSLS_ASSERT(numWordsWritten < estLength);
...
template <class CONTAINER>
int utf16ToUtf8Impl(CONTAINER *dstContainer, const unsigned short *srcString,
                    bsl::size_t *numCodePointsWritten, char errorByte,
                    BloombergLP::bdlde::ByteOrder::Enum byteOrder) {
  Utf16::ZeroBasedEnd<unsigned short> endFunctor;

  return BloombergLP::bdlde::ByteOrder::e_HOST == byteOrder
             ? localUtf16ToUtf8Container(dstContainer, srcString, endFunctor,
                                         NoOpSwapper<unsigned short>(),
                                         numCodePointsWritten, errorByte)
             : localUtf16ToUtf8Container(dstContainer, srcString, endFunctor,
...
int utf16ToUtf8Impl(CONTAINER *dstContainer, const unsigned short *srcString,
                    ...
                    bsl::size_t *numCodePointsWritten, char errorByte,
                    BloombergLP::bdlde::ByteOrder::Enum byteOrder) {
  Utf16::PtrBasedEnd<unsigned short> endFunctor(srcString + srcLengthInWords);
...
template <class CONTAINER>
int utf16ToUtf8Impl(CONTAINER *dstContainer, const bsl::wstring_view &srcString,
                    bsl::size_t *numCodePointsWritten, char errorByte,
                    BloombergLP::bdlde::ByteOrder::Enum byteOrder) {
  Utf16::PtrBasedEnd<wchar_t> endFunctor(srcString.data() + srcString.length());

  return BloombergLP::bdlde::ByteOrder::e_HOST == byteOrder
             ? localUtf16ToUtf8Container(dstContainer, srcString.data(),
                                         endFunctor, NoOpSwapper<wchar_t>(),
                                         numCodePointsWritten, errorByte)
             : localUtf16ToUtf8Container(dstContainer, srcString.data(),
                                         endFunctor, Swapper<wchar_t>(),
                                         numCodePointsWritten, errorByte);
}
...
int utf16ToUtf8Impl(CONTAINER *dstContainer,
                    ...
                    bsl::size_t *numCodePointsWritten, char errorByte,
                    BloombergLP::bdlde::ByteOrder::Enum byteOrder) {
  Utf16::PtrBasedEnd<char16_t> endFunctor(srcString.data() +
                                          srcString.length());

  return BloombergLP::bdlde::ByteOrder::e_HOST == byteOrder
             ? localUtf16ToUtf8Container(dstContainer, srcString.data(),
                                         endFunctor, NoOpSwapper<char16_t>(),
                                         numCodePointsWritten, errorByte)
             : localUtf16ToUtf8Container(dstContainer, srcString.data(),
                                         endFunctor, Swapper<char16_t>(),
                                         numCodePointsWritten, errorByte);
}
#endif

template <class CONTAINER>
int utf16ToUtf8Impl(CONTAINER *dstobject, const wchar_t *srcString,
                    bsl::size_t *numCodePointsWritten, char errorByte,
                    BloombergLP::bdlde::ByteOrder::Enum byteOrder) {
  Utf16::ZeroBasedEnd<wchar_t> endFunctor;

  return BloombergLP::bdlde::ByteOrder::e_HOST == byteOrder
             ? localUtf16ToUtf8Container(dstobject, srcString, endFunctor,
                                         NoOpSwapper<wchar_t>(),
                                         numCodePointsWritten, errorByte)
             : localUtf16ToUtf8Container(dstobject, srcString, endFunctor,
                                         Swapper<wchar_t>(),
...
bsl::size_t CharConvertUtf16::computeRequiredUtf16Words(const char *srcBuffer,
                                                        const char *endPtr) {
  return endPtr ? u::computeRequiredUtf16WordsImp(srcBuffer,
...
int CharConvertUtf16::utf8ToUtf16(bsl::wstring *dstWstring,
  ...
  return u::utf8ToUtf16Impl(dstWstring, srcString, numCodePointsWritten,
                            errorWord, byteOrder);
}
...
int CharConvertUtf16::utf8ToUtf16(bsl::u16string *dst16string,
                                  ...
                                  char16_t errorChar,
                                  ByteOrder::Enum byteOrder) {
  return u::utf8ToUtf16Impl(dst16string, srcString, numCodePointsWritten,
                            errorChar, byteOrder);
}
...
int CharConvertUtf16::utf8ToUtf16(
    ...
    const bsl::string_view &srcString, bsl::size_t *numCodePointsWritten,
    char16_t errorChar, ByteOrder::Enum byteOrder) {
  return u::utf8ToUtf16Impl(dst16string, srcString, numCodePointsWritten,
                            errorChar, byteOrder);
}
#endif
...
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR_STRING
int CharConvertUtf16::utf8ToUtf16(
    std::experimental::pmr::u16string *dst16string, const char *srcString,
    bsl::size_t *numCodePointsWritten, char16_t errorChar,
    ByteOrder::Enum byteOrder) {
  return u::utf8ToUtf16Impl(dst16string, srcString, numCodePointsWritten,
                            errorChar, byteOrder);
}
...
int CharConvertUtf16::utf8ToUtf16(bsl::vector<unsigned short> *dstVector,
                                  ...
                                  ByteOrder::Enum byteOrder) {
  return u::utf8ToUtf16Impl(dstVector, srcString, numCodePointsWritten,
                            errorWord, byteOrder);
}

int CharConvertUtf16::utf8ToUtf16(std::vector<unsigned short> *dstVector,
                                  const bsl::string_view &srcString,
                                  bsl::size_t *numCodePointsWritten,
                                  unsigned short errorWord,
                                  ByteOrder::Enum byteOrder) {
...
int CharConvertUtf16::utf8ToUtf16(
    ...
    const bsl::string_view &srcString, bsl::size_t *numCodePointsWritten,
    unsigned short errorWord, ByteOrder::Enum byteOrder) {
  return u::utf8ToUtf16Impl(dstVector, srcString, numCodePointsWritten,
                            errorWord, byteOrder);
}
#endif
...
int CharConvertUtf16::utf8ToUtf16(
    std::experimental::pmr::vector<unsigned short> *dstVector,
    const char *srcString, bsl::size_t *numCodePointsWritten,
    unsigned short errorWord, ByteOrder::Enum byteOrder) {
  return u::utf8ToUtf16Impl(dstVector, srcString, numCodePointsWritten,
                            errorWord, byteOrder);
}
...
int CharConvertUtf16::utf8ToUtf16(unsigned short *dstBuffer,
  ...
  return ByteOrder::e_HOST == byteOrder
             ? u::localUtf8ToUtf16(
                   dstBuffer, u::Capacity(dstCapacity), srcString.data(),
                   endFunctor, u::NoOpSwapper<unsigned short>(),
                   numCodePointsWritten, numWordsWritten, errorWord)
             : u::localUtf8ToUtf16(
                   dstBuffer, u::Capacity(dstCapacity), srcString.data(),
                   endFunctor, u::Swapper<unsigned short>(),
                   numCodePointsWritten, numWordsWritten, errorWord);
}

int CharConvertUtf16::utf8ToUtf16(
    unsigned short *dstBuffer, bsl::size_t dstCapacity, const char *srcString,
    bsl::size_t *numCodePointsWritten, bsl::size_t *numWordsWritten,
    unsigned short errorWord, ByteOrder::Enum byteOrder) {
  u::Utf8::ZeroBasedEnd endFunctor;

  return ByteOrder::e_HOST == byteOrder
             ? u::localUtf8ToUtf16(
                   dstBuffer, u::Capacity(dstCapacity), srcString, endFunctor,
                   u::NoOpSwapper<unsigned short>(), numCodePointsWritten,
                   numWordsWritten, errorWord)
             : u::localUtf8ToUtf16(
                   dstBuffer, u::Capacity(dstCapacity), srcString, endFunctor,
                   u::Swapper<unsigned short>(), numCodePointsWritten,
                   numWordsWritten, errorWord);
}

int CharConvertUtf16::utf8ToUtf16(wchar_t *dstBuffer, bsl::size_t dstCapacity,
                                  const bsl::string_view &srcString,
                                  bsl::size_t *numCodePointsWritten,
                                  bsl::size_t *numWordsWritten,
                                  wchar_t errorWord,
  ...
  return ByteOrder::e_HOST == byteOrder
             ? u::localUtf8ToUtf16(
                   dstBuffer, u::Capacity(dstCapacity), srcString.data(),
                   endFunctor, u::NoOpSwapper<wchar_t>(), numCodePointsWritten,
                   numWordsWritten, errorWord)
             : u::localUtf8ToUtf16(dstBuffer, u::Capacity(dstCapacity),
                                   srcString.data(), endFunctor,
                                   u::Swapper<wchar_t>(), numCodePointsWritten,
                                   numWordsWritten, errorWord);
}
...
int CharConvertUtf16::utf8ToUtf16(wchar_t *dstBuffer, bsl::size_t dstCapacity,
  ...
  return ByteOrder::e_HOST == byteOrder
             ? u::localUtf8ToUtf16(
                   dstBuffer, u::Capacity(dstCapacity), srcString, endFunctor,
                   u::NoOpSwapper<wchar_t>(), numCodePointsWritten,
                   numWordsWritten, errorWord)
             : u::localUtf8ToUtf16(dstBuffer, u::Capacity(dstCapacity),
                                   srcString, endFunctor, u::Swapper<wchar_t>(),
                                   numCodePointsWritten, numWordsWritten,
                                   errorWord);
}

#ifdef BSLS_LIBRARYFEATURES_HAS_CPP11_BASELINE_LIBRARY
int CharConvertUtf16::utf8ToUtf16(char16_t *dstBuffer, bsl::size_t dstCapacity,
                                  const char *srcString,
                                  bsl::size_t *numCodePointsWritten,
                                  bsl::size_t *numWordsWritten,
                                  char16_t errorWord,
  ...
  return ByteOrder::e_HOST == byteOrder
             ? u::localUtf8ToUtf16(
                   dstBuffer, u::Capacity(dstCapacity), srcString, endFunctor,
                   u::NoOpSwapper<char16_t>(), numCodePointsWritten,
                   numWordsWritten, errorWord)
             : u::localUtf8ToUtf16(dstBuffer, u::Capacity(dstCapacity),
                                   srcString, endFunctor,
                                   u::Swapper<char16_t>(), numCodePointsWritten,
                                   numWordsWritten, errorWord);
}
...
int CharConvertUtf16::utf8ToUtf16(char16_t *dstBuffer, bsl::size_t dstCapacity,
  ...
  return ByteOrder::e_HOST == byteOrder
             ? u::localUtf8ToUtf16(
                   dstBuffer, u::Capacity(dstCapacity), srcString.data(),
                   endFunctor, u::NoOpSwapper<char16_t>(), numCodePointsWritten,
                   numWordsWritten, errorWord)
             : u::localUtf8ToUtf16(dstBuffer, u::Capacity(dstCapacity),
                                   srcString.data(), endFunctor,
                                   u::Swapper<char16_t>(), numCodePointsWritten,
...
bsl::size_t
CharConvertUtf16::computeRequiredUtf8Bytes(const wchar_t *srcBuffer,
  ...
  return endPtr ? bdlde::ByteOrder::e_HOST == byteOrder
...
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP11_BASELINE_LIBRARY
bsl::size_t
CharConvertUtf16::computeRequiredUtf8Bytes(const char16_t *srcBuffer,
...
bsl::size_t
CharConvertUtf16::computeRequiredUtf8Bytes(const unsigned short *srcBuffer,
...
int CharConvertUtf16::utf16ToUtf8(bsl::string *dstString,
                                  ...
                                  bsl::size_t *numCodePointsWritten,
                                  char errorByte, ByteOrder::Enum byteOrder) {
  return u::utf16ToUtf8Impl(dstString, srcString, numCodePointsWritten,
                            errorByte, byteOrder);
}
...
int CharConvertUtf16::utf16ToUtf8(bsl::string *dstString,
                                  ...
                                  bsl::size_t *numCodePointsWritten,
                                  char errorByte, ByteOrder::Enum byteOrder) {
  return u::utf16ToUtf8Impl(dstString, srcString, srcLengthInWords,
                            numCodePointsWritten, errorByte, byteOrder);
}
...
int CharConvertUtf16::utf16ToUtf8(bsl::vector<char> *dstVector,
                                  ...
                                  bsl::size_t *numCodePointsWritten,
                                  char errorByte, ByteOrder::Enum byteOrder) {
  return u::utf16ToUtf8Impl(dstVector, srcString, numCodePointsWritten,
                            errorByte, byteOrder);
}
...
int CharConvertUtf16::utf16ToUtf8(
    ...
    const unsigned short *srcString, bsl::size_t *numCodePointsWritten,
    char errorByte, ByteOrder::Enum byteOrder) {
  return u::utf16ToUtf8Impl(dstVector, srcString, numCodePointsWritten,
                            errorByte, byteOrder);
}
...
int CharConvertUtf16::utf16ToUtf8(bsl::vector<char> *dstVector,
                                  ...
                                  bsl::size_t *numCodePointsWritten,
                                  char errorByte, ByteOrder::Enum byteOrder) {
  return u::utf16ToUtf8Impl(dstVector, srcString, srcLengthInWords,
                            numCodePointsWritten, errorByte, byteOrder);
}
...
int CharConvertUtf16::utf16ToUtf8(
    std::experimental::pmr::vector<char> *dstVector,
    const unsigned short *srcString, bsl::size_t srcLengthInWords,
    bsl::size_t *numCodePointsWritten, char errorByte,
    ByteOrder::Enum byteOrder) {
  return u::utf16ToUtf8Impl(dstVector, srcString, srcLengthInWords,
                            numCodePointsWritten, errorByte, byteOrder);
...
int CharConvertUtf16::utf16ToUtf8(
    std::experimental::pmr::vector<char> *dstVector,
    const bsl::wstring_view &srcString, bsl::size_t *numCodePointsWritten,
    char errorByte, ByteOrder::Enum byteOrder) {
  return u::utf16ToUtf8Impl(dstVector, srcString, numCodePointsWritten,
                            errorByte, byteOrder);
}
...
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
int CharConvertUtf16::utf16ToUtf8(
    std::experimental::pmr::vector<char> *dstVector, const wchar_t *srcString,
    bsl::size_t *numCodePointsWritten, char errorByte,
    ByteOrder::Enum byteOrder) {
  return u::utf16ToUtf8Impl(dstVector, srcString, numCodePointsWritten,
                            errorByte, byteOrder);
...
int CharConvertUtf16::utf16ToUtf8(
    std::experimental::pmr::vector<char> *dstVector,
    const bsl::u16string_view &srcString, bsl::size_t *numCodePointsWritten,
    char errorByte, ByteOrder::Enum byteOrder) {
  return u::utf16ToUtf8Impl(dstVector, srcString, numCodePointsWritten,
                            errorByte, byteOrder);
}
...
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
int CharConvertUtf16::utf16ToUtf8(
    std::experimental::pmr::vector<char> *dstVector, const char16_t *srcString,
    bsl::size_t *numCodePointsWritten, char errorByte,
    ByteOrder::Enum byteOrder) {
  return u::utf16ToUtf8Impl(dstVector, srcString, numCodePointsWritten,
                            errorByte, byteOrder);
...
int CharConvertUtf16::utf16ToUtf8(char *dstBuffer, bsl::size_t dstCapacity,
                                  const unsigned short *srcString,
                                  bsl::size_t *numCodePointsWritten,
                                  bsl::size_t *numBytesWritten, char errorByte,
                                  ByteOrder::Enum byteOrder) {
...
int CharConvertUtf16::utf16ToUtf8(char *dstBuffer, bsl::size_t dstCapacity,
                                  ...
                                  bsl::size_t *numBytesWritten, char errorByte,
                                  ByteOrder::Enum byteOrder) {
  u::Utf16::PtrBasedEnd<unsigned short> endFunctor(srcString +
                                                   srcLengthInWords);
...
int CharConvertUtf16::utf16ToUtf8(char *dstBuffer, bsl::size_t dstCapacity,
                                  ...
                                  bsl::size_t *numBytesWritten, char errorByte,
                                  ByteOrder::Enum byteOrder) {
  u::Utf16::PtrBasedEnd<wchar_t> endFunctor(srcString.data() +
                                            srcString.length());
  ...
  return ByteOrder::e_HOST == byteOrder
             ? u::localUtf16ToUtf8(
                   ...
                   endFunctor, u::NoOpSwapper<wchar_t>(), numCodePointsWritten,
                   numBytesWritten, errorByte)
             : u::localUtf16ToUtf8(dstBuffer, u::Capacity(dstCapacity),
                                   srcString.data(), endFunctor,
                                   u::Swapper<wchar_t>(), numCodePointsWritten,
                                   numBytesWritten, errorByte);
}
...
int CharConvertUtf16::utf16ToUtf8(char *dstBuffer, bsl::size_t dstCapacity,
  ...
  return ByteOrder::e_HOST == byteOrder
             ...
             : u::localUtf16ToUtf8(dstBuffer, u::Capacity(dstCapacity),
                                   srcString, endFunctor, u::Swapper<wchar_t>(),
                                   numCodePointsWritten, numBytesWritten,
                                   errorByte);
}
...
int CharConvertUtf16::utf16ToUtf8(char *dstBuffer, bsl::size_t dstCapacity,
                                  ...
                                  bsl::size_t *numBytesWritten, char errorByte,
                                  ByteOrder::Enum byteOrder) {
  u::Utf16::PtrBasedEnd<char16_t> endFunctor(srcString.data() +
                                             srcString.length());
