...
template <class TYPE> TYPE sb(TYPE x) {
...
// human-readable form.
{
  const bsl::vector<int> *p = reinterpret_cast<const bsl::vector<int> *>(&vec);
  return dumpVec(*p);
}
...
bsl::string dumpVec(const std::experimental::pmr::vector<int> &vec)
// Return a 'bsl::string' representation of the contents of 'vec' in
// human-readable form.
{
  bsl::ostringstream oss;
...
bsl::string dumpVec(const std::experimental::pmr::vector<unsigned int> &vec)
// Return a 'bsl::string' representation of the contents of 'vec' in
// human-readable form.
{
  const bsl::vector<int> *p = reinterpret_cast<const bsl::vector<int> *>(&vec);
...
bsl::string dumpVec(const std::experimental::pmr::vector<char> &vec)
// Return a 'bsl::string' representation of the contents of 'vec' in
// human-readable form.
{
  if (vec.empty()) {
...
bsl::ostream &fourByteUtf8String(bsl::ostream &stream, unsigned val) {
  ...
  stream << bsl::hex << '"' << hx << bs[0] << hx << bs[1] << hx << bs[2] << hx
         << bs[3] << '"' << bsl::dec;
...
bsl::ostream &threeByteUtf8String(bsl::ostream &stream, unsigned val) {
  ...
  stream << bsl::hex << '"' << hx << bs[0] << hx << bs[1] << hx << bs[2] << '"'
         << bsl::dec;
...
bsl::string dumpUtf8Vec(const bsl::vector<char> &utf8Vec) {
  ...
  for (unsigned u = 0; u < utf8Vec.size(); ++u) {
    oss << (u > 0 ? ", " : "") << "0x" << (unsigned)(unsigned char)utf8Vec[u];
  }
  oss << ')';
...
template <class VECTOR> bsl::string dumpUtf32Vec(const VECTOR &utf32Vec) {
  bsl::ostringstream oss;
...
static inline int generate15(int *nextSeed, int seed) {
...
template <class TYPE> struct NotEqual {
  ...
  // CREATOR
  explicit NotEqual(TYPE c) : d_word(c) {}
...
enum {
  NUM_BETTER_UTF8_TABLE = sizeof betterUtf8Table / sizeof *betterUtf8Table,
...
struct TestDriver {
  ...
  template <class VECTOR_C, class VECTOR_UI> static void testCase13();
  // Test 'utf-32 -> utf8' translation with zero error characters using
  // randomly generated sequences.

  template <class VECTOR_C, class VECTOR_UI> static void testCase12();
  ...
  template <class VECTOR> static void testCase9();
  // Test 'utf-32 -> utf8' translation using explicit 'errorByte'.

  template <class VECTOR, class STRING> static void testCase8();
  // Test 'utf-32 -> utf8' translation with zero error characters.

  template <class VECTOR, class STRING> static void testCase7();
  // Test 'utf-32 -> utf8' translation.

  template <class VECTOR> static void testCase6();
  // Test 'utf-32 -> utf8' translation with  error characters other than
  // 'default' or 0.

  template <class VECTOR> static void testCase5();
  ...
  for (int ti = 0; ti < 8 * 1000; ++ti) {
    ...
    for (int tj = 0; true; ++tj) {
      ...
      if (table.d_error) {
        ...
        utf32ExpVec.push_back(opposite ? sb(errorChar32) : errorChar32);
        ++utf8ExpNumBytesWritten;
    ...
    int rc = Util::utf8ToUtf32(&utf32OutVec, utf8InStr, errorChar32, endian);
    ASSERT(expRc == rc);
    LOOP5_ASSERT(dumpVec(idxVec), utf32ExpVec.size(), utf32OutVec.size(),
                 dumpVec(utf32ExpVec), dumpVec(utf32OutVec),
                 utf32ExpVec == utf32OutVec);
    ASSERT(!utf32OutVec.empty());

    bsl::size_t numCodePointsWritten = myRand15();
    rc = Util::utf32ToUtf8(&utf8OutStr, &utf32OutVec[0], utf32OutVec.size() - 1,
                           &numCodePointsWritten, errorChar, endian);
    ASSERT(0 == rc);
    LOOP2_ASSERT(utf32OutVec.size(), numCodePointsWritten,
                 utf32OutVec.size() == numCodePointsWritten);
    LOOP5_ASSERT(utf8OutStr, utf8InStr, expRc, utf8OutStr.length(),
                 utf8ExpNumBytesWritten,
    ...
    utf8ExpVec.clear();
    utf8ExpVec.insert(utf8ExpVec.begin(), utf8ExpStr.begin(),
                      utf8ExpStr.begin() + utf8ExpStr.length());
    utf8ExpVec.push_back(0);

    rc = Util::utf32ToUtf8(&utf8OutVec, &utf32OutVec[0], utf32OutVec.size() - 1,
                           &numCodePointsWritten, errorChar, endian);
    ASSERT(0 == rc);
    LOOP2_ASSERT(utf32OutVec.size(), numCodePointsWritten,
                 utf32OutVec.size() == numCodePointsWritten);
    ...
    for (bsl::size_t ii = utf8OutVec.size() + 1; 0 < ii--;) {
      ...
      rc = Util::utf32ToUtf8(&utf8OutVec[0], ii, &utf32OutVec[0],
                             utf32OutVec.size() - 1, &numCodePointsWritten,
                             &numBytesWritten, errorChar, endian);
      ASSERT(rc == (ii == utf8OutVec.size() ? 0 : Status::k_OUT_OF_SPACE_BIT));
      ...
      if (numCodePointsWritten) {
        ...
        ASSERT(0 == bsl::memcmp(&utf8OutVec[0], &utf8ExpVec[0],
                                numBytesWritten - 1));
      }
  ...
  for (int ti = 0; ti < 4 * 1000; ++ti) {
    ...
    *pc++ = 0;
    ...
    for (int zeroErrorWord = 0; zeroErrorWord < 2; ++zeroErrorWord) {
      ...
      int ret = Util::utf8ToUtf32(&utf32OutVec, utf8InBuf, errorWord, endian);
      ...
      bsl::size_t ncw = -1;
      ret = Util::utf32ToUtf8(&utf8OutVec, &utf32OutVec[0], &ncw, '?', endian);
      ...
      for (bsl::size_t len = utf32OutVec.size() + 1; len >= 2; --len) {
        ...
        bsl::fill(utf32OutBuf + 0, utf32OutBuf + UTF32_OUT_BUF_LEN, fillWord);

        ncw = -1;
        ret = Util::utf8ToUtf32(utf32OutBuf, len, utf8InBuf, &ncw, errorWord,
                                endian);
        LOOP6_ASSERT(ti, len, utf32OutVec.size(), ret, dumpUtf8Vec(utf8InVec),
        ...
        ASSERT(len > utf32OutVec.size() ? utf32OutVec.size() == ncw
                                        : ncw == len);
        ASSERT(bsl::find_if(utf32OutBuf + ncw, utf32OutBuf + UTF32_OUT_BUF_LEN,
                            NotEqual<unsigned int>(fillWord)) ==
               utf32OutBuf + UTF32_OUT_BUF_LEN);
        ASSERT(bsl::find(utf32OutBuf + 0, utf32OutBuf + ncw, 0) ==
               utf32OutBuf + ncw - 1);
        ASSERT(0 == bsl::memcmp(utf32OutBuf, &utf32OutVec[0],
                                (ncw - 1) * sizeof(int)));
        unsigned int *pos = bsl::find(utf32OutBuf + 0, utf32OutBuf + ncw - 1,
                                      opposite ? sb(errorWord) : errorWord);
        LOOP8_ASSERT(ncw, pos - utf32OutBuf, hx(errorWord), hx(sb(errorWord)),
                     (ret & Status::k_INVALID_INPUT_BIT), len,
                     dumpUtf32Vec(utf32OutVec), opposite,
                     0 == (ret & Status::k_INVALID_INPUT_BIT) ||
                         0 == errorWord || pos < utf32OutBuf + ncw - 1);
      }
    }
  }
}
  ...
  for (int ti = 0; ti < 5 * 1000; ++ti) {
    ...
    for (int zeroErrorWord = 0; zeroErrorWord < 2; ++zeroErrorWord) {
        ...
        for (; repeat;) {
          ...
          if (NUM_UTF8_TABLE == idx) {
            ...
            if (!prevTrunc) {
              ...
              if (errorWord) {
                utf8ExpVec.insert(utf8ExpVec.end(), utf8ErrorSeq.begin(),
                                  utf8ErrorSeq.end());
                utf32ExpVec.push_back(opposite ? sb(errorWord) : errorWord);
              }
            }
          } else {
            ...
            if (utf8Table[idx].d_error) {
              ...
              utf8ExpVec.insert(utf8ExpVec.end(), utf8Seq, utf8Seq + seqlen);
              utf32ExpVec.push_back(opposite ? sb(utf8Table[idx].d_midVal)
      ...
      *pc++ = 0;
      ...
      utf32ExpVec.push_back(0);
      ASSERT(pc <= utf8InBuf + MAX_UTF8_BYTES_INPUT);
      LOOP4_ASSERT(ti, utf8ExpVec.size(), pc - utf8InBuf, errorWord,
                   errorWord >= 0x80 ||
                       (int)utf8ExpVec.size() <= pc - utf8InBuf);
      bsl::vector<char> utf8InVec(utf8InBuf,
                                  utf8InBuf + bsl::strlen(utf8InBuf) + 1);
      ...
      LOOP3_ASSERT(ti, dumpUtf32Vec(utf32ExpVec), dumpUtf32Vec(utf32OutVec),
                   utf32ExpVec == utf32OutVec);
      ASSERT(bsl::find(utf32OutVec.begin(), utf32OutVec.end(), 0) ==
             utf32OutVec.end() - 1);
      ...
      for (bsl::size_t len = utf32ExpVec.size() + 1; len >= 2; --len) {
        ...
        ret = Util::utf8ToUtf32(utf32OutBuf, len, utf8InBuf, &ncw, errorWord,
                                endian);
        LOOP6_ASSERT(ti, len, utf32ExpVec.size(), ret, dumpUtf8Vec(utf8InVec),
                     dumpUtf32Vec(utf32ExpVec),
                     0 == (ret & Status::k_OUT_OF_SPACE_BIT) ||
                         (len < utf32ExpVec.size() + 1));
        ASSERT(errorsPresent || 0 == (ret & Status::k_INVALID_INPUT_BIT));
        ...
        ASSERT(bsl::find(utf32OutBuf + 0, utf32OutBuf + ncw, 0) ==
               utf32OutBuf + ncw - 1);
        ASSERT(0 == bsl::memcmp(utf32OutBuf, &utf32ExpVec[0],
                                (ncw - 1) * sizeof(int)));
      }
    }
  }
...
void TestDriver::testCase14() {
  ...
  for (int ti = 0; ti < 3 * 1000; ++ti) {
    ...
    for (int zeroErrorByte = 0; zeroErrorByte < 2; ++zeroErrorByte) {
      ...
      bool errorsPresent = false;
      unsigned int uc;
      for (const unsigned int *pui = &origUtf32InVec[0]; (uc = *pui); ++pui) {
        const bool IS_ERROR = (uc >= 0xd800 && uc <= 0xdfff) || uc > 0x10ffff;
        ...
        if (!IS_ERROR) {
          ...
          utf32ExpVec.push_back(opposite ? sb((unsigned int)errorByte)
                                         : errorByte);
        }
      ...
      utf8ExpVec.push_back(0);
      utf32ExpVec.push_back(0);
      LOOP3_ASSERT(zeroErrorByte, utf32ExpVec.size(), numCodePointsIn,
      ...
      int ret = Util::utf32ToUtf8(&utf8OutVec, &utf32InVec[0], &ncw, errorByte,
                                  endian);
      ASSERT(ret == (errorsPresent ? Status::k_INVALID_INPUT_BIT : 0));
      ...
      ncw = -1;
      ret = Util::utf32ToUtf8(&utf8OutStr, &utf32InVec[0], &ncw, errorByte,
                              endian);
      ASSERT(ret == (errorsPresent ? Status::k_INVALID_INPUT_BIT : 0));
      ASSERT(ncw == utf32ExpVec.size());
      ...
      ret = Util::utf8ToUtf32(&utf32OutVec, &utf8ExpVec[0],
                              zeroErrorByte ? 0 : '*', endian);
      ASSERT(0 == ret);
      ASSERT(utf32OutVec.size() == utf32ExpVec.size());
      ASSERT(utf32OutVec == utf32ExpVec);

      for (bsl::size_t len = utf8ExpVec.size() + 1; len >= 1; --len) {
        bsl::fill(&utf8OutBuf[0], &utf8OutBuf[sizeof(utf8OutBuf)], fillByte);
        ...
        ncw = -1;
        bsl::size_t nbw = -1;
        ret = Util::utf32ToUtf8(utf8OutBuf, len, &utf32InVec[0], &ncw, &nbw,
                                errorByte, endian);
        if (!errorsPresent) {
          ASSERT(0 == (ret & Status::k_INVALID_INPUT_BIT));
        }
        ...
        ASSERT(utf8OutBuf + sizeof(utf8OutBuf) ==
               bsl::find_if(utf8OutBuf + nbw, utf8OutBuf + sizeof(utf8OutBuf),
...
template <class VECTOR_C, class VECTOR_UI> void TestDriver::testCase13() {
  ...
  for (int ti = 0; ti < 10 * 1000; ++ti) {
    ...
    for (int tj = 1; tj < numCodePointsIn; ++tj) {
      ...
      if (!IS_ERROR) {
        utf8ExpVec.insert(utf8ExpVec.end(), utf8Str,
    ...
    VECTOR_C utf8OutVec;
    int ret =
        Util::utf32ToUtf8(&utf8OutVec, &utf32InVec.front(), &ncw, 0, endian);
    ASSERT(ret == (errorsPresent ? Status::k_INVALID_INPUT_BIT : 0));
    ...
    for (IntPtr len = utf8OutVec.size() + 1; len >= 0; --len) {
      ...
      ncw = -1;
      bsl::size_t nbw = -1;
      ret = Util::utf32ToUtf8(&bufferVec.front(), (bsl::size_t)len,
                              &utf32InVec.front(), &ncw, &nbw, 0, endian);
      ...
      ASSERT(!nbw || 0 == bsl::strncmp(&utf8OutVec[0], &bufferVec[0], nbw - 1));
...
template <class VECTOR_C, class VECTOR_UI> void TestDriver::testCase12() {
  VECTOR_UI utf32InVec;
  VECTOR_UI utf32ExpVec;
  VECTOR_C utf8ExpVec;
  VECTOR_C bufferVec;
  ...
  for (int ti = 0; ti < 5 * 1000; ++ti) {
    ...
    int ret =
        Util::utf32ToUtf8(&utf8OutVec, &utf32InVec.front(), &ncw, '?', endian);
    ASSERT(ret == (errorsPresent ? Status::k_INVALID_INPUT_BIT : 0));
    ...
    ret = Util::utf8ToUtf32(&utf32OutVec, &utf8OutVec.front(), '?', endian);
    ASSERT(0 == ret);
    ...
    for (IntPtr len = utf8OutVec.size() + 1; len >= 2; --len) {
      ...
      bsl::size_t nbw = -1;
      ret = Util::utf32ToUtf8(&bufferVec.front(), (bsl::size_t)len,
                              &utf32InVec.front(), &ncw, &nbw, '?', endian);
      LOOP5_ASSERT(ti, len, utf8OutVec.size(), expectedRet, ret,
                   expectedRet == (ret & Status::k_OUT_OF_SPACE_BIT));
      ASSERT((int)ncw <= numCodePoints);
...
void TestDriver::testCase11() {
  ...
  bsl::size_t ncw = -1;
  ret = Util::utf8ToUtf32(&utf32VecBuffer.front(), utf32Vec.size(),
                          charUtf8MultiLang, &ncw);
  ASSERT(0 == ret);
  ASSERT(utf32Vec.size() == ncw);
  ASSERT(!bsl::memcmp(&utf32VecBuffer.front(), &utf32Vec.front(),
                      sizeof(unsigned int) * ncw));
  ASSERT(utf32VecBuffer.end() ==
         bsl::find_if(utf32VecBuffer.begin() + ncw, utf32VecBuffer.end(),
                      NotEqual<unsigned int>(fillWord)));

  VECTOR_UI utf32VecOpposite;
  ret = Util::utf8ToUtf32(&utf32VecOpposite, charUtf8MultiLang, '?',
                          oppositeEndian);
  ASSERT(0 == ret);
  ...
  ncw = -1;
  ret = Util::utf8ToUtf32(&utf32VecOppositeBuffer.front(), utf32Vec.size(),
                          charUtf8MultiLang, &ncw, '?', oppositeEndian);
  ASSERT(0 == ret);
  ASSERT(utf32Vec.size() == ncw);
  ASSERT(!bsl::memcmp(&utf32VecOppositeBuffer.front(),
                      &utf32VecOpposite.front(), sizeof(unsigned int) * ncw));
  ASSERT(utf32VecBuffer.end() ==
         bsl::find_if(utf32VecBuffer.begin() + ncw, utf32VecBuffer.end(),
                      NotEqual<unsigned int>(fillWord)));
  ...
  ncw = -1;
  STRING utf8StrB;
  ret = Util::utf32ToUtf8(&utf8StrB, &utf32VecOpposite.front(), &ncw, '?',
                          oppositeEndian);
  ASSERT(0 == ret);
  ASSERT(utf32Vec.size() == ncw);
  ...
  utf8Vec.clear();
  ret = Util::utf32ToUtf8(&utf8Vec, &utf32VecOpposite.front(), &ncw, '?',
                          oppositeEndian);
  ASSERT(0 == ret);
  ASSERT(utf32Vec.size() == ncw);
  ...
  ncw = -1;
  bsl::size_t nbw = -1;
  ret = Util::utf32ToUtf8(&utf8VecBuffer[0], utf8VecBuffer.size(),
                          &utf32Vec.front(), &ncw, &nbw);
  ASSERT(0 == ret);
  ASSERT(utf32Vec.size() == ncw);
  ...
  nbw = -1;
  ret = Util::utf32ToUtf8(&utf8VecBuffer[0], utf8VecBuffer.size(),
                          &utf32VecOpposite.front(), &ncw, &nbw, '?',
                          oppositeEndian);
  ASSERT(0 == ret);
  ASSERT(utf32Vec.size() == ncw);
  ASSERT(origLen + 1 == nbw);
...
void TestDriver::testCase10() {
  ...
  for (int i = 0x80; i < 0x100; ++i) {
    ...
    ASSERT_FAIL(Util::utf32ToUtf8(&utf8DestVec, utf32Input, &ncw, ERROR_CHAR));
    ASSERT(minus1 == ncw);
    ...
    ASSERT_FAIL(Util::utf32ToUtf8(&utf8DestVec.front(), utf8DestVec.size(),
                                  utf32Input, &ncw, &nbw, ERROR_CHAR));
    ASSERT(minus1 == ncw);
    ASSERT(minus1 == nbw);
    ...
    cout << "UTF-8 <- UTF-32: null pointers";
  {
    ...
    ASSERT_FAIL(Util::utf32ToUtf8(&utf8DestVec.front(), utf8DestVec.size(), 0));
    ASSERT_FAIL(Util::utf32ToUtf8((char *)0, utf8DestVec.size(), utf32Input));
  }
  ...
  for (unsigned int errorWord = 0xd800; errorWord < 0xffffff;
       errorWord = errorWord < 0xdfff    ? errorWord + 1
                   : 0xdfff == errorWord ? 0X10ffff + 1
    ...
    bsl::size_t ncw = minus1;
    ASSERT_FAIL(Util::utf8ToUtf32(&utf32DestVec.front(), utf32DestVec.size(),
                                  utf8Input, &ncw, errorWord));
    ASSERT(minus1 == ncw);
    ASSERT(fillWord == utf32DestVec.front());

    ASSERT_FAIL(Util::utf8ToUtf32(&utf32DestVec.front(), utf32DestVec.size(),
                                  utf8Sr, &ncw, errorWord));
    ASSERT(minus1 == ncw);
    ASSERT(fillWord == utf32DestVec.front());
  }
...
template <class VECTOR> void TestDriver::testCase9() {
  if (verbose)
  ...
  for (int tm = 0; tm < 2 * NUM_UTF32_TABLE; ++tm) {
    ...
    for (int errChr = 1; errChr < 0x80; ++errChr) {
      ...
      int ret = opposite ? Util::utf32ToUtf8(&outVec, utf32Seq, &numCodePoints,
                                             (char)errChr, oppositeEndian)
...
template <class VECTOR, class STRING> void TestDriver::testCase8() {
  if (verbose)
  ...
  for (int tm = 0; tm < 2 * NUM_UTF32_TABLE; ++tm) {
    ...
    ret = opposite ? Util::utf32ToUtf8(&outStr, utf32Seq, &numCodePoints, 0,
                                       oppositeEndian)
    ...
    for (bsl::size_t len = compareVec.size() + 2; (bsl::size_t)-1 != len;
      ...
      expectedRet = IS_ERROR && len >= 2 ? Status::k_INVALID_INPUT_BIT : 0;
      expectedRet |= len < compareVec.size() ? Status::k_OUT_OF_SPACE_BIT : 0;
      ...
      const bsl::size_t expectedNumBytes = enc <= 2 ? enc
                                           : enc == expectedNumCodePoints - 1
                                               ? cvs - 1
                                               : cvs;

      bsl::size_t numBytes = -1;
      ret = opposite ? Util::utf32ToUtf8(outBuf, len, utf32Seq, &numCodePoints,
                                         &numBytes, 0, oppositeEndian)
                     : Util::utf32ToUtf8(outBuf, len, utf32Seq, &numCodePoints,
                                         &numBytes, 0);
      ...
      LOOP2_ASSERT(LINE, len,
                   0 == len || 0 == bsl::memcmp(outBuf, &compareStr[0],
                                                expectedNumBytes - 1));
      LOOP2_ASSERT(LINE, len, 0 == len || 0 == outBuf[expectedNumBytes - 1]);
    }
  }
  ...
  for (int tm = 0; tm < 2 * NUM_UTF32_TABLE; ++tm) {
    ...
    for (bsl::size_t len = compareVec.size() + 2; (bsl::size_t)-1 != len;
      ...
      expectedRet = IS_ERROR && len >= 1 ? Status::k_INVALID_INPUT_BIT : 0;
      expectedRet |= len < compareVec.size() ? Status::k_OUT_OF_SPACE_BIT : 0;
      ...
      const bsl::size_t expectedNumBytes = enc == expectedNumCodePoints ? cvs
                                           : enc == expectedNumCodePoints - 1
                                               ? cvs - 1
                                               : !!enc;
...
template <class VECTOR, class STRING> void TestDriver::testCase7() {
  if (verbose)
    Q(Testing Full Sequence);
  for (int tm = 0; tm < 8 * NUM_UTF32_TABLE; ++tm) {
    ...
    for (bsl::size_t len = compareVec.size() + 2; (bsl::size_t)-1 != len;
      ...
      ret = checkNumCPs
                ? checkNBytes
                      ? opposite ? Util::utf32ToUtf8(outBuf, len, utf32Seq,
                                                     &numCodePoints, &numBytes,
                                                     '?', oppositeEndian)
  ...
  for (int tm = 0; tm < 8 * NUM_UTF32_TABLE; ++tm) {
    ...
    ret = checkNumCPs
              ? opposite ? Util::utf32ToUtf8(&outVec, utf32Seq, &numCodePoints,
                                             '?', oppositeEndian)
                         : Util::utf32ToUtf8(&outVec, utf32Seq, &numCodePoints)
              : Util::utf32ToUtf8(&outVec, utf32Seq);
    ...
    ret = checkNumCPs
              ? opposite ? Util::utf32ToUtf8(&outStr, utf32Seq, &numCodePoints,
                                             '?', oppositeEndian)
                         : Util::utf32ToUtf8(&outStr, utf32Seq, &numCodePoints)
              : Util::utf32ToUtf8(&outStr, utf32Seq);
...
template <class VECTOR> void TestDriver::testCase6() {
  static unsigned int errorWords[] = {
      1,    10,        20,     'a',    'A',     256,     500,
  ...
  for (int tm = 0; tm < 4 * NUM_UTF8_TABLE; ++tm) {
    ...
    for (int te = 0; te < NUM_ERROR_WORDS; ++te) {
      ...
      int ret =
          useStringView
              ? opposite
                    ? Util::utf8ToUtf32(&outVec, sr, ERROR_WORD, oppositeEndian)
                    : Util::utf8ToUtf32(&outVec, sr, ERROR_WORD)
          : opposite ? Util::utf8ToUtf32(&outVec, UTF8_STRING, ERROR_WORD,
                                         oppositeEndian)
                     : Util::utf8ToUtf32(&outVec, UTF8_STRING, ERROR_WORD);
      ...
      unsigned int swappedErrorWord = opposite ? sb(ERROR_WORD) : ERROR_WORD;
...
template <class VECTOR> void TestDriver::testCase5() {
  if (verbose)
    Q(4 - char sequences);
  ...
  for (int tm = 0; tm < 4 * NUM_UTF8_TABLE; ++tm) {
    ...
    if (IS_ERROR) {
      bsl::memmove(&expectedOut[1], &expectedOut[2], 2 * sizeof(unsigned int));
    }
    if (opposite) {
      for (unsigned u = 0; u < sizeof(expectedOut) / sizeof(int); ++u) {
        expectedOut[u] = sb(expectedOut[u]);
    ...
    int ret = useStringView
                  ? opposite ? Util::utf8ToUtf32(&outVec, sr, 0, oppositeEndian)
                             : Util::utf8ToUtf32(&outVec, sr, 0)
              : opposite
                  ? Util::utf8ToUtf32(&outVec, UTF8_STRING, 0, oppositeEndian)
  ...
  for (int tm = 0; tm < 4 * NUM_UTF8_TABLE; ++tm) {
    ...
    int ret = useStringView
                  ? opposite ? Util::utf8ToUtf32(&outVec, sr, 0, oppositeEndian)
                             : Util::utf8ToUtf32(&outVec, sr, 0)
              : opposite
                  ? Util::utf8ToUtf32(&outVec, s.c_str(), 0, oppositeEndian)
    ...
    LOOP4_ASSERT(useStringView, opposite, LINE, IS_ERROR,
...
template <class VECTOR> void TestDriver::testCase3() {
  for (int tm = 0; tm < 4 * NUM_UTF8_TABLE; ++tm) {
    ...
    int ret = useStringView
              ...
              : opposite
                  ? Util::utf8ToUtf32(&outVec, UTF8_STRING, '?', oppositeEndian)
                  : Util::utf8ToUtf32(&outVec, UTF8_STRING);
    ASSERT(expectedRet == ret);
    LOOP4_ASSERT(useStringView, LINE, expectedCodePoints, outVec.size(),
  ...
  for (int tm = 0; tm < 4 * NUM_UTF8_TABLE; ++tm) {
    ...
    int ret =
        useStringView
            ? opposite ? Util::utf8ToUtf32(&outVec, sr, '?', oppositeEndian)
                       : Util::utf8ToUtf32(&outVec, sr)
        : opposite ? Util::utf8ToUtf32(&outVec, s.c_str(), '?', oppositeEndian)
  ...
  for (int tm = 0; tm < 4 * NUM_UTF8_TABLE; ++tm) {
    ...
    case 1: {
      expectedRet =
          (UTF8_STRING[1] & 0xf8) == 0xf8 ? Status::k_INVALID_INPUT_BIT : 0;
...
int main(int argc, char **argv) {
  ...
  case 18: {
    ...
    // 'bdlde::CharConvertUtf32' struct's utility functions, first converting
    // from UTF-8 to UTF-32, and then converting back to make sure the round
    // trip returns the same value.
    //
    ...
    // variable into which the number of code points written will be stored.  It
    // is not necessary to create a 'utf32CodePointsWritten' variable, since the
    // number of code points will be the size of the vector when we are done.
    //..
    bsl::vector<unsigned int> v32;
    //..
    // Note that it is a waste of time to 'v32.reserve(sizeof(utf8MultiLang))';
    // it is entirely redundant -- 'v32' will automatically be grown to the
    // correct size.  Also note that if 'v32' were not empty, that would not be
    // a problem
    // -- any contents will be discarded.
    //
    // Then, we do the translation to 'UTF-32':
    //..
    int retVal = bdlde::CharConvertUtf32::utf8ToUtf32(&v32, utf8MultiLang);
    //
    ASSERT(0 == retVal);     // verify success
    ASSERT(0 == v32.back()); // verify null terminated
                             //..
    // Next, we verify that the number of code points that was returned is
    // correct. Note that in UTF-32, the number of Unicode code points written
    // is the same as the number of 32-bit words written:
    //..
    enum {
    ...
    // UTF-32 words output and the number of bytes input.  The ASCII bytes will
    // take 1 32-bit word apiece, the Greek code points are double octets that
    // will become single 'unsigned int' values, the Chinese code points are
    // encoded as UTF-8 triple octets that will turn into single 32-bit words,
    // the same for the Hindi code points, and the quad code points are
    // quadruple octets that will turn into single 'unsigned int' words:
    //..
    enum {
      SHRINKAGE =
    ...
    // Declare a 'bsl::string' for our output, and a variable to count the
    // number of code points translated:
    //..
    bsl::string s;
    bsl::size_t codePointsWritten;
    //..
    // Again, note that it would be a waste of time for the caller to 'resize'
    // or 'reserve' 'v32'; it will be automatically 'resize'd by the translator
    // to the right length.
    //
    // Now, we do the reverse transform:
    //..
    retVal = bdlde::CharConvertUtf32::utf32ToUtf8(&s, v32.begin(),
                                                  &codePointsWritten);
    //..
  ...
  case 17: {
    ...
    TestDriver::testCase17<bsl::vector<char>, bsl::vector<unsigned int>,
                           bsl::string>();

    if (verbose)
      cout << "\tTesting std containers\n";
    TestDriver::testCase17<std::vector<char>, std::vector<unsigned int>,
                           std::string>();
    ...
    TestDriver::testCase17<std::experimental::pmr::vector<char>,
                           std::experimental::pmr::vector<unsigned int>,
  ...
  case 16: {
    ...
    if (verbose)
      cout << "\tTesting bsl containers\n";
    TestDriver::testCase16<bsl::vector<char>, bsl::vector<unsigned int>,
                           bsl::string>();

    if (verbose)
      cout << "\tTesting std containers\n";
    TestDriver::testCase16<std::vector<char>, std::vector<unsigned int>,
                           std::string>();
    ...
    TestDriver::testCase16<std::experimental::pmr::vector<char>,
                           std::experimental::pmr::vector<unsigned int>,
                           std::experimental::pmr::string>();
#endif
  ...
  case 15: {
    ...
    if (verbose)
      cout << "\tTesting bsl containers\n";
    TestDriver::testCase15<bsl::vector<char>, bsl::vector<unsigned int>,
                           bsl::string>();

    if (verbose)
      cout << "\tTesting std containers\n";
    TestDriver::testCase15<std::vector<char>, std::vector<unsigned int>,
                           std::string>();
    ...
    TestDriver::testCase15<std::experimental::pmr::vector<char>,
                           std::experimental::pmr::vector<unsigned int>,
                           std::experimental::pmr::string>();
#endif
  ...
  case 14: {
    ...
    if (verbose)
      cout << "\tTesting bsl containers\n";
    TestDriver::testCase14<bsl::vector<char>, bsl::vector<unsigned int>,
                           bsl::string>();

    if (verbose)
      cout << "\tTesting std containers\n";
    TestDriver::testCase14<std::vector<char>, std::vector<unsigned int>,
                           std::string>();
    ...
    TestDriver::testCase14<std::experimental::pmr::vector<char>,
                           std::experimental::pmr::vector<unsigned int>,
                           std::experimental::pmr::string>();
#endif
  ...
  case 13: {
    ...
    if (verbose)
      cout << "\tTesting bsl containers\n";
    TestDriver::testCase13<bsl::vector<char>, bsl::vector<unsigned int>>();

    if (verbose)
      cout << "\tTesting std containers\n";
    TestDriver::testCase13<std::vector<char>, std::vector<unsigned int>>();

#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
    if (verbose)
      cout << "\tTesting pmr containers\n";

    TestDriver::testCase13<std::experimental::pmr::vector<char>,
                           std::experimental::pmr::vector<unsigned int>>();
#endif
  ...
  case 12: {
    ...
    if (verbose)
      cout << "\tTesting bsl containers\n";
    TestDriver::testCase12<bsl::vector<char>, bsl::vector<unsigned int>>();

    if (verbose)
      cout << "\tTesting std containers\n";
    TestDriver::testCase12<std::vector<char>, std::vector<unsigned int>>();

#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
    if (verbose)
      cout << "\tTesting pmr containers\n";

    TestDriver::testCase12<std::experimental::pmr::vector<char>,
                           std::experimental::pmr::vector<unsigned int>>();
#endif
  ...
  case 11: {
    ...
    if (verbose)
      cout << "\tTesting bsl containers\n";
    TestDriver::testCase11<bsl::vector<char>, bsl::vector<unsigned int>,
                           bsl::string>();

    if (verbose)
      cout << "\tTesting std containers\n";
    TestDriver::testCase11<std::vector<char>, std::vector<unsigned int>,
                           std::string>();
  ...
  case 10: {
    ...
    if (verbose)
      cout << "\tTesting bsl containers\n";
    TestDriver::testCase10<bsl::vector<char>, bsl::vector<unsigned int>,
                           bsl::string>();

    if (verbose)
      cout << "\tTesting std containers\n";
    TestDriver::testCase10<std::vector<char>, std::vector<unsigned int>,
                           std::string>();
    ...
    TestDriver::testCase10<std::experimental::pmr::vector<char>,
                           std::experimental::pmr::vector<unsigned int>,
                           std::experimental::pmr::string>();
#endif
  } break;
  case 9: {
    ...
    if (verbose)
      cout << "TESTING UTF-32 -> UTF-8 WITH EXPLICIT 'errorByte'\n"
              "======================================================\n";
    if (verbose)
      cout << "\tTesting bsl containers\n";
    ...
    TestDriver::testCase9<std::experimental::pmr::vector<char>>();
#endif
  } break;
  case 8: {
    // --------------------------------------------------------------------
    ...
    if (verbose)
      cout << "Testing UTF-32 -> UTF-8 Translation\n"
              "===================================\n";
    ...
    TestDriver::testCase8<std::experimental::pmr::vector<char>,
                          std::experimental::pmr::string>();
#endif
  } break;
  case 7: {
    ...
    TestDriver::testCase7<std::experimental::pmr::vector<char>,
                          std::experimental::pmr::string>();
#endif
  ...
  case 6: {
    ...
    if (verbose)
      cout << "Testing UTF-8 -> UTF-32 OTHER VALUES OF ERRORCHAR\n"
              "=================================================\n";
    ...
    TestDriver::testCase6<std::experimental::pmr::vector<unsigned int>>();
#endif
  ...
  case 5: {
    ...
    if (verbose)
      cout << "UTF-8 -> UTF-32 Buffer Transl'n With 0 for errChr, to Vector\n"
              "============================================================\n";
    ...
    TestDriver::testCase5<std::experimental::pmr::vector<unsigned int>>();
#endif
  } break;
  case 4: {
    // --------------------------------------------------------------------
    ...
    if (verbose)
      cout << "Testing UTF-8 -> UTF-32 Buffer Translation (0 for errChr)\n"
              "=========================================================\n";
    ...
    for (int tm = 0; tm < 4 * NUM_UTF8_TABLE; ++tm) {
      ...
      for (int len = MAX_LEN; len >= 0; --len) {
        ...
        int expectedRet =
            IS_ERROR && len >= 3 ? Status::k_INVALID_INPUT_BIT : 0;
        expectedRet |=
            len < (IS_ERROR ? 3 : 4) ? Status::k_OUT_OF_SPACE_BIT : 0;
        const unsigned expectedCodePoints = bsl::min(len, 3 + !IS_ERROR);

        bsl::size_t numCodePoints = -1;
        const bsl::string_view sr(UTF8_STRING);
        int ret = useStringView
                      ? opposite ? Util::utf8ToUtf32(out32Buf, len, UTF8_STRING,
                                                     &numCodePoints, 0,
                                                     oppositeEndian)
                                 : Util::utf8ToUtf32(out32Buf, len, UTF8_STRING,
                                                     &numCodePoints, 0)
                  : opposite
                      ? Util::utf8ToUtf32(out32Buf, len, sr, &numCodePoints, 0,
                                          oppositeEndian)
                      : Util::utf8ToUtf32(out32Buf, len, sr, &numCodePoints, 0);
        ...
        expectedMatch =
            len >= expectedMatch ? expectedMatch : bsl::max(0, len - 1);
        ASSERT(out32Buf + MAX_LEN ==
               bsl::find_if(out32Buf + expectedCodePoints, out32Buf + MAX_LEN,
                            NotEqual<unsigned int>(fillWord)));
    ...
    for (int tm = 0; tm < 4 * NUM_UTF8_TABLE; ++tm) {
      ...
      for (int len = MAX_LEN; len >= 0; --len) {
        ...
        expectedRet |=
            len < (IS_ERROR ? 2 : 3) ? Status::k_OUT_OF_SPACE_BIT : 0;
        const unsigned expectedCodePoints = bsl::min(len, IS_ERROR ? 2 : 3);
        ...
        int ret =
            useStringView
                ? opposite
                      ...
                      : Util::utf8ToUtf32(out32Buf, len, sr, &numCodePoints, 0)
            : opposite ? Util::utf8ToUtf32(out32Buf, len, UTF8_STRING,
                                           &numCodePoints, 0, oppositeEndian)
                       : Util::utf8ToUtf32(out32Buf, len, UTF8_STRING,
                                           &numCodePoints, 0);
        LOOP4_ASSERT(LINE, len, expectedRet, ret, expectedRet == ret);
            ...
            useStringView
                ? opposite
                      ? Util::utf8ToUtf32(out32Buf, len, sr, &numCodePoints, 0,
                                          oppositeEndian)
                      : Util::utf8ToUtf32(out32Buf, len, sr, &numCodePoints, 0)
            : opposite ? Util::utf8ToUtf32(out32Buf, len, s.c_str(),
                                           &numCodePoints, 0, oppositeEndian)
                       : Util::utf8ToUtf32(out32Buf, len, s.c_str(),
                                           &numCodePoints, 0);
        LOOP4_ASSERT(LINE, len, expectedRet, ret, expectedRet == ret);
        ASSERT(expectedCodePoints == numCodePoints);
        ASSERT(0 == len || 0 == out32Buf[expectedCodePoints - 1]);
    ...
    for (int tm = 0; tm < 4 * NUM_UTF8_TABLE; ++tm) {
      ...
      for (int len = MAX_LEN; len >= 0; --len) {
        ...
        case 1: {
          expectedRet = len < 3 ? Status::k_OUT_OF_SPACE_BIT : 0;
          expectedRet |=
              len >= 3 && fiveOctet ? Status::k_INVALID_INPUT_BIT : 0;
          expectedCodePoints = bsl::min(len, fiveOctet ? 2 : 3);
        } break;
        default: { // 2 or more
          expectedRet = len >= 3 ? Status::k_INVALID_INPUT_BIT : 0;
          expectedRet |= len < 3 ? Status::k_OUT_OF_SPACE_BIT : 0;
        ...
        int ret =
            ...
            : opposite ? Util::utf8ToUtf32(out32Buf, len, s.c_str(),
                       ...
                       : Util::utf8ToUtf32(out32Buf, len, s.c_str(),
                                           &numCodePoints, 0);
        LOOP5_ASSERT(LINE, len, expectedRet, ret, TRUNC_BY, expectedRet == ret);
        ...
        LOOP3_ASSERT(LINE, expectedMatch, len,
                     0 == bsl::memcmp(out32Buf, expectedOut, expectedMatch));
      }
    }
  } break;
  case 3: {
    ...
    if (verbose)
      cout << "Testing UTF-8 -> UTF-32 Vector Translation\n"
              "==========================================\n";
    ...
    TestDriver::testCase3<std::experimental::pmr::vector<unsigned int>>();
#endif
  ...
  case 2: {
    ...
    if (verbose)
      cout << "Testing UTF-8 -> UTF-32 Buffer Translation\n"
              "==========================================\n";
    ...
    for (int mode = 0; mode < 8; ++mode) {
      ...
      for (int ti = 0; ti < NUM_UTF8_TABLE; ++ti) {
        ...
        if (opposite) {
          for (unsigned u = 0; u < sizeof(expectedOut) / sizeof(int); ++u) {
            expectedOut[u] = bsls::ByteOrderUtil::swapBytes(expectedOut[u]);
          }
        }
        ...
        for (int len = MAX_LEN; len >= 0; --len) {
          ...
          int expectedRet =
              IS_ERROR && len > 2 ? Status::k_INVALID_INPUT_BIT : 0;
          expectedRet |= len < 4 ? Status::k_OUT_OF_SPACE_BIT : 0;
          ...
          int ret =
              useStringView
                  ? (checkNumCPs
                         ? (opposite ? Util::utf8ToUtf32(out32Buf, len, sr,
                                                         &numCodePoints, '?',
                                                         oppositeEndian)
                                     : Util::utf8ToUtf32(out32Buf, len, sr,
                                                         &numCodePoints))
                         : Util::utf8ToUtf32(out32Buf, len, sr))
                  : (checkNumCPs
                         ? (opposite
                                ...
                                : Util::utf8ToUtf32(out32Buf, len, UTF8_STRING,
                                                    &numCodePoints))
                         : Util::utf8ToUtf32(out32Buf, len, UTF8_STRING));
          LOOP4_ASSERT(ti, len, expectedRet, ret, expectedRet == ret);
          ASSERT(!checkNumCPs || expectedCodePoints == numCodePoints);
          ASSERT(0 == len || 0 == out32Buf[expectedCodePoints - 1]);

          const size_t expectedMatch =
              (len >= 4 ? 4 : bsl::max(len - 1, 0)) * sizeof(unsigned int);
    ...
    for (int mode = 0; mode < 8; ++mode) {
      ...
      for (int ti = 0; ti < NUM_UTF8_TABLE; ++ti) {
        ...
        for (int len = MAX_LEN; len >= 0; --len) {
          ...
          const size_t expectedMatch =
              (len >= 3 ? 3 : bsl::max(len - 1, 0)) * sizeof(unsigned int);
    ...
    for (int mode = 0; mode < 8; ++mode) {
      ...
      for (int ti = 0; ti < NUM_UTF8_TABLE; ++ti) {
        ...
        for (int len = MAX_LEN; len >= 0; --len) {
          ...
          case 1: {
            expectedRet = len < 3 ? Status::k_OUT_OF_SPACE_BIT : 0;
            expectedRet |= len >= 3 && (UTF8_STRING[1] & 0xf8) == 0xf8
                               ? Status::k_INVALID_INPUT_BIT
                               : 0;
            expectedCodePoints = bsl::min(len, 3);
            expectedMatch =
                bsl::max(0, bsl::min(len - 1, 1)) * sizeof(unsigned int);
          } break;
          default: { // 2 or more
            expectedRet = len >= 3 ? Status::k_INVALID_INPUT_BIT : 0;
            expectedRet |= len < 3 ? Status::k_OUT_OF_SPACE_BIT : 0;
  ...
  case 1: {
    ...
    int sts = Util::utf8ToUtf32(utf32, 100, utf8, &numCodePoints);
    ASSERT(0 == sts);
    ...
    sts = Util::utf32ToUtf8(utf8Out, 100, utf32, &numCodePoints, &numBytes);
    ASSERT(0 == sts);
    ASSERT(13 == numCodePoints);
  ...
  case -1: {
    ...
    const unsigned chars[] = {'H',    'e',    'l',    'l',    'o',     '\n',
                              753,    10000,  100000, 0xd7ff, 0xe000,  0xd800,
                              0xd8ff, 0xdc00, 0xdcff, 0xdd99, 0x1f609, 0};
