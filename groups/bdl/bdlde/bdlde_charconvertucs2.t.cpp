...
template <class T> struct ArrayPrinter {
  const T *d_ptr;
  size_t d_size;

  ArrayPrinter(const T *ptr, size_t size) : d_ptr(ptr), d_size(size) {}
};

template <class T> ArrayPrinter<T> makeArrayPrinter(const T *ptr, size_t size) {
  return ArrayPrinter<T>(ptr, size);
}

template <class T> ostream &operator<<(ostream &os, const ArrayPrinter<T> &t) {
  os << "[";
  for (size_t i = 0; i < t.d_size; ++i) {
    os << " " << bsl::hex << bsl::showbase << bsl::setw(6) << bsl::setfill('0')
       << bsl::internal << unsigned(t.d_ptr[i]);
  }
  os << bsl::dec << " ]";
  return os;
}
...
void testCFunction1() {
  ...
  int retVal = BloombergLP::bdlde::CharConvertUcs2::utf8ToUcs2(
      buffer, buffSize, "Hello", &charsWritten);
...
void testCFunction2() {
  ...
  int retVal = BloombergLP::bdlde::CharConvertUcs2::utf8ToUcs2(buffer, buffSize,
                                                               "\xc3\x89"
                                                               "cole",
                                                               &charsWritten);
  ...
  retVal = BloombergLP::bdlde::CharConvertUcs2::ucs2ToUtf8(
      buffer2, buffer2Size, buffer, &charsWritten, &bytesWritten);

  ASSERT(0 == retVal);
  ASSERT(0 == bsl::strcmp(buffer2, "\xc3\x89"
                                   "cole"));
...
void processUtf8(const char *strU8) {
  ...
  int result = BloombergLP::bdlde::CharConvertUcs2::utf8ToUcs2(
      buffer, buffSize, strU8, &charsWritten);
...
void loadUCS2Hello(bsl::vector<unsigned short> *result) {
  int retVal = BloombergLP::bdlde::CharConvertUcs2::utf8ToUcs2(result, "Hello");
...
void checkCppRoundTrip() {
  ...
  int retVal =
      BloombergLP::bdlde::CharConvertUcs2::utf8ToUcs2(&result, "\xc3\x89"
                                                               "cole");
  ...
  // Reversing the conversion returns the original string:
  retVal = BloombergLP::bdlde::CharConvertUcs2::ucs2ToUtf8(
      &result2, &result.front(), &charsWritten);

  ASSERT(0 == retVal);
  ASSERT(result2 == "\xc3\x89"
                    "cole");
...
static const struct {
  ...
  int d_expectedResult;                 // expected result
    ...
    {L_,
     "\x01\x20\x7f" U8_000ff U8_007ff U8_00800 U8_0ffff,
     {0x1, 0x20, 0x7f, 0xff, 0x7ff, 0x800, 0xffff, 0},
     255,
     8,
     OK},

    {L_,
     "\x01\x20\x7f" U8_000ff U8_007ff U8_00800 "\xef",
     {0x1, 0x20, 0x7f, 0xff, 0x7ff, 0x800, '?', 0},
     255,
     8,
     BADC},

    {L_,
     "\x01\x20\x7f" U8_000ff U8_007ff U8_00800 "\xef\xbf",
     {0x1, 0x20, 0x7f, 0xff, 0x7ff, 0x800, '?', 0},
     255,
     8,
     BADC},
    ...
    {L_,
     U8_0ffff U8_00800 U8_007ff U8_000ff "\x7f\x20\x01",
     {0xffff, 0x800, 0x7ff, 0xff, 0x7f, 0x20, 0x1, 0},
     255,
     8,
     OK},
    ...
    {L_, "\xc2" U8_00080, {'?', 0}, 2, 2, BOTH},

    {L_, "\xc2" U8_00080, {'?', 0x80, 0}, 3, 3, BADC},

    // corrupted 2-octet character followed by an invalid character:
    {L_, "\xc2\xff", {'?', 0}, 2, 2, BOTH},
    {L_, "\xc2\xff", {'?', '?', 0}, 3, 3, BADC},
    ...
    {L_, "\xef \xef", {'?', ' ', 0}, 3, 3, BOTH},
    {L_, "\xef \xef", {'?', ' ', '?', 0}, 4, 4, BADC},
    {L_, "\xef" U8_00080, {'?', 0}, 2, 2, BOTH},

    {L_, "\xef" U8_00080, {'?', 0x80, 0}, 3, 3, BADC},
    ...
    {L_, "\xef\xbf ", {'?', 0}, 2, 2, BOTH},
    {L_, "\xef\xbf ", {'?', ' ', 0}, 3, 3, BADC},
    {L_, "\xef\xbf" U8_00080, {'?', 0}, 2, 2, BOTH},

    {L_, "\xef\xbf" U8_00080, {'?', 0x80, 0}, 3, 3, BADC},

    {L_, "\xef\xbf" U8_00080 " ", {'?', 0}, 2, 2, BOTH},

    {L_, "\xef\xbf" U8_00080 " ", {'?', 0x80, 0}, 3, 3, BOTH},

    {L_, "\xef\xbf" U8_00080 " ", {'?', 0x80, ' ', 0}, 4, 4, BADC},

    {L_, "\xef\xbf\xef\xbf ", {'?', '?', 0}, 3, 3, BOTH},

    {L_, "\xef\xbf\xef\xbf ", {'?', '?', ' ', 0}, 4, 4, BADC},

    {L_, "\xef\xbf \xef\xbf", {'?', 0}, 2, 2, BOTH},
...
void checkForExpectedConversionResultsU2ToU8(unsigned short *input,
  ...
  for (bsl::size_t bufSize = 0; bufSize < totalOutputLength; ++bufSize) {
    ...
    retVal = bdlde::CharConvertUcs2::ucs2ToUtf8(outputBuffer, bufSize, input,
  ...
  retVal = bdlde::CharConvertUcs2::ucs2ToUtf8(
      outputBuffer, totalOutputLength + 1, input, &charsWritten, &bytesWritten);
  ...
  retVal =
      bdlde::CharConvertUcs2::ucs2ToUtf8(&cppOutput, input, &cppCharsWritten);
...
void buildUpAndTestStringsU2ToU8(
    bsl::size_t idx, int depth, unsigned short *inputBuffer, char *outputBuffer,
    unsigned short *characterSizes, bsl::size_t totalOutputLength,
    bsl::size_t characterCount, unsigned short *inputCursor, char *outputCursor,
    int verbose, int veryVerbose) {
  ...
  *outputCursor = 0;
  ...
  checkForExpectedConversionResultsU2ToU8<STRING>(
      inputBuffer, outputBuffer, totalOutputLength, characterSizes,
      characterCount, verbose, veryVerbose);
  ...
  *inputCursor = 0;
  ...
  for (bsl::size_t i = 0; i < precomputedDataCount; ++i) {
    buildUpAndTestStringsU2ToU8<STRING>(i, depth - 1, inputBuffer, outputBuffer,
                                        characterSizes, totalOutputLength,
                                        characterCount, inputCursor,
                                        outputCursor, verbose, veryVerbose);
  }
}
...
void testSingleOctetPerturbation(const char *input, bsl::size_t perturbationPos,
                                 ...
                                 unsigned short *characterSizes,
                                 bsl::size_t characterCount,
                                 const PerturbationDesc &perturb, int verbose,
                                 int veryVerbose) {
  ...
  int retVal = bdlde::CharConvertUcs2::utf8ToUcs2(
      outputBuffer, characterCount - 1, inputBuffer, &charsWritten);
  ...
  retVal = bdlde::CharConvertUcs2::utf8ToUcs2(outputBuffer, characterCount,
                                              inputBuffer, &charsWritten);

  if (veryVerbose) {
    cout << "PERTURBING:" << "\n\tinputBuffer       =";
    printStr(inputBuffer);
    cout << "\n\toutputBuffer      ="
         << makeArrayPrinter(outputBuffer, characterCount)
         << "\n\tperturbation      = { '" << perturb.d_octetToConvertTo
         << "', '" << (perturb.d_isNewValid ? "Y" : "N") << "', '"
         << perturb.d_newCharacter << "', " << perturb.d_extraInvalidBefore
         << "', " << perturb.d_extraInvalidAfter << "}"
         << "\n\torigInput         =";
    printStr(input);
    cout << "\n\torigExpectedOutput="
         << makeArrayPrinter(origExpectedOutput, characterCount) << endl;
  }
...
void perturbUtf8AndCheckConversionFailures(const char *input,
  ...
  for (bsl::size_t currentChar = 0; currentChar < characterCount;
       ++currentChar) {
    ...
    case 1: {
      ...
      for (bsl::size_t i = 0; i < testCount; ++i) {
        testSingleOctetPerturbation(
            input, currentCharStart, currentChar, expected_output,
            totalInputLength, characterSizes, characterCount,
    ...
    case 2: {
...

      {
        ...
        for (bsl::size_t i = 0; i < testCount; ++i) {
          testSingleOctetPerturbation(
              input, currentCharStart + 1, currentChar, expected_output,
              totalInputLength, characterSizes, characterCount,
    ...
    case 3: {
...

      {
        ...
        const unsigned short newChar = static_cast<unsigned short>(
            ((0xc2 & 0x1f) << 6) | (input[currentCharStart + 1] & 0x3f));
        const PerturbationDesc perturb = {0xc2, true, newChar, 0, 1};

        testSingleOctetPerturbation(input, currentCharStart, currentChar,
                                    expected_output, totalInputLength,
                                    characterSizes, characterCount, perturb,
                                    verbose, veryVerbose);
      }
      ...
      // perturbing 3-octet character, octet 2
      {
        ...
        // data-driven since we must compute the resulting character.
        const unsigned short newChar = static_cast<unsigned short>(
            ((0xc2 & 0x1f) << 6) | (input[currentCharStart + 2] & 0x3f));
        const PerturbationDesc perturb = {0xc2, true, newChar, 1, 0};

        testSingleOctetPerturbation(input, currentCharStart + 1, currentChar,
                                    expected_output, totalInputLength,
                                    characterSizes, characterCount, perturb,
                                    verbose, veryVerbose);
      }
...

      {
        ...
        for (bsl::size_t i = 0; i < testCount; ++i) {
          testSingleOctetPerturbation(
              input, currentCharStart + 2, currentChar, expected_output,
              totalInputLength, characterSizes, characterCount,
...
void checkForExpectedConversionResultsU8ToU2(const char *input,
  ...
  perturbUtf8AndCheckConversionFailures(input, expected_output,
                                        totalInputLength, characterSizes,
                                        characterCount, verbose, veryVerbose);
  ...
  for (bsl::size_t bufSize = 0; bufSize < characterCount; ++bufSize) {
    ...
    retVal = bdlde::CharConvertUcs2::utf8ToUcs2(outputBuffer, bufSize, input,
                                                &charsWritten);

    LOOP5_ASSERT(L_, OUTPUT_BUFFER_TOO_SMALL, retVal, bufSize, characterCount,
                 OUTPUT_BUFFER_TOO_SMALL == retVal);
    LOOP3_ASSERT(L_, charsWritten, bufSize, charsWritten == bufSize);
  }

  unsigned short outputBuffer[256] = {0};
  bsl::size_t charsWritten = 0;

  retVal = bdlde::CharConvertUcs2::utf8ToUcs2(outputBuffer, characterCount + 1,
                                              input, &charsWritten);
  ...
  LOOP5_ASSERT(L_, characterCount + 1, charsWritten,
  ...
  retVal = bdlde::CharConvertUcs2::utf8ToUcs2(&cppOutput, input);
  ...
  LOOP3_ASSERT(L_, cppCharsWritten, 1 + characterCount,
               cppCharsWritten == 1 + characterCount);
  LOOP5_ASSERT(L_, characterCount + 1, cppCharsWritten,
               makeArrayPrinter(&(cppOutput[0]), characterCount + 1),
               makeArrayPrinter(expected_output, charsWritten),
               0 == memcmp(&(cppOutput[0]), expected_output, resultBytes));
}
...
void buildUpAndTestStringsU8ToU2(bsl::size_t idx, int depth, char *inputBuffer,
                                 unsigned short *outputBuffer,
                                 unsigned short *characterSizes,
                                 bsl::size_t totalOutputLength,
                                 bsl::size_t characterCount, char *inputCursor,
  ...
  *outputCursor = 0;
  ...
  checkForExpectedConversionResultsU8ToU2(inputBuffer, outputBuffer,
                                          totalOutputLength, characterSizes,
                                          characterCount, verbose, veryVerbose);
  ...
  *outputCursor = 0;
  ...
  for (bsl::size_t i = 0; i < precomputedDataCount; ++i) {
    buildUpAndTestStringsU8ToU2(i, depth - 1, inputBuffer, outputBuffer,
                                characterSizes, totalOutputLength,
                                characterCount, inputCursor, outputCursor,
                                verbose, veryVerbose);
  }
}
...
int runPlainTextPerformanceTest(void) {
  ...
  for (int i = 0; i < iterLimit; ++i) {
    ASSERT(SUCCESS == bdlde::CharConvertUcs2::utf8ToUcs2(ucs2Buffer_p, prideLen,
                                                         prideAndPrejudice,
                                                         &charsWritten));
    ASSERT(charsWritten == prideLen);

    ASSERT(0 == bdlde::CharConvertUcs2::ucs2ToUtf8(utf8Buffer_p, prideLen,
                                                   ucs2Buffer_p, &charsWritten,
                                                   &bytesWritten));
    ASSERT(charsWritten == prideLen);
  ...
  cout << "Performance test for C-style interface , converted " << prideLen
       << " characters " << "back and forth " << iterLimit << " times in "
       << s.elapsedTime() << " seconds." << endl;
  ...
  for (int i = 0; i < iterLimit; ++i) {
    ...
    if (cppPrideUcs2.size() != prideLen) {
      cout << "cppPrideUcs2.size() == " << cppPrideUcs2.size()
           << ", prideLen = " << prideLen << endl;
    }

    ASSERT(0 == bdlde::CharConvertUcs2::ucs2ToUtf8(
                    &cppPrideUtf8, &(cppPrideUcs2[0]), &charsWritten));
    ASSERT(charsWritten == prideLen);
    ASSERT(cppPrideUtf8.length() == prideLen - 1);

    if (cppPrideUtf8.length() != prideLen - 1) {
      cout << "cppPrideUtf8.length() == " << cppPrideUtf8.length()
           << ", prideLen = " << prideLen << endl;
    }
  }
...
struct TestDriver {
  // TEST CASES
  template <class STRING> static void testCase4();
  // Test 'ucs-2 -> utf-8' conversion.

  template <class VECTOR> static void testCase3();
  // Test 'utf-8 -> ucs-2' conversion.
};

template <class STRING> void TestDriver::testCase4() {
  for (int i = 0; i < NUM_TEST_4_DATA; ++i) {
    ...
    if (OK == RETURN) {
      ...
      int retVal =
          bdlde::CharConvertUcs2::ucs2ToUtf8(&cppResult, SPEC, &charsWritten);
...

  {
    for (bsl::size_t i = 0; i < precomputedDataCount; ++i) {
      ...
      buildUpAndTestStringsU2ToU8<STRING>(
          i, exhaustiveSearchDepth, inputBuffer, outputBuffer, characterSizes,
          totalOutputLength, characterCount, inputCursor, outputCursor, verbose,
          veryVerbose);
    }
  }
}

template <class VECTOR> void TestDriver::testCase3() {
  bslma::TestAllocator ca("container");
  ...
  for (int i = 0; i < NUM_TEST_3_DATA; ++i) {
    ...
    if (OK == RETURN || BADC == RETURN) {
      int cppRetVal = bdlde::CharConvertUcs2::utf8ToUcs2(&cppResult, SPEC);
      ...
      if (cppCharsWritten > 0) {
        LOOP4_ASSERT(LINE, makeArrayPrinter(OUTPUT, EXPECTED),
...

  {
    static const struct {
      ...
      unsigned short d_expectedWidth; // space we expect it to
                                      // use
    } DATA[] = {{L_, '?', 1}, {L_, '*', 1}, {L_, '\0', 0}, {L_, 0x1234, 1}};
    ...
    for (bsl::size_t i = 0; i < sizeof DATA / sizeof *DATA; ++i) {
      ...
      int cppRetVal =
          bdlde::CharConvertUcs2::utf8ToUcs2(&cppResult, "\xef", PLACEHOLDER);
      ...
      LOOP3_ASSERT(LINE, PLACEHOLDER, cppResult.at(0),
      ...
      LOOP3_ASSERT(LINE, 1 + WIDTH, cppCharsWritten,
                   static_cast<bsl::size_t>(1 + WIDTH) == cppCharsWritten);
    }
  }
}
...
int main(int argc, char **argv) {
  ...
  case 4: {
      ...
      cout << "\nTesting 'bdlde::CharConvertUcs2::ucs2ToUtf8'." << endl;
    {
      for (int i = 0; i < NUM_TEST_4_DATA; ++i) {
        ...
        int retVal = bdlde::CharConvertUcs2::ucs2ToUtf8(
            results, BUFSIZE, SPEC, &charsWritten, &bytesWritten);

        LOOP_ASSERT(LINE, charsWritten == EXPECTEDCHARS);
        LOOP_ASSERT(LINE, bytesWritten == EXPECTEDBYTES);
        LOOP_ASSERT(LINE, retVal == RETURN);
        LOOP_ASSERT(LINE, 0 == memcmp(OUTPUT, results, charsWritten));
        if (veryVerbose) {
          printf("%4d: SPEC     ", LINE);
          printStr(SPEC);
          printf("\n");
...

    {
      ...
      retVal = bdlde::CharConvertUcs2::ucs2ToUtf8(0, 0, 0, &charsWritten);
      ...
      bytesWritten = 1;
      retVal =
          bdlde::CharConvertUcs2::ucs2ToUtf8(0, 0, 0,
  ...
  case 3: {
      ...
      cout << "\nTesting 'bdlde::CharConvertUcs2::utf8ToUcs2'." << endl;
    {
      for (int i = 0; i < NUM_TEST_3_DATA; ++i) {
        ...
        int retVal = bdlde::CharConvertUcs2::utf8ToUcs2(results, BUFSIZE, SPEC,
                                                        &charsWritten);

        LOOP3_ASSERT(LINE, charsWritten, EXPECTED, charsWritten == EXPECTED);
        LOOP3_ASSERT(LINE, retVal, RETURN, retVal == RETURN);
        LOOP4_ASSERT(LINE, makeArrayPrinter(OUTPUT, EXPECTED),
                     makeArrayPrinter(results, charsWritten), charsWritten,
                     0 ==
                         memcmp(OUTPUT, results, charsWritten * sizeof(short)));
        if (veryVerbose) {
          printf("%4d: SPEC     ", LINE);
          printStr(SPEC);
          printf("\n");
          printf("    : EXPECTED = %d, charsWritten = %d\n",
                 static_cast<int>(EXPECTED), static_cast<int>(charsWritten));
          printf("    : RETURN   = %d retVal        = %d\n", RETURN, retVal);
...

      {
        ...
        for (bsl::size_t i = 0; i < sizeof DATA / sizeof *DATA; ++i) {
          ...
          unsigned short results[256] = {0};
          bsl::size_t resultsLen = sizeof results / sizeof *results;
          bsl::size_t charsWritten = -1;

          int expectedRetVal = INVALID_INPUT_CHARACTER;

          int retVal = bdlde::CharConvertUcs2::utf8ToUcs2(
              results, resultsLen, "\xef", &charsWritten, PLACEHOLDER);

          LOOP3_ASSERT(LINE, retVal, expectedRetVal, retVal == expectedRetVal);

          LOOP3_ASSERT(LINE, PLACEHOLDER, results[0],
                       PLACEHOLDER == results[0]);

          if (WIDTH > 0) {
            LOOP3_ASSERT(LINE, 0, results[1], 0 == results[1]);
          }
          LOOP3_ASSERT(LINE, 1 + WIDTH, charsWritten,
                       static_cast<bsl::size_t>(1 + WIDTH) == charsWritten);
        }
      }
      ...
      {
        int retVal = bdlde::CharConvertUcs2::utf8ToUcs2(0, 0, "");
        ASSERT(OUTPUT_BUFFER_TOO_SMALL == retVal);
        ASSERT(2 == retVal);

        bsl::size_t charsWritten = 1;
        retVal = bdlde::CharConvertUcs2::utf8ToUcs2(0, 0, "", &charsWritten);
        ASSERT(OUTPUT_BUFFER_TOO_SMALL == retVal);
        ASSERT(2 == retVal);
        ASSERT(0 == charsWritten);
      }
...

      {
        for (bsl::size_t i = 0; i < precomputedDataCount; ++i) {
          ...
          buildUpAndTestStringsU8ToU2(
              i, exhaustiveSearchDepth, inputBuffer, outputBuffer,
              characterSizes, totalOutputLength, characterCount, inputCursor,
              outputCursor, verbose, veryVerbose);
        }
      }
  ...
  case 1: {
    ...
    ASSERT(SUCCESS ==
           BloombergLP::bdlde::CharConvertUcs2::utf8ToUcs2(
               ucs2_buffer, sizeof ucs2_buffer / sizeof *ucs2_buffer, ecole));

    ASSERT(SUCCESS == BloombergLP::bdlde::CharConvertUcs2::ucs2ToUtf8(
                          utf8_buffer, sizeof utf8_buffer / sizeof *utf8_buffer,
                          ucs2_buffer));
