...
enum {
  ...
  k_OUT_OF_SPACE_BIT = BloombergLP::bdlde::CharConvertStatus::k_OUT_OF_SPACE_BIT
};
...
struct Capacity {
  ...
  // CREATORS
  explicit Capacity(bsl::size_t capacity);
...
// CREATOR
inline Capacity::Capacity(bsl::size_t capacity) : d_capacity(capacity) {}

// MANIPULATORS
inline void Capacity::operator--()
// Decrement 'd_capacity'.
{
  --d_capacity;
}

inline void Capacity::operator-=(int delta)
...
// ACCESSORS
inline bool Capacity::operator<(bsl::size_t rhs) const
// Return 'true' if 'd_capacity' is less than the specified 'rhs', and
// 'false' otherwise.
{
...
struct NoopCapacity {
  ...
  // CREATORS
  explicit NoopCapacity(bsl::size_t);
  // Create empty object.
...
// CREATOR
inline NoopCapacity::NoopCapacity(bsl::size_t)
// Create empty object.
{}

// MANIPULATORS
inline void NoopCapacity::operator--()
// No-op.
{}

inline void NoopCapacity::operator-=(int)
// No-op.
{}

// ACCESSORS
inline bool NoopCapacity::operator<(bsl::size_t) const
// Return 'false'.
{
  return false;
}
...
inline unsigned int Swapper::swapBytes(unsigned int x) {
...
inline unsigned int NoopSwapper::swapBytes(unsigned int x) { return x; }
...
public:
  // CREATORS
  explicit Utf8PtrBasedEnd(const char *end);
  ...
  const OctetType *skipContinuations(const OctetType *octets, int skipBy) const;
...
// CREATORS
inline Utf8PtrBasedEnd::Utf8PtrBasedEnd(const char *end)
    : d_end(reinterpret_cast<const OctetType *>(end)) {}

// ACCESSORS
inline bool Utf8PtrBasedEnd::isFinished(const OctetType *position) const {
  if (position < d_end) {
    return false; // RETURN
  } else {
...
inline const OctetType *
Utf8PtrBasedEnd::skipContinuations(const OctetType *octets, int skipBy) const {
...
inline bool Utf8PtrBasedEnd::verifyContinuations(const OctetType *octets,
                                                 int n) const {
  BSLS_ASSERT(n >= 1);
  BSLS_ASSERT(d_end >= octets);
...
// CREATORS
inline Utf8ZeroBasedEnd::Utf8ZeroBasedEnd() {}

// ACCESSORS
inline bool Utf8ZeroBasedEnd::isFinished(const OctetType *position) const {
  return 0 == *position;
}

inline const OctetType *
Utf8ZeroBasedEnd::skipContinuations(const OctetType *octets, int skipBy) const {
  // This function is not called in the critical path.  It is called when it
...
inline bool Utf8ZeroBasedEnd::verifyContinuations(const OctetType *octets,
                                                  int n) const {
  BSLS_ASSERT(n >= 1);
...
public:
  // CREATORS
  explicit Utf32PtrBasedEnd(const unsigned int *end);
  // Create a 'Utf8PtrBasedEnd' object with the end at the specified
  // 'end'.
...
// CREATORS
inline Utf32PtrBasedEnd::Utf32PtrBasedEnd(const unsigned int *end)
    : d_end_p(end) {}

// ACCESSORS
inline bool Utf32PtrBasedEnd::isFinished(const unsigned int *position) const {
  if (position < d_end_p) {
    return false; // RETURN
  } else {
...
// CREATORS
inline Utf32ZeroBasedEnd::Utf32ZeroBasedEnd() {}

// ACCESSORS
inline bool Utf32ZeroBasedEnd::isFinished(const unsigned int *position) const {
  return 0 == *position;
}

} // namespace

static inline const OctetType *constOctetCast(const char *ptr)
...
static inline OctetType *octetCast(char *ptr)
...
static inline bool isSingleOctet(OctetType oct)
// Return 'true' if the specified 'oct' is a single-octet UTF-8 sequence.
{
  return !(oct & k_ONE_OCTET_MASK);
}

static inline bool isContinuation(OctetType oct)
...
static inline bool isTwoOctetHeader(OctetType oct)
...
static inline bool isThreeOctetHeader(OctetType oct)
...
static inline bool isFourOctetHeader(OctetType oct)
...
static inline unsigned int decodeTwoOctets(const OctetType *octBuf)
...
static inline unsigned int decodeThreeOctets(const OctetType *octBuf)
...
static inline unsigned int decodeFourOctets(const OctetType *octBuf)
...
static inline bsl::size_t lookaheadContinuations(const OctetType *const octBuf,
                                                 int n)
...
static inline bool fitsInSingleOctet(unsigned int uc)
...
static inline bool fitsInTwoOctets(unsigned int uc)
// Return 'true' if the specified Unicode value 'uc' can be coded in two
// UTF-8 octets or less and 'false' otherwise.
{
  return 0 == (uc & (~(unsigned int)0
                     << (k_TWO_OCT_CONT_WID + k_CONTINUE_CONT_WID)));
}

static inline bool fitsInThreeOctets(unsigned int uc)
// Return 'true' if the specified Unicode value 'uc' can be coded in three
// UTF-8 octets or less and 'false' otherwise.
{
  return 0 == (uc & (~(unsigned int)0
                     << (k_THREE_OCT_CONT_WID + 2 * k_CONTINUE_CONT_WID)));
}

static inline bool fitsInFourOctets(unsigned int uc)
// Return 'true' if the specified Unicode value 'uc' can be coded in four
// UTF-8 octets or less and 'false' otherwise.
{
  return 0 == (uc & (~(unsigned int)0
                     << (k_FOUR_OCT_CONT_WID + 3 * k_CONTINUE_CONT_WID)));
}

static inline void encodeTwoOctets(OctetType *octBuf, unsigned int isoBuf)
...
// at by the specified 'octBuf'.
{
  ...
  octBuf[0] = (OctetType)(k_TWO_OCTET_TAG | ((isoBuf >> k_CONTINUE_CONT_WID) &
                                             ~k_TWO_OCTET_MASK));
}

static inline void encodeThreeOctets(OctetType *octBuf, unsigned int isoBuf)
...
// at by the specified 'octBuf'.
{
  ...
  octBuf[0] =
      (OctetType)(k_THREE_OCTET_TAG |
                  ((isoBuf >> 2 * k_CONTINUE_CONT_WID) & ~k_THREE_OCTET_MASK));
}

static inline void encodeFourOctets(OctetType *octBuf, unsigned int isoBuf)
...
// at by the specified 'octBuf'.
{
  ...
  octBuf[2] = (OctetType)(k_CONTINUE_TAG |
                          ((isoBuf >> k_CONTINUE_CONT_WID) & ~k_CONTINUE_MASK));
  octBuf[1] =
      (OctetType)(k_CONTINUE_TAG |
                  ((isoBuf >> 2 * k_CONTINUE_CONT_WID) & ~k_CONTINUE_MASK));
  octBuf[0] =
      (OctetType)(k_FOUR_OCTET_TAG |
                  ((isoBuf >> 3 * k_CONTINUE_CONT_WID) & ~k_FOUR_OCTET_MASK));
}

static inline bool isIllegal16BitValue(unsigned int uc)
...
static inline bool isIllegalFourOctetValue(unsigned int uc)
...
static inline bool isLegalUtf32ErrorWord(unsigned int uc)
...
static inline const OctetType *skipUtf8CodePoint(const OctetType *input)
...
static bsl::size_t utf32BufferLengthNeeded(const char *input,
...
static bsl::size_t utf8BufferLengthNeeded(const unsigned int *input,
                                          END_FUNCTOR endFunctor,
  ...
  for (; !endFunctor.isFinished(input); ++input) {
    ...
    ret += fitsInSingleOctet(uc) ? 1
           ...
           : fitsInThreeOctets(uc)
               ? (isIllegal16BitValue(uc) ? errorByteSize : 3)
           : fitsInFourOctets(uc)
               ? (isIllegalFourOctetValue(uc) ? errorByteSize : 4)
               : errorByteSize;
  }
...
private:
  // PRIVATE CREATORS
  Utf8ToUtf32Translator(unsigned int *output, bsl::size_t capacity,
                        END_FUNCTOR endFunctor, const OctetType *input,
...
public:
  // CLASS METHODS
  static int translate(unsigned int *output, bsl::size_t capacity,
                       END_FUNCTOR endFunctor, const char *input,
                       bsl::size_t *numWordsWritten, unsigned int errorWord);
...
// PRIVATE CREATORS
template <class CAPACITY, class END_FUNCTOR, class SWAPPER>
inline Utf8ToUtf32Translator<CAPACITY, END_FUNCTOR, SWAPPER>::
    Utf8ToUtf32Translator(unsigned int *output, bsl::size_t capacity,
                          END_FUNCTOR endFunctor, const OctetType *input,
                          unsigned int errorWord)
    : d_output(output), d_capacity(capacity), d_endFunctor(endFunctor),
      d_input(input), d_swappedErrorWord(SWAPPER::swapBytes(errorWord)),
      d_invalidSequences(false) {}

// PRIVATE MANIPULATORS
template <class CAPACITY, class END_FUNCTOR, class SWAPPER>
inline void
Utf8ToUtf32Translator<CAPACITY, END_FUNCTOR, SWAPPER>::advanceOutput() {
  ++d_output;
  --d_capacity;
}
...
int Utf8ToUtf32Translator<CAPACITY, END_FUNCTOR, SWAPPER>::translate(
    unsigned int *output, bsl::size_t capacity, END_FUNCTOR endFunctor,
    const char *input, bsl::size_t *numWordsWritten, unsigned int errorWord) {
  Utf8ToUtf32Translator<CAPACITY, END_FUNCTOR, SWAPPER> translator(
      output, capacity, endFunctor, constOctetCast(input), errorWord);
  ...
  *numWordsWritten = ncw;
  BSLS_ASSERT((bsl::is_same<CAPACITY, NoopCapacity>::value) || ncw <= capacity);
  BSLS_ASSERT(ncw <= (bsl::size_t)(translator.d_input - constOctetCast(input)));
...
private:
  // PRIVATE CREATOR
  Utf32ToUtf8Translator(OctetType *output, bsl::size_t capacity,
                        const unsigned int *input, const OctetType errorByte);
...
public:
  // PUBLIC CLASS METHOD
  static int translate(char *output, bsl::size_t capacity,
                       END_FUNCTOR endFunctor, const unsigned int *input,
                       bsl::size_t *numCodePointsWritten,
                       bsl::size_t *numBytesWritten, const char errorByte);
...
// PRIVATE CREATORS
template <class CAPACITY, class END_FUNCTOR, class SWAPPER>
inline Utf32ToUtf8Translator<CAPACITY, END_FUNCTOR, SWAPPER>::
    Utf32ToUtf8Translator(OctetType *output, bsl::size_t capacity,
                          const unsigned int *input, const OctetType errorByte)
    : d_output(output), d_capacity(capacity), d_input(input),
      d_errorByte(errorByte), d_numCodePointsWritten(0),
      d_invalidSequences(false) {}

// PRIVATE MANIPULATORS
template <class CAPACITY, class END_FUNCTOR, class SWAPPER>
inline void
Utf32ToUtf8Translator<CAPACITY, END_FUNCTOR, SWAPPER>::advance(unsigned delta) {
  d_capacity -= delta;
  d_output += delta;
...
template <class CAPACITY, class END_FUNCTOR, class SWAPPER>
inline int
Utf32ToUtf8Translator<CAPACITY, END_FUNCTOR, SWAPPER>::handleInvalidWord() {
  d_invalidSequences = true;
...
int Utf32ToUtf8Translator<CAPACITY, END_FUNCTOR, SWAPPER>::translate(
    char *output, bsl::size_t capacity, END_FUNCTOR endFunctor,
    const unsigned int *input, bsl::size_t *numCodePointsWritten,
    bsl::size_t *numBytesWritten, const char errorByte) {
  BSLS_ASSERT(0 == (0x80 & errorByte));

  Utf32ToUtf8Translator<CAPACITY, END_FUNCTOR, SWAPPER> translator(
      octetCast(output), capacity, input, errorByte);
  BSLS_ASSERT(translator.d_capacity >= 1);
...
int utf8ToUtf32Impl(VECTOR *dstVector, const char *srcString,
                    unsigned int errorWord,
                    BloombergLP::bdlde::ByteOrder::Enum byteOrder) {
  typedef Utf8ToUtf32Translator<NoopCapacity, Utf8ZeroBasedEnd, Swapper>
      SwapTranslator;
  typedef Utf8ToUtf32Translator<NoopCapacity, Utf8ZeroBasedEnd, NoopSwapper>
      NoSwapTranslator;
  Utf8ZeroBasedEnd endFunctor;

  bsl::size_t bufferLen = utf32BufferLengthNeeded(srcString, endFunctor);
  ...
  int ret =
      BloombergLP::bdlde::ByteOrder::e_HOST == byteOrder
          ? NoSwapTranslator::translate(&dstVector->front(), 0, endFunctor,
                                        srcString, &numWordsWritten, errorWord)
          : SwapTranslator::translate(&dstVector->front(), 0, endFunctor,
                                      srcString, &numWordsWritten, errorWord);
...
template <class VECTOR>
int utf8ToUtf32Impl(VECTOR *dstVector, const bsl::string_view &srcString,
                    unsigned int errorWord,
                    BloombergLP::bdlde::ByteOrder::Enum byteOrder) {
  typedef Utf8ToUtf32Translator<NoopCapacity, Utf8PtrBasedEnd, Swapper>
      SwapTranslator;
  typedef Utf8ToUtf32Translator<NoopCapacity, Utf8PtrBasedEnd, NoopSwapper>
      NoSwapTranslator;
  Utf8PtrBasedEnd endFunctor(srcString.data() + srcString.length());

  bsl::size_t bufferLen = utf32BufferLengthNeeded(srcString.data(), endFunctor);
  BSLS_ASSERT(bufferLen > 0);
  dstVector->resize(bufferLen);
  ...
  int ret = BloombergLP::bdlde::ByteOrder::e_HOST == byteOrder
                ? NoSwapTranslator::translate(&dstVector->front(), 0,
                                              endFunctor, srcString.data(),
                                              &numWordsWritten, errorWord)
                : SwapTranslator::translate(&dstVector->front(), 0, endFunctor,
                                            srcString.data(), &numWordsWritten,
                                            errorWord);
...
int utf32ToUtf8StringImpl(STRING *dstString, const unsigned int *srcString,
                          bsl::size_t *numCodePointsWritten,
                          unsigned char errorByte,
                          BloombergLP::bdlde::ByteOrder::Enum byteOrder) {
  typedef Utf32ToUtf8Translator<NoopCapacity, Utf32ZeroBasedEnd, NoopSwapper>
      NoSwapTranslator;
  typedef Utf32ToUtf8Translator<NoopCapacity, Utf32ZeroBasedEnd, Swapper>
      SwapTranslator;
  ...
  bsl::size_t bufferLen =
      BloombergLP::bdlde::ByteOrder::e_HOST == byteOrder
          ? utf8BufferLengthNeeded<Utf32ZeroBasedEnd, NoopSwapper>(
                srcString, endFunctor, errorByte)
  ...
  int ret = BloombergLP::bdlde::ByteOrder::e_HOST == byteOrder
                ? NoSwapTranslator::translate(begin, 0, endFunctor, srcString,
                                              numCodePointsWritten,
                                              &numBytesWritten, errorByte)
                : SwapTranslator::translate(begin, 0, endFunctor, srcString,
                                            numCodePointsWritten,
                                            &numBytesWritten, errorByte);
...
int utf32ToUtf8StringImpl(STRING *dstString, const unsigned int *srcString,
                          ...
                          unsigned char errorByte,
                          BloombergLP::bdlde::ByteOrder::Enum byteOrder) {
  typedef Utf32ToUtf8Translator<NoopCapacity, Utf32PtrBasedEnd, NoopSwapper>
      NoSwapTranslator;
  typedef Utf32ToUtf8Translator<NoopCapacity, Utf32PtrBasedEnd, Swapper>
      SwapTranslator;
  ...
  bsl::size_t bufferLen =
      BloombergLP::bdlde::ByteOrder::e_HOST == byteOrder
          ? utf8BufferLengthNeeded<Utf32PtrBasedEnd, NoopSwapper>(
                srcString, endFunctor, errorByte)
          : utf8BufferLengthNeeded<Utf32PtrBasedEnd, Swapper>(
                srcString, endFunctor, errorByte);
  BSLS_ASSERT(bufferLen > 0);
  dstString->resize(bufferLen);
  BSLS_ASSERT(dstString->length() == bufferLen);
...
template <class VECTOR>
int utf32ToUtf8VectorImpl(VECTOR *dstVector, const unsigned int *srcString,
                          bsl::size_t *numCodePointsWritten,
                          unsigned char errorByte,
                          BloombergLP::bdlde::ByteOrder::Enum byteOrder) {
  typedef Utf32ToUtf8Translator<NoopCapacity, Utf32ZeroBasedEnd, NoopSwapper>
  ...
  bsl::size_t bufferLen =
      BloombergLP::bdlde::ByteOrder::e_HOST == byteOrder
          ? utf8BufferLengthNeeded<Utf32ZeroBasedEnd, NoopSwapper>(
                srcString, Utf32ZeroBasedEnd(), errorByte)
          : utf8BufferLengthNeeded<Utf32ZeroBasedEnd, Swapper>(
                srcString, Utf32ZeroBasedEnd(), errorByte);
  BSLS_ASSERT(bufferLen > 0);
  dstVector->resize(bufferLen);
  ...
  int ret = BloombergLP::bdlde::ByteOrder::e_HOST == byteOrder
                ? NoSwapTranslator::translate(begin, 0, Utf32ZeroBasedEnd(),
                                              srcString, numCodePointsWritten,
                                              &numBytesWritten, errorByte)
                : SwapTranslator::translate(begin, 0, Utf32ZeroBasedEnd(),
                                            srcString, numCodePointsWritten,
                                            &numBytesWritten, errorByte);
...
int CharConvertUtf32::utf8ToUtf32(bsl::vector<unsigned int> *dstVector,
                                  const char *srcString, unsigned int errorWord,
...
int CharConvertUtf32::utf8ToUtf32(
    ...
    const char *srcString, unsigned int errorWord, ByteOrder::Enum byteOrder) {
  BSLS_ASSERT(dstVector);
  BSLS_ASSERT(srcString);
  BSLS_ASSERT(isLegalUtf32ErrorWord(errorWord));
...
int CharConvertUtf32::utf8ToUtf32(
    std::experimental::pmr::vector<unsigned int> *dstVector,
    const bsl::string_view &srcString, unsigned int errorWord,
    ByteOrder::Enum byteOrder) {
  BSLS_ASSERT(dstVector);
  BSLS_ASSERT(srcString.data());
  BSLS_ASSERT(isLegalUtf32ErrorWord(errorWord));
...
int CharConvertUtf32::utf8ToUtf32(unsigned int *dstBuffer,
  ...
  typedef Utf8ToUtf32Translator<Capacity, Utf8ZeroBasedEnd, Swapper>
      SwapTranslator;
  typedef Utf8ToUtf32Translator<Capacity, Utf8ZeroBasedEnd, NoopSwapper>
      NoSwapTranslator;
  ...
  return ByteOrder::e_HOST == byteOrder
             ? NoSwapTranslator::translate(dstBuffer, dstCapacity, endFunctor,
                                           srcString, numCodePointsWritten,
                                           errorWord)
             : SwapTranslator::translate(dstBuffer, dstCapacity, endFunctor,
                                         srcString, numCodePointsWritten,
                                         errorWord);
}
...
int CharConvertUtf32::utf8ToUtf32(unsigned int *dstBuffer,
  ...
  typedef Utf8ToUtf32Translator<Capacity, Utf8PtrBasedEnd, Swapper>
      SwapTranslator;
  typedef Utf8ToUtf32Translator<Capacity, Utf8PtrBasedEnd, NoopSwapper>
      NoSwapTranslator;
  Utf8PtrBasedEnd endFunctor(srcString.data() + srcString.length());
  ...
  return ByteOrder::e_HOST == byteOrder
             ? NoSwapTranslator::translate(dstBuffer, dstCapacity, endFunctor,
                                           srcString.data(),
                                           numCodePointsWritten, errorWord)
             : SwapTranslator::translate(dstBuffer, dstCapacity, endFunctor,
                                         srcString.data(), numCodePointsWritten,
                                         errorWord);
}
...
int CharConvertUtf32::utf32ToUtf8(bsl::string *dstString,
  ...
  return utf32ToUtf8StringImpl(dstString, srcString, numCodePointsWritten,
                               errorByte, byteOrder);
}
...
int CharConvertUtf32::utf32ToUtf8(std::experimental::pmr::string *dstString,
                                  const unsigned int *srcString,
                                  bsl::size_t *numCodePointsWritten,
                                  unsigned char errorByte,
                                  ByteOrder::Enum byteOrder) {
...
int CharConvertUtf32::utf32ToUtf8(bsl::string *dstString,
  ...
  return utf32ToUtf8StringImpl(dstString, srcString, srcStringLength,
...
int CharConvertUtf32::utf32ToUtf8(bsl::vector<char> *dstVector,
  ...
  return utf32ToUtf8VectorImpl(dstVector, srcString, numCodePointsWritten,
                               errorByte, byteOrder);
}
...
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR
int CharConvertUtf32::utf32ToUtf8(
    std::experimental::pmr::vector<char> *dstVector,
    const unsigned int *srcString, bsl::size_t *numCodePointsWritten,
    unsigned char errorByte, ByteOrder::Enum byteOrder) {
  BSLS_ASSERT(dstVector);
  BSLS_ASSERT(srcString);
  BSLS_ASSERT(errorByte < 0x80);
...
int CharConvertUtf32::utf32ToUtf8(bsl::vector<char> *dstVector,
  ...
  return utf32ToUtf8VectorImpl(dstVector, srcString, srcStringLength,
                               numCodePointsWritten, errorByte, byteOrder);
}
...
int CharConvertUtf32::utf32ToUtf8(
    std::experimental::pmr::vector<char> *dstVector,
    const unsigned int *srcString, bsl::size_t srcStringLength,
    bsl::size_t *numCodePointsWritten, unsigned char errorByte,
    ByteOrder::Enum byteOrder) {
  BSLS_ASSERT(dstVector);
  BSLS_ASSERT(srcString);
  BSLS_ASSERT(errorByte < 0x80);
...
int CharConvertUtf32::utf32ToUtf8(char *dstBuffer, bsl::size_t dstCapacity,
                                  const unsigned int *srcString,
                                  bsl::size_t *numCodePointsWritten,
  ...
  typedef Utf32ToUtf8Translator<Capacity, Utf32ZeroBasedEnd, NoopSwapper>
      NoSwapTranslator;
  typedef Utf32ToUtf8Translator<Capacity, Utf32ZeroBasedEnd, Swapper>
      SwapTranslator;

  int ret = ByteOrder::e_HOST == byteOrder
                ? NoSwapTranslator::translate(
                      dstBuffer, dstCapacity, Utf32ZeroBasedEnd(), srcString,
                      numCodePointsWritten, numBytesWritten, errorByte)
...
int CharConvertUtf32::utf32ToUtf8(char *dstBuffer, bsl::size_t dstCapacity,
  ...
  typedef Utf32ToUtf8Translator<Capacity, Utf32PtrBasedEnd, NoopSwapper>
      NoSwapTranslator;
  typedef Utf32ToUtf8Translator<Capacity, Utf32PtrBasedEnd, Swapper>
      SwapTranslator;
  ...
  int ret = ByteOrder::e_HOST == byteOrder
                ? NoSwapTranslator::translate(
                      dstBuffer, dstCapacity, endFunctor, srcString,
                      numCodePointsWritten, numBytesWritten, errorByte)
                : SwapTranslator::translate(dstBuffer, dstCapacity, endFunctor,
                                            srcString, numCodePointsWritten,
                                            numBytesWritten, errorByte);
