...
void insertUcs2Character(unsigned short **dstBufferPtr,
                         ...
                         bsl::size_t *charsWrittenPtr, unsigned short character)
...
// LOCAL HELPER FUNCTION
void skipBadUtf8Data(unsigned short **dstBufferPtr, bsl::size_t *dstCapacityPtr,
                     const unsigned char **srcPtr, bsl::size_t *charsWrittenPtr,
                     unsigned short errorCharacter, int *retCode,
                     int octetsToSkip)
...
// updating '*dstCapacityPtr' and the specified 'charsWrittenPtr'.
{
  ...
  *retCode |= k_INVALID_INPUT_CHARACTER;
  ...
  insertUcs2Character(dstBufferPtr, dstCapacityPtr, charsWrittenPtr,
                      errorCharacter);
}
...
// 'k_INVALID_INPUT_CHARACTER' bit in the specified '*retCode'.
{
  ...
  if ((firstByte & RANGE1_MASK) == RANGE1_BITS) {
    ...
    if ((EXTRA_BYTE_MARKER_MASK & src[1]) != EXTRA_BITS) {
      ...
      skipBadUtf8Data(dstBufferPtr, dstCapacityPtr, srcPtr, charsWrittenPtr,
                      errorCharacter, retCode, 1);
    ...
    insertUcs2Character(
        dstBufferPtr, dstCapacityPtr, charsWrittenPtr,
  ...
  if ((firstByte & RANGE3_MASK) == RANGE3_BITS) {
    ...
    skipBadUtf8Data(dstBufferPtr, dstCapacityPtr, srcPtr, charsWrittenPtr,
                    errorCharacter, retCode, octetsToSkip);
...
// 'retCode' argument would be required.
{
  ...
  if (*srcString <= TWO_OCTET_CHARACTER_UPPER_BOUND) {
    ...
    if (*dstCapacityPtr < 3) {
      ...
      nullFillRemainingBuffer(dstBufferPtr, dstCapacityPtr, charsWrittenPtr,
                              bytesWrittenPtr);
    ...
    dstBuffer[0] =
        (unsigned char)(RANGE1_BITS | (*srcString >> RANGE1_BYTE1_SHIFT));
  ...
  dstBuffer[0] =
      (unsigned char)(RANGE2_BITS | (*srcString >> RANGE2_BYTE1_SHIFT));
  dstBuffer[1] =
      (unsigned char)(EXTRA_BITS |
                      (EXTRA_MASK & (*srcString >> RANGE2_BYTE2_SHIFT)));
...
int utf8ToUcs2Impl(VECTOR *result, const char *srcString,
  ...
  const unsigned char *src = reinterpret_cast<const unsigned char *>(srcString);
  ...
  while (*src) {
    ...
    if (*src <= ONE_OCTET_CHARACTER_UPPER_BOUND) {
      ...
      *dstBuffer = *src;
      ...
      convertUtf8ToUcs2(&dstBuffer, &dstCapacity, &src, &charsWritten,
                        errorCharacter, &retCode);
    }
  }
...
int ucs2ToUtf8Impl(STRING *result, const unsigned short *srcString,
  ...
  while (*srcString) {
    ...
    if (*srcString <= ONE_OCTET_CHARACTER_UPPER_BOUND) {
      ...
      *dstBuffer = static_cast<unsigned char>(*srcString);
      ...
      convertUcs2ToUtf8(&dstBuffer, &dstCapacity, srcString, &charsWritten,
                        &bytesWritten);
...
int CharConvertUcs2::utf8ToUcs2(unsigned short *dstBuffer,
                                bsl::size_t dstCapacity, const char *srcString,
                                bsl::size_t *numCharsWritten,
                                unsigned short errorCharacter) {
  // Theory: We're mainly dealing with 7-bit ASCII, so we're going to try for
  // an "as tight as possible" inner loop for characters that fit in 7 bits,
...
int CharConvertUcs2::utf8ToUcs2(
    ...
    const char *srcString, unsigned short errorCharacter) {
  return utf8ToUcs2Impl(result, srcString, errorCharacter);
}
#endif

int CharConvertUcs2::ucs2ToUtf8(char *dstBuffer, bsl::size_t dstCapacity,
                                const unsigned short *srcString,
                                bsl::size_t *numCharsWritten,
  ...
  while (*srcString && dstCapacity) {
    if (*srcString <= ONE_OCTET_CHARACTER_UPPER_BOUND) {
      ...
      *dst = static_cast<unsigned char>(*srcString);
      ...
      convertUcs2ToUtf8(&dst, &dstCapacity, srcString, &charsWritten,
                        &bytesWritten);
      // A UTF-16 conversion routine would need a '&retCode' argument
      // passed in here.
    }
