...
#if defined(BSLS_PLATFORM_OS_UNIX) && defined(U_USE_UNIX_FILE_SYSTEM_INTERFACE)
...
struct WindowsInterfaceUtil {
  ...
  static BOOL GetFileTime(HANDLE hFile, LPFILETIME lpCreationTime,
...
struct NameRec {
  ...
  NameRec(const bsl::string &basename, bool foundAsPattern)
      : d_basename(basename), d_foundAsPattern(foundAsPattern)
  ...
  NameRec(const char *basename, bool foundAsPattern)
      : d_basename(), d_foundAsPattern(foundAsPattern)
  // Create a 'NameRec' object having the specified 'basename' and the
  // the specified 'foundAsPattern'.
  {
...
static inline bool shortIsDotOrDots(const char *path)
...
static inline int
performFStat(BloombergLP::bdls::FilesystemUtil::FileDescriptor descriptor,
...
static inline int performStat(const char *fileName, StatResult *statResult,
...
static int
bloombergLP_bdls_FileSystemUtil_isNotFilePermissionsError(const char *,
                                                          int errorNum)
...
#ifdef BSLS_PLATFORM_OS_WINDOWS
static inline void invokeFindClose(void *handle, void *)
...
static inline bool narrowToWide(bsl::wstring *result, const bsl::string &path)
...
inline bool wideToNarrow(STRING_TYPE *result, const bsl::wstring_view &path)
...
static inline int makeDirectory(const char *path, bool)
...
static inline int removeFile(const char *path)
...
static int localFcntlLock(int descriptor, int cmd, int type) {
  struct flock flk;
...
static inline void invokeGlobFree(void *pglob, void *)
...
static inline void invokeCloseDir(void *dir, void *)
...
static inline bool isDotOrDots(const char *path)
...
static inline int makeDirectory(const char *path, bool isPrivate)
// Create a directory.  Return 0 on success, 'k_ERROR_PATH_NOT_FOUND' if a
// component used as a directory in the specified 'path' either does not
// exist or is not a directory, 'k_ERROR_ALREADY_EXISTS' if the file system
// entry (not necessarily a directory) with the name 'path' already exists,
...
template <class STRING_TYPE>
int u_getSymbolicLinkTarget(STRING_TYPE *result, const char *path) {
  BSLS_ASSERT(path);
  BSLS_ASSERT(result);
  ...
  DWORD bytesReturned;
  bool ok =
      DeviceIoControl(hFile, FSCTL_GET_REPARSE_POINT, 0, 0, rdb,
  ...
  case IO_REPARSE_TAG_SYMLINK: { // symlink
    name = rdb->SymbolicLinkReparseBuffer.PathBuffer +
           rdb->SymbolicLinkReparseBuffer.PrintNameOffset / sizeof(wchar_t);
    nameLen = rdb->SymbolicLinkReparseBuffer.PrintNameLength / sizeof(wchar_t);
  } break;
  case IO_REPARSE_TAG_MOUNT_POINT: { // directory junction
    ...
    nameLen = rdb->MountPointReparseBuffer.PrintNameLength / sizeof(wchar_t);
  } break;
  default: {
    return -1; // RETURN
  }
...
template <class STRING_TYPE> int u_getWorkingDirectory(STRING_TYPE *path) {
  BSLS_ASSERT(path);
...
int u_findMatchingPaths(VECTOR_TYPE *result, const char *pattern) {
  ...
  return bdls::FilesystemUtil::visitPaths(
      pattern, bdlf::BindUtil::bind(&pushBackWrapper<VECTOR_TYPE>, result,
...
template <class STRING_TYPE>
bdls::FilesystemUtil::FileDescriptor
u_createTemporaryFile(STRING_TYPE *outPath, const bsl::string_view &prefix) {
  BSLS_ASSERT(outPath);

  bdls::FilesystemUtil::FileDescriptor result;
  STRING_TYPE localOutPath = *outPath;
  for (int i = 0; i < 10; ++i) {
    bdls::FilesystemUtil::makeUnsafeTemporaryFilename(&localOutPath, prefix);
    result = bdls::FilesystemUtil::open(localOutPath.c_str(),
...
FilesystemUtil::open(const char *pathName, FileOpenPolicy openPolicy,
                     FileIOPolicy ioPolicy, FileTruncatePolicy truncatePolicy) {
  BSLS_ASSERT(pathName);

  if (e_OPEN == openPolicy && e_TRUNCATE == truncatePolicy &&
      (e_READ_ONLY == ioPolicy || e_APPEND_ONLY == ioPolicy ||
  ...
  if (narrowToWide(&wide, pathName)) {
    descriptor =
        CreateFileW(wide.c_str(), accessMode,
...
int FilesystemUtil::remove(const char *fileToRemove, bool recursive) {
  ...
  if (isDirectory(fileToRemove)) {
    if (recursive) {
      ...
      for (bsl::vector<bsl::string>::size_type i = 0; i < children.size();
           ++i) {
...
int FilesystemUtil::read(FileDescriptor descriptor, void *buffer,
                         int numBytesToRead) {
  BSLS_ASSERT(buffer);
...
int FilesystemUtil::write(FileDescriptor descriptor, const void *buffer,
                          int numBytesToWrite) {
  BSLS_ASSERT(buffer);
...
int FilesystemUtil::map(FileDescriptor descriptor, void **address,
                        Offset offset, bsl::size_t len, int mode) {
  BSLS_ASSERT(address);
  ...
  hMap = CreateFileMapping(descriptor, NULL, protectAccess[mode][0],
                           (DWORD)(maxLength >> 32),
                           (DWORD)(maxLength & 0xFFFFFFFF), NULL);
  ...
  *address = MapViewOfFile(hMap, protectAccess[mode][1], (DWORD)(offset >> 32),
                           (DWORD)(offset & 0xFFFFFFFF), len);
  CloseHandle(hMap);
  if (!*address) {
    return -1; // RETURN
  }
...
// 3rd arg is sync
{
  ...
  static int pageSizeMask = MemoryUtil::pageSize() - 1;
  (void)pageSizeMask;

  BSLS_ASSERT(0 == (numBytes & pageSizeMask));
  BSLS_ASSERT(0 ==
              (reinterpret_cast<bsls::Types::UintPtr>(address) & pageSizeMask));
...
int FilesystemUtil::lock(FileDescriptor descriptor, bool lockWrite) {
  ...
  return !LockFileEx(descriptor, lockWrite ? LOCKFILE_EXCLUSIVE_LOCK : 0, 0, 1,
                     0, &overlapped);
}
...
int FilesystemUtil::tryLock(FileDescriptor descriptor, bool lockWrite) {
  OVERLAPPED overlapped;
  ZeroMemory(&overlapped, sizeof(overlapped));
  bool success = LockFileEx(descriptor,
                            LOCKFILE_FAIL_IMMEDIATELY |
                                (lockWrite ? LOCKFILE_EXCLUSIVE_LOCK : 0),
                            0, 1, 0, &overlapped);
  return success                                  ? 0
         : ERROR_LOCK_VIOLATION == GetLastError() ? k_ERROR_LOCKING_CONFLICT
                                                  : -1;
}
...
// otherwise.
{
  ...
  typedef FilesystemUtil_WindowsImpUtil<u::WindowsInterfaceUtil> WindowsImpUtil;
...
int FilesystemUtil::visitPaths(
    const char *pattern, const bsl::function<void(const char *)> &visitor) {
  ...
  if (bsl::string::npos != dirName.find_first_of("*?")) {
    ...
    while (!leaves.empty()) {
      ...
      if (bsl::string::npos != leaves.back().find_first_of("*?")) {
        ...
        for (it = workingPaths.begin(); it != workingPaths.end(); ++it) {
          int rc = visitPaths(
              it->c_str(),
              bdlf::BindUtil::bind(&pushBackWrapper<bsl::vector<bsl::string>>,
    ...
    handle = FindFirstFileExW(widePattern.c_str(), FindExInfoStandard,
                              &findDataW, FindExSearchNameMatch, NULL,
    ...
    for (bool sts = true; sts; sts = FindNextFileW(handle, &findDataW)) {
      ...
      if (0 != PathUtil::appendIfValid(&fullNamePath, narrowName)) {
        ...
        BSLS_ASSERT_OPT(0 && "FindFirstFileW returned an absolute path.");
      }
...
int FilesystemUtil::visitTree(
    const bsl::string_view &root, const bsl::string_view &pattern,
    const bsl::function<void(const char *path)> &visitor, bool sortFlag) {
  bsl::string rootDir;
  ...
  (void)bdlde::CharConvertUtf16::utf8ToUtf16(&widePattern, fullPattern, 0, '-');
  FileDescriptor handle = FindFirstFileExW(
      widePattern.c_str(), FindExInfoStandard, &foundData,
      FindExSearchNameMatch, NULL, FIND_FIRST_EX_CASE_SENSITIVE);
  if (INVALID_HANDLE_VALUE != handle) {
    bslma::ManagedPtr<FileDescriptor> handleGuard(&handle, 0, &invokeFindClose);
    for (bool sts = true; sts; sts = FindNextFileW(handle, &foundData)) {
      ...
      (void)bdlde::CharConvertUtf16::utf16ToUtf8(&narrowLeafName, wfn, 0, '-');
  ...
  widePattern.clear();
  (void)bdlde::CharConvertUtf16::utf8ToUtf16(&widePattern, fullPattern, 0, '-');
  handle = FindFirstFileExW(widePattern.c_str(), FindExInfoStandard, &foundData,
...
FilesystemUtil::getAvailableSpace(FileDescriptor descriptor) {
  ...
  typedef LONG __stdcall NTQUERYVOLUMEINFORMATIONFILE(
      HANDLE FileHandle, IO_STATUS_BLOCK * IoStatusBlock, PVOID FileInformation,
  ...
  static NTQUERYVOLUMEINFORMATIONFILE *pNQVIF =
      hNtDll ? (NTQUERYVOLUMEINFORMATIONFILE *)GetProcAddress(
...
namespace bdls {
FilesystemUtil::FileDescriptor
FilesystemUtil::open(const char *path, FileOpenPolicy openPolicy,
                     FileIOPolicy ioPolicy, FileTruncatePolicy truncatePolicy) {
  BSLS_ASSERT(path);
  ...
  case e_CREATE: {
    ...
    extendedFlags = S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH;
  } break;
...
int FilesystemUtil::close(FileDescriptor descriptor) {
  ...
  return (EBADF == errno) ? int(k_BAD_FILE_DESCRIPTOR) : int(k_UNKNOWN_ERROR);
}

FilesystemUtil::Offset FilesystemUtil::seek(FileDescriptor descriptor,
                                            Offset offset, int whence) {
  switch (whence) {
#if defined(U_USE_UNIX_FILE_SYSTEM_INTERFACE)
  case e_SEEK_FROM_BEGINNING: {
...
int FilesystemUtil::map(FileDescriptor descriptor, void **address,
                        Offset offset, bsl::size_t size, int mode) {
  BSLS_ASSERT(address);
...
int FilesystemUtil::tryLock(FileDescriptor descriptor, bool lockWriteFlag) {
  int rc =
      localFcntlLock(descriptor, F_SETLK, lockWriteFlag ? F_WRLCK : F_RDLCK);
  return -1 != rc                             ? 0
...
int FilesystemUtil::lock(FileDescriptor descriptor, bool lockWriteFlag) {
  int rc =
      localFcntlLock(descriptor, F_SETLKW, lockWriteFlag ? F_WRLCK : F_RDLCK);
  return -1 != rc ? 0 : EINTR == errno ? k_ERROR_LOCKING_INTERRUPTED : -1;
}
...
int FilesystemUtil::getLastModificationTime(bdlt::Datetime *time,
  ...
  *time = bdlt::EpochUtil::epoch();
  ...
  u::InterfaceUtil::time_t seconds = u::InterfaceUtil::get_st_mtime(fileStats);
  ...
  BSLS_ASSERT_SAFE(
      (0 <= nanoseconds) &&
      (nanoseconds < bdlt::TimeUnitRatio::k_NANOSECONDS_PER_SECOND));
...
int FilesystemUtil::visitTree(
...

  {
    ...
    int rc = ::glob(fullPattern.c_str(), GLOB_NOSORT,
                    isNotFilePermissionsError_p, &pglob);
    bslma::ManagedPtr<glob_t> globGuard(&pglob, 0, &invokeGlobFree);
    bsl::size_t numFound;
    switch (rc) {
    case 0: {
    ...
    for (unsigned ii = 0; ii < numFound; ++ii) {
      ...
      if (!shortIsDotOrDots(basename)) {
        ...
        if (0 == ::performStat(fullPath, &fileStats, false) &&
            (S_ISREG(fileStats.st_mode) || S_ISDIR(fileStats.st_mode))) {
...
int FilesystemUtil::getSymbolicLinkTarget(
    std::experimental::pmr::string *result, const char *path) {
  return u_getSymbolicLinkTarget(result, path);
}
#endif
...
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR_STRING
int FilesystemUtil::getWorkingDirectory(std::experimental::pmr::string *path) {
  return u_getWorkingDirectory(path);
}
#endif
...
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR_STRING
int FilesystemUtil::getSystemTemporaryDirectory(
    std::experimental::pmr::string *path) {
  return u_getSystemTemporaryDirectory(path);
}
#endif
...
int FilesystemUtil::growFile(FileDescriptor descriptor,
                             FilesystemUtil::Offset size, bool reserveFlag,
...
int FilesystemUtil::rollFileChain(const bsl::string_view &path, int maxSuffix) {
  ...
  bsl::size_t length = static_cast<int>(path.length()) + MAX_SUFFIX_LENGTH + 2;
  ...
  if (!exists(toName) || 0 == remove(toName, true)) {
    ...
    *fromSuffixPos = '\0';
    if (0 == maxSuffix && exists(fromName) && 0 != move(fromName, toName)) {
      maxSuffix = -1;
    }
  }
...
FilesystemUtil::FileDescriptor
FilesystemUtil::createTemporaryFile(std::string *outPath,
                                    const bsl::string_view &prefix) {
  return u_createTemporaryFile(outPath, prefix);
}
...
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR_STRING
FilesystemUtil::FileDescriptor
FilesystemUtil::createTemporaryFile(std::experimental::pmr::string *outPath,
                                    const bsl::string_view &prefix) {
  return u_createTemporaryFile(outPath, prefix);
}
#endif
...
void FilesystemUtil::makeUnsafeTemporaryFilename(
    bsl::string *outPath, const bsl::string_view &prefix) {
  u_makeUnsafeTemporaryFilename(outPath, prefix);
}

void FilesystemUtil::makeUnsafeTemporaryFilename(
    std::string *outPath, const bsl::string_view &prefix) {
  u_makeUnsafeTemporaryFilename(outPath, prefix);
}

#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR_STRING
void FilesystemUtil::makeUnsafeTemporaryFilename(
    std::experimental::pmr::string *outPath, const bsl::string_view &prefix) {
  u_makeUnsafeTemporaryFilename(outPath, prefix);
}
#endif
...
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR_STRING
int FilesystemUtil::createTemporaryDirectory(
    std::experimental::pmr::string *outPath, const bsl::string_view &prefix) {
  return u_createTemporaryDirectory(outPath, prefix);
}
#endif
...
int FilesystemUtil::createTemporarySubdirectory(
    bsl::string *outPath, const bsl::string_view &rootDirectory,
    const bsl::string_view &prefix) {
  return u_createTemporarySubdirectory(outPath, rootDirectory, prefix);
}

int FilesystemUtil::createTemporarySubdirectory(
    std::string *outPath, const bsl::string_view &rootDirectory,
    const bsl::string_view &prefix) {
  return u_createTemporarySubdirectory(outPath, rootDirectory, prefix);
}
...
int FilesystemUtil::createTemporarySubdirectory(
    ...
    const bsl::string_view &rootDirectory, const bsl::string_view &prefix) {
  return u_createTemporarySubdirectory(outPath, rootDirectory, prefix);
}
#endif

int FilesystemUtil::mapChecked(FileDescriptor descriptor, void **address,
                               Offset offset, bsl::size_t size, int mode) {
  BSLS_ASSERT(address);
...
WindowsInterfaceUtil::FileTimeToSystemTime(const FILETIME *lpFileTime,
                                           LPSYSTEMTIME lpSystemTime) {
  return ::FileTimeToSystemTime(lpFileTime, lpSystemTime);
}

WindowsInterfaceUtil::DWORD
WindowsInterfaceUtil::GetFileSize(HANDLE hFile, LPDWORD lpFileSizeHigh) {
  return ::GetFileSize(hFile, lpFileSizeHigh);
}

WindowsInterfaceUtil::BOOL
WindowsInterfaceUtil::GetFileTime(HANDLE hFile, LPFILETIME lpCreationTime,
                                  LPFILETIME lpLastAccessTime,
                                  LPFILETIME lpLastWriteTime) {
...
WindowsInterfaceUtil::BOOL
WindowsInterfaceUtil::SystemTimeToFileTime(const SYSTEMTIME *lpSystemTime,
                                           LPFILETIME lpFileTime) {
