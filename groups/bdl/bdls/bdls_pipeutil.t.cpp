...
void testCase1_makeCanonicalName(const STRING_TYPE &typeName, int test,
                                 int verbose) {
  (void)verbose;
  (void)test;
...
// 'pipeName' does not start with R"(\\.\pipe\)".
{
  ...
  HANDLE handle =
      CreateFileW(wPipeName.data(), 0, 0, NULL, OPEN_EXISTING, 0, NULL);
  ...
  case 2: {
...
#ifdef BSLS_PLATFORM_OS_WINDOWS
    {
      ...
      for (int i = 1; i <= 2; i++) {
        ...
        char buffer[200];
        sprintf(buffer, "bdls_pipeutil.t#2.%d#%d", i,
                bdls::ProcessUtil::getProcessId());

        const DWORD serverOpenMode =
            (1 == i ? PIPE_ACCESS_INBOUND : PIPE_ACCESS_OUTBOUND);
        ...
        HANDLE handle =
            CreateNamedPipeW(wPipeName.c_str(), serverOpenMode,
                             PIPE_READMODE_BYTE, 1, 256, 256, 0, NULL);
    ...
    }
    {
      ...
      for (int i = 1; i <= 2; i++) {
        ...
        char buffer[200];
        sprintf(buffer, "bdls_pipeutil.t#2.3.%d#%d", i,
                bdls::ProcessUtil::getProcessId());

        const DWORD serverOpenMode =
            (i == 1 ? PIPE_ACCESS_INBOUND : PIPE_ACCESS_OUTBOUND);
        ...
        const DWORD desiredAccess = (i == 1 ? GENERIC_WRITE : GENERIC_READ);

        HANDLE clientHandle = CreateFileW(wPipeName.c_str(), desiredAccess, 0,
    ...
    }
    {
      // P-4
      string pipeName;
      bdls::PipeUtil::makeCanonicalName(&pipeName, "bdls_pipeutil.t#2.4");
    ...
    }
    {
      // P-5
      char buffer[200];
      const int retval = GetModuleFileNameA(NULL, buffer, sizeof(buffer));
      ASSERT(0 < retval && retval < sizeof(buffer));
      ASSERT(!bdls::PipeUtil::isOpenForReading(buffer));
    }
#else
    if (verbose)
      puts("Skipping test case 2 on Unix");
    (void)veryVerbose;
#endif
  } break;
  case 1: {
    ...
    testCase1_makeCanonicalName<bsl::string>("bsl::string", test, verbose);
    testCase1_makeCanonicalName<std::string>("std::string", test, verbose);
