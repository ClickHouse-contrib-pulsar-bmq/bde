...
struct Parameters {
  ...
  bool d_isValidRaw;
    ...
    {L_, "c:\\", "c:\\", 0, 0, 0, 0, 1},
    {L_, "c:\\\\", "c:\\\\", 0, 0, 0, 0, 1},
    {L_, "c:\\one", "c:\\", "one", "c:\\", 1, 0, 1},
    {L_, "c:\\one\\", "c:\\", "one", "c:\\", 1, 0, 0},
    {L_, "c:\\one\\two", "c:\\", "two", "c:\\one", 2, 0, 1},
    {L_, "c:\\one\\two\\", "c:\\", "two", "c:\\one", 2, 0, 0},
    {L_, "c:\\\\one", "c:\\\\", "one", "c:\\\\", 1, 0, 1},
    {L_, "c:\\\\one\\", "c:\\\\", "one", "c:\\\\", 1, 0, 0},
    {L_, "c:\\\\one\\two", "c:\\\\", "two", "c:\\\\one", 2, 0, 1},
    {L_, "c:\\\\one\\two\\", "c:\\\\", "two", "c:\\\\one", 2, 0, 0},
    ...
    {L_, "one", "", "one", "", 1, 1, 1},
    {L_, "one\\", "", "one", "", 1, 1, 0},
    {L_, "one\\two", "", "two", "one", 2, 1, 1},
    {L_, "one\\two\\", "", "two", "one", 2, 1, 0},
    {L_, "one\\two\\\\", "", "two", "one", 2, 1, 1},
    {L_, "c:one\\two\\\\", "c:", "two", "c:one", 2, 0, 0},
    {L_, "\\\\server\\dir", "\\\\server\\dir", 0, 0, 0, 0, 1},
    {L_, "\\\\server\\dir\\", "\\\\server\\dir\\", 0, 0, 0, 0, 1},
    {L_, "\\\\server\\dir\\\\", "\\\\server\\dir\\\\", 0, 0, 0, 0, 1},
    {L_, "\\\\server\\dir\\one", "\\\\server\\dir\\", "one",
     "\\\\server\\dir\\", 1, 0, 1},
    {L_, "\\\\server\\dir\\one\\", "\\\\server\\dir\\", "one",
     "\\\\server\\dir\\", 1, 0, 0},
    {L_, "\\\\server\\dir\\one\\two", "\\\\server\\dir\\", "two",
     "\\\\server\\dir\\one", 2, 0, 1},
    {L_, "\\\\.\\dir\\one\\two\\", "\\\\.\\dir\\", "two", "\\\\.\\dir\\one", 2,
     0, 0},
    {L_, "\\\\server\\dir\\\\one", "\\\\server\\dir\\\\", "one",
     "\\\\server\\dir\\\\", 1, 0, 1},
    {L_, "\\\\server\\dir\\\\one\\", "\\\\server\\dir\\\\", "one",
    ...
    {L_, "\\\\server\\dir\\\\one\\two", "\\\\server\\dir\\\\", "two",
     "\\\\server\\dir\\\\one", 2, 0, 1},
    {L_, "\\\\server\\dir\\\\one\\two\\", "\\\\server\\dir\\\\", "two",
     "\\\\server\\dir\\\\one", 2, 0, 0},
    {L_, "\\\\server\\dir\\one\\two\\\\", "\\\\server\\dir\\", "two",
     "\\\\server\\dir\\one", 2, 0, 0},
    {L_, "\\\\?\\UNC\\server\\dir\\one\\two\\\\", "\\\\?\\UNC\\server\\dir\\",
     "two", "\\\\?\\UNC\\server\\dir\\one", 2, 0, 0},
    {L_, "\\\\?\\c:one\\two\\\\", "\\\\?\\c:", "two", "\\\\?\\c:one", 2, 0, 0},
    {L_, "\\\\?\\c:\\one\\two\\\\", "\\\\?\\c:\\", "two", "\\\\?\\c:\\one", 2,
     0, 0},
    {L_, "/", "/", 0, 0, 0, 0, 0},
    {L_, "c:/", "c:/", 0, 0, 0, 0, 1},
    {L_, "c://", "c://", 0, 0, 0, 0, 1},
    {L_, "c:/one", "c:/", "one", "c:/", 1, 0, 1},
    {L_, "c:/one/", "c:/", "one", "c:/", 1, 0, 0},
    {L_, "c:/one/two", "c:/", "two", "c:/one", 2, 0, 1},
    {L_, "c:/one/two/", "c:/", "two", "c:/one", 2, 0, 0},
    {L_, "c://one", "c://", "one", "c://", 1, 0, 1},
    {L_, "c://one/", "c://", "one", "c://", 1, 0, 0},
    {L_, "c://one/two", "c://", "two", "c://one", 2, 0, 1},
    {L_, "c://one/two/", "c://", "two", "c://one", 2, 0, 0},
    {L_, "one/", "", "one", "", 1, 1, 0},
    {L_, "one/two", "", "two", "one", 2, 1, 1},
    {L_, "one/two/", "", "two", "one", 2, 1, 0},
    {L_, "one/two//", "", "two", "one", 2, 1, 1},
    {L_, "c:one/two//", "c:", "two", "c:one", 2, 0, 0},
    {L_, "\\\\server/dir", "\\\\server/dir", 0, 0, 0, 0, 1},
    {L_, "\\\\server/dir/", "\\\\server/dir/", 0, 0, 0, 0, 1},
    {L_, "\\\\server/dir//", "\\\\server/dir//", 0, 0, 0, 0, 1},
    {L_, "\\\\server/dir/one", "\\\\server/dir/", "one", "\\\\server/dir/", 1,
     0, 1},
    {L_, "\\\\server/dir/one/", "\\\\server/dir/", "one", "\\\\server/dir/", 1,
     0, 0},
    {L_, "\\\\server/dir/one/two", "\\\\server/dir/", "two",
     "\\\\server/dir/one", 2, 0, 1},
    {L_, "\\\\./dir/one/two/", "\\\\./dir/", "two", "\\\\./dir/one", 2, 0, 0},
    {L_, "\\\\server/dir//one", "\\\\server/dir//", "one", "\\\\server/dir//",
     1, 0, 1},
    {L_, "\\\\server/dir//one/", "\\\\server/dir//", "one", "\\\\server/dir//",
     1, 0, 0},
    {L_, "\\\\server/dir//one/two", "\\\\server/dir//", "two",
    ...
    {L_, "/one", "/", "one", "/", 1, 0, 1},
    {L_, "/one/", "/", "one", "/", 1, 0, 0},
    {L_, "/one/two", "/", "two", "/one", 2, 0, 1},
    {L_, "/one/two/", "/", "two", "/one", 2, 0, 0},
    {L_, "//one", "//", "one", "//", 1, 0, 1},
    {L_, "//one/", "//", "one", "//", 1, 0, 0},
    {L_, "//one/two", "//", "two", "//one", 2, 0, 1},
    {L_, "//one/two/", "//", "two", "//one", 2, 0, 0},
...
{
  bsl::string::size_type position = path->find('\\');
  for (; position != bsl::string::npos; position = path->find('\\', position)) {
    (*path)[position] = '/';
  }
}

template <class STRING_TYPE> void usageExample() {
  /// Usage
  ///-----
...
#endif
  //..
  // A relative path may be appended to any other path, even itself.  An
  // absolute path may not be appended to any path, or undefined behavior will
  // result:
  //..
...
void test_getExtension(int verbose, int veryVerbose, int veryVeryVerbose) {
  ...
  } DATA[] = {// 1. Relative and absolute paths
...
//-  ----------------------------------  ----  -----------------------  -----
      ...
      {L_, "\\\\?\\UNC\\serv/dir/", 17, "\\\\?\\UNC\\serv/dir/", ""},
      {L_, "\\\\?\\UNC\\serv/dir/one", 17, "\\\\?\\UNC\\serv/dir/", "one"},
      {L_, "\\\\?\\UNC\\serv/dir//one", 18, "\\\\?\\UNC\\serv/dir//", "one"},
      {L_, "\\\\?\\UNC\\serv/dir///one", 19, "\\\\?\\UNC\\serv/dir///", "one"},
  ...
  for (size_t ti = 0; ti < NUM_DATA; ++ti) {
    ...
    if (EXP_HEAD != head) {
      cout << __FILE__ << ":" << __LINE__ << "\n\thead    : \"" << head << "\""
           << "\n\tEXP_HEAD: \"" << EXP_HEAD << "\"" << endl;
    }

    ASSERTV(LINE, EXP_TAIL, tail, EXP_TAIL == tail);
    if (EXP_TAIL != tail) {
      cout << __FILE__ << ":" << __LINE__ << "\n\ttail    : \"" << tail << "\""
           << "\n\tEXP_TAIL: \"" << EXP_TAIL << "\"" << endl;
    }
...
void test_appendIfValid(int verbose, int veryVerbose, int veryVeryVerbose) {
...

  {
    ...
    for (int i = 0; i < NUM_VALUES; ++i) {
      ...
      LOOP_ASSERT(LINE, 0 == rc);
      LOOP4_ASSERT(LINE, originalPath, filename, path, expectedResult == path);
    }
  }
  if (verbose) {
...

  {
    ...
    for (int i = 0; i < NUM_VALUES; ++i) {
      ...
      for (size_t subStrLen = 0; subStrLen < pathLen; ++subStrLen) {
        for (size_t offset = 0; offset < pathLen - subStrLen + 1; ++offset) {
          STRING_TYPE path(VALUES[i]);
          bsl::string_view filename(path.c_str() + offset, subStrLen);
          ...
          expectedResult += STRING_TYPE(filename.data(), filename.length());
...
#endif
          STRING_TYPE originalPath(path);
          STRING_TYPE originalFilename(filename.data(), filename.length());
          ...
          ASSERT(0 == rc);
          LOOP4_ASSERT(originalPath, originalFilename, path, expectedResult,
                       expectedResult == path);
        }
      }
    }
...
void test_leaflessAppend(int verbose, int veryVerbose, int veryVeryVerbose) {
  ...
  const char *absolute[] = {
      ...
      "\\\\machine/dir/hello"
#else
      "/logs", "/logs/hello", "/", "/hello"
#endif
  };
  const char *relative[] = {
#ifdef BSLS_PLATFORM_OS_WINDOWS
      "logs", "logs\\hello", "", "hello", "logs", "logs/hello",
      "",     "hello"
#else
      "logs", "logs/hello", "", "hello"
#endif
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    LOOP3_ASSERT(arrays[i][LEAFLESS_ORIGINAL], arrays[i][LEAFLESS_EXPECTED],
                 leafless,
                 leafless == STRING_TYPE(arrays[i][LEAFLESS_EXPECTED]));
  }
...
void test_nativeParsingTest(int verbose, int veryVerbose, int veryVeryVerbose) {
  ...
  for (int i = 0; i < NUM_PARAMETERS; ++i) {
    ...
    for (int j = 0; j < NUM_PARAMETERS; ++j) {
      ...
      int referenceCount;
      if (pj.d_isRelative) {
        ASSERT(0 == bdls::PathUtil::appendIfValid(&iTest, pj.d_path));
...
int main(int argc, char *argv[]) {
  ...
  case 8: {
    ...
    usageExample<std::experimental::pmr::string>();
#endif
  } break;
  case 7: {
    // --------------------------------------------------------------------
...
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR_STRING
    test_getExtension<std::experimental::pmr::string>(verbose, veryVerbose,
                                                      veryVeryVerbose);
#endif
...
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR_STRING
    test_splitFilename<std::experimental::pmr::string>(verbose, veryVerbose,
                                                       veryVeryVerbose);
#endif
  } break;
  case 4: {
...
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR_STRING
    test_appendIfValid<std::experimental::pmr::string>(verbose, veryVerbose,
                                                       veryVeryVerbose);
#endif
  } break;
  case 3: {
  ...
  case 2: {
    ...
    test_leaflessAppend<bsl::string>(verbose, veryVerbose, veryVeryVerbose);
    test_leaflessAppend<std::string>(verbose, veryVerbose, veryVeryVerbose);
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR_STRING
    test_leaflessAppend<std::experimental::pmr::string>(verbose, veryVerbose,
                                                        veryVeryVerbose);
#endif
  } break;
  case 1: {
    ...
    test_nativeParsingTest<bsl::string>(verbose, veryVerbose, veryVeryVerbose);
    test_nativeParsingTest<std::string>(verbose, veryVerbose, veryVeryVerbose);
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR_STRING
    test_nativeParsingTest<std::experimental::pmr::string>(verbose, veryVerbose,
                                                           veryVeryVerbose);
#endif
  } break;
  default: {
