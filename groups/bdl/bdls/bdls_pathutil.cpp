...
// ACCESSOR
inline bool IsSeparator::operator()(char ch) const {
...
// LOCAL CONSTANTS
static const char k_separators[] =
#ifdef BSLS_PLATFORM_OS_WINDOWS
    "\\/"
#else
    "/"
...
static void findFirstNonSeparatorChar(size_t *resultOffset, const char *path,
                                      int length = -1)
...
#define UNCW_UNCPREFIXLEN (UNCW_PREFIXLEN + 4)
  ...
  if (2 <= length && bsl::isalpha(static_cast<unsigned char>(path[0])) &&
    ...
    *resultOffset = rootNameEnd = 2;
  } else if (4 <= length && isSeparator(path[0]) && isSeparator(path[1]) &&
             '?' != path[2]) {
    ...
    const char *rootNameEndPtr =
        bsl::find_if(path + 3, path + length, isSeparator);
    ...
    const char *resultOffsetPtr =
        bsl::find_if(path + rootNameEnd, path + length, isSeparator);
    ...
    *resultOffset = (size_t)(resultOffsetPtr - path);
    ...
    if (UNCW_PREFIXLEN + 2 <= length &&
      ...
      *resultOffset = rootNameEnd = UNCW_PREFIXLEN + 2;
      ...
      const char *rootNameEndPtr =
          bsl::find_if(path + UNCW_UNCPREFIXLEN, path + length, isSeparator);
...
static void findFirstNonSeparatorChar(int *result, const char *path,
                                      int length = -1)
...
static const char *leafDelimiter(const char *path, int rootEnd, int length = -1)
...
// ignored.
{
  ...
  for (position = path + length - 1;
       position > path + rootEnd && !isSeparator(*position); --position) {
  }
  return position;
}

template <class STR_TYPE>
static inline const char *leafDelimiter(const STR_TYPE &path, int rootEnd) {
  return leafDelimiter(path.c_str(), rootEnd, static_cast<int>(path.length()));
}
...
int u_appendIfValid(STR_TYPE *path, const bsl::string_view &filename) {
  ...
  findFirstNonSeparatorChar(&nonSeparatorOffset, filename.data(),
...
template <class STR_TYPE>
void u_appendRaw(STR_TYPE *path, const char *filename, int length,
                 int rootEnd) {
  BSLS_ASSERT(path);
  ...
  if (0 < length) {
    if (0 > rootEnd) {
      findFirstNonSeparatorChar(&rootEnd, path->c_str(),
...
template <class STR_TYPE> int u_popLeaf(STR_TYPE *path, int rootEnd) {
  BSLS_ASSERT(path);
...
int u_getLeaf(STR_TYPE *leaf, const bsl::string_view &path, int rootEnd) {
  ...
  leaf->append(isSeparator(*lastSeparator) ? lastSeparator + 1 : lastSeparator,
               path.data() + length);
  return 0;
}

template <class STR_TYPE>
int u_getExtension(STR_TYPE *extension, const bsl::string_view &path,
                   int rootEnd) {
...
int u_getDirname(STR_TYPE *dirname, const bsl::string_view &path, int rootEnd) {
  ...
  if (0 > rootEnd) {
    findFirstNonSeparatorChar(&rootEnd, path.data(),
                              static_cast<int>(path.length()));
  }
  ...
  const char *lastSeparator =
      leafDelimiter(path.data(), rootEnd, static_cast<int>(path.length()));
...
void PathUtil::appendRaw(bsl::string *path, const char *filename, int length,
                         int rootEnd) {
  u_appendRaw(path, filename, length, rootEnd);
}

void PathUtil::appendRaw(std::string *path, const char *filename, int length,
                         int rootEnd) {
  u_appendRaw(path, filename, length, rootEnd);
}
...
void PathUtil::appendRaw(std::experimental::pmr::string *path,
                         const char *filename, int length, int rootEnd) {
  u_appendRaw(path, filename, length, rootEnd);
}
#endif
...
int PathUtil::getLeaf(bsl::string *leaf, const bsl::string_view &path,
                      int rootEnd) {
  return u_getLeaf(leaf, path, rootEnd);
}

int PathUtil::getLeaf(std::string *leaf, const bsl::string_view &path,
                      int rootEnd) {
  return u_getLeaf(leaf, path, rootEnd);
}
...
int PathUtil::getExtension(bsl::string *extension, const bsl::string_view &path,
                           int rootEnd) {
  return u_getExtension(extension, path, rootEnd);
}

int PathUtil::getExtension(std::string *extension, const bsl::string_view &path,
                           int rootEnd) {
  return u_getExtension(extension, path, rootEnd);
}
...
int PathUtil::getDirname(bsl::string *dirname, const bsl::string_view &path,
                         int rootEnd) {
  return u_getDirname(dirname, path, rootEnd);
}

int PathUtil::getDirname(std::string *dirname, const bsl::string_view &path,
                         int rootEnd) {
  return u_getDirname(dirname, path, rootEnd);
}
...
int PathUtil::getRoot(bsl::string *root, const bsl::string_view &path,
                      int rootEnd) {
  return u_getRoot(root, path, rootEnd);
}

int PathUtil::getRoot(std::string *root, const bsl::string_view &path,
                      int rootEnd) {
  return u_getRoot(root, path, rootEnd);
}
...
void PathUtil::splitFilename(bsl::string_view *head, bsl::string_view *tail,
                             const bsl::string_view &path, int rootEnd) {
  BSLS_ASSERT(head);
  BSLS_ASSERT(tail);
  ...
  if (pathEnd - 1 != lastSeparator) {
    *tail = bsl::string_view(lastSeparator + 1, pathEnd - lastSeparator - 1);
  } else {
    *tail = default_sv;
  }
  ...
  // Head detection.
  *head = bsl::string_view(pathBegin, lastSeparator > pathBegin + rootEnd
...
int PathUtil::getRootEnd(const bsl::string_view &path) {
  int result;
  findFirstNonSeparatorChar(&result, path.data(),
                            static_cast<int>(path.length()));
  return result;
}
