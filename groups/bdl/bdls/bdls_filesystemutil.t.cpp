...
// [22] createTemporaryFile(std::experimental::pmr::::string *, const
// bsl::string_view&) [23] createTemporaryDirectory(bsl::string *, const
// string_view&) [23] createTemporaryDirectory(std::string *, const
// string_view&) [23] createTemporaryDirectory(std::experimental::pmr::::string
// *, const string_view&) [24] int createDirectories(const string&, bool); [24]
// int createPrivateDirectory(const string&); [25] int visitTree(const char *,
// const string&, const Func&, bool); [25] int visitTree(const string&, const
// string&, const Func&, bool); [25] int visitPaths(const string&, const Func&);
...
// doesn't exist.
{
  Obj::FileDescriptor fd =
      Obj::open(fileName, Obj::e_OPEN_OR_CREATE, Obj::e_READ_WRITE);
...
static void localForkExec(bsl::string command) {
#ifdef BSLS_PLATFORM_OS_UNIX
  if (0 == fork()) {
    // child process
...
inline void makeArbitraryFile(const char *path) {
  Obj::FileDescriptor fd =
      Obj::open(path, Obj::e_OPEN_OR_CREATE, Obj::e_READ_WRITE);
  ASSERT(Obj::k_INVALID_FD != fd);
...
struct VisitTreeTestVisitor {
  ...
  void operator()(const char *filePath) { d_vec->push_back(filePath); }
};
...
public:
  // CREATOR
  MMIXRand() : d_reg(0) { memset(d_outBuffer, 0, sizeof(d_outBuffer)); }
...
struct TestUtil_UnixImpUtil {
  ...
  static bool
  epochOffsetIsConvertibleToDatetime(time_t seconds,
...
private:
  ...
  createTemporaryFile(bsl::string *path,
...
bool TestUtil::isBigtimeSupportAvailable(TestUtil::FileDescriptor fd) {
...
TestUtil::Offset TestUtil::estimateNumBlocks(const char *path) {
  ...
  const BOOL getDiskFreeSpaceStatus = ::GetDiskFreeSpaceW(
      volumePathName, &ignoredSectorsPerCluster, &bytesPerSector,
      &ignoredNumberOfFreeClusters, &ignoredTotalNumberOfClusters);
  ...
  const ULONG64 ul64FileSize = (ul64FileSizeHigh32 << 32) | ul64FileSizeLow32;
...
TestUtil::Offset TestUtil::estimateNumBlocks(FileDescriptor descriptor) {
  ...
  const BOOL getFileInformationStatus = ::GetFileInformationByHandleEx(
      descriptor, FileStandardInfo, &standardFileInfo,
...
bdlt::Datetime TestUtil::getMinFileTime() { return bdlt::Datetime(1, 1, 1); }
...
int TestUtil::setFileSize(FileDescriptor descriptor, Offset numBytes) {
  ...
  const LONG64 l64NewFilePosition = static_cast<LONG64>(
...
int TestUtil::setLastModificationTime(FileDescriptor descriptor,
  ...
  const BOOL setFileTimeStatus = ::SetFileTime(
      descriptor, s_DO_NOT_UPDATE_CREATION_TIME, s_DO_NOT_UPDATE_ACCESS_TIME,
...
// CLASS METHODS
bdlt::Datetime
TestUtil_UnixImpUtil::convertEpochOffsetToDatetime(time_t seconds,
                                                   long nanoseconds) {
  BSLS_ASSERT(epochOffsetIsConvertibleToDatetime(seconds, nanoseconds));

  const bsls::TimeInterval maxTimeSinceEpoch(
      static_cast<bsls::Types::Int64>(seconds), static_cast<int>(nanoseconds));
...
bool TestUtil_UnixImpUtil::epochOffsetIsConvertibleToDatetime(
    time_t seconds, long nanoseconds) BSLS_KEYWORD_NOEXCEPT {
...
int TestUtil_UnixImpUtil::setLastModificationTime(
    int fileDescriptor, const bdlt::Datetime &utcTime) {
...
TestUtil_WindowsImpUtil::createTemporaryFile(
    bsl::string *path, TempFileCloseBehavior::Enum closeBehavior) {
  bsl::string pathValue;
  int rc = getTemporaryFilePath(&pathValue);
  if (0 != rc) {
    return k_INVALID_FD; // RETURN
  ...
  const HANDLE fileDescriptor = ::CreateFileW(
      utf16Path.c_str(), GENERIC_READ | GENERIC_WRITE, k_NOT_SHARED,
      k_NO_SECURITY_ATTRIBUTES, CREATE_NEW,
      FILE_ATTRIBUTE_TEMPORARY | deleteOnCloseFlag, k_NO_TEMPLATE_FILE);
...
TestUtil_WindowsImpUtil::createEphemeralFile() {
  bsl::string path;
  return createTemporaryFile(&path, TempFileCloseBehavior::e_REMOVE_ON_CLOSE);
}
...
int FileDescriptorCloseGuard::closeAndRelease() {
  const bdls::FilesystemUtil::FileDescriptor fileDescriptor = d_fileDescriptor;
...
//..
void getFilesWithinTimeframe(bsl::vector<bsl::string> *vector, const char *item,
                             const bdlt::Datetime &start,
                             const bdlt::Datetime &end) {
  bdlt::Datetime datetime;
  int ret = bdls::FilesystemUtil::getLastModificationTime(&datetime, item);
...
void findMatchingFilesInTimeframe(bsl::vector<bsl::string> *result,
  ...
  bdls::FilesystemUtil::visitPaths(
      pattern, bdlf::BindUtil::bind(&getFilesWithinTimeframe, result,
                                    bdlf::PlaceHolders::_1, start, end));
}
//..
...
#define SC "/"
  ...
  if (verbose)
    cout << "\t\tTesting basic behavior." << endl;
  {
    for (int i = 0; i < NUM_PATHS; ++i) {
      ...
      for (int j = 0; j < NUM_PREFIXES; ++j) {
        ...
        int rc = Obj::createTemporarySubdirectory(&resultPath, ROOT, PREFIX);
        ...
        if (0 == EXP_RESULT) {
          ...
          ASSERTV(TYPE, ROOT_LINE, PREFIX_LINE, expectedPath,
                  resultPath.c_str(),
                  0 == bsl::strncmp(expectedPath.c_str(), resultPath.c_str(),
                                    expectedPath.length()));
          ...
          if ((verbose && (S_IRUSR | S_IWUSR | S_IXUSR) != info.st_mode) ||
              veryVerbose) {
            cout << "\t\tTemp file permissions 0" << bsl::oct
                 << static_cast<unsigned>(info.st_mode) << endl;
          }
#endif
          ...
          STRING_TYPE temp;
          rc = Obj::createTemporarySubdirectory(&temp, resultPath.c_str(), "");

          ASSERTV(TYPE, ROOT_LINE, PREFIX_LINE, temp.c_str(), rc, 0 == rc);
    ...
    cout << "\n\t\tNegative Testing." << endl;
  {
    ...
    ASSERT_PASS(Obj::createTemporarySubdirectory(&resultPath, root, prefix));
    ASSERT(0 == Obj::remove(resultPath));
...
template <class STRING_TYPE>
void testCase23_createTemporaryDirectory(const char *typeName, int test,
...
template <class STRING_TYPE>
void testCase22_createTemporaryFile(const char *typeName, int test,
                                    const bsl::string &tmpWorkingDir,
                                    int verbose, int veryVerbose,
                                    int veryVeryVerbose) {
  (void)verbose;
  ...
  if (fd != Obj::k_INVALID_FD) {
    ...
    Obj::FileDescriptor fd2 = Obj::createTemporaryFile(&fileName2, prefix);
    if (veryVerbose) {
      cout << ":" << fileName2 << ":\n";
    }
...
template <class STRING_TYPE>
void testCase21_makeUnsafeTemporaryFilename(const char *typeName, int test,
                                            const bsl::string &tmpWorkingDir,
                                            int verbose, int veryVerbose,
                                            int veryVeryVerbose) {
  (void)verbose;
  ...
  for (unsigned ii = 0; ii < k_NUM_NAMES; ++ii) {
    ...
    ASSERT(!bsl::strncmp(name.c_str(), prefix.c_str(), prefix.length()));
    ...
    for (unsigned jj = 0; jj < ii; ++jj) {
      ...
      const bsl::size_t minLen = bsl::min(name.length(), otherName.length());

      // 'numRandChars' is the number of random characters that both
      // names have.

      const int numRandChars = static_cast<int>(minLen - prefix.length());

      // Names created adjacently in sequence must differ in at least
      // 'numRandChars / 2' characters, all names must differ in at
      // least 2 characters.

      const int minNumCharDiffs = otherNameWasPrevious ? numRandChars / 2 : 2;
      ...
      for (bsl::size_t kk = prefix.length(); kk < minLen; ++kk) {
        ...
        const int absCharDiff = charDiff < 0 ? -charDiff : charDiff;
...
void testCase10_getFileSize(const char *typeName,
                            const bsl::string &tmpWorkingDir, int test,
...

  {
    ...
    bsl::string relFileNameStr = tmpWorkingDir + PS + bsl::string(fileName);
  ...
  {
    bsl::string fileName;
    const Obj::FileDescriptor fd = u::TestUtil::createTemporaryFile(&fileName);
    ...
    const Obj::Offset fileNumBlocks = u::TestUtil::estimateNumBlocks(fileName);
...

  {
    ...
    const int setFileSizeRc =
        u::TestUtil::setFileSize(fd, 2 * k_LARGEST_EXPECTED_BLOCK_SIZE);
    if (0 != setFileSizeRc) {
      return; // RETURN
    }
...

  {
    ...
    static const Obj::Offset k_4_GB_FILE_SIZE = 4LL * 1024LL * 1024LL * 1024LL;

    const int setFileSizeRc = u::TestUtil::setFileSize(fd, k_4_GB_FILE_SIZE);
    if (0 != setFileSizeRc) {
      return; // RETURN
    }
...

  {
    ...
    const struct {
      ...
      // the size of file to make and then measure
        ...
        // LINE   FILE SIZE
        // ---- ---------------
        {L_, 0LL},           {L_, 1LL},           {L_, 1023LL},
        {L_, 1024LL},        {L_, 1025LL},        {L_, 8191LL},
        {L_, 8192LL},        {L_, 8193LL},        {L_, 0xFFFFFFFELL},
        {L_, 0xFFFFFFFFLL},  {L_, 0x100000000LL}, {L_, 0x100000001LL},
        {L_, 0x1FFFFFFFFLL}, {L_, 0x200000000LL}, {L_, 0x200000001LL},
    };
    ...
    for (int i = 0; i != NUM_DATA; ++i) {
      ...
      const int setFileSizeRc = u::TestUtil::setFileSize(fd, FILE_SIZE);
...
template <class STRING_TYPE>
void testCase9_createSystemTemporaryDirectory(const char *typeName,
                                              const bsl::string &tmpWorkingDir,
                                              int verbose, int veryVerbose,
                                              int veryVeryVerbose) {
  (void)verbose;
...
void testCase6_move_normal(const char *oldName, const char *newName,
                           bool expectOldCreateFailure, bool expectMoveFailure,
                           int verbose, int veryVerbose, int veryVeryVerbose) {
  (void)verbose;
  (void)veryVerbose;
  (void)veryVeryVerbose;
  ...
  Obj::FileDescriptor fd =
      Obj::open(oldName, Obj::e_OPEN_OR_CREATE, Obj::e_READ_WRITE);
  bool createFailed = (Obj::k_INVALID_FD == fd);

  if (createFailed) {
    if (!expectOldCreateFailure) {
      ASSERTV(oldName, createFailed && "Did not expect file creation to fail");
    }
...
void testCase6_move_template(const OLD_STRING_TYPE &oldName,
                             ...
                             bool expectMoveFailure, int verbose,
...
template <class STRING_TYPE>
void testCase5_isRegularFile_isDirectory(const char *typeName, int test,
                                         const bsl::string &tmpWorkingDir,
                                         int verbose, int veryVerbose,
                                         int veryVeryVerbose) {
  (void)verbose;
...
#define CASE5_TMPDIR "tmp.filesystemutil.case5"
  ...
  struct ParametersByType {
    ...
    Parameters directory;
      ...
      {CASE5_TMPDIR PS "file", CASE5_TMPDIR PS "file2", CASE5_TMPDIR PS "dir"},
      {CASE5_TMPDIR PS "dir", CASE5_TMPDIR PS "dir2", CASE5_TMPDIR PS "file"}};
...
#ifndef BSLS_PLATFORM_OS_WINDOWS // (unix domain socket)
  {
    ...
    const int ADDR_LEN =
        (int)(sizeof(address.sun_family) + filename.size() + 1);
...
void testCase4_findMatchingPaths(const char *vectorTypeName,
                                 const char *stringTypeName, int test,
                                 const bsl::string &tmpWorkingDir, int verbose,
                                 int veryVerbose, int veryVeryVerbose) {
  (void)verbose;
  (void)veryVerbose;
  (void)veryVeryVerbose;
  ...
  if (verbose)
    cout << "\n\t+++++++++++++++ Testing " << vectorTypeName << " / "
         << stringTypeName << endl;

  const char *filenames[] = {
      "abcd", "zyx", "zy.z", "abc.zzz", "abc.def",
  };
...
#define PATHQ "bdls_filesystemutil.temp.3.futc?"
  ...
  struct Parameters {
    ...
    const char *result;
      ...
      {L_, PATH "/*/*foo*", ""},
      {L_, PATH "/*/*d*",
       PATH "/alpha/abc.def:" PATH "/alpha/abcd:" PATH "/beta/abc.def:" PATH
            "/beta/abcd"},
      ...
      {L_, PATHQ "/*b*/*.*",
       PATH "/beta/abc.def:" PATH "/beta/abc.zzz:" PATH "/beta/abcd:" PATH
            "/beta/zy.z:" PATH "/beta/zyx"},
      {L_, PATH "*/*/*.?",
       PATH "/alpha/abcd:" PATH "/alpha/zy.z:" PATH "/alpha/zyx:" PATH
            "/beta/abcd:" PATH "/beta/zy.z:" PATH "/beta/zyx"}
#else
      {L_, PATH "/*b*/*.?", PATH "/beta/zy.z"},
      {L_, PATHQ "/*b*/*.?", PATH "/beta/zy.z"},
      {L_, PATHQ "/*/abcd.*", ""},
      {L_, PATHQ "/*b*/*.*",
       PATH "/beta/abc.def:" PATH "/beta/abc.zzz:" PATH "/beta/zy.z"},
...
#undef PATHQ
  ...
  for (int i = 0; i < numFiles; ++i) {
    ...
    Obj::FileDescriptor f = Obj::open(path, Obj::e_CREATE, Obj::e_READ_WRITE);
    ...
    bsl::string rollup = ::rollupPaths<VECTOR_TYPE, bsl::string>(lookup);
#ifdef BSLS_PLATFORM_OS_WINDOWS
  ...
  for (int i = 0; i < NUM_PARAMETERS; ++i) {
...
#ifdef BSLS_PLATFORM_OS_WINDOWS
    replace_if(patternStr.begin(), patternStr.end(), ::isForwardSlash, *PS);
#endif
    ...
    bsl::string rollup = ::rollupPaths<VECTOR_TYPE, bsl::string>(resultPaths);
...
template <class STRING_TYPE>
void testCase2_CStringUtil(const char *stringTypeName, const STRING_TYPE &str,
                           const char *strPayload, int test, int verbose,
                           int veryVerbose, int veryVeryVerbose) {
  (void)verbose;
  (void)veryVerbose;
  (void)veryVeryVerbose;
...
void testCase2_CStringUtil_StringRef(const char *stringTypeName,
                                     ...
                                     const char *strPayload, int test,
                                     int verbose, int veryVerbose,
                                     int veryVeryVerbose) {
  (void)verbose;
  (void)veryVerbose;
...
int main(int argc, char *argv[]) {
  ...
  const bsl::string taskAbsolutePath =
      PS[0] == argv[0][0] || bsl::strstr(argv[0], ":\\") ||
              bsl::strstr(argv[0], ":/")
          ? bsl::string(argv[0])
  ...
  case 32: {
    ...
    for (int i = 0; i < TESTSIZE; ++i) {
      ...
      Obj::FileDescriptor fd =
          Obj::open(s.str(), Obj::e_OPEN_OR_CREATE, Obj::e_READ_WRITE);
      ASSERT(Obj::k_INVALID_FD != fd);
    ...
    const int START = 3;
    const int END = 6;
    UsageExample2::findMatchingFilesInTimeframe(&results, pattern.c_str(),
                                                modTime[START], modTime[END]);
  ...
  case 31: {
...
#endif
    //..
    // Suppose that we want to separate files into "old" and "new"
    // subdirectories on the basis of modification time.  We will provide paths
    // representing these locations, and create the directories if they do not
    // exist:
    //..
    ...
    ASSERT(0 == rc);
    //..
    // We know that all of our log files match the pattern "*.log", so let's
    // search for all such files in the log directory:
    //..
    bdls::PathUtil::appendRaw(&logPath, "*.log");
    bsl::vector<bsl::string> logFiles;
    Obj::findMatchingPaths(&logFiles, logPath.c_str());
    //..
    // Now for each of these files, we will get the modification time.  Files
    // that are older than 2 days will be moved to "old", and the rest will be
    // moved to "new":
    //..
    ...
    for (bsl::vector<bsl::string>::iterator it = logFiles.begin();
         it != logFiles.end(); ++it) {
      ASSERT(0 == Obj::getLastModificationTime(&modTime, *it));
      ASSERT(0 == bdls::PathUtil::getLeaf(&fileName, *it));
      bsl::string *whichDirectory =
          2 < (bdlt::CurrentTime::utc() - modTime).totalDays() ? &oldPath
                                                               : &newPath;
      bdls::PathUtil::appendRaw(whichDirectory, fileName.c_str());
      ASSERT(0 == Obj::move(it->c_str(), whichDirectory->c_str()));
  ...
  case 30: {
    ...
    testCase30_createTemporarySubdirectory<bsl::string>(verbose, veryVerbose);
    testCase30_createTemporarySubdirectory<std::string>(verbose, veryVerbose);
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR_STRING
    testCase30_createTemporarySubdirectory<std::experimental::pmr::string>(
        verbose, veryVerbose);
#endif
  } break;
  case 29: {
  ...
  case 28: {
...

    {
      ...
      // the test.
      {
        const Obj::FileDescriptor fd0 = u::TestUtil::createEphemeralFile();
      ...
      const bdlt::DatetimeInterval skew3 = endDatetime - lastModificationTime2;
      ...
      ASSERT_GT(clockFileCheck, skew1.fractionalDayInMicroseconds());
      ASSERT_GT(fileFileCheck, skew2.fractionalDayInMicroseconds());
      ASSERT_GT(clockFileCheck, skew3.fractionalDayInMicroseconds());
    }
    ...
    const bdlt::DatetimeInterval MOD_TIME_PRECISION(0, 0, 0, 0, 0, 1);
    ...
    bool haveBigtimeSupport = true;
    {
      ...
      ASSERT(Obj::k_INVALID_FD != fd);
      const u::FileDescriptorCloseGuard closeGuard(fd);
      haveBigtimeSupport = u::TestUtil::isBigtimeSupportAvailable(fd);
    }
    ...
    for (int i = 0; i != NUM_DATA; ++i) {
      ...
      if (!haveBigtimeSupport && MOD_TIME > max32BitUnixTimeAsDatetime) {
        continue;
      }
      ...
      if (!u::TestUtil::isValidModificationTime(MOD_TIME)) {
        ...
        LOOP_ASSERT_EQ(LINE, u::TestUtil::getMinFileTime(),
                       min32BitUnixTimeAsDatetime);
        LOOP_ASSERT_EQ(LINE, u::TestUtil::getMaxFileTime(),
                       max32BitUnixTimeAsDatetime);
        LOOP_ASSERT(LINE, (min32BitUnixTimeAsDatetime > MOD_TIME) ||
...

      {
        ...
        setModTimeStatus = u::TestUtil::setLastModificationTime(fd, MOD_TIME);
...
#undef BADTEST
  ...
  case 26: {
    ...
    if (verbose)
      cout << "TESTING GETFILESIZE(FD), MAP/MAPCHECKED/UNMAP\n"
              "=============================================\n";
    ...
    if (verbose)
      cout << "Testing page size\n";
    const int k_PAGE_SIZE = bdls::MemoryUtil::pageSize();
    ASSERT(1 == bdlb::BitUtil::numBitsSet(static_cast<uint32_t>(k_PAGE_SIZE)));
    if (verbose)
      P(k_PAGE_SIZE);
    ...
    for (int ti = 0; ti < 2; ++ti) {
      ...
      void *addressA;
      rc = checking ? Obj::mapChecked(fd, &addressA, 0, k_PAGE_SIZE, mapMode)
      ...
      off = Obj::seek(fd, 8 * k_PAGE_SIZE, Obj::e_SEEK_FROM_BEGINNING);
      bsl::memset(buffer, 0x2a, k_PAGE_SIZE);
      rc = Obj::write(fd, buffer, k_PAGE_SIZE);
      ASSERT(k_PAGE_SIZE == rc);

      const Obj::Offset endOff = Obj::seek(fd, 0, Obj::e_SEEK_FROM_CURRENT);
      ...
      preMapOff = off;
      rc = checking ? Obj::mapChecked(fd, &addressB, off4, k_PAGE_SIZE, mapMode)
      ...
      preMapOff = Obj::seek(fd, 6 * k_PAGE_SIZE, Obj::e_SEEK_FROM_BEGINNING);
      ...
      addressB = 0;
      rc = checking ? Obj::mapChecked(fd, &addressB, 3 * k_PAGE_SIZE,
                                      k_PAGE_SIZE / 8, mapMode)
                    : Obj::map(fd, &addressB, 3 * k_PAGE_SIZE, k_PAGE_SIZE / 8,
                               mapMode);
      ASSERT(0 == rc);
      ASSERT(0 != addressB);
      ...
      if (verbose)
        cout << "Failure/Negative testing " << (checking ? "with" : "without")
             << " checking.\n";
      ...
      if (checking) {
        ...
        rc = Obj::mapChecked(fd, &addressC, fSize - k_PAGE_SIZE,
                             2 * k_PAGE_SIZE, mapMode);
        ...
        rc = Obj::mapChecked(fd, &addressC, 3 * k_PAGE_SIZE, -k_PAGE_SIZE,
                             mapMode);
        ASSERTV(rc, Obj::k_ERROR_PAST_EOF == rc);
        ASSERT(!addressC);
        addressC = 0;
        ...
        ASSERT_OPT_FAIL(rc = Obj::mapChecked(fd, &addressC, k_PAGE_SIZE / 2,
                                             k_PAGE_SIZE, mapMode));
        if (verbose) {
          Q_(bad offset);
          P_(checking);
        ...
        ASSERT_FAIL(rc = Obj::mapChecked(fd, &addressC, -k_PAGE_SIZE,
                                         k_PAGE_SIZE, mapMode));
        ASSERT(!addressC);
        addressC = 0;
        ASSERT(57 == rc);
        ...
        ASSERT_FAIL(
            rc = Obj::mapChecked(fd, &addressC, k_PAGE_SIZE, 0, mapMode));
        ASSERT(!addressC);
        addressC = 0;
        ASSERT(57 == rc);
        rc = 57;
        ...
        ASSERT_FAIL(rc = Obj::mapChecked(fd, &addressC, 2 * k_PAGE_SIZE,
                                         k_PAGE_SIZE, 32 | mapMode));
        ASSERT(!addressC);
        addressC = 0;
        ASSERT(57 == rc);
      } else {
        ...
        ASSERT_OPT_PASS(rc = Obj::map(fd, &addressC, fSize - k_PAGE_SIZE,
                                      2 * k_PAGE_SIZE, mapMode));
        ASSERT(0 == rc);
        ASSERT(0 != addressC);
      ...
      const Obj::Offset fileSize = Obj::getFileSize(fd);
      if (checking || e_UNIX) {
        ASSERTV(9 * k_PAGE_SIZE, fileSize, 9 * k_PAGE_SIZE == fileSize);
      } else {
        // On Windows, mapping past the end of file grows the file.

        ASSERTV(10 * k_PAGE_SIZE, fileSize, 10 * k_PAGE_SIZE == fileSize);
      }
      ...
      for (char *pc = buffer, *end = buffer + k_PAGE_SIZE / 8; pc < end; ++pc) {
      ...
      bsl::size_t fileSize_t = static_cast<bsl::size_t>(Obj::getFileSize(fd));
      ...
      void *addressD = 0;
      rc = checking ? Obj::mapChecked(fd, &addressD, 0, fileSize_t, mapMode)
                    : Obj::map(fd, &addressD, 0, fileSize_t, mapMode);
      ASSERT(0 != rc);
  ...
  case 25: {
    ...
    for (int flags = 0; flags < 4; ++flags) {
      ...
      rc = stringFlag ? Obj::visitTree(rootStr, "meow*", meowVisitor, sortFlag)
  ...
  case 24: {
    ...
    if (verbose)
      cout << "TESTING: Specific error codes for 'createDirectories' et al.\n"
              "============================================================\n";

    if (verbose)
      cout << "Testing 'createDirectories'\n";
    {
    ...
    if (verbose)
      cout << "Testing 'createPrivateDirectory'\n";
    {
      const bsl::string &testBaseDir = "tmpDir";
  ...
  case 23: {
    ...
    //   createTemporaryDirectory(std::experimental::pmr::::string *, const
    //   string_view&)
    // --------------------------------------------------------------------
    ...
    testCase23_createTemporaryDirectory<bsl::string>(
        "bsl::string", test, tmpWorkingDir, verbose, veryVerbose,
        veryVeryVerbose);
    testCase23_createTemporaryDirectory<std::string>(
        "std::string", test, tmpWorkingDir, verbose, veryVerbose,
        veryVeryVerbose);
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR_STRING
    testCase23_createTemporaryDirectory<std::experimental::pmr::string>(
        "std::experimental::pmr::string", test, tmpWorkingDir, verbose,
  ...
  case 22: {
    ...
    //   createTemporaryFile(std::experimental::pmr::::string *, const
    //   bsl::string_view&)
    // --------------------------------------------------------------------
    ...
    testCase22_createTemporaryFile<bsl::string>("bsl::string", test,
                                                tmpWorkingDir, verbose,
                                                veryVerbose, veryVeryVerbose);
    testCase22_createTemporaryFile<std::string>("std::string", test,
                                                tmpWorkingDir, verbose,
                                                veryVerbose, veryVeryVerbose);
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR_STRING
    testCase22_createTemporaryFile<std::experimental::pmr::string>(
        "std::experimental::pmr::string", test, tmpWorkingDir, verbose,
        veryVerbose, veryVeryVerbose);
#endif
  } break;
  case 21: {
    ...
    testCase21_makeUnsafeTemporaryFilename<bsl::string>(
        "bsl::string", test, tmpWorkingDir, verbose, veryVerbose,
        veryVeryVerbose);
    testCase21_makeUnsafeTemporaryFilename<std::string>(
        "std::string", test, tmpWorkingDir, verbose, veryVerbose,
        veryVeryVerbose);
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR_STRING
  ...
  case 20: {
    ...
    LOOP_ASSERT(results.size(), NUM_FILES + NUM_VALID_NAMES == results.size());
    ...
    for (size_t i = 0; i < NUM_FILES; ++i) {
      ...
      const bsl::wstring path = L"bdls_filesystemutil.temp.18\\logs20\\" + name;
    ...
    for (FileSet::const_iterator itr = fileset.begin(), end = fileset.end();
         itr != end; ++itr) {
  ...
  case 19: {
    ...
    if (verbose)
      cout << "TESTING: Unix File Permissions for 'createDirectories' et al\n"
              "===========================================================\n";

#ifdef BSLS_PLATFORM_OS_WINDOWS
    if (verbose)
      cout << "TEST SKIPPED ON WINDOWS\n";
#else
    umask(0);
    ...
    if (verbose)
      cout << "Testing 'createDirectories'\n";
    {
      const bsl::string &testBaseDir =
          ::tempFileName(test, "tmp.bdls_filesystemutil_17.mkdir1");
    ...
    defined(BDLS_FILESYSTEMUTIL_UNIXPLATFORM_32_BIT_OFF)
      ...
      enum {
        EXPECTED_PERMS = S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP | S_IWGRP |
  ...
  case 18: {
    ...
    if (verbose)
      cout << "TESTING: Unix File Permissions for 'open'\n"
              "=========================================\n";
      ...
      cout << "Testing 'open'\n";
    {
      ...
      const bsl::string &testFile =
          ::tempFileName(test, "tmp.bdls_filesystemutil_16.open.txt");
      if (veryVerbose)
        P(testFile);

      (void)Obj::remove(testFile, false);

      FD fd = Obj::open(testFile, Obj::e_OPEN_OR_CREATE, Obj::e_READ_WRITE);
      ASSERT(Obj::k_INVALID_FD != fd);
...
#endif
      info.st_mode &= 0777;
      const bool eq = (S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH |
    ...
    if (verbose)
      cout << "Testing private 'open'\n";
    {
      typedef Obj::FileDescriptor FD;

      const bsl::string &testFile =
          ::tempFileName(test, "tmp.bdls_filesystemutil_16_pv.open.txt");
      if (veryVerbose)
        P(testFile);

      (void)Obj::remove(testFile, false);

      FD fd = Obj::open(testFile, Obj::e_CREATE_PRIVATE, Obj::e_READ_WRITE);
      ASSERT(Obj::k_INVALID_FD != fd);
  ...
  case 17: {
    ...
    if (isParent) {
      ...
      FD fd = Obj::open(testFile, Obj::e_OPEN_OR_CREATE,
      ...
      FD fd2 = Obj::open(testFile, Obj::e_OPEN, Obj::e_READ_WRITE);
      ASSERT(Obj::k_INVALID_FD != fd2);
      ...
      while (!Obj::exists(tag1) && (t = bdlt::CurrentTime::now()) < timeout) {
        if (veryVerbose)
          Q(Parent sleeping);
        ::localSleep(1);
      }
      ...
      if (verbose && Obj::exists(tag1))
        Q(Parent detected tag1);
      ...
      FD fdChild = Obj::open(testFile, Obj::e_OPEN, Obj::e_READ_WRITE);
  ...
  case 15: {
    ...
    const int READ_WRITE =
        bdls::MemoryUtil::k_ACCESS_READ | bdls::MemoryUtil::k_ACCESS_WRITE;
    int rc = 0;
    Obj::Offset offset = 0;
    ...
    FD writeFd =
        Obj::open(testFileName, Obj::e_OPEN_OR_CREATE, Obj::e_READ_WRITE);
    FD readFd = Obj::open(testFileName, Obj::e_OPEN, Obj::e_READ_ONLY);
  ...
  case 14: {
    ...
    if (isParent) {
      if (verbose)
        cout << "TRYLOCK TEST -- PARENT STARTING\n"
                "===============================\n";
      ...
      fdUnlocked =
          Obj::open(unlockedFn, Obj::e_OPEN_OR_CREATE, Obj::e_READ_WRITE);
      ASSERT(Obj::k_INVALID_FD != fdUnlocked);

      fdWrite =
          Obj::open(writeLockedFn, Obj::e_OPEN_OR_CREATE, Obj::e_READ_WRITE);
      ...
      ASSERT(Obj::exists(touchWhenDone) && "timed out after two minutes");
      ...
      if (verbose)
        cout << "TRYLOCK TEST -- PARENT FINISHED\n"
                "===============================\n";
      ...
      if (verbose)
        cout << "TRYLOCK TEST -- CHILD STARTING\n"
                "==============================\n";
      ...
      fdUnlocked = Obj::open(unlockedFn, Obj::e_OPEN, Obj::e_READ_WRITE);
      ASSERT(Obj::k_INVALID_FD != fdUnlocked);

      // Open the write file.

      fdWrite = Obj::open(writeLockedFn, Obj::e_OPEN, Obj::e_READ_WRITE);
      ASSERT(Obj::k_INVALID_FD != fdWrite);

      // Open the read file.  Note we open fdRead for 'write' so we can
      // try to lock it for write later

      fdRead = Obj::open(readLockedFn, Obj::e_OPEN, Obj::e_READ_WRITE);
      ASSERT(Obj::k_INVALID_FD != fdRead);
      ...
      ASSERT(Obj::k_ERROR_LOCKING_CONFLICT == Obj::tryLock(fdWrite, true));

      // Attempt to lock the write file for read, which should also fail.

      ASSERT(Obj::k_ERROR_LOCKING_CONFLICT == Obj::tryLock(fdWrite, false));

      // Attempt to lock the read file for write, which should also fail.

      ASSERT(Obj::k_ERROR_LOCKING_CONFLICT == Obj::tryLock(fdRead, true));
      ...
      if (verbose)
        cout << "TRYLOCK TEST -- CHILD FINISHED\n"
                "==============================\n";
  ...
  case 12: {
    ...
    if (verbose)
      cout << "APPEND TEST -- SINGLE PROCESS\n"
              "=============================\n";
    ...
    Obj::FileDescriptor fd =
        Obj::open(fileName, Obj::e_OPEN_OR_CREATE, Obj::e_READ_APPEND);
    ASSERT(Obj::k_INVALID_FD != fd);
  ...
  case 11: {
    ...
    if (veryVerbose)
      cout << "Simple 'findMatchingPaths' test.\n";
    {
      for (int i = 0; i < 4; ++i) {
        char name[16];
    ...
    if (veryVerbose)
      cout << "Test 'ENOTDIR' failure mode.\n";
    {
      ...
      Obj::FileDescriptor fd =
          Obj::open(name0, Obj::e_OPEN_OR_CREATE, Obj::e_READ_WRITE);
      Obj::close(fd);
      ...
      vector<string> paths;
      int rc = Obj::findMatchingPaths(&paths, "tmp" PS "*" PS "log" PS "*");
      ASSERTV(rc, 3 == rc);
      sort(paths.begin(), paths.end());
  ...
  case 10: {
    ...
    if (verbose)
      cout << "Testing 'getFileSize'\n"
              "=====================\n";

    // const char * is tested with each 'string' type
    testCase10_getFileSize<bsl::string>("bsl::string", tmpWorkingDir, test,
                                        verbose, veryVerbose, veryVeryVerbose);
    testCase10_getFileSize<std::string>("std::string", tmpWorkingDir, test,
                                        verbose, veryVerbose, veryVeryVerbose);
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR_STRING
    testCase10_getFileSize<std::experimental::pmr::string>(
        "std::experimental::pmr::string", tmpWorkingDir, test, verbose,
        veryVerbose, veryVeryVerbose);
#endif
  } break;
  case 9: {
    ...
    if (verbose)
      cout << "Testing 'getSystemTemporaryDirectory'\n"
              "=====================================\n";

    testCase9_createSystemTemporaryDirectory<bsl::string>(
        "bsl::string", tmpWorkingDir, verbose, veryVerbose, veryVeryVerbose);
    testCase9_createSystemTemporaryDirectory<std::string>(
        "std::string", tmpWorkingDir, verbose, veryVerbose, veryVeryVerbose);
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR_STRING
    testCase9_createSystemTemporaryDirectory<std::experimental::pmr::string>(
        "std::experimental::pmr::string", tmpWorkingDir, verbose, veryVerbose,
        veryVeryVerbose);
#endif
  } break;
  case 8: {
    ...
    if (verbose)
      cout << "Testing 'getAvailableSpace'\n"
              "===========================\n";
  ...
  case 7: {
    ...
    if (verbose)
      cout << "Testing 'rollFileChain' (files)\n"
              "===============================\n";
    ...
    for (int i = MAXSUFFIX; i >= 0; --i) {
      ...
      f = Obj::open(tmpFile, Obj::e_OPEN_OR_CREATE, Obj::e_READ_WRITE);
    ...
    for (int i = MAXSUFFIX; i >= 0; --i) {
      ...
      f = Obj::open(tmpFile + PS + "file", Obj::e_OPEN_OR_CREATE,
                    Obj::e_READ_WRITE);
    ...
    for (int i = 0; i < MAXSUFFIX; ++i) {
      int value = -1;
      tmpFile[pos] = "123456789"[i];
      f = Obj::open(tmpFile + PS + "file", Obj::e_OPEN, Obj::e_READ_ONLY);
      ASSERT(f != Obj::k_INVALID_FD);
      ASSERT(INT_SIZEOF(int) == Obj::read(f, &value, INT_SIZEOF(int)));
      ASSERT(0 == Obj::close(f));
...

    {
      ...
      bsl::string_view partialView(filename.data(), filename.length() - 2);
      ...
      for (bsl::vector<bsl::string>::const_iterator i = filenameVector.begin();
           i != filenameVector.end(); ++i) {
  ...
  case 6: {
...
#ifndef BSLS_PLATFORM_OS_WINDOWS
    testCase6_move_normal(NAMES[NAME_ASCII], NAMES[NAME_ASCII], false, false,
                          verbose, veryVerbose, veryVeryVerbose);
#endif

    testCase6_move_normal(NAMES[NAME_ASCII], NAMES[NAME_UTF8], false, false,
                          verbose, veryVerbose, veryVeryVerbose);

    testCase6_move_normal(NAMES[NAME_UTF8], NAMES[NAME_ASCII], false, false,
                          verbose, veryVerbose, veryVeryVerbose);
    ...
    testCase6_move_normal(NAMES[NAME_ANSI], NAMES[NAME_ASCII], isWindows, false,
                          verbose, veryVerbose, veryVeryVerbose);

    testCase6_move_normal(NAMES[NAME_ASCII], NAMES[NAME_ANSI], isWindows,
                          isWindows, verbose, veryVerbose, veryVeryVerbose);
...
#define CASE_6_TEST_TEMPLATE_2_TYPES(TYPE1, TYPE2)                             \
  testCase6_move_template<TYPE1, TYPE2>(                                       \
      ...
      (TYPE2)(const_cast<char *>(NAMES[NAME_UTF8])), false, false, verbose,    \
      veryVerbose, veryVeryVerbose)
...
#undef CASE_6_TEST_TEMPLATE_2_TYPES
  ...
  case 5: {
    ...
    if (verbose)
      cout << "Testing 'isRegularFile' & 'isDirectory'\n"
              "=======================================\n";

    testCase5_isRegularFile_isDirectory<bsl::string>(
        "bsl::string", test, tmpWorkingDir, verbose, veryVerbose,
        veryVeryVerbose);
    testCase5_isRegularFile_isDirectory<std::string>(
        "std::string", test, tmpWorkingDir, verbose, veryVerbose,
        veryVeryVerbose);
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR_STRING
    ...
    struct ParametersByType {
      Parameters regular;
      Parameters directory;
    } parameters = {{"tmp.filesystemutil.case4" PS "file",
  ...
  case 4: {
    ...
    testCase4_findMatchingPaths<bsl::vector<bsl::string>>(
        "bsl::vector<bsl::string>>", "bsl::string_view", test, tmpWorkingDir,
        verbose, veryVerbose, veryVeryVerbose);
    testCase4_findMatchingPaths<std::vector<std::string>>(
        "std::vector<std::string>>", "bsl::string_view", test, tmpWorkingDir,
        verbose, veryVerbose, veryVeryVerbose);
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR_STRING
    testCase4_findMatchingPaths<
        std::experimental::pmr::vector<std::experimental::pmr::string>>(
        "std::experimental::pmr::vector<std::experimental::pmr::string>>",
        "bsl::string_view", test, tmpWorkingDir, verbose, veryVerbose,
        veryVeryVerbose);
#endif
  } break;
  case 3: {
    ...
    enum { k_READ_BUFFER_SIZE = 64 };

    if (verbose)
      cout << "\nBootstrap, phase 1: 'open' file mode." << endl;
    {
      ...
      for (int ti = 0; ti < NUM_DATA; ++ti) {
        ...
        const Obj::FileOpenPolicy OPEN_POLICY = DATA[ti].d_openPolicy;
        const Obj::FileIOPolicy IO_POLICY = DATA[ti].d_ioPolicy;
        const Obj::FileTruncatePolicy INIT_POLICY = DATA[ti].d_initPolicy;
        ...
        if (EXISTS) {
          ...
          Obj::FileDescriptor fd =
              Obj::open(fileName, Obj::e_CREATE, Obj::e_WRITE_ONLY);
        ...
        Obj::FileDescriptor fd =
            Obj::open(fileName, OPEN_POLICY, IO_POLICY, INIT_POLICY);
        ...
        case Obj::e_READ_ONLY: {
          ...
          LOOP2_ASSERT(LINE, IO_POLICY, O_RDONLY == (flags & O_RDONLY));
        ...
        case Obj::e_WRITE_ONLY: {
          ...
          LOOP2_ASSERT(LINE, IO_POLICY, O_WRONLY == (flags & O_WRONLY));
          LOOP2_ASSERT(LINE, IO_POLICY, 0 == (flags & O_APPEND));
          LOOP2_ASSERT(LINE, IO_POLICY, 0 == (flags & O_RDWR));
        } break;
        case Obj::e_APPEND_ONLY: {
          ...
          LOOP2_ASSERT(LINE, IO_POLICY, 0 == (flags & O_RDONLY));
          LOOP2_ASSERT(LINE, IO_POLICY, O_WRONLY == (flags & O_WRONLY));
          LOOP2_ASSERT(LINE, IO_POLICY, O_APPEND == (flags & O_APPEND));
          LOOP2_ASSERT(LINE, IO_POLICY, 0 == (flags & O_RDWR));
        } break;
        ...
        if (Obj::e_CREATE == OPEN_POLICY && Obj::e_WRITE_ONLY == IO_POLICY) {
    ...
    if (verbose)
      cout << "\nBootstrap, phase 2: basic IO test." << endl;
    {
      bsl::string fileName(::tempFileName(test));
      ...
      const int numBytesA = static_cast<int>(sizeof(blockA) / sizeof(*blockA));
      const char blockB[] = {'0', '1', '2', '3', '4'};
      const int numBytesB = static_cast<int>(sizeof(blockB) / sizeof(*blockB));
      ...
      int numBytes = static_cast<int>(read(fd, buffer, k_READ_BUFFER_SIZE));
      LOOP2_ASSERT(numBytes, numBytesA, numBytes == numBytesA);
      LOOP2_ASSERT(blockA, buffer, 0 == memcmp(blockA, buffer, numBytesA));
    ...
    if (verbose)
      cout << "\nBootstrap, phase 3: 'read' and 'write'." << endl;
    {
      const char blockA[] = {'a', 'b', 'c', 'd', 'e', 'f', 'g'};
      const int numBytesA = static_cast<int>(sizeof(blockA) / sizeof(*blockA));
      const char blockB[] = {'0', '1', '2', '3', '4'};
      ...
      for (int i = 0; i < numLengths; ++i) {
        ...
        if (0 <= lengths[i]) {
          LOOP2_ASSERT(lengths[i], bytesWritten, lengths[i] == bytesWritten);
        } else {
      ...
      for (int i = 0; i < numLengths; ++i) {
        ...
        Obj::FileDescriptor fd =
            Obj::open(fileName, Obj::e_OPEN, Obj::e_READ_WRITE);
        LOOP_ASSERT(lengths[i], Obj::k_INVALID_FD != fd);
        ...
        LOOP_ASSERT(lengths[i], 0 == memcmp(buffer, blockA, bytesRead));
      ...
      if (veryVerbose) {
        cout << "\tPositional properties of 'write'" << endl;
      }
      ...
      for (int i = 0; i < numLengths; ++i) {
        ...
        LOOP2_ASSERT(lengths[i], bytesRead, lengths[i] + 1 == bytesRead);
      ...
      if (veryVerbose) {
        cout << "\tPositional properties of 'read'" << endl;
      }
      ...
      for (int i = 0; i < numLengths; ++i) {
        ...
        LOOP2_ASSERT(lengths[i], buffer[0], blockA[lengths[i]] == buffer[0]);
      ...
      if (veryVerbose) {
        cout << "\tCombined positional properties" << endl;
      }

      // Combined positional properties

      if (veryVerbose) {
        cout << "\tWrite followed by read" << endl;
      }
      ...
      for (int i = 0; i < numLengths; ++i) {
        ...
        int bytesWritten = Obj::write(fd, blockA, lengths[i]);
        LOOP2_ASSERT(lengths[i], bytesWritten, bytesWritten == lengths[i]);
        ...
        LOOP2_ASSERT(lengths[i], buffer[0], buffer[0] == blockA[lengths[i]]);
      ...
      if (veryVerbose) {
        cout << "\tRead followed by write" << endl;
      }
      ...
      cout << "\nBootstrap, phase 4: 'seek'." << endl;
    {
      ...
      // Seek in empty file.
      {
        ...
        Obj::Offset position = Obj::seek(fd, 0, Obj::e_SEEK_FROM_CURRENT);
      ...
      if (veryVerbose) {
        cout << "\tSeek in non-empty file." << endl;
      }
...

      {
        ...
        for (int i = 0; i < numLengths; ++i) {
          ...
          position = Obj::seek(fd, lengths[i], Obj::e_SEEK_FROM_BEGINNING);
...

      {
        ...
        for (int i = 0; i < numLengths; ++i) {
          ...
          Obj::Offset position =
              Obj::seek(fd, lengths[i], Obj::e_SEEK_FROM_BEGINNING);
          LOOP_ASSERT(position, lengths[i] == position);
...

      {
        for (int i = 0; i < numLengths; ++i) {
          ...
          LOOP2_ASSERT(lengths[i], bytesRead, numBytesA == bytesRead);
          LOOP_ASSERT(lengths[i], 0 == memcmp(buffer, blockA, numBytesA));
          ...
          position =
              Obj::seek(fd, numBytesA + lengths[i], Obj::e_SEEK_FROM_BEGINNING);
          LOOP_ASSERT(position, numBytesA + lengths[i] == position);
    ...
    if (verbose)
      cout << "\nBootstrap, phase 5: 'open' with truncation." << endl;
    {
      const Obj::FileOpenPolicy OP = Obj::e_OPEN;
      const Obj::FileOpenPolicy CR = Obj::e_CREATE;
      const Obj::FileOpenPolicy OC = Obj::e_OPEN_OR_CREATE;
      ...
      for (int ti = 0; ti < NUM_DATA; ++ti) {
        ...
        if (!EXISTS || Obj::e_TRUNCATE == INIT_POLICY) {
          LOOP_ASSERT(LINE, 0 == Obj::seek(fd, 0, Obj::e_SEEK_FROM_END));
    ...
    if (verbose)
      cout << "\nBootstrap, phase 6: 'open' with append." << endl;
    {
      const Obj::FileOpenPolicy OP = Obj::e_OPEN;
      const Obj::FileOpenPolicy CR = Obj::e_CREATE;
      const Obj::FileOpenPolicy OC = Obj::e_OPEN_OR_CREATE;
      ...
      for (int ti = 0; ti < NUM_DATA; ++ti) {
        ...
        if (Obj::e_WRITE_ONLY == IO_POLICY || Obj::e_READ_WRITE == IO_POLICY) {
          LOOP_ASSERT(LINE, 0 == Obj::seek(fd, 0, Obj::e_SEEK_FROM_BEGINNING));
          LOOP_ASSERT(LINE, numBytesA == Obj::write(fd, blockA, numBytesA));
          LOOP_ASSERT(LINE, 0 == Obj::seek(fd, 0, Obj::e_SEEK_FROM_BEGINNING));
          LOOP_ASSERT(LINE, numBytesB == Obj::write(fd, blockB, numBytesB));

          LOOP_ASSERT(LINE,
                      numBytesB == Obj::seek(fd, 0, Obj::e_SEEK_FROM_CURRENT));
        } else if (Obj::e_APPEND_ONLY == IO_POLICY ||
                   Obj::e_READ_APPEND == IO_POLICY) {
          LOOP_ASSERT(LINE, 0 == Obj::seek(fd, 0, Obj::e_SEEK_FROM_BEGINNING));
          LOOP_ASSERT(LINE, numBytesA == Obj::write(fd, blockA, numBytesA));
          Obj::Offset endpoint = Obj::seek(fd, 0, Obj::e_SEEK_FROM_END);
          LOOP_ASSERT(LINE, 0 == Obj::seek(fd, 0, Obj::e_SEEK_FROM_BEGINNING));
          LOOP_ASSERT(LINE, numBytesB == Obj::write(fd, blockB, numBytesB));
    ...
    if (verbose)
      cout << "\nFull test of enum-based four-argument open" << endl;
    {
      const Obj::FileOpenPolicy OP = Obj::e_OPEN;
      const Obj::FileOpenPolicy CR = Obj::e_CREATE;
      const Obj::FileOpenPolicy OC = Obj::e_OPEN_OR_CREATE;
      ...
      for (int ti = 0; ti < NUM_DATA; ++ti) {
        ...
        LOOP_ASSERT(LINE, 0 == Obj::seek(fd, 0, Obj::e_SEEK_FROM_CURRENT));
        ...
        if (Obj::e_READ_ONLY == IO_POLICY || Obj::e_READ_WRITE == IO_POLICY ||
            Obj::e_READ_APPEND == IO_POLICY) {
      ...
      cout << "\nNegative testing" << endl;
    {
...

      {
        ...
        ASSERT_PASS(fd = Obj::open(fileName.c_str(), Obj::e_OPEN_OR_CREATE,
                                   Obj::e_READ_ONLY));
...

      {
        ...
        ASSERT_FAIL(fd = Obj::open(static_cast<const char *>(0),
                                   Obj::e_OPEN_OR_CREATE, Obj::e_READ_ONLY));
  ...
  case 2: {
    ...
    //:   'std::experimental::pmr::string' arguments, without additional
    //:   allocations.
    //:
...

    {
      ...
      // bsl::string
      {
        ...
        testCase2_CStringUtil<bsl::string>("bsl::string", shortStr,
                                           SHORT_STRING, test, verbose,
                                           veryVerbose, veryVeryVerbose);

        bsl::string longStr(LONG_STRING, &ta2);
        testCase2_CStringUtil<bsl::string>("bsl::string", longStr, LONG_STRING,
                                           test, verbose, veryVerbose,
                                           veryVeryVerbose);
      }
      ...
      // std::string
      {
        ...
        testCase2_CStringUtil<std::string>("std::string", shortStr,
                                           SHORT_STRING, test, verbose,
                                           veryVerbose, veryVeryVerbose);

        std::string longStr(LONG_STRING);
        testCase2_CStringUtil<std::string>("std::string", longStr, LONG_STRING,
                                           test, verbose, veryVerbose,
                                           veryVeryVerbose);
      }
...
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR_STRING
      {
        ...
        testCase2_CStringUtil<std::experimental::pmr::string>(
            "std::experimental::pmr::string", shortStr, SHORT_STRING, test,
            verbose, veryVerbose, veryVeryVerbose);

        std::experimental::pmr::string longStr(LONG_STRING, &ta2);
        testCase2_CStringUtil<std::experimental::pmr::string>(
            "std::experimental::pmr::string", longStr, LONG_STRING, test,
            verbose, veryVerbose, veryVeryVerbose);
      }
#endif
      ...
      // bslstl::StringRef (SHORT STRING)
      {
        ...
        testCase2_CStringUtil_StringRef("bslstl::StringRef", shortStr,
                                        SHORT_STRING, test, verbose,
                                        veryVerbose, veryVeryVerbose);
      }
    }

    {
      ...
      // bslstl::StringRef (LONG STRING)
      {
        ...
        testCase2_CStringUtil_StringRef("bslstl::StringRef", longStr,
                                        LONG_STRING, test, verbose, veryVerbose,
                                        veryVeryVerbose);
      }
    }
  ...
  case 1: {
    ...
    if (verbose)
      cout << "Usage Example like Testing\n"
              "==========================\n";
    ...
    for (size_t ni = 0; ni < NUM_VALID_NAMES; ++ni) {
      ...
      bdls::PathUtil::appendRaw(&oldPath, "old");
      bdls::PathUtil::appendRaw(&newPath, "new");
      LOOP_ASSERT(oldPath, 0 == Obj::createDirectories(oldPath.c_str(), true));
      LOOP_ASSERT(newPath, 0 == Obj::createDirectories(newPath.c_str(), true));

      ASSERT(Obj::exists(oldPath));
      ASSERT(Obj::exists(newPath));

      enum { k_FILENAME_BUFFER_SIZE = 32 };
      ...
      for (int i = 0; i < NUM_TOTAL_FILES; ++i) {
        ...
        int filenameLength = sprintf(
            filenameBuffer, "filesystemutil%02d_%c.log", i, isOld ? 'o' : 'n');

        ASSERT(0 == bdls::PathUtil::appendIfValid(&logPath, filenameBuffer));

        if (Obj::exists(logPath.c_str())) {
          ASSERT(0 == Obj::remove(logPath.c_str(), true));
        }
        Obj::FileDescriptor f = Obj::open(
            logPath.c_str(), Obj::e_OPEN_OR_CREATE, Obj::e_READ_WRITE);
        LOOP_ASSERT(logPath, f != Obj::k_INVALID_FD);

        filenameBuffer[filenameLength++] = '\n';
        filenameBuffer[filenameLength++] = 0;

        ASSERT(filenameLength == Obj::write(f, filenameBuffer, filenameLength));
        ASSERT(0 == Obj::close(f));

#ifndef BSLS_PLATFORM_OS_WINDOWS
        bsls::TimeInterval threeDaysAgo =
            bdlt::CurrentTime::now() - bsls::TimeInterval(3 * 24 * 3600, 0);
        if (isOld) {
      ...
      for (vector<bsl::string>::iterator it = logFiles.begin();
           it != logFiles.end(); ++it) {
        ASSERTV(it->c_str(),
                0 == Obj::getLastModificationTime(&modTime, it->c_str()));
      ...
      LOOP5_ASSERT(ni, logPath, logPath.length(), NUM_OLD_FILES,
                   logFiles.size(), NUM_OLD_FILES == logFiles.size());
      bdls::PathUtil::popLeaf(&logPath);
      ...
      LOOP5_ASSERT(ni, logPath, logPath.length(), NUM_NEW_FILES,
                   logFiles.size(), NUM_NEW_FILES == logFiles.size());
      bdls::PathUtil::popLeaf(&logPath);
      bdls::PathUtil::popLeaf(&logPath);

      logFiles.clear();
      rc = Obj::findMatchingPaths(&logFiles, "tmp.non_existent_file.txt");
...
#else
    if (verbose)
      cout << "TESTING detection of large files\n"
              "================================\n";
  ...
  case -3: {
    ...
    if (verbose)
      cout << "SIMPLE 5 GIGABYTE FILE TEST CASE\n"
              "================================\n";
    ...
    LOOP_ASSERT(Util::getFileSizeLimit(), Util::getFileSizeLimit() > fiveGig);

    const char *fileName = "tmpFiveGig.txt";
    Util::FileDescriptor fd =
        Util::open(fileName, Obj::e_OPEN_OR_CREATE, Obj::e_READ_WRITE);
    ...
    for (;;) {
      ...
      if (bytesWritten >= mileStone) {
        cout << bytesWritten << " written -- last: " << rand.display() << endl;
    ...
    for (;;) {
      ...
      if (bytesRead >= mileStone) {
        cout << bytesRead << " read -- last: " << rand.display() << endl;
    ...
    ASSERT(0 == Util::map(fd, &startAddress, 0,
    ...
    for (;;) {
      ...
      if (bytesRead >= mileStone) {
        cout << bytesRead << " checked map -- last: " << rand.display() << endl;
    ...
    ASSERT(0 == Util::unmap(startAddress, static_cast<bsl::size_t>(threeGig)));
  ...
  if (verbose && Obj::exists(tmpWorkingDir)) {
    ...
    const bsl::string &lsCmd =
        (e_UNIX ? "/bin/ls -laF " : "dir /o ") + tmpWorkingDir;
