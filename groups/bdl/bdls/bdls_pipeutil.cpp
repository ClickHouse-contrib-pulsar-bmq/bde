...
inline void getPipeDir(STRING_TYPE *dir)
...
template <class STRING_TYPE> inline void getPipeDir(STRING_TYPE *dir) {
  BSLS_ASSERT(dir);
...
int u_makeCanonicalName(STRING_TYPE *pipeName,
                        const STRING_REF_TYPE &baseName) {
...
bool PipeUtil::isOpenForReading(const bsl::string_view &pipeName) {
  bsl::wstring wPipeName;
  ...
  HANDLE pipe = CreateFileW(wPipeName.data(), GENERIC_WRITE, 0, NULL,
...
int PipeUtil::send(const bsl::string_view &pipeName,
  ...
  int rc = static_cast<int>(write(pipe, message.data(), message.length()));
  (void)rc;
  close(pipe);
  return !(rc > 0);
}
