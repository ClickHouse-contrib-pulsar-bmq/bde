...
void testCase2_getOsInfo(const char *typeName, int test, int verbose,
...
int main(int argc, char *argv[]) {
  ...
  case 3: {
    ...
    // information about the operating system at runtime and writing it to the
    // console.
    //
    // First, we create strings for the operating system name ('osName'),
    // version
    // ('osVersion'), and patch ('osPatch'), and then call 'getOsInfo' to load
    // these strings with values for the operating system the task is executing
    // in:
    //..
    bsl::string name;
    bsl::string version;
    ...
    //..
    // Finally, the resulting console output on the Red Hat Enterprise Linux
    // Server 5.5 would be
    //..
    //  OS Name: Linux
  ...
  case 2: {
    ...
    testCase2_getOsInfo<bsl::string>("bsl::string", test, verbose, veryVerbose,
                                     veryVeryVerbose);

    testCase2_getOsInfo<std::string>("std::string", test, verbose, veryVerbose,
                                     veryVeryVerbose);

#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR_STRING
    testCase2_getOsInfo<std::experimental::pmr::string>(
        "std::experimental::pmr::string", test, verbose, veryVerbose,
        veryVeryVerbose);
#endif
  } break;
  case 1: {
