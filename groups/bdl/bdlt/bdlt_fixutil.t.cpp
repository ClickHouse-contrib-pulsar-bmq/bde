...
static const DefaultDateDataRow DEFAULT_DATE_DATA[] = {
    ...
    //----   ----   -----   ---    ----------
    {L_, 1, 1, 1, "00010101"},      {L_, 9, 9, 9, "00090909"},
    {L_, 30, 10, 20, "00301020"},   {L_, 842, 12, 19, "08421219"},
    {L_, 1847, 5, 19, "18470519"},  {L_, 2000, 2, 29, "20000229"},
...
static const DefaultZoneDataRow EXTENDED_ZONE_DATA[] = {
    // LINE   OFFSET    FIX
    //----   ------   -----
    {L_, 0, "Z"},   {L_, -120, "-02"}, {L_, 0, "-00"},
    {L_, 0, "+00"}, {L_, 60, "+01"},   {L_, 600, "+10"}};
...
static const BadZoneDataRow BAD_ZONE_DATA[] = {
    ...
    {L_, "+01:1"}, // length = 5
    {L_, "-01:1"},    {L_, "+1:12"},   {L_, "+12:1"},   {L_, "+0000"},
    {L_, "-0000"},    {L_, "+2360"},   {L_, "-2360"},   {L_, "+2400"},
    {L_, "-2400"},
...
int main(int argc, char *argv[]) {
...
#ifdef BSLS_LIBRARYFEATURES_HAS_CPP17_PMR_STRING
  if (verbose)
    cout << "std::experimental::pmr::string supported\n";
#endif
  ...
  case 10: {
    ...
    const bdlt::DatetimeTz sourceDatetimeTz(bdlt::Datetime(date, time),
                                            tzOffset);
    //..
    // For comparison with the FIX string generated below, note that streaming
    ...
    // 'sourceDatetimeTz', writing the output to a 'bsl::ostringstream', and
    ...
    int rc = bdlt::FixUtil::parse(&targetDatetimeTz, fix.c_str(),
    ...
    rc = bdlt::FixUtil::parse(&targetDatetime, fix.c_str(),
                              static_cast<int>(fix.length()));
    ASSERT(0 == rc);
    ...
    // influence the format of the FIX strings that are generated by this
    // component by passing that configuration object to 'generate'.  We also
    // take this opportunity to illustrate the flavor of the 'generate'
    // functions that outputs to a 'char *' buffer of a specified length.
    //
    // First, we construct the 'bdlt::FixUtilConfiguration' object that
    // indicates how we would like to affect the generated output FIX string. In
    // this case, we want to have microsecond precision displayed:
    //..
    ...
    // generated string.  A buffer of size 'bdlt::FixUtil::k_DATETIMETZ_STRLEN +
    ...
    // produce a FIX-compliant string for 'sourceDatetimeTz', this time writing
    // the output to a 'char *' buffer, and assert that both the return value
    // and the string that is produced are as expected.  Note that in comparing
    // the return value against 'BUFLEN - 1' we account for the fact that,
    ...
    //..
    rc = bdlt::FixUtil::generate(buffer, BUFLEN, sourceDatetimeTz,
                                 configuration);
    ASSERT(BUFLEN - 1 == rc);
    ASSERT(0 == bsl::strcmp(buffer, "20050131-08:59:59.123000+04:00"));
    //..
    // For comparison, see the output that was produced by the streaming
    // operator above.
    //
    ...
    // the parse into a second 'bdlt::DatetimeTz' object, and assert that the
    // parse was successful and that the target object has the same value as
    // that of the original (i.e., 'sourceDatetimeTz').  Note that 'BUFLEN - 1'
    // is passed and *not* 'BUFLEN' because the former indicates the correct
    // number of characters in 'buffer' that we wish to parse:
    //..
  ...
  case 9: {
    ...
    if (verbose) {
      cout << "\nTesting optional, leap, and fractional seconds." << endl;
    }
    {
      const struct {
        int d_line;
        ...
        int d_offset;
          ...
          // optional seconds
          {L_, "00010101-00:00", 0001, 01, 01, 00, 00, 00, 000, 000, 0},
          {L_, "00010101-00:01", 0001, 01, 01, 00, 01, 00, 000, 000, 0},
          {L_, "00010101-01:00", 0001, 01, 01, 01, 00, 00, 000, 000, 0},
          {L_, "00010101-01:01", 0001, 01, 01, 01, 01, 00, 000, 000, 0},

          // leap seconds
          {L_, "00010101-00:00:60.000", 0001, 01, 01, 00, 01, 00, 000, 000, 0},
          {L_, "99981231-23:59:60.999", 9999, 01, 01, 00, 00, 00, 999, 000, 0},

          // fractional seconds
          {L_, "00010101-00:00:00.0000001", 0001, 01, 01, 00, 00, 00, 000, 000,
           0},
          {L_, "00010101-00:00:00.0000009", 0001, 01, 01, 00, 00, 00, 000, 1,
           0},
          {L_, "00010101-00:00:00.00000001", 0001, 01, 01, 00, 00, 00, 000, 000,
           0},
          {L_, "00010101-00:00:00.00000049", 0001, 01, 01, 00, 00, 00, 000, 000,
           0},
          {L_, "00010101-00:00:00.00000050", 0001, 01, 01, 00, 00, 00, 000, 1,
           0},
          {L_, "00010101-00:00:00.00000099", 0001, 01, 01, 00, 00, 00, 000, 1,
           0},
          {L_, "00010101-00:00:00.0001", 0001, 01, 01, 00, 00, 00, 000, 100, 0},
          {L_, "00010101-00:00:00.0009", 0001, 01, 01, 00, 00, 00, 000, 900, 0},
          {L_, "00010101-00:00:00.00001", 0001, 01, 01, 00, 00, 00, 000, 10, 0},
          {L_, "00010101-00:00:00.00049", 0001, 01, 01, 00, 00, 00, 000, 490,
           0},
          {L_, "00010101-00:00:00.00050", 0001, 01, 01, 00, 00, 00, 000, 500,
           0},
          {L_, "00010101-00:00:00.00099", 0001, 01, 01, 00, 00, 00, 000, 990,
           0},
          {L_, "00010101-00:00:00.9994", 0001, 01, 01, 00, 00, 00, 999, 400, 0},
          {L_, "00010101-00:00:00.9995", 0001, 01, 01, 00, 00, 00, 999, 500, 0},
          {L_, "00010101-00:00:00.9999", 0001, 01, 01, 00, 00, 00, 999, 900, 0},
          {L_, "99981231-23:59:60.9999", 9999, 01, 01, 00, 00, 00, 999, 900, 0},
          {L_, "00010101-00:00:00.9999994", 0001, 01, 01, 00, 00, 00, 999, 999,
           0},
          {L_, "00010101-00:00:00.9999995", 0001, 01, 01, 00, 00, 01, 000, 000,
           0},
          {L_, "00010101-00:00:00.9999999", 0001, 01, 01, 00, 00, 01, 000, 000,
           0},
          {L_, "99981231-23:59:60.9999999", 9999, 01, 01, 00, 00, 01, 000, 000,
           0},

          // omit fractional seconds
          {L_, "00010101-00:00:60", 0001, 01, 01, 00, 01, 00, 000, 000, 0},
          {L_, "20141223-12:34:45", 2014, 12, 23, 12, 34, 45, 000, 000, 0},
          {L_, "20141223-12:34:45Z", 2014, 12, 23, 12, 34, 45, 000, 000, 0},
          {L_, "20141223-12:34:45+00:30", 2014, 12, 23, 12, 34, 45, 000, 000,
           30},
          {L_, "20141223-12:34:45-01:30", 2014, 12, 23, 12, 34, 45, 000, 000,
           -90},
      };
    ...
    if (verbose)
      cout << "\nTesting timezone offsets that overflow a 'Datetime'." << endl;
    {
      struct {
        int d_line;
        const char *d_input_p;
        ...
        int d_offset;
          ...
          {L_, "00010101-00:00:00.000+00:00", 0001, 01, 01, 00, 00, 00, 000, 0},
          {L_, "00010101-00:00:00.000+00:01", 0001, 01, 01, 00, 00, 00, 000, 1},
          {L_, "00010101-23:58:59.000+23:59", 0001, 01, 01, 23, 58, 59, 000,
           1439},

          {L_, "99991231-23:59:59.999+00:00", 9999, 12, 31, 23, 59, 59, 999, 0},
          {L_, "99991231-23:59:59.999-00:01", 9999, 12, 31, 23, 59, 59, 999,
           -1},
          {L_, "99991231-00:01:00.000-23:59", 9999, 12, 31, 00, 01, 00, 000,
           -1439},
      };
      const int NUM_DATA = static_cast<int>(sizeof DATA / sizeof *DATA);
      ...
      for (int tj = 0; tj < NUM_ZONE_DATA; ++tj) {
        ...
        for (int tc = 0; tc < NUM_CNFG_DATA; ++tc) {
          ...
          // without timezone offset in parsed string
          {
            ...
            ASSERTV(ILINE, JLINE, CLINE, TIMETZ.localTime() == Z.localTime());
          ...
          // with timezone offset in parsed string
          {
            ...
            const bdlt::TimeTz EXPTIMETZ(bdlt::Time(HOUR, MIN, SEC), OFFSET);
      ...
      for (int tj = 0; tj < NUM_TIME_DATA; ++tj) {
        ...
        for (int tk = 0; tk < NUM_ZONE_DATA; ++tk) {
          ...
          const TYPE X(
              bdlt::Datetime(YEAR, MONTH, DAY, HOUR, MIN, SEC, MSEC, USEC),
              OFFSET);
          const bsl::string BASE_EXPECTED(EXPECTED_DATE + '-' + EXPECTED_TIME +
                                          EXPECTED_ZONE);
