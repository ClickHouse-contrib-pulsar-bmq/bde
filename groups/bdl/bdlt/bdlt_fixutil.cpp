...
int parseDate(const char **nextPos, Date *date, const char *begin,
              const char *end)
...
int parseFractionalSecond(const char **nextPos, int *microsecond,
...
int parseTimezoneOffset(const char **nextPos, int *minuteOffset,
                        const char *begin, const char *end)
...
int parseTime(const char **nextPos, Time *time, int *tzOffset, bool *isNextDay,
              const char *begin, const char *end)
...
// reached is not an error.
{
  ...
  int localTzOffset = 0;
  if (p != end) {
    if (0 != u::parseTimezoneOffset(&p, &localTzOffset, p, end) || p != end) {
      return -1; // RETURN
    }
  }

  if (0 !=
      time->setTimeIfValid(hour, minute, second, millisecond, microsecond)) {
    return -1; // RETURN
  }
...
inline int generateInt(char *buffer, int value, int paddedLen, char separator)
// Write, to the specified 'buffer', the decimal string representation of
// the specified 'value' padded with leading zeros to the specified
...
int generateTimezoneOffset(char *buffer, int tzOffset,
                           const FixUtilConfiguration &configuration)
...
int generatedLengthForDateTzObject(int defaultLength, int tzOffset,
                                   const FixUtilConfiguration &configuration)
...
int generatedLengthForDatetimeTzObject(
    int defaultLength, int tzOffset, const FixUtilConfiguration &configuration)
// Return the number of bytes generated, when the specified 'configuration'
...
int generatedLengthForTimeTzObject(int defaultLength, int tzOffset,
                                   const FixUtilConfiguration &configuration)
// Return the number of bytes generated, when the specified 'configuration'
...
int FixUtil::generate(char *buffer, int bufferLength, const Datetime &object,
                      const FixUtilConfiguration &configuration) {
  BSLS_ASSERT(buffer);
  BSLS_ASSERT(0 <= bufferLength);
...
int FixUtil::generate(char *buffer, int bufferLength, const DatetimeTz &object,
                      const FixUtilConfiguration &configuration) {
  BSLS_ASSERT(buffer);
  BSLS_ASSERT(0 <= bufferLength);
