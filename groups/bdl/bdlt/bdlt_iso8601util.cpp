...
class Impl {
  ...
  template <class TYPE> struct IsString {
...
public:
  ...
  static int generate(STRING *string, const bsls::TimeInterval &object,
                      const GenerateConfiguration &configuration);
  template <class STRING>
  static int generate(STRING *string, const Date &object,
                      const GenerateConfiguration &configuration);
  template <class STRING>
  static int generate(STRING *string, const Time &object,
                      const GenerateConfiguration &configuration);
  template <class STRING>
  static int generate(STRING *string, const Datetime &object,
                      const GenerateConfiguration &configuration);
  template <class STRING>
  static int generate(STRING *string, const DateTz &object,
                      const GenerateConfiguration &configuration);
  template <class STRING>
  static int generate(STRING *string, const TimeTz &object,
                      const GenerateConfiguration &configuration);
  template <class STRING>
  static int generate(STRING *string, const DatetimeTz &object,
                      const GenerateConfiguration &configuration);
  template <class STRING>
  static int generate(STRING *string, const Util::DateOrDateTz &object,
                      const GenerateConfiguration &configuration);
  template <class STRING>
  static int generate(STRING *string, const Util::TimeOrTimeTz &object,
                      const GenerateConfiguration &configuration);
  template <class STRING>
  static int generate(STRING *string, const Util::DatetimeOrDatetimeTz &object,
                      const GenerateConfiguration &configuration);
...
template <class STRING>
inline int Impl::generate(STRING *string, const bsls::TimeInterval &object,
                          const GenerateConfiguration &configuration) {
  BSLS_ASSERT(string);
...
template <class STRING>
inline int Impl::generate(STRING *string, const Date &object,
                          const GenerateConfiguration &configuration) {
  BSLS_ASSERT(string);
...
template <class STRING>
inline int Impl::generate(STRING *string, const Time &object,
                          const GenerateConfiguration &configuration) {
  BSLS_ASSERT(string);
...
template <class STRING>
inline int Impl::generate(STRING *string, const Datetime &object,
                          const GenerateConfiguration &configuration) {
  BSLS_ASSERT(string);
...
template <class STRING>
inline int Impl::generate(STRING *string, const DateTz &object,
                          const GenerateConfiguration &configuration) {
  BSLS_ASSERT(string);
...
template <class STRING>
inline int Impl::generate(STRING *string, const TimeTz &object,
                          const GenerateConfiguration &configuration) {
  BSLS_ASSERT(string);
...
template <class STRING>
inline int Impl::generate(STRING *string, const DatetimeTz &object,
                          const GenerateConfiguration &configuration) {
  BSLS_ASSERT(string);
...
template <class STRING>
inline int Impl::generate(STRING *string, const Util::DateOrDateTz &object,
                          const GenerateConfiguration &configuration) {
  BSLS_ASSERT(string);
...
template <class STRING>
inline int Impl::generate(STRING *string, const Util::TimeOrTimeTz &object,
                          const GenerateConfiguration &configuration) {
  BSLS_ASSERT(string);
...
template <class STRING>
inline int Impl::generate(STRING *string,
...
int asciiPrefixToInt64(const char **nextPos, bsls::Types::Int64 *result,
...
int asciiToInt(const char **nextPos, int *result, const char *begin,
               const char *end)
// Convert the (unsigned) ASCII decimal integer starting at the specified
...
int parseDateRaw(const char **nextPos, int *year, int *month, int *day,
                 const char *begin, const char *end,
...
// parsing a date before 'end' is reached is not an error.
{
  ...
  const ssize_t minLength =
      basic ? sizeof "YYYYMMDD" - 1 : sizeof "YYYY-MM-DD" - 1;
...
int parseFractionalSecond(const char **nextPos, bsls::Types::Int64 *nanosecond,
                          const char *begin, const char *end,
...
// fractional second before 'end' is reached is not an error.
{
  ...
  tmp =
      (tmp + roundNanoseconds * 5) / (roundNanoseconds * 10) * roundNanoseconds;
...
int parseTimeRaw(const char **nextPos, int *hour, int *minute, int *second,
                 int *millisecond, bsls::Types::Int64 *microsecond,
                 bool *hasLeapSecond, const char *begin, const char *end,
...
// successfully parsing a time before 'end' is reached is not an error.
{
  ...
  const ssize_t minLength = basic ? sizeof "hhmmss" - 1 : sizeof "hh:mm:ss" - 1;
  ...
  if (p < end && ('.' == *p || ',' == *p)) {
    ...
    if (0 != u::parseFractionalSecond(&p, &nanosecond, p, end,
...
int parseZoneDesignator(const char **nextPos, int *minuteOffset,
                        const char *begin, const char *end)
...
int parseDate(Date *date, int *tzOffset, bool *hasZoneDesignator,
              const char *string, ssize_t length,
              ParseConfiguration configuration = ParseConfiguration())
...
// and a non-zero value (with no effect on the 'date') otherwise.
{
  ...
  const ssize_t minLength =
      configuration.basic() ? sizeof "YYYYMMDD" - 1 : sizeof "YYYY-MM-DD" - 1;
  ...
  *hasZoneDesignator = false;
  ...
  if (shuttle != end) {
    *hasZoneDesignator = true;
    if (0 != u::parseZoneDesignator(&shuttle, tzOffset, shuttle, end) ||
        shuttle != end) {
      return -1; // RETURN
    }
  }
...
int parseTime(Time *time, int *tzOffset, bool *hasZoneDesignator,
              const char *string, ssize_t length,
              ParseConfiguration configuration = ParseConfiguration())
// Parse the specified initial 'length' characters of the specified ISO
...
// value (with no effect on the 'time') otherwise.
{
  ...
  const ssize_t minLength =
      configuration.basic() ? sizeof "hhmmss" - 1 : sizeof "hh:mm:ss" - 1;
  ...
  if (0 != u::parseTimeRaw(&shuttle, &hour, &minute, &second, &millisecond,
                           &microsecond, &hasLeapSecond, shuttle, end, 1,
                           configuration) ||
...
int parseDatetime(Datetime *datetime, int *tzOffset, bool *hasZoneDesignator,
                  const char *string, ssize_t length,
                  ParseConfiguration configuration = ParseConfiguration())
// Parse the specified initial 'length' characters of the specified ISO
...
// 'datetime') otherwise.
{
  ...
  *hasZoneDesignator = false;
  ...
  if (0 != datetime->setDatetimeIfValid(year, month, day, hour, minute, second,
                                        millisecond,
...
int parseImp(Datetime *result, const char *string, ssize_t length,
             ParseConfiguration configuration = ParseConfiguration())
...
// non-zero value otherwise.
{
  ...
  if (0 != u::parseDatetime(&localDatetime, &tzOffset, &hasZoneDesignator,
...
int parseImp(DatetimeTz *result, const char *string, ssize_t length,
             ParseConfiguration configuration = ParseConfiguration())
// Parse the specified 'string' of length 'length' into the specified
// '*result', using the specified 'configuration'.  If
// 'configuration.basic()' is 'false', expect '-' and ':' in the 'Datetime'
...
int parseImp(Iso8601Util::DatetimeOrDatetimeTz *result, const char *string,
             ssize_t length,
             ParseConfiguration configuration = ParseConfiguration())
// Parse the specified 'string' of length 'length' into the specified
// '*result', using the specified 'configuration'.  If
...
inline int generateInt(char *buffer, int value, ssize_t paddedLen,
                       char separator)
// Write, to the specified 'buffer', the decimal string representation of
// the specified 'value' padded with leading zeros to the specified
...
int generateZoneDesignator(char *buffer, int tzOffset,
                           const GenerateConfiguration &configuration)
...
int generatedLengthForDateTzObject(ssize_t defaultLength, int tzOffset,
                                   const GenerateConfiguration &configuration)
...
int generatedLengthForDatetimeObject(ssize_t defaultLength,
                                     const GenerateConfiguration &configuration)
// Return the number of bytes generated, when the specified 'configuration'
...
int generatedLengthForDatetimeTzObject(
    ssize_t defaultLength, int tzOffset,
    const GenerateConfiguration &configuration)
// Return the number of bytes generated, when the specified 'configuration'
...
// '0 <= defaultLength'.
{
  ...
  return static_cast<int>(defaultLength - (6 - precision) -
                          (0 == precision ? 1 : 0));
}

int generatedLengthForTimeTzObject(ssize_t defaultLength, int tzOffset,
                                   const GenerateConfiguration &configuration)
// Return the number of bytes generated, when the specified 'configuration'
...
int parseIntervalImpl(bsls::Types::Int64 *weeks, bsls::Types::Int64 *days,
                      bsls::Types::Int64 *hours, bsls::Types::Int64 *minutes,
                      bsls::Types::Int64 *seconds,
                      bsls::Types::Int64 *nanoseconds, const char *string,
                      ssize_t length)
// Parse the specified initial 'length' characters of the specified ISO
...
// CLASS METHODS
int Iso8601Util::generate(char *buffer, ssize_t bufferLength,
...
int Iso8601Util::generate(char *buffer, ssize_t bufferLength,
  ...
  BSLS_ASSERT_SAFE(
      outLen == u::generatedLengthForTimeObject(k_TIME_STRLEN, configuration));
...
int Iso8601Util::generate(char *buffer, ssize_t bufferLength,
  ...
  BSLS_ASSERT_SAFE(outLen == u::generatedLengthForDatetimeObject(
                                 k_DATETIME_STRLEN, configuration));
...
int Iso8601Util::generate(char *buffer, ssize_t bufferLength,
  ...
  BSLS_ASSERT_SAFE(outLen == u::generatedLengthForDateTzObject(k_DATETZ_STRLEN,
                                                               object.offset(),
                                                               configuration));
...
int Iso8601Util::generate(char *buffer, ssize_t bufferLength,
  ...
  BSLS_ASSERT_SAFE(outLen == u::generatedLengthForTimeTzObject(k_TIMETZ_STRLEN,
                                                               object.offset(),
                                                               configuration));
...
int Iso8601Util::generate(char *buffer, ssize_t bufferLength,
  ...
  BSLS_ASSERT_SAFE(outLen ==
                   u::generatedLengthForDatetimeTzObject(
                       k_DATETIMETZ_STRLEN, object.offset(), configuration));
...
int Iso8601Util::generate(char *buffer, ssize_t bufferLength,
  ...
  if (object.is<Datetime>()) {
    if (bufferLength >= k_DATETIME_STRLEN) {
      outLen = generateRaw(buffer, object.the<Datetime>(), configuration);

      BSLS_ASSERT(outLen <= k_DATETIME_STRLEN);
    } else {
      char outBuf[k_DATETIME_STRLEN];

      outLen = generateRaw(outBuf, object.the<Datetime>(), configuration);
    ...
    if (bufferLength >= k_DATETIMETZ_STRLEN) {
      outLen = generateRaw(buffer, object.the<DatetimeTz>(), configuration);

      BSLS_ASSERT(outLen <= k_DATETIMETZ_STRLEN);
    } else {
      char outBuf[k_DATETIMETZ_STRLEN];

      outLen = generateRaw(outBuf, object.the<DatetimeTz>(), configuration);
...
int Iso8601Util::generate(bsl::string *string, const bsls::TimeInterval &object,
                          const GenerateConfiguration &configuration) {
...
int Iso8601Util::generate(std::string *string, const bsls::TimeInterval &object,
                          const GenerateConfiguration &configuration) {
  return u::Impl::generate(string, object, configuration);
}
...
int Iso8601Util::generateRaw(char *buffer, const DateTz &object,
  ...
  const int dateLen = generateRaw(buffer, object.localDate(), configuration);

  const int zoneLen = u::generateZoneDesignator(buffer + dateLen,
                                                object.offset(), configuration);
...
int Iso8601Util::generateRaw(char *buffer, const TimeTz &object,
  ...
  const int timeLen = generateRaw(buffer, object.localTime(), configuration);
...
int Iso8601Util::generateRaw(char *buffer, const DatetimeTz &object,
  ...
  const int datetimeLen =
      generateRaw(buffer, object.localDatetime(), configuration);
...
int Iso8601Util::parse(bsls::TimeInterval *result, const char *string,
                       ssize_t length) {
  BSLS_ASSERT(result);
  BSLS_ASSERT(string);
  BSLS_ASSERT(0 <= length);
  ...
  if (0 != u::parseIntervalImpl(&weeks, &days, &hours, &minutes, &seconds,
                                &nanoseconds, string, length)) {
    return -1; // RETURN
  }
...
int Iso8601Util::parse(Date *result, const char *string, ssize_t length,
...
int Iso8601Util::parse(Time *result, const char *string, ssize_t length,
                       ParseConfiguration configuration) {
  BSLS_ASSERT(result);
  BSLS_ASSERT(string);
  BSLS_ASSERT(0 <= length);
  ...
  if (0 != u::parseTime(&localTime, &tzOffset, &hasZoneDesignator, string,
...
int Iso8601Util::parse(Datetime *result, const char *string, ssize_t length,
                       ParseConfiguration configuration) {
  BSLS_ASSERT(result);
  BSLS_ASSERT(string);
  BSLS_ASSERT(0 <= length);
...
int Iso8601Util::parse(DateTz *result, const char *string, ssize_t length,
                       ParseConfiguration configuration) {
  BSLS_ASSERT(result);
  BSLS_ASSERT(string);
  BSLS_ASSERT(0 <= length);
  ...
  if (0 != u::parseDate(&localDate, &tzOffset, &hasZoneDesignator, string,
                        length, configuration)) {
    return -1; // RETURN
  }
...
int Iso8601Util::parse(TimeTz *result, const char *string, ssize_t length,
                       ParseConfiguration configuration) {
  BSLS_ASSERT(result);
  BSLS_ASSERT(string);
  BSLS_ASSERT(0 <= length);
...
int Iso8601Util::parse(DatetimeTz *result, const char *string, ssize_t length,
                       ParseConfiguration configuration) {
  BSLS_ASSERT(result);
  BSLS_ASSERT(string);
  BSLS_ASSERT(0 <= length);
...
int Iso8601Util::parse(DateOrDateTz *result, const char *string, ssize_t length,
                       ParseConfiguration configuration) {
  BSLS_ASSERT(result);
  BSLS_ASSERT(string);
  BSLS_ASSERT(0 <= length);
...
int Iso8601Util::parse(TimeOrTimeTz *result, const char *string, ssize_t length,
                       ParseConfiguration configuration) {
  BSLS_ASSERT(result);
  BSLS_ASSERT(string);
  BSLS_ASSERT(0 <= length);
...
int Iso8601Util::parse(DatetimeOrDatetimeTz *result, const char *string,
                       ssize_t length, ParseConfiguration configuration) {
  BSLS_ASSERT(result);
  BSLS_ASSERT(string);
  BSLS_ASSERT(0 <= length);
...
int Iso8601Util::parseRelaxed(Datetime *result, const char *string,
                              ssize_t length) {
  BSLS_ASSERT(result);
  BSLS_ASSERT(string);
  BSLS_ASSERT(0 <= length);
...
int Iso8601Util::parseRelaxed(DatetimeTz *result, const char *string,
                              ssize_t length) {
  BSLS_ASSERT(result);
  BSLS_ASSERT(string);
  BSLS_ASSERT(0 <= length);
...
int Iso8601Util::parseRelaxed(DatetimeOrDatetimeTz *result, const char *string,
                              ssize_t length) {
  BSLS_ASSERT(result);
  BSLS_ASSERT(string);
  BSLS_ASSERT(0 <= length);
