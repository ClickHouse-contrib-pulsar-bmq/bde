...
#define U_SAFE_COPY(dstBuf, srcBuf)                                            \
  do {                                                                         \
    enum { k_STATIC_ASSERT = 1 / (sizeof(srcBuf) <= sizeof(dstBuf)) };         \
    std::strcpy(dstBuf, srcBuf);                                               \
  } while (false)

void substitute(char *buffer, const char *pattern, const char *replacement)
...
//:   with ">>".
{
  ...
  for (char *pc = buffer; (pc = std::strstr(pc, pattern)); ++pc) {
    ...
    std::memmove(pc + replacementLen, remainder, std::strlen(remainder) + 1);
  }
}
...
// CLASS METHOD
const char *NameOf_Base::initBuffer(char *buffer, const char *functionName)
...
// the 'NameOf' constructor.
{
  ...
  static const char uselessPreamble[] = {"__cdecl BloombergLP::bsls::NameOf<"};
#else
  static const char uselessPreamble[] = {
      "__thiscall BloombergLP::bsls::NameOf<"};
#endif
#else
  static const char uselessPreamble[] = {"BloombergLP::bsls::NameOf<"};
#endif

  static const char compileTimeAssertArray[sizeof(uselessPreamble) ==
                                           k_USELESS_PREAMBLE_LEN + 1] = {0};
  ...
  if (std::strncmp(uselessPreamble, functionName, k_USELESS_PREAMBLE_LEN)) {
  ...
  std::strncpy(buffer, functionName + k_USELESS_PREAMBLE_LEN,
