...
template <class TYPE> struct Pretty {
  // DATA
...
int main(int argc, char *argv[]) {
  ...
  case 2: {
    ...
    // Next, we see that the 'NameOf' template class, when created with a type,
    // can be implicitly cast to a 'const char *' which points to a description
    // of the type.
    //..
    ASSERT(!std::strcmp("double", NameOf<double>()));
    ASSERT(!std::strcmp("int", NameOf<int>()));
    //..
    // Then, we see that when 'NameOf' is passed a 'typedef' or template
    // parameter, it resolves it to the original type:
    //..
    typedef int Woof;
    //
    ...
    //..
    // There is a problem with template code not knowing how to implicitly cast
    // the 'NameOf' type to 'const char *' for initializing or comparing with
    ...
    //..
    // Note that 'nameOfType' naturally returns a 'const char *' and needs no
    // help casting.  Note also that 'bsls::debugprint' is able to figure out
    // how to cast 'NameOf' directly to 'const char *' with no problems, as can
    // iostreams, so there is no problem with putting a 'NameOf' in a
    // 'LOOP_ASSERT' or 'ASSERTV'.  It is anticipated that displaying by the BDE
    // 'ASSERTV', 'LOOP_ASSERT, and 'P' macros will be the primary use of this
    // component.
    //..
    if (verbose) {
...
#error No function signature macro defined.
    ...
    ASSERTV(nameOfType(ii), NameOf<int>(), eq(nameOfType(ii), NameOf<int>()));
    ...
    ASSERTV(
        ...
        eq("bsls::NameOf<bsls::Stopwatch>", NameOf<NameOf<bsls::Stopwatch>>()));
  } break;
  default: {
