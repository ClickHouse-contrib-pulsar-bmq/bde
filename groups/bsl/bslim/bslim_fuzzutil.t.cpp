...
// [ 8] void
// consumeRandomLengthString(std::experimental::pmr::string*,FDV*,maxLen); [ 9]
// void consumeRandomLengthChars(bsl::vector<char>*,FDV*,maxLen); [ 9] void
// consumeRandomLengthChars(std::vector<char>*,FDV*,maxLen); [ 9] void
// consumeRandomLengthChars(pmr::vector<char>*,FDV*,maxLen);
// ----------------------------------------------------------------------------
// [10] USAGE EXAMPLE
// [ 1] BREATHING TEST
...
void generateBytes(bsl::uint8_t *buffer, bsl::size_t bufLen,
                   bsl::size_t numBytes)
...
template <class INTEGRAL> struct ConsumeIntegralTest {
  ...
  static void
  testConsumeIntegralInRange(ConsumeIntegralTestCases<INTEGRAL> *DATA,
                             int NUM_DATA)
  ...
  // the specified 'DATA' of specified 'NUM_DATA' size.
  {
    ...
    for (int ti = 0; ti < NUM_DATA; ++ti) {
      ...
      INTEGRAL i = Util::consumeNumberInRange<INTEGRAL>(&fdv, DATA[ti].d_min,
                                                        DATA[ti].d_max);

      bsl::size_t afterLength = fdv.length();

      if (veryVerbose) {
        T_ P_(LINE) P_(NUM_BYTES) P_(BYTES_REQ) P(i);
      }

      ASSERT(DATA[ti].d_max >= i);
      ASSERT(DATA[ti].d_min <= i);

      ASSERT(DATA[ti].d_numBytesRequired == beforeLength - afterLength);
...
//                  // '\' is forbidden.
//-----------------------------------------------------------------------------
int ggg(bsl::vector<bsl::uint8_t> *buf, const bsl::string_view &spec,
...
void gg(bsl::vector<bsl::uint8_t> *buf, const bsl::string_view &spec)
...
int main(int argc, char *argv[]) {
  ...
  case 10: {
    ...
    //..
    // Now, we pass this 'FuzzDataView' to 'FuzzUtil' to generate values within
    // the permissible range of the function under test:
    //..
  ...
  case 9: {
    ...
    //:     'std::experimental::pmr::vector<char>' -- with the same underlying
    //:     byte array, and verify that the above concerns are addressed in each
    //:     case.  (C-5)
    //
    // Testing:
...
#undef makeSV
    ...
    for (int ti = 0; ti < NUM_DATA; ++ti) {
      ...
      // 3 blocks to test the 3 overloads (C-5)
      {
        ...
        ASSERT(bsl::equal(result.begin(), result.end(), expected.begin()));
  ...
  case 8: {
    ...
    //:
    //:   5 Construct three 'fuzzDataView' objects -- one each for
    //:     'bsl::string', 'std::string', and 'std::experimental::pmr::string'
    //:     -- with the same underlying byte array, and verify that the above
    ...
    //   void
    //   consumeRandomLengthString(std::experimental::pmr::string*,FDV*,maxLen);
    // --------------------------------------------------------------------
    if (verbose)
      cout << endl
...
#undef makeSV
  ...
  case 7: {
...

    {
      ...
      const double DENORM_MIN = bsl::numeric_limits<double>::denorm_min();
      ...
      } DATA[] = {// LINE     NUM_BYTES     MIN             MAX   EXP
                  //----    ---------     ---            ---   ---
                  {L_, 0, 0, 0, 0},
                  {L_, 0, 0, DENORM_MIN, 0},
                  {L_, 1, LOW_DOUBLE, 0, 1},
      ...
      for (int ti = 0; ti < NUM_DATA; ++ti) {
        ...
        bsl::size_t beforeLength = fdv.length();
        double d = Util::consumeNumberInRange<double>(&fdv, DATA[ti].d_min,
                                                      DATA[ti].d_max);
        bsl::size_t afterLength = fdv.length();
    ...
    }
    {
      ...
      static const struct {
        ...
        float d_max;       // 'max' method argument
                  ...
                  //----     ---------     ---             ---
                  {L_, 0, 0, 0},         {L_, 0, 0, DENORM_MIN},
                  {L_, 1, LOW_FLOAT, 0}, {L_, 2, 0, HIGH_FLOAT},
                  {L_, 4, -5.32f, 0},    {L_, 8, 0, 55.32f},
                  {L_, 8, 100, 100},     {L_, 12, LOW_FLOAT, HIGH_FLOAT}};
      ...
      for (int ti = 0; ti < NUM_DATA; ++ti) {
        ...
        bslim::FuzzDataView fdv(KB_DATA + ti, NUM_BYTES);
        float d = Util::consumeNumberInRange<float>(&fdv, DATA[ti].d_min,
                                                    DATA[ti].d_max);
    ...
    }
    {
      ...
      for (int i = 0; i < NUM_TIMES; i++) {
        double d = Util::consumeNumberInRange<double>(&fdv, 0.0, 1.0);
        mean += d;
        variance += d * d;
        if (veryVeryVerbose) {
  ...
  case 6: {
           ...
           << "======================================" << endl;
    {
      ...
      for (bsl::size_t ti = 0; ti < NUM_ITERATIONS; ++ti) {
        ...
        bsl::size_t numBytes =
            Util::consumeNumberInRange<bsl::size_t>(&fdv_numBytes, 0, 15);
        ...
        if (0 == numBytes) {
          ...
          bsl::size_t numBytesConsumed = numBytes - fdv_double.length();
          ASSERT(bsl::min(DOUBLE_SIZE_REQ, numBytes) == numBytesConsumed);
        }
      }
    }
    {
      ...
      for (bsl::size_t ti = 0; ti < NUM_ITERATIONS; ++ti) {
        bslim::FuzzDataView fdv_numBytes(KB_DATA + ti, 1);
        bsl::size_t numBytes =
            Util::consumeNumberInRange<bsl::size_t>(&fdv_numBytes, 0, 7);
        ...
        if (0 == numBytes) {
          ...
          bsl::size_t numBytesConsumed = numBytes - fdv_float.length();
          ASSERT(bsl::min(FLOAT_SIZE_REQ, numBytes) == numBytesConsumed);
        }
      }
    }
  } break;
  ...
  case 4: {
    ...
    RUN_EACH_TYPE(ConsumeIntegralTest, testConsumeIntegral, int, unsigned int,
                  short, unsigned short, long, unsigned long,
                  bsls::Types::Int64, bsls::Types::Uint64, char, signed char,
                  unsigned char);
  } break;
  case 3: {
    // --------------------------------------------------------------------
           ...
           << "=======================================" << endl;
    {
      ...
      ConsumeIntegralTestCases<int> DATA_INT[] = {
      ...
      ConsumeIntegralTest<int>::testConsumeIntegralInRange(DATA_INT, NUM_DATA);
    }
    {
      if (verbose)
      ...
      ConsumeIntegralTestCases<unsigned int> DATA_UINT[] = {
          // LINE   LENGTH     MIN     MAX       BYTES
          //----  -------     ---     ---      ------
          {L_, 0, 0, 0, 0},
          {L_, 1, 0, 0, 0},
      ...
      ConsumeIntegralTest<unsigned int>::testConsumeIntegralInRange(DATA_UINT,
                                                                    NUM_DATA);
    }
    {
      if (verbose)
      ...
      ConsumeIntegralTestCases<short> DATA_SHRT[] = {
          // LINE   LENGTH     MIN     MAX       BYTES
          //----  -------     ---     ---      ------
          {L_, 0, 0, 0, 0},
          {L_, 1, 0, 0, 0},
    ...
    }
    {
      ...
      unsigned short MIN_USHRT = bsl::numeric_limits<unsigned short>::min();
      unsigned short MAX_USHRT = bsl::numeric_limits<unsigned short>::max();
      ConsumeIntegralTestCases<unsigned short> DATA_USHRT[] = {
          ...
          {L_, 12, 0, 512, 2},
          {L_, 12, 0, 65535, 2},
          {L_, 12, MIN_USHRT, static_cast<unsigned short>(MIN_USHRT + 10000),
           2},
          {L_, 12, static_cast<unsigned short>(MAX_USHRT - 10000), MAX_USHRT,
           2},
          {L_, 12, 0, MAX_USHRT, 2},
          {L_, 12, MIN_USHRT, 0, 0},
    ...
    }
    {
      ...
      ConsumeIntegralTestCases<long> DATA_LONG[] = {
          // LINE   LENGTH     MIN     MAX      BYTES
          //----  -------     ---     ---      ------
          {L_, 0, 0, 0, 0},
          {L_, 1, 0, 0, 0},
    ...
    }
    {
      ...
      const unsigned long MAX_ULONG = bsl::numeric_limits<unsigned long>::max();
      const unsigned long MIN_ULONG = bsl::numeric_limits<unsigned long>::min();
      bsl::size_t sizeOfULong = sizeof(unsigned long);
      ConsumeIntegralTestCases<unsigned long> DATA_ULONG[] = {
          // LINE   LENGTH     MIN     MAX     BYTES
          //----  -------     ---     ---      ------
          {L_, 0, 0, 0, 0},
          {L_, 1, 0, 0, 0},
      ...
      ConsumeIntegralTest<unsigned long>::testConsumeIntegralInRange(DATA_ULONG,
                                                                     NUM_DATA);
    }
    {
      if (verbose)
      ...
      ConsumeIntegralTestCases<bsls::Types::Int64> DATA_INT64[] = {
          // LINE   LENGTH     MIN     MAX     BYTES
          //----  -------     ---     ---      ------
          {L_, 0, 0, 0, 0},
          {L_, 1, 0, 0, 0},
      ...
      ConsumeIntegralTest<bsls::Types::Int64>::testConsumeIntegralInRange(
          DATA_INT64, NUM_DATA);
    }
    {
      if (verbose)
      ...
      ConsumeIntegralTestCases<bsls::Types::Uint64> DATA_UINT64[] = {
          // LINE   LENGTH     MIN     MAX     BYTES
          //----  -------     ---     ---      ------
          {L_, 0, 0, 0, 0},
          {L_, 1, 0, 0, 0},
      ...
      ConsumeIntegralTest<bsls::Types::Uint64>::testConsumeIntegralInRange(
          DATA_UINT64, NUM_DATA);
    }
    {
      if (verbose)
        cout << endl << "<char>" << endl;
      ...
      ConsumeIntegralTestCases<char> DATA_CHAR[] = {
          // LINE   LENGTH     MIN     MAX     BYTES
          //----  -------     ---     ---      ------
          {L_, 0, 0, 0, 0},
          {L_, 1, 0, 0, 0},
    ...
    }
    {
      ...
      const signed char MAX_SCHAR = bsl::numeric_limits<signed char>::max();
      const signed char MIN_SCHAR = bsl::numeric_limits<signed char>::min();

      ConsumeIntegralTestCases<signed char> DATA_SCHAR[] = {
          // LINE   LENGTH     MIN     MAX     BYTES
          //----  -------     ---     ---      ------
          {L_, 0, 0, 0, 0},
          {L_, 1, 0, 0, 0},
      ...
      ConsumeIntegralTest<signed char>::testConsumeIntegralInRange(DATA_SCHAR,
                                                                   NUM_DATA);
    }
    {
      if (verbose)
      ...
      const unsigned char MAX_UCHAR = bsl::numeric_limits<unsigned char>::max();
      const unsigned char MIN_UCHAR = bsl::numeric_limits<unsigned char>::min();
      ASSERT(0 == MIN_UCHAR);
      ConsumeIntegralTestCases<unsigned char> DATA_UCHAR[] = {
          // LINE   LENGTH     MIN     MAX     BYTES
          //----  -------     ---     ---      ------
          {L_, 0, 0, 0, 0},
          {L_, 1, 0, 0, 0},
      ...
      ConsumeIntegralTest<unsigned char>::testConsumeIntegralInRange(DATA_UCHAR,
                                                                     NUM_DATA);
    }
  } break;
  case 2: {
...
#undef makeSV
    ...
    for (int ti = 0; ti < NUM_DATA; ++ti) {
      ...
      if (ggg(&testBuf, SPEC, false) < 0) {
        ...
        ASSERT(bsl::equal(testBuf.begin(), testBuf.end(), EXPECTED.begin()));
      } else {
        ASSERT(false == VALID);
      }
      if (veryVerbose) {
  ...
  case 1: {
    ...
    const uint8_t stringData[] = {
        0x61, 0x63, 0x65, 0x67, 0x69, 0x21, 0x44, 0x46, 0x48, 0x50, 0x52, 0x54,
        0x42, 0x22, 0x72, 0x47, 0x49, 0x51, 0x53, 0x55, 0x56, 0x57, 0x58, 0x59};
      ...
      cout << endl << "BREATHING TEST" << endl << "==============" << endl;
    {
      ...
      if (veryVerbose) {
        cout << "Testing 'consumeNumberInRange<INTEGRAL>' 0, 0" << endl;
      ...
      if (veryVerbose) {
        cout << "Testing 'consumeNumberInRange<INTEGRAL>' 1, 1000" << endl;
        cout << "\tfdv.length(): " << fdv.length() << endl;
      }
      for (int j = 0; j < testIters; ++j) {
      ...
      if (veryVerbose) {
        cout << "Testing 'consumeNumberInRange<INTEGRAL>' -1000, 1" << endl;
        cout << "\tfdv.length(): " << fdv.length() << endl;
      }
      for (int j = 0; j < testIters; ++j) {
