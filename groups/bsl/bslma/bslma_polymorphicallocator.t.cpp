...
// platform library's 'std::experimental::pmr::polymorphic_allocator' template
// is used, so conformance to the C++17 standard (C++17 section 20.5.3.5
...
void *EqTestResource::do_allocate(std::size_t, std::size_t) { return 0; }
...
bool EqTestResource::do_is_equal(const bsl::memory_resource &other) const
  ...
  const EqTestResource *pOther = dynamic_cast<const EqTestResource *>(&other);
  if (pOther) {
...
template <class ALLOCATOR> class TestType {
...
public:
  ...
  TestType(bsl::allocator_arg_t, const ALLOCATOR &alloc = ALLOCATOR(),
           long a01 = 0, long a02 = 0, long a03 = 0, long a04 = 0, long a05 = 0,
           long a06 = 0, long a07 = 0, long a08 = 0, long a09 = 0, long a10 = 0,
...
template <class ALLOCATOR>
TestType<ALLOCATOR>::TestType(long a01, long a02, long a03, long a04, long a05,
                              long a06, long a07, long a08, long a09, long a10,
                              long a11, long a12, long a13, long a14)
    : d_value(a01 + a02 + a03 + a04 + a05 + a06 + a07 + a08 + a09 + a10 + a11 +
              a12 + a13 + a14) {}

template <class ALLOCATOR>
TestType<ALLOCATOR>::TestType(bsl::allocator_arg_t, const ALLOCATOR &alloc,
                              long a01, long a02, long a03, long a04, long a05,
                              long a06, long a07, long a08, long a09, long a10,
                              long a11, long a12, long a13, long a14)
    : d_allocator(alloc), d_value(a01 + a02 + a03 + a04 + a05 + a06 + a07 +
...
class CountingResource : public bsl::memory_resource {
  ...
  CountingResource &operator=(const CountingResource &) BSLS_KEYWORD_DELETED;
...
CountingResource::~CountingResource() { BSLS_ASSERT(0 == d_blocksOutstanding); }

void *CountingResource::do_allocate(std::size_t bytes, std::size_t alignment) {
...
public:
  ...
  Holder(bslmf::MovableRef<Holder> other, const allocator_type &allocator);
  ~Holder();
  //..
...
Holder<TYPE>::Holder(const TYPE &value, const allocator_type &allocator)
  ...
  BSLS_TRY { ::new (d_data_p) TYPE(value); }
  BSLS_CATCH(...) {
...
//..
template <class TYPE> Holder<TYPE>::~Holder() {
  if (!isEmpty()) {
...
#define TEST(EXP, ...)                                                         \
  do {                                                                         \
    ...
    TEST(0x00ff, 0x0001, 0x0002, 0x0004, 0x0008, 0x0010, 0x0020, 0x0040,
         0x0080);
    TEST(0x01ff, 0x0001, 0x0002, 0x0004, 0x0008, 0x0010, 0x0020, 0x0040, 0x0080,
         0x0100);
    TEST(0x03ff, 0x0001, 0x0002, 0x0004, 0x0008, 0x0010, 0x0020, 0x0040, 0x0080,
         0x0100, 0x0200);
    TEST(0x07ff, 0x0001, 0x0002, 0x0004, 0x0008, 0x0010, 0x0020, 0x0040, 0x0080,
         0x0100, 0x0200, 0x0400);
    TEST(0x0fff, 0x0001, 0x0002, 0x0004, 0x0008, 0x0010, 0x0020, 0x0040, 0x0080,
         0x0100, 0x0200, 0x0400, 0x0800);
    TEST(0x1fff, 0x0001, 0x0002, 0x0004, 0x0008, 0x0010, 0x0020, 0x0040, 0x0080,
         0x0100, 0x0200, 0x0400, 0x0800, 0x1000);
    TEST(0x3fff, 0x0001, 0x0002, 0x0004, 0x0008, 0x0010, 0x0020, 0x0040, 0x0080,
...
#undef TEST
  ...
  case 8: {
    ...
    for (int i = 0; i < numResources; ++i) {
      for (int j = i; j < numResources; ++j) { // 'j' can equal 'i'
        ...
        const bsl::memory_resource *expLastEqArg = (&r1 != &r2) ? &r2 : 0;
  ...
  case 6: {
    ...
    if (verbose)
      printf("\tTesting 'const_pointer'.\n");
    {
      ASSERT((bsl::is_same<TI::const_pointer, AI::const_pointer>::value));
      ASSERT((bsl::is_same<TF::const_pointer, AF::const_pointer>::value));
    }

    if (verbose)
      printf("\tTesting 'void_pointer'.\n");
    {
    ...
    if (verbose)
      printf("\tTesting 'const_void_pointer'.\n");
    { ASSERT((bsl::is_same<TI::const_void_pointer, const void *>::value)); }

    if (verbose)
      printf("\tTesting 'difference_type'.\n");
    {
      ASSERT((bsl::is_same<TI::difference_type, AI::difference_type>::value));
      ASSERT((bsl::is_same<TF::difference_type, AF::difference_type>::value));
    }

    if (verbose)
      printf("\tTesting 'size_type'.\n");
    {
      ...
      printf("\tTesting 'rebind_traits'.\n");
    {
      ...
      ASSERT(
          (bsl::is_same<TI::rebind_traits<long>::allocator_type, AL>::value));

      ASSERT((bsl::is_convertible<TF::rebind_traits<long>, TL>::value));
      ASSERT(
          (bsl::is_same<TF::rebind_traits<long>::allocator_type, AL>::value));
#endif
    }
  ...
  case 5: {
    ...
    if (verbose)
      printf("\tTesting 'const_reference'.\n");
    {
  ...
  case 3: {
    ...
    bsl::polymorphic_allocator<int> ai2(&ta);
    ASSERT(&ta == ai2);
    bsl::polymorphic_allocator<int> ai3(ai2);
    ASSERT(&ta == ai3);
  ...
  case 1: {
    ...
    ASSERT(daAlloc == Traits::select_on_container_copy_construction(taAlloc));
