...
protected:
  ...
  bool do_is_equal(const bsl::memory_resource &) const BSLS_KEYWORD_NOEXCEPT
...
void MemoryResourceProtocolTest::do_deallocate(void *, std::size_t,
                                               std::size_t) {
  markDone();
}
...
class PassthroughTest : public bsl::memory_resource {
  ...
  void do_deallocate(void *p, size_t bytes,
                     size_t alignment) BSLS_KEYWORD_OVERRIDE;
...
public:
  // CREATORS
  PassthroughTest()
      : d_lastBytesArg(0), d_lastAlignmentArg(0), d_lastBlock_p(0),
        d_lastOther_p(0) {}
...
protected:
  ...
  //..
  // Now we can declare the public interface, comprising the default
  // constructor, the destructor, and an accessor to return the current block
  // count; all other public members are inherited from the base class:
  //..
public:
  // CREATORS
...
template <class TYPE> class PolymorphicAllocator {
...
template <class TYPE> class Holder {
  ...
  // 'std::experimental::pmr::polymorphic_allocator'), the allocator is
  // *required* for all constructors except the move constructor:
  //..
public:
  // TYPES
...
int main(int argc, char *argv[]) {
  ...
  case 3: {
    ...
    ASSERT(obj.lastAlignmentArg() == bsls::AlignmentUtil::BSLS_MAX_ALIGNMENT);
    ASSERT(obj.lastBlock() == p);
