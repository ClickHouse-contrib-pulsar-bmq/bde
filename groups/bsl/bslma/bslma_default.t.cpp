...
// CREATORS
inline my_CountingAllocator::my_CountingAllocator()
    : d_numBlocksInUse(0), d_numBlocksTotal(0) {}

// ACCESSORS
inline int my_CountingAllocator::numBlocksInUse() const {
  return d_numBlocksInUse;
}

inline int my_CountingAllocator::numBlocksTotal() const {
...
// CREATORS
inline my_Id::my_Id(const char *id, bslma::Allocator *basicAllocator)
    : d_allocator_p(bslma::Default::allocator(basicAllocator)) {
  d_buffer_p = (char *)d_allocator_p->allocate(strlen(id) + 1);
  strcpy(d_buffer_p, id);
}

inline my_Id::my_Id(const my_Id &original, bslma::Allocator *basicAllocator)
    : d_allocator_p(bslma::Default::allocator(basicAllocator)) {
...
// ACCESSORS
inline const char *my_Id::id() const { return d_buffer_p; }
//..
...
public:
  // CREATORS
  my_IdPair(const char *id, const char *alias,
...
// CREATORS
inline my_IdPair::my_IdPair(const char *id, const char *alias,
                            bslma::Allocator *basicAllocator)
    : d_id(id, bslma::Default::allocator(basicAllocator)),
      d_alias(alias) // drat! (forgot to pass along 'basicAllocator')
{}

inline my_IdPair::~my_IdPair() {}

// ACCESSORS
inline const char *my_IdPair::alias() const { return d_alias.id(); }

inline const char *my_IdPair::id() const { return d_id.id(); }
//..
...
private:
  // PRIVATE CREATORS
  explicit my_Singleton(const char *id, bslma::Allocator *basicAllocator = 0);
...
// CLASS METHODS
inline const my_Singleton &my_Singleton::singleton() { return *s_singleton_p; }

// CREATORS
inline my_Singleton::my_Singleton(const char *id,
                                  bslma::Allocator *basicAllocator)
    : d_id(id, bslma::Default::globalAllocator(basicAllocator)) {}

inline my_Singleton::~my_Singleton() {}

// ACCESSORS
inline const char *my_Singleton::id() const { return d_id.id(); }
//..
...
void my_Singleton::initSingleton(const char *id,
  ...
  s_singleton_p = new (singleton.buffer()) my_Singleton(id, basicAllocator);
}
//..
...
int main(int argc, char *argv[]) {
  ...
  my_CountingAllocator mU;
  bslma::Allocator *U = &mU;
  my_CountingAllocator mV;
  bslma::Allocator *V = &mV;
  ...
  case 12: {
    ...
    // instances of 'my_CountingAllocator'.  Note that the default allocator is
    ...
    // desired allocator in the call to 'initSingleton' to make our intentions
    // as clear as possible.  Of course, because of the way the 'my_Singleton'
    // constructor was written, the result would have been the same if no
    // allocator had been specified.  As in previous examples, the states of the
    // default and global allocators are asserted before and after initializing
    // the singleton:
    //..
  ...
  case 11: {
    ...
    //
    // Next, the default allocator is set and locked identically to what was
    // done in usage example 1:
    //..
    static my_CountingAllocator defaultCountingAllocator;
    ...
    ASSERT(bslma::Default::defaultAllocator() == &defaultCountingAllocator);
    //..
    // Now we instantiate an instance of 'my_IdPair' without explicitly
    // specifying an allocator.  As a result, the object uses the default
    // allocator.  The assertions verify the expected changes in the state of
    // the default allocator:
    //..
    ASSERT(0 == defaultCountingAllocator.numBlocksInUse());
    ASSERT(0 == defaultCountingAllocator.numBlocksTotal());
    ...
    ASSERT(2 == defaultCountingAllocator.numBlocksTotal());
    //..
    // Next we instantiate a second instance of 'my_IdPair', this time supplying
    // it with an instance of the counting allocator that is distinct from the
    // default allocator.  The assertions in the following code fragment that
    // are commented out indicate the *expected* states of the allocators (i.e.,
    // in a bug-free implementation of 'my_IdPair') after the object has been
    // constructed and again after it has been destroyed.  However, due to the
    // (intentional) bug in the constructor, the uncommented assertions reveal
    ...
    // within the body of a constructor, or any other member function, to
    // allocate dynamic memory that is *temporarily* needed by the method (and,
    // hence, not owned by the object after the method has returned).  Thus, the
    // invariant that must hold immediately after a method of an object returns
    // is that the value returned by 'defaultCountingAllocator.numBlocksInUse()'
    // must be *identical* to what it was immediately prior to calling the
    // method.  Of course, note that the above invariant pertains to cases in
    // *single*-threaded programs where the object allocator in use by the
    // instance is *distinct* from the default allocator.  Also note that the
    // value returned by 'defaultCountingAllocator.numBlocksTotal()' *can*
    // differ across function invocations (i.e., even with correct code).
  ...
  case 10: {
    ...
    // Next we set the default allocator to an instance of our counting
    // allocator. Note that immediately after successfully setting it, we lock
    // the default allocator, so that subsequent calls to
    // 'bslma::Default::setDefaultAllocator' fail.  (The default allocator can
    // still be modified by calling 'bslma::Default::setDefaultAllocatorRaw',
    // but calling that function in production code is anti-social.  Our usage
    // examples expressly do *not* call that method.)  With the possible
    // exception of test drivers, the default allocator should be set and locked
    // early in 'main' before threads are started and before objects are
    // initialized:
    //..
    static my_CountingAllocator defaultCountingAllocator;
    ...
    // instance, 'idA', is not supplied with an allocator, so it uses the
    // default allocator.  The second instance, 'idB', is supplied with an
    // instance of 'my_CountingAllocator'.  The assertions track the states of
    // the two allocators at each point in the code fragment.  In particular,
    // note that the state of the default allocator does not change during the
    // lifetime of 'idB':
    //..
    ASSERT(0 == defaultCountingAllocator.numBlocksInUse());
    ASSERT(0 == defaultCountingAllocator.numBlocksTotal());
  ...
  case 4: {
    ...
    if (veryVerbose)
      printf("\tIgnoring 'allocator' side-effects.\n");
  ...
  case 3: {
    ...
    //   5) (C++17) 'std::experimental::pmr::get_default_resource' returns the
    //   value of
    ...
    //   'defaultAllocator' and 'std::experimental::pmr::get_default_resource'
    //   return the value that was passed to the most recent *successful* call
    //   to 'setDefaultAllocator'.  Include individual tests to verify that a
  ...
  case 2: {
    ...
    //      to 'setDefaultAllocatorRaw',
    //      'std::experimental::pmr::get_default_resource' returns a pointer to
    //      the same object as 'defaultAllocator'.
    //
    // Plan:
    //   Call 'defaultAllocator' in the first substantive line of the test
    ...
    //   'std::experimental::pmr::get_default_resource' return the expected
    //   results. Note that side-effects of 'defaultAllocator' are ignored in
    //   this test case.
    //
    // Testing:
  ...
  case 1: {
    ...
    if (veryVerbose)
      printf("\tBreathing default allocator.\n");
    ...
    if (veryVerbose)
      printf("\tBreathing global allocator.\n");
