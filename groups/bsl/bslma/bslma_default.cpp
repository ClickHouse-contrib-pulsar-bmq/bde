...
Allocator *Default::determineAndReturnDefaultAllocator() {
  ...
  void *installed =
      bsls::AtomicOperations::testAndSwapPtr(&s_defaultAllocator, 0, requested);
  if (!installed) {
...
int Default::setDefaultAllocator(Allocator *basicAllocator) {
  ...
  void *const previouslyRequested = bsls::AtomicOperations::swapPtr(
      &s_requestedDefaultAllocator, basicAllocator);
...
Allocator *Default::setGlobalAllocator(Allocator *basicAllocator) {
  Allocator *previous = (Allocator *)bsls::AtomicOperations::swapPtrAcqRel(
      &s_globalAllocator, basicAllocator);

  return previous ? previous : &NewDeleteAllocator::singleton();
}
