...
// allocator support using 'std::experimental::pmr::polymorphic_allocator'
// instead of type erasure is currently part of version 3 of the Library
// Fundamentals Technical Specification (LFTS 3), after acceptance of paper
// P0987 (http://wg21.link/P0987).  This component follows the P0987
// specification, substituting 'bsl::allocator' for
...
template <class PROTOTYPE> struct Function_ArgTypes {
...
template <class RET, class ARG> struct Function_ArgTypes<RET(ARG)> {
  ...
  // PUBLIC TYPES
  BSLS_DEPRECATE_FEATURE("bsl", "deprecated_cpp17_standard_library_features",
                         "do not use")
...
private:
  ...
  Function_Variadic &operator=(const Function_Variadic &) BSLS_KEYWORD_DELETED;
...
template <class PROTOTYPE, class FUNC> struct Function_IsInvocableWithPrototype;
...
private:
  ...
  template <class TYPE> struct Decay : MovableRefUtil::Decay<TYPE> {
  ...
  struct IsInvocableWithPrototype
      : BloombergLP::bslstl::Function_IsInvocableWithPrototype<PROTOTYPE,
                                                               FUNC> {
  ...
  bool operator!=(const function &)
      const; // Declared but not defined
             // Since 'function' does not support 'operator==' and 'operator!=',
             // they must be deliberately suppressed; otherwise 'function'
             // objects would be implicitly comparable by implicit conversion to
             // 'UnspecifiedBool'.
...
public:
  ...
  function(allocator_arg_t, const allocator_type &allocator,
  ...
  template <class FUNC>
  function(
      BSLS_COMPILERFEATURES_FORWARD_REF(FUNC) func, // IMPLICIT
      typename enable_if<
  ...
  template <class FUNC>
  function(
      allocator_arg_t, const allocator_type &allocator,
...
#ifdef BSLS_PLATFORM_CMP_IBM
  template <class FUNC>
  function(allocator_arg_t, const allocator_type &allocator, FUNC *func,
           typename enable_if<is_function<FUNC>::value, int>::type = 0)
      : Base(allocator) {
    /// Implementation Note
    ///- - - - - - - - - -
  ...
  // type specified in 'PROTOTYPE'.
  {
    ...
    function(allocator_arg, this->get_allocator(),
             BSLS_COMPILERFEATURES_FORWARD(FUNC, rhs))
        .swap(*this);
    return *this;
  }
...
public:
  // PUBLIC TYPES
  template <class FUNCTOR> struct StripSignature {};
...
template <class FP, class PROTOTYPE = typename FunctionDeductionHelper::
                        StripSignature<decltype(&FP::operator())>::Sig>
function(FP) -> function<PROTOTYPE>;
// Deduce the template parameter 'PROTOTYPE' from the signature of the
// 'operator()' of the functor supplied to the constructor of 'function'.

template <class ALLOC, class FP,
          class PROTOTYPE = typename FunctionDeductionHelper::StripSignature<
...
// CREATORS
template <class RET, class... ARGS>
inline bslstl::Function_Variadic<RET(ARGS...)>::Function_Variadic(
...
template <class RET, class... ARGS>
inline RET
bslstl::Function_Variadic<RET(ARGS...)>::operator()(ARGS... args) const {
...
template <class FUNC>
inline void
bsl::function<PROTOTYPE>::installFunc(BSLS_COMPILERFEATURES_FORWARD_REF(FUNC)
                                          func) {
  ...
  this->d_rep.installFunc(BSLS_COMPILERFEATURES_FORWARD(FUNC, func), invoker);
}
...
inline bsl::function<PROTOTYPE>::function(allocator_arg_t,
                                          const allocator_type &allocator)
    BSLS_KEYWORD_NOEXCEPT : Base(allocator) {}
...
template <class PROTOTYPE>
inline bsl::function<PROTOTYPE> &bsl::function<PROTOTYPE>::operator=(
...
template <class PROTOTYPE>
inline void
bsl::function<PROTOTYPE>::swap(function &other) BSLS_KEYWORD_NOEXCEPT {
  this->d_rep.swap(other.d_rep); // Won't throw
}

template <class PROTOTYPE>
template <class TP>
inline TP *bsl::function<PROTOTYPE>::target() BSLS_KEYWORD_NOEXCEPT {
...
#ifdef BSLS_COMPILERFEATURES_SUPPORT_OPERATOR_EXPLICIT
template <class PROTOTYPE>
inline bsl::function<PROTOTYPE>::operator bool() const BSLS_KEYWORD_NOEXCEPT {
...
template <class PROTOTYPE>
inline typename bsl::function<PROTOTYPE>::allocator_type
...
template <class TP>
inline const TP *
bsl::function<PROTOTYPE>::target() const BSLS_KEYWORD_NOEXCEPT {
...
template <class PROTOTYPE>
inline bsl::function<PROTOTYPE>::operator BloombergLP::bdef_Function<
    PROTOTYPE *> &() BSLS_KEYWORD_NOEXCEPT {
  typedef BloombergLP::bdef_Function<PROTOTYPE *> Ret;
  return *static_cast<Ret *>(this);
}

template <class PROTOTYPE>
inline bsl::function<PROTOTYPE>::operator const BloombergLP::bdef_Function<
    PROTOTYPE *> &() const BSLS_KEYWORD_NOEXCEPT {
  typedef const BloombergLP::bdef_Function<PROTOTYPE *> Ret;
  return *static_cast<Ret *>(this);
}

template <class PROTOTYPE>
inline BloombergLP::bslma::Allocator *
bsl::function<PROTOTYPE>::allocator() const BSLS_KEYWORD_NOEXCEPT {
  return get_allocator().mechanism();
}

template <class PROTOTYPE>
inline bool bsl::function<PROTOTYPE>::isInplace() const BSLS_KEYWORD_NOEXCEPT {
...
// FREE FUNCTIONS
template <class PROTOTYPE>
inline bool bsl::operator==(const bsl::function<PROTOTYPE> &f,
                            bsl::nullptr_t) BSLS_KEYWORD_NOEXCEPT {
  return !f;
}

template <class PROTOTYPE>
inline bool bsl::operator==(bsl::nullptr_t, const bsl::function<PROTOTYPE> &f)
    BSLS_KEYWORD_NOEXCEPT {
  return !f;
}
...
template <class PROTOTYPE>
inline void bsl::swap(bsl::function<PROTOTYPE> &a,
