...
template <class CHAR> struct TestData {
  ...
  static const CHAR *emptyStringFunc() {
    static CHAR buf[] = {0};
    return &buf[0];
  }

  static const CHAR *nonEmptyStringFunc() {
  ...
  static const CHAR *stringValue1Func() {
  ...
  static const CHAR *stringValue2Func() {
  ...
  static const CHAR *maxStringFunc() {
    static CHAR buf[2] = {std::numeric_limits<CHAR>::max(), 0};
    return &buf[0];
  }

  static const CHAR *minStringFunc() {
    static CHAR buf[2] = {std::numeric_limits<CHAR>::min(), 0};
    return &buf[0];
  }
};

template <> char const *TestData<char>::s_emptyString_p = "";
template <>
char const *TestData<char>::s_nonEmptyString_p =
    "Tangled Up in Blue - Bob Dylan";
template <> char const *TestData<char>::s_stringValue1_p = "abcde";
template <> char const *TestData<char>::s_stringValue2_p = "abcfg";

template <> wchar_t const *TestData<wchar_t>::s_emptyString_p = L"";
template <>
wchar_t const *TestData<wchar_t>::s_nonEmptyString_p =
    L"Tangled Up in Blue - Bob Dylan";
template <> wchar_t const *TestData<wchar_t>::s_stringValue1_p = L"abcde";
template <> wchar_t const *TestData<wchar_t>::s_stringValue2_p = L"abcfg";
...
bslstl::StringRef::size_type getNumBlanks(const bslstl::StringRef &stringRef)
...
template <class CHAR> void debugprint(const basic_string<CHAR> &value) {
  BSLMF_ASSERT(sizeof(CHAR) <= sizeof(int));
...
template <class CHAR_TYPE> class ImplicitlyConvertibleToString {
...
inline ConvertibleToStringViewType<TYPE, TRAITS>::ConvertibleToStringViewType(
    const TYPE *value)
    : d_value_p(value) {}

// ACCESSORS
template <class TYPE, class TRAITS>
inline ConvertibleToStringViewType<
    TYPE, TRAITS>::operator bsl::basic_string_view<TYPE, TRAITS>() const {
  return bsl::basic_string_view<TYPE, TRAITS>(d_value_p);
}

template <class TYPE, class TRAITS>
inline const TYPE *ConvertibleToStringViewType<TYPE, TRAITS>::data() const {
  return d_value_p;
}

template <class TYPE, class TRAITS>
inline size_t ConvertibleToStringViewType<TYPE, TRAITS>::length() const {
...
template <class CHAR_TYPE> class TestDriver {
  // TYPES
...
public:
  ...
  static void stretch(String *object, size_t size,
  ...
  static void stretchRemoveAll(String *object, size_t size,
                               const CHAR_TYPE &value = CHAR_TYPE());
...
template <class TYPE> void TestDriver<TYPE>::testCase15() {
  // --------------------------------------------------------------------
  ...
  for (int ti = 0; ti < k_NUM_DATA; ++ti) {
    ...
    if (veryVerbose)
      printf("\tbsl::string vs. string_view\n");
    {
    ...
    if (veryVerbose)
      printf("\tstd::string vs. string_view\n");
    {
    ...
    if (veryVerbose)
      printf("\tStringRefImp vs. string_view\n");
    {
    ...
    if (veryVerbose)
      printf("\tStringRefData vs. string_view\n");
    {
...
template <class CHAR_TYPE> void TestDriver<CHAR_TYPE>::testCase14() {
  // ------------------------------------------------------------------------
  ...
  if (verbose)
    printf("TESTING CONSTRUCTION AND STRING VIEWS: %s\n",
...
template <class CHAR_TYPE> void TestDriver<CHAR_TYPE>::testCase10() {
  // ------------------------------------------------------------------------
  // TESTING ITERATORS
  //
  // Concerns:
  ...
  {
    // Empty string
    bslstl::StringRefImp<CHAR_TYPE> mX(TestData<CHAR_TYPE>::s_emptyString_p);
    const bslstl::StringRefImp<CHAR_TYPE> &X = mX;
    ASSERT(X.rbegin() == X.rend());
  }
  ...
  {
    // None-empty string
    bslstl::StringRefImp<CHAR_TYPE> mX(TestData<CHAR_TYPE>::s_nonEmptyString_p);
    const bslstl::StringRefImp<CHAR_TYPE> &X = mX;
...
template <class CHAR_TYPE> void TestDriver<CHAR_TYPE>::testCase9() {
  // --------------------------------------------------------------------
  // TESTING: StringRefImp(const StringRefImp&, size_type, size_type)
  //
  // Concerns:
  ...
  if (verbose)
    printf("\tTest various values for 'startIndex'\n");
  {
    ...
    Obj original(input, LENGTH);
    const Obj &ORIGINAL = original;
    for (std::size_t i = 0; i <= LENGTH; ++i) {
      Obj x(ORIGINAL, i, INT_MAX);
      const Obj &X = x;
  ...
  if (verbose)
    printf("\tTest various values for 'numCharacters'\n");
  {
    const char value[] = "abcdefg";
    CHAR_TYPE input[sizeof(value)];
    copyStringValue(input, value);
  ...
  if (verbose)
    printf("\tTest both 'startIndex' and 'numCharacters'\n");
  {
    const char value[] = "abcdefg";
    CHAR_TYPE input[sizeof(value)];
    copyStringValue(input, value);
    ...
    for (std::size_t startIdx = 0; startIdx <= LENGTH; ++startIdx) {
      for (std::size_t length = 0; length < LENGTH + 1; ++length) {
        ...
        Obj x(ORIGINAL, startIdx, length);
        const Obj &X = x;

        ASSERT(ORIGINAL.begin() + startIdx == X.begin());
        if (startIdx + length < LENGTH) {
          ASSERT(ORIGINAL.end() == X.end() + LENGTH - (length + startIdx));
        } else {
          ASSERT(ORIGINAL.end() == X.end());
        }
    ...
    printf("\tNegative Testing.\n");
  {
    ...
    if (verbose)
      printf("\tNegative testing 'startIndex <= or.length'.\n");
    {
...
template <class CHAR_TYPE>
void TestDriver<CHAR_TYPE>::stretch(String *object, size_t size,
...
template <class CHAR_TYPE>
void TestDriver<CHAR_TYPE>::stretchRemoveAll(String *object, size_t size,
                                             const CHAR_TYPE &value) {
  ASSERT(object);
...
int TestDriver<CHAR_TYPE>::getValues(const CHAR_TYPE **values) {
  ...
  static const CHAR_TYPE initValues[NUM_VALUES] = {
      CHAR_TYPE(VA), CHAR_TYPE(VB), CHAR_TYPE(VC), CHAR_TYPE(VD),
      CHAR_TYPE(VE), CHAR_TYPE(VF), CHAR_TYPE(VG), CHAR_TYPE(VH),
      CHAR_TYPE(VI), CHAR_TYPE(VJ), CHAR_TYPE(VK), CHAR_TYPE(VL)};
...
template <class CHAR_TYPE>
int TestDriver<CHAR_TYPE>::ggg(String *object, const char *spec,
                               int verboseFlag) {
...
template <class CHAR_TYPE>
bsl::basic_string<CHAR_TYPE> &TestDriver<CHAR_TYPE>::gg(String *object,
                                                        const char *spec) {
...
template <class CHAR_TYPE>
bsl::basic_string<CHAR_TYPE> TestDriver<CHAR_TYPE>::g(const char *spec) {
  String object((bslma::Allocator *)0);
  return gg(&object, spec);
}

template <class CHAR_TYPE>
bsl::basic_string<CHAR_TYPE> TestDriver<CHAR_TYPE>::g(size_t length,
                                                      CHAR_TYPE seed) {
...
template <class CHAR> const CHAR *ptrFunction(const CHAR *string) {
  return string;
}
...
// 'std::strings's, 'StringRefImp's, and 'const CHAR *'s.
{
  ...
  if (verbose)
    std::cout << "TESTING ACCESSORS AND COMPARISONS: " << charTypeName
              << "\n"
                 "=================================\n";
  ...
  static struct Data {
    ...
    const int d_len; // -2:  default construct string & string ref
                     ...
                     //          constructor
  } DATA[] = {{L_, TestData<CHAR>::emptyStringFunc(), -1},
...
#define IPF ptrFunction(IPC)
    ...
    {
      const std::size_t ACTUAL_LEN = -2 == ILEN ? 0
                                     : -1 == ILEN
...
#define JPF ptrFunction(JPC)
      ...
      ASSERTV(IL, JL, IS, JS, COMPARE_PC, COMPARE_SR, COMPARE_PC == COMPARE_SR);
...
int main(int argc, char *argv[]) {
  ...
  case 16: {
    ...
    ASSERT(0 == empty.size());
    //..
    // Then, we call 'getNumBlanks' on a string literal and assert that the
    // number of blanks returned is as expected:
    //..
    ...
    ASSERT(42 == numBlanks);
    //..
    // Then, we construct a 'bslstl::StringRef' object, 'line', that refers to
    // only the first line of the 'poem':
    //..
    ...
    //..
    // Next, we use the 'assign' method to make 'line' refer to the second line
    // of the 'poem':
    //..
    ...
    //..
    // Next, we make a 'bslstl::StringRef' object that refers to a string that
    // will be able to hold embedded null characters:
    //..
  ...
  case 15: {
    ...
    if (verbose)
      printf("CONVERSION of 'string_view' W.R.T. 'std::basic_string'\n"
             "======================================================\n");
  ...
  case 14: {
    ...
    if (verbose)
      std::cout << "TESTING CONSTRUCTION AND STRING VIEW\n"
                   "====================================\n";
  ...
  case 13: {
    ...
    const size_t TEST_STR_LENGTH = bsl::char_traits<char>::length(testStr);

    const bsl::basic_string_view<char> SV(testStr, TEST_STR_LENGTH);
    const bslstl::StringRefData<char> SRD(testStr, testStr + TEST_STR_LENGTH);
    const bslstl::StringRefImp<char> SR(testStr, testStr + TEST_STR_LENGTH);
  ...
  case 8: {
    ...
    for (int ti = 0; ti < NUM_DATA; ++ti) {
      ...
      std::size_t hash_value = static_cast<size_t>(bslh_hash_function(o));
      std::size_t bsl_hash_value = static_cast<size_t>(bsl_hash_function(o));
    ...
    for (; i != end; ++i) {
      if (verbose && i->second > 1) {
        std::cout << "Hash value " << std::hex << i->first << std::dec
                  << " occurs " << i->second << " time(s)" << std::endl;
      }
  ...
  case 7: {
    ...
    static const struct {
      ...
      const char *d_result_p;
        ...
        //---- ------- ------- ------
        {L_, "", "", ""},       {L_, " ", " ", "  "},    {L_, "a", "", "a"},
        {L_, "a", " ", "a "},   {L_, "", "a", "a"},      {L_, " ", "a", " a"},
        {L_, "a", "a", "aa"},   {L_, "a", "b", "ab"},    {L_, "b", "a", "ba"},
        {L_, "ab", "", "ab"},   {L_, "ab", " ", "ab "},  {L_, "ab", "c", "abc"},
        {L_, "a", "bc", "abc"}, {L_, "ab", "cd", "abcd"}};
  ...
  case 6: {
                ...
                << "\n=  =  =  =  =  =  =  =  =  =  =  =  =  =  " << std::endl;
    {
      ...
      ASSERT(X1.end() == NON_EMPTY_STRING + std::strlen(NON_EMPTY_STRING));
      ...
      ASSERT(X2.end() == NON_EMPTY_STRING + std::strlen(NON_EMPTY_STRING));

      x2.assign(NON_EMPTY_STRING + 2, NON_EMPTY_STRING + 6);
...
#define TEST_TYPE(INT_TYPE)                                                    \
  {                                                                            \
    ...
    ASSERT(Y.end() == NON_EMPTY_STRING + std::strlen(NON_EMPTY_STRING));       \
  }
...
#undef TEST_INT_TYPE
                ...
                << "\n=  =  =  =  =  =  =  =  =" << std::endl;
    {
      ...
      ASSERT(X3.end() == NON_EMPTY_STRING + std::strlen(NON_EMPTY_STRING));
      ...
      ASSERT(X4.end() == NON_EMPTY_STRING + std::strlen(NON_EMPTY_STRING));
  ...
  case 5: {
    ...
    static const struct {
      ...
      bool d_gteq;
      bool d_eq;
    } DATA[] = {// line string1 string2   lt     gt    lteq   gteq   eq
                //---- ------- ------- ------ ------ ------ ------  --
  ...
  case 4: {
                ...
                << "\n= = = = = = = = = = = = = = = =" << std::endl;
    {
      ...
      if (veryVerbose)
        std::cout << "\tEXPECTED FORMAT: " << static_cast<bsl::string>(ES)
                  << std::endl;
      std::ostringstream out;
      out << ES;
      if (veryVerbose)
        std::cout << "\tACTUAL FORMAT:   " << out.str() << std::endl;
      ...
      if (veryVerbose)
        std::cout << "\tEXPECTED FORMAT: " << static_cast<bsl::string>(ES1)
                  << std::endl;
      std::ostringstream out1;
      out1 << ES1;
      if (veryVerbose)
        std::cout << "\tACTUAL FORMAT:   " << out1.str() << std::endl;
      ...
      if (veryVerbose)
        std::cout << "\tEXPECTED FORMAT: " << NON_EMPTY_STRING << std::endl;
      std::ostringstream nesOut;
      nesOut << NES;
      if (veryVerbose)
        std::cout << "\tACTUAL FORMAT:   " << nesOut.str() << std::endl;
      ...
      if (veryVerbose)
        std::cout << "\tEXPECTED FORMAT: " << static_cast<bsl::string>(NESS)
                  << std::endl;
      std::ostringstream nessOut;
      nessOut << NESS;
      if (veryVerbose)
        std::cout << "\tACTUAL FORMAT:   " << nessOut.str() << std::endl;
      ...
      fmtOut.str(std::string());
      fmtOut << std::left << std::setfill('-') << std::setw(10) << ES;
      ASSERT(fmtOut.str() == bsl::string(10, '-'));

      fmtOut.str(std::string());
      fmtOut << std::right << std::setfill('*') << std::setw(10) << ES;
      ASSERT(fmtOut.str() == bsl::string(10, '*'));

      fmtOut.str(std::string());
      fmtOut << std::left << std::setfill(' ')
             << std::setw(static_cast<int>(NES.length() + 10)) << NES;
      ASSERT(fmtOut.str() == NES + bsl::string(10, ' '));

      fmtOut.str(std::string());
      fmtOut << std::right << std::setfill('?')
             << std::setw(static_cast<int>(NES.length() + 10)) << NES;
      ASSERT(fmtOut.str() == bsl::string(10, '?') + NES);

      fmtOut.str(std::string());
      fmtOut << std::left << std::setfill('?')
             << std::setw(static_cast<int>(NES.length() + 10)) << NES << NES;
  ...
  case 2: {
...

    {
      ...
      ASSERTV(X2, X2.end() == NON_EMPTY_STRING + std::strlen(NON_EMPTY_STRING));
...
#define TEST_TYPE(INT_TYPE)                                                    \
  {                                                                            \
    ...
    ASSERTV(X, X.end() == NON_EMPTY_STRING + std::strlen(NON_EMPTY_STRING));   \
  }
...
#undef TEST_INT_TYPE
             ...
             "=\n";
    {
      ...
      for (size_t start = 0; start < std::strlen(NON_EMPTY_STRING); ++start) {
        {
          if (veryVeryVerbose) {
        ...
        for (size_t length = 1; length <= std::strlen(NON_EMPTY_STRING) - start;
             ++length) {
          if (veryVeryVerbose) {
          ...
          LOOP2_ASSERT(start, length, X3.begin() == NON_EMPTY_STRING + start);
          LOOP2_ASSERT(start, length,
                       X3.end() == NON_EMPTY_STRING + start + length);
        }
        for (size_t length = std::strlen(NON_EMPTY_STRING) - start + 1;
             length <= 2 * std::strlen(NON_EMPTY_STRING); ++length) {
          if (veryVeryVerbose) {
            std::cout << "SR(SR, " << start << ", " << length << ")\n";
          }
          Obj x3(x2, start, length);
      ...
      for (size_t length = 0; length <= 2 * std::strlen(NON_EMPTY_STRING);
           ++length) {
        if (veryVeryVerbose) {
        ...
        ASSERT(X3.length() == 0);
        ASSERT(X3.begin() == X3.end());
        ASSERT(X3.begin() == NON_EMPTY_STRING + std::strlen(NON_EMPTY_STRING));
        ASSERT(X3.end() == NON_EMPTY_STRING + std::strlen(NON_EMPTY_STRING));
      }
...
#define TEST_TYPE(INT_TYPE_START, INT_TYPE_LENGTH)                             \
  {                                                                            \
    ...
    Obj x(NON_EMPTY_STRING,                                                    \
          static_cast<INT_TYPE_START>(std::strlen(NON_EMPTY_STRING) / 3),      \
          static_cast<INT_TYPE_LENGTH>(std::strlen(NON_EMPTY_STRING) / 3));    \
    const Obj &X = x;                                                          \
    ASSERT(!X.isEmpty());                                                      \
    ASSERT(X.length() == std::strlen(NON_EMPTY_STRING) / 3);                   \
    ASSERT(X.begin() != X.end());                                              \
    ASSERT(X.begin() == NON_EMPTY_STRING + std::strlen(NON_EMPTY_STRING) / 3); \
    ASSERT(X.end() ==                                                          \
           NON_EMPTY_STRING + 2 * std::strlen(NON_EMPTY_STRING) / 3);          \
  }

#define TEST_INT_TYPE(INT_TYPE_START, LITERAL_ZERO_START, INT_TYPE_LENGTH,     \
                      LITERAL_ZERO_LENGTH)                                     \
...
#undef L0
...

    {
      ...
      ASSERT(s_emptyString_p.data() + s_emptyString_p.length() == X1.end());

      // Non-empty string
      bsl::string s_nonEmptyString_p(NON_EMPTY_STRING);
      Obj x2(s_nonEmptyString_p);
      const Obj &X2 = x2;
      ASSERT(!X2.isEmpty());
      ASSERT(X2.length() == std::strlen(NON_EMPTY_STRING));
      ASSERT(X2.begin() != X2.end());
      ...
      ASSERT((&*X2.begin() + (X2.end() - X2.begin())) ==
             (&*s_nonEmptyString_p.begin() +
              (s_nonEmptyString_p.end() - s_nonEmptyString_p.begin())));
    }
                ...
                << "\n  =  =  =  =  =  =  =  =  =  =  =  =  =  =" << std::endl;
    {
      ...
      ASSERT(XC2.end() == NON_EMPTY_STRING + std::strlen(NON_EMPTY_STRING));
    }
  ...
  case -1: {
    ...
    for (int jj = k_NUM_TRIALS; 0 < jj; --jj) {
      ...
      const bool EQ = eqv[idxB] == eqv[idxA];
      (void)EQ;
    }
    sw.stop();
    struct Times {
    ...
    Times empty;
    sw.accumulatedTimes(&empty.d_systemTime, &empty.d_userTime,
                        &empty.d_wallTime);
    P_(empty.d_systemTime);
    P_(empty.d_userTime);
    P(empty.d_wallTime);
    ...
    Times eqTime;
    sw.accumulatedTimes(&eqTime.d_systemTime, &eqTime.d_userTime,
                        &eqTime.d_wallTime);
    eqTime -= empty;
    P_(eqTime.d_systemTime);
    P_(eqTime.d_userTime);
    P(eqTime.d_wallTime);
    ...
    Times eqCTime;
    sw.accumulatedTimes(&eqCTime.d_systemTime, &eqCTime.d_userTime,
                        &eqCTime.d_wallTime);
    eqCTime -= empty;
    P_(eqCTime.d_systemTime);
    P_(eqCTime.d_userTime);
    P(eqCTime.d_wallTime);
    ...
    Times ltTime;
    sw.accumulatedTimes(&ltTime.d_systemTime, &ltTime.d_userTime,
                        &ltTime.d_wallTime);
    ltTime -= empty;
    P_(ltTime.d_systemTime);
    P_(ltTime.d_userTime);
    P(ltTime.d_wallTime);
    ...
    Times ltCTime;
    sw.accumulatedTimes(&ltCTime.d_systemTime, &ltCTime.d_userTime,
                        &ltCTime.d_wallTime);
    ltCTime -= empty;
    P_(ltCTime.d_systemTime);
    P_(ltCTime.d_userTime);
    P(ltCTime.d_wallTime);
  } break;
  default: {
